***************************************************************************
*
* PROG: HACKING LANCE		version: 0.00I		date:	10/05/2013
*
*==========================================================================
* FUNCTION:
*	Allows Lance replay at 12.5 KHz / 25 KHz / 50 KHz for a MOD on STE.
* FILES:
*	- HACKL??I.PRG	this program
...
*	08/05/2013	17:00	C00??	0.00I	Add BPM support
*	10/05/2013	23:00	C00??	0.00I	Sample Offset skip bug
***************************************************************************
***************************************************************************
* PROGRAMSTART
***************************************************************************
...
zstrbpm		dc.b	'BPM ACTIVE',0
zstrnobpm	dc.b	'BPM NOT ACTIVE',0
...
l_inicod12
	...
	move	lc_bpm_code(pc),d0
	lea	ll_mt_mix_bpm_03(pc),a0
	move	d0,(a0)
	lea	ll_mt_mix_bpm_12(pc),a0
	move	d0,(a0)
	bra	l_inicodok_e
l_inicod25
	cmp	#$0002,d0		25 KHz ?
...
lc_bpm_code
	lsl	#2,d0			x 80(50KHz) / x 40(25 KHz) / x 20(12.5 KHz)
	lsl	#3,d0			x 80(50KHz) / x 40(25 KHz) / x 20(12.5 KHz)
	lsl	#4,d0			x 80(50KHz) / x 40(25 KHz) / x 20(12.5 KHz)
...
l_main_trash_lp1
	...
	move	z_old_color(pc),$FFFF8240.w
...
q_handle_bpm:
	move	z_bpm_active(pc),d0
	beq	l_handle_bpm_9
	lea	mt_BPM_struct(pc),a0
	move	mt_vbl_frames(a0),mt_frames_left(a0)
	clr	mt_frame_idx(a0)
l_handle_bpm_0
*	move.l	mt_bpm_counter(a0),d1
	move.l	(a0),d1
	add.l	#$7FFF,d1		frames_to_do = bmp_counter + 0.49999f
	swap	d1
	move	mt_frames_left(a0),d0
	cmp	d0,d1			frames_to_do <= frames_left ?
	bgt.s	l_handle_bpm_2		yes / no
	tst	d1			frames_to_do = 0 ?
	beq.s	l_handle_bpm_1		no / yes
	move	d1,mt_frames_to_do(a0)
*	sub	d1,mt_bpm_counter(a0)	bpm_counter -= frames_to_do
	sub	d1,(a0)
	move.l	a0,-(sp)
	bsr	mt_mixer
	move.l	(sp)+,a0
	move	mt_frames_to_do(a0),d0
	sub	d0,mt_frames_left(a0)	frames_left -= frames_to_do
	addq	#1,mt_frame_idx(a0)	next frames block
l_handle_bpm_1
	move.l	a0,-(sp)
	bsr	mt_music		get new PT data
	move.l	(sp)+,a0
*	move.l	#125,d0			new_bpm_counter_value =
*	mulu	mt_vbl_frames(a0),d0	125 x vbl_frames
	move.l	#3125,d0
	divu	mt_read_bpm(a0),d0	/ read_bpm
	move	d0,d1
	swap	d1
	clr	d0
	divu	mt_read_bpm(a0),d0
	move	d0,d1
*	add.l	d1,mt_bpm_counter(a0)	bpm_counter += new_bpm_counter_value
	add.l	d1,(a0)
	bra.s	l_handle_bpm_0
l_handle_bpm_2
	tst	d0			frames_left = 0 ?
	beq.s	l_handle_bpm_3		no / yes
	move	d0,mt_frames_to_do(a0)	frames_to_do = frames_left
*	sub	d0,mt_bpm_counter(a0)	bpm_counter -= frames_to_do
	sub	d0,(a0)
	move.l	a0,-(sp)
	bsr	mt_mixer
	move.l	(sp)+,a0
	addq	#1,mt_frame_idx(a0)	next frames block
l_handle_bpm_3
	cmp	#3,mt_frame_idx(a0)
	subq	#1,mt_frame_idx(a0)
	beq.s	l_handle_bpm_8
	moveq	#$FFFFFF80,d2
	moveq	#$1F,d3
	...
	add	d0,a1
	addq	#4,a1
	move	mt_frame_idx(a0),d0
	add	d0,d0
	add	d0,d0
	move	0(a1,d0),d1		left LCM
	and	2(a1,d0),d1		AND right LCM
	bpl.s	l_handle_bpm_4		if negative => no update
	and.b	d2,-4(a1,d0)		take out previous link
	subq	#1,mt_frame_idx(a0)
	beq.s	l_handle_bpm_8
	move	mt_frame_idx(a0),d0
	add	d0,d0
	...
	move	0(a1,d0),d1		left LCM
	and	2(a1,d0),d1		AND right LCM
	bpl.s	l_handle_bpm_8		if negative => no update else 1 link
	and.b	d2,(a1)			take out previous link
	bra.s	l_handle_bpm_8
l_handle_bpm_4
	subq	#1,mt_frame_idx(a0)
	beq.s	l_handle_bpm_8
*					we have 3 stages
	move	4(a1),d1		left LCM middle
	and	6(a1),d1		right LCM middle
	bpl.s	l_handle_bpm_5
	move.b	4(a1),d0
	and.b	d3,d0
	and.b	d2,(a1)
	or.b	d0,(a1)
	...
	bra.s	l_handle_bpm_8
l_handle_bpm_5
*					we have two updates with timers
	move.b	(a1),d1
	and.b	d3,d1			values of frames_left at 2nd stage
	move.b	4(a1),d0
	and.b	d3,d0			values of frames_left at 2nd stage
	sub.b	d1,d0			sub frames from 1st part
	and.b	d2,4(a1)
	or.b	d0,4(a1)
l_handle_bpm_8
	rts
...
instmfp:
	...
	clr.b	$FFFFFA09.w
	and.b	#$0F,$FFFFFA1D.w			stop Timer C
	bset	#5,$FFFFFA09.w				allow Timer C
	bset	#5,$FFFFFA15.w				allow Timer C
	bclr	#3,$FFFFFA17.w
	lea	vbl_trash(pc),a0
	move.l	a0,$70.w
...
l_mt_init16
	...
	move	z_autovoldetect(pc),d0
	or	z_autobpmdetect(pc),d0
	beq	lm_chkpatused6
	lea	$43C(a0),a3		points to patterns
	moveq	#0,d4			result = no BPM detected
...
lm_chkpatused2
	...
	tst	d4			BPM already detected ?
	beq.s	lm_chkpatused3a		yes / no
	bra.s	lm_chkpatused5

lm_chkpatused3
	cmp	#$F,d1
	bne.s	lm_chkpatused3a
	move.b	-1(a5),d1
	cmp.b	#32,d1
	bcs.s	lm_chkpatused3a
	cmp.b	#125,d1
	beq.s	lm_chkpatused3a
	moveq	#1,d4			BPM dectected
	tst	d5			volume control already detected ?
	bne.s	lm_chkpatused5		no / yes
lm_chkpatused3a
...
lm_chkpatused5
	move	z_autovoldetect(pc),d0
	beq.s	lm_chkpatused5a
	lea	z_volume_active(pc),a3
	move	d5,(a3)
lm_chkpatused5a
	move	z_autobpmdetect(pc),d0
	beq.s	lm_chkpatused5b
	lea	z_bpm_active(pc),a3
	move	d4,(a3)
lm_chkpatused5b
lm_chkpatused6
	movem.l	a0-a2,-(sp)
...
*	clr	mt_PatternPos
	...
	lea	mt_BPM_struct(pc),a0
*	clr.l	mt_bpm_counter(a0)
	clr.l	(a0)
	move	#125,mt_read_bpm(a0)
	move	#25,mt_vbl_frames(a0)
...
l_mt_init_trash0
	...
	move.l	#$80008000,(a1)+	LCM volume updates
	move.l	#$80008000,(a1)+	LCM volume updates
	move.l	#$80008000,(a1)+	LCM volume updates
...
mt_make_tables2
	...
	tst.l	d0
	smi	d1
	bmi	mt_make_tabl_rts
	lea	mt_ffreq_table(pc),a1
	move.l	a0,(a1)
	move.l	mt_freq_table(pc),a1
	move	#$3FF,d7
lm_mk_tab2_0
	moveq	#0,d0
	move	(a1)+,d0
	add	#78,d0
	divu	mt_BPM_struct+mt_vbl_frames(pc),d0
	move	d0,d1
	swap	d1
	move	(a1)+,d0
	divu	mt_BPM_struct+mt_vbl_frames(pc),d0
...
mt_bpmtimerdata	dc.b	10,20,29,39,49,59,69,79,88,98,108,118
		dc.b	128,137,147,157,167,177,187,196,206,216,226,236
...
mt_Paula:
	...
	move.b	(a0),d0
	beq.s	l_mt_Paula_notim	= 0 => end list
	bpl.s	l_mt_Paula_timer	> 0 => new timer
	addq.b	#1,d0
	beq.s	l_mt_Paula_notim	-1 => end list
	subq.b	#1,d0
	and	#$001F,d0
	beq.s	l_mt_Paula_notim
l_mt_Paula_timer
	subq.b	#1,d0
	and	#$001F,d0
	pea	mt_Paula_2(pc)
	move.l	(sp)+,$114.w
	and.b	#$0F,$FFFFFA1D.w			stop Timer C
	move.b	mt_bpmtimerdata(pc,d0),$FFFFFA23.w
	or.b	#$70,$FFFFFA1D.w			divided by 200

l_mt_Paula_notim
	move	(a0)+,d0		left volume
	bmi.s	l_mt_set_pointer	if < 0 => no change
	and	#$007F,d0		take out link
...
mt_bpmtim_data2	dc.b	10,20,29,39,49,59,69,79,88,98,108,118
		dc.b	128,137,147,157,167,177,187,196,206,216,226,236
...
mt_Paula_2:
	...
	move.b	(a0),d0
	beq.s	l_mt_Paula2notim	= 0 => end list
	bpl.s	l_mt_Paula2timer	> 0 => new timer
	addq.b	#1,d0
	beq.s	l_mt_Paula2notim	-1 => end list
	subq.b	#1,d0
	and	#$001F,d0
	beq.s	l_mt_Paula2notim
l_mt_Paula2timer
	subq.b	#1,d0
	and	#$001F,d0
	pea	mt_Paula_3(pc)
	move.l	(sp)+,$114.w
	move.b	mt_bpmtim_data2(pc,d0),$FFFFFA23.w
	or.b	#$70,$FFFFFA1D.w			divided by 200

l_mt_Paula2notim
	move	(a0)+,d0		left volume
	bmi.s	l_mt_set2_right		if < 0 => no change
	and	#$007F,d0		take out link
	move	mt_LCM_mask(pc),$24(a1)
	add	d0,d0
	move	lmt_LCM_vol_tab2(pc,d0),d0
	or	mt_LCM_left(pc),d0
	move	d0,$22(a1)
l_mt_set2_right
	move	(a0)+,d0		right volume
	bmi.s	l_mt_no2_right		if < 0 => no change
	move	mt_LCM_mask(pc),d1
	add	d0,d0
	move	lmt_LCM_vol_tab2(pc,d0),d0
	or	mt_LCM_right(pc),d0
...
**	move.l	mt_logic_buf(pc),sp
	...
	move	z_bpm_active(pc),d0
	beq.s	l_mt_mix_bpm_030
*	move	mt_BPM_struct+mt_vbl_frames(pc),d0
	moveq	#25,d0
	sub	mt_BPM_struct+mt_frames_left(pc),d0
	move	d0,d1			x 1
	add	d0,d0			x 2
	add	d0,d0			x 4
	add	d1,d0			x 5
ll_mt_mix_bpm_03
	lsl	#4,d0			x 80(50KHz) / x 40(25 KHz) / x 20(12.5 KHz)
	add	d0,sp			correct buffer pointer
	move	mt_BPM_struct+mt_frame_idx(pc),d0
	add	d0,d0
	add	d0,d0
	add	d0,a5
	tst	d0
...
*	move.l	mt_sample_point(a4),a1		Sample 1 pointer
	...
	move	z_bpm_active(pc),d3
	beq	l_mt_mix_bpm_031

	move.l	d1,mt_voltabptr(a3)		save volume tab ptr

	move	mt_period(a3),d3		Period 0

	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_038
	cmp	#216,d3
	bcc	l_mt_proceed_038
...
*  VVVVV
	...
	move.l	0(a2,d3),d4			long VBL step
	move.l	mt_ffreq_table(pc),a2		Frame step table
	add	d3,a2

	move.l	d4,d3				coma part
	swap	d3				integer part
	add	mt_add_iw(a3),d4		add previous coma rest
	negx	d3				addx to d3
	neg	d3				= bytes to read this time

	move	(a2)+,d0			Frame step int part
	move	(a2)+,d1			Frame step coma part
	move	mt_BPM_struct+mt_frames_to_do(pc),d2
	mulu	d2,d0				result int part
	mulu	d2,d1				result coma part
	swap	d0
	add.l	d1,d0				final real result
	move	d0,d1
	swap	d0
	add	mt_add_iw(a3),d1		add previous coma rest
	negx	d0				addx to d0
	neg	d0				= bytes to read this time
	move	d1,mt_add_iw(a3)		store coma part left
	lea	78(a0,d3),a2			new final pointer
	move.l	a2,d6
	lea	0(a0,d0),a2			new final pointer
...
l_mt_proceed_039
	...
	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_03A
	cmp	#216,d4
	bcc	l_mt_proceed_03A
...
*  VVVVV
	...
	move.l	0(a2,d4),d5			long VBL step
	move.l	mt_ffreq_table(pc),a2		Frame step table
	add	d4,a2

	move.l	d5,d4				coma part
	...
	move	d0,d1
	swap	d0
	add	mt_add_iw(a4),d1		add previous coma rest
	negx	d0				addx to d0
	neg	d0				= bytes to read this time
	move	d1,mt_add_iw(a4)		store coma part left
	lea	78(a1,d4),a2			new final pointer
	sub.l	a2,d7				sample start ptr - new end	
	lea	0(a1,d0),a2			new final pointer
...
l_mt_proceed_03B
	...
	move.l	mt_voltabptr(a3),d1		restore volume tab ptr

	bra	l_mt_mix_bpm_032
...
l_mt_mix_bpm_032
	...
	move	z_bpm_active(pc),d3
	beq.s	l_mt_mix_bpm_03A
	lea	l_mt_return03-$10(pc),a6
*	move	mt_BPM_struct+mt_vbl_frames(pc),d3
	moveq	#25,d3
	sub	mt_BPM_struct+mt_frames_to_do(pc),d3
	lsl	#4,d3				x 16 = $10
	add	d3,a6
	jmp	(a6)
l_mt_mix_bpm_03A
...
*	move.l	mt_sample_point(a4),a1		Sample 1 pointer
	...
	move	z_bpm_active(pc),d3
	beq	l_mt_mix_bpm_033

	move	mt_period(a3),d3		Period 0

	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_03C
	cmp	#216,d3
	bcc	l_mt_proceed_03C
...
l_mt_mix_bpm_034
	...
	move	z_bpm_active(pc),d3
	beq.s	l_mt_mix_bpm_03B
	lea	l_mt_return03a-$0E(pc),a6
*	move	mt_BPM_struct+mt_vbl_frames(pc),d3
	moveq	#25,d3
	sub	mt_BPM_struct+mt_frames_to_do(pc),d3
	add	d3,d3				x 2
...
****************************************************************
	...
	lea	l_mt_return03b-4(pc),a6			frame 00
	addq	#4,a6					frame 01
	jmp	(a5)
l_mt_return03b
*	rept	24
...
l_mt_common_03a
	...
	move	z_bpm_active(pc),d7
	beq.s	l_mt_proceed_03E
	move.l	mt_smp_endpoint(a3),a0
	move.l	mt_smp_endpoint(a4),a1
	bra.s	l_mt_proceed_03F
l_mt_proceed_03E
	move	z_octave3_cheat(pc),d7
...
*	move.l	mt_sample_point(a4),a1		Sample 1 pointer
	...
	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_128
	cmp	#216,d3
	bcc	l_mt_proceed_128
...
l_mt_proceed_129
	...
	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_12A
	cmp	#216,d4
	bcc	l_mt_proceed_12A
...
l_mt_mix_bpm_122
	...
	move	z_bpm_active(pc),d3
	beq.s	l_mt_mix_bpm_12A
	lea	l_mt_return12-$10(pc),a6
*	move	mt_BPM_struct+mt_vbl_frames(pc),d3
	moveq	#25,d3
	sub	mt_BPM_struct+mt_frames_to_do(pc),d3
	lsl	#4,d3				x 16 = $10
...
*	move.l	mt_sample_point(a4),a1		Sample 1 pointer
	...
	move	z_bpm_active(pc),d3
	beq	l_mt_mix_bpm_123

	move	mt_period(a3),d3		Period 0

	move	z_octave3_cheat(pc),d7
	beq	l_mt_proceed_12C
	cmp	#216,d3
	bcc	l_mt_proceed_12C
...
l_mt_mix_bpm_124
	...
	move	z_bpm_active(pc),d3
	beq.s	l_mt_mix_bpm_12B
	lea	l_mt_return12a-$0E(pc),a6
*	move	mt_BPM_struct+mt_vbl_frames(pc),d3
	moveq	#25,d3
	sub	mt_BPM_struct+mt_frames_to_do(pc),d3
	add	d3,d3				x 2
...
****************************************************************
	...
	lea	l_mt_return12b-4(pc),a6			frame 00
	addq	#4,a6
	jmp	(a5)
l_mt_return12b
*	rept	24
...
l_mt_common_12a
	...
	move	z_bpm_active(pc),d7
	beq.s	l_mt_proceed_12E
	move.l	mt_smp_endpoint(a3),a0
	move.l	mt_smp_endpoint(a4),a1
	bra.s	l_mt_proceed_12F
l_mt_proceed_12E
	move	z_octave3_cheat(pc),d7
	or	z_bpm_active(pc),d7
	beq.s	l_mt_proceed_126
	move	mt_period(a3),d0		Period 0
	cmp	#216,d0
	bcc.s	l_mt_proceed_126
	move.l	mt_smp_endpoint(a3),a0
l_mt_proceed_126
	tst	d7
...
mt_BPM_struct	ds.l	4
	rsreset
mt_bpm_counter	rs.l	1
mt_read_bpm	rs.w	1		direct value from MOD
mt_vbl_frames	rs.w	1		25 for Lance
mt_frames_left	rs.w	1		25 or less
mt_frames_to_do	rs.w	1		25 or less
mt_frame_idx	rs.w	1		values: 0 / 1 / 2

**mt_amiga_freq	dc.l	7090000
mt_freq_list	ds.l	1
mt_ffreq_table	ds.l	1
...
mt_trash_str	ds.l	128
*				1 pointer		4 bytes
*				3 LCM update groups	12 bytes
*				x 32 trash groups
*				LCM update group
*				3 times left part / right part
*				left part U00FFFFF 0VVVVVVV
*				0VVVVVVV	Volume to set
*				00FFFFF		Next frame (0 end)
*				U (0: update 1: no update)
*				right part Uxxxxxxx 0VVVVVVV
*				0VVVVVVV	Volume to set
*				U (0: update 1: no update)
...
mt_Set_BPM
	lea	mt_BPM_struct(pc),a1
	move	d0,mt_read_bpm(a1)
	rts
