 ...
 *	numbers 0,1,2,3 and the 10 s p trk will be assumed
...
struct buf *buf;
{
	...
	if(((dev >= 4) && (blk > RP_B_MAX)) || ((dev < 4) && (blk > CD_B_MAX))){
...
short trackn;{
	...
	case 4: case 5: case 6: case 7:
		/* non-cd type (10 p surface) */

		p_trk = RP_PERTRK;
		break;
	}

	trk = (trackn == NOTATRACK)? (blk / p_trk) : trackn;
...
short nsects;{
...
#ifdef EXBUF
	register short count = 0, sect, err;
	char addr[20];

	while(count < 10){
		if(err = read_addr(addr))
			count++;
		else{
			sect = addr[2] + 1;		/* sector number */
			sect = (sect > nsects)? sect - nsects:sect;
			err = read_sect(buffer + (SIZE * (sect - 1)),sect,nsects - sect + 1,0);
			if(sect != 1)
				err |= read_sect(buffer,1,sect - 1,0);
			if(err)
				count++;
			else
				break;
	...
	while(count < 10){
		if(err = read_sect(buffer,1,nsects,0))
			count++;
		else
			break;
	}
...
short nsects;{
	...
	while(count < 10){
		if(err = read_addr(addr))
			...
			sect = addr[2] + 1;		/* sector number */
			sect = (sect > nsects)? sect - nsects:sect;
			err = write_sect(buffer + (SIZE * (sect - 1)),sect,nsects - sect + 1,0);
			if(sect != 1)
				err |= write_sect(buffer,1,sect - 1,0);
			if(err)
				count++;
			else
				break;
	...
	while(count < 10){
		if(err = write_sect(buffer,1,nsects,0))
			count++;
		else
			break;
	}
...
struct fdcntrl *args;{
	...
	tmpb.b_bno = 0;	/* allows us to use deverr for error reports */
	tmpb.b_dev = dev;	/*	"	"	"	" */
	...
	case READADDR: case READTRACK: case WRITETRACK:
		...
		if ((set_track(fds->d_curtrk)) != args->trackno){
			...
			raw_seek(args->trackno);
		}
			
		fds->d_curtrk = args->trackno;
	...
	case MULTIREAD: case MULTIWRITE:
		 ...
		 */
		reps = diskset(&tmpb,(args->trackno * 2) | args->sideno);
		if(reps == ERROR)
			return ERROR;
