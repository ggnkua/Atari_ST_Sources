...
/*
 * Eopen - makes an entry in User File Table, returns file descriptor.
 *		Uses single buffer for all file operations, used for
 *		execl().
 */
eopen(name, mode)
int mode;
char *name;
{
	return gen_open(name, (mode+1)&3, EOPEN, 0, f_open);
}
