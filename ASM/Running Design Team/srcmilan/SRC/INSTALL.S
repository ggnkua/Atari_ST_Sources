...
sdg_go_on
		move.w	d0,pd_dsp_exponent(a6)
		move.l	d1,pd_inter_entf1(a6)
		add.l	d1,d1
		move.l	d1,pd_inter_entf2(a6)
...
sc_make_entry
	...
	; -----------------------------
	; schaue in einem ersten schritt nach, ob
	; die farbtabelle schon einmal existiert
	...
	; (kompromiss zum underground -> dunkel)
		...
		subq.w	#1,d1
		bmi.s	scme_you_can
scme_loop	cmp.w	(a0,d1.w*2),d0
		beq.s	scme_out
		dbra	d1,scme_loop

	; -----------------------------
	; farbtabelle einfach ans ende anhaengen
...
; da die farbtabellen nicht doppelt vorkommen
; duerften, muesste die angabe eindeutig sein ...

sc_delete_entry

		movem.l	d2/a0-a1,-(sp)
		lea	sc_stack,a0		; zeiger stack
		lea	sc_stack_pos,a1		; anzahl
		move.w	(a1),d1			; anzahl holen
		move.w	d1,d2

scde_loop
		subq.w	#1,d1
		bmi.s	scde_not_found		; noch eintraege vorhanden?
		cmp.w	(a0,d1.w*2),d0		; vergleichen
		bne.s	scde_loop		; bis gefunden
...
;		lea	sm_scde,a0		; fehler message
;		jsr	install_message		; ausgeben
