...
MONSTERWALKTIME1000	equ	200
MONSTERHITTIME1000	equ	500
MONSTERSHOOTTIME1000	equ	200
MONSTERDIETIME1000	equ	200
...
; ---------------------------------------------------------
; 07.07.00/vk
; grafik-/texturaniminierung der gegner.
; a2 = gegner (muss unveraendert bleiben)
animateSingleMonsterTexture

		movea.l	levelPtr,a6
		movea.l	LEVMONSTERS(a6),a0
		adda.l	a6,a0
		movea.l	MONSDAT(a0),a0
		adda.l	a6,a0
...
; a6 = level
asmtWalk
		move.w	MONGRAPHICTYPE(a2),d1
		movea.l	(a0,d1.w*4),a3
		adda.l	a6,a3				; a3 = entsprechende sdat fuer grafikdaten (mongrafiktype)

		move.w	MONAWALK(a2),d4			; zeitcounter fuer laufanimation holen
		add.w	vblTime1000,d4			; vergangene zeit in msek. addieren
		cmpi.w	#MONSTERWALKTIME1000,d4		; zeitgrenze erreicht?
		blt.s	asmtwWalkSame			; nein -> dann bleibt es bei aktueller animationsphase

		moveq	#0,d4				; zeitcounter resetten

		move.w	MONAWALKPOS(a2),d0
		addq.w	#1,d0
		cmp.w	SDATWALKANZ(a3),d0
		blt.s	asmtwAnimPosOk
		moveq	#0,d0
asmtwAnimPosOk	move.w	d0,MONAWALKPOS(a2)

asmtwWalkSame
		move.w	d4,MONAWALK(a2)

		move.w	MONAWALKPOS(a2),d0
		add.w	SDATWALKPTR(a3),d0
		bsr	asmtSetAnimation
...
asmtShoot
		...
		move.w	MONASHOOT(a2),d4		; zeitcounter fuer schussanimation holen
		add.w	vblTime1000,d4			; vergangene zeit in msek. addieren
		cmpi.w	#MONSTERSHOOTTIME1000,d4	; zeitgrenze erreicht?
		blt.s	asmtsShootSame			; nein -> dann bleibt es bei aktueller animationsphase

		moveq	#1,d4				; zeitcounter resetten

		move.w	MONASHOOTPOS(a2),d0
		addq.w	#1,d0
		cmp.w	SDATSHOOTANZ(a3),d0
		blt.s	asmtsAnimPosOk
		moveq	#0,d0
		moveq	#0,d4
asmtsAnimPosOk	move.w	d0,MONASHOOTPOS(a2)

		cmpi.w	#3,d0				; in dieser animationsphase wollen wir dem spieler nun energie abziehen
		bne.s	asmtsShootSame

		nop
; todo		bsr	monsterShootsToPlayer		; a2 = monster

asmtsShootSame
		move.w	d4,MONASHOOT(a2)

		move.w	MONASHOOTPOS(a2),d0
		add.w	SDATSHOOTPTR(a3),d0
		bsr	asmtSetAnimation
asmtsOut
		rts
...
asmtHit
		...
		move.w	MONAHIT(a2),d4
		add.w	vblTime1000,d4			; vergangene zeit in msek. addieren
		cmpi.w	#MONSTERHITTIME1000,d4		; zeitgrenze erreicht?
		blt.s	asmthHitSame			; nein -> dann bleibt es bei aktueller animationsphase

		moveq	#0,d4				; flag fuer getroffen-animation loeschen (ende erreicht)
		bset	#4,MONSHOOTTYPE+1(a2)		; flag fuer bereits einmal getroffen setzen

		tst.w	MONSHOOTTYPE(a2)		; soll monster gleich zurueckschiessen?
		bmi.s	asmthHitSame			; negatives byte (gegner schiesst generell nicht) -> nein
		btst	#0,MONSHOOTTYPE+1(a2)		; entsprechendes flag/bit pruefen
		beq.s	asmthHitSame			; nicht gesetzt -> dann auch nicht schiessen und normal weiter

		move.w	#1,MONASHOOT(a2)		; gegner schiesst bei naechstem animationsdurchlauf
asmthHitSame

		move.w	d4,MONAHIT(a2)

		move.w	SDATHITPTR(a3),d0
		bsr	asmtSetAnimation
...
asmtDied
		...
		move.w	MONADIED(a2),d4			; zeitcounter fuer sterbeanimation holen
		bmi.s	asmtdDiedSame			; negativ -> gegner ist bereits komplett gestorben
		add.w	vblTime1000,d4			; vergangene zeit in msek. addieren
		cmpi.w	#MONSTERDIETIME1000,d4		; zeitgrenze erreicht?
		blt.s	asmtdDiedSame			; nein -> dann bleibt es bei aktueller animationsphase

		moveq	#1,d4				; zeitcounter resetten (muss auf 1 sein, bei 0 wuerde die sterbesequenz nicht aktiv sein)

		move.w	MONADIEDPOS(a2),d0
		addq.w	#1,d0
		cmp.w	SDATDIEDANZ(a3),d0
		blt.s	asmtdAnimPosOk

		moveq	#-1,d4
		bra.s	asmtdDiedSame

asmtdAnimPosOk	move.w	d0,MONADIEDPOS(a2)

asmtdDiedSame
		move.w	d4,MONADIED(a2)

		move.w	MONADIEDPOS(a2),d0
		add.w	SDATDIEDPTR(a3),d0
		bsr	asmtSetAnimation
...
; ---------------------------------------------------------
; 07.07.00/vk
; setzt endgueltig die animationsdaten (textur) in die
; lineflag-struktur des monsters ein
; d0 = animationsstufe
; d4 = reserviert - zeitcounter fuer animationsphasenaenderung
; a2 = monster
; a3 = sdat fuer grafiktyp des monsters
asmtSetAnimation

		moveq	#0,d3				; flag fuer spiegelverkehrtes zeichnen loeschen

		lsl.w	#5,d0				; 32 bytes pro animationsphase
		cmpi.l	#-1,SDATSDAT+4(a3,d0.w)		; teste innerhalb der phase eintrag 2 von 4
		beq.s	asmtsaSetNow			; wenn -1, dann kein seitentest erforderlich, einfach eintrag 1 setzen

		bsr.s	asmtsaCalcSideOffset		; addiert ggf. seitenoffset (sdat) auf d0

asmtsaSetNow
		move.w	SDATSDAT(a3,d0.w),MONLINE+16+2(a2)	; xyoffi

		move.b	SDATSDAT+2(a3,d0.w),MONLINE+8+1(a2)	; text_nr fuer rueck- und vorderseite

		moveq	#0,d1
		move.b	SDATSDAT+3(a3,d0.w),d1			; hoehe des monsters
		add.w	MONLINE+8+2(a2),d1			; h1 addieren (muss ja bereits durch die monline gesetzt sein)
		move.w	d1,MONLINE+12(a2)			; neue h2 schreiben

		move.w	SDATSDAT+16(a3,d0.w),MONADDFLAGS+2(a2)	; left_right

		move.b	SDATSDAT+18(a3,d0.w),MONLINE+17(a2)	; breite

		tst.w	d3
		beq.s	asmtsasnNoSChg
		bset	#MONADDFLAGMIRRORBIT-24,MONADDFLAGS(a2)		; bit setzen (spiegelverkehrtes zeichnen)
		bra.s	asmtsasnSideOk
asmtsasnNoSChg	bclr	#MONADDFLAGMIRRORBIT-24,MONADDFLAGS(a2)		; bit loeschen
asmtsasnSideOk
		bclr	#MONADDFLAGTRANSBIT-24,MONADDFLAGS(a2)
		rts


; ---------------------------------------------------------
; 09.07.00/vk
; berechnet die winkeldifferenz zwischen monster und spieler.
; addiert auf d0 das notwendige seitenoffset (sdat).
; d0 = animationsphase (fuer sdat)
; a2 = monster
; belegt: d3-d4/a3
; rueckgabe: d0 = inkl. addiertem offset
;            d3 = flag fuer spiegelverkehrtes zeichnen evtl. gesetzt
asmtsaCalcSideOffset

		move.b	MONLINE+12+3(a2),d1		; alpha des monsters

		movea.l	playerDataPtr,a6
		move.w	PDALPHA(a6),d2			; alpha des spieler holen
		lsr.w	#1,d2				; auf bereich [0..255] anpassen
		sub.b	d1,d2
		bmi.s	asmtsaDaNeg
...
asmtsaPL1	cmpi.b	#$60,d2
		bgt.s	asmtsacsoOut

		addi.w	#12,d0				; links
		moveq	#1,d3				; flag fuer spiegelverkehrtes zeichnen setzen
		bra.s	asmtsacsoOut
