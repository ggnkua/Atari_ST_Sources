...
; ---------------------------------------------------------
; setzt die richtigen grafikdaten (textur) des ueber-
; gebenen monsters
; a0 = zeiger auf (allgemeines) monsdat
; a1 = reserviert
; a2 = zeiger auf (aktuelles) monster
; d7 = reserviert
amSetTexturData
		tst.w	MONADIED(a2)
		bne.s	amstdDied
	...
	; keiner sonderbewegung, also nur normal laufen lassen...
		...
		move.w	MONAWALK(a2),d4			; counter fuer phasenwechsel holen
		add.w	vblTime1000,d4			; counter erhoehen
		cmpi.w	#MONWALKTIME1000,d4		; soll in naechste phase gewechselt werden?
		blt.s	amstdWalkSame			; nein -> dann nichts aendern

		moveq	#0,d4
		move.w	MONGRAPHICTYPE(a2),d1		; phasenwechsel: counter wieder resetten
		movea.l	(a0,d1.w*4),a3			; richtige sdat fuer grafik typ (achtung: relativer pointer)
		adda.l	levelPtr,a3			; absoluten pointer generieren
		move.w	MONAWALKPOS(a2),d0		; phasenposition holen
		addq.w	#1,d0				; phasenposition erhoehen
		cmp.w	SDATWALKANZ(a3),d0		; letzte phase schon ueberschritten?
		blt.s	amstdWalkNoEnd			; nein -> dann ueberspringen
		moveq	#0,d0				; wieder auf erste phase setzen
amstdWalkNoEnd	move.w	d0,MONAWALKPOS(a2)		; phasenposition zurueckschreiben
		add.w	SDATWALKPTR(a3),d0		; offset fuer walk-phasen addieren
		bsr	amstdSetPhase			; schreibt die richtige animationsphase in die linieneigenschaften
amstdWalkSame	move.w	d4,MONAWALK(a2)			; counter zurueckschreiben
		bra.s	amstdOut
...
; ---------------------------------------------------------
; setzt die texturdaten der uebergebenen phase fuer
; die monsterlinie
; a0 = zeiger auf (allgemeines) monsdat
; a1 = reserviert
; a2 = zeiger auf (aktuelles) monster
; a3 = zeiger auf sdat fuer (aktuellen) mongrafiktyp
; d0 = animationsstufe (ggf. mit offset)
; d7 = reserviert
amstdSetPhase
		movem.l	d0-a6,-(sp)
		...
		lsl.w	#5,d0				; 32 bytes pro phasenstufe

		cmpi.l	#-1,SDATSDAT+4(a3,d0.w)		; wenn -1, dann kein seitentest
		beq.s	amstdspSetNow

		move.b	MONLINE+12+3(a2),d1		; alpha (bits 7..0)
		movea.l	playerDataPtr,a6
		move.w	PDALPHA(a6),d2			; bereich [0..511]
		lsr.w	#1,d2
		sub.b	d1,d2
		bmi.s	amstdspAlphaNeg

		cmpi.b	#$20,d2
		bgt.s	amstdspPCase1
		addq.w	#8,d0				; hinten (nicht sicher)
		bra.s	amstdspSetNow
amstdspPCase1	cmpi.b	#$60,d2
		bgt.s	amstdspSetNow
		addi.w	#12,d0
		moveq	#%01000000,d3
		bra.s	amstdspSetNow
...
amstdspAlphaNeg
		...
		addq.w	#8,d0				; hinten (nicht sicher)
		bra.s	amstdspSetNow
amstdspNCase1	cmpi.b	#$60,d2
		bgt.s	amstdspSetNow
		addi.w	#4,d0

amstdspSetNow
		move.w	SDATSDAT(a3,d0.w),18(a2)	; xyoffi
		move.b	SDATSDAT+2(a3,d0.w),9(a2)	; textnr
		move.b	8+3(a2),d2
		add.b	SDATSDAT+3(a3,d0.w),d2
		move.w	d2,12+3(a2)
		move.w	SDATSDAT+16(a3,d0.w),22(a2)	; leftright (breitenangaben)
		move.b	SDATSDAT+18(a3,d0.w),17(a2)	; breite (in lineflag)
		move.b	d3,21(a2)			; sidechange
