***************************************************************************
*
* PROG: HACKING LANCE		version: 0.00S		date:	08/02/2022
*
* - version 0.00S, a cleaned up version using static buffer to use in demos
*	version 0.00S by spkr/smfx hands, ggn/kua mind and evl/dhs motivation
*==========================================================================
* FUNCTION:
...
*	08/02/2022	21:19	?????	0.00S	Demosystem adapted version
***************************************************************************


my_Space	equ 250000

zreplayfreq	dc.w		$0001	Values 3 (50KHz)/ 2 (25KHz)/ 1 (12.5KHz)
...
lance_hack_init_example
	lea		myMod,a0
	move.l	#myModEnd-myMod,d0

***************************************************************************
* PROC: lance_hack_init
*	Run this before running the modplayer....
* RECEIVES:
*	- A0.l		address of Modfile
*	- D0.l		size of Modfile
***************************************************************************
lance_hack_init
	bsr		mt_init
;	jsr		instmfp						; is this needed?
	jsr		mt_music
	jsr		mt_mixer
	lea		mt_trash_rd_idx,a0
	move	#1,(a0)			read index set to 1
	move.w	#$4e75,lance_hack_init
	rts

***************************************************************************
* PROC: lance_hack_play
*	Call this every vbl to run the player
***************************************************************************
lance_hack_play
	move.b	mt_trash_active,d0
	beq.s	.skip
		jsr		mt_frame_play
.skip
	jsr	mt_Paula
...
qmalloc:
	movem.l	d1/a1,-(sp)
	move.l	myMallocPtr,a0
	move.l	a0,d1
	addq.l	#1,d0
	bclr	#0,d0			request even number of bytes
	add.l	d0,d1
	move.l	d1,myMallocPtr
	move.l	myMallocLeft,d1
	sub.l	d0,d1
	bge		.ok
		move.b	#0,$ffffc123			; if you hit this, it means youre out of memory
		illegal
.ok
	move.l	d1,myMallocLeft
	movem.l	(sp)+,d1/a1
	rts
...
myMallocSpace	ds.b	my_Space
myMallocLeft	dc.l	my_Space
myMallocPtr		dc.l	myMallocSpace
