...
// 						echo "\tdc.w\t" . ($last-$offset) . "\n";
 ...
 //						echo "\tdc.w\t" . ($abscode-(($streakCount-1)*4)+2) . "\n";
						...
						$newlist[$z][$x][] = array('y'=>$y, 'count'=>$streakCount);
...
//	}
	...
	// this writes the data to generate the clear routs
	echo "genCodeData\n";
	$offset = 1024;
	$ylist = array();
	foreach($maxYArray as $x => $y){
		if($y!=0){
			echo "\tdc.b\t" . $x . "," . $y . "\n";
			$offset += $y*4+4;
			$ylist[$x] = $offset;
...
//	ksort($ylist);
	...
	// now expose the new clear lists
	$offset = 1024;
	for($f=0;$f<360;$f++){
		echo "clearframe_" . $f . "\n";
		for($x=0;$x<20;$x++){
			$nr_entries = count($newlist[$f][$x]);
			if($nr_entries == 0){
				// immediately skip to next part
				if($x!=19){
							echo "\tdc.w\t" . ($offset-8) . "\t; ". $derp . " NEXT (EMPTY)\n";			// next
				}
				else{
							echo "\tdc.w\t" . ($offset-2) . "\t; ". $derp . " RTS\n";			// rts					
				}
			}	
			else{
				$derp = 0;
				for($e=0;$e<$nr_entries;$e++){
					$y = $newlist[$f][$x][$e]['y'];
					$count = $newlist[$f][$x][$e]['count'];

					$startOffset = $ylist[$y];
					$startOffset -= 4;				// for the next-iter
					$startOffset -= ($count*4);
					echo "\tdc.w\t" . $startOffset . "\t; y:" . $y . " count:" . $count . "\n";
					$derp += $count;
					if($e==($nr_entries-1)){			// last
						if($x!=19){
							echo "\tdc.w\t" . ($offset-8) . "\t; ". $derp . " NEXT\n";			// next
						}
						else{
							echo "\tdc.w\t" . ($offset-2) . "\t; ". $derp . " RTS\n";			// rts
						}
