...
machine	equ	1  ; 0=STE, 1=TT  (fix by ray//.tSCc. 2k3)
...
n_reallength	equ	40 ; w
                                   ...
                                   ; rsreset
mt_sample_point	equ	0  ; rs.l	1
mt_sample_end	equ	4  ; rs.l	1
mt_loop_start	equ	8  ; rs.l	1
mt_volume	equ	12 ; rs.w	1
mt_period	equ	14 ; rs.w	1
mt_add_iw	equ	16 ; rs.w	1
mt_check_dummy	equ	18 ; rs.w	1
...
mt_sononew
	...
	ble.s	.mt_set_loop1
	move.l	n_loopstart(a6),d0
.mt_set_loop1	move.l	d0,n_loopstart(a6)
	rts
...
mt_doretrig	move.l	n_start(a6),mt_sample_point(a5)	; set sampledata pointer
	...
	bne.s	.mt_set_loop2
	moveq	#0,d0
.mt_set_loop2	move.l	d0,mt_loop_start(a5)
...
.mt_make_first	move.l	d0,(a0)+
	...
	moveq	#$72,d0
.mt_maker2	move.l	mt_amiga_freq,d1
	move.w	d0,d2
	add.w	d2,d2
	divu.w	d2,d1
	moveq	#0,d2
...
.mt_maker3	move.l	d0,d1
	...
	move.w	d1,d3
	addq.w	#1,d3
.mt_no_round2	move.l	a0,(a1)+
	moveq	#0,d4
	moveq	#24,d7
	moveq	#0,d1
.mt_make_it1	moveq	#0,d1
	add.w	d3,d4
	addx.w	d2,d1
	move.w	d1,d5
	mulu.w	#23<<7,d5
...
mt_return_Paula
	...
	rts
		  ...
		  ; rsreset
mt_temp_regs	equ	0 ; rs.l	2
mt_temp_old_sam	equ	4 ; rs.w	2
mt_channel	macro
	move.l	(a3),d0
	bne.s	.mt_v0_active\1
	lea	mt_dummy_tab,a3
.mt_v0_active\1	move.l	(a4),d0
	bne.s	.mt_v1_active\1
	lea	mt_dummy_tab,a4
.mt_v1_active\1	move.w	mt_volume(a3),d0
	cmp.w	mt_volume(a4),d0
	bge.s	.mt_no_swap\1
	exg	a3,a4
.mt_no_swap\1	move.w	mt_volume(a3),d0
	moveq	#0,d1
	move.w	d0,(a1)
	beq.s	.mt_set_zero2\1
	...
	move.w	(a1,d1.w),d1
.mt_set_zero2\1	move.l	mt_volume_tab,a1
	asl.w	#8,d1
	add.l	a1,d1
...
.mt_return\1
	...
	blt.s	.mt_no_end_v0\1
	move.l	mt_loop_start(a3),d0
	beq.s	.mt_no_loop_v0\1
	move.l	a0,d1
	...
	bra.s	.mt_no_end_v0\1
.mt_no_loop_v0\1	move.w	mt_check_dummy(a3),d2
	bne.s	.mt_no_end_v0\1
	moveq	#0,d2
	move.l	d2,(a3)+
	move.l	d2,(a3)+
	move.l	d2,(a3)+
...
.mt_no_end_v0\1
	...
	blt.s	.mt_no_end_v1\1
	move.l	mt_loop_start(a4),d0
	beq.s	.mt_no_loop_v1\1
	move.l	a1,d1
	...
	bra.s	.mt_no_end_v1\1
.mt_no_loop_v1\1	move.w	mt_check_dummy(a4),d2
	bne.s	.mt_no_end_v1\1
	moveq	#0,d2
	move.l	d2,(a4)+
	move.l	d2,(a4)+
	move.l	d2,(a4)+
	move.l	d2,(a4)+
	move.l	d2,(a4)+
.mt_no_end_v1\1	lea	mt_dummy_tab,a3
	move.l	4(a3),(a3)
	endm
...
mt_set_left	move.w	mt_LCM_mask,$24(a6)
	...
	move.w	.mt_LCM_vol_tab_l(pc,d0.w),d0
	or.w	mt_LCM_left,d0
	move.w	d0,$22(a6)
...
mt_set_right	move.w	mt_LCM_mask,$24(a6)
	...
	add.w	d0,d0			; opt hint: use scaled 030 indexing
	move.w	.mt_LCM_vol_tab_r(pc,d0.w),d0
