...
equalizer
	...
	lea	eq1p,a0			ptr to note freq for channel 1
	bsr.s	eq_handler
	lea	eq2p,a0
	bsr.s	eq_handler
	lea	eq3p,a0
...
eq_zero
	...
	lea	6(a2),a2
eq_right_bar
	dbf	d0,eq_draw_loop		move to next bar
	rts
...
eq_handler
	move.w (a0),d0			get note frequency
...
;leaves with d1 holding bar number.
	...
	moveq #0,d1
eq_get_bar
	cmp.w	(a1)+,d0		compare with table note
...
;-----------
eq_got_bar
	lea	eq_table(pc),a1		ptr to table holding current bar state
...
eq_hc1
	move.b	-1(a1),d0		get side band freq height value
...
addit	SUBQ #1,D0
	BMI.S doneadd
	MOVE.L (A5),A2			; current sample end address(shadow amiga!)
...
player:	move.l a0,usp
bmod	lea.l sndbuff,a0
	move.w (a0)+,d7
	tst.w (a0)
	bmi.s loopit
contsnd	move.l a0,bmod+2
	add d7,d7
	and #$fff8,d7
	lea $ffff8800.w,a0
...
mt_GetNewNote
	...
	LEA	ch1s(pc),A5
	LEA	mt_chan1temp(PC),A6
	move.w 	(a6),eq4p		equaliser variable
	BSR.S	mt_PlayVoice
	LEA	ch2s(pc),A5
	LEA	mt_chan2temp(PC),A6
	move.w 	(a6),eq3p		equaliser variable
	BSR.S	mt_PlayVoice
	LEA	ch3s(pc),A5
	LEA	mt_chan3temp(PC),A6
	move.w 	(a6),eq2p		equaliser variable
	BSR.S	mt_PlayVoice
	LEA	ch4s(pc),A5
	LEA	mt_chan4temp(PC),A6
	move.w 	(a6),eq1p		equaliser variable
	BSR.S	mt_PlayVoice
	BRA	mt_SetDMA
...
freqs		dc.l 0
		incbin 	"c:\tracker\data\freq60.bin
vols		incbin 	"c:\tracker\pt_volta.dat
mt_data		incbin	"f:\mods\cebit",0
