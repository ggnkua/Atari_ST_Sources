 ;--------------------------------------------------------------------------
; Picture Tweening intro routine for Hallucinations.
; (C) March 1993 Griff.
;--------------------------------------------------------------------------
...
do2tst		MACRO
		...
		TST.W D6
		BEQ.S .ok\@
		ADDQ #1,(A2)			; add 1 to count
		MOVE.W D0,(A6)+			; store this x
		MOVE.W D1,(A6)+			; store this y

.ok\@		
		ENDM

test_cols	MOVE.L A6,(A1)
.pass2		MOVEQ #0,D1			; y=0
.y2_lp		MOVEQ #0,D0			; x=0
		MOVEQ #20-1,D7
.x2_lp		MOVEM.W (A0)+,D2/D3/D4/D5	; fetch bpl 1-4
		REPT 16	
		do2tst
		ADDQ #1,D0			; x=x+1
		ENDR				
...
calc_grads	LEA coltestsc_ptrs(PC),A0
		LEA coltestde_ptrs(PC),A1
		LEA coltestsc_cnt(PC),A2
		LEA coltestde_cnt(PC),A3
.col_grad_lp	MOVE.W (A2),D0		; source no crds		
...
coltestsc_ptrs	DS.L 1
coltestsc_cnt	DS.W 1
coltestde_ptrs	DS.L 1
coltestde_cnt	DS.W 1
