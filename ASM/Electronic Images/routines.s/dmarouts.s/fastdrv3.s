***********************************************************************
* This is the 100% finished version of the Music Driver.
* (C) 1990-1991 Nic Alderton aka Count Zero of Electronic Images.
* (a few little optimisations by Griff 02/09/91)
***********************************************************************

	opt	o+,ow-		OPTIMISATION ON(NIC!)
 	opt	p+		ENSURE RELOCATABLE
...
_o_toneblow	equ	2+8
_o_tonebhi	equ	2+12
_o_toneclow	equ	2+16
_o_tonechi	equ	2+20
_o_noisefreq	equ	2+24
_o_control	equ	2+28
_o_volumea	equ	2+32
_o_volumeb	equ	2+36
_o_volumec	equ	2+40
_o_waveformlow	equ	2+44
_o_waveformhi	equ	2+48
_o_waveformdef	equ	2+52
...
**********
	...
	move.l	VBLSAVE(PC),$70.w	Restore Old Vbl

	move.w	#$777,$ffff8240.w
	
	move.w	#$2300,sr
	
	move.l	STACKSAVE(PC),-(a7) User mode
	move.w	#$20,-(a7)
	trap	#1
	addq.l	#6,a7
...
C_Z_I
 ...
 IFNE	quickmake
	bra	SUPEROUTPUT
 ENDC
	dc.b	"Count Zero (Electronic Images) Music 01/07/91."
	dc.b	"Phone 071-431-4915."
	dc.b	"Please credit me if you use this music! Thanks!"
	even

* Initialise/turn off the tune. 40% faster than before.
...
DO_OUTPUT
	...
	movem.l (A0),D0-D7/A0-A5		regs $00 TO $0D
	tst.w	__changedwave(a6)
	beq.s	OUT_SAMEWAVE
	movem.l	D0-D7/A0-A5,$FFFF8800.W	CHANGE WAVE TOO!
	clr.w	__changedwave(a6)
	rts
OUT_SAMEWAVE
	movem.l	D0-D7/A0-A4,$FFFF8800.W	NO CHANGE WAVE
	rts

OUTPUT	DC.L $00000000
	DC.L $01000000
	DC.L $02000000
	DC.L $03000000
	DC.L $04000000
...
**************
	...
	add.w	d0,d0
	add.w	d0,d0
	jmp	COMMAND_JUMPTABLE(pc,d0.w)
COMMAND_JUMPTABLE
	bra.w	CONTINUE_NOTE	$f0
	bra.w	REST		$f1
	bra.w	SET_VOLUME	$f2
	bra.w	SET_INSTRUMENT	$f3
	bra.w	SET_NOISE		$f4
	bra.w	SET_NOISESTATUS	$f5
	bra.w	SET_CHORD1	$f6
	bra.w	SET_CHORD2	$f7
	bra.w	SET_CHORD3	$f8
	bra.w	SET_WAVEFORM	$f9
	bra.w	SET_WAVENOTE	$fa
	bra.w	SET_PORTSPEED	$fb	
	bra.w	SET_WAVEPERIOD	$fc
	bra.w	SET_QUICKCHORD	$fd
	bra.w	SET_FREQTRANS	$fe
	bra.w	SET_NOTETRANS	$ff
CONTINUE_NOTE
...
REAL_NOTE
	clr.w 	d2		Clear D2
	move.w	d2,_porting(a6)	Tranponse off
	move.w    d2,_portvalue(a6)
	move.w	d2,_portspeed(a6)
	move.w	d2,_portcount(a6)
	move.w	#-1,_miniloopcount(a6)
	move.w	d2,_waveisused(a6)
...
* Synth processing
*************
* Time Critical so has been optimised about 15%.

NO_MINI	addq.w	#6,_synthoff(a6)	Increment Synth offset
	move.l	_synthbase(a6),a0	Synth base
	adda.w	_synthoff(a6),a0	As an offset address
	moveq	#0,d1
	move.b	5(a0),d0
	move.b	4(a0),d1
	cmpi.b	#$ff,d0
	...
	addq.w	#6,d1
	sub.w	d1,_synthoff(a6)	(fall thru to SYNTH)
SYNTH	tst.w	_miniloopcount(a6)	Is there a minicount on?
	ble.s	NO_MINI		Nope
...
NOT_LOOP_BLOCK
	...
	ext.w	d2
	ext.w	d3
	and.w	#$ff,d0		safe but could i use ext.w d0?
	moveq	#%11,d6
...
*************
* HMMMM... NOTHING MUCH CAN BE IMPROVED HERE.
