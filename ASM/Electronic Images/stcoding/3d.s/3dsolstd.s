...
; Updated/improved :-									;		
; Fully clipped         - Jan 1990(arghh! stolen from ST world)		;
...
; The faces are drawn in Z order, faces that face away are eliminated	;
;-----------------------------------------------------------------------;
...
; A5 points to shape data.(in 8 view format)
...
; A0 now points no. of faces in shape data.

		LEA new_coords(PC),A4		
		LEA insert(PC),A3
		LEA averagezs(PC),A1
		MOVE.W (A0)+,D3			no of faces
		MOVE.L A0,sfacelist+2
		SUBQ #1,D3				faces-1 for dbf
		MOVEQ #0,D5				face counter
		MOVE.L D5,root
avfacelp	MOVE D5,no(a1)
		MOVE.L (A0)+,D1			COL\no of verts
		MOVE.W D1,D2
		ADD D2,D2
		ADD D2,D5
...
r1		SUBQ #1,D6			; rows left on right
		BMI Right_rchd		;
		MOVE.L D3,D0		; left  x wholepart.w/fraction.w
		SWAP D0			; x1
		MOVE.L D4,D1		; right x   "       "        "
		SWAP D1			; x2
		ADD.L A1,D3			; add gradients				
		ADD.L A2,D4			;
Hline		SUB D0,D1			; dx
		MOVEQ #15,D2		; (15 AND X1)
		AND D0,D2			; 
		...
		CMP #15,D1			; is hline over one chunk?
...
one_case	ADD D1,D1				; remaining pixels(0-15)
		OR e_masks(PC,D1.W),D2 		; combine end mask with start mask
		MOVE D2,D0				
		NOT D0				; d0 = data d2 = mask
chunk1	OR D0,(A0)+
		OR D0,(A0)+
		...
		RTS
		DCB.W 20,$FFFF
e_masks	DC.W $7FFF,$3FFF,$1FFF,$0FFF
...
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
; Create 8 views of an object								;
; A0-> ptr to object we wanna create the 8 views for.				;		
; A1-> ptr to buffer to store 8 view in.					 	;
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
...
cube		DC.W 0,0,0
		...
		DC.W -200,200,200
		DC.W 6
		DC.W 09*16,4,03*6,02*6,01*6,00*6
		DC.W 10*16,4,02*6,06*6,05*6,01*6
		DC.W 11*16,4,03*6,07*6,06*6,02*6
		DC.W 12*16,4,06*6,07*6,04*6,05*6
		DC.W 13*16,4,01*6,05*6,04*6,00*6
		DC.W 14*16,4,04*6,07*6,03*6,00*6
...
thargoid	DC.W 0,0,0	
		...
		DC.W -252,252,-100
		DC.W 10
		DC.W 1*16,8,00*6,01*6,02*6,03*6,04*6,05*6,06*6,07*6
		DC.W 2*16,8,15*6,14*6,13*6,12*6,11*6,10*6,09*6,08*6
		DC.W 3*16,4,06*6,14*6,15*6,07*6
		DC.W 4*16,4,05*6,13*6,14*6,06*6
		DC.W 5*16,4,04*6,12*6,13*6,05*6
		DC.W 6*16,4,03*6,11*6,12*6,04*6
		DC.W 3*16,4,02*6,10*6,11*6,03*6
		DC.W 4*16,4,01*6,09*6,10*6,02*6
		DC.W 5*16,4,00*6,08*6,09*6,01*6
		DC.W 6*16,4,07*6,15*6,08*6,00*6
...
station	DC.W 0,0,0
		...
		DC.W -200,200,0
		DC.W 14
		DC.W 13*16,3,00*6,01*6,02*6
		DC.W 14*16,3,09*6,11*6,10*6
		DC.W 13*16,3,04*6,08*6,09*6
		DC.W 14*16,3,01*6,03*6,08*6
		DC.W 13*16,3,03*6,07*6,11*6
		DC.W 14*16,3,00*6,05*6,07*6
		DC.W 13*16,3,05*6,06*6,10*6
		DC.W 14*16,3,02*6,04*6,06*6
		DC.W 15*16,4,00*6,02*6,06*6,05*6
		DC.W 09*16,4,04*6,02*6,01*6,08*6
		DC.W 11*16,4,00*6,07*6,03*6,01*6
		DC.W 09*16,4,07*6,05*6,10*6,11*6
		DC.W 15*16,4,08*6,03*6,11*6,09*6
		DC.W 11*16,4,06*6,04*6,09*6,10*6
...
asteriod	DC.W 0,0,0
		...
		DC.W -100,200,0
		DC.W 20
		DC.W 01*16,3,01*6,02*6,04*6
		DC.W 14*16,3,01*6,00*6,02*6
		DC.W 15*16,3,02*6,00*6,05*6
		DC.W 01*16,3,02*6,05*6,06*6
		DC.W 10*16,3,02*6,06*6,04*6
		DC.W 11*16,3,04*6,06*6,09*6
		DC.W 13*16,3,04*6,09*6,08*6
		DC.W 10*16,3,01*6,04*6,08*6
		DC.W 01*16,3,03*6,01*6,08*6
		DC.W 09*16,3,00*6,01*6,03*6
		DC.W 10*16,3,07*6,11*6,10*6
		DC.W 13*16,3,00*6,07*6,05*6
		DC.W 01*16,3,10*6,05*6,07*6
		DC.W 10*16,3,06*6,05*6,10*6
		DC.W 09*16,3,06*6,10*6,09*6
		DC.W 14*16,3,09*6,10*6,11*6
		DC.W 15*16,3,08*6,09*6,11*6
		DC.W 10*16,3,03*6,08*6,11*6
		DC.W 01*16,3,03*6,11*6,07*6
		DC.W 11*16,3,00*6,03*6,07*6
...
Ship		DC.W 0,0,0
		...
		DC.W 0,0,-300
		DC.W 10	
		DC.W 1*16,3,0*6,1*6,5*6
		DC.W 2*16,3,1*6,3*6,5*6
		DC.W 3*16,3,1*6,2*6,3*6
		DC.W 4*16,3,5*6,3*6,4*6
		DC.W 5*16,3,6*6,2*6,1*6
		DC.W 6*16,3,6*6,1*6,0*6
		DC.W 7*16,3,4*6,3*6,6*6
		DC.W 8*16,3,5*6,4*6,6*6
		DC.W 9*16,3,3*6,2*6,6*6
		DC.W 10*16,3,0*6,5*6,6*6
...
House		DC.W 0,0,0
		...
		DC.W 200,150,0
		DC.W 7
		DC.W 1*16,4,0*6,1*6,2*6,3*6
		DC.W 2*16,4,7*6,6*6,5*6,4*6
		DC.W 3*16,5,0*6,3*6,7*6,4*6,8*6
		DC.W 3*16,5,1*6,9*6,5*6,6*6,2*6
		DC.W 4*16,4,8*6,9*6,1*6,0*6
		DC.W 5*16,4,4*6,5*6,9*6,8*6
		DC.W 7*16,4,6*6,7*6,3*6,2*6
...
log_base	DC.L $268000
phy_base	DC.L $270000
		DC.L $278000
