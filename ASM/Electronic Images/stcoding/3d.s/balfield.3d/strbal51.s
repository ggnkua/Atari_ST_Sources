...
; Please NOTE this is not a lame add+perspect starfield! (as  TLB!)	;
; Each Star(ball) is rotated about x,y,z and uses proper perspective.	;
;-----------------------------------------------------------------------;

;no_strs	EQU 74		; ste
no_strs		EQU 110		; mega ste
spheight	EQU 16
...
ballsequence	DC.W 150,0,0,0,-8
restart		DC.W 50,0,0,0,-8
		DC.W 4,2,0,0,-8
		DC.W 4,4,0,0,-8
		DC.W 4,6,0,0,-8
		DC.W 4,8,0,0,-8
...
Trans_balls	LEA balls_pos(PC),A1		; co-ords
zspeed		MOVE.W #10,A0			; z speed
		MOVE.W #160,A2			; x centre
		MOVE.W #116,A3			; y centre
		LEA translated(PC),A4		; translated storage
		MOVE #256,A5			; max Z
		MOVE #512,A6			; Z plane =-256(reset star)
...
star_lp		MOVE.W (A1)+,D0			; get x
		...
		BCC Star_off			; yes then reset it
cont1		MOVE D2,(A1)+			; otherwise store...
		SUB A5,D2			; z-256 (-256 to 256 range)
		MOVE D0,D3			
		MOVE D1,D4			; dup
		MOVE D2,D5
...
offz		MOVE.W D0,(A4)+
		MOVE.W #220,(A4)+		; z<=0 then do not draw!! 	
		MOVE.W D2,(A4)+		
		DBF D7,star_lp			
		RTS
...
Plot_em		LEA bold_pos1+2(PC),A3		; assume case 1
		TST switch
		BNE.S .case_1
.case_2		LEA bold_pos2+2(PC),A3		; no its case 2!
.case_1		LEA blmul_160+32(PC),A1
		LEA translated(PC),A4		; translated co-ords
		LEA ballxlookup(PC),A5
		...
		PEA (A3)
		MOVE #no_strs-1,D7 
.plot_ball	MOVE.L D3,A2			; screen base in A2
		MOVEM.W (A4)+,D0-D2
		CMP D5,D1			; clip y
		BCC .noplot
		CMP D4,D0			; clip x
		...
		MOVE.W (A6,D2),D2
		MOVE.W D2,(A3)+
		ADD.W D2,D0			; add z buf ptr.(sprite size) 
		MOVE.L D6,A0
		MOVE.L (A0,D0),A0
		JSR (A0)
.noplot		DBF D7,.plot_ball
...
Initballfield	bsr Makeballrouts
		bsr make160table
		bsr makexlookup
		bsr makezlookup
		bsr BRandom_gen
...
; Z lookup 32 words 0*64,1*64 to 7*64(for 8 sprite sizes)

makezlookup	LEA ballzlookup,A0
		MOVEQ #0,D2
		MOVEQ #7,D0
.makez1lp	MOVEQ #31,D1
.makez2lp	MOVE.W D2,(A0)+
		DBF D1,.makez2lp
		ADD #64,D2
		DBF D0,.makez1lp		
		MOVEQ #31,D1
.makez3lp	MOVE.W #7*64,(A0)+
		DBF D1,.makez3lp
		RTS
...
BRandom_gen	LEA balls_pos(PC),A3
		MOVE #no_strs-1,D4
.f_rand_x	BSR BRand
		DIVU #400,D0
		SWAP D0
...
ballzlookup	ds.l 16*9	  ; z lookup
...
ball16		incbin balls16.spr
		...
		SECTION BSS
balls_pos	DS.W no_strs*3
bold_pos1	DS.L no_strs+1
bold_pos2	DS.L no_strs+1
