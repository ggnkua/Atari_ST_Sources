; New 3d matrix routines. uses much less multiplications.
...
dist		EQU 256		; viewpoint distance (perspective)
letsgo		
		IFEQ gemrun
		MOVE.W #4,-(SP)
		TRAP #14
...
* Matrix(3,3) cosz*cosx
	...
	move.w d3,md06+2
	movem.w	(a0)+,d1-d3
	asr.w	#1,d1		; we only loose two bits accuracy
	asr.w	#1,d2		; here.... as opposed to 4 originally!!!
	asr.w	#1,d3		;
	and.w	d6,d1		;
	and.w	d6,d2		;
	and.w	d6,d3		;
...
trig:	  	incbin d:\3d.s\newnomul.3d\trig2c.dat ; 16k arccosine table.
acos:		equ	trig+(2*8192)	
