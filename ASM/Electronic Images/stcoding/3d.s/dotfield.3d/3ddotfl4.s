;-----------------------------------------------------------------------;
;									;
;       3d Space field using dots. (REAL 3d X,Y,Z translation!)		;
;									;
;     (C) May 1991 Martin Griffiths (Griff of Electronic Images).	;
;									;
;(This version uses MULS for Perspective and has MAX of apprx 160 stars);
;									;
;-----------------------------------------------------------------------;

no_strs		EQU 150
demo		EQU 0
screen1		EQU $E0000		; screen addresses(tut tut!)
screen2		EQU $E8000
...
letsgo
		...
		BSR Random_gen
		BSR make160tab
		BSR crplotmasks			; initialisation
		BSR makeperstab
		BSR ClearScreens
		BSR Set_Cols
		BSR Init_ints
...
wait_space	CMP.B #$39,$FFFFFC02.W
		BNE wait_space
...
; Save Gem Ints and Install our own.

Init_ints	LEA old_mfp(PC),A0
...
ClearScreens	MOVE.L log_base(PC),A0
		BSR cls
		MOVE.L phy_base(PC),A0
		BSR cls
		RTS

cls		MOVE #32000/16,D0
		MOVEQ #0,D1
cls_lp		MOVE.L D1,(A0)+
		MOVE.L D1,(A0)+
		MOVE.L D1,(A0)+
...
ballsequence	DC.W 400,0,0,0,18
restart		DC.W 128,2,0,0,18
		DC.W 2,4,0,0,18
		DC.W 2,6,0,0,18
		DC.W 124,8,0,0,18
		DC.W 128,8,2,0,18
...
Star_off	ADD #1024,D2			; reset z
		...
		AND #1023,D1
		SUB #512,D1 
		MOVEM.W D0-D1,-4(A0)		; store new rand x
		BRA conts
...
colours		DC.W $000,$333,$555,$555,$777,$777,$777,$777
