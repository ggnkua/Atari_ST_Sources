...
Draw_object	LEA $FFFF8A00.W,A6    	; point to blitter
		MOVEQ #-1,d5
		MOVE.L d5,40(a6)
		MOVE.W d5,44(a6)	; set end masks
		MOVE.W #8,dst_xinc(A6)
		MOVE.W #8,src_xinc(A6)
		MOVE.B #2,HOP(A6)      ; hop
...
.lp1		MOVEM.W (A4),D0/D1/D2/D3
		...
		MOVEM.W miny(PC),D0-D3  

.fillit		SUB.W D0,D1		; d1=dy
		BEQ .miss
		LEA blmul_160(PC),A4
		ADD.W D0,D0
		MOVE.W (A4,D0),D0
		LSR #1,D2		; start chunk
		AND.W #$FFF8,D2	
		LSR #1,D3
		AND.W #$FFF8,D3		; end chunk
		ADD D2,D0
		ADD D0,A6		; me
		SUB.W D2,D3		; dx
		MOVE #160,D4
		SUB.W D3,D4
		LSR #3,D3
		ADDQ #1,D3		; round up xcount

		MOVE.L thisoldpolyptr(PC),A3
		MOVE.L A6,(A3)+
		MOVE.W D1,(A3)+
		MOVE.W D3,(A3)+
		MOVE.W D4,(A3)+
		MOVE.L A3,thisoldpolyptr

		LEA $FFFF8A00.W,A4    	
		MOVE.L A6,src_addr(A4) ; source address
		LEA 160(A6),A6
		MOVE.L A6,dst_addr(A4) ; dest address
		MOVE.W D1,y_count(A4); 
		MOVE.W D3,x_count(A4)	; no of words to write		
		MOVE.W D4,dst_yinc(A4)
		MOVE.W D4,src_yinc(A4)
		MOVE.W #%1100000000000000,line_num(A4)	
.miss		MOVE.W (SP)+,D0
		SUBQ #1,D0
		BNE .facelp
		MOVE.L thisoldpolyptr(PC),A3
...
minx		DC.W 0
maxx		DC.W 0
oldpolyptr1	DC.L oldlist1
oldpolyptr2	DC.L oldlist2
thisoldpolyptr	DC.L oldlist1
oldlist1	DC.W -1
		DS.W 200
oldlist2	DC.W -1
		DS.W 200

clearoldpolys	LEA $FFFF8A00.W,A6    	; point to blitter
		MOVEQ #-1,d5
		MOVE.L d5,40(a6)
		MOVE.W d5,44(a6)	; set end masks
		MOVE.W #8,dst_xinc(A6)
		...
		LEA oldpolyptr1(PC),A0
		MOVEM.L (A0)+,A1/A2
		MOVE.L A1,-(A0)
		MOVE.L A2,-(A0)
		MOVE.L A1,thisoldpolyptr
.letscontinue	TST.W (A1)
		BMI.S .out
		MOVE.L (A1)+,A2
		MOVE.W (A1)+,D1
		MOVE.W (A1)+,D3
		MOVE.W (A1)+,D4
		ADDQ #1,D1
		MOVE.L A2,dst_addr(A6) ; dest address
		MOVE.W D1,y_count(A6); 
		MOVE.W D3,x_count(A6)	; no of words to write		
...
.nowr\@
		...
		SUB.W D0,A2	; -*8
		ADD.W D0,D0
		SUB.W D0,A2	; -*16 so its a wooky *26
		JMP (A2)
...
blmul_160	ds.w 200	  ; *160 table + extra for clip.
