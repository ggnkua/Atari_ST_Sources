...
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
; Routine to draw a 1 plane line					;
; D0-D3 holds x1,y1/x2,y2 A1 -> screen base. A3-> *160 table		;
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
;A6/A5/A4/D7

Drawline	CMP.W D0,D2			; CLIP ON X				
		BGE.S .gofordraw
		EXG D0,D2			; reorder
		EXG D1,D3			; draw from LEFT to RIGHT
.gofordraw	MOVE.W #160,D6			; assume top to bottom
		SUB.W D0,D2			; dx
		SUB.W D1,D3			; dy
		BGE.S .posdy
		NEG.W D3			; abs(dy)
		NEG.W D6			; draw bottom to top
