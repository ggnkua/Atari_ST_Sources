;-----------------------------------------------------------------------;
;     Coded By Griff of Electronic Images (Inner Circle) (c) 1992.	;
;-----------------------------------------------------------------------;
...
letsgo		
		...
		LEA plasma10(PC),A2		; plasma1/2 or 3!
		BSR Make_plasma
...
syncloop	MOVE.B (A0),D1
		...
		move.w #64*208,x_count(a6) ; no of words to write		
		move.b #2,HOP(a6)          ; hop
		move.b #3,OP(a6)           ; replace
...
; Make 512 colour table.

makecolours	LEA colours(PC),A0
		MOVEQ #7,D0
.rlp		MOVEQ #7,D1
.glp		MOVEQ #7,D2
.blp		MOVE.W D0,D3
...
cyclecolours	LEA logcolours,A0
		LEA colourtab(PC),A1
		LEA colours(PC),A2
		LEA 64*2(A1),A3
		LEA 64*4(A1),A4
...
stack		DS.L 4
colours		DS.W 512
colourtab	DS.W 64*210
logcolours	DS.W 64*210
