...
var col1,col2,x:byte;
 begin
  ...
  x:=x+length(title);
  repeat
   Select_background(col1);
   printat(x,y,' YES ');
   Select_background(col2);
   printat(x+5,y,' NO ');
   getkey(key,funckey);
   case key of
     LEFTKEY:
...
{ Now we actually get round to producing the compressed file!!! }
   ...
   errorcode:=IOResult; if errorcode<>0 then exit;
   assign(outfile,new_name); rewrite(outfile);
   errorcode:=IOResult; if errorcode<>0 then exit;
...
{ Read 32 bit filelength from packed file - 4 bytes! }
 assign(infile,filename);
 reset(infile); errorcode:=IOResult; if errorcode<>0 then exit;
