...
sam_lengthflg	RS.B 1
sam_startflg	RS.B 1	
sam_lplengthflg	RS.B 1
sam_lpstartflg	RS.B 1	
...
move_samstart	MACRO
		MOVE.L \1,sam_start(\2)
		ST.B sam_startflg(\2)
		ENDM

move_samlpstart	MACRO
		MOVE.L \1,sam_lpstart(\2)
		ST.B sam_lpstartflg(\2)
		ENDM

move_samlength	MACRO
		MOVE.W \1,sam_length(\2)
		ST.B sam_lengthflg(\2)
		ENDM

move_samlplength MACRO
		MOVE.W \1,sam_lplength(\2)
		ST.B sam_lplengthflg(\2)
		ENDM
...
Vbl_play:	LEA music_on(PC),A0
		...
		LEA ch1s(PC),A0
		SF.B sam_startflg(A0)
		SF.B sam_lengthflg(A0)
		SF.B sam_lpstartflg(A0)
		SF.B sam_lplengthflg(A0)
		LEA ch2s(PC),A0
		SF.B sam_startflg(A0)
		SF.B sam_lengthflg(A0)
		SF.B sam_lpstartflg(A0)
...
.noloop		MOVEQ #0,D7			; no loop-no frequency
		LEA nulsamp(PC),A6		; no loop-point to nul
.isloop		ADD.L D5,A6

		MOVE.L D1,Vfreq(A0)		; store PERIOD int.w/frac.w
		MOVE.W D2,Vvoltab(A0)		; store Volume address of volume tab.
		TST.B sam_startflg(A5)
		BEQ.S .miss1
		MOVE.L A2,Vaddr(A0)		; store address
.miss1		TST.B sam_lengthflg(A5)
		BEQ.S .miss2
		MOVE.L D0,Voffy(A0)		; store offset
.miss2		TST.B sam_lpstartflg(A5)
		BEQ.S .miss3
		MOVE.L A6,Vlpaddr(A0)		; store loop addr
.miss3		TST.B sam_lplengthflg(A5)
		BEQ.S .miss4
		MOVE.L D5,Vlpoffy(A0)		; store loop offset.
		MOVE.L D7,Vlpfreq(A0)		; store loop freq int.w/frac.w
.miss4		RTS
...
.nores		BSR add12			; and do remaining
		LEA Voice1Set(PC),A4
		MOVE.L A0,Vaddr(A4)		; store voice address
		MOVE.L D0,Voffy(A4)		; store offset for next time
		SWAP D6
		MOVE.W D6,Vfrac(A4)		; store frac part
		MOVEM.W D7/A2,Vfreq(A4)
		LEA Voice2Set(PC),A4
		MOVE.L A1,Vaddr(A4)		; same for chan 2
		MOVE.L D1,Voffy(A4)		
		MOVE.W D4,Vfrac(A4)
		MOVEM.W D5/A3,Vfreq(A4)
		MOVE.L (SP)+,A4
...
initvoice:	MOVE.L	A2,Vaddr(A0)    ; point voice to nul sample
		MOVE.L	#2,Voffy(A0)		
		MOVE.L	D0,Vfreq(A0)   	; period=0
		MOVE.W  D0,Vfrac(A0)
		MOVE.L	D0,Vlpfreq(A0)  ; period=0
		MOVE.W	D0,Vvoltab(A0)	; volume=0
		MOVE.L	A2,Vlpaddr(A0)  ; and loop point to nul sample
		MOVE.L	#2,Vlpoffy(A0)
		RTS
...
mt_trenoc
	move_samstart n_start(A6),A5
	move_samlength n_length(A6),A5
...
mt_SetDMA
	...
	LEA ch4s,A5
	LEA mt_chan4temp(PC),A6
	move_samlpstart n_loopstart(A6),A5
	move_samlplength n_replen(A6),A5
...
mt_FilterOnOff
	...
	AND.B	#1,D0
	ASL.B	#1,D0
	AND.B	#$FD,shadow_filter
	OR.B	D0,shadow_filter
	RTS	
