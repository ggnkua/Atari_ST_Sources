; 8 bit sample one...  NEW MIX (and seems to bit 8 bit accurate!).
;-----------------------------------------------------------------------;
;									;
;			   TTP .MOD Player				;
...
load_mod	BSR Wait_Vbl
		...
		BSR.S clsfast			; clear the screen
...
thetext	
 dc.b 4,4,4
 dc.b 3,"ProWanker V2.1a Replay",4
 dc.b 3,"Programmed by Griff of Electronic Images",4
 dc.b 3,"8-BIT BETA TEST replay by The Phantom for Gregsy poos!",4
 dc.b 3,"- 50Khz STE/TT Version v1.3 -",4,4
...
do2chans	MOVEM.L A2-A3/A5-A6,-(SP)
		...
		MOVE.W Vfrac(A5),D4		; frac.w offset
		...
		MOVE.W D4,Vfrac(A5)		; store frac part
		MOVE.L A1,sam_start(A3)		; same for chan 2
		MOVE.L D1,sam_length(A3)		
		MOVE.W D5,Vfrac(A6)
		RTS
ciaem_tab	INCBIN f:\turds\CIA_EMU.TAB
...
;	MOVE.B	3(A3,D4.L),n_volume(A6)
	move.w	(a3)+,n_finetune(a6)	; Two in one!  Even addr.
	MOVE.W	(A3),D3 ; Get repeat
;	TST.W	D3
	BEQ.S	mt_NoLoop
	MOVE.L	n_start(A6),D2	; Get start
	ADD.W	D3,D3			; Was ASL.L (agg!).
	ADD.L	D3,D2		; Add repeat
	MOVE.L	D2,n_loopstart(A6)
	MOVE.L	D2,n_wavestart(A6)
	MOVE.W	(A3)+,D0	; Get repeat
	ADD.W	(A3),D0	; Add replen
	MOVE.W	D0,n_length(A6)
	MOVE.W	(A3),n_replen(A6)	; Save replen
	move.l	d5,a3
	MOVEQ	#0,D0
...
mt_NoLoop
	...
	MOVE.L	D2,n_loopstart(A6)
	MOVE.L	D2,n_wavestart(A6)
	MOVE.W	2(A3),n_replen(A6)	; Save replen
	move.l	d5,a3
	MOVEQ	#0,D0
	MOVE.B	n_volume(A6),D0
	MOVE.W	D0,sam_vol(A5)	; Set volume
...
;	MOVEM.L	D0-D1/A0-A1,-(SP)
	move.l	a1,d4
	MOVE.W	(A6),d6			; D1 replaced with D6.
	AND.W	#$0FFF,d6
...
mt_ftuloop
	...
	BHS.S	mt_ftufound
	endr
	tst.w	(a1)+			; Last one ends it!
mt_ftufound
	clr.w	d5			; Normally a moveq.
	MOVE.B	n_finetune(A6),d5
	lsl.w	#3,d5
	move.w	d5,d6
	lsl.w	#3,d5
	add.w	d6,d5
...
mt_CheckEfx
	...
	AND.W	#$0F,D0		; Used to be AND.B so just in case...
...
j0:	bra.w	mt_arpeggio
j1:	bra.w	mt_portaup
j2:	bra.w	mt_portadown
j3:	bra.w	mt_toneportamento
j4:	bra.w	mt_vibrato
j5:	bra.w	mt_toneplusvolslide
j6:	bra.w	mt_vibratoplusvolslide
j7:	bra.w	tremolo_special		; Does 'setback'.
j8:	bra.w	mt_Return	
j9:	bra.w	mt_Return	
j10:	bra.w	volume_special		; Does 'setback'.	
j11:	bra.w	mt_Return	
j12:	bra.w	mt_Return	
j13:	bra.w	mt_Return	
j14:	bra.w	mt_e_commands	
;	bra.w	mt_Return

SetBack	MOVE.W	n_period(A6),sam_period(A5)	; *** JUST IN CASE ***
;	CMP.B	#7,D0
;	BEQ	mt_Tremolo
;	CMP.B	#$A,D0
;	BEQ	mt_VolumeSlide
...
mt_FinePortaUp
	TST.B	mt_counter
	BNE.S	mt_Return2		; Normally just Return (.S opt).
...
mt_FinePortaDown
	TST.B	mt_counter
	BNE.S	mt_Return2		; Shortened, as the one above.
	MOVE.B	#$0F,mt_LowMask
mt_PortaDown
	CLR.W	D0
	MOVE.B	n_cmdlo(A6),D0
...
jet:
	...
	bra.w	mt_PerNop
	endr
	bra.w	mt_SampleOffset
	bra.w	mt_PerNop
	bra.w	mt_PositionJump
	bra.w	mt_VolumeChange
	bra.w	mt_PatternBreak
	bra.w	mt_E_Commands
	bra.w	mt_SetSpeed
...
js0:	bra.w	mt_FilterOnOff
js1:	bra.w	mt_FinePortaUp
js2:	bra.w	mt_FinePortaDown
js3:	bra.w	mt_SetGlissControl
js4:	bra.w	mt_SetVibratoControl
js5:	bra.w	mt_SetFineTune
js6:	bra.w	mt_JumpLoop
js7:	bra.w	mt_SetTremoloControl
js8:	bra.w	mt_no8
js9:	bra.w	mt_RetrigNote
js10:	bra.w	mt_VolumeFineUp
js11:	bra.w	mt_VolumeFineDown
js12:	bra.w	mt_NoteCut
js13:	bra.w	mt_NoteDelay
js14:	bra.w	mt_PatternDelay
js15:	bra.w	mt_FunkIt
...
mt_FunkIt
	...
	BEQ	mt_Return
mt_UpdateFunk
;	MOVEM.L	A0/D1,-(SP)	; And D1 replaced with D6.
	move.l	a0,d5
	MOVEQ	#0,D0
	MOVE.B	n_glissfunk(A6),D0
	LSR.B	#4,D0
...
testfilename	DC.B "f:\modcon.mr\totally.mod"
endtestfilename
		ENDC
