...
music_int	MOVEM.L	A0-A6/D0-D7,-(A7)
		...
		LEA 4(A6),A5
		MOVEQ #0,D7
		MOVE.L D7,(A5)	; strobe -> signal dsp to go!
		LEA ch1s(PC),A1
		MOVEQ #0,D6
		BSR.S send_voice
		LEA ch2s(PC),A1
...
.wait1		BTST #1,2(A6)			;; sync 
		...
		MOVE.L #$24665268,D1
		DIVU.L D0,D1
.zero		MOVE.L D1,(a5)			; send freq
.wait2		BTST #0,2(A6)			;; sync 
		BEQ.S .wait2			;; 
		MOVE.L (A5),D4			; get no. of bytes to add
.wait3		BTST #0,2(A6)			;; sync 
		BEQ.S .wait3			;; 
		MOVE.L (A5),D5			; get no. of samples
		ADDQ #2,D5			; +2 
		MOVE.W shadow_dmacon(PC),D2	; dma control (!)
		MOVE.L D5,(A5)			; send no. of samples to do.
		SUBQ #1,D5			; -1 for dbf
		BTST D6,D2			; voice dma on?
		BEQ vcoff			; no, then send zeros...
vce_don		MOVEQ #0,D0
		MOVEQ #0,D1
		MOVE.W sam_lplength(A1),D1
		MOVE.W sam_length(A1),D0
		BEQ.S .nosetnew
		ADD.L D0,D0
		ADD.L D1,D1
		CMP.L #4,D1
		SGE.B cur_haslpflag(a1)
		...
		MOVE.L sam_lpstart(A1),D2
		MOVE.L D2,cur_lpstart(A1)
		ADD.L D1,D2
		MOVE.L D2,cur_lpend(A1)
		SF cur_lpflag(A1)
		MOVE.L D7,sam_start(A1)
		MOVE.W D7,sam_length(A1)
		MOVE.L D7,sam_lpstart(A1)
		MOVE.W D7,sam_lplength(A1)

.nosetnew	MOVE.L cur_end(A1),A2
		MOVE.L cur_lpend(A1),A3

check		MOVE.L cur_pos(A1),A0		; current voice address
		ADD.L D4,A0			; + "offset to add"
		TST.B cur_lpflag(A1)
		BNE.S check_loop
check_main	CMP.L A2,A0			; past end of sample?
		BLT.S check_done			
		SUB.L A2,A0			; bytes past.
		ADD.L cur_lpstart(A1),A0
check_loop	TST.B cur_haslpflag(a1)
		BEQ.S vcoff
		...
		MOVE.L A0,D0
		MOVE.L A3,D1
		MOVE.L cur_lpstart(A1),A0
		SUB.L A0,D1			; loop length
		DIVU D1,D0
		SWAP D0
		AND.L #$FFFF,D0
		ADD.L D0,A0
check_done	MOVE.L A0,cur_pos(A1)		; store new address
...
vcmain		MOVE.B (A0)+,(A4)
		...
		DBEQ D5,vcmain
		BNE.S vce_done
hit_looppoint	ST cur_lpflag(A1)
		TST.B cur_haslpflag(a1)
		BEQ.S vcoff_cont
cont_loop	SUBQ #1,D5
		BMI.S vce_done
		MOVE.L cur_lpstart(A1),A0
...
DspProg		incbin dspaula1.bin
DspProgEnd
		EVEN
Module		INCBIN "d:\mods\silents1.mod"
