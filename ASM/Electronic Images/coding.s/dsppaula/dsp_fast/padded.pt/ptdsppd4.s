...
; This one pads samples to gain a few extra scan lines!			;
; and is therefore the fast player that can be made.			;
;-----------------------------------------------------------------------;
...
music_int	MOVEM.L	A0-A6/D0-D7,-(A7)
		...
		LEA $ffffa200.w,A6		
		LEA 4(A6),A5
		CLR.L (A5)		; strobe -> signal dsp to continue.
		LEA ch1s(PC),A1
		BSR.S send_voice
		LEA ch2s(PC),A1
		BSR.S send_voice
...
.wait2		BTST #0,2(A6)			; sync 
		BEQ.S .wait2			; 
		MOVE.L (A5),D0			; get no. of bytes to add
...
.no_loop	MOVE.L A2,vcaddr(A1)		; store new ptr.
		MOVE.L (A5),D0
		DIVU #3,D0			; / 3
		ADDQ.W #2,D0			; + 1 (round up)+1 for interpolation
		EXT.L D0
		MOVE.L D0,(A5)			; send a word saying the  no. of samples we're gonna send to the dsp.
		MOVEQ #7,D2
		AND.W D0,D2
		LSR.W #3,D0
		NEG D2
...
Module		INCBIN "d:\mods\bombjazz.mod"
		SECTION BSS
		DS.B 60000
workspc		DS.W 1
