...
.clearmem	MOVE.L D0,(A0)+
		...
		PEA Start(PC)			
		MOVE.L fileptr,-(SP)		;ptr to path + filename
		MOVE.L #$4B0000,-(SP)		;load 'n' execute
		TRAP #1		
		LEA 16(SP),SP
exitme		CLR -(SP)
		TRAP #1

; This is run in supervisor mode...
...
b8sc_finbarr	DC.W 0
b8sc_text	
 dc.b "                          Yo! Hopefully this is definately fixed now... I ran it 10 times and it didn't crash... Off to fix the other one now...                "
 dc.b 0,0
		EVEN
...
player:		move.w (a1)+,d7
		bmi.s .loopit
.contsnd	lsl #3,d7
		movem.l sound_look(pc,d7.w),d6/d7
		movem.l d6/d7,$ffff8800.w
		rte
.loopit		lea sndbuff(pc),a1
		move.w (a1)+,d7
		bra.w .contsnd
sound_look:	incbin awesome.inc\2chnmovl.tab
...
mt_data		INCBIN A:\MODULES\C64.MOD
		EVEN
