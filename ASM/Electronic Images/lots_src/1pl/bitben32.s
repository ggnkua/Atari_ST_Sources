; 32*32 1 plane Bit Bending Routine takes a max of 60% cpu time !!!
; (C) 2nd April 1991 Martin Griffiths (Griff Of The Inner Circle.)
...
.wait_vbl	cmp vbl_timer(pc),d0		; wait for vbl
		beq.s .wait_vbl
		not $ffff8240.w
...
erase_bitbend	move.l log_base(pc),a1
		tst switch
		bne.s .cse1
		move.l old_ptr1(pc),a4		; fetch old ptr (case 1)
		bra.s .cse2
.cse1		move.l old_ptr2(pc),a4		; fetch old ptr (case 2)
.cse2		moveq #0,d0
		moveq #19,d1
.lp		move.l a1,a0
		add (a4),a0
...
i		set 0
		...
		add d0,d0
		add d0,d0
		lea jmptab1(pc),a4		; but we add rout address!!
		add d0,a4
		move.l (a4),i+(320-speed)*16(a1)
		move.l (a4),i-(16*speed)(a1)
		clr d0
		move.b (a2)+,d0
		add d0,d0
		add d0,d0
		lea jmptab2(pc),a4		; byte 2 rout address
		add d0,a4
		move.l (a4),i+4+(320-speed)*16(a1)		
		move.l (a4),i+4-(16*speed)(a1)
		clr d0
		move.b (a2)+,d0
		add d0,d0
		add d0,d0
		lea jmptab3(pc),a4		; byte 3 rout address
		add d0,a4
		move.l (a4),i+8+(320-speed)*16(a1)		
		move.l (a4),i+8-(16*speed)(a1)
		clr d0
		move.b (a2)+,d0
		add d0,d0
		add d0,d0
		lea jmptab4(pc),a4		; byte 4 rout address
		add d0,a4
		move.l (a4),i+12+(320-speed)*16(a1)		
		move.l (a4),i+12-(16*speed)(a1)
i		set i+16
		endr
		add #4*speed,d1			; advance pos within char
		cmp #128,d1
...
buff_ptr	dc.w 0
wave_ptr	dc.w 0
text		dc.b " HELLO AND WELCOME TO A ST BIT BENDER.... SUCKERS!!! ",0
		even
		ds.l 4*speed
buffer		rept 320*8
		dc.l routs
...
.crlp		moveq #70,D1
		...
		dbf d0,.crlp
		rts
;make_wave2	lea verticalbuf1,A0
;		move #2047,D0
;		move #160,D2
;		move #10*160,D1
;.crlp		add D2,d1
...
doOR1		or.b d0,(a0)
		even
font32		incbin 1plane.inc\c_chars.dat
		SECTION BSS
jmptab1		ds.l 256
jmptab2		ds.l 256
jmptab3		ds.l 256
