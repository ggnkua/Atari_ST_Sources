;-------------------------------------------------------------------------
; 2 Bit bending 48*48 scroller for STE only.
; (C) 1991 Griff of Electronic Images (TLT Convention).
; Coded in 3 hours , so this is why it is so shit.
;--------------------------------------------------------------------------
...
calc_dist	
		MOVEM.L sd_dist_tab+2(PC),D0-D7/A0-A3
		MOVEM.L D0-D7/A0-A3,sd_dist_tab
		MOVEM.L sd_dist_tab+2+48(PC),D0-D7/A0-A3
		MOVEM.L D0-D7/A0-A3,sd_dist_tab+48
...
notwang		MOVE D1,sd_dister
		...
		ADD D0,D0
		ADD D0,D0			x4
		MOVE.W D0,sd_dist_tab+94	place in table
...
nowaiting	MOVE.L sd_scr_point(PC),A0	curr text ptr
		...
		LEA bufsize*4(A2),A2		skip 2 buffers(2 pix step)	
		ADDQ #4,D2			add 2(2 pixel step)
		CMP #16,D2			end of chunk?
		BNE.S pos_ok			no then skip
		ADD.W #96,sd_inletswitch
		CMP #96*3,sd_inletswitch
		BNE.S .notnewletter
		CLR sd_inletswitch
		ADDQ.L #1,A0			after 32 pixs,next letter...
.notnewletter	ADDQ.W #2,sd_scrlpos		onto next chunk 
...
i		SET 0
		...
		ROL.L #1,D3
		MOVE.W D3,i+50+(bufsize*2)(A2)
		MOVE.W D3,i-2+(bufsize*2)(A2)
		ROL.L #1,D3
		MOVE.W D3,i+50+(bufsize*3)(A2)
		MOVE.W D3,i-2+(bufsize*3)(A2)
i		SET i+linewid
		ENDR
...
sd_text		dc.b "    ELECTRONIC IMAGES OF     THE INNER CIRCLE  PROUDLY PRESENT A FUCKING NICE STE SCROLLER...        "
		dc.b 0
		EVEN
...
chartable	REPT 33
		...
		DC.L fontbuf+(29*288) ;.
		DC.L fontspce
		DC.L fontbuf+(32*288) ;0
		DC.L fontbuf+(33*288) ;1
		DC.L fontbuf+(34*288) ;2
		DC.L fontbuf+(35*288) ;3
		DC.L fontbuf+(36*288) ;4
		DC.L fontbuf+(37*288) ;5
		DC.L fontbuf+(38*288) ;6
		DC.L fontbuf+(39*288) ;7
		DC.L fontbuf+(40*288) ;8
		DC.L fontbuf+(41*288) ;9
		DC.L fontspce
		DC.L fontspce
		DC.L fontspce
		DC.L fontspce
		...
		DC.L fontbuf+(28*288) ;?
		DC.L fontspce
		DC.L fontbuf+(00*288) ;A
		DC.L fontbuf+(01*288) ;B
		DC.L fontbuf+(02*288) ;etc
		DC.L fontbuf+(03*288)
		DC.L fontbuf+(04*288)
		DC.L fontbuf+(05*288)
		DC.L fontbuf+(06*288)
		...
		DC.L fontbuf+(23*288)
		DC.L fontbuf+(24*288)
		DC.L fontbuf+(25*288) ;Z
fontspce	DS.B 96*3
fontbuf		incbin font48.dat
