; Ste 48*48 Distscroll routine using Blitter in HOG mode.
; this takes about 18% cpu time and only uses 1 scroll buffer.
...
calc_dist	LEA s_dist_tab+2(PC),A0
		MOVEM.L (A0)+,D0-D6/A1-A6
		MOVEM.L D0-D6/A1-A6,-54(A0)
		MOVEM.L (A0)+,D0-D6/A1-A3
		MOVEM.L D0-D6/A1-A3,-42(A0)
		MOVE s_dist_ang(PC),D0
		ADD s_dist_step(PC),D0
		AND #$7FF,D0
		MOVE D0,s_dist_ang
		MOVE s_dister(PC),D1
		ADD s_dist_step(PC),D1
		CMP s_maxi_dist(PC),D1
		BLE.S s_notwang
 		SUB s_maxi_dist(PC),D1
sel_wave	MOVE.L s_wave_ptr(PC),A1
		LEA s_dist_step(PC),A2
		MOVE (A1)+,(A2)+
		BPL.S s_nrapwave 
		LEA s_wave_tab(PC),A1
		MOVE (A1)+,-2(A2)
s_nrapwave	MOVE.L (A1)+,(A2)+
		MOVE.L A1,s_wave_ptr
s_notwang	MOVE D1,s_dister
		LEA s_trig_tab(PC),A1		ptr to sin table
		MOVE (A1,D0),D0			sin(xd)
		MULS s_size(PC),D0		*size (magnitude of wave)
		ADD.L D0,D0
		SWAP D0				normalise
		ADD #48,D0
		ADD D0,D0
		ADD D0,D0			x4
		MOVE.W D0,s_dist_tab+90		place in table
		LEA s_scr_point(PC),A1
		MOVE.L (A1)+,A0			curr text ptr+curr pix buffer ptr
		MOVE (A1),D6			pixel offset
		ADDQ #4,D6			add 4(4 pixel step)
		CMP #16,D6
		BNE.S s_pos_ok			no then skip
		ADDQ.W #8,s_scrlpos		onto next chunk 
		CMP #208,s_scrlpos		in pix buffer.
		BNE.S s_nowrapscr			reached end of buffer?		
		CLR s_scrlpos			if yes reset buffer position
s_nowrapscr	MOVEQ #0,D6			+ pix position
		ADD #384,s_inletswitch		font is 64 wide
		CMP #3*384,s_inletswitch
		BNE.S s_pos_ok 			so there are two chunks
		CLR s_inletswitch
		ADDQ.L #1,A0			after 32 pixs,next letter...
s_pos_ok	TST.B 1(A0)	
		BNE.S s_notwrap			end of text?
		LEA s_text(PC),A0		yes restart text
s_notwrap	MOVE D6,(A1)			otherwise restore
		MOVE.L A0,-(A1)			
		LEA s_scbuffer,A3			reset pixel chunk offset
		MOVEQ #0,D0
		MOVE.B (A0)+,D0
		LEA s_char_tab(PC),A5
		ADD D0,D0
		ADD D0,D0
		MOVE.L (A5,D0),A5
		MOVE s_inletswitch(PC),d0
		ADD D0,A5
		...
		LEA s_char_tab(PC),A6
		ADD D0,D0
		ADD D0,D0
		MOVE.L (A6,D0),A6
norm		ADD s_scrlpos(PC),A3
...
i		SET 0
		REPT no_lines
		MOVEM.L (A5)+,D2-D3 
		MOVEM.L D2-D3,(i*linewid)+200(A3)
		MOVEM.L D2-D3,(i*linewid)-208+200(A3)
i		SET i+1
		ENDR
...
copy_buf	MOVE.L log_base(PC),A2		screen base
		...
		move.w #8,46(a6)
		move.w #-150,48(a6)
		move.w #20,54(a6)     ; no of words to write		
		move.b #2,58(a6)      ; hop
...
i		set 0		
		...
		ADD D0,A1
		SWAP D0
		MOVE.L A1,(a6)           ; source address
		MOVE.L A2,50-36(a6)      ; dest address
		MOVE.W D1,56-36(A6)      ; plot 4 lines
		MOVE.W D0,60-36(a6)	
		LEA 168(A2),A2
i		set i+1
		endr
...
s_littleadd	DC.W 0
s_scr_point	DC.L s_text
s_scx		DC.W 0
s_scrlpos	DC.W 0
s_inletswitch	DC.W 0
s_text		dc.b "  WOW!!  COOOO... THIS DON'T HALF LOOK GOOD... BET YOU CAN'T READ IT THOUGH!        "
		dc.b 0
		EVEN
log_base	DC.L $F0000
		DC.L $F8000
...
s_dist_ang	DC.W 0
s_dist_step	DC.W 8
s_maxi_dist	DC.W 1024
s_size		DC.W 0
s_dister	DC.W 0
s_wave_ptr	DC.L s_wave_tab
s_wave_tab	DC.W 8,1024,40
		DC.W 32,1024*8,40
		DC.W 128,4096,1
		DC.W 64,4096,4
		DC.W 8,1024,16
...
s_dist_tab	DCB.W no_lines+1,48
		
s_trig_tab	dc.w	$0000,$FF37,$FE6E,$FDA5,$FCDC,$FC13,$FB4A,$FA82 
