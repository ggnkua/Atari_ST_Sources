;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
;  Conversion of the TRANSFORMER Demo from Kefrens MegaDemo IV(Amiga)   ;
;  All coding, design etc by Martin Griffiths...                        ;
;-----------------------------------------------------------------------;
;-----------------------------------------------------------------------;
...
Rotate	LEA tweens,A5
		...
		BSR Show_object
		BSR sw_clr
		add #framesize,frameno
		cmp #framesize*(no_frames-1),frameno
		bne.s a
		clr frameno
...
; Routine for calcing tween frames...

Create_frames
		LEA Grid(PC),A0
		LEA Grid1(PC),A1
		LEA tweens,A2
		LEA tweengrads(PC),A3
		LEA tweenxyslongs(PC),A4
		MOVEM.L (A0)+,D0-D4
		MOVEM.L D0-D4,(A2)
		LEA 20(A1),A1
		LEA 20(A2),A2
		MOVEQ #48,D7
calcgrads	MOVEm.W (A0),D0-d2
		ext.l d0
		ext.l d1
		ext.l d2
		MOVE.W D0,(A2)+
		...
		MOVEQ #no_frames-1,D6
frame_lp	LEA tweenxyslongs(PC),A0
		LEA tweengrads(PC),A3
		MOVEQ #48,D7
		CLR D3
coordlp		MOVEM.L (A0),D0-D2
		ADD.L (A3)+,D0
		ADD.L (A3)+,D1
		ADD.L (A3)+,D2
		MOVEM.L D0-D2,(A0)
...
; Really accurate word int/word frac calculator
gradient	macro
		DIVU D0,D1
		SWAP D0
		MOVE D1,D0
...
tweengrads	DS.L 49*3
tweenxyslongs	DS.L 49*3

Grid		DC.W 0,0,0
		DC.W 49
		DC.L 0,0,$1B00000
Gridcords	DC.W -600,600,0
		DC.W -400,600,0
		DC.W -200,600,0
		DC.W 0,600,0
		DC.W 200,600,0
