...
/*
	...
	*/
	...
	for (i=0; i<n; i++)
		...
		{
                	draw_poly2(np,obj,&perspected_crds[0][0],&normals[0][0]);
		}
		obj += np;
        }
}
		...
		{ 	int x;
			...
			{	short z = z1>>16;
				...
				{	int nd;
					...
					m2 = m1/nd;
					n2 = n1/nd;
					light_model(l2,m2,n2,
		*(screen_ptr+(x*(MAX_X>>2))+(in[MXMP] & 0x0000ffff)) );
				}
...
{ 	int n6=n*6,i,e_dy,dy,dy2;
	...
	{ 	int x1 = in[xc1]>>16;
		...
		int xdx = (x2-x1)+1;
		if (xdx>0)
		{ 	 gline(x1,  in,  xdx); gline2();
#pragma 	    aux gline modify [eax ebx ecx edx esi edi] 	\
			 	parm [esi][ebx][ecx] = 		\
			"	mov	ax,WORD ptr 62[ebx]	"\
			"	movsx   edi,WORD ptr 60[ebx]	"\
			"	lea	esi,[esi*4+esi]		"\
			"	add	edi,[screen_ptr]	"\
			"	sal	esi,4			"\
			"	mov	dx,03c4h		"\
...
{ 	ClearDisplay();
	...
	{	case 0: Draw_Poly_Object(Sphere,rx,ry,rz,ox,oy,oz);
		...
		case 3: Draw_Poly_Object(Station,rx,ry,rz,ox,oy,oz);
	    		Draw_Poly_Object(Cube,rx*2,ry*2,rz*2,ox,oy,640);
			break;
