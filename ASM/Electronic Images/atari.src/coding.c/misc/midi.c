...
void Quit(void);
void Help(void);
void Save(struct midi_event *);
struct midi_event *Load(void);
void Edit(void);
void Output(void);
void Input(void);
void initalise(void);
...
void main(void)
{
 ...
 do {
	...
	opt = getch();
	switch (opt) {
		case '1' : clear(track_data); track_data=record();
			break;
		case '2' : play(track_data);
			break;
		/*	case '3' : Edit();
			break;*/
		case '4' : clear(track_data);track_data=Load();
			break;
		case '5' : Save(track_data);
			break;
		case '6' : Help();
			break;
		case 'Q' : 	_setcursortype(_NORMALCURSOR); exit(0);
			break; }
} while (opt != '7');
}
...
struct midi_event *record()	/* record a track, stop on any keypress */
{
	...
	sendcmd(STARTREC);
	printf("\nPress any key to record, any key to stop");
	getch();
...
char *filename;
	...
	while(size--) {
		...
		fread(&event,sizeof(struct file_midi_event),1,fp);
		nextnode = store(nextnode,event.data_cnt,event.data[0],event.data[1],event.data[2],event.data[3]);
	}
	return firstnode;
}

void Save(struct midi_event *data)
{
	FILE *fp;
	int x,y;
	char *filename;
	int size;
	struct midi_event *node;
	struct file_midi_event event;
	...
	node=data;
	while(node !=NULL) {
		event.data_cnt=node->data_cnt;
		event.data[0]=node->data[0];
		event.data[1]=node->data[1];
		event.data[2]=node->data[2];
		event.data[3]=node->data[3];
		fwrite(&event,sizeof(struct file_midi_event),1,fp);
		node=node->next;
	}
