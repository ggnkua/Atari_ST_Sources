...
#pragma aux rend_gourpolyz parm [eax] [ebx] modify [eax ebx ecx edx edi esi];
	rend_gourpolyz(min_y,max_y);
...
{ 	ClearDisplay();
	SetLightSource(lxa,lya,lza);
   	Draw_Poly_Object(Cube,rx,ry*2,rz*2,ox,oy,600);
	switch (shape)
