...
#pragma aux interpolate parm [ebx] [eax] [ecx] [edi] = \
"	sal	ebx,16			" \
"	cmp	ecx,0			" \
"	jle	noxl			" \
"	sal	eax,16			" \
...
{ 	int n6=n*6,i,e_dy,dy,dy2;
	...
	{	left -= 6;
		...
		interpolate(left[7],left[1],dy2,&xc2[y1]);
		interpolate(left[8],left[2],dy2,&zc2[y1]);
		interpolate(left[9],left[3],dy2,&ic2[y1]);
		e_dy -= dy2;
	}	
	...
	{	y1 = right[0]; dy2 = right[6]-y1;
		interpolate(right[1],right[7],dy2,&xc1[y1]);
		interpolate(right[2],right[8],dy2,&zc1[y1]);
		interpolate(right[3],right[9],dy2,&ic1[y1]);
	    	right +=6;
		e_dy -= dy2;
	}
	...
	char *sc_ptr = screen_ptr + (min_y >>2);
   	unsigned short mask = (256 << (min_y & 3)) +2;
	for (i = min_y ; i <= max_y ; i++)
	{ 	int x1 = xc1[i]>>16;
	    	int i1 = ic1[i];
	    	int z1 = zc1[i];
		int xdx = ((xc2[i]>>16)-x1)+1;
		if (xdx>0)
		{ 	int dZ    = ((zc2[i] - z1)/ xdx);
		    	short daI = ((ic2[i] - i1)/ xdx)>>8;
#pragma aux gline parm [esi] [ebx] [ecx] =		 	\
			"	mov	edi,[sc_ptr]		" \
			"	lea	esi,[esi*4+esi]		" \
			"	sal	esi,4			" \
			"	add	edi,esi			" \
...
{	int L1[3]  = { 0,0,-LSCALE };
	...
	V[2] = L[2] = L2[2];
	switch (shape)
	{	case 1: Draw_Poly_Object(Station,wibble,wibble,900, 0,0,oz);
	    		Draw_Poly_Object(Cube,wibble>>1,0,wibble, 0,0,oz);
			break;
