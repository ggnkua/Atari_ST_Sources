...
{	char dsp_buf[14000];
	...
	while (Dsp_Lock() != 0) 
	{	int res=xalert(	1,1,X_ICN_ALERT,NULL,SYS_MODAL,BUTTONS_CENTERED,TRUE,title,
	...
	devconnect(DMAPLAY,DAC,CLK_25M,CLK_33K,NO_SHAKE);
	devconnect(DSPXMIT,EXTOUT,CLK_25M,CLK_33K,NO_SHAKE);
	devconnect(EXTINP,EXTOUT,CLK_25M,CLK_33K,NO_SHAKE);
	devconnect(ADC,EXTOUT,CLK_25M,CLK_33K,NO_SHAKE);	
...
void DSP_DeInit(void)
{
	...
	devconnect(DSPXMIT,EXTOUT,CLK_25M,CLK_33K,NO_SHAKE);
	devconnect(EXTINP,DAC,CLK_25M,CLK_33K,NO_SHAKE);
	dsptristate(0,0);		/* connect dsp to sound matrix */
	Dsp_Unlock();
}
