...
Note_Off:	MOVEQ	#0,D3
		get_next_byte	D3
donoteoff:	bfclr	channel_noteflags(a3,d1.l){d2:1}
		MOVE.W	Mtrk_curr_chan(A6),D1
		LEA	ch1s,A5
		MOVEQ	#no_voices-1,D5
.lp		cmp.w	cur_period(a5),d2
		bne.s	.nah
		cmp.b	assoc_chan(a5),d1
		beq.s	yup
.nah		LEA	sam_vcsize(a5),a5
		DBF	D5,.lp
		rts
yup:		SF.B	sustain_flag(a5)
...
.nah		ADDQ	#1,D4
		...
		bftst	channel_noteflags(a3,d1.l){d2:1}
		Beq	.noteon
		TST.W	D3
		BEQ	donoteoff	; zero vol?
.noteon		bfset	channel_noteflags(a3,d1.l){d2:1}
...
.start_sound	bfclr	(voiceused_bitfield){d4:1}
		...
		MOVEQ	#0,D6
		MOVE.B	channel_inst(a3,D1.l),D6
		...
		ADD.L	D2,D4
		MOVE.L	(octave_table,d4.l*4),D6
		MOVE.L	A2,A1
.find_lp:	CMP.L	high_freq(A1),D6
		BHI.S	.notquite
		CMP.L	low_freq(A1),D6
		BLO.S	.notquite
		MOVE.L	A1,A2
...
.done:		MOVE.L	D6,cur_foundper(a5)
		...
		moveq	#0,d4
		move.w	sample_rate(a2),d4
		swap	d4
		divu.l	#freq,d4
		lsl.l	#8,d4
		move.l	d4,sample_rate_ratio(a5)
		CLR.W	cur_env_offset(a5)
		btst.b	#sustain__flag,modes(a2)
		sne.b	sustain_flag(a5)
.justvol	muls	channel_volume(a3,d1.l),d3
		asr.l	#7,d3
		MOVE	D3,note_vol(a5)
.doneit:	RTS
...
Control_Change:	MOVEQ	#0,D3
		...
		CMP.B	#7,D2
		BNE.S	.notvol
		MOVE.W	D3,channel_volume(A3,D1.L)	
.notvol:	RTS
...
Program_Change:	
		MOVE.B	D2,channel_inst(A3,D1.L); store patch no. in channel info.
		RTS
...
PitchBend:	get_next_byte	d0
		...
		LSL.W	#7,D0
		OR.W	D0,D2
		MOVE.W	D2,channel_bend(A3,D1.L)	
		RTS
...
.wait8:		BTST.B #1,-4(A6)
		...
		moveq	#0,d6
		moveq	#0,d7
.find_lp1:	bftst	(voiceused_bitfield){d7:1}
		bne.s	.next
