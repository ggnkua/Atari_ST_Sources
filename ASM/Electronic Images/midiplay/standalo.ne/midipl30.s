...
.start_sound	bfclr	(voiceused_bitfield){d4:1}
		...
		clr	d4
.nxt		cmp.w	max_midi_note(a2),d2
		ble.s	.ok
		move.w	max_midi_note(a2),d4
		MOVE.L	wave_size(a2),d0	; sample length
		LEA.L	(padding_size+96,A2,D0.L),A2
		bra.s	.nxt

.ok		sub.w	d4,d2
		mulu	#1024*4,d2
		MOVE.L	(octave_table,d2.l),D6
		lsl.l	#8,d6
		lsl.l	#2,d6
		MOVE.L	D6,cur_intfrac(a5)
