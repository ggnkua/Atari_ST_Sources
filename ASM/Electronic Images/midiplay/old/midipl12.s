...
TIMER_A_VAL	EQU	124
TICKS_PER_INT	EQU	(1000000*100)/(2457600/TIMER_A_VAL)
...
.notrig		
		...
		bgt.s	.not_sussing
		moveq	#3,d2
.not_sussing	SF.B	trig_note_off(A5)
.ok
.nooff
		MOVE.B	env_rate(A2,d2),d1
...
tabby		DC.W	512*13,64*13,8*13,1*13
...
Note_On		MOVEQ	#0,D3
		...
		LEA	ch1s(pc),a4
		move	#(no_channels-1)*sam_vcsize,d4
.find_lp1	tst.b	voice_used(a4,d4)
		beq.s	.next
		cmp.b	assoc_inst_no(a4,d4),d6
		bne.s	 .next
		CMP.W	sam_period(A4,D4),d2
		BEQ.S	.justvol
.next		SUB	#sam_vcsize,D4
		...
		move.w	d5,d4
		addq	#1,d5
		and.w	#no_channels-1,d5
		move.w	d5,curr_voice
		mulu	#sam_vcsize,d4
.okfoundvoice	add.w	d4,a4
		move.b	d6,assoc_inst_no(a4)
		move.b	d1,assoc_chan_no(a4)
		st.b	trig_note_flag(a4)
		st.b	voice_used(a4)
		MOVE.W 	D2,sam_period(A4)
		MOVE.W	D3,sam_vol(A4)		; store volume
.doneit		RTS
...
.sync2		BTST.B #0,$FFFFA202.W
		...
		MOVE.B #6,$FFFFFA19.W		; timer a event mode.
		MOVE.B #TIMER_A_VAL,$FFFFFA1F.W
		RTS
...
music_data	
		...
		;INCBIN D:\MUSIC.MID\canzonii.MID
		;INCBIN D:\MUSIC.MID\JARRE3.MID
