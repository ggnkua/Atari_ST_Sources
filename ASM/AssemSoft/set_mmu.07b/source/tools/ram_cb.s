...
* 	Program: RAM_CB								*
* 	Version: 0.2								*
*	Purpose: Translation Tree cache-mode = Copy Back  setter		*
*	Author:  Odd Skancke - AssemSoft productions.				*
*	Status:  FreeWare - Only to be distributed with MiNT kernel releases.	*
*										*
* RAM_CB is a small tool that sets cache mode to Copy Back in page descriptors	*
* from logical address 0x00000000 up to the 1st invalid page descriptor or	*
* 1 GB RAM.									*
*										*
...
start_up:	move.l	4(sp),a5	;Address of basepage in A5
		...
		moveq	#0,d0
		move.l	#1000*1024*1024,d2
		moveq	#1,d1
		move.l	#(c_copyback<<d_cache_pos),pd_flags
