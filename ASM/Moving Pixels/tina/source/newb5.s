...
.waitblit:
	...
	move.w	#fw/2+1,x_count(a6)
	move.w	#fh/2,y_count(a6)
	move.w	#fw*2,src_yinc(a6)
	move.w	#(sw*2)-(fw),dst_yinc(a6)
...
setupsound:
	move.l	#slength,-(sp)	;thats 1sec of sound
	move.w	#$48,-(sp)
	trap	#1
	addq.l	#6,sp
	tst.l	d0
...
startsound:
	...
	move.w	#%0000000000000011,-(sp)	;play repeat
	move.w	#$88,-(sp)
	trap	#14
	addq.l	#4,sp
	move.w	#190,soundcounter
	bsr	getswapsound
...
my114:
	move.w	d0,-(sp)
	subq.w	#1,soundcounter
	bpl	tstsync
	move.w	#199,soundcounter	;.5 second sound counter
	move.w	#1,soundflag
tstsync:
	move.b	c25,d0
	ror.b	#1,d0
