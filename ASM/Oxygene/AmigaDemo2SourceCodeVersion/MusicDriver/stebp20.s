...
BS_INIT:
	...
	move.w	#(dataInitEnd-dataInitStart)/2-1,d0
	lea	dataInitStart(pc),a0
	lea	dataBackup(pc),a1
.cp1:	move.w	(a0)+,(a1)+
		dbf	d0,.cp1
...
.ok:
	move.w	#(dataInitEnd-dataInitStart)/2-1,d0
	lea	dataInitStart(pc),a1
	lea	dataBackup(pc),a0
.cp2:	move.w	(a0)+,(a1)+
		dbf	d0,.cp2
...
BS_int3:addq.l	#$4,d0
	...
	add.l	d1,d0
	move.l	pSong(pc),d1
	add.l	d1,d0			; Adresse du gros buffer de sample
	...
	moveq	#15-1,d1		; Init 15 samples
;	adda.l	#$00000020,a1
	lea	32(a1),a1

BS_int4:	move.l	d0,(a0)+	; adresse du sample

	cmpi.b	#-$01,(a1)		; utilis‚ ?
	beq.w	BS_int5			; non on continue
	move.w	$0018(a1),d2		; oui: taille du sample en mot
	mulu	#$0002,d2		; puis en samples
	add.l	d2,d0			; prochaine adresse
...
BS_MX0:	move.b	#$01,$001a(a2)
	...
	lsl.l	#$6,d3
	adda.l	d3,a4
	move.l	a4,(a1)			; sample adress
	move.l	a4,$0004(a2)
	move.w	$02(a3,d7.w),$0004(a1)	; replen
	move.w	$02(a3,d7.w),$0008(a2)	; volume
	tst.b	$04(a3,d7.w)
	beq.w	BS_MX2
	move.l	BS_D14,a4
	clr.l	d3
...
BS_MX6:	lea	BS_D16,a4
	ext.w	d4
	asl.w	#$1,d4
	move.w	-$02(a4,d4.w),$0006(a1)		; period
	rts
...
dataBackup:	ds.b	$222		; (dataInitEnd - dataInitStart)
