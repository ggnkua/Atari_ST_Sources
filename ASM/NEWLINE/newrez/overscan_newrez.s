;THIS HACk MIGHT DAMAGE YOUR MONITOR
;----------------------------------------------------;
;                                                    ;
...
mfill          = $34
clip           = $36
UserDevInit    = $7a ;.l  122  ; ptr to user routine before dev_init
UserEscInit    = $7e ;.l  126  ; ptr to user routine before esc_init
...
vid_pal3       = $ff8246
vid_palF       = $ff825e
add_len        = $0 ;screenmem is always the same

               TEXT 
;                    include "../_inc_tin/bugaboo.s"
...
;#
;#############################################################
ScreenTab:     DC.w 320,1280,1280             ;   X-Auflîsung Low,Mid,High
               DC.w 400,200,200             ;   Y-Auflîsung
               DC.w 80,160,160             ;   Bytes pro Zeile

               DC.l 32000-80*400,32000-160*200,32000-160*200     ;   VideoAdd  <> Memtop Offset
...
;left or right shift
 ...
 REM
               ...
               bra       DoTitle
DoIntro:
               bsr       Intro               ; GrowBox und Sound ausgeben
 EREM
...
not_okk:       
               ...
               pea       msg_title(pc)       ; Titelzeile ausgeben
               move.w    #Cconws,-(sp)
               trap      #gemdos
               addq.l    #6,sp
               move.w    inst_ok,d0
...
main_3: 
               cmpi.w    #3,d0
               bne       main_4
               pea       msg_already(pc)     ; 'Schon installiert' Meldung
               move.w    #Cconws,-(sp)       ; ausgeben
               trap      #gemdos
               addq.l    #6,sp               ;  und
               bra       wait_key            ; einfach beenden
main_4:               
               pea       msg_skip(pc)     ; 'Schon installiert' Meldung
               move.w    #Cconws,-(sp)       ; ausgeben
               trap      #gemdos
               addq.l    #6,sp               ;  und
;-----------------------------
...
;unusable
               ...
               bne.s     .nolow
               ;set 160x400x16 mode
               move.b    #2,$ffff8260.w      ;2/0 mode
               move.b    #0,$ffff8261.w
.nolow:
 EREM
;set rez according to control key status at start
     move.w    specialkeys,D0
     and.b     #%100,D0
     cmp.b     #0,D0
     bne.s     .nolow
     ;set 320x400x4 mode
     move.w    #0,Reso
     move.b    #2,$ffff8260.w      ;2/1 mode
     move.b    #1,$ffff8261.w
     move.l    #$ffff000F,$ffff8240.w
     move.l    #$00f90000,$ffff8244.w
.nolow:
     cmp.b     #%100,D0
     bne.s     .nohi
     ;set 1280x400x4 mode
     move.w    #1,Reso
     move.b    #0,$ffff8260.w      ;0/2 mode
     move.b    #2,$ffff8261.w
.nohi:
     moveq     #0,D0
 REM
;not needed, we activated our own screenmode, thanks
               clr.w     d0
...
;for some reason we can't rely on that we're in supervisor mode when PatchIt is called?
...
;----------------------------------------------------------
PatchIt:     
;do Setoverscan_Super in supervisor
 move.w    #-1,-(sp)              
 trap      #gemdos
 addq      #2,SP

 REM
               bsr       SetOverscan         ; LineA Werte nochmal eintragen
...
NoTerm:
               cmpi.w    #-1,(a0)        ; Der Overscan-Modus wird
               bne       NoSuperInit     ; Programme den Bildschirm umsetzen
               movem.l   d0-d7/a0-a6,-(sp)
               bsr       SetOverscan_super_reso
               bsr       SetOverscan         ;
;               bsr       ClearScreen         ;
               movem.l   (sp)+,d0-d7/a0-a6
               rte
NoSuperInit:
               movea.l   OldDos(pc),a0
               jmp       (a0)
;################################################ XBIOS-Trap
               DC.b "XBRAOVER"
OldXbios:      DC.l 0
...
;----------------------------  Videoaddresse auf 'memtop + D0'
               ...
               beq       no_buff_1a           ;     hinter Sicherheits-
...
;----------------------------  v_bas_ad auf 'memtop + D1' setzen
               move.l    MyMemtop,v_bas_add
               tst.b     BadTos              ; Bei BlitterTOS
               beq       no_buff_2a           ;     hinter Sicherheitspuffer
...
;for some reason there are chances were not in supervisor mode?
SetOverscan_reso:   
;               clr.l     d2
;               move.b    sshiftmd,d2         ; Aktuelle Aufîsung
;               move.w    d2,Reso             ;    merken
SetOverscan:   clr.l     d2
               move.w    Reso,D2             ;    merken
               add.w     d2,d2               ; mal 2 als Word-Offset in Tabelle
               lea.l     ScreenTab(pc),a0    ; TabellenZeiger holen
               move.w    0(a0,d2.w),RezX     ; X-Auflîsung
               move.w    6(a0,d2.w),RezY     ; Y-Auflîsung
...
dolinea:
 ...
 cmp.w    #0,D0
 bne.s    .nolow
 move.w #2,v_planes(a0)
.nolow 
 move.w   Reso,D0
 cmp.w    #1,D0
 bne.s    .nohi
 move.w #1,v_planes(a0)
.nohi 
...
Copy_1:        move.l    (a1)+,(a0)+         ; Alten Schirm kopieren
               dbf       d3,Copy_1           ;
               move.w    d2,d3               ;
               blt.s     noendclear          ; can be negative beacuse, reasons
Copy_2:        clr.b     (a0)+               ;
...
msg_title:     DC.b 13,10
               DC.b 'Ù Overscan NewRez v1.6n   K.Isakovic Ù',13,10
               DC.b '|                         B.Gebauer  |',13,10
               DC.b '|-little patch by tIn/newline.-------|',13,10
               DC.b "| This version doesn't do overscan   |",13,10
               DC.b "| instead it enables two of the 3 new|",13,10
               DC.b '| ST rez czietz discovered, see      |',13,10
               DC.b 'atari-forum.com/viewtopic.php?t=34108|',13,10
               DC.b '|------------------------------------|',13,10
               DC.b '|   WARNING WARNING WARNING WARNING  |',13,10
               DC.b '| Do not use a CRT TV or other non-  |',13,10
               DC.b '| multisync device, damage possible! |',13,10
               DC.b '|   WARNING WARNING WARNING WARNING  |',13,10
               DC.b '| This only works from AUTO folder.  |',13,10
               DC.b '| Boot in low rez and you should get |',13,10
               DC.b '| a 160x400x16 72Hz resolution on    |',13,10
               DC.b '| RGB Pins.                          |',13,10
               DC.b '| Boot with control key pressed and  |',13,10
               DC.b "| you'll get a 1280x200x2 50Hz rez   |",13,10
               DC.b '| on mono pin                        |',13,10
               DC.b '| Best used wit a vga adapter with   |',13,10
               DC.b '| mono/rgb switch like the ubeswitch |',13,10
               DC.b '| or centuriontech.eu ST2VGA         |',13,10
               DC.b '|------------------------------------|',13,10,0
msg_installed: DC.b '| installed.                         |',13,10
               DC.b '|                                    |',13,10
               DC.b 'ı Hold Shift-Key for Help            ı',13,10,0
msg_notactive: DC.b 'ı NOT aktivated.                     ı',13,10,0
msg_noTos:     DC.b '| NOT installed.                     |',13,10,7,7
               DC.b 'ı Only with TOS 1.2/1.4 or BETA-TOS  ı',13,10,0
msg_already:   DC.b 'ı already installed...               ı',13,10,7,7,0
msg_skip:      DC.b 'ı skipped by user...                 ı',13,10,7,7,0
...
msg_help:      DC.b 27,'H',27,'p'
               ...
               DC.b '|-little patch by tIn/newline.-------|',13,10
               DC.b "| This version doesn't do overscan   |",13,10
               DC.b "| instead it enables the new ST rez  |",13,10
               DC.b '| czietz discovered, see             |',13,10
               DC.b 'atari-forum.com/viewtopic.php?t=34108|',13,10
               DC.b '|------------------------------------|',13,10
               DC.b '|   WARNING WARNING WARNING WARNING  |',13,10
               DC.b '| Do not use a CRT TV or other non-  |',13,10
               ...
               DC.b '| a 160x400x16 72Hz resolution on    |',13,10
               DC.b '| RGB Pins.                          |',13,10
               DC.b '| Boot in hi rez and you should get  |',13,10
               DC.b '| a 1280x200x2 50Hz resolution on    |',13,10
               DC.b '| mono pin                           |',13,10
               DC.b '| Best used wit a vga adapter with   |',13,10
               DC.b '| mono/rgb switch like the ubeswitch |',13,10
               DC.b '| or centuriontech.eu ST2VGA         |',13,10
               DC.b '|------------------------------------|',13,10
               DC.b 'ı   Q     : Quit                     ı',13,10,0
...
save_name:     DC.b 'A:\AUTO\NEWREZ.PRG',0
