...
;
;
; scroller 320x64 but only with car of 8x8
; (use set of register)
; sc39.s
; code by gloky/mjj
; 
...
blip
	...
	jsr affichebuffscrolldemiCARA8
	...
	jsr affichebuffscrolldemiCARA4
...
;spleenauto2	equ *+2
	...
	lea .retour,a6
	; 0,ff,ff00,ffff    0,3,12,15
	; dc.w 0,1,3,6,7,8,9,12,14,15
	; dc.w 0,1,6,7,8,9,14,15	


	movem.w (a1)+,d0/d2/a2/a4
	jmp (a5)
.retour
...
*spleenauto1 equ *+2
	...
	lea .retour,a6
	movem.w (a1)+,d0/d1/d3/d4/d5/d6/a3/a4
...
.retour
	...
	rept lnty-1
	movem.w (a1)+,d1/d3/d4/d5/d6/a3/a4
	lea 160(a0),a0
	lea 10+4(a6),a6
	jmp (a5)
	endr
...
tablebinvalueo1	dc.w 0,3,12,15
regvalueoptim1	dc.w 0,$FF,$FF00,$FFFF
tablebinvalueo2	dc.w 0,1,6,7,8,9,14,15	
regvalueoptim2	dc.w 0,$000F,$0FF0,$0FFF,$F000,$F00F,$FFF0,$FFFF
...
andd1 macro
	...
	endm
putanypatern4
; d0.b = valeur repete 2 fois en w
; a1 = buffer 4 word destination
	lea regvalueoptim1+2,a0
	;move.w (a0)+,(a1)+
	andd0
	andd0
	andd0
...
starttext
		dc.b ' MAKE LOVE NOT WAR !!!!!!! il fait beau aujourd''hui, et je suis fatigu'
		DC.B ' bientot midi on va manger des frites mais je reve un peu '
		dc.b '         '
...
bufferscroll
		...
		ds.w 1		; rts ? dbf ?
		ds.l lny*lnx*2  cas align 16 +8 ;instruction move.w dn,dep(a0)
		ds.w 1
		ds.l lny*lnx*2 ; cas align 16    ;a chaque long correspond une
		ds.w 1		; rts ? dbf ?
