...
*  Module name  : Demonstration program
*  Symbol prefix: -
*
...
*  1995:
*     Oct 31: some messages and comments were changed
*     Nov 09: the "reserved" strings in <skeys> were changed (the codes are now
*              used for the additional keys on the Macintosh keyboard)
*  1998:
*     Jul 08: new function: print character class flags
*             function header facelifting (modern argument list style)
*     Aug 21: function for character class flags removed
*             (flag overview added to helptext)
*
*******************************************************************************/
/*KEY _NAME="NKCC demo program" */
/*END*/
...
static int disable_opt(unsigned long *sflags);
   ...
   /* handle command: classify character(s) */
static int classify(unsigned long *sflags);
...
static unsigned long enter_option(
...
/***************************************************************************
*
*  classify: handle command: classify character(s)
*
*  Out:
*        exit status:
*        TRUE        leave command mode
...
unsigned long *sflags)              /* ^ to special key flags (NKS_...) */
{
   ...
   int      key,                    /* key code */
            i,j;
   unsigned long flags;             /* class flags (NKC_...) */


   UNUSED(sflags);

   printf("C\n\nWhich character (Esc=all): ");
   key = nkc_conin();               /* get key code */

   if (key & NKF_FUNC)              /* Esc or any other functional key? */
      {
      i = 0;                        /* show all */
      j = 255;
      printf("all\n");
      }
   else                             /* otherwise (one character only): */
      {
      i = j = key & 0xff;           /* mask off the flag byte */
      printf("%c\n",(unsigned char)i);
      }

   for (; i <= j; i++)              /* character loop: */
      {                             /* get classification */
      flags = nkc_class((unsigned char)i);

      printf("$%02x %c = %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
             i,
             (i < 32) ? '.' : (unsigned char)i,
             (flags & NKC_ALPHA)  ? 'A' : ' ',
             (flags & NKC_ALNUM)  ? 'N' : ' ',
             (flags & NKC_UPPER)  ? 'U' : ' ',
             (flags & NKC_LOWER)  ? 'L' : ' ',
...
unsigned long *sflags)              /* ^ to special key flags (NKS_...) */
{
   ...
   printf("H\n\n"                   /* just show text */
          "Commands:                           Character classes:\n"
          "   Esc   leave command mode            A      alpha character\n"
          "   +x    enable option x               N      alphanumeric character\n"
          "   -x    disable option x              U      upper case character\n"
          "   C     classify character(s)         L      lower case character\n"
          "   S     show option status            D      decimal digit\n"
          "   Q     quit program                  O      octal digit\n"
          "   H     show this help text!          $      hexadecimal digit\n"
          "                                       %%      binary digit\n"
          "Options:                               S      space\n"
          "   A     direct ASCII input            -      dash\n"
          "   C     Control key emulation         !      punctuation mark\n"
          "   D     ALL deadkeys                  I      ASCII character\n"
          "   ^     circumflex deadkey            C      control character\n"
          "   ~     tilde deadkey                 G      graphics character\n"
          "   \'     accent agui deadkey           P      printable character\n"
          "   `     accent grave deadkey\n"
          "   ¹     umlaut deadkey\n"
          "   \"     another umlaut deadkey\n"
          "   ø     scandinavian deadkey\n"
          "   ,     cedil deadkey\n"
          "   /     slash deadkey\n"
          "\n"
          );
...
unsigned long *sflags)              /* ^ to special key flags (NKS_...) */
{
   ...
   static struct                    /* table of supported key codes and */
      ...
      } keytab[] =
      {
         ...
         {F_IGNORE | 'C',classify},          /* classify character(s) */
...
/*END*/
   ...
   a1 = nkc_init(NKI_NO200HZ,0,NULL);  /* init NKCC */
                                    ...
                                    /* show header */
   printf("NKCC demonstration program   (c) 1989-1998 Harald Siegmund\n"
