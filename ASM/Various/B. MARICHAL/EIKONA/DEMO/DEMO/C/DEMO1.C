/****************************************************************************/
/* DEMO1.C :  Exemple de r‚cup‚ration de fichier ressource cr‚‚ avec EIKONA */
...
char	alerte[] = "[0][\10 Programme de d‚monstration  |"
							 ...
							 "ressources cr‚‚s avec EIKONA|"
							 "\10 Vous  pouvez  d‚placer  et|"
							 "s‚lectionner les ic“nes...][ DEMO1.C |   O K   ]"; 
...
void main( void )
{
  ...
  graf_mouse( 0, 0L );
	...
	rsrc_load( "icones.rsc" );				/* Chargement des ic“nes */
	rsrc_gaddr( 0, 0, &newdesk );			/* R‚cup‚ration de l'adresse de l'arbre */
	
	wind_get( 0, WF_WORKXYWH, w0, w0+1, w0+2, w0+3 );	
	w0[0] -= 1; w0[1] -= 1;						/* Calcul et ajustement des dimensions du bureau */
	w0[2] += 2; w0[3] += 2;
	
	newdesk[0].ob_spec.index = 0x00fe11c3L; /* Transformer le cadre en rectangle gris */
	newdesk[0].ob_x = w0[0];								/* Ajuster ses dimensions */
	newdesk[0].ob_y = w0[1];
