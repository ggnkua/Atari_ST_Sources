...
.okrelo		
		...
		****
		...
		; les icones couleur !
		; une sous-routine n'est pas du luxe...
...
.yopicons
		...
		; r‚glage largeur et hauteur du pix
		
		moveq	#0,d0
		
		move.w	ib_wicon(a3),d0
		move.l	d0,([_rsl_pix,a6],pix_w)
		
		move.w	ib_hicon(a3),d0
		move.l	d0,([_rsl_pix,a6],pix_h)
...
.joli
		...
		move.w	d0,cic_planes(a3)
		
		; cas de transformation de bitmap couleur
		; (les masques restent inchang‚s)
		
		move.l	cic_pdata(a0),([_rsl_pix,a6],pix_addr)	; adresse bitmap source
		move.w	#1,([_rsl_pix,a6],pix_type)		; format vdi
		move.w	_rsl_bpp(a6),([_rsl_pix,a6],pix_bpp)	; bits/pixel source
		...
		****
		...
		move.l	cic_psdata(a0),([_rsl_pix,a6],pix_addr)	; adresse bitmap source
		move.w	#1,([_rsl_pix,a6],pix_type)		; format vdi
		move.w	_rsl_bpp(a6),([_rsl_pix,a6],pix_bpp)	; bits/pixel source
...
**************************************************************************************
*** Routine de relocation des adresse d'un fichier ressource charge depuis le disk ***
**************************************************************************************
newrsc_reloc
