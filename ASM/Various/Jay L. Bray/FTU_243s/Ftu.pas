...
{  Procedure Poke ( A:Long_Integer; B:Byte ); External;
  ...
  Begin
    ...
    Maxtime := ( Get_Timer + 2200 );  I := TimeOut;
    Repeat
      Repeat
        Seconds := ( MaxTime-Get_Timer ) Div 200;
        Show_Secs_Left;  Show_Cur_Time;  Show_Connected;  I := Scan_Input;
      Until ( I > 0 ) Or ( No_Carrier ) Or ( Get_Timer > Maxtime );
      Ch := Chr( I );  Upper_C( Ch );
    Until ( Ch = 'H' ) or ( Ch = 'R' ) or ( No_Carrier ) Or ( Get_Timer > Maxtime );
    If ( Ch = 'H' ) or ( Get_Timer > MaxTime ) Then Disconnect;
  End;
  ...
  Begin
    ...
    Repeat
      ...
      Else Begin
        ...
        Show_Secs_Left;  Show_Cur_Time;  Show_Connected;  I := Scan_Input;
      End;
    Until ( I>Timeout ) Or ( No_Carrier ) Or ( Get_Timer>Maxtime );
  ...
  Begin
    ...
    If Exists( S ) Then H := FOpen( S, Update_Mode )
      Else H := FCreate( S, 0 );
    If ( H < 0 ) Then Begin
      H := Abs( H ); Writev( Q, 'Error -', H ); If ( H > 70 ) Then H := 70;
      Q := Concat( Q, '  ', Err[H] );
      CrLf;
  ...
  Begin
    ...
    End;
    If ( I = 0 ) Then Begin
      Line := Concat( 'Invalid Drive In Pathname: ', F );
      Show_Error( Line, -68 );
      Drv := Dd;
    End;
    Change_Drive := I;
  ...
  Begin
    ...
    X := ChDir( C );  If ( X < 0 ) Then Show_Error( S, X )
    Else Make_Work_Path;
  End;
  ...
  Begin
    ...
    End;
    If ( L<3 ) then Begin
      Line := Concat( 'Pathname for ', F, ' Too Short!!!' );
      Show_Error( Line, -68 );
    End;
  End;
  ...
  Var
    ...
    F : CString;
  Begin
    T := Get_STime;  Blkio[1] := Shr( T, 8 );  Blkio[2] := ( T & 255 );
    I := Get_SDate;  Blkio[3] := Shr( I, 8 );  Blkio[4] := ( I & 255 );
    MakeCString( S, F );
    H := Open_File( F, 0 );
    If ( H > 0 ) Then Begin
      X := GSDTOF( Blkio, H, 1 );
...
{  Function Security_Chk : Boolean;
  ...
  Begin
    ...
    S := U_Handle;  Lower_S( S );  UserRec.UHandle := U_Handle;
  ...
  Begin
    ...
    Repeat
      ...
      End;
      Get_Sig_Nums;
    Until ( Sig_Num <> 0 ) or ( Tot_Sig ) or ( No_Carrier );
...
{$I Msg_Edit.Pas }
  ...
  Begin
    ...
    For I := 1 to Cnt Do Begin
      ...
      Temp := System2.Paths[ X ]; DescL[1] := Concat( Temp, F );
      Desc_Path := Concat( System2.Desc_Path[ X ], F );
      Log( 1 );  Update_DL_Status;
    End;
      ...
      If ( Dl_Ratio_Ok( BM_Num, L, L1 ) ) Then Begin
        ...
        If ( L <= TimeLeft ) Then Begin
          ...
          If ( Carrier ) And ( Ch <> 'A' ) Then Begin
            ...
            Else Begin
              Cnt := Get_Ftu_Xfr;
              Writev( DescL[1], 'Files Transfered = ', Cnt );  Log( 1 );
...
{$I Dos_Shll.Pas }
  ...
  Begin
    ...
    EchoNE( '|   Version 2.43    Mar 16, 1996    |' );
  ...
  Begin
    About;
    If ( Get_SysData ) And ( Get_Ftu_User ) Then Begin
    ...
    Else Begin
      DescL[1] := ' ';
      DescL[2] := Concat( 'Error Loading FTU Files At: ', Mil_Time );
      Log( 2 );
    End;
  End;
