 ...
 *
 *	Special bits OMU: Note 3 arguments can be passed to the returning
 ...
 *			if so returns number, ignores ignored signals.
 */
istermsig(proc)
struct procs *proc;
{
	...
	/* Check for default signals recieved ignore defered ones */
	n = 1;
	for(i = 0; i < NSIG; i++){
		if(proc->sig & n){
