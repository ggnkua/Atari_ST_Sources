...
si_init(){
	...
	PIAB->bdir = INPUT;
	PIAB->cdir = INPUT;
	PIAB->cdir |= (SISEL | SIRESET);	/* Sets A4 SEL as an output */
...
char	*data;
{
	...
	for(count = 0; count < 10; count++){
		...
		hdmerr();		/* Clears any data on scsi bus */
		hdrestore(dev);
	}
...
char	*data;
{
	...
	switch(dcb->opcode){
	case SIREAD_CAP:
		if(err = si_read(data, sizeof(struct Sicap))) return err;
...
si_cmdend(){
	...
	if(err = si_read(status, 2)) return err;
