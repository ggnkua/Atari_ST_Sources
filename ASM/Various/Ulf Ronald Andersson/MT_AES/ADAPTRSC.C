...
extern WORD	vdi_handle;													/* Handle der VDI-Workstation fr Userdefs */
...
{
	radio_bgcol = 0;														/* Annahme: Hintergrundfarbe bei 2D ist weiž */
...
static WORD	cdecl check_button( PARMBLK *parmblock )
{
	...
	*(GRECT *) rect = *(GRECT *) &parmblock->pb_x;				/* Objekt-Rechteck... */
	...
	if ( parmblock->pb_currstate & SELECTED )
	{
		...
		vsl_color( vdi_handle, 1 );									/* schwarz - fr das Kreuz */
		xy[0] = rect[0] + 2;
		xy[1] = rect[1] + 2;
		xy[2] = rect[2] - 2;
		xy[3] = rect[3] - 2;
