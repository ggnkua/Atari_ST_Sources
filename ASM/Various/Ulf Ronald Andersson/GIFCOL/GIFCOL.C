...
#define SCREENPT_p ((int **) 0x45E)
#define VBASE2_p ((char *) 0xffff8201L)
...
int EOFCode, OutCount, MaxCode, OldCode, InCode;
...
{	int BlockLength;
	...
	BlockLength	= (uint) (uchar) Getbyte();
	while (BlockLength>0)
...
int ReadCode(void)
{
	...
	Rofp	= Raster + (BitOffset>>3);
/* Pick up our 24-bit chunk */
	RawCode	= (*Rofp++) + ((uint)*Rofp++ << 8) + ((long)*Rofp++ << 16);
	RawCode	= RawCode >> (BitOffset&7);
	BitOffset = BitOffset + CodeSize;
	return (int) RawCode & ReadMask;
}
...
void AddToPixel (int Index)
{
	...
	{	if (!shrink  ||  ((XC & shrink)  &&  ((YC+1) & shrink)))
		{	x	= (XC-xs) >> shrink;
			y	= (YC-ys) >> shrink;
			if	(((int)x >= 0)  &&  (x < 320)  && ((int)y >= 0)  &&  (y < 200))
	...
	{	x	= XC-xs;
		y	= YC-ys;
		if	(((int)x >= 0)  &&  (x < 160)  &&  ((int)y >= 0)  &&  (y < 100))
		{	XY	= ((y*160)+x/2)&0xfffc;
			...
			if	((realind = Green[Index]*6/256) != 0)
				realind+=5;
...
int 	*fix_screen(int *sc_ad,int rez)
{	void	*o_SSP;
	void	*o_scr;
	o_scr = Physbase();
	o_SSP = (void *) Super(0L);
...
char *argv[];
{
	...
	wind_handle = phys_handle;
	v_opnvwk( work_in, &wind_handle, work_out );
...
/* Get memory for the raster data array, and the input file data array */
	...
	Raster2=(int*)(((long)Raster2+639L)&0xffffff00L);
	for(I=0; I<16000; I++)
		Raster2[I] = 0;
	for(I=0; I<2048; I++)
	{	Prefix[I]=0;
...
/*	Raster = (uchar *) AllocM(size);*/
	RasterPtr = Raster;
	RastP2=Raster;
	if (0L>(Fread(handle,size,(void *)Raster)))
	{	Fclose(handle);
		my_exit(3,"Read error!\n");
	}
	printf("file read\n");
	Fclose(handle);
	printf("file closed\n");
...
/* Now read in values from the image descriptor */
	...
	if	((B = Getbyte()) != ',')
	{	/* Image separator had better be ',' */
		my_exit(-98,"Corrupted data!\n");
	}
	LeftOfs	= Getword();
	TopOfs	= Getword();
	Width	= Getword();
...
reclaim it for the Picture array */
	...
	rez=Getrez();
	if	(rez == 2L)
	{	my_exit(4,"Color only!\n");
	}
	if	((xe-xs) > 1280  ||  (ye-ys) > 800)
	{	my_exit(5,"Picture too big!\n");
	}
	pscr=Physbase();
	shrink=0;
	...
	getchar();
crit_1:
	Setscreen((void *) -1,Raster2,0);
	fix_screen(Raster2,0);
...
/* Read one key, then pack it in */
	getchar();
crit_2:
	fix_screen(pscr, (int)rez);
	Setscreen((void *) -1, pscr, (int) rez);
	...
	{ /* NeoChrome format */
		...
		{	my_exit(7,"Creation error!\n");
		}
		if	((size = Fwrite(handle,32128L,Raster2)) < 0)
		{	Fclose(handle);
			my_exit(8,"Write error!\n");
		}
		printf("Saved.\n");
		Fclose(handle);
	}
