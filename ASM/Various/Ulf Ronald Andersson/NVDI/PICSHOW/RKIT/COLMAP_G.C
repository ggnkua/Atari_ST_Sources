...
COLOR_TAB	*_create_ctab( int16 vdi_handle )
{
	...
	{
		ctab->magic = 'ctab';
		ctab->length = length;											/* LÑnge der Farbtabelle */
		ctab->format = 0;
		ctab->reserved = 0;
...
INVERSE_CTAB	*_create_inverse_ctab( COLOR_TAB *ctab, int16 no_bits, int16 levels )
{
	...
	if ( inverse_ctab )
	{
		...
		inverse_ctab->no_hidden_values = 0;							/* keine Ñhnlichen Farben */
		...
		if ( no_entries > 0 )											/* Tabelle anlegen? */
		{
			...
			if ( dist_buf )
			{
				...
				for ( value = 0; value < ctab->no_colors; value++ )	/* fÅr jede Farbe den gesamten WÅrfel untersuchen */
				{
					int32	rdist;
					int32	gdist;
					int32	bdist;
