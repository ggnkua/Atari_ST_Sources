...
#define NORM_FREQINDEX		24
#define MAX_FREQINDEX		48

#define MIN_FRAMEFREQ		12			/* frame freq. limits */
#define NORM_FRAMEFREQ		50
#define MAX_FRAMEFREQ		200
...
extern int		listentries;		/* Anzahl EintrÑge in der Liste */
*/
...
void string_concat( char *n );
void string_concat_init( void );
*/
/* -------------------------------------------------------------------- */
/*	  Global Variables. 												*/
/* -------------------------------------------------------------------- */
...
GRECT			screen; 			/* Maûe & Lage des Bildschirms */
PRIVATE int 	wchar, hchar;		/* Breite & Hîhe eines Zeichens */

int		sysflags;			/* MAGIC 4/5 special functions. */
...
PRIVATE long chipfrequency[] = {
	...
	6378024L,			/* -2	LA# */
	6757281L,			/* -1	SI	*/
	7159090L,			/* 0	DO Normalstimmung NTSC */
	7584792L,			/* 1	DO# */
	8035807L,			/* 2	RE	*/
	8513641L,			/* 3	RE# */
	9019888L,			/* 4	MI	*/
 ...
 * Es werden auch die nîtigen Userblocks initialisiert
 */
/************************************************************************/
long init_resource(void)
{
	...
	for ( i=0; i<NUM_OBS; i++)
	{
		...
		/* Objekt-Hîhe an ST Medium-rez anpassen */
		tmp = cobj->ob_height;
...
/* redraw a button, visible. (for modal dialog boxes)
 */
 ...
 *	Ist die Pfadangabe leer/ungÅltig, wird der Default Pfad benutzt.
 *
 * Param:	path:	Zeiger auf den zu trimmenden Pfad.
 *			file:	Zeiger auf Puffer fÅr filename oder NULL
 * Return:	TRUE wenn der Pfad komplett geÑndert werden muûte.
 */
 ...
 *			path:	geÑnderter Pfad
 *			file:	geÑnderter Name
 */
/************************************************************************/
int fileselect(char *path, char *file, char *title)
{
	char	mypath[256+64], myfile[64];
	WORD 	butt, fret, nfiles, sortmode=SORTBYNAME;
	void	*fsel;
	char	*pattern;

	strcpy(mypath,modpath);
	trim_path(mypath, myfile);

	/* ok boys and girls: MAGIC fileselector vorhanden? */
	if (sysflags & 0x08)
	{
		fsel = fslx_do(title,			/* fsel box title */
    			mypath, 256,			/* path buffer and maxlen */
    			myfile, 64,				/* name buffer and max len */
    			"*.MOD,mod.*\0"
    			"*\0",					/* patterns */
				(XFSL_FILTER*)0L,		/* no filter function */
				"\0",					/* no extra paths */
				&sortmode,				/* sort mode */
				GETMULTI,				/* get multiple files */
				&butt, &nfiles,
				&pattern );
        if (fsel) {						/* fileselector open? */
        	if (butt) {
				clear_modfile_list();
       			expand_wildcards(mypath,myfile);
           		while (--nfiles>0) {		/* mehrere Files ausgewÑhlt? */
                	fslx_getnxtfile(fsel, myfile);   /* alle abholen */
	       			expand_wildcards(mypath,myfile);
				}
	...
	{
		strcat(mypath, MOD_MASK);				/* Maske dazu */
		...
		{
			if (trim_path(mypath, NULL))		/* Pfad ungÅltig? */
				return FALSE;
...
{
	int	buf[8];
	buf[0]=msg;		buf[1]=ap_id;	buf[2]=16;		buf[3]=0;
	buf[4]=0;		buf[5]=0;		buf[6]=0;		buf[7]=VERSION;
	appl_write(rxer,16,buf);
}

/* wait for TIMER and MESAG events: */
int wait_message(int delay)
{
	kk2.ev_mflags = MU_MESAG | MU_TIMER;
	kk2.ev_mtlocount = delay;
	kk2.ev_mthicount = 0;
	return EvntMulti( &kk2 );
}
...
int send_paula_message(int rx_id, int msg, int *mbuf)
{
	...
	evnt = wait_message(2000);			/* timeout = 2 sec */
	 
	if (evnt & MU_MESAG)
	{
		if ( kk2.ev_mmgpbuf[1]==rx_id && 
			(kk2.ev_mmgpbuf[0]==MP_ACK || kk2.ev_mmgpbuf[0]==MP_NAK))
			return TRUE;		/* OK */
...
int transfer_job(int other, int argc, char *argv[])
{
	...
	if (!_app) return FALSE;	/* accessory: can't transfer any job. */

	if (_GemParBlk.global[1]!=1)		/* Multitasking */
	{
		if (other==ap_id) return FALSE;	/* only one copy running! */
	}
	else								/* Singletasking */
	{
		if (argc<=1) return FALSE;		/* Never transfer empty jobs */
		do {							/* Wait for possible ACC response */
			evnt = wait_message(20);
			if ((evnt & MU_MESAG) && (kk2.ev_mmgpbuf[0]==MP_ACK))
				other = kk2.ev_mmgpbuf[1];
		} while (evnt!=MU_TIMER);
		if (other==0) return FALSE;		/* No ACC response! */
	};
	...
	*(char **)&mbuf[3] = p;
	/* mbuf[6]=1; mbuf[7]=0; */
	ret=send_paula_message(other, MP_START, mbuf);
	if (p) free(p); return ret;
}
...
int check_commandline(int argc, char *argv[])
{
	...
	if (make_modlist(p))
	{
		...
		if (error_alert(p_load(modpath)) == 2) mp_stop();
		else p_play();
	}
	return TRUE;
}
...
void set_the_window_dialog( void )
{
	...
	t[WSWAPCHANNEL].ob_state	= 1 & p_swapchannel;
...
void main_window_commands(int button)
{
	...
	case WSHUFFLE:
			...
			break;
	case WSWAPCHANNEL:
			p_swapchannel = 1 & t[WSWAPCHANNEL].ob_state;
			break;
...
void draw_indicators( int mask )
{
	...
	if (mask & IND_LOAD)			/* CPU LOAD - display */
	{
		...
		if (currload>1000) p_overload=30;	/* fÅr Petra */
		if (p_overload)
		{
	...
	if (mask & IND_MULTI)
	{
		t[WSHUFFLE].ob_state = (get_num_listentries()>1) ? DISABLED : NORMAL;
		if (omode!=get_num_listentries())
		{
...
int wform_do(OBJECT *tree)
{
	...
	*/
	...
	do {
		...
		{	/* Das war fÅr Form-Do gemÅnzt. */
			wind_get( 0, WF_TOP, &wh, &dum, &dum, &dum);

			/* if the clicked window wasn't TOP then TOP IT! */
			if (wh != wi_handle)
				wind_set( wi_handle, WF_TOP, &dum, &dum, &dum, &dum);

			/* now perform the desired click action */
			wind_update(BEG_UPDATE);
		...
		if (kk.ev_mwich & MU_KEYBD)
		{
			...
			if (kk.ev_mmokstate & 0x0004)	 /* control key pressed */
			{
				key += '@';
				switch (key) {
				case 'I':	whichob=WINFO;			break;
				case 'O':	whichob=WLOAD;			break;
				case 'U':	close_the_window();
							wi_handle = -1;
							cflag=0; whichob=0; 	/* Exit! */
							break;
				case 'W':	wind_set(wi_handle, WF_BOTTOM);
							break;
				}
			}
			else
			{
				...
				case 'C':	whichob=WSWAPCHANNEL;	break;
...
/************************************************************************/
/*
 * accloop: ZusÑtzlicher Programmcode fÅr ACC Betrieb:
 */
/************************************************************************/
void accloop( void )
{
		...
		if (kk.ev_mwich & MU_MESAG)
		{
			...
			case MP_START:
				...
				else
				{
					...
					if (_GemParBlk.global[1]!=1) msg=AC_OPEN;
				};
				break;
...
int main(int argc, char *argv[])
{
	...
	/* look for other instances of PAULA */
	...
	/* Bei ACC's muû am Speicher gefummelt werden! */
	e = p_global_init( ((_app) ? NULL : _BasPag) );

	sysflags=0;
	
	if ( appl_find( "?AGI" ) == 0		/* appl_getinfo() vorhanden? */
		|| _GemParBlk.global[0] >= 0x0401 )											/* mindestens AES 4.01? */
	{
	...
	do {							/* ACC: endless loop				*/
		...
		else
		{
			...
			if (open_the_window())	/* Open & Display the window		*/
			{
				...
				/* application and arguments passed: */
				if (_app && argc>1)
