********************************************************************************
*     ATAPI extensions thing v0.20 for QUBIDE (c) 2001 Thierry Godefroy.       *
...
IDE_DATA  equ       8                             Data address register (word).
IDE_ERROR equ       1                             Error register (read).
IDE_FEATR equ       1                             Feature register (write).
IDE_NSECT equ       2                             Sector count register.
IDE_SECTR equ       3                             "sector" register.
IDE_LCYL  equ       4                             "lcyl" register.
IDE_HCYL  equ       5                             "hcyl" register.
IDE_CURRT equ       6                             "current" register.
IDE_STATS equ       7                             Status register (read).
IDE_CMD   equ       7                             Command register (write).
IDE_ASTAT equ       7                             Alternate status register (r).
IDE_CTRL  equ       14                            Control register (write).
...
INIT      MOVEQ     #0,D0                         | Get system info.
          ...
          ANDI.B    #$1E,D2                       Get rid of display/SH flags.
          CMPI.B    #$0A,D2                       Is this an GC ?
          BEQ.S     INIT2                         If yes then continue init...
          CMPI.B    #$0C,D2                       Is this a SGC ?
          BNE.S     INIT_END                      If no, then return with err.
INIT2     MOVEQ     #-2,D0                        "Invalid job Id" error code.
...
*ENDPKT7  MOVE.B    #IDE.NIEN+IDE.CTLM,IDE_CTRL(A2)
...
IDENTDV10 
*         MOVE.B    #IDE.NIEN,IDE_CTRL(A1)        Disable interrupts from drive.
...
********************************************************************************
* Configuration post-processing routine:

POST_CONF MOVEM.L   A0-A2,-(SP)                   Save context.
          LEA       REG_ADDR,A0                   Configured string address.
          MOVE.W    (A0)+,D1                      Number of characters.
          MOVEQ     #0,D2                         Clear bytes in result.
          BRA.S     POSTCONF1                     Start conversion.
POSTCONF4  LSL.L     #4,D2                         Make room for next nibble.
          MOVE.B    (A0)+,D0                      Get next character.
          ANDI.B    #$DF,D0                       Upper case it.
          CMPI.B    #'A',D0                       Is it an alphabetic char ?
          BPL.S     POSTCONF2                     If yes, jump over...
          SUBI.B    #'0',D0                       Turn character into a value.
          BRA.S     POSTCONF3                     And jump to common part...
POSTCONF2 SUBI.B    #'A'-10,D0                    Turn character into a value.
POSTCONF3 ANDI.B    #$F,D0                        Just in case...
          OR.B      D0,D2                         Store nibble into result.
POSTCONF1 DBRA      D1,POSTCONF4                  Process next char if any...
          MOVEM.L   (SP)+,A0-A2                   Restore context
          LEA       IDE_CTRLB,A0                  IDE controllers table base.
          MOVEQ     #3,D1                         Four addresses to write.
POSTCONF5 MOVE.L    D2,(A0)+                      Store IDE controller address.
          ADDQ.L    #8,D2                         | Calculate nex controller
          ADDQ.L    #8,D2                         |  address.
          DBRA      D1,POSTCONF5                  Store next address if any...
          MOVEQ     #0,D1                         Item is available.
          MOVEQ     #0,D0                         No error.
          RTS                                     Return.
...
IDE_CTRLB dc.l      $4FFF00                       First IDE controller reg base.
          dc.l      $4FFF10                       ...
          dc.l      $4FFF20                       ...
          dc.l      $4FFF30                       Fourth IDE controller...
...
* Configuration block:
          ...
          dc.b      '<<QCFX>>'                    Config block Id.
          dc.b      '01'                          Required config level.
          dc.w      16                            Length of software name.
          dc.b      'ATAPI for '                  | Software name.
          dc.b      'QUBIDE'                      |
          dc.w      4                             Version string lentgh.
          dc.l      VERSION                       Version string.

          dc.b      0                             Item type = string.
          dc.b      0                             Selection keystroke (none).
          dc.w      REG_BLOC-*                    Ptr configurable string block.
          dc.w      0                             Pre-processing routine (none).
          dc.w      POST_CONF-*                   Post-processing routine.
          dc.w      CONFIGTXT-*                   Pointer to item description.
          dc.w      TEXT_ATTR-*                   Pointer to item attribute.

          dc.w      -1                            End of configurable items.

REG_BLOC  dc.w      9                             8 characters maximum.
REG_ADDR  dc.w      6                             | Configurable string (hexa
          dc.b      '4FFF00'                      |  address of IDE registers).
          ds.b      4                             Room for 8 chars+terminator.
CONFIGTXT dc.w      40                            |
          dc.b      'IDE registers base address'  | Invite string.
          dc.b      ' (hexadecimal)'              |
TEXT_ATTR dc.w      0                             Text item attributes.
