...
light_source { <-6, 17, 3> colour White fade_distance 100 fade_power 1 }
//light_source { <-2, 16, 8> colour Grey70 fade_distance 100 fade_power 1 }
light_source { <0, 5, -7> colour White fade_distance 100 fade_power 1 }
...
union{
  ...
  box{ < 6,-20,1> < 4,10,0.5>}
//  box{ <-6, 10,1> < 6,10,0.5>}
  box{ <-6,-20,1> <-5.5,10,-20>}
  box{ < 6,-20,1> < 5.5,10,-20>}
  difference{
    cylinder{ <0,10,1> <0,10,-20> 6 }
    union{
      box{ <-7,10,2> <7,3,-21> }
      cylinder{ <0,10,0.5> <0,10,-19.5> 5.5}
    }
  }
  polygon {
    5, <0,0>, <0,4>, <4, 4>, <4,0>, <0,0>
    pigment{ image_map{ gif "bee.gif" } scale <4,4,0> }
   translate <-2.5,10.5,1.01>
  }
  
  
  texture{ pigment{color <0.8,0.8,0.8>} }
}
