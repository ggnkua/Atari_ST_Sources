...
; takes around 30% and it's a TRUE 50kHz replay routine.
; And thanks to trash mode you don't have to count with
; maximum time the replay takes (thanks BLADE for the tip).
;---------------------------------------------------------- 
;	M†rten R†nge
;	Mejerigatan 2/373
;	412 76 GOTHENBURG
...
;----------------------------------------------------------
; If you like my routine please send me a mail and tell me
; since I need motivation and response from people that
; like my code gives me a kick (It's better than sex :-)).
; If there's a interest for a 25kHz version than mail me.
;----------------------------------------------------------
; Some Greets:
; ÿÿÿÿÿÿÿÿÿÿÿÿ
...
;----------------------------------------------------------
; Some notes:
; DON'T use Trap0 because I am using that to switch between
; Supervisor- and Usermode ( yes,dirty tricks ) .
;----------------------------------------------------------
; P.S. This replay routine supports every PT2.2 command D.S
;----------------------------------------------------------
;      - Lance / M†rten R†nge      1994/01/01
;----------------------------------------------------------
mt_frame_number	equ	3
;----------------------------------------------------------
mt_init	jmp	mt_init_all
mt_vbl	jmp	mt_play_frames
mt_trash_on	jmp	mt_trash_mode_on
mt_trash_off	jmp	mt_trash_mode_off
mt_ask_buf_full	jmp	mt_ask_buffer
mt_ask_position	jmp	mt_give_pos
mt_update	jmp	mt_update_frames
mt_end	jmp	mt_exit
;----------------------------------------------------------
n_note	equ	0  ; w
n_cmd	equ	2  ; w
...
mt_volume	rs.w	1
mt_period	rs.w	1
mt_old_period	rs.w	1
mt_add_iw	rs.w	1
mt_next_voice	rs.w	1
	opt	a+,o+

	section	text
mt_init_all	lea	mt_data,a0
...
mt_trenoc	move.l	n_start(a6),mt_sample_point(a5)	; set start
	moveq	#0,d0
...
setback	move.w	n_period(a6),mt_period(a5)
	...
	beq	mt_volumeslide
mt_return2	rts
mt_pernop	move.w	n_period(a6),mt_period(a5)
	rts
...
mt_glissskip	move.w	d2,mt_period(a5) ; set period
	rts
...
mt_vibrato3	move.w	d0,mt_period(a5)
...
mt_tremolook	move.w	d0,mt_volume(a5)
	move.b	n_tremolocmd(a6),d0
	lsr.w	#2,d0
	and.w	#$003c,d0
...
mt_init_Paula	bsr	mt_make_freq
	bsr	mt_make_tables
	bsr	mt_make_frame_f
	bsr	mt_make_voltab
	bsr	mt_make_divtab
...
.mt_no_round	move.w	d4,(a0)+
	...
	bne.s	.mt_init_div
	rts
mt_make_zero	move.l	mt_zero_code_p,a0
	move.l	mt_replay_len,d0
	asr.w	#2,d0
	subq.w	#1,d0
	move.w	.mt_clear_op,d1
.mt_maker	move.w	d1,(a0)+
	dbra	d0,.mt_maker
	move.w	#$4ed6,(a0)
	rts
.mt_clear_op	move.l	d2,(a0)+
...
mt_make_code	move.l	mt_codes_p,a0
	move.l	mt_code_chunk,a1
	move.l	mt_freq_list,a2
	moveq	#22,d7
.mt_maker	move.l	a1,(a0)+
	moveq	#39,d6
.mt_make_it	move.w	(a2)+,d0
	beq.s	.mt_set_copy
	tst.w	d6
	beq.s	.mt_set_op0
	move.w	(a2),d0
	beq.s	.mt_set_op0
	move.w	.mt_fetch_op1,(a1)+
	dbra	d6,.mt_make_it
	move.w	#$4ed6,(a1)+
	dbra	d7,.mt_maker
	rts
.mt_set_op0	move.w	.mt_fetch_op0,(a1)+
.mt_set_copy	move.w	.mt_copy_op,(a1)+
	dbra	d6,.mt_make_it
	move.w	#$4ed6,(a1)+
	dbra	d7,.mt_maker
	rts
.mt_fetch_op0	move.b	(a0)+,d0
.mt_fetch_op1	move.b	(a0)+,(sp)+
.mt_copy_op	move.b	d0,(sp)+

mt_init_frm_tab	lea	mt_replay_frames,a0
	move.l	mt_replay_len,d0
	move.l	(a0),d1
	move.w	#(1<<mt_frame_number)-1,d7
.mt_init_loop	move.l	d1,(a0)
	add.l	d0,d1
	lea	$10(a0),a0
	dbra	d7,.mt_init_loop
	rts
...
mt_play_frames	move.w	mt_ready_frames,d0
	bne.s	.mt_continue
	moveq	#-1,d0
	rts
.mt_continue	lea	mt_ready_frames,a0
	subq.w	#1,d0
	move.w	d0,(a0)
	lea	mt_play_frame,a0
	move.w	(a0),d0
	move.w	d0,d1
	add.w	#1<<4,d1
	and.w	#((1<<mt_frame_number)-1)<<4,d1
	move.w	d1,(a0)
	lea	mt_replay_frames,a0
	add.w	d0,a0
	move.l	(a0)+,d4
	...
	or.w	d3,d2
	move.w	d1,(a1)+
	lea	.mt_LCM_vol_tab,a0
	lea	$ffff8900.w,a6
	jmp	mt_LCM_set_conf(pc,d2.w)

.mt_LCM_vol_tab	dc.b	0
	dc.b	2,5,7,8,9,10,10,11,11,12,12,13,13,13,14,14
	dc.b	14,14,15,15,15,15,16,16,16,16,16,16,17,17,17,17
	dc.b	17,17,17,18,18,18,18,18,18,18,18,18,18,19,19,19
	dc.b	19,19,19,19,19,19,19,19,19,20,20,20,20,20,20,20

mt_LCM_set_conf	jmp	mt_set_nil
	jmp	mt_set_left
	jmp	mt_set_right
	jmp	mt_set_all
...
mt_set_nil
	...
	endr
	movep.l	d4,$1(a6)
	add.l	mt_replay_len,d4
	movep.l	d4,$d(a6)
	move.w	mt_frequency,$20(a6)
	move.w	mt_start,(a6)
	move.w	mt_trash_mode,d0
	beq	mt_force_update
	rts

mt_set_left	move.w	mt_LCM_mask,$24(a6)
	move.w	mt_LCM_left,d3
	or.b	(a0,d0.w),d3
	move.w	d3,$22(a6)
	movep.l	d4,$1(a6)
	add.l	mt_replay_len,d4
	...
	move.w	mt_start,(a6)
	move.w	mt_trash_mode,d0
	beq.s	mt_force_update
	rts

mt_set_right	move.w	mt_LCM_mask,$24(a6)
	move.w	mt_LCM_right,d3
	or.b	(a0,d1.w),d3
	move.w	d3,$22(a6)
	movep.l	d4,$1(a6)
	add.l	mt_replay_len,d4
...
mt_set_all	move.w	mt_LCM_mask,$24(a6)
	...
	move.w	mt_frequency,$20(a6)
	move.w	mt_start,(a6)
	move.w	mt_LCM_mask,d2
	move.w	mt_LCM_right,d3
	or.b	(a0,d1.w),d3
.mt_test_LCM	cmp.w	$24(a6),d2
	bne.s	.mt_test_LCM
	move.w	d3,$22(a6)
	move.w	mt_trash_mode,d0
	beq.s	mt_force_update
	rts
mt_force_update	move.w	sr,d1
	ori.w	#$700,sr
	lea	mt_replay_frames,a0
	lea	mt_update_buf,a1
	lea	mt_update_frame,a2
	move.w	(a2),d0
	lea	(a0,d0.w),a0
	move.l	(a0)+,(a1)
	lea	4(a0),a0
	move.w	d0,-(sp)
	add.w	#1<<4,d0
	andi.w	#((1<<mt_frame_number)-1)<<4,d0
	move.w	d0,(a2)
	moveq	#0,d0
	...
	lsr.w	#4,d0
	move.w	d0,(a0)+
	lea	mt_ready_frames,a0
	addq.w	#1,(a0)
	move.w	d1,sr
	bsr	mt_Paula
	bsr	mt_music
	lea	mt_replay_frames,a1
	move.w	(sp)+,d0
	lea	4(a1,d0.w),a1
	move.w	mt_left_volume,(a1)+
	move.w	mt_right_volume,(a1)+
	rts

mt_trash_mode_on
	lea	mt_trash_mode,a0
	moveq	#-1,d0
	move.w	d0,(a0)
	rts
mt_trash_mode_off
	...
	clr.w	(a0)
	rts
mt_ask_buffer	move.w	mt_ready_frames,d0
	cmp.w	#((1<<mt_frame_number)-1),d0
	seq	d0
	ext.w	d0
	rts

mt_give_pos	move.w	mt_play_frame,d0
	lea	mt_replay_frames,a0
	add.w	d0,a0
	lea	8(a0),a0
	move.w	(a0)+,d0
...
mt_update_frames
	bra.s	.mt_update_test
.mt_update_loop	move.w	sr,d1
	ori.w	#$700,sr
	lea	mt_replay_frames,a0
	lea	mt_update_buf,a1
	lea	mt_update_frame,a2
	...
	move.w	mt_left_volume,(a1)+
	move.w	mt_right_volume,(a1)+
.mt_update_test	move.w	mt_ready_frames,d0
	cmp.w	#((1<<mt_frame_number)-1),d0
	blt.s	.mt_update_loop
	moveq	#0,d0
	rts
mt_Paula	lea	mt_save_SSP,a0
	move.l	sp,(a0)
	and.w	#$fff,sr
	bra.s	mt_emulate

mt_return_Paula	move.l	mt_save_SSP,sp
	rts
	rsreset
mt_temp_regs	rs.l	2
...
mt_channel	macro
	lea	.mt_return,a0
	move.l	a0,mt_return(a6)
	move.l	mt_sample_point(a3),d0
	seq	d0
	move.l	mt_sample_point(a4),d1
	seq	d1
	add.b	d0,d1
	bcs	mt_no_voice
	bne	mt_one_voice
...
.mt_return
	endm
mt_no_v_adder	macro	\1
	move.w	mt_period(a\1),d0
	cmp.w	mt_old_period(a\1),d0
	beq.s	.mt_nochange_v\1
	clr.w	mt_add_iw(a\1)
	move.w	d0,mt_old_period(a\1)
.mt_nochange_v\1
	...
	move.w	d0,d1
	swap	d0
	add.w	mt_add_iw(a\1),d1
	negx.w	d0
	neg.w	d0
	move.w	d1,mt_add_iw(a\1)
	add.w	#75,d0
	move.l	mt_sample_point(a\1),a0
	add.w	d0,a0
	move.l	a0,mt_sample_point(a\1)
	cmp.l	mt_sample_end(a\1),a0
	blt.s	.mt_no_end_v\1
	move.l	mt_loop_start(a\1),d0
	beq.s	.mt_no_loop_v\1
	move.l	a0,d1
	sub.l	mt_sample_end(a\1),d1
	neg.l	d0
	add.l	mt_sample_end(a\1),d0
	divu	d0,d1
	clr.w	d1
	swap	d1
	add.l	mt_loop_start(a\1),d1
	move.l	d1,mt_sample_point(a\1)
	bra.s	.mt_no_end_v\1
.mt_no_loop_v\1	moveq	#0,d2
	move.l	d2,(a\1)+
	move.l	d2,(a\1)+
	move.l	d2,(a\1)+
	move.l	d2,(a\1)+
...
mt_emulate	lea	mt_save_USP,a0
	...
	lea	mt_channel_0,a3
	move.l	(a3),d0
	bne.s	.mt_no_reset_v0
	move.w	d0,mt_volume(a3)
.mt_no_reset_v0	lea	mt_next_voice(a3),a3
	move.l	(a3),d0
	bne.s	.mt_no_reset_v1
	move.w	d0,mt_volume(a3)
.mt_no_reset_v1	lea	mt_next_voice(a3),a3
	move.l	(a3),d0
	bne.s	.mt_no_reset_v2
	move.w	d0,mt_volume(a3)
.mt_no_reset_v2	lea	mt_next_voice(a3),a3
	move.l	(a3),d0
	bne.s	.mt_no_reset_v3
	move.w	d0,mt_volume(a3)
.mt_no_reset_v3	lea	mt_channel_0,a3
	move.l	mt_sample_point(a3),d0
	sne	d3
	move.w	mt_volume(a3),d0
	sne	d4
	and.b	d4,d3
	lea	mt_next_voice(a3),a3
	move.l	mt_sample_point(a3),d0
	sne	d4
	move.w	mt_volume(a3),d0
	sne	d5
	...
	or.b	d5,d3
	and.b	d3,d4
	bne.s	.mt_no_need_cls
	move.l	mt_update_buf,a0
	move.l	mt_zero_code_p,a1
	lea	.mt_no_need_cls,a6
	moveq	#0,d2
	jmp	(a1)
.mt_no_need_cls		
	lea	mt_channel_p,a0
...
mt_no_voice	move.l	mt_sample_point(a3),d0
	beq.s	.mt_no_v0
	mt_no_v_adder	3
.mt_no_v0	move.l	mt_sample_point(a4),d0
	beq.s	.mt_no_v1
	mt_no_v_adder	4
.mt_no_v1	move.l	mt_return(a6),a6
	jmp	(a6)

mt_one_voice	move.l	mt_sample_point(a3),d0
	bne.s	.mt_no_swap
	exg	a3,a4
.mt_no_swap	move.w	mt_volume(a3),d0
	...
	bge.s	.mt_no_swap1
	exg	a3,a4
.mt_no_swap1	move.w	mt_volume(a3),d0
	beq	mt_no_voice
	move.w	d0,(a1)
	move.l	mt_sample_point(a4),d0
	beq.s	.mt_no_v1
	mt_no_v_adder	4
.mt_no_v1	move.w	mt_period(a3),d3
	cmp.w	mt_old_period(a3),d3
	beq.s	.mt_no_change_v0
	move.w	d3,mt_old_period(a3)
	clr.w	mt_add_iw(a3)
.mt_no_change_v0
...
.mt_return
	...
	move.l	mt_return(a6),a6
	jmp	(a6)

mt_two_voices	move.w	mt_volume(a3),d0
	cmp.w	mt_volume(a4),d0
	bge.s	.mt_no_swap
	exg	a3,a4
.mt_no_swap	move.w	mt_volume(a3),d0
...
.mt_no_change_v0
	...
	cmp.w	mt_old_period(a4),d4
	beq.s	.mt_no_change_v1
	move.w	d4,mt_old_period(a4)
	clr.w	mt_add_iw(a4)
.mt_no_change_v1
	add.w	d4,d4
	add.w	d4,d4
...
mt_codes_p	dc.l	mt_onecodes
mt_zero_code_p	dc.l	mt_zero_code
mt_mixer_chunk	dc.l	mt_mix_chunk
mt_code_chunk	dc.l	mt_codes_chunk
mt_channel_p	dc.l	mt_channel_0
	dc.l	mt_channel_1
	dc.l	mt_channel_2
	dc.l	mt_channel_3
mt_trash_mode	dc.w	-1
mt_replay_frames
	dc.l	mt_frames
	ds.w	6
	ds.w	$10*(1<<mt_frame_number-1)
mt_ready_frames	dc.w	0
mt_play_frame	dc.w	0
mt_update_frame	dc.w	0
mt_update_buf	dc.l	0
...
mt_left_temp	dc.l	0,0,0,0
mt_right_temp	dc.l	0,0,0,0
mt_channel_0	dc.l	0
	dc.l	0
	dc.l	0
	dc.w	0
...
mt_channel_3	dc.l	0
	...
	dc.w	$3ff
	dc.w	0
mt_data	incbin	"f:\mods\bestsid.mod"
...
mt_zero_code	ds.w	501
mt_mix_chunk	ds.w	61517
mt_codes_chunk	ds.w	1240
mt_frames	ds.w	2000*(1<<mt_frame_number)/2