 ...
 *
 * $Source: u:\usr\src\scsi\cbhd\RCS\srchdrv.c,v $
 *
 * $Revision: 1.8 $
 *
 * $Author: Steffen_Engel $
 *
 * $Date: 1997/03/24 08:39:54 $
 *
 * $State: Exp $
 *
 *****************************************************************************
 ...
 *
 * $Log: srchdrv.c,v $
 * Revision 1.8  1997/03/24  08:39:54  Steffen_Engel
 * routine zum lesen von daten eingebaut. dient zu lasttests
 *
 * Revision 1.7  1997/01/18  11:25:24  Steffen_Engel
 * Ergebniskontrolle von Scsi.Open auf 0xff im Highbyte wegen EinschrÑnkung des
 * Adreûbereiches.
 *
 ...
 *
 ***********************************************/
void dump(UCHAR *data, ULONG len)
{{{
UWORD count1, count2;

  /* Schleife Åber 16 Bytes */
  for (count1 = 0; count1 < (UWORD) len/16; count1++)
  {
...
LONG result;
  ...
  for (BlockCount = 0x0; BlockCount < DUMPBLOCKS; BlockCount++)
  {
    SuperOn();
...
{{{
  ...
  while (ret == 0)
  {
    ...
    else
    {
      ...
      for (LUNcount = 0; LUNcount <=7; LUNcount++)
      {
        SuperOn();
        SetScsiUnit(handle, LUNcount, MaxLen);
        /* erst den Header */
    ...
    if ((LONG) handle > 0)
    {
      ...
      {
if (Bus.BusNo == 3 && Dev.SCSIId.lo == 1)
        dumpblocks();
      }
#endif
...
{{{
  ...
  printf("  Ω Steffen Engel 1995-98\n\n");

  if (init_scsiio() && init_scsi())
  {
    printf("SCSIdriver revision %d.%02d \n", DriverRev/0x100, DriverRev % 0x100);
    search_drives();
  }
  else
