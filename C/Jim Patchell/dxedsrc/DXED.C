...
char ver[]="DXED  ver 0.95";	/*	program revision number	*/
extern int	gl_apid;
extern int patch;
extern int midi_ch;
extern char DXEDRSC[];
...
extern char temp[128],edit[155],pedit[94],bulk_buff[4096],per_bulk[4096];
...
main()
{
	...
	*/
	...
	wh = open_window(xdesk,ydesk,wdesk,hdesk,voice_name,WI_KIND);
	/*
...
int item,whand,vw;
{
	...
	{
		case ED_VOICE:
			ed_voice(vw,1,whand);	/*	copy voice by number to edit buff	*/
...
int item,whand,vw;
{
	switch(item)
	{
		...
		case M_F_ARNG:
			arrange_bank(vw);
			break;
		case M_F_DATA:	/*	print out data for midi file	*/
			print_data(vw);
			break;
		case M_F_CAT:	/*	print out catalog of all voices	*/
			print_catalog(vw);
			break;
		case M_F_QUIT:
			return(1);
