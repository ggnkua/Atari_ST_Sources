...
do_new(vw,wh)
int vw,wh;	/*	virtual workstation handle	*/
{
	int i;
...
char file[];
{
	...
	if (strlen(file_path) == 0)
	{
		...
		sprintf(in_path,"%c:%s\\*.VCE",drive + 'A',temp);
	}
	else
	{
	...
	*/
	...
	{
		++p;	/*	point to beginning of extender	*/
		if(strcmp(p,"VCE") != 0)
		{
			*p = '\0';	/*	wrong extender	*/
			strcat(in_file,"VCE");
...
int vw,mode,wh;
{
	...
	/*
		select a voice by patch number
	*/
	if((index = voice_sel("buffer",bulk_buff,vw)) >= 0)
	{
