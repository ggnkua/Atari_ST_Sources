...
extern char voice_name[11];	/*	voice currently being edited	*/
...
int vw;
{
	...
	/*
		LOOKUP table for index/algorythm number
	*/
	static int alg_index[32] = {AL01,AL02,AL03,AL04,AL05,AL06,AL07,
		AL08,AL09,AL10,AL11,AL12,AL13,AL14,AL15,AL16,AL17,AL18,AL19,
		AL20,AL21,AL22,AL23,AL24,AL25,AL26,AL27,AL28,AL29,AL30,AL31,AL32};

	misc = (MISC *)(&edit[126]);
	rsrc_gaddr(R_TREE,ALGORITH,&box);
	for(i=0;i<32;++i)
		box[alg_index[i]].ob_state = NORMAL;
	i = alg_index[misc->alg_select];
	box[i].ob_state = SELECTED;
	form_center(box,&x,&y,&w,&h);
	form_dial(FMD_START,x,y,0,0,x,y,w,h);
	form_dial(FMD_GROW,x,y,0,0,x,y,w,h);
	objc_draw(box,0,2,x,y,w,h);
	...
	form_dial(FMD_SHRINK,x,y,0,0,x,y,w,h);
	form_dial(FMD_FINISH,x,y,0,0,x,y,w,h);
	for(i=0;(form_result != alg_index[i]) && ( i < 32);++i);
	misc->alg_select = i;
...
int wh;	/*	window handle	*/
{
	...
	*/
	...
	if(form_result == OTH_OK)
	{
		...
		strcpy(voice_name,name);
		wind_set(wh,WF_NAME,voice_name,0,0);
	}
