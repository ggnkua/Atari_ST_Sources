...
#define		MAX_BLOCK		8192L /* was 4096L */
...
GLOBAL BOOLEAN OpenDevice( DEV_LIST *port )
{
	...
	if( (dev->buf=(BYTE *) STiKmalloc( MAX_BLOCK * sizeof( BYTE ) ))==NULL )
	{
		CloseDevice( port );
		return( FALSE );
	}
...
GLOBAL WORD SetTxBuffer( DEV_LIST *port, WORD size )
{
	...
	if( (mem=(BYTE *) STiKmalloc( size ))==NULL )
		return( -1 );
...
GLOBAL BOOLEAN PortSendByte( DEV_LIST *port, BYTE c )
{
	...
	if( dev->bios>=0 )
	{
		...
		return( TRUE );*/
		return SendBlock( dev, (BYTE *) &c, 1L, FALSE );
	}
...
LOCAL BOOLEAN InitStdDevices( MAPTAB *maps, BOOLEAN has_bconmap, WORD num_devices )
{
	...
	for( i=1; i<=loop; i++ )
	{
		dptr = (DEVICES *)STiKmalloc( sizeof( DEVICES ) );
		memset( dptr, 0, sizeof(DEVICES) );
		if( dptr==NULL )
			return( FALSE );
...
LOCAL BOOLEAN CreateSpeedlist( DEVICES *dev )
{
	if ( (dev->speeds = (LONG *)STiKmalloc( MAX_SPEEDS * sizeof( LONG * ) )) == NULL)
		return (FALSE);

	memset( dev->speeds, 0, MAX_SPEEDS * sizeof( LONG * ) );
