/*
falcon.c (module de base pour falcon) (mode 256 couleurs)
...
Pour le son, vous pouvez choisir 2 implementations:
- Son gere par interruption
- Son DMA (meilleur, mais frequence egale a 12.5, 25 ou 50 khz)
  (pour avoir le son DMA definissez DMASOUND)
...
NE PAS UTILISER CETTE EXTENSION POUR DES SOFTS DU DOMAINE PUBLIC!
...
pcstart.o, exemple.c, falcon.c et falcon2.s ensemble
afin d'obtenir l'executable exemple.prg.

falcon2.s peut etre assemble avec l'assembleur devpac 2
...
=
pcstart.o
falcon2.s (ou falcon2.o si produit par devpac)
falcon.c
exemple.c
...
/* KEYMAP est aussi defini dans falcon2.s */
...
static int initscreen(void)
{
	...
	bigbloc=(long)memalloc(76800L+255L);
	if (bigbloc)
	{
	...
	{
		case 3:
			nbcol = (int)xbios(94,0,256,bakpal);
			xbios(94,0,nbcol,bakpal);		/* Sauvegarde palette */
			bloc32k = gemdos(0x15,76800L);
			...
			if(sblon())					/* Est-il en fonction ?*/
			{
				...
				if(mon_type!=0)
				{
					if(mon_type==2)	/* Moniteur VGA */
						myresbloc.chglist=&chg_vga256;
					else			/* tele ou ecran */
						myresbloc.chglist=&chg_rgb256;
			...
			if(sblres == 0L)
			{
				...
				xbios(5,bloc32k,bloc32k,3,0x8003|(Vsetmode(-1)&0x20));
				if ((int)xbios(89)==2) roffset=6400L;
			}
			break;
		default:
			return(0);
...
{
	hide();
	bmove((void *)pt_ecr_actif,(void *)pt_ecr_travail,64000L);
	show();
}
...
void plot(int x,int y,int c)
{
	...
	if (c>=0) setcolor(c);
	ptr=(long *)(pt_ecr_travail+y*320L+(x&0xfff0L));
	nm=0x80008000L;
	nm>>=x&15;
	m=~nm;
	*(ptr++)=(*ptr&m)|(nm&ma);
...
void vline(int x,int y,int y1,int c)
{
	...
	nm>>=x&15;
	m=~nm;
	for(i=y;i!=y1+dy;i+=dy,ptr+=dy*80-4)
	{
		*(ptr++)=(*ptr&m)|(nm&ma);
		*(ptr++)=(*ptr&m)|(nm&mb);
		*(ptr++)=(*ptr&m)|(nm&mc);
...
void hline(int x,int y,int x1,int c)
{
	...
	if (c>=0) setcolor(c);
	ptr=(long *)(pt_ecr_travail+y*320L+cx*16L);
...
void getbloc(void **blocptr,int x,int y,int lx,int ly)
{
	...
	if (*blocptr)
	{
		...
		if (bloc->id)
		{
			...
			matrice=bloc->bloc;
			ecran=pt_ecr_travail+((long)x)+y*320L;
			haut=ly-1;
			larg=(lx/16)-1;
			recal=(char*)(8*(40-(lx/8)));
					...
					int lxs,int lys)
{
	...
	if (*blocptr)
	{
		...
		if (bloc->id)
		{
			...
			ecran=pt_ecr_travail+(long)(x&0xfff0)+y*320L;
...
void affchar(int x,int y,char c)
{
	...
	if ((c>31)&&(c<=126))
	{
		...
		ecran=pt_ecr_travail+(x&0xfff0)+y*320L;
...
void printchar(char c)
{
	...
	{
		ycur-=8;
		bmove((void *)(pt_ecr_travail+8*320L),(void *)pt_ecr_travail,192*320L);
		t=color;
		pbox(0,192,320,8,0);
		setcolor(t);
	}
