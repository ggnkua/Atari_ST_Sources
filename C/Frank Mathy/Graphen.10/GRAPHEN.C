...
char *wtitel = " Graphentheorie-Programm V 1.0 von Frank Mathy/TOS (C) 1991";
...
void make_alist(void)				/* Erstelle Adjazenzliste	*/
	{
	int z,s,i;
	for(i=0; i<ezahl; i++) azahl[i]=0;		/* Liste leeren		*/
	for(z=0; z<ezahl; z++)
		for(s=0; s<ezahl; s++)
			if((am[z][s])&&(s!=z))
				alist[z][azahl[z]++]=s;	/* Setzen	*/
	}

unsigned char tsn[MAXECKEN];			/* Suchposition			*/
int nummer,iter;

void tsuch(int vor,int von)					/* Rekursiver Algorithmus	*/
	{
	int zu,i;
	tsn[von]=nummer++;								/* Eintragen*/
	for(zu=0; zu<azahl[von]; zu++)
		if(!tsn[alist[von][zu]])
			{
			show[von][alist[von][zu]]=2;			/* Fett		*/
			if(!digraph) show[alist[von][zu]][von]=2;
			wind_redraw(window,wx,wy,wb,wh);
			evnt_button(1,1,1,&i,&i,&i,&i);			/* Taste	*/
			noprell();
			tsuch(von,alist[von][zu]);				/* Suche...	*/
			}
	if(vor>=0)
		{
		show[vor][von]=1;							/* Normal	*/
		if(!digraph) show[von][vor]=1;
		wind_redraw(window,wx,wy,wb,wh);
		evnt_button(1,1,1,&i,&i,&i,&i);				/* Taste	*/
		noprell();	
		}
...
void tiefensuche(void)
	{
	int button,key,mx,my,ap,i,von;
	int z,s;
	
	wind_update(BEG_MCTRL);				/* GEM-AES informieren	*/
	
	make_alist();						/* Adjazenzliste erst.	*/
	for(z=0; z<ezahl; z++)
		for(s=0; s<ezahl; s++)
			if(am[z][s]) show[z][s]=3;
	wind_redraw(window,wx,wy,wb,wh);	/* Neuzeichnen			*/
	
	wind_set(window,WF_INFO,"Tiefensuche (Anfangssecke wÑhlen) - FÅr Ende hier klicken...");
	evnt_button(1,1,1,&mx,&my,&button,&key);/* Maustasten	*/
	noprell();
	ap=searchpoint(mx-wx,my-wy);				/* Punkt suchen		*/
	if((mx>=wx)&&(mx<wx+wb)&&(my>=wy)&&(my<wy+wh)&&(ap!=-1))
	{	
	wind_set(window,WF_INFO,"Tiefensuche wird durchgefÅhrt - Maustaste betÑtigen...");

	/* Jetzt folgt der Tiefensuche-Algorithmus					*/
	
	for(i=0; i<ezahl; i++) tsn[i]=0;
	nummer=1;
	tsuch(-1,ap);						/* FÅr Anfangspunkt		*/
	for(von=0; von<ezahl; von++)		/* Restlicher T.-Wald	*/
		if(!tsn[von]) tsuch(-1,von);	/* Tiefensuche ausfÅhren*/
...
void bsuch(void)
	{
	int button,key,mx,my,ap,i,j;
	int z,s,marken;
	char text[80];
	do	{
		sprintf(text,"Breitensuche, Tiefe %d - Maustaste betÑtigen...",iter);
		wind_set(window,WF_INFO,text);
		marken=0;							/* Flagge lîschen	*/
		for(i=0; i<ezahl; i++)				/* Alle Ecken		*/
			if(tsn[i]==iter)
				{
				for(j=0; j<azahl[i]; j++)	/* Liste durchsuchen*/
					if(!tsn[alist[i][j]])	/* Wenn unmarkiert	*/
						{
						tsn[alist[i][j]]=iter+1;/* Markieren	*/
						show[i][alist[i][j]]=2;	/* Fette Kante	*/
						if(!digraph) show[alist[i][j]][i]=2;
						wind_redraw(window,wx,wy,wb,wh);
						evnt_button(1,1,1,&mx,&my,&button,&key);
						noprell();
						show[i][alist[i][j]]=1;	/* Normale Kante*/
						if(!digraph) show[alist[i][j]][i]=1;
						marken=1;			/* Flagge setzen	*/
						}
				}
		iter++;
		} while(marken);
...
void breitensuche(void)
	...
	make_alist();						/* Adjazenzliste erst.	*/
	for(z=0; z<ezahl; z++)				/* Alles gestrichelt	*/
		for(s=0; s<ezahl; s++)
			if(am[z][s]) show[z][s]=3;
	wind_redraw(window,wx,wy,wb,wh);	/* Neuzeichnen			*/
	
	wind_set(window,WF_INFO,"Breitensuche (Anfangssecke wÑhlen) - FÅr Ende hier klicken...");
	evnt_button(1,1,1,&mx,&my,&button,&key);	/* Maustasten	*/
	noprell();
	ap=searchpoint(mx-wx,my-wy);				/* Punkt suchen	*/
	if((mx>=wx)&&(mx<wx+wb)&&(my>=wy)&&(my<wy+wh)&&(ap!=-1))
	{	
	/* Jetzt folgt der Breitensuche-Algorithmus					*/

	for(i=0; i<ezahl; i++)					/* Marken lîschen	*/
		tsn[i]=0;
	iter=1;									/* Erste Iteration	*/

	tsn[ap]=iter;							/* Anfangssuchbaum	*/
	bsuch();								/* Breitensuche		*/

	for(i=0; i<ezahl; i++)
		if(!tsn[i])
			{
			tsn[i]=iter;					/* Neuer Suchbaum	*/
			bsuch();						/* Breitensuche		*/
			}
...
void do_menu(void)							/* MenÅ-Bearbeitung	*/
	...
	switch(msg[4])
		...
		case INFORM:	i=0;						/* ZÑhler 0	*/
						...
						sprintf(text,"[1][ Graphentheorie-Programm V 1.0| (C) 1991 Frank Mathy/TOS | %d Ecken | %d Kanten ][ OK ]",ezahl,i);
						form_alert(1,text);
						break;
		...
		case PRIM:		prim();			/* Prim-Alg. fÅr MAB	*/
						break;
		case TSUCH:		tiefensuche();	/* Tiefensuche			*/
						break;
		case BSUCH:		breitensuche();	/* Breitensuche			*/
						break;
...
void zentral(void)
	...
	menu_ienable(menu,TSUCH,i);
	menu_ienable(menu,BSUCH,i);
