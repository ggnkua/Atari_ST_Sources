...
char *wtitel = " Graphentheorie-Programm V 0.7 von Frank Mathy/TOS (C) 1991";
...
int searchpoint(int x,int y)
	...
	if(ezahl) for(i=0; i<ezahl; i++)
		{
		dx=(long) ex[i] - x;			/* AbstÑnde berechnen	*/
		dy=(long) ey[i] - y;
...
void do_menu(void)							/* MenÅ-Bearbeitung	*/
	...
	switch(msg[4])
		...
		case INFORM:	i=0;						/* ZÑhler 0	*/
						...
						sprintf(text,"[1][ Graphentheorie V 0.7 | (C) 1991 Frank Mathy/TOS | %d Ecken | %d Kanten ][ OK ]",ezahl,i);
						form_alert(1,text);
						break;
...
void zentral(void)
	...
	menu_ienable(menu,WARSHALL,0);
	menu_ienable(menu,DIJKSTRA,0);
	menu_ienable(menu,FLOYD,0);
	menu_ienable(menu,PRIM,0);
