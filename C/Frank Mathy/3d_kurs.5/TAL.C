...
enum { M_SHADING } dispmode=M_SHADING;
...
void do_rotate(int rotstep,int rotpts,float *p)
	{
	int i,j,s;
	...
	for(s=1; s<=rotstep; s++)
		...
		for(i=0; i<rotpts-1; i++)		/* FlÑchen hinzufÅgen	*/
			{
			addtriangle(&pa[i][0],&pb[i][0],&pa[i+1][0]);
			addtriangle(&pb[i][0],&pb[i+1][0],&pa[i+1][0]);
			}
...
int getobject(void)
	{
	int cmd,obnr,test,rotpts,rotstep,i;
	float k[MAXSTEP*2],radius,w;
	...
	while(((cmd=befehl_einlesen())!=ENDOBJECT)&&(cmd!=FILEEND))
		...
		switch(cmd)							/* Zeichenbefehl	*/
			{
			case TRIANGLE:	test=werte_einlesen(&k[0],9);
							if(test==ERROR)
								{
							...
							break;
			case RECTANGLE:	test=werte_einlesen(&k[0],12);
							if(test==ERROR)
								{
							...
							break;
			case ROTATE:	test=werte_einlesen(&k[0],2);
							rotstep=(int) k[0];
							rotpts=(int) k[1];
							...
							break;
			case BALL:		test=werte_einlesen(&k[0],2);
							if((test==ERROR)||(k[0]<3)||(k[0]>MAXSTEP))
								{
...
void shade_triangles(void)
	...
	for(tri=0; tri<disptri; tri++)
		...
		drawtri[tri].shade=(unsigned) ((1.0-color)*28.5);
		putch((int)'.');
		}
	}
...
/* Horizontale Linie ziehen von (x1,y,z1) nach (x2,y,z2) */
/* in der Farbe c										 */
void d_hline(int y,double x1,double z1,double x2,double z2,int c)
	{
	double fhilf;
	double tz;			/* TemporÑrer Wert fÅr z		*/
	double dz;			/* Delta-Wert fÅr z				*/
	...
	for(tix=(int) (x1+0.5), tz=z1; tix <= (int) (x2+0.5); tix++) 
		{			/* Zeile darstellen 				*/
		d_setpixel(tix, y,(int) tz,c);
		tz+=dz;
		}
	}

/* Dreieck mit den Koordinaten x[..], y[..], z[..] in Farbe c */
void d_drawtri(double *x,double *y,double *z,int c)
	{
	double fhilf;								/* Hilfsvariable		*/
	double tx0,tz0,dx0,dz0;						/* Werte Kante 0		*/
	double tx1,tz1,dx1,dz1;						/* Werte Kante 1		*/
	int ty;										/* Scanzeile			*/
	...
	while(ty < (int) y[midp])	   /* Bis oberere DreieckshÑlfte fertig */
		{
		d_hline(ty,tx0,tz0,tx1,tz1,c);    		/* Zeile darstellen		*/
		ty++;									/* NÑchste Zeile		*/
...
void draw_shadetri(int nr)
	...
	if((drawtri[nr].z[0]>0)&&(drawtri[nr].z[1]>0)&&(drawtri[nr].z[2]>0))
		...
		d_drawtri(x,y,z,drawtri[nr].shade);		/* Darstellen	*/
		}
	}
...
void main(int argc, const char *argv[])
	...
	printf("TOS-Animation-Language V2.0\n(C)1992 Frank Mathy\n\n");
