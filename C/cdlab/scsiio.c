...
/****************************************************************************
 *
 * Definitionen für SCSI-Calls in C
 *
 ...
 * virtuelles RAM wird berücksichtigt
 *
...
/* In und Out erst über eigene Prozeduren, wegen virtuellem RAM */
...
{{{
  if (!isinit)
  {
    ...
    if (HasVirtual)
    {
      /* bei virtuellem RAM müssen wir jetzt einen Puffer im ST-RAM allozieren */
      VirtBuffer = (char *)Mxalloc(64L*1024L, 0); /* 64 kB im ST-RAM */
      if ((LONG)VirtBuffer <= 0)
        {
          if (VirtBuffer == NULL)
          {
            /* ohne Puffer läuft hier nix */
            return FALSE;
          }
          else
          {
