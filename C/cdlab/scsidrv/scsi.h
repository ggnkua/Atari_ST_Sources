...
#include <portab.h>
#include <scsidrv/scsidefs.h>           /* Typen für SCSI-Lib */
...
#define DIRECTACCESSDEV  0       /* Gerät mit Direktzugriff (Festplatte) */
...
/* allgmeine Struktur für ModeSense/Select */
typedef struct{
  BYTE ModeLength;
  BYTE MediumType;
  UBYTE DeviceSpecs;  /* Geräteabhängig */
  BYTE BlockDescLen;
} tParmHead;
 ...
 * Variablen
 *****************************************************************************/
GLOBAL long ScsiFlags;   /* Wert für tScsiCmd.Flags */
...
LONG Inquiry(void  *data, BOOLEAN Vital, UWORD VitalPage, WORD length);
  /* Inquiry von einem Gerät abholen */
...
void SetBlockSize(ULONG NewLen);
  /*
   * SetBlockLen legt die Blocklänge für das SCSI-Gerät fest
   * (normalerweise 512 Bytes).
   */

ULONG GetBlockSize();
  /*
   * GetBlockLen gibt die aktuell eingestellte Blocklänge zurück.
   */


void SetScsiUnit(tHandle handle, WORD Lun, ULONG MaxLen);
  /*
   * SetScsiUnit legt das Gerät fest an das die nachfolgenden Kommandos
   * gesendet werden und wie lang die Transfers maximal sein dürfen.
   */


/*-------------------------------------------------------------------------*/
/*-                                                                       -*/
/*- Zugriff für Submodule (ScsiStreamer, ScsiCD, ScsiDisk...)             -*/
/*-                                                                       -*/
/*-------------------------------------------------------------------------*/
