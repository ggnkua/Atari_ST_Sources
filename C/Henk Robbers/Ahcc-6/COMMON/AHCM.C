...
#define LARGE 1
#define IFIRST 0
#define TEST_OUT_OF_MEMORY 0	/* If 1, right shift simulates out of memory condition */
...
void *new_block(XA_memory *base, long size, bool svcur, XA_key key, XA_key type)
{
	...
	new = nil;
	if (break_in)
		form_alert(1, "[0][Testing: | Out of memory][Ok]");
	else
#endif
	{
		if (base->mode)
...
global
void *XA_alloc(XA_memory *base, size_t size, XA_run_out *ranout, XA_key key, XA_key type)
{
	XA_block *blk = nil;
	bool large = false;
	long s;
	...
	if (s > base->chunk - blockprefix)
	{
		...
		blk = new_block(base, ns, false, key, type);
		if (!blk)
			return nil;
		large = true;
	}
	...
	if (!blk)
	{
		...
		blk = new_block(base, base->chunk, true, key, type);
	}
...
void *XA_realloc(XA_memory *base, void *area, size_t size, XA_run_out *ranout, XA_key key, XA_key type)
{
	...
	if (area eq nil)
		return XA_alloc(base, size, ranout, key, type);
	...
	if (blk)
	{
		...
		*/
		...
		if (new_size > old_size)
		{
			...
			new = XA_alloc(base, size, ranout, key, type);
			if (new)
			{
...
global
bool XA_leaked(XA_memory *base, XA_key key, XA_key type, XA_report *report, short nr)
{
...
static
void XA_follow(XA_memory *base, XA_block *blk, XA_list *list, XA_report *report, short nr)
{
	...
	while (un)
	{
		...
		if (!ptrok(n))
		{
			report(base, blk, n, "invalid next ptr", nr);
			return;
		}

		if ((long)n < (long)blk + blockprefix)
		{
			report(base, blk, n, "next ptr below block", nr);
			return;
		}

		if ((long)n > (long)blk + blk->size - unitprefix)
		{
			report(base, blk, n, "next ptr beyond block", nr);
			return;
		}
	...
	if (n ne list->last)
	{
		report(base, blk, n, "incistent next list", nr);
		return;
	}
	...
	while (un)
	{
		...
		if (!ptrok(n))
		{
			report(base, blk, n, "invalid prior ptr", nr);
			return;
		}

		if ((long)n < (long)blk + blockprefix)
		{
			report(base, blk, n, "prior ptr below block", nr);
			return;
		}

		if ((long)n > (long)blk + blk->size - unitprefix)
		{
			report(base, blk, n, "prior ptr beyond block", nr);
			return;
		}
	...
	if (n ne list->first)
	{
		report(base, blk, n, "inconsistent prior list", nr);
		return;
	}
}
...
void XA_sanity(XA_memory *base, XA_report *report, short nr)
{
	...
	while (blk)
	{
		...
		if (!ptrok(blk))
		{
			report(base, blk, nil, "invalid block pointer", nr);
			return;
		}
		...
		if (blk->magic ne XA_MAGIC)
		{
			...
			report(base, blk, nil, "not a block", nr);
			return;
		}
#endif
		if (blk->size < 0)
		{
			...
			report(base, blk, nil, "negative block size", nr);
			return;
		}
		...
		if (blk->size < blockprefix + unitprefix)
		{
			...
			report(base, blk, nil, "block size too small", nr);
			return;
		}
		...
		while (at < to)
		{
			if (!ptrok(at))
			{
				report(base, blk, (XA_unit *)at, "invalid unit ptr", nr);
				return;
			}
			...
			if (at->size < 0)
			{
				...
				report(base, blk, (XA_unit *)at, "negative unit size", nr);
				return;
			}
			...
			{
				printf("unit size %ld 0x%08lx at %ld blk 0x%08lx\n", at->size, at->size, (long)at - (long)blk, blk);
				report(base, blk, (XA_unit *)at, "unit size too small", nr);
				return;
			}
			...
			if ((long)at + at->size > (long)blk + blk->size)
			{
				...
				report(base, blk, (XA_unit *)at, "unit size too large", nr);
				return;
			}
		...
		if (at != to)
		{
			...
			report(base, blk, (XA_unit *)pr, "unit sizes dont fit block", nr);
			return;
		}

		XA_follow(base, blk, &blk->free, report, nr);
		XA_follow(base, blk, &blk->used, report, nr);
...
bool XA_present(XA_memory *base, void *area)
{
	if (!base)
		base = &XA_heap_base;
	return find_unit(base, area) ne nil;
}
...
static
void blocks(XA_memory *base, FILE *fp)
{
	XA_block *blk;
	long total = 0;
...
void pr_blocks(void)
{
	...
	if (fp)
	{
		blocks(&XA_heap_base, fp);
		blocks(&XA_local_base, fp);
		blocks(&XA_file_base, fp);
		blocks(&XA_global_base, fp);
	}
	Cconin();
}
