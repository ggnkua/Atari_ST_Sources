...
/* fÅr positive x. phi(x) ist die Ver-  */
...
/***************************************************/
/*                                                 */       
/* table erstellt eine Tabelle fÅr die Umkehrfunk- */
/* tion von phi(x).                                */ 
/*                                                 */
/***************************************************/
{
...
/*------Hierzu wird eine BinÑrsuche und lineare Interpolation--*/
/*------verwendet.---------------------------------------------*/
...
/*---Initialisierungen------------------------------------*/
  ...
  tfactr = 0.9;          /*---Reduktionsfaktor fÅr "Temperatur"--*/
  reduct = 0.5;          /*---Reduktionsfaktor fÅr sigma[]-------*/
...
/*---Lese Eingabedaten vom File INPUT.DAT-----------------*/
  ...
  level  = 1.0/(float)maxtry;   /*--Schwelle fÅr Akzeptanzverh.--*/
...
/*----Nominalwert akzeptiert werden soll.----------------------*/
      ...
      if (delta < 0.0)
        ...
        {
          idet = idet + 1;
          if (temp >= 0.000001)  /*--BerÅcksichtigung der einge- --*/
            {                    /*--schrÑnkten Genauigkeit des----*/
                                 /*--Datentyps float---------------*/
...
/*----PrÅfe, ob bereits genug ZufallsÑnderungen erfolgt sind.--*/

      if ((itry < maxtry) && (isucc < msucc)) goto con1;
      
/*----Falls ja, gehe zur nÑchstniedrigeren Temperatur.---------*/
...
/*----Gebe Optimierungsdaten des gegenwÑrtigen Iterations- ----*/
/*----schrittes aus.-------------------------------------------*/
