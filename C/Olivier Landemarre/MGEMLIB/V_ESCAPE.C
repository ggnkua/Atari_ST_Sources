 ...
 *
 *	06.01.2001 ol (olivier.landemarre@utbm.fr)
 *		- binding correction of vq_calibrate()
 *
 *11.01.2001 ol (olivier.landemarre@utbm.fr)
 *		speed optimisation for: v_bit_image(), v_output_window()
*/
...
void v_bit_image(int handle, char *filename, int aspect, int x_scale,
						int y_scale, int h_align, int v_align,	INT16 *pxyarray)
{
	int n;
...
void v_output_window(int handle, INT16 *pxyarray)
{
/*	vdi_ptsin[0] = pxyarray[0];
	vdi_ptsin[1] = pxyarray[1];
	vdi_ptsin[2] = pxyarray[2];
...
void v_trays(int handle, int input, int output, INT16 *set_input, INT16 *set_output)
{
	vdi_intin[0] = input;
	vdi_intin[1] = output;
	vdi_control[0] = 5;
...
int vq_calibrate (int handle, INT16 *flag)
{
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 0;	/* mod 1 a priori! puisque pas d'entr‚e! */
	vdi_control[5] = 77;
	vdi_control[6] = handle;
	vdi(&vdi_params);
	*flag = vdi_intout[0];
...
void vq_scan(int handle, INT16 *g_slice, INT16 *g_page, INT16 *a_slice, INT16 *a_page, INT16 *div_fac)
{
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 0;
...
void vq_tray_names(int handle, char *input_name, char *output_name, INT16 *input, INT16 *output)
{
...
int vs_calibrate (int handle, int flag, INT16 *rgb)
{
	*((INT16 **)(&vdi_intin[0])) = rgb;
	vdi_intin[2] = flag;
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 3;
...
*/
void vq_tdimensions(int handle, INT16 *xdimension, INT16 *ydimension)
{
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 0;
...
void vt_axis(int handle, int xres, int yres, INT16 *xset, INT16 *yset)
{
	vdi_intin[0] = xres;
	vdi_intin[1] = yres;
	vdi_control[0] = 5;
...
void vt_resolution( int handle, int xres, int yres, INT16 *xset, INT16 *yset)
{
	vdi_intin[0] = xres;
	vdi_intin[1] = yres;
	vdi_control[0] = 5;
...
void v_write_meta(int handle, int numvdi_intin, INT16 *avdi_intin, int num_ptsin, INT16 *a_ptsin)
{

	vdi_params.intin = (INT16 *) &avdi_intin[0];
	vdi_params.ptsin = (INT16 *) &a_ptsin[0];
...
void vq_chcells(int handle, INT16 *n_rows, INT16 *n_cols)
{
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 0;
...
void vq_curaddress(int handle, INT16 *cur_row, INT16 *cur_col)
{
	vdi_control[0] = 5;
	vdi_control[1] = 0;
	vdi_control[3] = 0;
