		   ...
		   unsigned char *color_indexes,int *color_table)
{
  ...
  if (nb_colors < (_R * _G * _B)) {
   /* fprintf(stderr,"zdither: not enough colors\n");*/
    if(!gl_exception_error(12L)) Cconws("zdither: not enough colors\015\012");
    exit(1);
  }
  for(i=0;i<nb_colors;i++) color_table[i]=0;
  zb->nb_colors=nb_colors;
  zb->ctable=my_Malloc(nb_colors * sizeof(int));
  if(zb->ctable==NULL) return;
  for (r = 0; r < _R; r++) {
    for (g = 0; g < _G; g++) {
      for (b = 0; b < _B; b++) {
	r1=(r*255) / (_R - 1);
  ...
  zb->dctable=my_Malloc( DITHER_TABLE_SIZE );
  if(zb->dctable==NULL) return;
