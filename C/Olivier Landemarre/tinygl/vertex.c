...
void glopColor(GLContext *c,GLParam *p)
{
  ...
  c->longcurrent_color[0]= p[5].ui;/* voir calcul v‚ritable dans clip.c  gl_transform_to_viewport() */
...
{
  GLViewport *v;
  Tiny_double zsize=(1<<(ZB_Z_BITS+ZB_POINT_Z_FRAC_BITS));
  v=&c->viewport;
  ...
  if (c->lighting_enabled) {
    ...
    m=&c->matrix_model_projection.m[0][0];
    	...
    	v->pc.Z = (v->coord.X * m[8] + v->coord.Y * m[9] + 
               v->coord.Z * m[10] + m[11]);
      if((m[12]==0.0f)&&(m[13]==0.0f)&&(m[14]==0.0f)/*c->matrix_mode==GL_PROJECTION*/)
      {	/* cas sans perspective ! */
      	v->pc.W =m[15];
      }
      else
      {
...
void glopVertex(GLContext *c,GLParam *p)
{
  ...
  if (n >= c->vertex_max) {
    ...
    newarray = my_Malloc(sizeof(GLVertex)*c->vertex_max);
    if (!newarray) {
    	gl_exception_error(2L);
  ...
  if (c->lighting_enabled) {
    ...
    v->color=c->current_color;        plus besoin ! 
  }*/
  /* tex coords */
  ...
  switch( c->begin_type) {
   ...
   case GL_TRIANGLE_STRIP:
    if (cnt>=3) {  
      if (n==3) n=0;
      switch(cnt&1)  /* correction de sens des triangles pour respecter GL_FRONT et GL_BACK */
      {
      	case 0:
