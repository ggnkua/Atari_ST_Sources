...
void glopRotate(GLContext *c,GLParam *p)
{
  ...
  default:
    {
      Tiny_double cost, sint;
      /* normalize vector */
      Tiny_double len = u[0]*u[0]+u[1]*u[1]+u[2]*u[2];
      if (len == 0.0f) return;
      len = 1.0f / sqrt(len);
      u[0] *= len;
      u[1] *= len;
...
{
  Tiny_double *m;
  Tiny_double x=p[1].f,y=p[2].f,z=p[3].f;
...
void glopFrustum(GLContext *c,GLParam *p)
{
  Tiny_double *r;
  M4 m;
  Tiny_double left=p[1].f;
  Tiny_double right=p[2].f;
  Tiny_double bottom=p[3].f;
  Tiny_double top=p[4].f;
  Tiny_double near=p[5].f;
  Tiny_double farp=p[6].f;
  Tiny_double x,y,A,B,C,D;
