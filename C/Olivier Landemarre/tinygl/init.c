...
void initSharedState(GLContext *c)
{
  ...
  s->lists=my_calloc(sizeof(GLList *),MAX_DISPLAY_LISTS);
  s->texture_hash_table=
    my_calloc(sizeof(GLTexture *),TEXTURE_HASH_TABLE_SIZE);
  alloc_texture(c,0);
}
...
void glInit(void *zbuffer1)
{
  ...
  GLViewport *v;
  int i;
  c=my_calloc(1,sizeof(GLContext));
  if(c==NULL) return;
  gl_ctx=c;
  c->zb=zbuffer;
  /* allocate GLVertex array */
  c->vertex_max = POLYGON_MAX_VERTEX;
  c->vertex = my_Malloc(POLYGON_MAX_VERTEX*sizeof(GLVertex));
  ...
  for(i=0;i<3;i++) {
    c->matrix_stack[i]=my_calloc(c->matrix_stack_depth_max[i],sizeof(M4));
