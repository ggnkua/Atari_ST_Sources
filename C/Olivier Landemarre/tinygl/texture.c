...
static void free_texture(GLContext *c,int h)
{
  ...
  for(i=0;i<MAX_TEXTURE_LEVELS;i++) {
    im=&t->images[i];
    if (im->pixmap != NULL) my_Free(im->pixmap); /* les textures apparatiennent dor‚navant au client sans transformation si origine 256*256*/
  }
  my_Free(t);
}
GLTexture *alloc_texture(GLContext *c,int h)
{
  ...
  t=my_calloc(1,sizeof(GLTexture));
 ...
 /* do_free=0;*/
  ...
  if (width != 256 || height != 256) {
   ...
   /* do_free=1;*/
    ...
    memcpy(pixels1,pixels,256*256*3);
  }
  im=&c->current_texture->images[level];
  im->xsize=width;
  im->ysize=height;
  if (im->pixmap!=NULL) my_Free(im->pixmap);
 /* im->pixmap=my_Malloc(width*height*2);
  gl_convertRGB_to_5R6G5B(im->pixmap,pixels1,width,height);
  on travaille dorenavant en RVB 24bits
  */
  im->pixmap=pixels1; /* image 24 bits directe */
  
/*  if (do_free) my_Free(pixels1); faut maintenant le garder puisque RVB 24*/
}
...
void glopTexEnv(GLContext *c,GLParam *p)
{
  ...
  error:
  	 if(gl_exception_error(8L)) return;
