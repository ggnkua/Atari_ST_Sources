...
void gl_transform_to_viewport(GLContext *c,GLVertex *v)
{
 /* float winv;*/
  ...
  {
  		v->zp.x= (int) ( v->pc.X * c->viewport.scale.X 
  	                + c->viewport.trans.X );
  		v->zp.y= (int) ( v->pc.Y * c->viewport.scale.Y 
                   + c->viewport.trans.Y );
  		v->zp.z= (int) ( v->pc.Z * c->viewport.scale.Z 
                   + c->viewport.trans.Z );
  }
  
  /* color */
  if (c->lighting_enabled)
  {  /* avant passait tj par la pas de if() */
...
static double clip_xmin(V4 *c,V4 *a,V4 *b)
{
...
static double clip_xmax(V4 *c,V4 *a,V4 *b)
{
	double t,dX,dY,dZ,dW,den;
  dX=(b->X-a->X);
  dY=(b->Y-a->Y);
...
static double clip_ymin(V4 *c,V4 *a,V4 *b)
{
	double t,dX,dY,dZ,dW,den;
  dX=(b->X-a->X);
  dY=(b->Y-a->Y);
...
static double clip_ymax(V4 *c,V4 *a,V4 *b)
{
	double t,dX,dY,dZ,dW,den;
  dX=(b->X-a->X);
  dY=(b->Y-a->Y);
...
static double clip_zmin(V4 *c,V4 *a,V4 *b)
{
	double t,dX,dY,dZ,dW,den;
  dX=(b->X-a->X);
  dY=(b->Y-a->Y);
...
static double clip_zmax(V4 *c,V4 *a,V4 *b)
{
	double t,dX,dY,dZ,dW,den;
  dX=(b->X-a->X);
  dY=(b->Y-a->Y);
                                  ...
                                  GLVertex *p0,GLVertex *p1,GLVertex *p2,int clip_bit)
{
  ...
  if (co == 0) {
    ...
    if (clip_bit == 6) {
      ...
      if(!gl_exception_error(101L)) Cconws("Clipping error\015\012");
      return;
    }
