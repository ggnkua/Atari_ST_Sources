...
#endif
#ifdef _GEMLIB_H_
	#define ldg_global	((WORD *)aes_global)
#else
	#ifdef __PUREC__
	#define ldg_global	((WORD *)_GemParBlk.global)
	#else
	#define ldg_global	((WORD *)aes_global)
	#endif
#endif
...
/* from version 0.99 */
	UWORD flags;			/* library flags ( shared, resident, locked) */
/* from version 0.99c */
...
typedef struct ldglib {
	...
	UWORD flags;			/* library flags (shared, locked, resident) */
