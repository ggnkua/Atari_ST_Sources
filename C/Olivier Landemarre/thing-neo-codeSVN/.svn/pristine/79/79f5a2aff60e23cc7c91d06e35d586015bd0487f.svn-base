...
static OBJECT sepline = { -1, -1, -1, (SEPARATOR << 8) | G_STRING, LASTOB, DRAW3D | NORMAL, "--------------------", 0, 1, 1 };
...
short tool_init(char *apname) {
	...
	if ((tb.sys & SY_MTOS) || (tb.sys & SY_MAGX) || (appl_find("?AGI\0\0\0\0") >= 0)) {
		...
		if (appl_getinfo(2, &out[0], &out[1], &out[2], &out[3]) == 1) {
			...
			if (out[3] & 1) /*(out[3] == 1) correction OL 2/02/2023 */
				tb.sys |= SY_NRSC; /* Neues RSC-Format vorhanden */
		}
	}

	/* Systemsprache ermitteln */
	tb.sysLanguageId = getSystemLanguage(tb.sysLanguageCode, tb.sysLanguageCodeLong, apname );
	...
	if (tb.sys & SY_AGI) {
		...
		if (appl_getinfo(12, &out[0], &out[1], &out[2], &out[3]) == 1) {
			...
			if (out[0] & 0x0010)/*(out[0] & 0x0008) correction OL 2/02/2024 */ {
				tb.sys |= SY_SHUT;
		...
		if (appl_getinfo(11, &out[0], &out[1], &out[2], &out[3]) == 1) {
			...
			if ((out[0] & 0x0080) && (out[2/*3*/] & 0x0001)){
				tb.sys |= SY_ICONIFY;
}
			/* WF_OWNER vorhanden? */
			if (out[0] & 0x10)
	...
	if(!(tb.sys & SY_MSHELL)&& (tb.sys & SY_ASEARCH)) {
		char name[100]; short type, ap_id = 0 , retour ;
		retour = appl_search(2,name,&type,&ap_id);
		if((retour == 1) && (ap_id == tb.app_id)) tb.sys |= SY_MSHELL;
		if ((retour == 0) || (ap_id == 0)) {
			retour = appl_search(APP_FIRST,name,&type,&ap_id);
			while((retour == 1) && (ap_id != tb.app_id)) {
				retour = appl_search(APP_NEXT,name,&type,&ap_id);
			}
			if( (ap_id == tb.app_id) && (type &APP_SHELL) ) tb.sys |= SY_MSHELL;
		}
	}
...
void pop_set(FORMINFO *fi, POPMENU *pop) {
	...
	if (newtype == -1)
		newtype = popup->ob_type&0xFF;

	oldtype = form->ob_type&0xFF;

	if(newtype==G_USERDEF) { /* ajout OL le 2/02/2024 */
		UBPARM *ubparm;
		USERBLK *userblk=popup->ob_spec.userblk;
		ubparm = (UBPARM *) userblk->ub_parm;
		newtype = ubparm->ob_type & 0xFF;
		if ((oldtype == G_TEXT) || (oldtype == G_BOXTEXT)) {
		form->ob_spec.tedinfo->te_ptext	= (newtype == G_STRING) ? ubparm->ob_spec.free_string
						: ubparm->ob_spec.tedinfo->te_ptext;
		}
		else Cconws("USERDEF not expected\015\012");
		return;
	}

	
	if ((oldtype == G_TEXT) || (oldtype == G_BOXTEXT)) {
		form->ob_spec.tedinfo->te_ptext	= (newtype == G_STRING) ? popup->ob_spec.free_string
...
short poplist_handle(POPLIST *pop, short px, short py, short width, short oh, short *pok,
	...
	static char tmp[256]; /* modifi‚ le 30/01/2024 OL passage en static car setObjectText transfere les adresses !*/
	tree = rs_trindex[0];
...
static void bubble_message(short id, short obj, short mx, short my) {
	...
	if (global_txt == NULL) {
		if (tb.sys & SY_XALLOC)
			global_txt = (char *)Mxalloc(256, 0x23); /* readable */
		else
...
short fselect(char *fs_einpath, char *fs_einsel, short *fs_eexbutton, char *elabel,
	...
	if (!fdm)
		fdm = (Fdm_Str *)Mxalloc(sizeof(lpath), (tb.sys & SY_XALLOC) ? 0x23 : 0x3);
