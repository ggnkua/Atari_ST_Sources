...
void de_config(short mode, short ret) {
	...
	if (!mode) {
		...
		case COIMGAPPLY:
			if ((glob.sysinfo&3)==1)/* MyAES with default background */
			{
				(void) shel_write(96,0,0,dconf->imgname,"");
				glob.sysinfo |=4 ;
			}
			else
			{
		...
		case COFIMG:
			if (dconf->mode == 0) {
				...
				{
					if (full_fselect(fname, NULL, "*.JPG", 0, rs_frstr[TXIMGSEARCH], 0, 301, &fi_config) == 1) {
						strcpy(dconf->imgname, fname);
						strShortener(objectTree[COIMG].ob_spec.tedinfo->te_ptext, dconf->imgname, 30);
						frm_redraw(&fi_config, COIMG);	
					}
...
void dl_saveconf(void) {
	...
	if(speckey&8) {
		...
		strcat(path,"*.inf");
		strcpy(file, "thing.inf");
		fselect(path, file, &ret, "config file name", 0, 0);
		if ((ret == 1) && *path && *file) {
			graf_mouse(BUSYBEE, 0L);
			ret = conf_save(0, file);
			graf_mouse(ARROW, 0L);
		}
		return;
	}
...
void dl_loadconf(void) {
	short ret, i, speckey, loadname=0;
	...
	if (loadname || frm_alert(1, rs_frstr[ALDOLOADCONF], altitle, conf.wdial, 0L) == 1) {
		graf_mouse(BUSYBEE, 0L);
		...
		if(loadname) ret = conf_load(file);
		else ret = conf_load(NULL);
		dl_drives(0, 1, 0);
...
void dl_iconload(void) {
	...
	if (frm_alert(1, rs_frstr[ALLOADICON], altitle, conf.wdial, 0L) == 1) {
		...
		/* Icon-Resource laden */
/*		if(tb.desk.g_h>1000)
			sprintf(rname, "%s%s\\icons\\big\\%s", tb.homepath, PNAME_RSC, FNAME_RSC);
		else 
			sprintf(rname, "%s%s\\icons\\small\\%s", tb.homepath, PNAME_RSC, FNAME_RSC);*/
		create_path_icon(rname, FNAME_RSC);
...
void dl_changeres(void) {
	char sname[MAX_PLEN], *p=NULL, *args=NULL;
	...
	if (!file_exists(p, 1, 0L)) {
		(void)shel_envrn(&args,"RSMASTER_ARGS=");
...
void dl_changeres_nomagic(void) {
	char *p=NULL, *args=NULL, sname[MAX_PLEN];
	XATTR xattr;
	short atype, is_gr;
...
void dl_shutdown(void) {
	...
	if ((p = get_xxmaster("SDMASTER")) == 0L) p = sname;
	if (!file_exists(p, 1, 0L)) {
/*		shel_write(1, 1, SHW_SINGLE, p, "\4-1 0");
		shel_write(1, 1, SHW_CHAIN, p, "\4-1 0");*/
		shel_write(SWM_LAUNCH, 0, 0, p, "\4-1 0");
/*		glob.done = 2; */
	} else {
		aesmsg[0] = AP_TFAIL;
		aesmsg[1] = 1;
