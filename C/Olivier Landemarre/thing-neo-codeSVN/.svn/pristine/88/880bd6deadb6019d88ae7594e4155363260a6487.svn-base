...
typedef struct {
	...
	unsigned char	dta_attribute;		/* file attribute */
	unsigned short	dta_time;			/* file time stamp */
	unsigned short	dta_date;			/* file date stamp */
	unsigned long	dta_size;		/* file size */
	char			dta_name[255];	/* file name */
} _DTA;
#endif
...
static short get_next_file(char *dirpath, GNF *handle, short follow, char *name,
	...
	if (dirpath != NULL) {
		...
		handle->old = (_DTA *)Fgetdta();
		(void)Fsetdta((DTA *)&handle->dta);
		strcpy(handle->full, dirpath);
		if (strrchr(handle->full, 0)[-1] != '\\')
			strcat(handle->full, "\\");
		strcat(handle->full, "*.*");
		if (Fsfirst(handle->full, 0x17)) {
			(void)free(handle->full);
			(void)Fsetdta((DTA *)handle->old);
			handle->magic = 0L;
			return (0);
		}
...
/*fprintf(stderr, "\n7.1 (namelen %d; dir: %ld, name: %s)", namelen, handle->dir, name);*/
			err = Dxreaddir(namelen, handle->dir, name, (XATTR *)xattr, &xerr);
			if (err == -32L)
				handle->dxreaddir = 0;
			else if (err || xerr) {
gnf_derror:
