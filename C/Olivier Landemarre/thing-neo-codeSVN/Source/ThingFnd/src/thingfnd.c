...
void di_find(void) {
	...
	*p = 0;
	...
	if(find.filemask==NULL) find.filemask="*.*";
	...
	if(find.contentmask==NULL) find.contentmask="";  
...
static unsigned long __CDECL the_time(void)
{
	return(*hz_200);
}

static int test_time_update(void)
{
	static unsigned long time_tick=0L;
	unsigned long cur_tick;


	cur_tick = (unsigned long) Supexec((long (*)())the_time);
	if(cur_tick-50L>time_tick) /* Minimum 250ms between calls */
	{
		time_tick = cur_tick;
		return 1;
	}
...
/*	if (!init_cicon())
	...
	{ BYTE code[6], locname[50];
		(void) getSystemLanguage(code, locname, "thgfnd" );
		/* Resource laden */
		sprintf(rsrcName, "%s%s\\thgfnd\\%s.rsc", tb.homepath, PNAME_RSC, locname);
		if (!rsc_load(rsrcName, &rinfo)) {
			sprintf(rsrcName, "%s%s\\thgfnd\\thgfnd.rsc", tb.homepath, PNAME_RSC);
			if (!rsc_load(rsrcName, &rinfo)) {
				frm_alert(1, "[3][THGFND.RSC nicht gefunden!|THGFND.RSC not found!][ OK ]", altitle, 1, 0L);
				return (FALSE);
			}
		}
	}
	...
	if (tb.sys & SY_XALLOC)
		aesapname = (char *)Mxalloc(9 + MAX_AVLEN, 0x23);
	else
		aesapname = (char *)Malloc(9 + MAX_AVLEN);
	/* Bei Fehler raus */
	if (!aesapname) {
