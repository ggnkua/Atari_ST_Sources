...
int strcasecmp(const char *scan1, const char *scan2);
...
short mw_init(void) {
	...
	for (i = 0; i < glob.numicon; i++) {
		obj = &glob.rtree[i + 1];
		if ((obj->ob_type == G_ICON)||(obj->ob_type == G_CICON)) {
...
short main_init(void) {
	short i, l, x, y, w, h;
	short illtype, type;
	char *p=NULL, aname[9], rsrcName[250];
	OBJECT *tree;
...
/*	if (!init_cicon())
  ...
  { BYTE code[6], locname[50];
		(void) getSystemLanguage(code, locname, "thgicn" );
		/* Resource laden */
		sprintf(rsrcName, "%s%s\\thgicn\\%s.rsc", tb.homepath, PNAME_RSC, locname);

		if (!rsc_load(rsrcName, &rinfo)) {
			sprintf(rsrcName, "%s%s\\thgicn\\thgicn.rsc", tb.homepath, PNAME_RSC);
			if (!rsc_load(rsrcName, &rinfo)) {
...
/*		if (i == MAINMENU)
	...
	if (tb.sys & SY_XALLOC)
		aesapname = (char *)Mxalloc(9 + MAX_AVLEN, 0x42);
	else
