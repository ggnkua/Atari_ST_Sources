...
static void change_ext(char *path,char *ext) {
	char *last_point=NULL;
	while(*path) {
		if(*path=='.') last_point = path;
		if((*path=='\\')||(*path=='/')) last_point = NULL;
		path++;
	}
	if(last_point==NULL) last_point=path;
	*last_point++ = '.';
	while(*ext) {
		*last_point++ = *ext++;
	}
...
/*-------------------------------------------------------------------------
 ...
 -------------------------------------------------------------------------*/
short conf_save(short tmp, char *othername) {
	FILE *fh;
	...
	if (((glob.sysinfo&3)==1) && (glob.sysinfo&4)) { /* MyAES with default background / New image background */
		(void) shel_write(96,2,0,"",""); /* save cache background desktop */
		glob.sysinfo &= ~4;
	}
	...
	if (tmp)
		...
		if(othername) fname = othername;
		else fname = FNAME_INF; /* Normale Datei */
	...
	if (fh) {
		...
		ret = 0L;
		exist = !file_exists(name, 1, &xattr);
...
short conf_load(char *othername) {
	FILE *fh;
	...
	/* Datei îffnen */
	fh = fopen(name, "r");
	if(!fh) { /* on retente en local permet de rÇcupÇrer les config sans HOME*/
		char *locp=NULL;
		if(othername) return ok;
		(void)shel_envrn(&locp,"THINGDIR=");
		if (locp) { short longueur;
			strcpy(name, locp);
			longueur = (short) strlen(name) - 1;
			if ((longueur > 0) && (name[longueur] != '\\')) strcat(name, "\\");
			strcat(name, PNAME_CON);
			strcat(name, "\\");
			strcat(name, FNAME_INF);
			fh = fopen(name, "r");
		}
...
/*        fprintf(stderr, "fileopen = %s, fileview = %s, fileprint = %s\n", aptr->fileopen, aptr->fileview, aptr->fileprint);*/
		...
		strcat(name, glob.rname);
		strcat(name, "\\");
		if(othername) strcat(name, othername);
		else strcat(name, FNAME_INF);
		change_ext(name,"dsk");
...
short conf_iload(void) {
	...
	/* Dateinamen erzeugen */
/*	if(tb.desk.g_h>1000)
		sprintf(name, "%s%s\\icons\\big\\%s", tb.homepath, PNAME_RSC, FNAME_ICN);
	else 
		sprintf(name, "%s%s\\icons\\small\\%s", tb.homepath, PNAME_RSC, FNAME_ICN);*/
		create_path_icon(name, FNAME_ICN);
