...
void icon_update(short n) {
	...
	for (i = s; i <= e; i++, q++) {
		...
		if (q->class != IDFREE) {
			...
			if (conf.isnap) {
				if(tb.desk.g_w>800) { /* high resolution grid 8x8 pixels */
					x = q->x;
					y = q->y;
					q->x = (x + 7) & 0xfff0;
					q->y = (y + 7) & 0xfff0;
				...
				else {  /* low resolution grid 4x4 pixels */
					x = q->x;
					y = q->y;
					q->x = (x + 3) & 0xfff8;
					q->y = (y + 3) & 0xfff8;
			...
			if (y + h > tb.desk.g_y + tb.desk.g_h) {
					if(q->class!=IDPRT && q->class!=IDCLIP && q->class!=IDTRASH) y = tree[i].ob_y = tb.desk.g_h - h;
					rd = 1;
			}
