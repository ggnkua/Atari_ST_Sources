...
#if 0
OBJECT *get_tree(int numtree, RSINFO *rinfo)
{ /*OBJECT *obj=NULL;
	(void)rsc_gaddr(R_TREE, numtree, &obj,rinfo);
	return obj;*/
	return rs_trindex[numtree];
}

char *get_frstr(int numchaine, RSINFO *rinfo)
{ char *chaine=NULL;
	(void)rsc_gaddr(R_STRING, numchaine, &chaine,rinfo);
	return chaine;
}
#endif

static short is_color_icon(void) /* return  if color icons are supported */
{	short ap_gout[5];
	
	ap_gout[3]=0;
	if (tb.sys & SY_AGI) {	
		if(appl_getinfo(2,&ap_gout[1],&ap_gout[2],&ap_gout[3],&ap_gout[4])==0) ap_gout[3]=0; /* no color icons */
 	}
	else {
		if (_AESversion >= 0x340) ap_gout[3]=1;
	}

	return ap_gout[3];
}

void create_path_icon(char *nameout, char *name_file)
{
	if((tb.planes==2) && (tb.desk.g_h<300) && (tb.desk.g_w>600) ) /* typically STMid */
		sprintf(nameout, "%s%s\\icons\\stmid\\%s", tb.homepath, PNAME_RSC, name_file);
	else
	if(is_color_icon()==0) {
		sprintf(nameout, "%s%s\\icons\\b&w\\%s", tb.homepath, PNAME_RSC, name_file);
	}
	else
	if (!(tb.sys & SY_MULTI)) {
		sprintf(nameout, "%s%s\\icons\\colortos\\%s", tb.homepath, PNAME_RSC, name_file);
	}
	else
	{
	  char dummy[257];
		if((tb.sys&SY_MTOS) && !(tb.sys & (SY_MAGX|SY_NAES|SY_GNVA)) & !appl_getinfo_str(96,dummy,dummy,dummy,dummy) ) sprintf(nameout, "%s%s\\icons\\colortos\\%s", tb.homepath, PNAME_RSC, name_file);
		else
		if(tb.desk.g_h>1000)
			sprintf(nameout, "%s%s\\icons\\big\\%s", tb.homepath, PNAME_RSC, name_file);
		else 
			sprintf(nameout, "%s%s\\icons\\small\\%s", tb.homepath, PNAME_RSC, name_file);
		
	}
	
}

static void getname_app(char *nameout, char *name_file)
{
	if(is_color_icon()==0) {
		sprintf(nameout, "%sbw", name_file);
	}
	else strcpy(nameout,name_file);
}
...
short main_init(void) {
	short i, j, l, x, y, w, h, fok, rex/*,retour*/;
	char tbuf[34];
	char name[MAX_PLEN];
	char *p, *sp, rsrcName[250];
	WINOPEN *openwin;
	APPLINFO *aptr;
	OBJECT *tree;
	struct passwd *pwd;
...
DEBUGLOG((1, "main_init: Toolbox initialised, logging enabled\n"));
	if(tb.resx>800) MAXICON = 200L; 
	if(tb.resx>1280) MAXICON = 400L;
	name[0]=0;
	if (tb.sys & SY_AGI) { char dummy[257];
		(void)appl_getinfo_str(96,name,dummy,dummy,dummy);
		name[8] = 0;
		if(!strcmp("MyAeS",name)) 
		{	char *p=NULL;
			glob.sysinfo|=1; /* MyAES found */
			(void)shel_envrn(&p,"AES_NO_BACKGROUND=");
			if(p)
			{
				glob.sysinfo|=2;
			}
			(void)appl_control(tb.app_id,17,(void *)"app_notexture=true");
		}
	}
/*	{
		short handle;
...
/*	p = getenv("HOME");*/
  ...
  sprintf(almsg, "MAIN: glob.cpath=%s", glob.cpath); debugMain(almsg);
	...
	if (i >= 0)
		wind_delete(i);
	/* Resource laden */
		getname_app(newname, "thing");
		(void) getSystemLanguage(code, locname, newname );
		sprintf(rsrcName, "%s%s\\thing\\%s.rsc", tb.homepath, PNAME_RSC, (char *)locname /*tb.sysLanguageCodeLong*/);
	if (!rsc_load(rsrcName, &rinfo)) {
		sprintf(rsrcName, "%s%s\\thing\\thing.rsc", tb.homepath, PNAME_RSC);
		if (!rsc_load(rsrcName, &rinfo)) {
			sprintf(rsrcName, "%sthing.rsc", tb.homepath);
			if (!rsc_load(rsrcName, &rinfo)) {
				strcpy(tb.homepath,"C:\\thing\\");
				sprintf(rsrcName, "%s%s\\thing\\%s.rsc", tb.homepath, PNAME_RSC, (char *)locname );
				if (!rsc_load(rsrcName, &rinfo)) {
...
/*(void) rsc_gaddr(R_TREE, LANGUAGE, (void *)&tree,&rinfo);	 */
	 
		(void) getSystemLanguage(code, locname, "thgtxt" );
		sprintf(rsrcName, "%s%s\\thgtxt\\%s.rsc", tb.homepath, PNAME_RSC, (char *)locname);
		if (!rsc_load(rsrcName, &rinfo2)) {
			sprintf(rsrcName, "%s%s\\thgtxt\\thgtxt.rsc", tb.homepath, PNAME_RSC);
			if (!rsc_load(rsrcName, &rinfo2)) {
				sprintf(rsrcName, "%sthgtxt.rsc", tb.homepath);
				if (!rsc_load(rsrcName, &rinfo2)) {			
					frm_alert(1, "[3][THGTXT.RSC nicht gefunden!|THGTXT.RSC not found!][ OK ]", altitle, 1, 0L);
					conf.userdef = 0;							/* Keine RSC, dann muss auch kein USERDEF freigeben werden */
					return (FALSE);
				}
			}
	...
	/* if(tb.sys&SY_MINT)*//* Anpassungen an MiNT */
	{
		...
		Psignal(SIGINT, (__sig_handler *) 1L);
		Psignal(SIGSYS, (__sig_handler *) 1L);
		Psignal(SIGABRT, (__sig_handler *) 1L);
		Psignal(SIGTERM, (__sig_handler *) sigTerm);
/*
 ...
 */
		Psignal(SIGQUIT, (__sig_handler *) 1L);
		Psignal(SIGHUP, (__sig_handler *)  1L);
	}
...
if(1)
	...
	tree[ABLOGOLO].ob_height = tree[ABLOGOLO].ob_spec.bitblk->bi_hl;
	tree[ABLOGOLO].ob_spec.bitblk->bi_y = 0;
	if ((tb.ch_hb < 12) || (is_color_icon()==0)) {
		tree[ABLOGOHI].ob_flags |= HIDETREE;
		tree[ABLOGOLO].ob_flags &= ~HIDETREE;
/*		if(tb.ch_hb < 12)*/
		{
			if((tree[ABVER].ob_y - tree[ABLOGOLO].ob_y - tree[ABLOGOLO].ob_height)>0)
				tree[ABLOGOLO].ob_y = tree[ABLOGOLO].ob_y + ( (tree[ABVER].ob_y - tree[ABLOGOLO].ob_y - tree[ABLOGOLO].ob_height)/2);
			else {
				l = tree[ABLOGOLO].ob_y + tree[ABLOGOLO].ob_height - tree[ABVER].ob_y + 4;
				tree->ob_height += l;
				i = ABVER;
	...
	create_path_icon(name, FNAME_RSC);
	if (!rsc_load(name, &glob.rinfo)) {
		sprintf(name, "%s%s\\icons\\b&w\\%s", tb.homepath, PNAME_RSC, FNAME_RSC);
		if (!rsc_load(name, &glob.rinfo)) {
...
/*rsrc_gaddr(0,POPFSHORT,pop_fkeydgi.tree);
pop_fkeyfd.tree = pop_fkeydgi.tree;*/
	pop_mem.tree = rs_trindex[POPPMEM];

/*rsrc_gaddr(0,POPPMEM,pop_mem.tree);*/
	...
	if(MAXICON>92) { OBJECT *newdesk;
		newdesk = (OBJECT *) pmalloc(sizeof(OBJECT)*(long)(MAXICON + 2));
		if (newdesk) { int i;
			memcpy((char *)newdesk, (char *)rs_trindex[DESKTOP], sizeof(OBJECT)*(92+1));
			rs_trindex[DESKTOP] = newdesk;
			newdesk->ob_tail = MAXICON;
			for(i=92; i<=MAXICON; i++) {
				memcpy((char *)&newdesk[i],(char *)&newdesk[1],sizeof(OBJECT));
				newdesk[i-1].ob_next = i;
			}
			newdesk[i-1].ob_next = 0;
			newdesk[i-1].ob_flags |= LASTOB;
		}
		else MAXICON = 92;
	}
	...
	if (!scrp_read(p) || !*p) {
		...
		/* Eventuell CLIPBRD? */
/*		sp = getenv("CLIPBRD");*/
		(void)shel_envrn(&sp,"CLIPBRD=");
		if(sp)
			(void)shel_envrn(&sp,"SCRAPDIR="); /* Eventuell SCRAPDIR? */
		/*	sp = getenv("SCRAPDIR"); */ 
...
/* Supprime OL le 2/02/2024 incompatibilite avec pop_set
...
*/
	...
	if (conf.imguse && ((glob.sysinfo&3)!=1)) /* background img && !MyAES && !AES_NO_BACKGROUND */
		glob.img_ok =
...
/*	if (conf.userdef) {           !!!!!!!!!!!!!!! crash under XaAES to check OL
LINOUT
		for (i = 0; i < rinfo.rsc_file->rsh_ntree; i++)
...
void main_loop(void) {
	...
	while (!glob.done) {
		...
		mevent.ev_mflags = MU_KEYBD | MU_BUTTON | MU_MESAG /*| MU_TIMER*/; /* Trouble on TOS with MU_TIMER */
		if (tb.fi)
			mevent.ev_mflags |= MU_M1;
		mevent.ev_mtlocount = 1000;
...
/* printf("Thing : Key %x (%d)\015\012", mevent.ev_mkreturn, mevent.ev_mkreturn);*/
