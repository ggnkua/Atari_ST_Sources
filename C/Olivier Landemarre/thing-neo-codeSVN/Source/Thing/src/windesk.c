...
void wpath_tree(WININFO *win) {
	...
	ICONBLK *iblk, *tiblk;
	long iy, my;
	short i, n, nname, next, mx, nx, ix, iw, ih;
	short tx, tw;
	short out[8], namelen;
	short uidlen, gidlen;
	WP_ENTRY *entry;
	...
	if (wpath->index.text) {
		...
		for (i = 0; i < wpath->e_total; i++) {
			...
			if (entry->link == 2)
				pt_tree->ob_state |= (CROSSED|CHECKED);
			else if (entry->link == 1)
				pt_tree->ob_state |= CHECKED;
			pt_tree->ob_width = namelen;

			icn_setimg(&wpath->wicon[i], pt_tree, entry->iconimg, entry->smallname); /* !!!!!!!!!!!!!!!!!!!!!! */
		...
		if (nx == 0)
			my--;
		wpath->imy = my;
		tree->ob_height = iy + ih /*my * ih*/; /* we should limit height of object as it use short */
		wpath->ih = ih;
	}
	wpath_tree_r: ;
	if (conf.autosize) {
...
DEBUGLOG((0, "wpath_update: Link is orphaned\n"));
			...
			if (useit) {
				...
				if (wpath->e_num >= lmax) {
					...
					ename = entry->name;
					strcpy(ename, fname);
					strShortener(entry->smallname, fname, SMALL_FLEN-2);
	...
	if (!wpath->o_total) {
		...
		if (!wpath->o_sel) {
			...
			if (wpath->o_total == 1)
				...
				sprintf(is, rs_frstr[TXWPINFO], fs, wpath->o_total);   /* !!!!!!!!!!!!!!! to fix */
		} else {
