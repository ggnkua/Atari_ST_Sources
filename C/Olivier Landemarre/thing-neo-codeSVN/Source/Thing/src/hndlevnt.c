...
void handle_menu(short title, short item, short ks) {
	...
	if (ks & K_CTRL) {
		...
		case MDRIVE:
			...
			{ int locredraw=0; ICONDESK *p;
			p = desk.dicon + 1;
			for (i = 1; i <= MAXICON; i++, p++) {
				if((p->class == IDCLIP) || (p->class == IDPRT) || (p->class == IDTRASH)) {
					if(p->y>tb.desk.g_h+tb.desk.g_y) {
						p->y-=tb.desk.g_h;
						icon_update(i);
						locredraw=1;
					}
			...
			if(locredraw) desk_draw(tb.desk.g_x, tb.desk.g_y, tb.desk.g_w, tb.desk.g_h);
			}
			break;
		case MCHANGEREZ:
...
void desk_popup(short mx, short my) {
	...
	if (ok >= 0) { int redraw=0;
		...
		case 0:
			...
			if(redraw) desk_draw(tb.desk.g_x, tb.desk.g_y, tb.desk.g_w, tb.desk.g_h);
			break;
		case 2:
			fi_config.init();
			break;
