...
static myfalseFILE glob_file;

static myfalseFILE *myfopen(char *name, char *conf) {
	glob_file.stream = NULL;
	glob_file.buffer = NULL;
	glob_file.current = NULL;
	glob_file.size = 0L;
	glob_file.gemdoshandle = Fopen(name,0);
	if(glob_file.gemdoshandle>0L) {
		glob_file.size = Fseek(0L,(int)glob_file.gemdoshandle,2);
		if(glob_file.size>1024L) {
			(void)Fseek(0L,(int)glob_file.gemdoshandle,0);
		
			glob_file.current = glob_file.buffer = Malloc(glob_file.size+1L);
			if(glob_file.buffer) {
				(void)Fread((int)glob_file.gemdoshandle,glob_file.size,glob_file.buffer);
				glob_file.buffer[glob_file.size] = 0;
				return &glob_file;
			}
		}
		Fclose((int)glob_file.gemdoshandle);
		glob_file.gemdoshandle = 0;
	}
	else glob_file.gemdoshandle = 0L;
	glob_file.stream = fopen(name, conf);
	if(glob_file.stream) return &glob_file;
	else return NULL;
}

static int myfclose(myfalseFILE *stream) {
	if(!stream) return 0;
	if(stream->buffer) (void)Mfree(stream->buffer);
	if(stream->gemdoshandle > 0L) {
		return (int)Fclose((short)stream->gemdoshandle);
	}
	return fclose(stream->stream);
}

static int myfeof(myfalseFILE *stream) {
	if(!stream) return 1;
	if(stream->current) {
		if(*stream->current == 0) return 1;
		return 0; 
	}
	else return feof(stream->stream);
}

static char* myfgets(char *buf, int n, myfalseFILE *stream) {
	char *encours;
	if(!stream) return NULL;
	encours = stream->current;
	if(stream->current) {
		if(*stream->current==0) return NULL;
		while((*stream->current!=0) && (*stream->current!='\015') && (*stream->current!='\012')) stream->current++;
		while((*stream->current=='\012') || (*stream->current=='\015')) stream->current++;

		return encours;
	}
	return fgets(buf, n, stream->stream);
}
...
short wgrp_open(char *name, WININFO *uwin, unsigned long wait) {
	...
	char *inbuf,buffer[1024], /* *inbuf5 = &inbuf[5],*/ *tx;
	unsigned long id;
	myfalseFILE *fh;
				...
				if (inbuf[0] != '#') {
					...
					if (id == 'OFIL') {
						...
						if (id == 'INFO') {
							...
							grok = 1;
							if (realwin) {
								tx = get_text(&inbuf[5], wgrp->title, MAX_FLEN - 1);
						...
						if (realwin && (id == 'IGTA')) {
							/* Attribute ermitteln? */
							gtaok = 1;
							sscanf(&inbuf[5], "%d", &wgrp->getattr);
						}
						if (id == 'IACL') {
							/* Automatisch schlieûen? */
							aclok = 1;
							sscanf(&inbuf[5], "%d", &wgrp->autoclose);
						}
						if (id == 'PRNT') {
							/* Vaterobjekt */
							wgrp->parent[MAX_PLEN - 1] = 0;
							strncpy(wgrp->parent, &inbuf[5], MAX_PLEN - 1);
						...
						if (realwin && (id == 'FONT')) {
							/* Zeichensatz etc. */
							sscanf(&inbuf[5], "%d %d %d %d", &font.id, &font.size, &wgrp->fcol16, &wgrp->bcol16);
							if (font.size < 1)
								font.size = 1;
							wgrp->fcol16 &= 15;
							wgrp->bcol16 &= 15;
						...
						if (id == 'FCOL') {
							...
							cok = 1;
							sscanf(&inbuf[5], "%d %d", &wgrp->fcol, &wgrp->bcol);
							wgrp->fcol &= 1;
							wgrp->bcol &= 1;
						}
						if (id == 'BPAT') {
							...
							bpok = 1;
							sscanf(&inbuf[5], "%d %d", &wgrp->bpat, &wgrp->bpat16);
							wgrp->bpat &= 7;
						}
						if (realwin && (id == 'BPIC')) {
							/* Bild fÅr Hintergrund */
							if (get_buf_entry(&inbuf[5], wgrp->img, &tx))
						...
						if ((id == 'OOPT') && (new != NULL))
							sscanf(&inbuf[5], "%d", &new->paralways);
						if ((id != 'OFIL') && (id != 'OFLD'))
							new = NULL;
					}
				}
...
/*	wgrp_eabs(grp, item, xname); 
	item->aptype = grp->getattr ? is_appl(xname) : is_app(xname, 0);!!!!!!! supprimÇ OL  */
	item->aptype = is_app(item->name, 0); /* simplification OL pour aller plus vite recherche application seulement par extension plus par attributs träs lent  */
...
void wgrp_tree(WININFO *win) {
	...
	if (wgrp->text) {
		...
		for (i = 0; i < wgrp->e_num; i++) {
			...
			pt_tree->ob_next = i + 2;
			pt_tree->ob_flags = NONE;
			
			if (entry->sel)
				pt_tree->ob_state = SELECTED;
			else
				pt_tree->ob_state = NORMAL;
			if (namelen < 76)
				pt_tree->ob_width = 76;
			else
				pt_tree->ob_width = namelen;

			icn_setimg(&wgrp->wicon[i], pt_tree, entry->iconimg, entry->title);
			...
			if (entry->class == EC_FOLDER) {
				if (wild_match1("[A-Za-z]:\\", entry->name)) {
					iblk->ib_char = 0x1000 | nkc_toupper(*entry->name);
			...
			pt_tree->ob_height = iblk->ib_ytext + tb.fs_ch + 2;
			pt_tree->ob_x = ix + 2;
			pt_tree->ob_y = iy + 4 + (ih - iblk->ib_ytext);
			...
			if (nx >= mx) {
				...
				ix = 0;
				iy += (ih + 2 * tb.fs_ch + 4);
				if(iy>30000L) iy=30000L; /* limit of height link to short of gem object !!!!!! */
				my++; /* count lines */
			}
...
/*		my = iy / ih + 1;*/
		...
		if (nx == 0)
			my--;
		wgrp->imy = my;
		tree[0].ob_height = iy + ih /*my * ih*/; /* we should limit height of object as it use short */
		wgrp->ih = ih;
	}
}
...
void wgrp_esel1(WININFO *win, WG_ENTRY *entry, short sel, GRECT *wrect, short *rd) {
	...
	if (osel != entry->sel && !entry->prevsel) {
		...
		if (wgrp->text) {
			...
			y = (long)yg * ((long)wgrp->clh + 1L) + (long)win->work.g_y + 2L - wgrp->offy;
...
WG_ENTRY *wgrp_efind(WININFO *win, short x, short y, WG_ENTRY **prev) {
	...
	my = (long)y - (long)win->work.g_y - wgrp->offy;
	...
	if ((x == -1) || (x >= win->work.g_x && x <= win->work.g_x + win->work.g_w - 1 && y >= win->work.g_y && y <= win->work.g_y + win->work.g_h - 1)) {
		...
		if (wgrp->text) {
			...
			while (item) {
				...
				py = (long)ty * ((long)wgrp->clh + 1L) + (long)win->work.g_y + 2L - wgrp->offy;
	...
	while (litem) {
		if (litem->sel) {
			...
			if (wgrp->text) {
				...
				/* Als Icons */
				/* Koordinaten/Maûe holen */
				objc_offset(wgrp->tree, i + 1, &x, &locy);
				y = locy;
				...
				if (rc_intersect((GRECT *)&tb.desk, (GRECT *)&irect) && rc_intersect((GRECT *)&desk.sel.win->work, (GRECT *)&irect)) {
					short locy;
					/* Polygonzug berechnen */
					wc_icon(&pxy[p * 18], &x, &locy, &w, &h, ix, iw, tx, ty, tw);
					y = locy;
					p++;
					/* Gesamtrechteck anpassen */
					if (x < tx1)
					...
					if (litem->sel) {
						if (i > MAXICON) {
							...
							case EC_FOLDER:
								if (wild_match1("[A-Za-z]:\\", litem->name)) {
									...
									if (desk.dicon[i].spec.drive) {
										desk.dicon[i].spec.drive->drive = nkc_toupper(*litem->name) - 'A';
									}
								} else {
							...
							if(!desk.dicon[i].spec.data) {
								...
								icon_update(i);
								objc_offset(rs_trindex[DESKTOP], i, &x, &locy);
								y = locy;
...
void wgrp_exsel(WININFO *win, short mx, short my, short mk, short ks) {
	...
	if (wgrp->e_num == 0 || (!wgrp->text && !tree)) {
		...
		if (wgrp->text) {
			...
			maxy = wgrp->imy * ((long)wgrp->clh + 1L) + 3L - (long)win->work.g_h;
		} else {
