...
#include <basepage.h>
#ifdef __PUREC__
extern BASEPAGE *_BasPag; /* defined in pcxstart.s */
#else
...
short run_app(APPLINFO *aptr, char *cmd, short is_gr, short argv, short doex,
	...
	if((tb.sys & SY_GNVA) && !(tb.sys & SY_MINT)) { /* !!!!!!! A revoir */
		if(doex&0x400) doex=0x400;
		else doex=0;
	}
	if (0/*argv*/) { /* !!!!!!!!!!!!!!!!!! a revoir marche pas si fichiers avec espace on sera limite a 254 caracteres */
		cmd[0] = 0x7f;
					...
					if (/*res_exit(appname)*/0) {
						...
						*rex = 1; /* Flag setzen, damit in aufrufenden Prozeduren kein
						...
						if (aret) {
							...
							if (Pexec(0, aptr->startname, cmd, 0L) == 0L)
