...
static void winRedraw(struct wininfo *win, short type, GRECT *area) {
	...
	/*short*/unsigned long anzahlEintraege; /* Anzahl der angezeigten EintrÑge im Fenster */
	...
	if (text) {
		...
		limit = (long)win->work.g_y + (long)win->work.g_h;
		for (i = 0; i < anzahlEintraege; i++) {
			...
			if ((y>=0) && (y<limit) && rc_intersect((GRECT *)area, (GRECT *)&trect)) {
				/*
...
static void winSlide(struct wininfo *win, short mode, short h, short v, short type) {
	OBJECT *tree;
	long hsize, vsize, hpos, vpos, lv;
	short dx,/* dy, lv,*/ lh, offx,/* offy,*/ imx;
	/*short*/long anzahlEintraege; /* Anzahl der angezeigten EintrÑge im Fenster */
	short text; /* Textdarstellung */
	short tlen; /* LÑnge eines Eintrags */
	short clw, clh; /* Breite/Hîhe einer Textzelle */
	short pat; /* Hintergrund-Pattern */
	short width/*, height*/; /* Breite und Hîhe des Bereiches (Text oder Icon)*/
	/*short*/long ih; /* Hîhe des Ordner-Icons */
	...
	if (text) {
		...
		if (type == WPATH) {
			...
			height = wpath->imy * (long)(clh + 1) + 3L;
		} else {
			width = tlen * wgrp->imx + 20 - clw * 2;
			height = wgrp->imy * (long)(clh + 1) + 3L;
		}
	} else {
		/* Breite und Hîhe des Icon-Dialogs */
		width = tree->ob_width;
		height = (long)tree->ob_height;
	}
...
#ifdef OFFSET
			vpos = (long)win->offy * 1000L / (height - (long)win->work.g_h);
#else
			if (type == WPATH)
				vpos = (long) wpath->offy * 1000L /  (height - (long)win->work.g_h);
			else
				vpos = (long) wgrp->offy * 1000L /  (height - (long)win->work.g_h);
#endif
		} else
			vpos = 0;
	...
	case S_ABS:
		...
		if (v != -1) {
			if (height > win->work.g_h)
				offy = /*(short)*/ ( (height - (long)win->work.g_h) * (long) v / 1000L);
			else
				offy = 0;
		} else {
#ifdef OFFSET
...
void wf_set(WININFO *win) {
	...
	if (win->class == WCPATH || win->class == WCGROUP) {
		...
		case WCGROUP:
			...
			/* Falls Focus vorhanden, dann ggf. glen */
			if (wgrp->focus > -1)
				wgrp_showsel(win, 1);
			else {
				wgrp->focus = 0;
...
static void draw_minicicon(ICONIMG *ic, short *pxy, short sel) {
	...
	if (ic->sub_code != 0L)
		mini_icon.ob_type = G_CICON;
	else mini_icon.ob_type = G_ICON;
	mini_icon.ob_flags = LASTOB;
	mini_icon.ob_state = sel ? SELECTED : NORMAL;
	mini_icon.ob_spec.index = (long)ic->siconblk;
