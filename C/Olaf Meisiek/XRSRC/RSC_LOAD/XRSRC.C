 ...
 *
 *
 * Remarque : Pour utiliser ces routines, vous devez imp‚rativement
 *            posseder un original d'Interface.
 *            
 ******************************************************************************/
...
#define SAVE_MEMORY FALSE	/* r‚server de la place pour les ic“nes ? */
...
LOCAL VOID fix_chp (WORD *pcoord, WORD flag)
{
	...
	if (!flag && ncoord == 0x50)
		...
		ncoord *= (flag ? xgl_hbox : xgl_wbox);	/* xgl_wbox, xgl_hbox = Zlargeur & hauteur du dessin en points */
...
{	WORD np, color, pxy[8], backup[32], rgb[3];
	...
	if (xscrn_planes >= 8)
	{
		if (xscrn_planes > 8)
		{
			...
			pixel.mp = backup;	/* restaurer l'ancien point */
			vro_cpyfm (xvdi_handle, S_ONLY, pxy, &pixel, &screen);
