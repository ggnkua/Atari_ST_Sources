...
/* very slightly adapted for the LazyTick project */
...
int crc32file(char *name, unsigned long *crc)
{
	  ...
	  printf("$ %s LTCK Calculating CRC32",tdate);      
    
      while ((fread(&c,1,1,fin))==1) /* fa: binary routine */
      {
            oldcrc32 = UPDC32((int) c, oldcrc32);
            if(((count++)%25000)==0)
            	printf(".");
      }
	  printf("\n");
