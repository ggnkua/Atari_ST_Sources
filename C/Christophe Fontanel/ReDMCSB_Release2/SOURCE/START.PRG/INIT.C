/* Custom INIT.C for START.PRG, based on original supplied with Megamax C and modified by FTL. Changes from original:
- Removed call to GEMDOS Mshrink() to return unused memory to the system
- Removed _argc, _argv and call to _initargcv()
