...
/******************************************************************************/
/* Modes fÅr STEFcntrl (STEmulator */
#define SET_INFOTEXT	1
#define SET_WINDOWTITLE	2
...
typedef struct
{
	...
	int32 reserved[1];	/* Currently unused */
  META_INFO_2 *info;  /* Pointer to a second structure from MetaDOS V2.30! */
} METAINFO;
#define META_INFO_1 METAINFO
...
typedef struct
{
   int8  Name[34];   /* Descriptive text         */
   int32 colors;
   int16 xres,yres;
   int32 fileptr;    /* Pointer to the mode file */
} MOD_DESC;
...
typedef struct  {
   int8  name[34];
   int16 mode_id;       /* Running number                        */
   int16 xres,yres;     /* Width/height in pixels                */
   int16 fx,fy;         /* Width/height in total                 */
   int32 colors;        /* Number of simultaneous colours        */
   int16 org;           /* Organisation of the memory            */
   int16 fh10,fv10;     /* Line-freq.*10, frame-freq.*10         */
   int16 aspect100;     /* 100*pixel-width/pixel-height          */
   int16 disp_scan100;
   int32 clk;           /* Dot clock in Hz                       */
   /*
   * Register of the graphics controller
   */
   int16 Clock;         /* Frequency number, must be translated  */
                        /* via the IcsLut before writing into
                           the clock register (see below)        */
   int8  misc_out;
   int8  ts[10];        /* Timing sequencer regs.                */
   int8  crtc[35];      /* CRT controller regs.                  */
   int8  gdc[10];       /* Graphics display controller           */
   int8  atc[24];       /* Attributes of controller              */
   int8  reserved[18];
} PMODE;
...
typedef struct
{
   int8   xbra_id[4];       /* XBRA ID: 'CDOT'                      */
   int8   version[4];       /* At present: '1.10'                   */
   int8  *sm194_addr;       /* Pointer to large screen memory       */
   int8  *sm124_addr;       /* Pointer to SM124 screen memory       */
   LINEA   *line_A;         /* Pointer to LINE_A block              */
   int16  scrn_width;       /* Screen width                         */
   int16  scrn_height;      /* Screen height                        */
   int16  scrn_byte_width;  /* Screen width in bytes                */
   int16  scrn_planes;      /* Screen colour planes                 */
   int16  PhysMode;         /* Mode mumber                          */
   int16  VirtMode;         /* x/16 & y/16 each coded in 1 byte     */
   PMODE   *ModStruct;      /* Pointer to current graphic mode      */
   int16  grau;   /* Flag for the VDI driver (Display: Grey/colour) */
   int16  BiosText;         /* Flag for the VDI driver              */
   int16  CurText;          /* Flag for the VDI driver              */
   int16  BlitMode;         /* Flag for the VDI driver              */
} SCREEN;
*/
...
int32 STEFcntrl ( int16 func, int32 special1,  int32 special2 );
...
/******************************************************************************/
/*																										*/
/* Crazy-Dots Funktionen																			*/
/*																										*/
/******************************************************************************/

/* SCREEN *GetScreenDescriptor( void ); */
void *GetScreenDescriptor( void );
void ResetScreen( void );
int32 GetNumberOfPModes( void );
MOD_DESC *GetPMode( int16 n );
PMODE *ReadMode( int16 n );
void CopyModeToActiveMode( void );
int16 ActiveModeNumber( void );
PMODE *GetActiveModeDesc ( void );
int32 GetRegisterbase( void );
int32 GetFeatures( void );
int32 GetActiveFeatures( void );
