...
char *GetObjectInfo ( int obj )
{
	switch ( obj )
	{
	case IS_LEFT2 :		return "Schneller RÅcklauf" ;
	case IS_LEFT  :		return "Abspielen rÅckwÑrts" ;
	case IS_STOP  :		return "Stop" ;
	case IS_PAUSE :		return "Pause" ;
	case IS_RIGHT :		return "Abspielen vorwÑrts" ;
	...
	case IS_TRASH :		return "MÅlleimer" ;
	case IS_DISPLAY :	return "Einzelbild / Sequenz" ;
	case TX_DRIVE  :	return "Laufwerk +/-" ;
	case TX_FILE_TYPE :	return "Dateiformat (JPG/TGA/FLM)" ;
	case TX_NAME  :		return "Filmname" ;
	case IS_VOLUME :	return "LautstÑrke" ;
	case IS_PARAM :		return "X/Y Anzahl Sequenz" ;
	case IS_MIN   :		return "Minimum" ;
	case IS_COUNT :		return "ZÑhler" ;
...
case TX_NAME :	strcpy ( CurrentFilmName,
					ToolsTree[TX_NAME].ob_spec.tedinfo->te_ptext ) ;
				break ;
...
# endif
	...
	case TX_DRIVE :	{
						...
						txt = ToolsTree[TX_DRIVE].ob_spec.tedinfo->te_ptext ;
						if ( buttons & 2 )
							{ if ( *txt > 'a' ) (*txt)-- ; else	break ;	}
						else
							{ if ( *txt < 'p' ) (*txt)++ ; else	break ;	}
						CurrentDrive = *txt ;
						ObjectDraw ( ToolsTree, TX_DRIVE ) ;
					}
					break ;
	case TX_FILE_TYPE :
					{
						...
						txt = ToolsTree[TX_FILE_TYPE].ob_spec.tedinfo->te_ptext ;
						switch ( *txt )
						{	/* j->t->b	*/
				default :	*txt = 'j' ;	SequenceFileFormat = JPGformat ; WindowMessage ( " Format : JPG" ) ; break ;
				case 'j' :	*txt = 't' ;	SequenceFileFormat = TGAformat ; WindowMessage ( " Format : TGA" ) ; break ;
				case 't' :	*txt = 'c' ;	SequenceFileFormat = TICformat ; WindowMessage ( " Format : TIC" ) ; break ;
				case 'c' :	*txt = 'f' ;	SequenceFileFormat = BLKformat ; WindowMessage ( " Format : FLM" ) ; break ;
						}
						ObjectDraw ( ToolsTree, TX_FILE_TYPE ) ;
					}
					break ;

	case TX_NAME :	StartEditTextField ( obj, NULL, DONTset ) ;			break ;
...
# define PressAnyKey() printf ( "- bitte Taste drÅcken !\n" ) ; getch();
...
# if TEST
	printf ( "\033H* ScreenEye " __DATE__ " / " __TIME__ "\n" ) ;
# endif
...
# endif
					...
					wind_get ( 0, WF_WORKXYWH, &DeskTop.x, &DeskTop.y,
										   &DeskTop.w, &DeskTop.h );
					ToolBarInit();
					strcpy ( ToolsTree[TX_NAME].ob_spec.tedinfo->te_ptext,
								CurrentFilmName ) ;
					SetToolNumber ( IS_MAX, Maxi ) ;
					SetToolNumber ( IS_PARAM, NumXYFields ) ;
...
# if ! DEBUG
				...
				{
					form_alert ( 1, "[1][| SCRN_EYE lÑuft nur | im True Color Modus ! ][ OK ]");
					acc_stop () ;
				}
# endif
				v_clsvwk ( vdi_handle );
		...
		{	/* resource file not found - English message */
			form_alert ( 1, "[1][| SCRN_EYE Error | Resource file | "RSC_NAME" | not found! ][ OK ]");
			acc_stop () ;
		}
