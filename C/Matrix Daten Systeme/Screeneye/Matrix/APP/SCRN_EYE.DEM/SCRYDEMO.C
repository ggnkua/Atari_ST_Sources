...
void resize_window ( void )
{
	...
	BlitWindow.dst.window.w = MatDigiF.image.w / Zoom.x ;
	BlitWindow.dst.window.h = MatDigiF.image.h / Zoom.y ;
...
/*** example functions, not used in this demo program ***/
...
void CommandLoop ( void )
{
	...
	case DELETE :
				printf ( "\033Edelete film" ) ;
				if ( CheckSequenceAvailable() )
					DeleteFilm();
				break ;

	case 'L' :
	case 'l' :	printf ( "\033Eload film" ) ;
				if ( CheckSequenceEmpty()
						&& MallocFilm() > 0 )
					NewRunMode ( LOADfilm ) ;
				else
