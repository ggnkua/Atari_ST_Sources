...
# if 0
# define Tprintf(p) printf p
# else
# define Tprintf(p) 
# endif

# define INFORMATION DEBUG

# define RSC_NAME "MATDIGIF.RSC"
...
# include "\pc\app\dig_wind\coltable.h"
...
char CurrentFileDir[128] = "f:\\film" ;
char CurrentFilmName[8] = "f:film" ;
char BaseName[128] ;

unsigned ScreenDumpNumber = 0 ;
unsigned PictureNumber = 0 ;
...
void auto_check ( void )
{
	if ( madi_ok )
	{
		video_signal = PUP_AUTO ;
		video_source = DEFsource ;
		mouse_busy () ;
		chk_set_chan_sig ( (int *)&video_source,
						   (int *)&video_signal,
						   (int *)&dummy ) ;
		mouse_arrow () ;
...
{
	if ( madi_ok )
		i2c_initrom ( video_signal, video_source, DmsdType ) ;
    check_source_pop() ;
    check_signal_pop() ;
...
/* ----------------------------------- get_pic ------------------- */
void get_pic ( TGrabMode gmode, SPEED_MODE spd )
{
# if DEBUG
	if ( madi_ok && bit_planes == 16 )
# else
	if ( madi_ok )
# endif
	{
		if ( gmode == GRAB_ON )
			GrabFieldIntoFifo ( WaitUntilGrabbed, FIELDnext ) ;
		spd = spd ;
...
void update_hardware ( void )
{
	...
	if ( madi_ok )
	{
		if ( ( video_signal == PUP_AUTO ) || ( video_source == PUP_AUTO ) )
		{
			video_signal = PUP_AUTO ;
			video_source = DEFsource ;
			set_window_title () ;
		...
		{
			i2c_initrom ( video_signal,
						  video_source, DmsdType ) ;
			set_window_title () ;
		}
	}
...
{
	strlwr ( CurrentFilmName ) ;
	if ( *CurrentFilmName >= 'a' && *CurrentFilmName <= 'a'+15
		 && CurrentFilmName[1] == ':' )
	{
		*CurrentFileDir = *CurrentFilmName ;
	...
	{
		WindowMessage ( "* Unzul„ssiger Name ( d:name ! )" ) ;
		return FALSE ;
	}
}
...
bool SaveStart ( void )
{
	...
	if ( CheckSequenceAvailable() )
	{
		...
		if ( MakeBaseName() )
		{
			...
			result = WriteSequenceHeader ( &Sequence, BaseName ) ;
			mouse_arrow();
			if ( result == OK )
			{
...
Tprintf ( ( "- LoadNext ok \n" ) ) ;	
		...
		else if ( result != FILEopenError || frame == Mini )
			ReadError ( result ) ;
		mouse_on();
...
# else
			if ( ! DisplayFrame ( &Sequence, Sequence.current, &BlitWindow.dst ) )
# endif
				get_pic ( GRAB_OFF, GEM_SPEED ) ;
		}
	}
...
char *GetObjectInfo ( int obj )
{
	...
	case IS_PHOTO :		return "Einzelbild schnappen" ;
	...
	case IS_NAME  :		return "Filmname ( Laufwerk:Name )" ;
...
case IS_NAME :	strcpy ( CurrentFilmName,
					ToolsTree[IS_NAME].ob_spec.tedinfo->te_ptext ) ;
				break ;
...
case F3 :		tail_y = 0 ;
				NewZoomCode ( max_y > (240+480)/2 ? Z41 : Z21 ) ;
				return 0;
				
case F4 :		tail_y = 0 ;
				NewZoomCode ( max_y > (240+480)/2 ? Z82 : Z42 ) ;
				return 0;
...
# endif
		...
		case 'd' :
		case 'D' :		get_pic ( GRAB_ON, GEM_SPEED ) ;
						clear_keyboard () ;
						return 0 ;
		case 'f' :
		case 'F' :		video_signal = PUP_CVBS ;
						if ( madi_ok )
							i2c_initrom ( video_signal,
								      	  video_source, DmsdType ) ;
			            set_window_title ( ) ;
		...
		case 'P' :		mouse_on();
						if ( aes_version >= AES_FALCON )
							...
							handle_params ( NULL, 0, MD_PARAM, NO_EDIT ) ;
						update_hardware();
						mouse_off();
						return 0 ;
		case 'r' :
		...
		case 'Z' :		NewZoomCode ( ( menu_zoom +(NUMzoomCodes-1) ) % NUMzoomCodes ) ;
						return 0;
		case 'z' :		NewZoomCode ( ( menu_zoom + 1 ) % NUMzoomCodes ) ;
						return 0;
# if TEST
		case 'v' :
...
# if TEST > 1
		case 'e' :
		case 'E' :		if ( madi_ok )
							save_a_file ( &BlitWindow.dst.window, &video_pos,
									  	  ???, &Drive ) ;
...
# if DEBUG
	if ( bit_planes != 16 )
		return 50 ;
# endif
	return whandle == top && !Pause && RunMode != IDLE
...
int SnapFilmPicture ( void )
{
	...
	if ( ( frame = RecordFrame ( &Sequence, frame, WaitUntilGrabbed ) ) >= 0 )
	{
...
/* ------------------------------------------------- MouseButton ------- */
void MouseButton ( int buttons, int clicks, int x, int y )
{
	int obj ;
	...
	mouse_off();
	if ( ToolsVisible && ( obj = objc_find ( ToolsTree, 0, 999, x, y ) ) > 0 )
	{
		if ( verbose )
			printf ( " - obj:%3d", obj ) ;
	...
	case IS_ZOOM :	if ( buttons & 2 )
						NewZoomCode ( ( menu_zoom +(NUMzoomCodes-1) ) % NUMzoomCodes ) ;
					else
						NewZoomCode ( ( menu_zoom + 1 ) % NUMzoomCodes ) ;
					break ;

	case IS_TOOLS : ToolsVisible = ToolBarOn = FALSE ;
					reset_window ( CurrentStyle ) ;
					break ;
	...
	case IS_PAUSE :	Pause = ToggleTool ( obj ) ;
					CheckCurrentDisplayed ( &Sequence, &BlitWindow.dst ) ;
					break ;
...
# endif
	...
	case IS_TV : 	NewRunMode ( DIGITIZE );
					break ;
	...
	case IS_NAME :	StartEditTextField ( obj, NULL, DONTset ) ;			break ;
...
void DigitizeToLiveBox ( void )
{
	...
	{
		DigitiseFrame ( &LiveImageBuffer, &Zoom, WaitUntilGrabbed ) ;
...
# if DEBUG
		if ( DisplaySequenceOn )
		{
			if ( bit_planes == 16 && ToolsVisible && ! Fast )
				DigitizeToLiveBox();
		}
		else	/* full screen 	*/
		{
...
case RECORD :
		if ( ( i = RecordCurrFrame ( &Sequence, WaitUntilGrabbed ) ) >= 0 )
		{
			if ( !Fast )
			{
				SetCounter ( i ) ;
...
void event_loop ( void )
{
	...
	do
	{
		...
		{
			if ( ! Pause && TimerEvent ( x, y ) && !Fast && ToolsVisible )
				StepFilm ( Step >= 0 ) ;
		...
		if ( event & MU_BUTTON )
			MouseButton ( buttons, clicks, x, y ) ;
...
void main ( void )
{
	...
	if ( appl_id >= 0 )
	{
		...
		if ( rsrc_load ( RSC_NAME ) != 0 )
		{
			atexit ( free_rsrc ) ;
			if ( _app == 0 )				/* = ACC */
				menu_id = menu_register ( appl_id, "  MatDigiF" ) ;
...
# endif
					...
					wind_get ( 0, WF_WORKXYWH, &DeskTop.x, &DeskTop.y,
										   &DeskTop.w, &DeskTop.h );
					ToolBarInit();
					strcpy ( ToolsTree[IS_NAME].ob_spec.tedinfo->te_ptext,
								CurrentFilmName ) ;
					SetToolNumber ( IS_MAX, Maxi ) ;
					SetToolNumber ( IS_PARAM, NumXYFields ) ;
# if 0
					madi_ok = SetDmsdType ( AnyShiftPressed() ? ALTdmsdType : DEFdmsdType, LCAtypColor ) == 0 ;
# else
			/* Zoom ??? */
					menu_zoom = Z42 ;
					Zoom = ZoomTable [menu_zoom] ;
					madi_ok = LoadMatDigiFLcaData ( DEFdmsdType, DIGcolor, &Zoom, TRUE ) == 0 ;
# endif
					if ( madi_ok )
						madi_ok = i2c_initrom ( PUP_CVBS, 1, DmsdType ) ;

					SetToFullSize ( &MatDigiF, NULL,
									 DmsdType == 7191 ? DIGdx7191 : DIGdx9051,
									 DIGdyF, 16 ) ;
...
# if ! DEBUG
				...
				{
					form_alert ( 1, "[1][| MATDIGIF l„uft nur | im True Color Modus ! ][ OK ]");
					acc_stop () ;
				}
# endif
				v_clsvwk ( vdi_handle );
		...
		{	/* resource file not found - English message */
			form_alert ( 1, "[1][| MATDIGIF Error | Resource file | "RSC_NAME" | not found! ][ OK ]");
			acc_stop () ;
		}
