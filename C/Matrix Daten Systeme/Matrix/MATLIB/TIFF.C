					  ...
					  char *software, char *version )
{
	...
	if ( tiff->head != NULL )
	{
		...
		if ( tiff->dir != NULL )
		{
			...
			strcpy ( tiff->dir->software, software ) ;
			strcat ( tiff->dir->software, " " ) ;
			strcat ( tiff->dir->software, version ) ;
			strcpy ( tiff->dir->make, "Matrix Daten Systeme GmbH" ) ;
...
/* ---------------------------------------------- init_tiff ------- */
TIFFpic *init_tiff ( void *buf, int w, int h,
					  char *software, char *version,
					  int *result )
{
	TIFFpic *tiff ;
