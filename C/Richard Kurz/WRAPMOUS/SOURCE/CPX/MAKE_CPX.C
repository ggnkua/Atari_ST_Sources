/*  MAKE_CPX.C
 *  Erzeugt einen Header und linkt ihn mit WRAP_CPX.CP
 *  zum fertigen WRAP_CPX.CPX.
 *  
...
void main(void)
/* Macht aus WRAP_CPX.CP ein CPX-Modul. */
{
    ...
    header.cpx_id=(long)'WrAp'; /* ID-String des CPX-Moduls. Sollte */
    ...
    puts("WRAP_CPX.CPX ist in Arbeit!");
    fp=fopen("wrap_cpx.cpx","wb");
    if(fp==NULL)
    {
        puts("wrap_cpx.cpx kann nicht erzeugt werden!!");
        exit(1);
    }
    fwrite(&header,sizeof(CPXHEAD),1,fp);
    fp2=fopen("wrap_cpx.cp","rb");
    if(fp2==NULL)
    {
        puts("wrap_cpx.cp kann nicht geîffnet werden!!");
        exit(1);
    }
    while(!feof(fp2))
    {
