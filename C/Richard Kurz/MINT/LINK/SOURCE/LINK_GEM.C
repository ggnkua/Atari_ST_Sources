    ...
    if(!mint)
    {
        for(;;)
        {
            ...
            if((pipe[0]==AC_OPEN||pipe[0]==VA_START||pipe[0]==NEO_START))
                form_alert(1,"[3][ |ACHTUNG!|Link lÑuft nur mit MiNT][ Okay ]");
        }
    }

	/* Solange bis das Signal SIGTERM eintrifft! */
    while(Done<=1)
    {
...
void get_sigterm(void)
/* Diese Funktion wird von MiNT aufgerufen, wenn das Signal SIGTERM */
/* an Link gesendet wurde!											*/
{
	Done=99;
}/* get_sigterm */
...
/* Der Anfang */
{
    ...
    else
    {
        ...
        if(_app)
        {
            ...
            else form_alert(1,"[3][ |ACHTUNG!|Link lÑuft nur mit MiNT][ Okay ]");
        }
        else
        {
            menu_id=menu_register(appl_id,"  Link ");
            init_dialog();
            event_loop();
        }
        appl_exit();
