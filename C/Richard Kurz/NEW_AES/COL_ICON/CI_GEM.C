...
extern OBJECT *menu,*dialog,*info;
...
/* Der Anfang */
{
    ...
    aes_ver=_GemParBlk.global[0];
  	if(aes_ver<0x331)
  	{
    	form_alert(1,"[1][ |AES-DEMO lÑuft erst|ab AES Version >= 3.31][ OK ]");
        appl_exit();
    	exit(1);
    }
    else
    {
    	if(rsrc_load("CI_RSC.RSC"))
    	{
    		init_dialog();
    	...
    	{
    		form_alert(1,"[3][ |Datei CI_RSC.RSC|nicht gefunden!][ OK ]");
    	}
        appl_exit();
    }
    exit(0);
...
/* der aktuellen Rechteckliste                              */
{
	...
	if(aes_ver>=0x331)
	{	
		objc_sysvar(0,AD3DVALUE,0,0,&oh,&ov);
		t2.g_x-=oh;
		t2.g_y-=ov;
		t2.g_w+=oh+oh;
		t2.g_h+=ov+ov;
