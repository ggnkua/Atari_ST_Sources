...
typedef struct {
	int zone;			/* Zone */
	int netIndex;		/* For Zone... index to net linked list */
	int netCount;		/* Number of nets in this zone */
} ZONESLIST;

typedef struct {
	int net;			/* Our net */
	size_t nodeIndex;	/* Index to nodes list */
	int nodeCount;		/* Number of nodes in this net */
} NETSLIST;
...
struct s_domainList {
	...
	int 		nZones;					/* Number of zones in domain */
	POINTBOSSLIST *pointBossList;		/* List of systems with points */
	int 		nPoints;				/* Number of entries in pointBossList */
...
void buildOpus(DOMAINLIST *dlist, struct _ndi *idx, size_t idxCount)
{
	...
	while(i < idxCount)
	{
		if(idx->node == -2)		/* Going to a new zone */
		{
			...
			dlist->zoneList[nextZone].netIndex = (int)nextNet;
