...
*
* Man beachte, daž die MT-Variante der AES- Funktionen gegenber
* dem Standard-Binding grunds„tzlich das global[]- Feld mit
* bergeben muž. Auch wenn TOS und MagiC dieses Feld nur bei den
* folgenden Funktionen ben”tigen:
*
...
* und die Ressourcen nicht zuweisen k”nnen. Dort ist die ap_id
* wie ein VDI-Workstation-Handle zu verstehen, w„hrend in TOS
* und MagiC die ap_id direkt die Task beschreibt.
*
* Um die Programmierung des AES zu vereinfachen, enth„lt
* MT_AES.H fr jede AES-Funktion aužer den oben aufgefhrten ein
* Makro, das automatisch das global-Feld mit bergibt. Dabei ist
* darauf zu achten, daž im jeweiligen Aufrufkontext das richtige
* global[] bergeben wird. Sicherer ist der Verzicht auf die
...
*
* -	7.7.96
*	wind_set_string() erg„nzt
* -	26.7.96
*	global-Feld fr MultiTOS und N.AES erg„nzt.
* -	10.9.96 (sb)
*	-	form_xdial() korrigiert
*	-	diverse #defines im Header hinzugefgt, Deklarationen
*		korrigiert
*	-	global und _GemParBlk in MT_AESS.S definiert (Kompatibilit„t)
*	-	fr wdlg_xx(), lbox_xx() und fnts_xx() die 
*		Typen DIALOG, LIST_BOX und FNT_DIALOG hinzugefgt
* -	11.9.96 (sb)
...
*	-	alle #define-Ausdrcke im Header verwenden als letzten
*		Parameter statt global 0L. In diesem Fall wieder im AESPB
*		_GemParBlk.global eingetragen, und der AES-Aufruf wird krzer.
*	-	global ist nicht mehr als extern deklariert, sondern nur
*		noch ber GemParBlk zu erreichen (kompatibel zu Pure C)
*	-	MT_EVNT_multi() hinzugefgt
*	-	_appl_yield() in MT_AESS.S hinzugefgt
*	-	MT_fnts_update() hinzugefgt
* -	12.9.96
*	-	MT_wind_get() korrigiert
* -	13.9.96
*	-	MT_appl_getinfo() hinzugefgt
* -	15.9.96
*	-	MT_objc_xedit() hinzugefgt
*	-	MT_form_dial() hinzugefgt
*	-	MT_form_do() hinzugefgt
* -	18.9.96
*	-	MT_xfrm_popup() besetzt intout[1] vor dem Aufruf, um auch bei
*		fehlender Funktion sinnvolle Ergebnisse zurckzuliefern.
*
****************************************************************/
...
static void __aes( PARMDATA *d, WORD *ctrldata,	WORD *global )
{
	...
	if ( global )				/* wurde global bergeben? */
		pb.global	= global;
	else						/* _GemParBlk.global fr Hauptprogramm */
							...
							EVNT *event, WORD *global )
{
	...
	d.intin[14] = (WORD) ms;					/* W”rter drehen */
...
WORD	MT_lbox_get_first( LIST_BOX *box, WORD *global )
{
	PARMDATA d;
	static WORD c[] = {174,1,1,1};
...
void	MT_lbox_set_slider( LIST_BOX *box, WORD first, GRECT *rect,
					WORD *global )
{
	PARMDATA d;
...
void	MT_lbox_scroll_to( LIST_BOX *box, WORD first, GRECT *box_rect,
