...
cnt =0;
 ...
 read_stuff(pix6,scr6,0);         /* read palette to alt palette(pal4) */
 ...
 do
     {
      ...
      Setpalette( newpal );
       vro_cpyfm(handle,3,pxyarray,&tempMFDB,&theMFDB);
      Vsync();
      Setcolor(6,CURRENT_SKY);  /* redisplay sky */
       double_buffer(&theMFDB,&tempMFDB,pxyarray);
   ...
   /*     enc_true = events();  */     /* returns 1 if encounter */
      ...
      command = Bconin(2);
       ...
       if (command == 0x190050 || command == 0x190070)/* P key to PAUSE */
        {v_gtext(handle,40,84,"PAUSED");
         user_pause();
         v_gtext(handle,40,84,"      ");
        }
       ...
       if(command == 0x220047 || command == 0x220067) /* G for game options */
          game_options();   /* save,quit,credits,sound on/off etc */ 
   ...
   }while (command != 0x610000);        /* Undo key to quit */


Supexec( on );

puts("Press RETURN to exit to Desktop");      
getchar();
...
int pxyarray[8];
   ...
   /*v_gtext(handle,1,41,"in move");*/

    /*v_gtext(handle,44,65,"         "); */
    switch(dir)
     {
     case 'N': {
...
user.sp=20;
 ...
 strcpy(user.armor,"None          ");       /* initally set to None*/
...
points[0] = points[4] = 0;
points[1] = points[5] = 0;
points[2] = points[6] = 319;
points[3] = points[7] = 199;


Setscreen(temp,screen,-1); 

theMFDB->fd_addr = (long) temp;   /* DEST addr is back screen */
                                  /* but first lets copy whole screen
                                     to back screen */
saveaddr = tempMFDB->fd_addr;      /*save it */
tempMFDB->fd_addr = (long) screen; /* set new source */

vro_cpyfm(handle,3,points,tempMFDB,theMFDB);  /* copy wholescreens */

tempMFDB->fd_addr = saveaddr; /* restore */
                        /* ok now lets copy pic to back screen */
vro_cpyfm(handle,3,pxyarray,tempMFDB,theMFDB);  /* copy new pic */


                               /* swap screens */
  temp_scr_hold = screen;
  screen = temp;
  temp = temp_scr_hold;
Setscreen(screen,screen,-1);  /* and tell cpu to use em */
...
char tyee;
    ...
    cell = map[y][x].room;
   sprintf(string,"Room:%d",cell);
   v_gtext(handle,1,8,string);
