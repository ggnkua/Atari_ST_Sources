...
const char __ident_e_gem[] = "$EnhancedGEM v"VERSION" $";
...
boolean init_gem(char *m_entry)
{
	...
	if ((ap_id = appl_init())>=0 && (aes_version = AES_VERSION)>0)
	{
		...
		grhandle = graf_handle(&dummy,&dummy,&dummy,&dummy);
		if (open_work(&x_handle,work_out))
		{
			...
			if (m_entry && (!_app || aes_version>=0x0400 || magx))
			...
			vst_font(x_handle,ibm_font_id);
			vst_height(x_handle,ibm_font,&gr_bw,&gr_bh,&gr_cw,&gr_ch);
			gr_dcw = gr_cw<<1;
...
int appl_getfontinfo(int ap_gtype,int *height,int *id,int *type)
{
	...
	if (aes_version>=0x0400 || magx)
	{
		...
		if (dummy && vst_font(x_handle,*id)!=*id && !fonts_loaded && vq_gdos())
	...
	*id = attrib[0];
	...
	if (vst_font(x_handle,*id)!=*id && !fonts_loaded && vq_gdos())
	{
		vst_load_fonts(x_handle,0);
		fonts_loaded = TRUE;
	}
...
int cdecl draw_cyclebutton(PARMBLK *pb)
{
	...
	if (big_img<0)
	{
		...
		_text_attr(ibm_font_id,ibm_font,color);
...
int cdecl draw_text(PARMBLK *pb)
{
	...
	if (text->font==SMALL)
		_text_attr(small_font_id,small_font,color);
	else
		_text_attr(ibm_font_id,ibm_font,color);
...
void menu_enable(OBJECT *tree,boolean enable,boolean show)
{
	if (_app)
	{
		...
		if (show && (aes_version<0x0400 || menu_bar(NULL,-1)==ap_id))
			menu_bar(tree,1);
	}
}
