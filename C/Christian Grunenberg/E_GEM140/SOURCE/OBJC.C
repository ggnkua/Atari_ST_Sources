...
void ob_draw_list(reg DIAINFO *info,reg int *ob_lst,reg GRECT *area)
{
	reg OBJECT *tree = info->di_tree;

	if (info==NULL || info->di_flag==CLOSED || info->di_iconified)
		return;
	else if (_dia_len>0 && (info->di_flag>=WINDOW || info!=_dia_list[_dia_len-1]))
		return;
	...
	else
	{
		...
		{
			GRECT work;
			reg int *objs,ob,handle = info->di_handle,pxy[4];
			reg GRECT *w=&work;

			wind_update(BEG_UPDATE);
			wind_get(handle,WF_FIRSTXYWH,&w->g_x,&w->g_y,&w->g_w,&w->g_h);
			...
			while (w->g_w>0 && w->g_h>0)
			{
				...
				wind_get(handle,WF_NEXTXYWH,&w->g_x,&w->g_y,&w->g_w,&w->g_h);
			}
