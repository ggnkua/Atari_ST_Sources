...
static int do_popup(OBJECT *menu,boolean wrap,int last,int *dc)
{
	...
	for (;;)
	{
		...
		{
			if (old>0)
				objc_change(menu,old,0,desk.g_x,desk.g_y,desk.g_w,desk.g_h,menu[old].ob_state & (~SELECTED),1);

			if (current>0)
				objc_change(menu,current,0,desk.g_x,desk.g_y,desk.g_w,desk.g_h,menu[current].ob_state | SELECTED,1);
				...
				default:
					{
						...
						if ((obj=_is_hotkey(menu,scan_2_ascii(events.ev_mkreturn,events.ev_mmokstate)))>0)
						{
							if ((menu[obj].ob_state & DISABLED)==0 && (menu[obj].ob_flags & (SELECTABLE|HIDETREE))==SELECTABLE)
							{
								if (old>0)
									objc_change(menu,old,0,desk.g_x,desk.g_y,desk.g_w,desk.g_h,menu[old].ob_state & (~SELECTED),1);
								objc_change(menu,obj,0,desk.g_x,desk.g_y,desk.g_w,desk.g_h,menu[obj].ob_state | SELECTED,1);
								Event_Timer(100,0);
