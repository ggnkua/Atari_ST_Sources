...
int xalert(int def,int undo,int icn,BITBLK *usr_icn,int center,int width,char *title,char *txt,char *btns)
{
	char buttons[128],text[512];
	reg OBJECT *tree = alert_tree,*obj;
	reg char *p,*q,*b;
	reg int i,k,m,w;
	int x,y,c,user=usr_icn!=NULL;

	if (txt==NULL || btns==NULL)
		return (FAIL);
	...
	k = xdialog(alert_tree,title,TRUE,FALSE,AUTO_DIAL|MODAL|NO_ICONIFY);
