...
void graf_rt_slidebox(SLINFO *sl,int obj,int double_click)
{
	...
	if (info->di_flag>=WINDOW)
	{
		...
		wind_xget(info->di_win->handle,WF_FIRSTXYWH,&list.g_x,&list.g_y,&list.g_w,&list.g_h);
		if (list.g_w>0 && list.g_h>0)
		{
			wind_xget(info->di_win->handle,WF_WORKXYWH,&area.g_x,&area.g_y,&area.g_w,&area.g_h);
			top = rc_equal(&area,&list);
		}
		else
			return;
	}
	...
	event.ev_mbmask = 3;
	event.ev_mbclicks = 1;
	event.ev_mbstate = event.ev_mthicount = 0;
