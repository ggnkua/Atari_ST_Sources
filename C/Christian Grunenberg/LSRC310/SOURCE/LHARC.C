...
_DTA	_dta;
int 	file2,args,c_err,skipped,cmd,cmdupdate,cmdlist,errorlevel,oldtos;
int		_gemdos=SUCCS,Nfile,fbfiles,found,outfile,sfx;
long	arcpos0,arcpos1,lastarcpos,lastarclen,nextarcpos,nextoffset,arclen,maxlen,old_afx,buffer_cnt;
long	act_len,file_len,bsize,o_handle=-1,i_handle=-1,o_dev=-1,i_dev=-1,crypt_size,bufsize;
uchar	act_dir[MAXPATH],basedir[MAXPATH],workdir[MAXPATH],incldir[MAXPATH],inclpath[MAXPATH];
uchar	*unpack="*.ZOO,*.ZIP,*.AR[CJ],*.LZ[HS],*.LHA,*.T[GA]Z,*.GIF,*.GZ,*.JPG,*.HPK";
uchar	*infname,*bbuf,*ebuf,*fbuf,*fbnxt,*o_dir,print_buf[1536];
ulong	fblft,fbmax;
filebuf	*fblast;

uchar	SystemId,has_crc,back_1,back_2,compress;
int 	patno,exno,maxblk=64,UnixFile,regcnt,wild_arc,multi_arc,all;
int		fn_name=12,pt_name=128,Case=_PC_CASECONV,min_len,garbage,Device;

int 	n,heapsize,hdr_len,header_len,ignfile;
uchar	*buf=text_buf,*pager,*com_name,*buffer_start,*buffer_last;
uchar	flg_r,flg_p,flg_x,flg_m,flg_a,flg_c,flg_v,flg_w,flg_z,flg_g,flg_L=2,flg_N,flg_B,flg_W,flg_5,flg_o,flg_l;
uchar	flg_d,flg_u,flg_s,flg_e,flg_unpacked,flg_j,flg_chk,flg_S,flg_R,flg_I,flg_U=1,flg_K,flg_D,base;
uchar	flg_t,flg_arc,flg_h,flg_i,flg_backup,flg_f,flg_4,flg_q,flg_A,flg_X,flg_Y,pnt='.',buffered;
...
uchar	swi[]="lo5UwCrpxmacntvhifzedgjqsLSRBAWXKYDybuk4PMNI";
uchar	*swipos[]={&flg_l,&flg_o,&flg_5,&flg_U,&flg_w,&flg_chk,&flg_r,&flg_p,&flg_x,&flg_m,&flg_a,&flg_c,&flg_n,&flg_t,&flg_v,&flg_h,&flg_i,
		&flg_f,&flg_z,&flg_e,&flg_d,&flg_g,&flg_j,&flg_q,&flg_s,&flg_L,&flg_S,&flg_R,&flg_B,&flg_A,&flg_W,&flg_X,&flg_K,&flg_Y,&flg_D};
...
uchar	arcname[MAXPATH],pathname[MAXPATH],backup1[MAXPATH],backup2[MAXPATH],key_word[256],*key;
uchar	filename[MAXPATH],dosfilename[MAXPATH],matchfilename[MAXPATH],comment[MAXCOMMENT+256];
uchar	buffer[(BUFFERSIZ+128+32)*3],*buffer_1,*buffer_3,*buffer_gen;
...
int sseek(FILE *file,long pos,long offset)
{
	if (pos>=arclen)
	{
...
{
	register uchar c,com[MAXCOMMENT+256],*dst=com,*src=comment;
...
int slash(register uchar *path,int set)
{
	if (path && *path)
	{
		while (*path++);
...
int chk_wild(uchar *fname,uchar *fspec,int Wild,int Case)
{
	if (*fspec)
	{
...
int gethdr(FILE *arc,LzHead *h)
{
	...
	*comment='\0';
	...
	if (!unix && !UnixFile && Case!=_PC_CASECONV)
	{
...
int read_attr(uchar *path,_DTA *dta)
{
	XATTR xattr;
	int ok;

	ok=(int) Fxattr(0,path,(uchar *) &xattr);
...
int execappend(void)
{
	register filebuf *f0;
	register uchar *q;
	int update,in_arc,cnt=0,old=0,file=1;
#ifdef __SHELL__
	int found;
#endif
...
#ifdef __SHELL__
	if (shell)
		SetMsg((flg_d) ? "Deleting original files..." : "Clearing archiv-bits...");
	else
#endif
	#if GERMAN
		print((flg_d) ? "Lîsche Original-Dateien..." : "Lîsche Archiv-Bits...",1);
	#else
		print((flg_d) ? "Deleting original files..." : "Clearing archiv-bits...",1);
	#endif
...
int openarc1(long size,uchar *buffer)
{
	int search;
...
void openbackup1(void)
{
	...
	if (Frename(0,infname=arcname,backup1))
...
void pack_afx(void)
{
	...
	while ((f0=f0->next)!=NULL)
	{
		...
		else
		{
			...
			case 2:
			#if GERMAN
				m="Datei bereits in LHarc-Format";
			#else
...
int extract(void)
{
	...
	while (gethdr(file1,&Hdr1))
	{
		...
		if (matchpat(matchfilename,-1,Hdr1.Attr))
		{
			...
			{
				if (cmd=='E' && (succs=tstdir(strcat(pathname,"\\"),1))!=0)
				{
					cnt++;
					if (UnixFile)
...
void make_attr(uchar *buf,int attr)
{
...
uchar *list_format="               %8lu %8lu %3d.%1d%% %2d-%02d-%02d %2d:%02d:%02d ---- -   - %04X  [%d]";
uchar *list_end="-------------- -------- -------- ------ -------- --------\r\n";

int list(void)
{
	register uchar buf[120],*p;
...
void OneNewFile(uchar *p)
{
	...
	if (!patno && !base && !cmdlist && (p[len]=='\\' || p[len]==':'))
	{
...
void copyfile(FILE *Source,FILE *Dest,uchar *outfname,long size,int crcflg,int bufflg)
{
	register uchar *ptr;
