...
{
  if ( clipboard.img.fd_addr ) free( clipboard.img.fd_addr ) ;
  if ( clipboard.mask.fd_addr ) free( clipboard.mask.fd_addr ) ;
    ...
    default       :
                    switch( config.type_undo )
                    {
                      case TUNDO_MEMORY : type_alloc = 1 ;
                                          break ;
                      case TUNDO_KO :     memory_left = (size_t) Malloc( -1L ) ;
                                          if ( memory_left < limit_bytes )
                                            type_alloc = 2 ;
                                          else
                                            type_alloc = 1 ;
                                          break ;
                      case TUNDO_SMART :  pt_alloc = malloc( taille ) ;
...
void FreeUndoBuffer(UNDO_DEF *undo)
{
  ...
  vimage->NbUndoBuffers-- ;
  if ( undo == NULL ) return ;
  if ( undo->img.fd_addr ) free( undo->img.fd_addr ) ;
  if ( undo->spec ) free( undo->spec ) ;
  if ( undo->palette ) free( undo->palette ) ;
  if ( undo->disk )
  {
    char name[200] ;
...
UNDO_DEF *AllocateUUndoBuffer(GEM_WINDOW *gwnd)
{
  ...
  redo = (UNDO_DEF *) calloc( 1, sizeof(UNDO_DEF) ) ;
  if ( redo )
  {
    VXIMAGE *vimage ;
