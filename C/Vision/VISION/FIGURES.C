...
int check_color(VXIMAGE *vimage, int color)
{
  ...
  if ( !Truecolor )
    if ( ( 1 << vimage->inf_img.nplans ) <= color )
    {
      form_stop( 1, msg[MSG_COLORERROR] ) ;
      err = 1 ;
    }
...
void action_gomme(GEM_WINDOW *gwnd)
{
  ...
  do
  {
    ...
    sprintf( buf, msg[MSG_XY], posx+xm-xi, posy+ym-yi ) ;
...
void traite_point(GEM_WINDOW *gwnd)
{
  ...
  do
  {
    ...
    sprintf( buf, msg[MSG_XY], posx+(xm-xi)/zoom_level, posy+(ym-yi)/zoom_level ) ;
    GWSetWindowInfo( gwnd, buf ) ;
  }
  while (bm == 1) ;
  vs_clip(handle, 0, xy) ;
...
void traite_trace(GEM_WINDOW *gwnd)
{
  ...
  posx = (int) (0.5+(double)posx/1000.0*(double)(vimage->raster.fd_w-wx)) ;
  posy = (int) (0.5+(double)posy/1000.0*(double)(vimage->raster.fd_h-wy)) ;
  ...
  do
  {
    ...
    sprintf( buf, msg[MSG_XY], posx+xm2-xi, posy+ym2-yi ) ;
    GWSetWindowInfo( gwnd, buf ) ;
  }
  while (bm == 1) ;
  vs_clip(handle, 0, xy) ;
...
void traite_peinture(GEM_WINDOW *gwnd)
{
  ...
  {
    form_stop(1, msg[MSG_ZOOMFILL]) ;
    return ;
  }

  vswr_mode(handle, mode_vswr) ;
  gwnd->GetWorkXYWH( gwnd, &x, &y, &w, &h) ;
  GWSetWindowInfo( gwnd, msg[MSG_FILL] ) ;
  zoom_level = vimage->zoom_level ;
  gwnd->GetWorkXYWH( gwnd, &xi, &yi, &wx, &wy) ;
  wx   = wx/zoom_level ;
  wy   = wy/zoom_level ;
...
/*  update_tools(mesg, flag) ;*/
  ...
  do
  {
    ...
    sprintf( buf, msg[MSG_XY], posx+xy[0]-x, posy+xy[1]-y ) ;
    GWSetWindowInfo( gwnd, buf ) ;
  }
  while (bm == 0) ;
  v_hide_c(handle) ;
...
void traite_cercle(GEM_WINDOW *gwnd, int fill)
{
  ...
  do
  {
    ...
    if ((xm2 != xm1) || (ym2 != ym1))
    {
      ...
      sprintf( buf, msg[MSG_R], abs(dx), abs(dy) ) ;
      GWSetWindowInfo( gwnd, buf ) ;
    }
  }
  while (bm == 1) ;
  ...
  GWSetWindowInfo( gwnd, msg[MSG_CERCLE] ) ;
...
void traite_rectangle(GEM_WINDOW *gwnd, int fill)
{
  ...
  do
  {
    ...
    sprintf( buf, msg[MSG_LH], abs(xm2-xm), abs(ym2-ym) ) ;
    GWSetWindowInfo( gwnd, buf ) ;
  }
  while (bm == 1) ;
  ...
  x2 = xm1 ; y2 = ym1 ;
  dx = dy = 0 ;
  GWSetWindowInfo( gwnd, msg[MSG_RECTANGLE] ) ;
...
void traite_ligne(GEM_WINDOW *gwnd)
{
  ...
  do
  {
    ...
    if ((xm2 != xm1) || (ym1 != ym2))
    {
      sprintf( buf, msg[MSG_XY], vimage->x1 + (xm2-xi) / vimage->zoom_level, vimage->y1 + (ym2-yi) / vimage->zoom_level ) ;
      GWSetWindowInfo( gwnd, buf ) ;
      v_hide_c(handle) ;
      line(xm, ym, xm1, ym1) ;
      if ((km & 0x03) == 0)
