...
int de_entrelace(MFDB *out)
{
  ...
  if ( NaturalFormat )
  {
    new_natural_format = (unsigned char *) malloc( (long)out->fd_w*(long)out->fd_h ) ;
    err = ( new_natural_format == NULL ) ;
    if ( err ) free( NaturalFormat ) ;
...
int gif_decompress(unsigned char *buffer, long taille, MFDB *out, char *name, GEM_WINDOW *wprog)
{
  ...
  data = (unsigned char *) malloc(DCPRS_SIZE) ;
  ...
  i2r_data    = data ;
  StdProgWText( wprog, 1, msg[MSG_DECODAGELZW] ) ;
...
int gif_compress(MFDB *img, FILE *stream, GEM_WINDOW *wprog)
{
  ...
  buffer = (unsigned char *) malloc(img->fd_w) ;
  if (buffer == NULL) return(-3) ;
  StdProgWText( wprog, 1, msg[MSG_ENCODAGELZW] ) ;
