...
int tiff_tags(FILE *stream, INFO_IMAGE *inf)
{
  ...
  tags = (TIFF_TAG *) malloc(nb_tags*sizeof(TIFF_TAG)) ;
...
int tiff_read(FILE *stream, MFDB *out, GEM_WINDOW *wprog)
{
  ...
  itab_sbc = (unsigned int *) malloc(taille) ;
  ...
  itab_so = (unsigned int *) malloc(taille) ;
  if (itab_so == NULL)
  {
    free(itab_sbc) ;
    return(-3) ;
  ...
  if (Compression == 32773U)
  {
    ...
    dcp_out = (unsigned char *) malloc(size) ;
    if (dcp_out == NULL)
    {
      free(itab_sbc) ;
      free(itab_so) ;
  ...
  if (Compression == 5) StdProgWText( wprog, 1, msg[MSG_DECODAGELZW] ) ;
  else if (Compression == 32773U) StdProgWText( wprog, 1, msg[MSG_DECODAGEPCB] ) ;
...
int tiff_load(char *name, MFDB *out, GEM_WINDOW *wprog)
{
  ...
  {
    NaturalFormat = malloc( (long)out->fd_w*(long)out->fd_h ) ;
...
int tiff_sauve(char *name, MFDB *img, INFO_IMAGE *info, GEM_WINDOW *wprog)
{
  ...
  ltab_so = (unsigned long *) malloc(sizeof(long)*StripsPerImage) ;
  if (ltab_so == NULL)
  {
    fclose(stream) ;
    return(-3) ;
  ...
  ltab_sbc = (unsigned long *) malloc(4*StripsPerImage) ;
  if (ltab_sbc == NULL)
  {
    free(ltab_so) ;
    fclose(stream) ;
  ...
  if (Compression == 5) StdProgWText( wprog, 1, msg[MSG_ENCODAGELZW] ) ;
  if ((Compression == 5) && (MachineInfo.cpu >= 30L)) StdProgDisplay030( wprog, 1 ) ;
  ...
  if (BitsPerSample != 1) free(buffer) ;
  free(ltab_sbc) ;
  free(ltab_so) ;
