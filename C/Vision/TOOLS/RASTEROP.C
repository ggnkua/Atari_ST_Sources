...
int raster_zoom(MFDB *in, MFDB *out, int xy[4], int x_level, int y_level)
{
  ...
  if ( local_alloc ) free ( temp.fd_addr ) ;
...
{
  out->fd_w = 0 ;
  if (x_level > 0) out->fd_w = (1+xy[2]-xy[0])*x_level ;
  else
  if (x_level < 0) out->fd_w = (1+xy[2]-xy[0])/(-x_level) ;
  if (out->fd_w % 16) out->fd_w = (out->fd_w & 0xFFF0)+16 ;
  out->fd_wdwidth = out->fd_w/16 ;
  out->fd_h = 0 ;
  if (y_level > 0) out->fd_h = (1+xy[3]-xy[1])*y_level ;
  else             out->fd_h = 1+xy[3]-xy[1] ;
  ...
  if (x_level != 0)
  {
    if (x_level > 0) zoom_cup(in, out, xy, x_level) ;
    else             zoom_cdown(in, out, xy, -x_level) ;
  }

  /* Traitement des lignes */
  xy[2] = out->fd_w-1 ; xy[3] = xy[3]-xy[1] ;
  xy[0] = 0 ;           xy[1] = 0 ;
  if (x_level == 0) vro_cpyfm(handle, S_ONLY, xy, in, out) ;
  if (y_level != 0)
  {
    if (y_level > 0) zoom_lup(out, out, xy, y_level) ;
    else             zoom_ldown(out, out, xy, -y_level) ;
  }
...
void zoom_pcxstdplane(MFDB *in, MFDB *out, int pc, GEM_WINDOW *wprog)
{
  ...
  src = (unsigned char *) malloc(in->fd_w) ;
  if (src == NULL) return ;
  dst = (unsigned char *) malloc(out->fd_w) ;
  if (dst == NULL)
  {
    free(src) ;
    return ;
  }
  tab = (int *) malloc(out->fd_w << 1) ;
  if (tab == NULL)
  {
    free(dst) ;
    free(src) ;
...
int raster_pczoom(MFDB *in, MFDB *out, int pcx, int pcy, GEM_WINDOW *wprog)
{
  ...
  if ((pcx != 100) && (pcy != 100)) free(temp.fd_addr) ;
...
int smart_hsym(MFDB *img)
{
  ...
  lo_ligne = (16*img->fd_wdwidth*img->fd_nplanes)/8 ;
...
int pixel_init(MFDB *img)
{
  ...
  pixel_mode = 0 ;
  pixel_lgoff = (long *) malloc(img->fd_h*sizeof(long)) ;
  if (pixel_lgoff == NULL)
  {
    pixel_mode = -1 ;
  ...
  if (img->fd_nplanes <= 8)
    pixel_adrin = (unsigned char *) malloc(nbpts) ;
  else
...
{
  if (pixel_adrin) free(pixel_adrin) ;
  free(pixel_lgoff) ;
}
...
int raster_rfree(MFDB *in, MFDB *out, int angle, GEM_WINDOW *wprog)
{
  ...
  tincx = (int *) malloc(sizeof(int)*out->fd_w) ;
  if (tincx == NULL)
  {
    free(buffer) ;
    free(out->fd_addr) ;
  ...
  tincy = (int *) malloc(sizeof(int)*out->fd_w) ;
  if (tincy == NULL)
  {
    free(tincx) ;
    free(buffer) ;
...
/* Test de perfs
{
  ...
  clock_t t0 ;
  clock_t times[10] ; 
  int           *pti = (int *) malloc( 2L * nb_pts ) ;
  unsigned char *ptc = (unsigned char *) malloc( nb_pts ) ;
