...
extern void	*start,	*curr_aff,	*sel_obj;		/* void: pour ne pas charger "object.h"	*/
...
int attr[10];
	...
	case WA_DNPAGE :				/* Page vers le bas	*/
		if ((nw->w_lin + hc) < n_obj)	/* Si pas d‚j… … la fin	*/
		{
			nw->w_lin = min (nw->w_lin + hc,	 n_obj - hc);	/* Nouvelle ligne	*/
			dummy=0;
			curr_aff=cherche_objet(start,	nw->w_lin,	&dummy);
			redraw (xw,	 yw,	 ww,	 hw);	/* R‚affichage	*/
			sliders (nw); 				/* Actualiser les sliders	*/
...
int attr[10];
	...
	hc=hw/attr[9];
		...
		/* Calcul de la ligne :	*/
	nw->w_lin = ((long)slide * (n_obj - (long)hc) / 1000);
	if (nw->w_lin + hc > n_obj)	/* Corrections	*/
		nw->w_lin = n_obj - hc;
	if (nw->w_lin < 0)
		nw->w_lin = 0;
...
int attr[10];
	...
	if(n_obj==0)
		...
		wind_set (nw->w_handle,	 WF_VSLSIZE,					/* Taille slider vertical	*/
						(int)( (long)hc *1000/n_obj) );

	nw->w_lin = min (nw->w_lin,	 n_obj - hc); /* Calculer la ligne	*/
	...
	slide = (long)nw->w_lin * 1000 / (long)(n_obj - hc); 	/* Position slider vertical	*/
	if (slide < 0)
		slide = 0;
	if (slide > 1000)
		slide = 1000;
