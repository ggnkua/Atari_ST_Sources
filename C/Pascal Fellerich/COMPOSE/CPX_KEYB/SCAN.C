...
Keyword keywords[] = {
	...
	"+",		OP_PLUS,
	"-",		OP_MINUS,
	/* "%",		OP_MODULO, !!CONFLICTS WITH %BINCONST!!	*/
	...
	"ORDERsensitive",	K_ORDER,
	"NOORDERsensitive",	K_NOORDER,
	"NOTHing",			K_NOTHING,
	"KEYBoard",			K_KEYBOARD,
	"BOTH",				K_BOTH,
...
/*
	...
	if (new_table->name[0]==NUL) return TRUE;
...
BOOL get_symbol(void)
{
	...
	symbol.token=0;					/* zero out the current symbol */
	symbol.name[0]=0;
...
BOOL get_constant(void)
{
	...
	case '5':	case '6':	case '7':	case '8':	case '9':
		...
		if (len==0L) type = 0;				/* signal failure */
			...
			strncpy(symbol.name, ptr, len); symbol.name[len]=NUL;
		break;
	...
	case '%':				/* binary constant: %0100100011111 */
		...
		if (len!=0L) {
			type = BINCONST;
			strncpy(symbol.name, ptr+i, len); symbol.name[len]=NUL;
			len+=i;
		}
		break;
