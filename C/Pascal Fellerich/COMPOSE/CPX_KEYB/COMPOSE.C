	...
	do {
		...
		if (button == MESSAGE) {
			...
			case M_COMPOSE:
				...
				Compose(currstate.compose | (Compose(INQUIRE) & ~ON) );
				break;
			...
			case M_USEDTABLE:
				...
				/* geladene Einstellungen auslesen */
				currstate.compose = (int)Compose(INQUIRE);
				currstate.extkey  = (int)Extkey(INQUIRE);
				currstate.alt_nnn = (int)Alt_nnn(INQUIRE);
				currstate.deadkey = (int)Deadkey(INQUIRE,NULL);
				currstate.mapping = (int)Cbioskeys(INQUIRE);
	...
	do {
		...
		if (button == MESSAGE) {
			...
			case C_COMPMODE:
				...
				if (newcompose & IMPLICIT_COMPOSE) old=2;
				objc_offset(tree, C_COMPMODE, &popbut.g_x, &popbut.g_y);
				popbut.g_w=ObW(C_COMPMODE); popbut.g_h=ObH(C_COMPMODE);

				new=xpcb->Popup(pop(POPCOMPMODE), 3, old, IBM,
												&popbut, ObRectP(ROOT));
				if ( new >=0  &&  new!=old ) {
					newcompose &= ~(MULTICHAR_MODE|IMPLICIT_COMPOSE);
					if (new==1) newcompose |= MULTICHAR_MODE;
					if (new==2) newcompose |= MULTICHAR_MODE|IMPLICIT_COMPOSE;
					set_compstate(tree, newcompose);
...
void set_compstate(OBJECT *tree, int state)
{
	if (state & IMPLICIT_COMPOSE)
					TedText(C_COMPMODE) = say(MC_IMPLICIT);
	else if (state & MULTICHAR_MODE)
					TedText(C_COMPMODE) = say(MC_EXPLICIT);
...
/*
void read_ckb(void)               old file format!
{
	long 	hn;
	long	fs;
	CKB_FILE_HEADER *cfh;
