...
bufsize equ 26000	; maximum size of buffer
...
_mod_init:
	...
	beq.s	mi_reterr	; If 8-bit samples are not supported, return error
...
minogsxb:
	...
	move.w	#3,bufmode
	tst.w	d0
	beq.s	mibmokay		; If Timer-A is not okay, we test for MFP-i7
	move.w	#1,-(sp)
	move.w	#1,-(sp)		; MFP-i7
	move.w	#135,-(sp)
	trap	#14				; Setinterrupt()
	...
	move.w	#4,bufmode
	tst.w	d0
	beq.s	mibmokay		; If also MFP-i7 fails, we try NSetinterrupt
	pea 	i_gsxb
	move.w	#1,-(sp)
	move.w	#2,-(sp)
	move.w	#135,-(sp)
...
notmode0:
	cmp.w	#1,d0
	beq.s	setbo3			; Polling with looping
	cmp.w	#2,d0
	bne.s	notmode2
...
copyrt1:
	dc.b	"XBIOS MOD Player",0
copyrt2:
	dc.b	"(C) by Griff & Thothy",0
