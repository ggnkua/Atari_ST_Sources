...
V 1.7
	...
	140594 ga: TC-Bugs (charlinewidth/weiûe Buttons) weg. (BB/BC)
	140594 ga: ZurÅcksetzen der Bank2 beim Speichern behoben (ach, was bin ich gut...)
	...
	141094 ga: weiûer Rand bei (links+rechts)%4=2 weg
	231094 ga: SM124: Wunderflag wurde fÑlschlicherweise gesetzt->weg
	231094 ga: Hsync mit Shift Ñnderbar/Popup fÅr 32 MHz
	231094 ga: Accessory-Save saved Saver jetzt sicher ab!
	051194 ga: Saver reagiert nicht mehr auf MIDI-Ints
	061194 ga: Shift/alternate beim auflîsungswechsel, 'n' beim booten
...
V final (me)
	210996 me: Auf geht's.
			   - Abbruch mit Ctrl-Q.
			   - FFC auch fÅr nichtkonfigurierte
			     NichtkompatibilitÑtsmodi.
	...
	270996 me: - Cheat-Modus, Teil 2
			   - Erkennung der Landessprache und laden des
			     entsprechenden RSC-Files:
			     	BLOWCONF.RSC fÅr Deutschland
			     	BLOWFRA.RSC  fÅr Frankreich
			     	BLOWGB.RSC	 fÅr Englang/Amerika
			   - FX-Takterkennung
	040197 ga: Bei SM124/TV sollte man keine MS-Daten einstellen kînnen
	040197 ga: Etwas groûzÅgigere Frequenzbereiche TV/SM
 */
...
void main()
{
	...
	while(1) {
		...
		*/
		...
		if ((ex_ob==OPTIONS)&&(act_TREE==TREE1))	/* muû nach dem if (act_TREE=TREE1) stehen! */
		{
		...
		if ((ex_ob==LOAD)&&(act_TREE==TREE1))	/* muû nach dem if (act_TREE=TREE1) stehen! */
		{
			act_TREE=TREE3;
			format_configs(config_pos);
