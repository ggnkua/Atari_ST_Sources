...
box.MinY = live_top + (live_lines - height)>>1;
...
sput_cmap();	/* make colors visible! */
stash_cmap();	/* continue making colors visible ... */
...
save_cscreen = cscreen;
draw_on_screen();
save_zscale_cursor = zscale_cursor;
zscale_cursor = 0;
...
restore_buffers()
{
Setscreen(cscreen = ocscreen, -1L, -1);
if (cur_menu == NULL)
	rezoom();
if (!omouse_on)
	hide_mouse();
...
unstash_cmap();
hide_mouse();
Setscreen(cscreen = save_cscreen, -1L, -1);
zscale_cursor = save_zscale_cursor;
see_top_lines();
show_mouse();
wait_penup();
}
