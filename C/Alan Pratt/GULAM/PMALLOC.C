...
#define	NULL		0L
#define local	static	/* remove static for debug */
...
# define P_(s) ()
#endif
local lstchunks P_((HEAP *heap));
local insert P_((HEAP *heap, MCB *q));
local HEAP *allocheap P_((UI x));
#undef P_
...
register UI	x;
{
	...
	if (heap = allocheap(((UI)(max(x, HEAPSIZE)))))
...
register MCB *mcb;
{
	...
	{  /* GEMDOS Mfree() if this heap is now unused */
	   ...
	   && (UL) mcb->msize + (UL) ((uint)sizeof(HEAP)) + (UL) MI == heap->hsize)
	   {	h->hlink = heap->hlink;
		Mfree(heap);
	}  }
	return 0;
