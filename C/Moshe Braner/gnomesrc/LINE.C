	...
	register int	used;
{
	...
	block = LINESIZE + size;
	lp = (LINE *) malloc(block);
	if (lp == NULL) {
		dtmpbufs();			/* mb: clear temp bufs	*/
		lp = (LINE *) malloc(block);	/*  and try again	*/
		if (lp == NULL) {
...
linsert(n, c, overstrike)
{
	...
	doto = curwp->w_doto;
	if (insert)
		newsize = llength(lp1) + n;
	else
		newsize = doto + n;
	if (newsize > lsize(lp1)) {		/* hard: need new space */
		if ((lp2=lalloc(newsize))==NULL)
			return (FALSE);
		cp1 = &(lp1->l_text[0]);		/* old text  */
		cp2 = &(lp2->l_text[0]);		/* new space */
		cp3 = &(lp1->l_text[doto]);		/* cursor */
		while (cp1 < cp3)
			*cp2++ = *cp1++;	/* copy text left of dot */
		if (insert) {			/* leave gap, copy rest  */
			cp2 += n;
			cp3 = &(lp1->l_text[llength(lp1)]);
			while (cp1 < cp3)
				*cp2++ = *cp1++;
		}
		lp1->l_bp->l_fp = lp2;		/* link in new line	*/
		lp2->l_fp = lp1->l_fp;
		lp1->l_fp->l_bp = lp2;
		lp2->l_bp = lp1->l_bp;
		free((char *) lp1);
		...
		if (overstrike) {
			if (llength(lp2) < doto+n)
				lp2->l_used = doto+n;
		} else {
			lp2->l_used += n;
			cp2 = &(lp2->l_text[llength(lp2)]);
			cp1 = cp2-n;
			cp3 = &(lp2->l_text[doto]);
			while (cp1 > cp3)
				*--cp2 = *--cp1;
	...
	cp2 = &(lp2->l_text[doto]);	/* Add the new characters */
	for (i=0; i<n; i++)
		*cp2++ = c;
	...
	while (wp != NULL) {
		...
		if (wp->w_dotp == lp1) {
			wp->w_dotp = lp2;
			if (wp==curwp || (insert && (wp->w_doto > doto)))
				wp->w_doto += n;
		}
		if (wp->w_markp == lp1) {
			wp->w_markp = lp2;
			if (insert && (wp->w_marko > doto))
				wp->w_marko += n;
		}
		wp = wp->w_wndp;
	}
...
ldelnewline()
{
	...
	if (llength(lp2) <= lsize(lp1)-llength(lp1)) {
		cp1 = &lp1->l_text[llength(lp1)];
		cp2 = &lp2->l_text[0];
		while (cp2 < &lp2->l_text[llength(lp2)])
			*cp1++ = *cp2++;
		wp = wheadp;
		while (wp != NULL) {
			if (wp->w_linep == lp2)
	...
	while (cp1 < &lp1->l_text[llength(lp1)])
		*cp2++ = *cp1++;
	cp1 = &lp2->l_text[0];
	while (cp1 < &lp2->l_text[llength(lp2)])
		*cp2++ = *cp1++;
	lp1->l_bp->l_fp = lp3;
	lp3->l_fp = lp2->l_fp;
	lp2->l_fp->l_bp = lp3;
	...
	register int size;
{
	...
	if (newkbuf == NULL) {
		dtmpbufs();			/* mb: clear temp bufs	*/
		newkbuf = malloc(size);		/*  and try again	*/
		if (newkbuf == NULL)
			return (FALSE);
	}
	if (kbufp != NULL) {
