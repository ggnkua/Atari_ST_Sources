...
*/
	...
	/*	if (*p=='<') { cmd=1; inquote=0; } */
		...
		if (*p=='>') {
			...
			if (memcmp( p-3, "listing", 7 )==0) { inpre=1; }
			if (memcmp( p-4, "/listing", 8 )==0) { inpre=0; }
		}
		...
		/*	y=work->g_y; */
			...
			while(start<(cs->buff+finfo.size)) { /* until past end */
				...
				/*	if (handle_escape_events()==27) { break; }	/* escape from format loop */
					...
					while (*start!='>') { start++; }
					if (cmd==1)  { cs->cmd_end=start+1; }
					if (cmd==2)  { cs->cmd_tail_end=start+1; }
