...
#define VERSION "0.96  Jan  5, 2000"
...
void generic_window_build_buffer( int wx, char *filename )
{
	FILE *fid;
	struct FILEINFO info;
	...
	if (prefs.window_buffers!='Y') { return; }
	if (dfind(&info, filename, 0)) { return; } 	/* file not found */
	overage = (info.size/10)+100;
	overage+= info.size;
/*	if (overage<10000) { overage=10000; } */
	buff = calloc( overage, sizeof(char) );	/* allocate buffer */
	if (buff==NULL) { return; }				/* insufficient memory */
...
void generic_window_text( int wid, char *filename )
{
	...
	if (wx!=9) {
		if (windows[wx].buffer==NULL) { generic_window_build_buffer( wx, filename ); }
	}
	if (windows[wx].buffer!=NULL)  {
...
void do_message_events(int *mbuff)
{
	...
	case WM_CLOSED:
		...
		if (mbuff[3]==windows[3].id) { handle_generic_close( 3 ); }
		if (mbuff[3]==windows[7].id) { handle_generic_close( 7 ); }
