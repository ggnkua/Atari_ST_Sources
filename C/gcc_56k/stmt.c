/* Expands front end tree to back end RTL for GNU C-Compiler
   ...
   $Id: stmt.c,v 1.10 91/10/23 17:06:48 pete Exp $
...
extern rtx local_function_incoming_arg ( );
#endif
...
#if defined( DSP56000 )
      /* unsigend equality is special on the 56k. */
      if ( unsignedp )
      {
	  emit_jump_insn (gen_bequ (label));
      }
      else
      {
...
#if defined( DSP96000 ) || defined( DSP56000 )
	     /* an overflow case breaks the sequential series. */
	     && ! tree_int_cst_equal (np->low, build_int_2 (0, 0))
#endif
  ...
  for (parm = DECL_ARGUMENTS (fndecl); parm; parm = TREE_CHAIN (parm))
	  ...
	  || DECL_ARG_TYPE (parm) == NULL)
	{
	 ...
	 as it should be during execution of this function.  */
      ...
      /* this, and it's later #if ... are needed to get multi-word
	 arguments to work correctly on an upward growth stack. We
	 kludge here: we don't want to bump the cuumulative stack offset if
	 the arg is (in)coming in a reg. I did this to minimize diff lines
	 and to get it to work for the 56/96 with register args - not in
	 a way to fix gcc in general. */
      if ( ! FUNCTION_INCOMING_ARG ( args_so_far, passed_mode, 
				    DECL_ARG_TYPE ( parm ), 
				    ! ( TREE_CHAIN ( parm ) == 0 && vararg )))
      {
	  ADD_PARM_SIZE ( stack_args_size, 
...
#else
      stack_offset = stack_args_size;
      stack_offset.constant = - stack_offset.constant;
#endif
	  ...
	  )
	{
	  ...
	  /* Add it in.  */
#if ! defined( DSP56000 ) && ! defined( DSP96000 )
      /* this, and it's earlier #if ... are needed to get multi-word
	 arguments to work correctly on an upward growth stack. */
     ...
     rtx sizex;
{
  register int size;
#if defined( DSP56000 )
  if ( CONST_DOUBLE == GET_CODE ( sizex ))
  {
      sizex = gen_rtx ( CONST_INT, VOIDmode, CONST_DOUBLE_LOW ( sizex ));
  }
#endif  
...
#if defined( DSP96000 ) || defined( DSP56000 )
    {
	current_func_info |= FUNC_RETURNS_VOID;
...
#if defined( DSP96000 ) || defined( DSP56000 )
      /* let our epilog know if it should tst or ftst ! */
      if (( SFmode == GET_MODE ( DECL_RTL ( DECL_RESULT ( subr )))) ||
	  ( DFmode == GET_MODE ( DECL_RTL ( DECL_RESULT ( subr )))))
      {
	  current_func_info |= FUNC_RETURNS_FLOAT;
      }
#endif
