/* Convert tree expression to rtl instructions, for GNU compiler.
   Copyright (C) 1988 Free Software Foundation, Inc.
    ...
    $Id: expr.c,v 1.25 91/11/22 19:42:59 pete Exp $
...
#if defined( _MSDOS )
void error_with_decl ( tree decl, ... );
#endif
     ...
     int unsignedp;
{
...
#endif
#ifdef HAVE_zero_extendpsidi2
	  if (HAVE_zero_extendpsidi2 && from_mode == PSImode)
	    emit_unop_insn (CODE_FOR_zero_extendpsidi2, to, from, ZERO_EXTEND);
	  else
#endif
...
#endif
#ifdef HAVE_extendpsidi2
      else if (HAVE_extendpsidi2 && from_mode == PSImode)
	emit_unop_insn (CODE_FOR_extendpsidi2, to, from, SIGN_EXTEND);
#endif
  ...
  if (to_mode == SImode && from_mode == PSImode)
    {
      if (unsignedp)
	{
#ifdef HAVE_zero_extendpsisi2
	  if (HAVE_zero_extendpsisi2)
	    emit_unop_insn (CODE_FOR_zero_extendpsisi2, to, from, ZERO_EXTEND);
	  else
#endif
	    abort ();
	}
      else
	{
	  ...
	  if (HAVE_extendpsisi2)
	    emit_unop_insn (CODE_FOR_extendpsisi2, to, from, SIGN_EXTEND);
	  else
#endif
	    abort ();
	}
  ...
  if (to_mode == HImode && from_mode == PSImode &&
      GET_MODE_BITSIZE (to_mode) > GET_MODE_BITSIZE (from_mode))
    {
      if (unsignedp)
	{
#ifdef HAVE_zero_extendpsihi2
	  if (HAVE_zero_extendpsihi2)
	    {
	      emit_unop_insn (CODE_FOR_zero_extendpsihi2, to, from, ZERO_EXTEND);
	      return;
	    }
#endif
	}
	...
	{
#ifdef HAVE_extendpsihi2
	  if (HAVE_extendpsihi2)
	    {
	      emit_unop_insn (CODE_FOR_extendpsihi2, to, from, SIGN_EXTEND);
	      return;
	    }
#endif
	}
  ...
  if (to_mode == PSImode && from_mode == HImode &&
      GET_MODE_BITSIZE (to_mode) > GET_MODE_BITSIZE (from_mode))
    {
      if (unsignedp)
	{
#ifdef HAVE_zero_extendhipsi2
	  if (HAVE_zero_extendhipsi2)
	    {
	      emit_unop_insn (CODE_FOR_zero_extendhipsi2, to, from, ZERO_EXTEND);
	      return;
	    }
#endif
	}
	...
	{
#ifdef HAVE_extendhipsi2
	  if (HAVE_extendhipsi2)
	    {
	      emit_unop_insn (CODE_FOR_extendhipsi2, to, from, SIGN_EXTEND);
	      return;
	    }
#endif
	}
  ...
  if (to_mode == QImode && from_mode == PSImode &&
      GET_MODE_BITSIZE (to_mode) > GET_MODE_BITSIZE (from_mode))
    {
      if (unsignedp)
	{
#ifdef HAVE_zero_extendpsiqi2	
  if (HAVE_zero_extendpsiqi2)
	    {
	      emit_unop_insn (CODE_FOR_zero_extendpsiqi2, to, from, ZERO_EXTEND);
	      return;
	    }
#endif
	}
	...
	{
#ifdef HAVE_extendpsiqi2
	  if (HAVE_extendpsiqi2)
	    {
	      emit_unop_insn (CODE_FOR_extendpsiqi2, to, from, SIGN_EXTEND);
	      return;
	    }
#endif
	}
  ...
  if (to_mode == PSImode && from_mode == QImode &&
      GET_MODE_BITSIZE (to_mode) > GET_MODE_BITSIZE (from_mode))
    {
      if (unsignedp)
	{
#ifdef HAVE_zero_extendqipsi2
	  if (HAVE_zero_extendqipsi2)
	    {
	      emit_unop_insn (CODE_FOR_zero_extendqipsi2, to, from, ZERO_EXTEND);
	      return;
	    }
#endif
	}
	...
	{
#ifdef HAVE_extendqipsi2
	  if (HAVE_extendqipsi2)
	    {
	      emit_unop_insn (CODE_FOR_extendqipsi2, to, from, SIGN_EXTEND);
	      return;
	    }
#endif
	}
  ...
  if (from_mode == SImode && to_mode == PSImode)
    {
  ...
  if (from_mode == HImode && to_mode == PSImode)
    {
#ifdef HAVE_trunchipsi2
      if (HAVE_trunchipsi2)
	{
	  emit_unop_insn (CODE_FOR_trunchipsi2, to, from, UNKNOWN);
	  return;
	}
#endif
      abort ();
  ...
  if (from_mode == PSImode && to_mode == HImode)
    {
#ifdef HAVE_truncpsihi2
      if (HAVE_truncpsihi2)
	{
	  emit_unop_insn (CODE_FOR_truncpsihi2, to, from, UNKNOWN);
	  return;
	}
#endif
      abort ();
  ...
  if (from_mode == QImode && to_mode == PSImode)
    {
#ifdef HAVE_truncqipsi2
      if (HAVE_truncqipsi2)
	{
	  emit_unop_insn (CODE_FOR_truncqipsi2, to, from, UNKNOWN);
	  return;
	}
#endif
      abort ();
  ...
  if (from_mode == PSImode && to_mode == QImode)
    {
#ifdef HAVE_truncpsiqi2
      if (HAVE_truncpsiqi2)
	{
	  emit_unop_insn (CODE_FOR_truncpsiqi2, to, from, UNKNOWN);
	  return;
	}
#endif
      abort ();
     ...
     int align;
{
  ...
  if (GET_CODE (size) == CONST_INT
      ...
      if (HAVE_movstrsi)
	{
	  ...
	  rtx fee = gen_rtx (CONST_INT, VOIDmode, align);
	  rtx foo = gen_movstrsi (x, y, size, fee );
	  emit_insn ( foo );
#else
     ...
     rtx size;
{
  register rtx temp;
#if defined( DSP56000 )
  if ( GET_CODE (size) == CONST_DOUBLE )
  {
      size = gen_rtx ( CONST_INT, VOIDmode, CONST_DOUBLE_LOW ( size ));
  }
#endif
...
#endif
{
  ...
  /* NOTE: this fix will no longer work if FUNCTION_ARG_PARTIAL_NREGS
     is really used !!!! -tje */
  for (count = 0; count < nargs; count++)
    {
      regvec[count] = FUNCTION_ARG (args_so_far,
				    argvec[count].mode, (tree)0, 1);
  ...
  for (count = 0; count < nargs; count++, argnum += inc)
	...
	emit_push_insn (val, mode, 0, 0, partial, reg, 0, argblock,
      ...
      if (reg == 0 || partial != 0)
#if defined( DSP56000 )
	  /* see comment in expand_call ~line 4450, expr.c */
	  if ((( DFmode == mode ) ||
	       ( SFmode == mode ) ||
	       ( DImode == mode )) && ( 'l' == memory_model ))
	  {
	      arg_size = 1;
	  }
	  else
...
#if defined( DSP56000 ) || defined( DSP96000 )
	      /* we perform Pmode additions as Pmode <= Pmode + SImode.
	       * it is really wrong to convert the index to Pmode, because
	       * Pmode + Pmode is not defined. 
	       */
	    index = convert (integer_type_node, index);
#else
...
#if defined( DSP56000 ) || defined( DSP96000 )
						 /* whoa! there is no such
						  * thing as a pointer mpy!
						  * we need to do an uint mpy
						  * and add int + ptr to get
						  * the ptr which will be the
						  * subj of INDIRECT_REF.
						  */
						 integer_type_node,
#else
...
/* ??? cv's were used to effect here to combine additive constants
 ...
 binop2:
#if defined( DSP56000 ) || defined( DSP96000 )
  /* it is possible that the operands to a Pmode PLUS_EXPR are ordered 
   * backwards. The insn (shape) defines op0 to to be Pmode and op1 to
   * be SImode. We might see the operands in the wrong oder here. The
   * exp is still correct, but expand_binop in optabs.c will munge the
   * SImode operand into Pmode and the Pmode operand into SImode. In
   * order for expand_binop to work correctly with pointer adds, we need
   * to ensure that the order of the operands will match the shape at
   * this point.
   */
  if (( PLUS_EXPR == code ) &&
      ( Pmode != SImode ) &&
...
#if defined( DSP56000 )
	  /* we have to kludge this: the compiler must know that DI is not
	     equivalent to SI, but that both require a single memory location.
	     */
	  if ((( DFmode == TYPE_MODE ( type ) ) ||
	       ( SFmode == TYPE_MODE ( type ) ) ||
	       ( DImode == TYPE_MODE ( type ) )) && ( 'l' == memory_model ))
	  {
	      size = 1;
	  }
	  else
...
#if defined( DSP56000 ) || defined( DSP96000 )
  current_func_info |= FUNC_ISNT_LEAF;
#endif
  return target;
}

  ...
  else if (TYPE_MODE (TREE_TYPE (pval)) != BLKmode)
    {
	 ...
	 will prevent such mismatches.  */
	  ...
	  /* we have to kludge this: the compiler must know that DI is not
	     equivalent to SI, but that both require a single memory location.
	     */
	  if (( DImode == TYPE_MODE ( TREE_TYPE ( pval ))) && 
	      ( 'l' == memory_model ))
	  {
	      used = size = 1;
	  }
     ...
     rtx if_false_label, if_true_label;
{
  ...
  switch (code)
    ...
    case EQ_EXPR:
#if defined( DSP56000 )
      /* due to the un-unsigned features of this chip, we need an unsigned
	 version of EVERY conditional. */
      comparison = compare (exp, EQ, EQU, EQ, EQU);
#else
    ...
    case NE_EXPR:
#if defined( DSP56000 )
      comparison = compare (exp, NE, NEU, NE, NEU);
#else
