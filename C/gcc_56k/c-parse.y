/* YACC parser for C syntax.
   ...
   $Id: c-parse.y,v 1.13 92/05/13 22:07:08 pete Exp $
...
#if defined( TARGET_PROCESS_PRAGMA_LINE )
	      /* we need pragmas in embedded-processor-ville! */
	      return TARGET_PROCESS_PRAGMA_LINE( finput );
#else
	    goto skipline;
#endif
#else
...
#if ! defined( DSP96000 ) && ! defined( _MSDOS )
      /* WATCOM C has a problem with shifting '1' 32-bits to the left. */
...
#else
	    (spec_long_long
		   ? ((shorts[7]<<24) + (shorts[6]<<16) + (shorts[5]<<8) + shorts[4]) : 0)));
#endif
	...
	while (c != '"' && c >= 0)
	  {
		...
		if (c < 0)
		  goto skipnewline;
#if defined( DSP96000 )
		/* this won't work because chars are 32 bits on the 96k. */
#else
...
#if defined( DSP56000 ) || defined( DSP96000 )
	    /* we take the following approach: import the char string as
	       usual, then run a compiler resident copy of the target
	       mbstowcs function to create the wide string. */
#else
	    if (wide_flag)
	      {
	...
	if (wide_flag)
	  {
...
#if defined( DSP56000 ) || defined( DSP96000 )
	    /* The above comment does not hold for the 56k or 96k. */

	    {
		int wchars = ( p - token_buffer ) + 1; /* overkill, I know. */
		...
		*p = '\0'; /* terminate the string. */
		...
		wchars = target_mbstowcs ( wide_buffer, token_buffer + 1 );
		
		if ( -1 == wchars )
		{
		    error ("invalid wide char string");
		}
		widep = wide_buffer + wchars;
	    }
#else
