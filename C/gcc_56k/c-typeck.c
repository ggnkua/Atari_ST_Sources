/* Build expressions with type checking for C compiler.
   ...
   $Id: c-typeck.c,v 1.6 91/12/19 14:48:53 jeff Exp $
...
{
#if defined( DSP56000 )
  if ( bits <= TYPE_PRECISION ( ptr_type_node ))
...
#if defined( DSP96000 )
	       && ( TREE_TYPE ( val ) != double_type_node ))
#else
     ...
     register tree ptrop, intop;
{
...
#if defined( DSP56000 )
  /* we want to do an integer multiply and supply the add with a SImode
   * second operand. */
  intop = build_binary_op (MULT_EXPR, intop, size_exp);

  if (TYPE_PRECISION (TREE_TYPE (intop)) != BITS_PER_UNIT)
    intop = convert (type_for_size (BITS_PER_UNIT, 0), intop);
  ...
  if (TYPE_PRECISION (TREE_TYPE (size_exp)) != POINTER_SIZE)
    size_exp = convert (type_for_size (POINTER_SIZE, 0), size_exp);
     ...
     register tree op0, op1;
{
  ...
  tree restype = type_for_size ( BITS_PER_UNIT, 0);
#else
     ...
     tree value;
{
  switch (TREE_CODE (value))
	...
	return initializer_constant_valid_p (TREE_OPERAND (value, 0));
#if defined( DSP56000 )
      /* allow for (cast*) 0x11, where pointers are smaller than all
	 fucking integer types, ppptht. */
