/* Optimize jump instructions, for GNU compiler.
   ...
   $Id: jump.c,v 1.5 91/08/06 09:59:35 jeff Exp $
  ...
  if (noop_moves)
	...
	if (GET_CODE (insn) == INSN)
	 ...
	 scheduling peepholes on the sparc.  */
...
#if defined( DSP56000 )
		/* this optimization is not so correct for the 56k, because
		   if we're dealing with a conversion to/from DI mode, we might
		   need to move the LSW into the MSW or the other way around.
		   Even if the reg numbers are the same, the operation must be
		   performed. */
	    {
		enum machine_mode src_mode, dst_mode;
		rtx pick = SET_SRC ( body );
		
		while ( SUBREG == GET_CODE ( pick ))
		{
		    pick = XEXP ( pick, 0 );
		}
		src_mode = GET_MODE ( pick );
		pick = SET_DEST ( body );
		...
		dst_mode = GET_MODE ( pick );
		
		if (( src_mode == dst_mode ) ||
		    ( DImode != src_mode && DImode != dst_mode ))
		{
#endif
		if (sreg == dreg && sreg >= 0)
		  delete_insn (insn);
...
#if defined( DSP56000 ) || defined( DSP96000 )
	  /* Do loops are a special form of JUMP_INSN. the rtl that
	     represents this HW construct is not as general as a typical
	     jump. some of the optimizations performed here actually 
	     hurt performance and create incorrect code when applied to 
	     do loops. we disallow jump optimization on "do" and "od" */

#if defined( HAVE_do ) && defined( HAVE_od )
	  if ( JUMP_INSN == GET_CODE ( insn ) &&
	      ( CODE_FOR_do == INSN_CODE ( insn ) || 
	       CODE_FOR_od == INSN_CODE ( insn )))
	  {
	      continue;
	  }
#endif
...
#if defined( DSP56000 )
    /* privide cases for all unsigned comparisons. */
    case EQU:
      return NEU;
