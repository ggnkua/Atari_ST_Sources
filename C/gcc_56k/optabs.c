/* Expand the basic unary and binary arithmetic operations, for GNU compiler.
   ...
   $Id: optabs.c,v 1.11 92/03/31 21:32:19 pete Exp $
     ...
     enum optab_methods methods;
{
  ...
  /* although addition is commutative, swapping operands on a pointer add
   * will cause gcc to do mode mungeing on the operands and we'll probably
   * get an abort when something tries to do a (SUBREG:SI (REG:PSI)).
   */
  if ((( binoptab == add_optab ) && (( Pmode == SImode ) || Pmode != mode ))
#else
...
#endif
#ifdef HAVE_cmpstrpsi
      if (HAVE_cmpstrpsi
	  && GET_CODE (size) == CONST_INT
	  && INTVAL (size) < (1 << GET_MODE_BITSIZE (PSImode)))
	emit_insn (gen_cmpstrpsi (x, y, size,
				 gen_rtx (CONST_INT, VOIDmode, align)));
      else
#endif
#ifdef HAVE_cmpstrsi
...
#if defined( DSP96000 )
static enum insn_code floattab[2][2][2];
#else
...
can_float_p (fltmode, fixmode, unsignedp)
     ...
     int unsignedp;
{
  return floattab[fltmode != SFmode][fixmode == DImode][unsignedp];
}
#else
...
{
#if defined( DSP96000 )
  enum insn_code p = CODE_FOR_nothing, *q = & floattab[0][0][0];
    
  for ( ; q <= & floattab[1][1][1]; q++ )
  {
      *q = p;
  }
  ...
  if (HAVE_floatsisf2)
    floattab[0][0][0] = CODE_FOR_floatsisf2;
#endif
#ifdef HAVE_floatdisf2
  if (HAVE_floatdisf2)
    floattab[0][1][0] = CODE_FOR_floatdisf2;
#endif
#ifdef HAVE_floatsidf2
  if (HAVE_floatsidf2)
    floattab[1][0][0] = CODE_FOR_floatsidf2;
#endif
#ifdef HAVE_floatdidf2
  if (HAVE_floatdidf2)
    floattab[1][1][0] = CODE_FOR_floatdidf2;
#endif
#ifdef HAVE_floatsisf2
  if (HAVE_floatsisf2)
    floattab[0][0][1] = CODE_FOR_floatunssisf2;
#endif
#ifdef HAVE_floatdisf2
  if (HAVE_floatdisf2)
    floattab[0][1][1] = CODE_FOR_floatunsdisf2;
#endif
#ifdef HAVE_floatsidf2
  if (HAVE_floatsidf2)
    floattab[1][0][1] = CODE_FOR_floatunssidf2;
#endif
#ifdef HAVE_floatdidf2
  if (HAVE_floatdidf2)
    floattab[1][1][1] = CODE_FOR_floatunsdidf2;
#endif
#else
  enum insn_code *p;
  for (p = floattab[0];
     ...
     int unsignedp;
{
...
#else
  /* 96k has floatu.[sx] - no assemetry between signed/unsigned. */
#endif
...
#if defined( DSP96000 )
  if ((icode = can_float_p (GET_MODE (to), GET_MODE (from), unsignedp))
      != CODE_FOR_nothing)
#else
  if ((icode = can_float_p (GET_MODE (to), GET_MODE (from)))
...
#endif
    {
      ...
      if ( unsignedp )
        {
	  emit_unop_insn (icode, to, from, UNSIGNED_FLOAT);
        }
      else
        {
      ...
      emit_unop_insn (icode, to, from, FLOAT);
	   ...
	   && ((icode = can_float_p (DFmode, GET_MODE (from), unsignedp))
	       != CODE_FOR_nothing))
#else
  else if (GET_MODE (to) == SFmode
...
#if defined( DSP96000 )
	   && (can_float_p (GET_MODE (to), DImode, unsignedp) !=
	       CODE_FOR_nothing
	       || can_float_p (DFmode, DImode, unsignedp) != CODE_FOR_nothing))
#else
...
#if defined( DSP96000 )
      else if ((icode = can_float_p (DFmode, DImode, unsignedp))
	        != CODE_FOR_nothing)
#else
...
#endif
#ifdef HAVE_divmodpsi4
  if (HAVE_divmodpsi4)
    sdivmod_optab->handlers[(int) PSImode].insn_code = CODE_FOR_divmodpsi4;
#endif
...
#endif
#ifdef HAVE_udivmodpsi4
  if (HAVE_udivmodpsi4)
    udivmod_optab->handlers[(int) PSImode].insn_code = CODE_FOR_udivmodpsi4;
#endif
...
#endif
#ifdef HAVE_andcbpsi3
  if (HAVE_andcbpsi3)
    andcb_optab->handlers[(int) PSImode].insn_code = CODE_FOR_andcbpsi3;
#endif
#ifdef HAVE_andcbsi3
  if (HAVE_andcbsi3)
...
#ifdef HAVE_tstpsi
  if (HAVE_tstpsi)
    tst_optab->handlers[(int) SImode].insn_code = CODE_FOR_tstpsi;
#endif
#ifdef HAVE_tstsi
  if (HAVE_tstsi)
...
#if defined( DSP56000 )
/* see comment in expr.c */
#if HAVE_bequ
  if (HAVE_bequ)
    bcc_gen_fctn[(int) EQU] = gen_bequ;
#endif
#endif
#if HAVE_beq
  if (HAVE_beq)
...
#if HAVE_sequ
  if (HAVE_sequ)
    setcc_gen_fctn[(int) EQU] = gen_sequ;
#endif
#endif
#if HAVE_seq
  if (HAVE_seq)
...
#if HAVE_sneu
  if (HAVE_sneu)
    setcc_gen_fctn[(int) NEU] = gen_sneu;
#endif
#endif
#if HAVE_sne
  if (HAVE_sne)
