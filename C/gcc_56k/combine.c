/* Optimize by combining instructions for GNU compiler.
   ...
   $Id: combine.c,v 1.5 91/10/23 16:07:43 pete Exp $
...
/* Problem: The combiner will join register loads with arithmetic
   insns. This keeps MACs from being detected, and makes extra work for the
   reload pass. We can't just keep loads from combining, because 
   this would turn off auto-increment and auto-decrement use. */
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
  /* ok - here is where we screen for the merging of loads with 
     arithmetic ops. First, we find the REG that is the object of the 
     load. */
  if (( i1 ) &&
      ( INSN == GET_CODE ( i1 )) &&
      ( SET == GET_CODE ( PATTERN ( i1 ))) &&
      ( REG == GET_CODE ( SET_DEST ( PATTERN ( i1 )))) &&
      ( MEM == GET_CODE ( SET_SRC ( PATTERN ( i1 )))) &&
      (( reg_mentioned_p ( SET_DEST ( PATTERN ( i1 )), PATTERN ( i2 ))) ||
       ( reg_mentioned_p ( SET_DEST ( PATTERN ( i1 )), PATTERN ( i3 )))))
  {
      return 0;
  }
  
  if (( SET == GET_CODE ( PATTERN ( i2 ))) &&
      ( REG == GET_CODE ( SET_DEST ( PATTERN ( i2 )))) &&
      ( MEM == GET_CODE ( SET_SRC ( PATTERN ( i2 )))) &&
      ( reg_mentioned_p ( SET_DEST ( PATTERN ( i2 )), PATTERN ( i3 ))))
  {
      return 0;
  }
#endif
