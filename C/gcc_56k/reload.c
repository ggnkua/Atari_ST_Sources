/* Search an insn for pseudo regs that must be in hard regs and are not.
   ...
   $Id: reload.c,v 1.7 91/10/23 16:50:58 pete Exp $
...
#if defined( DSP96000 ) 
  /* for the 96k in the l memory_model, all MODE_SIZEs are one so that
     memory indexing is done correctly for doubles, chars, etc. this
     makes the above MODE_SIZE based qualification inaccurate. if we
     are in the l memory_model, and the MODE_SIZEs for in and out differ,
     and either of in or out is either DImode, SFmode, or DFmode, then
     we can't use this trick. actually SFmode and DFmode, because they
     are 96 bits in registers and 32/64 bits in memory, will fuck up *any*
     MODE_SIZE base check. So, we'll always qualify with mode instead of
     MODE_SIZE. ps. on the 96k if the modes differ, then the sizes differ.*/
  if ( GET_MODE ( out ) != GET_MODE ( in ))
    {
      return 0;
    }
#endif
#if defined( DSP56000 ) 
  /* similar to above 96k comment. */
  if (( GET_MODE ( out ) != GET_MODE ( in )) && ( 'l' == memory_model ))
  {
      enum machine_mode outmode = GET_MODE ( out ), inmode = GET_MODE ( in );
      
      if ( ! ((( DImode == outmode ) || ( SFmode == outmode ) ||
	       ( DFmode == outmode )) &&
	      (( DImode == inmode ) || ( SFmode == inmode ) ||
	       ( DFmode == inmode ))))
      {
	  return 0;
      }
  ...
  for (this_alternative_number = 0;
      ...
      for (i = 0; i < noperands; i++)
	{
...
#if defined( DSP56000 )
	  /* the idea here is to keep meaningful subregs that are applied
	     to accumulators from being used as arithmetic operands. the
	     admissibility of these subregs is specifed on a per insn basis
	     using the target machine info mechanism. */

	  if ( SUBREG == GET_CODE ( operand ) &&
	      ( REG != GET_CODE ( SUBREG_REG ( operand )) ||
	       ( DImode != GET_MODE ( operand ) &&
		( DSP56_A_REGNUM == REGNO ( SUBREG_REG ( operand )) ||
		 DSP56_B_REGNUM == REGNO ( SUBREG_REG ( operand ))) &&
		DImode == GET_MODE ( SUBREG_REG ( operand )) &&
		! insn_machine_info 
		[ INSN_CODE ( insn )].can_accept_subreg_acc_p )))
	  {
	      force_reload = 1;
	  }
#endif
	  ...
	  while (*p && (c = *p++) != ',')
		...
		if (GET_CODE (operand) == REG
#if defined( DSP56000 )
		    && reg_fits_class_p (operand, REG_CLASS_FROM_LETTER (c),
#else
