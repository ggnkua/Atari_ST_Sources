/* Convert RTL to assembler code and output it, for GNU compiler.
   Copyright (C) 1987, 1988, 1989 Free Software Foundation, Inc.
	...
	$Id: final.c,v 1.20 91/10/23 16:39:59 pete Exp $
     ...
     int nopeepholes;
{
  ...
  switch (GET_CODE (insn))
    ...
    case CODE_LABEL:
#if defined( DSP56000 )
      /* labels cause n register values to become undefined. */
      clear_n_reg_values ( );
#endif
      CC_STATUS_INIT;
      if (prescan > 0)
	...
	if (GET_CODE (body) == ASM_INPUT)
	  {
#if defined( DSP56000 )
	      /* there's also no telling what that did to the n registers! */
	      clear_n_reg_values ( );
#endif
	    ...
	    && insn != last_ignored_compare)
	  {
		...
		if (! find_reg_note (insn, REG_INC, 0)
		    ...
		    && ! volatile_refs_p (PATTERN (insn)))
		  {
		      ...
		      if ( cc_status.mdep == next_cc_use ( insn ))
		      {
     ...
     register rtx x;
{
  ...
  if (GET_CODE (y) == REG)
    {
#if defined( DSP56000 )
	if (( DImode != GET_MODE ( x )) &&
	    (( DSP56_A_REGNUM == REGNO ( y ) || DSP56_B_REGNUM == REGNO ( y ))
	     && DImode == GET_MODE ( y )) &&
	    ( ! RELOAD_SUBREG_P ( x )))
	{
	    /* use this meaningless rtl flag to tell print_operand
	       to go for the LSW of this DST_REG due to the SUBREG
	       on a DImode. */
