/* Medium-level subroutines: convert bit-field store and extract
   ...
   $Id: expmed.c,v 1.6 91/10/22 15:19:23 jeff Exp $
...
#if defined( DSP56000 ) || defined( DSP96000 )
/* we change this to facilitate the use of incs and decs. this function
   is only called by emit_iv_init_code in loop.c. it is trying to compute
   a value into a target register. the problem is that the GNU version of
   this function does not take advantage of the target parameter of 
   expand_expr, and this sometimes causes an extra copy to be emitted. */

rtx
expand_mult_add (x, mult, add, mode, unsignedp, target)
     rtx x, mult, add, target;
     enum machine_mode mode;
     int unsignedp;
{
  ...
  return expand_expr (sum, target, VOIDmode, 0);
}
#else
rtx
