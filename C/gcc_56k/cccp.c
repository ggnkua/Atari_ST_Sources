/* C Compatible Compiler Preprocessor (CCCP)
   $Id: cccp.c,v 1.24 91/10/31 19:52:12 pete Exp $
...
#else
#if ! defined( NeXT )
/* the next string.h file already defines index and rindex */
#define index strchr
...
#if defined ( _MSDOS )
int error ( char * msg, ... );
int error_with_line ( int line, char * msg, ... );
...
#if defined( _MSDOS )
void *xmalloc (), *xrealloc (), *xcalloc ();
char *savestring ();
#else
char *xmalloc (), *xrealloc (), *xcalloc (), *savestring ();
#endif
#if defined( _MSDOS )
void fancy_abort (), pfatal_with_name (), perror_with_name ();
#else
...
#ifndef VMS
#if defined( DSP56000 ) || defined( DSP96000 )
      /* we want to avoid system include dirs at all cost ! */
    { 0, GCC_INCLUDE_DIR }
#else
     ...
     char **argv;
{
...
#if defined( DSP56000 ) || defined( DSP96000 )
  {
	  ...
	  include_defaults[0].fname = 
	      xmalloc ( dsploclen + sizeof ( "include" ));
	  strcpy ( include_defaults[0].fname, dsploc );
	  strcat ( include_defaults[0].fname, "include" );
      }
  }
#endif 
#ifdef RLIMIT_STACK
...
#ifdef VMS
  {
    ...
    char *s;
#if ! defined( NeXT )
    /* the next string.h file already defines rindex */
    extern char *rindex ();
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
/* jam this wide open, 'cause it is dynamic off of DSPLOC in this version. */
  max_include_len = 1024;
#else
#ifndef VMS
  max_include_len
...
#if defined( DSP56000 ) || defined( DSP96000 )
	  fatal ("-traditional option NOT supported");
#else
	  traditional = 1;
	  dollars_in_ident = 1;
#endif
...
#if defined( _MSDOS )
    {
	/* we need to know whether we're a g56k or a g96k */
	if ( 0 == strncmp ( "__DSP56K__", pend_defs[i],
			   sizeof ( "__DSP56K__" ) - 1 ))
	{
	    is_dsp56k_p = 1;
	}
  ...
  for (i = 1; i < argc; i++)
    if (pend_files[i]) {
#if defined ( _MSDOS )
	    /* we need to specify the input files as text, so that DOS
	       Ctrl-Ms are weeded out by the system calls. */
...
#if defined( _MSDOS )
      /* output in "binary" mode so that DOS Ctrl-Ms aren't inserted into 
	 the output. */
      deps_stream = fopen (output_file, "ab");
#else
  ...
  else if (! freopen (out_fname, "wb", stdout))
#else
...
#if defined( _MSDOS )
  if (file_size_and_mode (fname, &st_mode, &st_size) < 0)
#else
...
#else
    bcopy (basep, fp->buf, (int) st_size);
#endif
    fp->length = st_size;
    free (basep);
  }
...
#else
    install (symname, sym_length, T_MACRO, (int) defn, hashcode);
#endif
  }
...
#if defined ( _MSDOS )
{
    int how_many_slashes = 0;
    char *safe_name, *look = ip->fname, *src = ip->fname;
    ...
    look = ( safe_name = xmalloc ( how_many_slashes + ( look - src )));
	...
	*look = *src;
    ...
    sprintf (line_cmd_buf, "# %d \"%s\"", ip->lineno, safe_name );
    free ( safe_name );
}
#else
...
#if ! defined( OUTPUT_LINE_COMMANDS )
...
#endif
{
    ...
    /* make the faulty assumption that arg1 - arg3 exist */
    arg1 = va_arg( ap, int );
    arg2 = va_arg( ap, int );
    arg3 = va_arg( ap, int );
#endif
...
#if defined( NeXT )
  do_undef (str,str + strlen ((char*)str) - 1, NULL, kt);
#else
...
#endif
{
    ...
    /* make the faulty assumption that arg exists */
    arg = va_arg( ap, char * );
#endif
     ...
     int number, size;
{
  ...
  register void *ptr = malloc (total);
  if (ptr != 0) {
    if (total > 100)
      bzero (ptr, total);
    else {
	 ...
	 It must be safe because malloc values are always well aligned.  */
      ...
      register long *zl = (long *) (((char*)ptr) + total - 4);
...
int
#if defined( _MSDOS )
file_size_and_mode (file_name, mode_pointer, size_pointer)
     char *file_name;
#else
...
#if defined( _MSDOS )
  if (stat (file_name, &sbuf) < 0) return (-1);
#else
