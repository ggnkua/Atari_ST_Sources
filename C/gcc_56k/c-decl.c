/* Process declarations and variables for C compiler.
   ...
   $Id: c-decl.c,v 1.10 91/10/23 15:44:57 pete Exp $
...
#if defined( DSP56000 ) || defined( DSP96000 )
    error ( "-traditional option not supported" ); 
#else
...
#if defined( DSP56000 ) || defined( DSP96000 )
  /* bind the run-time and load-time counters to this declaration. */
  { 
      extern char *x_run, *x_load, *y_run, *y_load;
      extern char *l_run, *l_load, *p_run, *p_load;

      DECL_X_RUN(tem) = x_run;
      DECL_X_LOAD(tem) = x_load;
      DECL_Y_RUN(tem) = y_run;
      DECL_Y_LOAD(tem) = y_load;
      DECL_L_RUN(tem) = l_run;
      DECL_L_LOAD(tem) = l_load;
      DECL_P_RUN(tem) = p_run;
      DECL_P_LOAD(tem) = p_load;
  }
#endif
...
#if defined( DSP96000 )
		      warning ( "long long is currently implemented as long" );
#else
		      longlong = 1;
#endif
		  }
     ...
     tree declarator, declspecs;
{
  ...
  { 
      ...
      DECL_X_RUN(current_function_decl) = x_run;
      DECL_X_LOAD(current_function_decl) = x_load;
      DECL_Y_RUN(current_function_decl) = y_run;
      DECL_Y_LOAD(current_function_decl) = y_load;
      DECL_L_RUN(current_function_decl) = l_run;
      DECL_L_LOAD(current_function_decl) = l_load;
      DECL_P_RUN(current_function_decl) = p_run;
      DECL_P_LOAD(current_function_decl) = p_load;
  }
#endif
  return 1;
}
