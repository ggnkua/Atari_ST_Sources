/* Compiler driver program that can handle many languages.
   ...
   $Id: gcc.c,v 1.43 92/04/01 18:28:28 pete Exp $
...
****** DSP56000/DSP96000 EXTENSION ******
 %z	substitute the argument of the current flag. usefull for renaming 
	flags. example: on the DSP*6000 there is no "-o filename" flag. We
	must generate a command sequence of the form "-B filename" in order
	to perform this we need to get at "part2" of switches[*].
	%{o*:-B %zo --}    <--- this does the trick
...
"%{crt:%zc%s}%{!crt:%{mx-memory:crt056x.cln%s} \
 %{!mx-memory:%{ml-memory:crt056l.cln%s}%{!ml-memory:crt056y.cln%s}}}"
#else     /* DSP96002 */
#define STARTFILE_SPEC  \
 "%{crt:%zc%s}%{!crt:%{mx-memory:crt096x.cln%s} \
  %{!mx-memory:%{my-memory:crt096y.cln%s}%{!my-memory:crt096l.cln%s}}}"
#endif    /* DSP56000 */
...
#define SWITCH_TAKES_ARG(CHAR)      \
    ( (CHAR) == 'r' || (CHAR) == 'D' || (CHAR) == 'U' || (CHAR) == 'o' \
   || (CHAR) == 'j' || (CHAR) == 'e' || (CHAR) == 'T' || (CHAR) == 'u' \
...
#define WORD_SWITCH_TAKES_ARG(STR) (!strcmp (STR, "Tdata") \
   || !strcmp(STR,"crt")) || !strcmp(STR,"asm")
#else
#define WORD_SWITCH_TAKES_ARG(STR) (!strcmp (STR, "Tdata"))
#endif   /* DSP */
#endif   /* WORD_SWITCH_TAKES_ARG */
...
struct compiler compilers[] =
{
#if defined( DSP56000 )
  {".c",
   "mcpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} -undef \
    %{mx-memory: -D__X_MEMORY} \
    %{!mx-memory:%{ml-memory:-D__L_MEMORY} %{!ml-memory:-D__Y_MEMORY}} \
    -trigraphs -$ -D__STRICT_ANSI__ %P %c %{pedantic} \
    %{!-fno-opt:-D__OPTIMIZE__} %{Wcomment*} %{Wtrigraphs} %{Wall} %i \
    %{!M*:%{!E:%g.cpp}}%{E:%{o*}}%{M*:%{o*}} |\n\
    %{!M*:%{!E:g56-cc1 %g.cpp -ansi -fstrength-reduce %{!Q:-quiet} -dumpbase %i \
    %{g} %{!g: %{S*: -g}} %{d*} %{m*} %{!fno-opt:-O} %{O} %{f*} %{W*} %{w} \
    %{pedantic} %{v:-version} %{S*:%{o*} %{!o*:-o %b.asm}} %{!S*:-o %g.asm} |\n\
    %{!S*:%{alo: alo56 -i %g.asm}} \
    %{S*:%{!o*:%{alo: alo56 -i %b.asm}} %{o*:%{alo: alo56 -i %zo}}} |\n\
    %{S:%{!o*:%{g: annotate -keep %b.asm} %{!g: %{S: annotate %b.asm}}} \
        %{o*:%{g: annotate -keep %zo} %{!g: %{S: annotate %zo}}}} |\n\
    %{!S*:asm56000 -c %{g} %{asm:%za} %{c:%{o*:-B %zo --}%{!o*:-B %w%b.cln --}}\
    %{!c:-B %d%w%b.cln --} %g.asm\n }}}"},

  {".i",
   "g56-cc1 %i -ansi -fstrength-reduce %{!Q:-quiet} -dumpbase %{d*} %{m*} %{f*}\
    %{g} %{!g: %{S*: -g}} %{!-fno-opt:-O} %{O} %{W*} %{w} %{pedantic}\
    %{v:-version} %{S*:%{o*} %{!o*:-o %b.asm}} %{!S*:-o %g.asm} |\n\
    %{!S*:asm56000 -c %{g} %{asm:%za} %{c:%{o*:-B %zo --}%{!o*:-B %w%b.cln --}}\
    %{!c:-B %d%w%b.cln --} %g.asm\n }"},
  ...
  {".c",
   "mcpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} -undef \
    %{mx-memory: -D__X_MEMORY} \
    %{!mx-memory:%{my-memory:-D__Y_MEMORY} %{!my-memory:-D__L_MEMORY}} \
    -trigraphs -$ -D__STRICT_ANSI__ %P %c %{pedantic} \
    %{!-fno-opt:-D__OPTIMIZE__} %{Wcomment*} %{Wtrigraphs} %{Wall} %i \
    %{!M*:%{!E:%g.cpp}}%{E:%{o*}}%{M*:%{o*}} |\n\
    %{!M*:%{!E:g96-cc1 %g.cpp -ansi -fstrength-reduce %{!Q:-quiet} -dumpbase %i \
    %{d*} %{m*} %{f*} %{g} %{!g: %{S*: -g}} %{!-fno-opt:-O} %{O} %{W*} %{w} \
    %{pedantic} %{v:-version} %{S*:%{o*}%{!o*:-o %b.asm}} %{!S*:-o %g.asm} |\n\
    %{!S*:%{alo: alo96 -i %g.asm}} \
    %{S:%{!o*:%{alo: alo96 -i %b.asm}} %{o*:%{alo: alo96 -i %zo}}} |\n\
    %{S:%{!o*:%{g: annotate -keep %b.asm} %{!g: %{S: annotate %b.asm}}} \
        %{o*:%{g: annotate -keep %zo} %{!g: %{S: annotate %zo}}}} |\n\
    %{!S*:asm96000 -c %{g} %{asm:%za} %{c:%{o*:-B %zo --}%{!o*:-B %w%b.cln --}}\
    %{!c:-B %d%w%b.cln --} %g.asm\n }}}"},

  {".i",
   "g96-cc1 %i -ansi -fstrength-reduce %{!Q:-quiet} -dumpbase %{d*} %{m*} %{f*} \
    %{g} %{!g: %{S*: -g}} %{!-fno-opt:-O} %{O} %{W*} %{w} %{pedantic} \
    %{v:-version} %{S*:%{o*}%{!o*:-o %b.asm}} %{!S*:-o %g.asm} |\n\
    %{!S*:asm96000 -c %{g} %{asm:%za} %{c:%{o*:-B %zo --}%{!o*:-B %w%b.cln --}}\
    %{!c:-B %d%w%b.cln --} %g.asm\n }"},
...
#if defined( DSP56000 )
char *link_spec = "%{!c:%{!M*:%{!E:%{!S*:dsplnk -c %{g} %{j:%zj} \
%{r:-R %zr%s} %{o:-B %zo --} %{!o:-B a.cld --} %l %{F*} \
%{I} %S %o %{mx-memory:%L lib56cx.clb%s} \
%{!mx-memory:%{ml-memory: %L lib56cl.clb%s}%{!ml-memory:%L lib56cy.clb%s}} \
%{L*}\n}}}}";
#endif
...
char *link_spec = "%{!c:%{!M*:%{!E:%{!S*:dsplnk -c %{g} %{j:%zj} \
%{r:-R %zr%s} %{o:-B %zo --} %{!o:-B a.cld --} %l %{F*} \
%{I} %S %o %{mx-memory:%L lib96cx.clb%s} \
%{!mx-memory:%{my-memory: %L lib96cy.clb%s}%{!my-memory:%L lib96cl.clb%s}} \
%{L*}\n}}}}";
#endif


#if ! defined( DSP56000 ) && ! defined( DSP96000 )
char *link_spec = "%{!c:%{!M*:%{!E:%{!S:ld -dc -dp -e start -X %{o*} %l\
 %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\
 %{y*} %{!A:%{!nostdlib:%S}} \
 %{L*} %o %{!nostdlib:%{g:-lg} %L gnulib%s}\n }}}}";
#endif

...
#if defined( DSP96000 )
#if defined( _MSDOS )
#define STANDARD_EXEC_PREFIX "c:\\dsp\\bin\\g96-"
#else
#define STANDARD_EXEC_PREFIX "/usr/local/dsp/bin/gcc96-"
#endif /* _MSDOS */
#endif /* DSP96000 */
...
#if defined( _MSDOS )
#define STANDARD_EXEC_PREFIX "c:\\usr\\local\\lib\\gcc-"
#else
...
#if defined( _MSDOS )
char *standard_exec_prefix_1 = "c:\\dsp\\bin\\";
#else
char *standard_exec_prefix_1 = "/usr/local/dsp/bin/";
#endif  /* DOS */

#endif  /* DSP */

#if !defined( DSP56000 ) && !defined( DSP96000 )
char *standard_exec_prefix_1 = "/usr/local/gnu/lib/gcc-";
#endif


#if defined( DSP96000 ) || defined( DSP56000 )
#if defined( _MSDOS )
#define STANDARD_STARTFILE_PREFIX "c:\\dsp\\lib\\"
#else
#define STANDARD_STARTFILE_PREFIX "/usr/local/dsp/lib/"
#endif /* _MSDOS */
#endif /* DSP96000 */
...
#if defined ( _MSDOS )
char *standard_startfile_prefix_1 = "c:\\dsp\\lib\\";
char *standard_startfile_prefix_2 = "c:\\dsp\\lib\\";
#else
...
choose_temp_base ()
{
...
#if defined( _MSDOS )
  /* force the fabulous PC to use the current directory as the temp space */
  temp_filename = (char *) xmalloc ( sizeof("ccXXXXXX") );
  strcpy (temp_filename, "ccXXXXXX");
#else
  if (base == (char *)0)
    {
#ifdef P_tmpdir
  ...
  if (len > 0 && temp_filename[len-1] != PATH_DELIMITER)
    temp_filename[len++] = PATH_DELIMITER;
...
#if defined( _MSDOS )
  /* fabulous pc requires .exe extension */
  char *ptmp = xmalloc( strlen(prog)+5 );
  strcpy( ptmp, prog );
  strcat( ptmp, ".exe" );
  prog = ptmp;
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
  /* the fabulous DOS OS limits the command line length */
  {
      FILE *arg_file;
      int i = 0, command_line_length = 0;
      int arg_file_name_length = 5;
		
      while ( argv[ i ] != NULL )
      {
	  command_line_length += strlen( argv[i] );

	  /* send everything over 100 characters to temp file */
	  if ( command_line_length > 100 )
	  {
	      tmp_file_used = 1;
	      i -- ;     /* this assumes that the program name < 100 */
	      break;
	  }
      ...
      if ( tmp_file_used )
      {
	  /* point to insert the -F flag and filename argument */
	  int f_point = i;
	    
	  /* create temp file */
	  arg_file_name_length += strlen( temp_filename );
	  arg_file_name = (char*) xmalloc( arg_file_name_length );
	  strcpy ( arg_file_name, temp_filename );
	  strcat ( arg_file_name, ".arg" );

	  record_temp_file (arg_file_name, 1, 1);

	  /* open file */
	  if ( ( arg_file = fopen( arg_file_name, "w" ) ) == NULL )
	  {
	      perror( "arg_file open" );
	      exit ( -1 );
	  }

	  /* blow remaining argv contents to file */
	  while ( argv[ i ] != NULL )
	  {
	      fprintf( arg_file, "%s\n", argv[ i ] );
	      i ++ ;
	  ...
	  /* add "-F arg_file_name" */
	  argv[ f_point ] = (char *) xmalloc ( 3 );
	  strcpy( argv[ f_point ], "-F" );
	  f_point ++ ;
	  argv[ f_point ] = arg_file_name;

	  /* reset vector end point with NULL */
	  f_point ++ ;
	  argv[ f_point ] = NULL;
      }
  }
...
#else
  /* Exec the program.  */
  if ( (*func) ( P_WAIT, program, argv) != 0 )
  {
      perror_with_name(program);
      exit (-1);
      /* NOTREACHED */
  }
#endif  /* ! DOS */
    ...
    char **argv;
{
...
#if defined( DSP56000 )
    env_exec_prefix = getenv ("G56_EXEC_PREFIX");
#endif

#if defined( DSP96000 )
    env_exec_prefix = getenv ("G96_EXEC_PREFIX");
#endif
        ...
        else if ( (argv[i][0] == '-') && (argv[i][1] == 'l') )
	{
	    ...
	    /* pass absolute path stuff through */
	    if ( argv[i][2] != PATH_DELIMITER )
	    {
		len = strlen( argv[i] ) + 6;
		string = (char *) xmalloc ( len );

		sprintf( string,"lib%s.clb", argv[i]+2 );
		string = find_file( string );
		len = strlen( string ) + 5;
		argv[i] = (char*) xmalloc( len );

		sprintf( argv[i], "-L%s", string );
		free( string );
	    }
	}
	  ...
	  case 'z':
	    {
	       ...
	       f = 0;
	       /* search for -o option */
	       while ( switches[ f ].part1[0] != flag )
	       {
	          f ++ ;
	       }

	       local_size = strlen( switches[ f ].part2 );
	       obstack_grow ( &obstack, switches[ f ].part2, local_size );
	       arg_going = 1;
	    }
	    break;
...
#if defined( DSP56000 ) || defined( DSP96000 )
char **original_argv, ***addr_of_original_argv;

/* value of DSPLOC environment variable */
char *dsploc = NULL;
#endif
     ...
     char **argv;
{
...
#if defined( DSP56000 ) || defined( DSP96000 )
  {
	  ...
	  /* allow for growth. */
	  dsploc = strcpy ( (char*) xmalloc ( strlen ( dsploc ) +
				     sizeof ( "$dsp$" )), dsploc );
	  
	  /* root the dsp dir at dsploc: usually /usr/local */
	  while ( '\0' != ( * manip ))
	  {
	      ++ manip;
	  }
	  ...
	  if ( dirstr[0] != ( * ( -- manip )))
	  {
	      strcat ( dsploc, dirstr );
	  }
	  strcat ( dsploc, "dsp" );
	  strcat ( dsploc, dirstr );
	  
	  dsploclen = strlen ( dsploc );

	  /* jam dsploc into everything that needs it */
	  
	  standard_exec_prefix = 
	      (char*) xmalloc ( dsploclen + sizeof ( "bin" ) + 1 );
	  strcpy ( standard_exec_prefix, dsploc );
	  strcat ( standard_exec_prefix, "bin" );
	  strcat ( standard_exec_prefix, dirstr );

	  standard_startfile_prefix = 
	      (char*) xmalloc ( dsploclen + sizeof ( "lib" ) + 1 );
	  strcpy ( standard_startfile_prefix, dsploc );
	  strcat ( standard_startfile_prefix, "lib" );
	  strcat ( standard_startfile_prefix, dirstr );
      }
  }
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
  /* the fabulous IBM-PC forces us to extend command-line lengths via files */
  original_argv = argv;
  extend_argv( &argc, &argv );

#if defined( _MSDOS ) && ! defined( __TURBOC__ )
  {
      void fix_original_argv( void );
      atexit( fix_original_argv );
  }
#endif
#endif
  ...
  if (vflag)
    {
      extern char *motorola_version_string;
      extern char *version_string;

#if defined( DSP56000 )
      fprintf (stderr, "g56k version %s -- GNU %s\n", motorola_version_string, version_string);
#endif

#if defined( DSP96000 )
      fprintf (stderr, "g96k version %s -- GNU %s\n", motorola_version_string, version_string);
#endif
...
{
#if defined( NeXT )
  int value = (int) malloc ((size_t)size);
#else
...
{
#if defined( NeXT )
  int value = (int) realloc ((void*)ptr, (size_t)size);
#else
...
#if defined( _MSDOS ) && ( defined( DSP56000 ) || defined( DSP96000 ))
void
fix_original_argv(void)
{
    /* restore original argv */
    *addr_of_original_argv = original_argv;
}
#endif
