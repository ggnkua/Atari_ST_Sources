/* Language-indepednent node constructors for parse phase of GNU compiler.
   ...
   $Id: tree.c,v 1.9 91/11/22 19:45:04 pete Exp $
     ...
     tree for_type;
{
  ...
  if (TREE_CODE (op) == COMPONENT_REF
    ...
    {
#if defined( DSP56000 )
      /* the problem: under the L memory model, DECL_SIZE must be 1 for
	 DImode, SFmode and DFmode vars, for addressing purposes. We want
	 to avoid using DECL_SIZE or TYPE_SIZE for gauging precision. */

      int innerprec = (((( DImode == DECL_MODE ( TREE_OPERAND ( op, 1 ))) ||
			 ( DFmode == DECL_MODE ( TREE_OPERAND ( op, 1 ))) ||
			 ( SFmode == DECL_MODE ( TREE_OPERAND ( op, 1 )))) &&
			( 'l' == memory_model )) ? 48 :
