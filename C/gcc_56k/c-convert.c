/* Language-level data type conversion for GNU C.
   ...
   $Id: c-convert.c,v 1.5 91/10/23 15:43:40 pete Exp $
...
#else
      /* this looks like it is wrong in general, but it only shows up
	 when pointers are smaller than ints. the fact that when we try
	 to convert an int (24 bits) into a pointer (16 bits), we call
	 convert which recursively calls this function with the same 
	 arguments. this leads to infinite stack growth && BOOM! */
#endif
     ...
     tree type, expr;
{
  ...
  if (form == POINTER_TYPE)
	...
	expr = fold (build (CONVERT_EXPR,
			    type_for_size (BITS_PER_UNIT, 0), expr));
#else
	expr = fold (build (CONVERT_EXPR,
