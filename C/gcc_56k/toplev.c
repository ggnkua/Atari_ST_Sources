/* Top level of GNU C compiler
   Copyright (C) 1987, 1988, 1989 Free Software Foundation, Inc.
	...
	$Id: toplev.c,v 1.37 91/10/31 17:04:25 pete Exp $
...
void fatal( char * s, ... );
void error ( char *, ... );
void error_with_file_and_line ( char * file, ... );
void error_with_decl ( tree decl, ... );
void error_for_asm ( rtx insn, ... );
void warning_with_file_and_line ( char * file, ... );
void warning ( char * s, ... );
void warning_with_decl ( tree decl,  ... );
void sorry ( char * s, ... );
...
#if defined( _MSDOS )
/* there really is a duplicate definition in c-parse.y */
extern int lineno;
#else
int lineno;
#endif
...
/* version string for internal tracking of released versions */
...
#if defined( DSP56000 ) || defined( DSP96000 )
/* no effect dummy flag */

int flag_no_effect = 0;
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
/* strength-reduce and dsp loop opto on! */
int flag_strength_reduce = 1;
#else
...
# else
/* zero means make the text shared if supported.  */

int flag_shared_data = 1;
...
struct { char *string; int *variable; int on_value;} f_options[] =
{
  ...
  {"opt", &flag_no_effect, 0},
  {"coalescer", &flag_no_effect, 0}
#else
  {"delayed-branch", &flag_delayed_branch, 1}
#endif
};
...
#endif
{
    ...
    /* make the faulty assumption that v and v2 exist */
    v = va_arg( ap, int );
#endif
  error (s, v);
#if defined (_MSDOS )
     ...
     char *name;
{
  ...
  if (write_symbols == GDB_DEBUG)
    ...
    TIMEVAR (symout_time,
	     {
	       struct stat statbuf;
	       stat (main_input_filename, &statbuf);
...
#endif
/* we don't do gdb format symbols. 
...
{
#if defined( DSP56000 ) || defined( DSP96000 )
    /* accomodate any run-time/load-time counter #pragmas. */
    push_counters ( decl );
#endif
...
#if defined( DSP96000 ) || defined( DSP56000 )
  /* there are situations where blocks have been merged or rearranged
   * by the loop optimizer. this opens up new opportunities for constant
   * propagation and possibly jump elimination. we may have found a single
   * iteration loop as well.
   */

  if ( optimize && ( TARGET_DO_LOOP_GENERATION ))
  {
      TIMEVAR (cse_time, reg_scan (insns, max_reg_num (), 0));
      TIMEVAR (cse_time, cse_main (insns, max_reg_num ()));
      TIMEVAR (jump_time, jump_optimize (insns, 0, 0));
  }
#endif
#if defined( DSP56000 )
  /* constant folding can leave modified "wierdo" rtl insns around, 
     which might be replaced by a more conventional insn. for example,
     the div template, assuming that the div result was statically 
     computed in cse, might look like this:
     
     ( parallel [ ( set ( reg ) ( const_int ))
                  ( clobber ( reg ))
		  ( clobber ( reg )) ] ).
     ...
     ( set ( reg ) ( const_int )),
      ...
      thereby avoiding two needless register kills. */
     
  TIMEVAR (cse_time, simplify_cse_victims ( insns ));
...
#if defined( DSP56000 )
/* at this point, we have to sort all of the compares into signed and
   unsigned. */
  ...
  TIMEVAR (global_alloc_time,
	   {
		 ...
		 ( global_alloc ( global_reg_dump ? global_reg_dump_file : 0 ),
		  (( TARGET_RELOAD_CLEANUP ) ? cleanup_reloads( insns ) : 0 ));
	     else
	       reload (insns, 0,
...
#if defined( DSP56000 )
  /* mpy(and mac) shapes require a move/shift sequence to normalize output(and
     input) data. here, we pass through the insns and detect those macs whose 
     additive inputs are mac/mpy outputs. we mark these insn pairs as not 
     needing the normalization shifts between them. For example:
  ...
  if ( TARGET_NORMALIZATION_REDUCTION )
  {
      TIMEVAR (combine_time, reduce_norm_freq ( insns ));
  }
#endif
...
#if defined( DSP96000 ) || defined( DSP56000 )
  /* reset to "no information" for the next function. */
  current_func_info = FUNC_NO_INFO;
#endif
  parse_time -= gettime () - start_time;
}

...
#if defined ( _MSDOS )
/* the fabulous MSDOS forces use to allocate a fake argv incase we must
   reallocate argv later. */
int
main (argc, argv, envp)
     int argc;
     char **argv;
     char **envp;
{
    ...
    fake_argv = (char **) xmalloc( sizeof( char* ) * ( argc + 1 ) );
    while ( argv[ i ] != NULL )
    {
	fake_argv[ i ] = (char*) xmalloc ( sizeof( char ) * ( strlen( argv[ i ] ) + 1 ) );
	strcpy( fake_argv[ i ], argv[ i ] );
	i ++ ;
    }
    
    fake_main( argc, fake_argv, envp );
}

fake_main (argc, argv, envp)
     int argc;
     char **argv;
     char **envp;
{
...
#define __VERSION__ "[unknown]"
	    ...
	    fprintf (stderr, " compiled by WATCOM C.\n");
#else
  ...
  if ( TARGET_X_MEMORY )
  {
      mem_space = 'x';
      memory_model = 'x';
      mode_size[ (int) DFmode ] = 6;
      mode_size[ (int) SFmode ] = 6;
  }
  else if ( TARGET_L_MEMORY )
  {
      mem_space = 'y';
      memory_model = 'l';
      mode_size[ (int) DFmode ] = 3;
      mode_size[ (int) SFmode ] = 3;
  }
  else /* default model */
  {
      mem_space = 'y';
  ...
  if ( TARGET_X_MEMORY )
  {
      mem_space = 'x';
      memory_model = 'x';
      mode_size[ (int) DFmode ] = 2;
  }
  else if ( TARGET_Y_MEMORY )
  {
      mem_space = 'y';
      memory_model = 'y';
      mode_size[ (int) DFmode ] = 2;
...
#if defined( __HIGHC__ ) || defined( _INTELC32_ ) || defined( __WATCOMC__ )
int
bcopy ( b1, b2, length )
    char* b1, * b2;
    int length;
