/* Analyze RTL for C-Compiler
   ...
   $Id: rtlanal.c,v 1.4 91/04/01 09:55:49 jeff Exp $
  ...
  if (code == REG)
    {
      i = REGNO (x);
#if defined( DSP56000 ) || defined( DSP96000 )
      /* pseudo regs don't overlap in the same manner as hard ones. */
      return (( endregno > i ) &&
	      ( regno < ( i + (( FIRST_PSEUDO_REGISTER > i ) ?
			       HARD_REGNO_NREGS ( i, GET_MODE ( x )) : 1 ))));
#else
     ...
     rtx reg, x;
{
  int regno = REGNO (reg);
#if defined( DSP56000 ) || defined( DSP96000 )
  /* pseudo regs don't overlap as hard regs do. */
  int endregno = ( regno < FIRST_PSEUDO_REGISTER ) ? 
      regno + HARD_REGNO_NREGS ( regno, GET_MODE ( reg )) : regno + 1;
#else
