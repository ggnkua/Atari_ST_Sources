/* Output variables, constants and external declarations, for GNU compiler.
   Copyright (C) 1987, 1988, 1989 Free Software Foundation, Inc.
	...
	$Id: varasm.c,v 1.17 92/04/01 17:01:25 pete Exp $
...
text_section ()
{
  ...
  extern int counter_pragma_seen;

/* we want to ensure that #pragmas that affect the counters are always
   observed. */
  if (counter_pragma_seen || in_section != in_text)
    {
      counter_pragma_seen = 0; 
#else
...
/* we want to ensure that #pragmas that affect the counters are always
   observed. */
  if (counter_pragma_seen || in_section != in_data)
    {
      counter_pragma_seen = 0; 
#else
...
{
#if defined ( _MSDOS )
  enum kind { RTX_DOUBLE, RTX_INT } kind;
  enum machine_mode mode;
#else
     ...
     rtx x;
{
  ...
  if ( mode == DImode )
  {
      /* we only use DImode to represent the 64bit register output of a mpy.
	 we never care about the MSW - the DImode trick is used to tell the
	 compiler that a register has been clobbered. Thus, we don't really
	 implement DImode. DImode constants found here are really constant
	 folded mult output. Force the mode to SImode, which it really is 
	 anyway. */
...
#if defined( DSP56000 )
      /* it's really tough (impossible) to print out integers based on size
	 when we're using the 'l' memory_model. So, we print out all integer
	 constant as big as we need to, and the assembler selects the right
	 data size based on the latest org directive. */

      if ( 'l' == memory_model )
      {
	  if (( CONST_INT == GET_CODE ( x )) ||
	      ( CONST_DOUBLE == GET_CODE ( x )))
	  {
	      ASM_OUTPUT_DOUBLE_INT ( asm_out_file, x );
	  }
	  else
	  {
	      /* it must be an address constant. */
	      ASM_OUTPUT_CHAR ( asm_out_file, x );
	  }
	  size -= 1;
      }
      else /* use size to diferentiate between long/int. */
      {
	  if ( 1 == size )
	  {
	      /* int */
...
# else
#if defined( DSP96000 ) || defined( DSP56000 )
	  /* really doesn't matter for the 56k, but we wanna be consistent. */
	  if ( double_type_node != TREE_TYPE ( exp ))
#else
	  if ( size == 1 )
#endif
	      ASM_OUTPUT_FLOAT (asm_out_file, d);
...
#if defined( DSP56000 ) || defined( DSP96000 )
	  if (( ARRAY_TYPE == TREE_CODE ( TREE_TYPE ( exp ))) &&
	      ( integer_type_node == TREE_TYPE ( TREE_TYPE ( exp ))))
	  {
	      int count = size;
	      int *element = (int*) TREE_STRING_POINTER( exp );
