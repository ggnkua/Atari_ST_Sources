/* Reload pseudo regs into hard regs for insns that require hard regs.
   ...
   $Id: reload1.c,v 1.23 91/10/23 16:53:10 pete Exp $
      ...
      for (insn = first; insn; insn = NEXT_INSN (insn))
	{
	  ...
	  looks for non-strict memory addresses.  */
	      ...
	      for (i = 0; i < n_reloads; i++)
		{
		  ...
		  /* we can't use MODE_SIZE as an ordering. */
		  if ( reload_outmode[i] > mode )
		    {
	...
	if (group_size[i] > 1)
	  {
		  ...
		  && HARD_REGNO_MODE_OK (j, group_mode[i]))
		{
		  ...
		  /* we need to take into acount the auto-increment at the
		     top of the loop. this extra inc by one can make us
		     overlook back-to-back groups. */
		  j += (( 1 < k ) ? ( k - 1 ) : 0 );
#else
		  j += k;
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
      /* this is part of the kludge to clean up the reload fiasco. 
	 basically, we comb thru the code for the following situation:

	 ( SET ( MEM1 ) ( REG1 ))
	 ( SET ( REG2 ) ( MEM2 )).

	 we check to see that MEM1 and MEM2 are the same and that they're
	 reload generated addresses. We may have found a spurious reload.
	 In any case, we can save the gratuitous memory read by doing a 
	 
	 ( SET ( REG2 ) ( REG1 ))

	 in it's place. We go on to check whether the memory write is also
	 gratuitous. If we encounter a subsequent write to the same location
	 and no possible read from it (in the same basic block), then we
	 can assume that the memory write can be deleted. If REG1 is the
	 same as REG2, we can delete that move as well. */

    if ( TARGET_RELOAD_CLEANUP )
    {
	int block = 0;
	for ( ; block < n_basic_blocks; ++ block )
	{
	    rtx step = basic_block_head[ block ], ahead;
	    rtx end = basic_block_end[ block ];

	    while ( step != end )
	    {
		if ( INSN == GET_CODE ( step ) &&
		    SET == GET_CODE ( PATTERN ( step )) &&
		    MEM == GET_CODE ( SET_DEST ( PATTERN ( step ))) &&
		    RTX_RELOAD_GENERATED_P ( XEXP ( SET_DEST ( PATTERN ( step
									)),
						   0 )) &&
		    ! MEM_VOLATILE_P ( SET_DEST ( PATTERN ( step ))) &&
		    REG == GET_CODE ( SET_SRC ( PATTERN ( step ))) &&
		    end != ( ahead = NEXT_INSN ( step )) &&
		    INSN == GET_CODE ( ahead ) &&
		    SET == GET_CODE ( PATTERN ( ahead )) &&
		    REG == GET_CODE ( SET_DEST ( PATTERN ( ahead ))) &&
		    MEM == GET_CODE ( SET_SRC ( PATTERN ( ahead ))) &&
		    rtx_equal_p ( SET_DEST ( PATTERN ( step )),
				 SET_SRC ( PATTERN ( ahead ))))
		{
		    rtx next_use = NEXT_INSN ( ahead );
		    
		    while ( next_use && next_use != NEXT_INSN ( end ))
		    {
			if ( INSN == GET_CODE ( next_use ) &&
			    reg_mentioned_p ( SET_DEST ( PATTERN ( step )),
					     PATTERN ( next_use )))
			{
			    if ( SET == GET_CODE ( PATTERN ( next_use )) &&
				rtx_equal_p ( SET_DEST ( PATTERN ( next_use )),
					     SET_DEST ( PATTERN ( step ))))
			    {
				if ( rtx_equal_p ( SET_SRC ( PATTERN ( step )),
						  SET_DEST ( PATTERN ( ahead ))))
				{
				    PUT_CODE ( step, NOTE );
				    PUT_CODE ( ahead, NOTE );
				    INSN_DELETED_P ( step ) = 1;
				    INSN_DELETED_P ( ahead ) = 1;
				}
				else
				{
				    SET_DEST ( PATTERN ( step )) =
...
#if defined( DSP56000 ) || defined( DSP96000 )
      /* this is part of the kludge to clean up the reload fiasco. 
	 here we tag this address 'x' as a reload generated stack slot.
	 these stack slots are not to be referenced via an alias, except
	 in the case of a programmer bug, where the behavior is undefined
	 anyway. */
      RTX_RELOAD_GENERATED_P ( XEXP ( x, 0 )) = 1;
#endif
    }
}
...
#endif
#if defined( DSP96000 )
/* kludge to cover passing reg-vars bug. */
  for (i = FIRST_PSEUDO_REGISTER - 1; i >= 0; i--)
  {
      if ( 6 > potential_reload_regs[i] )
      {
	  potential_reload_regs[i] = -1;
      }
  }
...
#if defined( DSP96000 ) || defined( DSP56000 )
      /* we ran into the same problem in reload.c. MODE_SIZE, because all
	 integral type occupy one memory word, does *not* create a size based
	 ordering on modes. the 56k should be ok for now. (except in l mode )*/
      if (( VOIDmode == reload_inmode[j] ) ||
	  ( reload_outmode[j] >  reload_inmode[j] ))
	{
	  mode = reload_outmode[j];
	}
      else
	{
...
#if defined( DSP96000 ) || defined( DSP56000 )
      /* again, MODE_SIZE isn't useful as an ordering. */
      if ( reload_outmode[r] > reload_mode )
	{
...
#if defined( DSP56000 )
	  /* the problem here is that the subregs created here have no real
	     meaning. they are just created to mask the mode of the reloadreg.
	     in final.c, alter_subreg does a special trick to allow subregs
	     on the lsw of a DImode in a or b to access the correct part of
	     the register. the subregs created here confuse it. (at least they
	     used to!). */
	  if (GET_MODE (reloadreg) != mode)
	  {
	      reloadreg = gen_rtx (SUBREG, mode, reloadreg, 0);
	      RELOAD_SUBREG_P ( reloadreg ) = 1;
	  }
...
#if defined( DSP56000 )
	  /* see above comment on SUBREGS, reload, and accumulators. */
	  if (GET_MODE (reloadreg) != mode)
	  {
	      reloadreg = gen_rtx (SUBREG, mode, reloadreg, 0);
	      RELOAD_SUBREG_P ( reloadreg ) = 1;
	...
	= emit_insn_before (gen_move_insn (reloadreg, incloc), insn);
	 ...
	 output an insn to increment it directly.  */
	  ...
	  (REGNO (incloc) < FIRST_PSEUDO_REGISTER
	   || reg_renumber[REGNO (incloc)] >= 0))
#else
	  /* GNU makes an invalid assumtion here: they assume that they
	     can inc any hard reg which has the same mode as the reload
	     reg. We need to check that we have an rx reg on the 56k or
	     an rx, or dx.l reg on the 96k. otherwise we must inc the 
	     reload reg, copy it, and dec it. */

      if (( REG == GET_CODE ( incloc )) &&
	  ( FIRST_PSEUDO_REGISTER > REGNO ( incloc )) &&
#if defined( DSP56000 )
	  ( ADDR_REGS == REGNO_REG_CLASS ( REGNO ( incloc )))
#else
	  (( ADDR_REGS == REGNO_REG_CLASS ( REGNO ( incloc ))) ||
	   ( LO_DATA_REGS == REGNO_REG_CLASS ( REGNO ( incloc ))))
#endif
	  )
#endif
	{
