/* Common subexpression elimination for GNU compiler.
   ...
   $Id: cse.c,v 1.9 92/02/21 15:23:10 pete Exp $
     ...
     rtx x;
{
  ...
  switch (code)
	...
	return ( new && ( new->mode == x->mode )) ? new : x;
#else
	return new ? new : x;
#endif
      }
     ...
     rtx insn;
{
  ...
  /* sometimes this cse pass takes an insn created by a gen_ function and
     transforms it so that no define_insn will match it. we'll keep a copy
     of the original pattern. if, after the transformation, the insn no
     longer matches, then we'll slam the old pattern back in and live with
     it. */
  rtx original_pattern = copy_rtx ( PATTERN ( insn ));
#endif  
  ...
  if (GET_CODE (x) == SET)
      ...
      if (GET_CODE (SET_SRC (x)) == CALL)
	{
	  canon_reg (SET_SRC (x));
#if defined ( DSP56000 ) || defined ( DSP96000 ) || defined ( DSP56100 )
	  /* make sure the mangled insn matches. */
	  { 
	      enum rtx_code code = GET_CODE ( insn );
	      
	      if (( INSN == code || JUMP_INSN == code || CALL_INSN == code ) &&
		  ( 0 > asm_noperands ( PATTERN ( insn ))))
	      {
		  INSN_CODE ( insn ) = -1;
		  if ( -1 == recog_memoized ( insn ))
		  {
		      PATTERN ( insn ) = original_pattern;
		      (void) recog_memoized ( );
		  }
		  INSN_CODE ( insn ) = -1;
	      }
