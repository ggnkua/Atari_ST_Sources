/* Move constant computations out of loops.
   ...
   $Id: loop.c,v 1.23 92/05/27 10:54:42 pete Exp $ 
...
#if defined( DSP96000 ) || defined( DSP56000 )
static rtx find_mem ( );
static rtx get_bcond_label ( );
static int const_expr_p ( );
static rtx find_best_equiv ( );
static rtx compute_loop_count ( );
static int reg_really_used_between_p ( );
static int biv_qualified_p ( );
static int no_jumps_out_of_loop_p ( );
static int insn_is_do_p ( );
static int insn_is_od_p ( );
static int pre_existing_test_p ( );
static enum rtx_code flip_cond_code ( );
#endif
     ...
     int nregs;
{
  ...
  /* for our dsp chips, this linv movement scoring mechanism is bad.
   * some movables are more important than others because they can gate
   * things like array strength reduction and do loop generation. we
   * have decided to pay the register lifetime penalty to float most
   * invariants out. this area needs more work.
   */
  int threshold = loop_has_call ? 50 : 100;
#else
...
#if defined( DSP56000 ) || defined( DSP96000 )
  /* This REG_LIBCALL thing doesn't fit well with the movables concept.
   * This change blows away movables that have REG_LIBCALL affixed.
   * Later passes of the depend on REG_LIBCALL / REG_RETVAL sequences
   * remaining intact. If we move an insn with REG_LIBCALL affixed out
   * of the loop we open a Pandora's box. If we shift the 
   * REG_LIBCALL onto the following insn then we create another
   * problem: if the original REG_LIBCALL insn isn't moved out
   * of the loop, and the new REG_LIBCALL / REG_RETVAL sequence
   * is, then part of the call setup will not be performed.
   * Our solution, for now, is to disallow the movement of 
   * a REG_LIBCALL annotated insn.
   */
#endif
...
#if defined( DSP56000 ) || defined( DSP96000 )
      else if ( find_reg_note ( m->insn, REG_LIBCALL, 0 ))
	{
	    m->done = 1;
	}
#endif
     ...
     int nregs;
{
  ...
  if (loop_dump_stream)
    fprintf (loop_dump_stream, "\n");
#if defined( DSP96000 ) || defined( DSP56000 )
  if ( TARGET_LINV_PLUS_BIV_PROMOTION )
  {
      rtx scan_back = loop_end;

      /* scan from the end of the loop forward. */
      for ( ; loop_start != scan_back ; scan_back = PREV_INSN ( scan_back ))
      {
	  /* MEM rtx with + expr address. */
	  rtx mem_ref; 
	  /* PLUS expr which is the MEM arg. */
	  rtx add_expr = NULL;
	  /* INSN rtx of original computation of the address. */
	  rtx indirect_insn = NULL, indirect_reg;
	  /* secondary scan pointer. */
	  rtx peek = NULL;
	  /* the loop invariant, the basic induction variable.
	   * prior_... is the operand before it was striped of subregs.
	   */
	  rtx linv = NULL, biv = NULL, prior_linv, prior_biv;
	  /* the analog pointer register */
	  rtx analog;
	  /* the increment or decrement value */
	  rtx biv_increment;
	  
	  /* we only consider insns which ref memory. */
	  if (( INSN != GET_CODE ( scan_back )) ||
	      ( NULL == ( mem_ref = find_mem ( PATTERN ( scan_back )))))
	  {
	      continue;
	  }

	  /* since we don't have data-flow info yet, look back thru this 
	   * block and see if we can find the definition of the address reg.
	   * we don't do hard regs yet.
	   */
	  if ( REG == GET_CODE ( indirect_reg = XEXP ( mem_ref, 0 )))
	  {
	      /* bail if this is a hard reg. It may be set by a call. */
	      if ( REGNO ( indirect_reg ) < FIRST_PSEUDO_REGISTER )
	      {
		  continue;
	      }
	      
	      for ( peek = PREV_INSN ( scan_back );
		   ( peek != loop_start ) && 
		  ...
		  if (( INSN == GET_CODE ( peek )) &&
		      ...
		      ( indirect_reg == SET_DEST ( PATTERN ( peek ))))
		  {
		      add_expr = SET_SRC ( PATTERN ( peek ));
		      break;
		  }
	      }
	      
	      /* couldn't find the definition of the address register. */
	      if ( NULL == add_expr )
	      {
		  continue;
	      }
	  ...
	  {
	      /* otherwise, the address of the mem-ref is the expr we're 
	       * looking for.
	       */
	      add_expr = XEXP ( mem_ref, 0 );
	  }
	  if ( PLUS != GET_CODE ( add_expr ))
	  {
	      continue;
	  }
	  /* ok - we have a MEM with an address that consists of a PLUS 
	   * expression. we now need to show that one of the operands is
	   * a loop invariant, and that the other is a basic induction 
	   * variable. find the linv first.
	   */
	  
	  /* deal with subregs. */
	  prior_linv = ( linv = XEXP ( add_expr, 0 ));
	  if ( SUBREG == GET_CODE ( linv ))
	  {
	      linv = XEXP ( linv, 0 );
	  }
	  
	  switch ( GET_CODE ( linv ))
	  {
	  case SYMBOL_REF:
	  case CONST:
	  case CONST_INT:
	      /* by setting the biv, we indicate that we have found linv */
	      prior_biv = ( biv = XEXP ( add_expr, 1 ));
	      break;
	      
	  case REG:
	      if ( ! reg_set_between_p ( linv, loop_start, loop_end ))
	      {
		  /* although a reg, it is inv in the loop. */
		  prior_biv = ( biv = XEXP ( add_expr, 1 ));
	      }
	      else 
	      {
		  /* the 0th arg isn't the linv. try the 1th. */
		  prior_linv = ( linv = XEXP ( add_expr, 1 ));
	      }
	      break;
	  ...
	  /* we have an unknown in the + expr: bail out. */
	  if ( ! linv )
	  {
	      continue;
	  }

	  /* we need to check the other operand and see if it is a viable
	   * linv. 
	   */
	  if ( ! biv )
	  {
	      /* check for subregs again. */
	      if ( SUBREG == GET_CODE ( linv ))
	      {
		  linv = XEXP ( linv, 0 );
	      }
	  ...
	  /* maybe subreg again ? */
	  if ( SUBREG == GET_CODE ( biv ))
	  {
	      biv = XEXP ( biv, 0 );
	  }
	  
	  /* this biv could just be a copy of a real biv via a subreg due
	   * to mode considerations. we need to find the best_equiv for 
	   * biv because if the biv we have is really a copy, then induct_var
	   * and class_struct will not be set properly for the copy. 
	   */

          {
	      rtx tmp = find_best_equiv ( biv, scan_back, loop_start );
	  ...
	  if (( REG != GET_CODE ( biv )) ||
	      ( BASIC_INDUCT != induct_var[ REGNO ( biv ) ]))
	  {
	      /* we have failed to find a linv and biv. */
	      continue;
	  }

	  /* the GNU biv info really isn't reliable. we need a more stringent
	   * set of constraints.
	   */

          {
	      rtx label = loop_start, jump = loop_end;

	      while ( label && ( NOTE == GET_CODE ( label )))
	      {
	      ...
	      while ( jump && ( NOTE == GET_CODE ( jump )))
	      {
		  jump = PREV_INSN ( jump );
	      }
	      if (( ! jump ) || ( JUMP_INSN != GET_CODE ( jump )))
	      {
		  continue;
	      }

	      if ( ! biv_qualified_p ( biv, NEXT_INSN ( label ),
				      PREV_INSN ( jump )))
	      {
		  continue;
	      }
	  ...
	  /* now we need to know whether the biv is inc/dec'ed before or
	   * after it's use here. ( here is @ peek ( if it is defined ), or
	   * @ scan_back ). Since we don't have full data flow info, if
	   * we run into a BARRIER, JUMP_INSN, or CODE_LABEL, then we punt.
	   */

	  indirect_insn = peek;
	  if ( ! peek )
	  {
	      peek = PREV_INSN ( scan_back );
	  }
	  else /* the indirect insn could actually be a biv inc ! */
	  {
	      peek = PREV_INSN ( peek );
	  }
	  
	  while (( NULL != peek ) && ( loop_start != peek ))
	  {
	      if (( BARRIER == GET_CODE ( peek )) ||
		  ( JUMP_INSN == GET_CODE ( peek )) ||
		  (( CODE_LABEL == GET_CODE ( peek )) &&
		   ( loop_start != PREV_INSN ( peek ))))
	      {
		  peek = NULL;
		  break;
	      }
	  ...
	  /* sometimes a constant will get CSE'ed out of an increment, because
	   * constants are, in general, expensive. we need to see whether the
	   * biv inc/dec modifier is actually a constant.
	   */
	  biv_increment = class_struct[ REGNO ( biv ) ]->biv->add_val;

	  if ( CONST_INT != GET_CODE ( biv_increment ))
	  {
	      rtx tmp = loop_start;

	      while (( PREV_INSN ( tmp )) &&
		     ( CODE_LABEL != GET_CODE ( tmp )) &&
		     ( JUMP_INSN != GET_CODE ( tmp )))
	      {
		  tmp = PREV_INSN ( tmp );
	      }
	      
	      if (( REG != GET_CODE ( biv_increment )) ||
		  reg_set_between_p ( biv_increment, loop_start, loop_end ))
	      {
		  continue;
	      }
	      biv_increment =
		  find_best_equiv ( biv_increment, loop_start, tmp );
	  
	      if (( ! biv_increment ) ||
		  ( CONST_INT != GET_CODE ( biv_increment )))
	      {
		  continue;
	      }
	  }
      
	  /* we can only handle inc/dec by 1 with (r)+/- addr modes. */
	  if (( 1 != INTVAL ( biv_increment )) &&
	      ( -1 != INTVAL ( biv_increment )))
	  {
	      continue;
	  }
	  
	  /* if loop_start == peek, the biv update must take place after 
	   * our use. create an analog biv and initialize it right before
	   * loop_start. replace our use of the add_sub_expr with the new
	   * analog iv, and place it's inc/dec right after the insn where
	   * it is used.
	   */

	  analog = gen_reg_rtx ( Pmode );
	  XEXP ( mem_ref, 0 ) = analog;

	  /* we need to create the original add_expr in terms of the 
	   * best_equiv linv and biv, along with any subregs needed. 
	   * prior_... will be used to determine whether a subreg for ...
	   * is needed, and also the mode (if needed).
	   */
	  add_expr = gen_rtx ( PLUS, Pmode,
			      ((( prior_biv == biv ) ||
				( prior_biv->mode == biv->mode )) ? biv :
			       gen_rtx ( SUBREG, prior_biv->mode, biv )),
			      ((( prior_linv == linv ) ||
				( prior_linv->mode == linv->mode )) ? linv :
			       gen_rtx ( SUBREG, prior_linv->mode, linv )));
	  
	  if ( 0 > INTVAL ( biv_increment ))
	  {
	      emit_insn_after ( 
			       gen_rtx ( SET, VOIDmode, analog,
					gen_rtx ( MINUS, Pmode, analog, 
						 const1_rtx )),
			       scan_back );
	  }
	  else
	  ...
	  {
	      emit_insn_before ( 
				gen_rtx ( SET, VOIDmode, analog, add_expr ),
				loop_start );
	  }
      }
  }

  /* DO LOOPS: qualify and possibly convert this loop into a do loop.
   * to qualify a loop as a potential do loop:
   * (1) there must be only one conditional back branch.
   * (2) the cmp for (1) must be between a BIV and a loop invariant.
   * (3) let c be the increment stride of the biv. c must be constant
   * and c must be a power of two. 
   *
   * if the loop qualifies, a canonical equivalent of the looping mechanism
   * must be constructed: for ( i = a; i <= b; i += c ). If all of a, b, and
   * c are known constants, we can determine the loop count as:
   * _____________
   * | 1 + b - a |
   * | --------- |
   * |     c     |
   *
   * if not, we need to generate a string of insns which will compute the
   * loop count at runtime, and check for a zero or negative count. 
   *
   * we then need to pull the loop control to the top of the loop into 
   * a form that the code generator will recognize. We also need to eliminate
   * the BIV and the loop invariant of the comparison.
   */
  /* use the do ... while (0) construct so that a break is equivalent to
   * a rejection of the loop.
   */
  if ( TARGET_DO_LOOP_GENERATION )
  do
  {
      /* these four insns are identified when the loop is qualified.
       * they consist of the top of the loop label, the single back-branch,
       * the BIV increment, and the loop invariant/BIV compare insn.
       */
      rtx compare_insn, back_branch_insn, looping_label;
      
      /* these four rtx expressions are identified when the loop is qualified:
       * the loop_bound which the biv is compared against, the biv (duh),
       * the initial value of the biv ( as best we can determine - if it is
       * vaiable, it is null ), and the increment value of the biv. continue
       * relation is the rtx used as the decision maker in the back branch
       * insn.
       */
      rtx loop_bound, biv, biv_initial, biv_increment, continue_relation;
      
      /* these rtx are created within this section of code after (except
       * bypass_label) we have found the loop to be "DOable". loop_count
       * is a reg rtx which has the number of loop iterations computed into
       * it. bypass_label is jumped to if the loop is not to be executed at
       * all.
       */
      rtx loop_count, bypass_label;
      
      rtx tmp, tmp2;
      
      /* find the looping label (1st label in the loop, with no preceeding
       * insns).
       */
      tmp = loop_start;
      while ( tmp = NEXT_INSN ( tmp ), tmp && ( NOTE == GET_CODE ( tmp )));
      if (( ! tmp ) || ( CODE_LABEL != GET_CODE ( tmp )))
      {
	  break;
      }
      looping_label = tmp;

      /* find the back branch insn. */
      tmp = loop_end;
      while ( tmp = PREV_INSN ( tmp ), tmp && ( NOTE == GET_CODE ( tmp )));
      if (( ! tmp ) || ( JUMP_INSN != GET_CODE ( tmp )) ||
	  ( looping_label != get_bcond_label ( tmp )))
      {
	  break;
      }
      back_branch_insn = tmp;
      continue_relation = XEXP ( SET_SRC ( PATTERN ( tmp )), 0 );
      
      /* scan down the loop and make sure that only one branch references 
       * the looping label. we need to make sure that there aren't any 
       * extra back-branches which would screw the possibility of using a
       * do. gee, it would be nice to have a control flow graph here ... 
       */
      for ( tmp = NEXT_INSN( looping_label );
	   tmp && ( tmp != back_branch_insn );
	   tmp = NEXT_INSN ( tmp ))
      {
	  if ( JUMP_INSN == GET_CODE ( tmp ))
	  {
	      if (( PARALLEL == GET_CODE ( PATTERN ( tmp ))) &&
		  ( CLOBBER == GET_CODE ( XVECEXP ( PATTERN ( tmp ),0,0))) &&
		  ((( 3 == XVECLEN ( PATTERN ( tmp ), 0 )) &&
		    ( USE == GET_CODE ( XVECEXP ( PATTERN ( tmp ),0,1 ))) &&
		    ( SET == GET_CODE ( XVECEXP ( PATTERN ( tmp ),0,2 )))) ||
		   (( 2 == XVECLEN ( PATTERN ( tmp ), 0 )) &&
		    ( SET == GET_CODE ( XVECEXP ( PATTERN ( tmp ),0,1 ))))))
	      {
		  /* check for a "do" or "od" shape. these are ok. */
		  ;
	      }
	      else if ( SET != GET_CODE ( PATTERN ( tmp )))
	      {
		  tmp = 0;
		  /* code gen bug on sun4. */
		  break;
	      }
	      else
	      {
		  switch ( GET_CODE ( SET_SRC ( PATTERN ( tmp ))))
		  {
		  case LABEL_REF:
		      if ( SET_SRC ( PATTERN ( tmp )) == looping_label )
		      {
			  tmp = 0;
		      }
		      break;
		  ...
		  case IF_THEN_ELSE:
		      tmp2 = get_bcond_label ( tmp );
		      if (( ! tmp2 ) || ( tmp2 == looping_label ))
		      {
			  tmp = 0;
		      }
		      break;
      ...
      /* make sure that there are no jumps out of the loop. this function
       * also ensures that we won't nest do instructions too deeply by checking
       * the encountered depth against MAX_DO_LOOP_NESTING. 
       */
      if ( ! no_jumps_out_of_loop_p ( loop_start, loop_end ))
      {
	  break;
      }
      
      /* find the compare or tst instruction before this conditional jump. */
      while ( tmp = PREV_INSN ( tmp ), tmp && ( NOTE == GET_CODE ( tmp )));
      if (( ! tmp ) || ( INSN != GET_CODE ( tmp )) || 
	  ( SET != GET_CODE ( PATTERN ( tmp ))) || 
	  ( cc0_rtx != SET_DEST ( PATTERN ( tmp ))))
      {
	  break;
      }
      compare_insn = tmp;
      
      /* use the precomputed information provided by the strength-reduction
       * part of this pass: ensure that one of the operands of the cmp, or
       * that the operand to the tst is actually a biv.
       */
      if ( COMPARE == GET_CODE ( tmp2 = SET_SRC ( PATTERN ( tmp ))))
      {
	  if (( REG == GET_CODE ( biv = XEXP ( tmp2, 0 ))) &&
	      ( nregs > REGNO ( biv )) &&
	      ( BASIC_INDUCT == induct_var[ REGNO ( biv ) ] ))
	  {
	      loop_bound = XEXP ( tmp2, 1 );
	  }
	  else if (( REG == GET_CODE ( biv = XEXP ( tmp2, 1 ))) &&
		   ( nregs > REGNO ( biv )) &&
		   ( BASIC_INDUCT == induct_var[ REGNO ( biv ) ] ))
	  {
	      loop_bound = XEXP ( tmp2, 0 );
	  }
	  else
	  {
	      break;
      ...
      else if ( REG == GET_CODE ( biv = SET_SRC ( PATTERN ( tmp ))))
      {
	  if (( nregs > REGNO ( biv ) ) ||
	      ( BASIC_INDUCT != induct_var[ REGNO ( biv ) ] ) ||
	      ( 0 == class_struct[ REGNO ( biv ) ] ))
	  {
	      break;
	  }
	  loop_bound = const0_rtx;
      }
      else
      {
	  break;
      }
      /* check to see if the biv meets a more stringent set of specs. */

      if ( ! biv_qualified_p ( biv, looping_label, back_branch_insn ))
      {
	  break;
      }
      
      /* biv really is a biv. we've verified that with the existance
       * of induct_var for that regno. loop_bound may or may not be a valid
       * loop bound for us. we will try to determine whether or not loop bound
       * is or isn't loop invariant, and whether or not it is a constant.
       */
      loop_bound = find_best_equiv ( loop_bound, compare_insn, loop_start );
      if ( NULL == loop_bound )
      {
	  /* the loop bound may not be invariant within the loop. */
	  break;
      }

      /* attempt to find the initial values of the biv and (possibly) an
       * initial value for the loop bound.
       */
      tmp = loop_start;
      while ( tmp = PREV_INSN ( tmp ), ( tmp ) && ( PREV_INSN ( tmp )) &&
	     ( CODE_LABEL != GET_CODE ( tmp )));
      
      biv_initial = ( tmp2 = find_best_equiv ( biv, loop_start, tmp )) ?
	  tmp2 : biv;
      
      loop_bound = ( tmp2 = find_best_equiv ( loop_bound, loop_start, tmp )) ?
	  tmp2 : loop_bound;
	  ...
	  biv_increment = find_best_equiv ( biv_increment, loop_start, tmp );
      ...
      /* qualify the loop increment as a positive or negative power of
       * two. if not, the divide cost is too great a risk.
       */
      
      /* if biv_initial and biv are the same pseudo reg, we need to copy 
       * biv into a new pseudo register and call it biv_initial. this is
       * so that biv_inital remains constant down past the loop where it
       * may be used in the reconstruction of the induction variable.
       */

      if (( REG == GET_CODE ( biv_initial )) &&
	  ( REGNO ( biv_initial ) == REGNO ( biv )))
      {
	  biv_initial = gen_reg_rtx ( GET_MODE ( biv ));
	  
	  emit_insn_before ( gen_rtx ( SET, VOIDmode, biv_initial, biv ),
			    loop_start );
      }

      /* convert the loop into canonical form ( for ( i=a; i<=b; i +=c ))
       * if possible. emit instructions to compute (and possibly check) the
       * loop count into a new pseudo-reg. Also determine the (@loop end)
       * resulting value of the induction variable, and set it to that value
       * after the loop if the biv will be eliminated by the DO transformation
       * also emit nop bounded do target. we need nop bounding in order to
       * have nested do loops and conditionals within do loops.
       */
      
      emit_label_after ( bypass_label = gen_label_rtx ( ), loop_end );
      emit_insn_before ( const0_rtx, bypass_label );
      emit_insn_after ( const0_rtx, bypass_label );
      
      if ( NULL == ( loop_count = 
		    compute_loop_count ( loop_start, tmp, bypass_label, 
					biv_initial, continue_relation,
					loop_bound, biv_increment )))
      {
	  continue;
      }
      
      /* create a burly-ass ( parallel [ ... ] ) JUMP_INSN. this should match
       * the "do" shape. we also need to make a bogus back-branch so that any
       * jump optimizer will see that there is an inherent back branch in
       * the do-loop label. this back branch will match some funky-ass 
       * ( parallel [ ... ] ) JUMP_INSN.
       */

      /* in the case of a single iteration loop, no iteration control is
       * needed.
       */
      if (( CONST_INT != GET_CODE ( loop_count )) ||
	  ( 1 != INTVAL ( loop_count )))
      {
	  /* an aside: putting the do-shape before the NOTE_INSN_LOOP_BEG
	   * affects the depth counter in flow.c. This ensures that when
	   * a "do d5,L111" instruction is issued, d5's reg_n_ref won't be
	   * counted as in the loop.
	   */
	  emit_jump_insn_before ( 
	    gen_rtx ( PARALLEL, VOIDmode,
	      gen_rtvec ( 3,
		gen_rtx ( CLOBBER, VOIDmode, cc0_rtx ),
		gen_rtx ( USE, VOIDmode, loop_count ),
		gen_rtx ( SET, VOIDmode, pc_rtx,
		  gen_rtx ( IF_THEN_ELSE, VOIDmode,
		    gen_rtx ( EQ, SImode, cc0_rtx, const0_rtx ),
		    gen_rtx ( LABEL_REF, Pmode, bypass_label ),
		    pc_rtx )))), loop_start );
	  ...
	  emit_jump_insn_before ( 
		...
		gen_rtx ( CLOBBER, VOIDmode, cc0_rtx ),
		gen_rtx ( SET, VOIDmode, pc_rtx, 
		  gen_rtx ( LABEL_REF, Pmode, looping_label )))), loop_end );
      }

      /* eliminate the old compare and back branch instructions
       * by replacing them with NOTEs. This may open up opportunities for
       * data-flow driven dead code elimination as well.
       */

      PUT_CODE ( compare_insn, NOTE );
      NOTE_LINE_NUMBER ( compare_insn ) = NOTE_INSN_DELETED;
      PUT_CODE ( back_branch_insn, NOTE );
      NOTE_LINE_NUMBER ( back_branch_insn ) = NOTE_INSN_DELETED;
      
      /* decide whether or not to jam out a ( set ... ) which will set
       * the induction variable to the loop termination value. If the
       * induction variable is being used within the loop, don't. Otherwise
       * set the iv after the loop so that the iv ( set ... ) within the
       * loop might be blown out as dead code.
       */

      if (( ! reg_really_used_between_p ( biv, loop_start, 
				  class_struct[ REGNO ( biv ) ]->biv->insn ))
	  &&
	  ( ! reg_really_used_between_p ( biv, 
				  class_struct[ REGNO ( biv ) ]->biv->insn,
				  loop_end )))
      {
	  if ( CONST_INT == GET_CODE ( loop_count ))
	  {
	      /* we have a const loop count <=> a,b,c are consts. */

	      emit_insn_after ( 
			       gen_rtx ( SET, VOIDmode, biv,
					gen_rtx ( CONST_INT, VOIDmode,
						 INTVAL ( biv_initial ) + 
						 INTVAL ( biv_increment ) * 
						 INTVAL ( loop_count ))),
			       bypass_label );
	  }
	  else /* REG == GET_CODE ( loop_count ) */
	  {
	      rtx biv_initial_reg, mult_reg = gen_reg_rtx ( SImode );
	      
	      
	      biv_initial_reg = ( REG == GET_CODE ( biv_initial )) ?
				 biv_initial :
				 gen_reg_rtx ( GET_MODE ( biv ));
	      
	      emit_insn_after ( gen_rtx ( SET, VOIDmode, biv,
					 gen_rtx ( PLUS, GET_MODE ( biv ),
						  biv_initial_reg,
						  mult_reg )), 
			       bypass_label );
	      
	      emit_insn_after ( gen_rtx ( SET, VOIDmode, mult_reg,
					 gen_rtx ( MULT, SImode,
						  biv_increment,
						  loop_count )), 
			       bypass_label );

	      if ( biv_initial_reg != biv_initial )
	      {
		  emit_insn_after ( gen_rtx ( SET, VOIDmode, biv_initial_reg,
					     biv_initial ), bypass_label );
	      }
	  }
      }
  } while ( 0 );
     ...
     rtx insert_before;
{
  ...
  start_sequence ();
#if defined( DSP56000 ) || defined( DSP96000 )
  /* since we're the only fn to call expand_mult_add, we've augmented it
     to take target as an extra argument. see expmed.c for more details. */
  result = expand_mult_add (b, m, a, GET_MODE (reg), 0, reg);
#else
...
#if defined( DSP96000 ) || defined( DSP56000 )
/* find the first mem expression encountered. */
static rtx
find_mem ( pattern )
    rtx pattern;
{
    ...
    if ( MEM == GET_CODE ( pattern ))
    {
	return pattern;
    }
    {
	int i = GET_RTX_LENGTH ( GET_CODE ( pattern )) - 1;
	char* f = GET_RTX_FORMAT ( GET_CODE ( pattern ));
	rtx found;
	
	while ( i >= 0 )
	{
	    if ( 'e' == f[i] &&
		( found = find_mem ( XEXP ( pattern, i ))))
	    {
		return found;
	    }
	    if ( 'E' == f[i] )
	    {
		int j = XVECLEN ( pattern, i ) - 1;

		while ( j >= 0 )
		{
		    if ( found = find_mem ( XVECEXP ( pattern, i, j )))
		    {
			return found;
		    }
		    -- j;
...
/* find the label referenced by this conditional jump insn. if the jump
 * isn't conditional, return NULL.
 */
static rtx
get_bcond_label ( insn )
    rtx insn;
{
    rtx pat = PATTERN ( insn );
    
    if ( IF_THEN_ELSE != GET_CODE ( pat = SET_SRC ( pat )))
    {
	return NULL;
    }
    if ( LABEL_REF == GET_CODE ( XEXP ( pat, 1 )))
    {
	return XEXP ( XEXP ( pat, 1 ), 0 );
    }
    if ( LABEL_REF == GET_CODE ( XEXP ( pat, 2 )))
    {
	return XEXP ( XEXP ( pat, 2 ), 0 );
    }
    return NULL;
}

/* is expr a const expression ? */
static int
const_expr_p ( expr )
    rtx expr;
{
    ...
    case PLUS:
    case MINUS:
	return ( const_expr_p ( XEXP ( expr, 0 )) &&
		const_expr_p ( XEXP ( expr, 1 )));
...
/* try to find the "best" equivalent rtx for this rtx - we are trying to 
 * show that this rtx is actually loop invariant.
 */
static rtx 
find_best_equiv ( init, use_insn, stop_insn )
    rtx init, use_insn, stop_insn;
{
    /* use the limit technique to stop a feedback situation. */
    static int max_search_depth = 10;
    rtx tmp;
    
    /* a constant expression is the *best* loop invariant possible ! */
    if ( const_expr_p ( init ))
    {
	return init;
    }
    
    /* anything but a REG would be illegal here. */
    if ( REG != GET_CODE ( init ))
    {
	return NULL;
    }
    
    /* step from the use_insn up towards the stop_insn, looking for an insn
     * that sets init. 
     */

    tmp = use_insn;
    while ( tmp = PREV_INSN ( tmp ), 
	   ( tmp != stop_insn ) && 
	   ( CODE_LABEL != GET_CODE ( tmp )) &&
	   ! (( INSN == GET_CODE ( tmp )) &&
	      ( SET == GET_CODE ( PATTERN ( tmp ))) &&
	      ( init == SET_DEST ( PATTERN ( tmp )))));
    
    /* do we have an acceptable SET ? */
    if (( INSN == GET_CODE ( tmp )) &&
	( SET == GET_CODE ( PATTERN ( tmp ))) &&
	( init == SET_DEST ( PATTERN ( tmp ))) &&
	( max_search_depth ))
    {
	rtx src = SET_SRC ( PATTERN ( tmp ));
	while ( SUBREG == GET_CODE ( src ))
	{
	    src = XEXP ( src, 0 );
	}
	
	-- max_search_depth;
	tmp = find_best_equiv ( src, use_insn, stop_insn );
	++ max_search_depth;
	return tmp;
    }

    /* we have a register which is supposedly our loop invariant loop bound:
     * now we make sure that it is really invariant in the loop.
     */

    if ( reg_set_between_p ( init, stop_insn, use_insn ))
    {
	return NULL;
    }
    return init;
}
/* compute the loop count from the parameters given. if the computation will
 * take place at run time, we may need to check for <=0 iterations, and bypass
 * the loop. form: for ( i = a; i rel b; i += c ). return NULL if this loop
 * is un-do-able.
 */
static rtx
compute_loop_count ( start, search_bound, bypass, a, rel, b, c )
    rtx start, search_bound, bypass, a, rel, b, c;
{
    int tmp, signed_cmp = 0;
    unsigned int utmp;
    enum rtx_code code = GET_CODE ( rel );
    
#define CLEAN_TO_BITS_PER_UNIT( x )\
    ((((unsigned int)x)<<(HOST_BITS_PER_INT-BITS_PER_UNIT))>>\
     (HOST_BITS_PER_INT-BITS_PER_UNIT))

    /* if everything is a constant, we can figure out the loop count now. */
    if (( CONST_INT == GET_CODE ( a )) &&
	( CONST_INT == GET_CODE ( b )) &&
	( CONST_INT == GET_CODE ( c )))
    {
	int a_s, b_s, c_s;
	unsigned int a_u, b_u, c_u;
...
#if defined( DSP56000 )
	case NEU:
	    /* this is analogous to the NE case, but for unsigned values */
#endif
	case NE:
	    /* this is a special case: we really have a > or < situation.
	     * determine which, and set a_u, b_u, c_u accordingly. 
	     */
	    if ( 0 > INTVAL ( c ))
	    {
		/* case GT or GTU */
	...
	/* compute ceiling( ( b - a + 1 ) / c ) as the iteration count. if 
	 * we count zero iterations, bail.
	 */
	if ( signed_cmp )
	{
	    tmp = CLEAN_TO_BITS_PER_UNIT ( 1 + b_s - a_s ) / c_s;
	    tmp += ( CLEAN_TO_BITS_PER_UNIT ( c_s * tmp ) != 
		     CLEAN_TO_BITS_PER_UNIT ( 1 + b_s - a_s )) ? 1 : 0;
	...
	{
	    utmp = CLEAN_TO_BITS_PER_UNIT ( 1 + b_u - a_u ) / c_u;
	    utmp += ( CLEAN_TO_BITS_PER_UNIT ( c_u * utmp ) !=
		      CLEAN_TO_BITS_PER_UNIT ( 1 + b_u - a_u )) ? 1 : 0;
    ...
    /* if a or b is non-constant, we need to have a power of two const for c */
    /* for 1st release: only consider the case of +/- 1. */
    else if (( CONST_INT == GET_CODE ( c )) &&
	     (((( 1 == INTVAL ( c ))/* ||
		(( 0 < INTVAL ( c )) && ( 0 == INTVAL ( c ) % 2 ))*/) &&
	       (( LT == code ) || ( LTU == code ) ||
		( LE == code ) || ( LEU == code ))) ||
	      ((( -1 == INTVAL ( c ))/* ||
		(( 0 > INTVAL ( c )) && ( 0 == ( - INTVAL ( c )) % 2 ))*/) &&
		...
		( GE == code ) || ( GEU == code )))))
    {
	/* we can do do-loops for modes <= SImode in width. */
	if (( GET_MODE_SIZE ( SImode ) < GET_MODE_SIZE ( GET_MODE ( a ))) ||
	    ( GET_MODE_SIZE ( SImode ) < GET_MODE_SIZE ( GET_MODE ( b ))))
	{
	    return 0;
	}
	
	/* force both operands into registers for the loop count computation */
	if ( REG != GET_CODE ( a ))
	{
	    rtx tmp = gen_reg_rtx (( VOIDmode == GET_MODE ( a )) ?
				   (( VOIDmode == GET_MODE ( b )) ? SImode :
				    b->mode ) : a->mode );
	    
	    emit_insn_before ( gen_rtx ( SET, VOIDmode, tmp, a ), start );
	    a = tmp;
	}

	if ( REG != GET_CODE ( b ))
	{
	    rtx tmp = gen_reg_rtx (( VOIDmode == GET_MODE ( b )) ?
				   a->mode : b->mode );
	    
	    emit_insn_before ( gen_rtx ( SET, VOIDmode, tmp, b ), start );
	    b = tmp;
	}
	
	/* generate loop count computation and verification code. */
	if ( 1 == INTVAL ( c ))
	{ 
	    rtx ret_tmp = gen_reg_rtx ( SImode );
	    rtx cond = gen_rtx ( flip_cond_code ( code ), VOIDmode, cc0_rtx,
				const0_rtx );
	    rtx target = gen_rtx ( LABEL_REF, Pmode, bypass );
	    rtx const1_reg;
	    
	    if ( ! pre_existing_test_p ( start, search_bound, a, b, 
					GET_CODE ( cond ), target, pc_rtx ))
	    {
		emit_insn_before ( gen_rtx ( SET, VOIDmode, cc0_rtx, 
					    gen_rtx ( COMPARE, VOIDmode, 
						     a, b )), start );
		
		emit_insn_before ( gen_rtx ( SET, VOIDmode, pc_rtx,
					    gen_rtx ( IF_THEN_ELSE,
						     VOIDmode, cond, target,
						     pc_rtx )), start );
	    }
	    ...
	    case LT:
	    case LTU:
		emit_insn_before ( gen_rtx ( SET, VOIDmode, ret_tmp,
					    gen_rtx ( MINUS, b->mode, b, a )),
				  start );
		return ret_tmp;
		break;
		...
		const1_reg = gen_reg_rtx ( SImode );
		
		emit_insn_before ( gen_rtx ( SET, VOIDmode, const1_reg,
					    const1_rtx ), start );
		
		emit_insn_before ( gen_rtx ( SET, VOIDmode, ret_tmp,
					    gen_rtx ( PLUS, SImode, ret_tmp,
						     const1_reg )),
				  start );
    ...
    /* we can't (realistically) do this loop. */
    return NULL;
}

/* we want to be able to trace back through dsts to tell if the reg was really
 * ever used in other than an increment situation. we punt on feedback 
 * situations where an inc or self-mod might involve multiple regs. we go ahead
 * with this simple recursive code and punt if we go > 10 searches deep. 
 */
static int
reg_really_used_between_p ( reg, from_insn, to_insn )
    rtx reg, from_insn, to_insn;
{
    ...
    for ( insn = NEXT_INSN ( from_insn );
	 insn != to_insn; insn = NEXT_INSN ( insn ))
    {
	    ...
	    && reg_mentioned_p (reg, PATTERN (insn)))
	{
	    ...
	    {
		if (( reg != SET_DEST ( PATTERN ( insn ))) &&
		    ( max_search_depth ))
		{
		    -- max_search_depth;
		    
		    if ( reg_really_used_between_p ( SET_DEST ( PATTERN ( insn )),
						    from_insn, to_insn ))
		    {
			++ max_search_depth;
			return 1;
...
static int
biv_qualified_p ( biv, label, branch )
    rtx biv, label, branch;
{
    /* if we have multiple sets of this register, the GNU biv info is
     * essentially unreliable. we don't yet have any dataflow info so ...
     */
    if ( 1 < n_times_set[ REGNO ( biv ) ] )
    {
	return 0;
    }

    {
	rtx peek = NEXT_INSN ( label );
	rtx goal = class_struct[ REGNO ( biv ) ]->biv->insn;
	
	/* scan from the loop continuation label downward to see if 
	 * we can find the insn which sets the biv. if we encounter a
	 * label or a jump abandon the search and search upward from
	 * the back-branch insn. if we still can't find the set, then
	 * we really can't be sure that the biv will be altered on each 
	 * iteration. again, it would have been nice to have dataflow 
	 * info here ...
	 */
	while (( CODE_LABEL != GET_CODE ( peek )) &&
	       ( JUMP_INSN != GET_CODE ( peek )))
	{
	    if ( peek == goal )
	    {
...
static int
no_jumps_out_of_loop_p ( start, end )
    rtx start, end;
{
    int do_depth = 1;
    
    rtx step = start;
    int label_max = 50, label_high_water = 0;
    int jump_max = 50, jump_high_water = 0;
    rtx jump[ 50 ], label[ 50 ];
    int index;

    for ( ; step != end; step = NEXT_INSN ( step ))
    {
	if ( ! step )
	{
	    return 0;
	...
	if ( insn_is_do_p ( step ))
	{
	    if (( ++ do_depth ) > MAX_DO_LOOP_NESTING )
	    {
		return 0;
	    }
	    continue;
	...
	if ( CALL_INSN == GET_CODE ( step ))
	{
	    return 0;
	}
	
	if ( JUMP_INSN == GET_CODE ( step ))
	{
	    rtx label_referenced;
	    
	    if ( ! ( label_referenced = get_bcond_label ( step )))
	    {
		if ( PARALLEL != GET_CODE ( PATTERN ( step )))
		{
		    if (( SET != GET_CODE ( PATTERN ( step ))) ||
			( LABEL_REF != 
			 GET_CODE ( SET_SRC ( PATTERN ( step )))))
		    {
			return 0;
		    }
		    label_referenced = XEXP ( SET_SRC ( PATTERN ( step )),
					     0 );
		}
		else
		{
	    ...
	    for ( index = 0; index < label_high_water; ++ index )
	    {
		if ( label[ index ] == label_referenced )
		{
		    label_referenced = NULL;
		    break;
		}
	    }
	    if ( label_referenced )
	    {
		for ( index = 0; index < jump_high_water; ++ index )
		{
		    if ( ! jump[ index ] )
		    {
			jump[ index ] = label_referenced;
			break;
		    }
		}
		if ( index == jump_high_water )
		{
		    if ( jump_high_water == jump_max )
		    {
			return 0;
		    }
		    jump[ jump_high_water ++ ] = label_referenced;
		}
	    }
	}
	else if ( CODE_LABEL == GET_CODE ( step ))
	{
	    for ( index = 0; index < jump_high_water; ++ index )
	    {
		if ( jump[ index ] == step )
	    ...
	    if ( label_max == label_high_water )
	    {
		return 0;
	    }
	    label[ label_high_water ++ ] = step;
	}
    }
    for ( index = 0; index < jump_high_water; ++ index )
    {
...
insn_is_do_p ( insn )
    rtx insn;
{
    if (( JUMP_INSN == GET_CODE ( insn )) &&
	( PARALLEL == GET_CODE ( PATTERN ( insn ))) &&
	( 3 == XVECLEN ( PATTERN ( insn ), 0 )))
    {
	return 1;
    }
    return 0;
...
static int
rtx_label_equiv_p ( lab1, lab2 )
    rtx lab1, lab2;
{
    if ( rtx_equal_p ( lab1, lab2 ))
    {
	return 1;
    }
    
    if (( LABEL_REF == GET_CODE ( lab1 )) &&
	( LABEL_REF == GET_CODE ( lab2 )))
    {
	rtx ref_insn = XEXP ( lab1, 0 );
	rtx walker = XEXP ( lab2, 0 );
	
	do 
	{
	    walker = PREV_INSN ( walker );
	...
	while (( NULL != walker ) &&
	       (( NOTE == GET_CODE ( walker )) ||
		( CODE_LABEL == GET_CODE ( walker )) ||
		(( INSN == GET_CODE ( walker )) &&
		 ( const0_rtx == PATTERN ( walker )))));
	
	walker = XEXP ( lab2, 0 );
	
	do 
	{
	    walker = NEXT_INSN ( walker );
...
static int
pre_existing_test_p ( base_insn, search_bound, 
		     cmp_a, cmp_b, cond_code, target, drop )
    rtx base_insn, search_bound, cmp_a, cmp_b, target, drop;
    enum rtx_code cond_code;
{
    /* look backwards up the basic block, checking for mods to cmp_a or
       cmp_b, and or CALL_INSNs */

    rtx insn = PREV_INSN ( base_insn );
    
    if ((( REG != GET_CODE ( cmp_a )) && ( ! CONSTANT_P ( cmp_a ))) ||
	(( REG != GET_CODE ( cmp_b )) && ( ! CONSTANT_P ( cmp_b ))))
    {
	return 0;
    }
    
    while (( NULL != insn ) && ( JUMP_INSN != GET_CODE ( insn )))
    {
	if ( reg_set_p ( cmp_a, insn ) || reg_set_p ( cmp_b, insn ) ||
	    ( CALL_INSN == GET_CODE ( insn )))
	{
	    return 0;
	}
	insn = PREV_INSN ( insn );
    ...
    /* check to see that the jmp insn is compatable */

    {
	rtx body = PATTERN ( insn );
	
	if (( SET != GET_CODE ( body )) || ( pc_rtx != SET_DEST ( body )))
	{
	    return 0;
	}
	...
	if (( IF_THEN_ELSE != GET_CODE ( body )) ||
	    ((( ! rtx_equal_p ( XEXP ( body, 0 ), 
			       gen_rtx ( cond_code, VOIDmode,
					cc0_rtx, const0_rtx ))) ||
	      ( ! rtx_label_equiv_p ( XEXP ( body, 1 ), target )) ||
	      ( ! rtx_label_equiv_p ( XEXP ( body, 2 ), drop ))) &&
	     (( ! rtx_equal_p ( XEXP ( body, 0 ), 
			       gen_rtx ( flip_cond_code ( cond_code ), 
					VOIDmode, cc0_rtx, const0_rtx ))) ||
	      ( ! rtx_label_equiv_p ( XEXP ( body, 1 ), drop )) ||
	      ( ! rtx_label_equiv_p ( XEXP ( body, 2 ), target )))))
	{
	    return 0;
	}
	
	/* the jmp checked out, move on to the cmp */
	
	do
	{
	    insn = PREV_INSN ( insn );
	}
	while (( NULL != insn ) && ( NOTE == GET_CODE ( insn )));
	
	if (( NULL == insn ) || ( INSN != GET_CODE ( insn )))
	{
	    return 0;
	}
	...
	if (( SET != GET_CODE ( body )) ||
	    ( cc0_rtx != SET_DEST ( body )) ||
	    ( COMPARE != GET_CODE ( SET_SRC ( body ))))
	{
	    return 0;
	}
	
	{ 
	    rtx best_val1 = find_best_equiv ( XEXP ( SET_SRC ( body ), 0 ),
					     base_insn, search_bound );

	    rtx best_val2 = find_best_equiv ( XEXP ( SET_SRC ( body ), 1 ),
					     base_insn, search_bound );
	    
	    rtx best_cmp_a = find_best_equiv ( cmp_a, 
					      base_insn, search_bound );

	    rtx best_cmp_b = find_best_equiv ( cmp_b,
					      base_insn, search_bound );

	    best_val1 = ( best_val1 ) ? 
		best_val1 : XEXP ( SET_SRC ( body ), 0 );

	    best_val2 = ( best_val2 ) ? 
		best_val2 : XEXP ( SET_SRC ( body ), 1 );
	    
	    best_cmp_a = ( best_cmp_a ) ? best_cmp_a : cmp_a;
	    
	    best_cmp_b = ( best_cmp_b ) ? best_cmp_b : cmp_b;
	    
	    if (( ! rtx_equal_p ( best_cmp_a, best_val1 )) ||
		( ! rtx_equal_p ( best_cmp_b, best_val2 )))
	    {
		return 0;
	    }
	}
...
flip_cond_code ( orig_code )
    enum rtx_code orig_code;
{
    switch ( orig_code )
    {
    case NE:
