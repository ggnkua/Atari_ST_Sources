/* Generate from machine description:
   ...
   $Id: genconfig.c,v 1.2 91/01/15 09:47:43 jeff Exp $
...
/* we want to specify the maximium number of asm operands in the manual. 
   we force these MAX_s to be >= the max specified in the manual. */
#if defined( MIN_RECOG_OPERANDS )
  printf ("\n#define MAX_RECOG_OPERANDS %d\n", 
	  ( MIN_RECOG_OPERANDS > max_recog_operands_flag + 4 ) ?
	  MIN_RECOG_OPERANDS : max_recog_operands_flag + 4 );
#else
...
#if defined( DSP56000 ) || defined( DSP96000 )
#if defined( MIN_DUP_OPERANDS )
  printf ("\n#define MAX_DUP_OPERANDS %d\n", 
	  ( MIN_DUP_OPERANDS > max_dup_operands_flag ) ?
	  MIN_DUP_OPERANDS : max_dup_operands_flag );
#else
