/* C-compiler utilities for types and variables storage layout
   ...
   $Id: stor-layout.c,v 1.6 91/10/23 17:07:24 pete Exp $
...
#if defined( DSP96000 ) || defined( DSP56000 )
  /* we may want to eventually check for memory_model here ! */
  return BLKmode;
#endif
     ...
     tree type;
{
  ...
  switch (TREE_CODE (type))
...
#if defined( DSP56000 )
      /* this is lame, but in L mode, we still need to let the compiler think
	 that DImode has 48 bits of precision, while also making it think that
	 it occupies one storage location. */
      if ( LONG_TYPE_SIZE == TYPE_PRECISION ( type ))
      {
	  TYPE_MODE ( type ) = DImode;
	  TYPE_SIZE ( type ) = build_int (( 'l' == memory_model ) ? 1 : 2 );
      }
      else
      {
	  TYPE_SIZE ( type ) = 
	      build_int ( GET_MODE_SIZE ( TYPE_MODE ( type )));
      }
#else
    ...
    case REAL_TYPE:
      {
#if defined( DSP96000 ) 
	  /* 1) on the 96k SP (floats) and SEP (double) numbers use the
	     same amount of memory under the l memory model, (so the
	     TYPE_PRECISION test isn't always valid. 
	     2) on the 56k, we make everything SFmode, as DFmode and SFmode
	     are exactly the same thing. this occurs naturally with the
	     FSF code below. */
...
#if defined( DSP56000 )
      /* a pointer must occupy an entire mem unit. */
      TYPE_SIZE (type) = build_int ( 1 );
#else
