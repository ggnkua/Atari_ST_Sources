...
Exakte Abfragemethoden:
- appl_getinfo()
- "SMAL"-Cookie

Neu implementiert:
- Abfrage von appl_find("?AGI")

Dieser Code darf frei verwendet werden und wird stÑndig aktualisiert.
*/
                              ...
                              FONT_ATTR *Small, int *Addfnt)
{
   ...
   register int i;
	...
	if (has_agi<0)
   	has_agi=(_GemParBlk.global[0]>=0x400 	/* AES 4.0 ? 	*/
            || appl_find("?AGI")==0) 			/* "?AGI" ? 	*/
            || (getcookie('MagX',&cookie)		/* MagiC!2.0 ? */
                && ((int **)cookie)[2][24]>=0x200);
             
   xxxx_getinfo = (has_agi) ? appl_getinfo : afnt_getinfo;
   ...
   {  /* Hîhe ermitteln */
      ...
      if (getcookie('SMAL', &cookie) && (cookie & 0xFFFF0000L))
      {
         /* aus 'SMAL'-Cookie */
         Ibm->chhgt = (int)((cookie & 0xFFFF0000L) >> 16);
      }
      else
      {  /* aus graf_handle */
   ...
   if (xx_return == 0)
   {
      ...
      if (getcookie('SMAL', &cookie) && (cookie & 0x0000FFFFL))
         Small->chhgt = (int) (cookie & 0x0000FFFFL);
      else
