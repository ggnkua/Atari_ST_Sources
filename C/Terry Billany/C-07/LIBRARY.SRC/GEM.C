 ...
 * - 25/9/88 vr_recfl,v_pieslice,vqt_name
 *   accessory creation,evnt_multi,evnt_timer
 * - 20/11/88 rsrc_load,form_center,rsrc_free,evnt_mesag
 *   menu_ienable
 * - 4/12/88 appl_write,v_justified
*/
 ...
 *
 * assumes a startup prg.s or acc.s of
 *	. _bstk 2048
 *	. _estk 4
 *	taa 7 6
 *	lll 4 0   ; not in acc
 *	sgl _estk ; not in acc
 *	lag _estk 7
 *	jsr _cprg ( or _cacc)
...
vqt_name(h, n, name) char *name; { /* query face name & index */
int *ip,i;
	ii0=n;
	vdc(130, 0, 1, 0, h);
	ip=&io1;
	for (i=0;i<32;i++) name[i]=*ip++;
	return(io0);
}
/* graphic primitives */
...
vr_recfl(h, xy) short *xy; { /* draw a filled rectangle */
...
evnt_multi(
  ...
  ox, oy, obut, ostate, okret, obret) 
	int *buff, *ox, *oy, *obut, *ostate, *okret, *obret;
{
	ct0 = 25;
	ct1 = 16;
	ct2 = 7;
...
evnt_timer(lo, hi) {	/* wait for timer event */
	ct0 = 24;
	ct1 = 2;
	ct2 = 1;
	ct3 = ct4 = 0;
...
evnt_mesag(buff) int *buff; {	/* wait for event message */
	ct0 = 23;
	ct2 = ct3 = 1;
	ct1 = ct4 = 0;
	ai0 = buff;
...
appl_write(id,len,buff) int *buff; {	/* send event message */
	ct0 = 12;
	ct1 = 2;
	ct2 = 1;
	ct3 = ct4 = 0;
