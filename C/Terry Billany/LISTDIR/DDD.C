...
char *dfix();
	...
	if(file_cell->file_att != 0x10) {
		...
		printf("%s/",dfix((file_cell->date >> 5) & 0x0F,2)); /* month */
		printf("%s  ",dfix(file_cell->date & 0x1F,2));	   /* day */

		printf("%s:",dfix(file_cell->time >> 11,2));	/* hour */
		printf("%s",dfix((file_cell->time >> 5) & 0x3F,2)); /* mins */
	}
	printf("\n");
}
/*-------------------------*/
...
return(i); }
/*-------------------------------------------*/
/* return pointer to string holding int 'n'  */
/* right-aligned & zero-filled to length 'l' */
/*-------------------------------------------*/
char * dfix(n,l)
{
sprintf(work,"0000%d",n);
return &work[strlen(work)-l];
}
