...
int incnbplanes(MFDB *data, int nb_plan, int* cancel_stdvdi)
{
  ...
  {                            /* VDI s'occupe de la conversion */
    unsigned char* pt_vdi2tos = get_ptvdi2tos( nb_plan ) ;
    int            xy[8] ;
    int            color_index[2] ; /*= {1, 0} ;*/

    color_index[0] = pt_vdi2tos[1] ;
    color_index[1] = pt_vdi2tos[0] ;
  ...
  for ( i = 0; img_fmt[i]; i++ )
  {
    ...
    if ( img_module )
    {
      ...
      else if ( DImgGetIIDFromFile( name ) != inf->lformat )
      {
        ...
        if ( ext )
        {
          LoggingDo(LL_INFO, "File extension is %s, inconsistent with IID %08lX, enforcing default format extension %s  ", ext, img_module->Capabilities.iid, img_module->Capabilities.file_extension) ;
          *ext = 0 ;
          strcat( name, "." ) ;
          strcat( name, img_module->Capabilities.file_extension ) ;
        }
