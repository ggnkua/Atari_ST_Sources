...
static char* human_readable_alloc_size(size_t size, char* string)
{
  ...
  if ( i != 0 ) sprintf(string, "%lu%s (%lu bytes)", (unsigned long)size, unit[i], (unsigned long)nbytes) ;
  else          sprintf(string, "%lu bytes", (unsigned long)nbytes) ;
    ...
    if (index == -1)
    {
      if (XAllocData.Config.MaxSimultaneousAllocs+XAllocData.OMaxSimultaneousAllocs <= XAllocData.VeryMaxSimultaneousAllocs)
      {
         ...
         if (npUser)
         {
           ...
           memzero(&npUser[XAllocData.Config.MaxSimultaneousAllocs], XAllocData.OMaxSimultaneousAllocs*sizeof(void*)) ;
...
static void freeWithType(void* p, int allocType)
{
  switch(allocType)
  {
    case TA_STRAM:  LoggingDo(LL_MEM, "Freeing ST-RAM at $%p", p) ;
                    free(p) ;
                    break ;
    case TA_TTRAM:
    case TA_STPREF:
    case TA_TTPREF: LoggingDo(LL_MEM, "Freeing xT-RAM at $%p", p) ;
                    Mfree(p) ;
                    break ;
...
static int remove_ref_and_free(void* puser)
{
  ...
  if (free_it)
  {
    ...
    if ( XAllocData.Config.Flags & FA_RESET_MEM_ON_FREE )
      memset( puser, 0xAA, h->UsableSize ) ; /* Let's corrupt data in case app would use it */
    XAllocData.Stats.NbBytesAllocatedNow -= h->UsableSize ; /* Update now as h will become invalid after freeWithType (bug#54) */
...
size_t XmemAvailableG(void) /* From Thorsten Otto, see http://www.atari-forum.com/viewtopic.php?f=16&t=34298&start=50#p381668 */
{
  DISKINFO d ;
  size_t   coresize ;
  size_t   thissize ;
...
static void* smart_malloc(size_t xsize, char* filename, long linenumber, int* flags)
{
  ...
  if ( p )
  {
    ...
    LoggingDo(LL_MEM, "smart_alloc header=$%p, puser=$%p, xsize=%ld, tail=$%p at %s, line %ld", h, puser, xsize, t, filename, linenumber) ;
    if (add_ref(puser) < 0)
    {
       static int mad ;
  ...
  {
    clock_t now = clock() ;
    clock_t dt_ms = (1000L * (now - XAllocData.Stats.LastStatsTime) ) / CLK_TCK ;

    if (dt_ms > (1000L*XAllocData.Config.StatsEverynSec) )
    {
      XallocStats() ;
      XAllocData.Stats.LastStatsTime = now ;
    }
  }
...
void* __Xrealloc(void* puser, size_t usize, char* filename, long linenumber)
{
  void* new_puser = NULL ;
  char* sfn = short_filename(filename) ;
  int   cm = -1 ;

  if ( puser == NULL ) new_puser = __Xalloc(usize,sfn,linenumber) ;
  else if ( usize == 0 ) Xfree(puser) ;
  else
  {
    cm = perform_checks(puser) ;
    if (cm & 0x01)
      LoggingDo(LL_ERROR, "$%p to be re-allocated from %s, line %ld is NOT referenced", puser, sfn, linenumber) ;
    if (cm & 0x02)
  ...
  /* Only proceed to realloc if safe */
  if ( cm == 0 )
  {
    /* We are not as smart as stdlib realloc as we */
    /* Are going to alloc/free for this request    */
    new_puser = __Xalloc(usize, sfn, linenumber) ;
    if ( new_puser )
    {
      ABLOCK_HEADER* h = GET_HEADER(puser) ;

      memcpy(new_puser, puser, h->UsableSize) ;
      if ( usize > h->UsableSize )
        memzero((char*)new_puser+h->UsableSize, usize-h->UsableSize) ; /* Zero new available data */
...
void __Xfree(void* puser, char* filename, long linenumber)
{
  ...
  if ( puser == NULL )
  {
    LoggingDo(LL_WARNING, "Trying to free NULL pointer from %s, line %ld, ignoring", sfn, linenumber) ;
    return ;
  }

  LoggingDo(LL_MEM, "About to free $%p at %s line %ld", puser, sfn, linenumber) ;
  cm = perform_checks(puser) ;
  if (cm & 0x01)
    LoggingDo(LL_ERROR, "$%p to be freed from %s, line %ld is NOT referenced", puser, sfn, linenumber) ;

  if (cm & 0x02)
    LoggingDo(LL_ERROR, "Allocated memory at $%p (file %s, line %ld) has been corrupted at start ($%p)", puser, sfn, linenumber, GET_HEADER(puser)) ;

  if (cm & 0x04)
  {
    ABLOCK_TAILER* t = GET_TAILER(puser) ;

    LoggingDo(LL_ERROR, "Allocated memory at $%p (file %s, line %ld) has been corrupted at end ($%p): %lX", puser, sfn, linenumber, t, t->Magic) ;
  }
...
void* __Xcalloc(size_t num_elems, size_t elem_size, char* filename, long linenumber)
{
  ...
  if ( p ) memzero(p, num_elems*elem_size) ;
...
int XallocInit(XALLOC_CONFIG* config)
{
  ...
  memzero(&XAllocData, sizeof(XAllocData)) ;
  ...
  if (XAllocData.pUser)
  {
    memzero(XAllocData.pUser, config->MaxSimultaneousAllocs*sizeof(void*)) ;
    ...
    if (XAllocData.FileNameBuffer)
    {
      ...
      memzero(XAllocData.FileNameBuffer, config->MaxReferencedSrcFilesSize*sizeof(char)) ;
      XAllocData.FileNameBufferSize = 0 ;
      if ( (Sversion() < 0x1900) || (tt_ram == 0) ) allocType = TA_STRAM ;
      else if (allocType == TA_AUTO)                allocType = TA_TTPREF ;
