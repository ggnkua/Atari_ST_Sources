...
void OnInitPrinterDialog(void* w)
{
  ...
  dlg_user_data->sizex = (float)dlg_user_data->info.mfdb.fd_w*(float)dlg_user_data->info.lpix/10000.0 ; /* En cm */
  sprintf(buffer, "%.1f", dlg_user_data->sizex) ;
  write_text(adr_print, PRINT_SIZEX, buffer) ;
  dlg_user_data->sizey = (float)dlg_user_data->info.mfdb.fd_h*(float)dlg_user_data->info.hpix/10000.0 ; /* En cm */
