  ...
  if ( in->fd_nplanes <= 8 )
  {
    ...
    for ( y = 0; y < in->fd_h; y++ )
    {
      ...
      for (x = 0; x < nb_px; x++)
      {
        ...
        {
          if (intensity < matrix_line[i]) *pt++ = local_back ;
          else                            *pt++ = local_white ;
        }
      }
...
static int dither_bichro(INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  ...
  short           rgb_threshold = inf->dither_param ;
  short           x, line ;
  ...
  else if ( in->fd_nplanes <= 8 )
  {
    ...
    unsigned char* vdi2tos ;
    short*         vdi_palette = (short*) inf->palette ;
    short*         pal_item ;

    vdi2tos = get_ptvdi2tos( in->fd_nplanes) ;
    if ( vdi2tos == NULL ) return EIMG_DATAINCONSISTENT ;

    tos_indexes = Xalloc( align16(inf->mfdb.fd_w) ) ;
    if ( tos_indexes == NULL )
    {
      Xfree( out->fd_addr ) ;
      out->fd_addr = NULL ;
    ...
    for ( line = 0; line < in->fd_h; line++, ptc_line += size_line )
    {
...
int dither(int method, INFO_IMAGE* inf_in,INFO_IMAGE* inf_out, GEM_WINDOW* wprog)
{
  ...
  switch( method )
  {
    ...
    case DITHER_GREY:       if ( nplanes_out == 1 ) ret = dither_bichro( inf_in, &inf_out->mfdb, wprog ) ; /* Grey makes no sense for 1 plane */
                            else                    ret = dither_gris( inf_in, inf_out, wprog, force_allocate ) ;
                            break ;

    case DITHER_DUALTONE:   ret = dither_bichro( inf_in, &inf_out->mfdb, wprog ) ;
                            break ;
  }
