...
{
  return form_sign( default_return, 1, string ) ;
}
...
{
  return form_sign( default_return, 3, string ) ;
}
...
/*  val_kbshift = (unsigned char**) ( (*(unsigned long *)0x4F2 ) + 0x24L ) ;*/
  ...
  if ( !Truecolor )
  {
    if ( screen.fd_nplanes < 4 ) old_pal = (int*) Xcalloc( 16, 3 * sizeof(int) ) ;
    else                         old_pal = (int*) Xcalloc( 1 << screen.fd_nplanes, 3 * sizeof(int) ) ;
