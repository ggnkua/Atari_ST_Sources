...
static int count_files_callback(long nfiles_so_far, char* current_filename, void* user_data)
{
  ...
  if ( cfd && (nfiles_so_far - cfd->previous_nfiles > 16) )
  {
    ...
    if ( now-cfd->last_check > cfd->dclock_min )
    {
      ...
      cfd->cancel_counting = PROG_CANCEL_CODE( GWProgRange( cfd->wcount_files_prog, cfd->fake_pc, -100, NULL ) ) ; /* Fake progress as we don't know, just to handle cancellation */
...
{
  short ids[] = { ALBUM_REMOVE, ALBUM_ADDIMG, ALBUM_ADDFOLDER, ALBUM_INFOS, ALBUM_CATEGORIE1, ALBUM_CATEGORIE2, ALBUM_CATEGORIE3,
