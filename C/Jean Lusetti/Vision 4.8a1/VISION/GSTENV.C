...
void update_view(GEM_WINDOW* gw)
{
  ...
  GWGetHSlider( gw, &xi ) ;
  GWGetVSlider( gw, &yi ) ;
  rc = (int) (((long)xi*(long)(vimage->inf_img.mfdb.fd_w-wx))/1000L) ;
  rl = (int) (((long)yi*(long)(vimage->inf_img.mfdb.fd_h-wy))/1000L) ;
...
void traite_move(GEM_WINDOW* gwnd)
{
  ...
  new = (int) (0.5+1000.0*(float)vimage->x1/(float)(vimage->inf_img.mfdb.fd_w-wx)) ;
  GWSetHSlider( gwnd, new ) ;
  new = (int) (0.5+1000.0*(float)vimage->y1/(float)(vimage->inf_img.mfdb.fd_h-wy)) ;
...
void OnVImgDraw(GEM_WINDOW* wnd, int xywh[4])
{
  ...
  /* Debug crash on Magic when loading demoscne1.jpg */
...
{
/*  unsigned char* thickness = &config.thickness[0] ;*/
  unsigned short key_thick[] = { THICKNESS1, THICKNESS2, THICKNESS3, THICKNESS4, THICKNESS5,
...
/*  switch( key )
  {
    case THICKNESS1: changed = thickness[0] ;
                     break ;

    case THICKNESS2: changed = thickness[1] ;
                     break ;

    case THICKNESS3: changed = thickness[2] ;
                     break ;

    case THICKNESS4: changed = thickness[3] ;
                     break ;

    case THICKNESS5: changed = thickness[4] ;
                     break ;

    case THICKNESS6: changed = thickness[5] ;
                     break ;

    case THICKNESS7: changed = thickness[6] ;
                     break ;

    case THICKNESS8: changed = thickness[7] ;
                     break ;

    case THICKNESS9: changed = thickness[8] ;
                     break ;
  }
*/
  if ( changed )
...
void OnVImgHSlider(GEM_WINDOW* wnd, int pos)
{
  ...
  vimage->x1 = (int) (((long)pos*(long)(vimage->inf_img.mfdb.fd_w-wx))/1000L) ;
  vimage->x2 = vimage->x1 + wx - 1 ;
...
void OnVImgVSlider(GEM_WINDOW* wnd, int pos)
{
  ...
  vimage->y1 = (int) (((long)pos*(long)(vimage->inf_img.mfdb.fd_h-wy))/1000L) ;
  vimage->y2 = vimage->y1 + wy - 1 ;
...
void OnVImgSize(GEM_WINDOW* wnd, int xywh[4])
{
  ...
  else
  {
    ...
    xi = (int) (0.5+1000.0*(float)vimage->x1/(float)(vimage->inf_img.mfdb.fd_w-wxi)) ;
    yi = (int) (0.5+1000.0*(float)vimage->y1/(float)(vimage->inf_img.mfdb.fd_h-whi)) ;
    GWSetHSlider( wnd, xi) ;
    GWSetVSlider( wnd, yi) ;
    wxe = (int) (0.5+1000.0*(float)wxi/(float)vimage->inf_img.mfdb.fd_w) ;
    whe = (int) (0.5+1000.0*(float)whi/(float)vimage->inf_img.mfdb.fd_h) ;
...
int OnVImgFulled(GEM_WINDOW* wnd)
{
  ...
  xi = (int) (0.5+1000.0*(float)vimage->x1/(float)(vimage->inf_img.mfdb.fd_w-wxee)) ;
  yi = (int) (0.5+1000.0*(float)vimage->y1/(float)(vimage->inf_img.mfdb.fd_h-whee)) ;
  GWSetHSlider( wnd, xi ) ;
  GWSetVSlider( wnd, yi ) ;
  wxe = (int) (0.5+1000.0*(float)wxee/(float)vimage->inf_img.mfdb.fd_w) ;
  whe = (int) (0.5+1000.0*(float)whee/(float)vimage->inf_img.mfdb.fd_h) ;
  GWSetHSliderSize( wnd, wxe ) ;
  GWSetVSliderSize( wnd, whe ) ;
  if ( vimage->zoom_level != 1 )
  {
