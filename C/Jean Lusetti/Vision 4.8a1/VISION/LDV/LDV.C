...
static void LDVFreeParameters(LDV_MODULE* ldv_module)
{
  ...
  {
    if ( (ldv_module->IniSpec.Params[i].Type == LDVT_FILE) && ldv_module->IniSpec.Params[i].DefaultValue.str )
    {
      Xfree( ldv_module->IniSpec.Params[i].DefaultValue.str ) ;
      ldv_module->IniSpec.Params[i].DefaultValue.str = NULL ;
    }
...
static int LDVGetIniSpec(char* base_path, LDV_MODULE* ldv_module)
{
  ...
  if ( ( hini != NULL ) && FileExist( name ) )
  {
    ...
    for ( i = 0; i < LDVNBMAX_PARAMETERS; i++ )
    {
      ...
      if ( ldv_module->IniSpec.Params[i].Type == LDVT_SHORT )
      {
        /* MinValue */
        buf[0] = 0 ;
        ...
        if ( ldv_module->IniSpec.Params[i].MinValue.s > ldv_module->IniSpec.Params[i].MaxValue.s )
          ldv_module->IniSpec.Params[i].Name[0] = 0 ; /* Ignore */
      }
      else if ( ldv_module->IniSpec.Params[i].Type == LDVT_FILE )
      {
        buf[0] = 0 ;
        GetIniKey( hini, name, "DefaultValue", buf ) ;
        if ( LDVSetValueFromString( buf, ldv_module->IniSpec.Params[i].Type, &ldv_module->IniSpec.Params[i].DefaultValue ) == 0 )
          ldv_module->IniSpec.Params[i].Name[0] = 0 ; /* String allocation failed, ignore parameter */
      }
    }
...
long cdecl ImGetInf(char* filename, void* v)
{
  ...
  if ( !err )
  {
    ...
    if ( ldv_img->Raster.fd_nplanes == 24 ) ldv_img->Raster.fd_nplanes = 32 ; /* FIXEME: because this is how image io works internally */
...
static int GetBestImgFormat(LDV_CAPS* caps)
{
  ...
  switch( caps->NbPlanes )
  {
    ...
    case 2   :
    case 4   :
    case 8   : if ( caps->Flags & LDVF_SPECFORMAT )       img_format = LDVF_SPECFORMAT ;
               else if ( caps->Flags & LDVF_STDFORMAT )   img_format = LDVF_STDFORMAT ;
               else if ( caps->Flags & LDVF_ATARIFORMAT ) img_format = LDVF_ATARIFORMAT ;
               else                                       img_format = LDVF_ATARIFORMAT ;
               break ;

    case 16  :
    case 24  :
    case 32  : if ( caps->Flags & LDVF_SPECFORMAT )       img_format = LDVF_SPECFORMAT ;
               else if ( caps->Flags & LDVF_ATARIFORMAT ) img_format = LDVF_ATARIFORMAT ;
               else                                       img_format = LDVF_ATARIFORMAT ;
               break ;
    ...
    case LDVF_ATARIFORMAT : if ( UseStdVDI )
                            {
                              if ( ldv_img->Raster.fd_nplanes == screen.fd_nplanes )
                              {
                                ...
                                /*    resolution but do nothing on a different number of planes, e.g.:         */
...
LDV_STATUS LDVDo(LDV_MODULE* ldv, LDV_PARAMS* pldv_params, LDV_IMAGE* pldv_img, int flags)
{
  ...
  if ( LDV_SUCCESS( err ) )
  {
    ...
    /* No need to transform back image if LDV did not change anything         */
