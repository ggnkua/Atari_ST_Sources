...
static short LoadProfile(IMG_REPORT* img_report)
{
  ...
  if ( path_ldv && language )
  {
    ...
    found = 1 ;
    for ( i = 0; found && (i < ARRAY_SIZE(keys)); i++ )
      found = Vapi->InGetKeyFromFile( filename, img_report->profile, keys[i], &img_report->save_options[i][0] ) ;
    if ( !found )
    {
    ...
    found = 1 ; /* We loaed what exists, no need to move to hard-coded profile */
  }
...
{
  /* Quick check to see if all piwel in diagonal are grey */
  ...
  add_pixel +=  add_pixel*16L*(long)in->fd_wdwidth ; /* Offset to next piwel in diagonal */
...
static short AnalyzeFromVISION(IMG_REPORT* img_report)
{
  ...
  if ( status == 0 )
  {
    ...
    long  r, g, b, diff_grey, ref_diff_grey ;
    short i, grey1 = 0 ;
