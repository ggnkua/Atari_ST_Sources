...
static int name_in_mask(char *fname, char *mask)
{
  int  in_mask = 1 ;
...
static int nested_dirs(char* d1, char* d2)
{
  char* d ;
  int   stop = 0 ;
...
static int IFR_Callback(IFR_DATA* ifrData)
{
...
static void SNfind_files(IFR_DATA* ifrData, int attrib)
{
  DTA  *old_dta, dta ;
  int  ret ;
  char name[PATH_MAX] ;
...
static void SNfolder_recurse(IFR_DATA* ifrData, int attrib)
{
  DTA  *old_dta, dta ;
  int  ret ;
  char mask[PATH_MAX] ;
