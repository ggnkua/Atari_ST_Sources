...
size_t lzw_Cdecompress(unsigned char *in, unsigned char *out, size_t nb_bytein)
{
  size_t nb_out ;
  long   last_bit ;
  int    fini   = 0 ;
...
int lzw_init(int ticode, unsigned char *start, int flag)
{
  ...
  {
    oldcode = (int *) Xcalloc(TABLE_SIZE, sizeof(int)) ;
    if (oldcode == NULL) return(-3) ;
    currentcode = (int *) Xcalloc(TABLE_SIZE, sizeof(int)) ;
    if (currentcode == NULL)
    {
      Xfree(oldcode) ;
      oldcode = NULL ;
    ...
    newcode = (int *) Xcalloc(TABLE_SIZE, sizeof(int)) ;
    if (newcode == NULL)
    {
      Xfree(currentcode) ;
      currentcode = NULL ;
  ...
  {
    sizeofstring = (int *) Xcalloc(MAXN, sizeof(int)) ;
    if (sizeofstring == NULL) return(-3) ;
    dstack = (unsigned char *) Xcalloc(MAXN, sizeof(char)) ;
    if (dstack == NULL)
    {
      Xfree(sizeofstring) ;
      sizeofstring = NULL ;
    ...
    suffix = (unsigned char *) Xcalloc(MAXN, sizeof(char)) ;
    if (suffix == NULL)
    {
      Xfree(dstack) ;
      dstack = NULL ;
    ...
    prefix = (unsigned int *) Xcalloc(MAXN, sizeof(int)) ;
    if (prefix == NULL)
    {
      Xfree(suffix) ;
      suffix = NULL ;
...
void lzw_end(long *nb_out)
{
  if (operation == LZW_COMPRESS)
  {
    ...
    *nb_out     = *nb_out+block_count ;
    if (type_flag == GIF) fwrite(&block_count, 1, 1, stream) ;
    fwrite(cprs_buffer, block_count, 1, stream) ;
    if (newcode != NULL) Xfree(newcode) ;
    newcode = NULL ;
    if (currentcode != NULL) Xfree(currentcode) ;
    currentcode = NULL ;
    if (oldcode != NULL) Xfree(oldcode) ;
    oldcode = NULL ;
  }
  else
  {
    if (prefix != NULL) Xfree(prefix) ;
    prefix = NULL ;
    if (suffix != NULL) Xfree(suffix) ;
    suffix = NULL ;
    if (dstack != NULL) Xfree(dstack) ;
    dstack = NULL ;
  ...
  while (pt_in <= last_data)
  {
    ...
    for (;;)
    {
      if (currentcode[hx] == 0)
      {
        ...
        if (pt_cprs >= 250)
        {
          block_count = (unsigned char) pt_cprs ;
          *nb_out      = *nb_out+block_count ;
          if (type_flag == GIF) fwrite(&block_count, 1, 1, stream) ;
          if (fwrite(cprs_buffer, 1, block_count, stream) != block_count)
            return(-2) ;
        ...
        if (d >= first_code+limit)
        {
          ...
          else
          {
            output_code(clear_code) ;
            last_ind = 0 ;
            memset(currentcode, 0, TABLE_SIZE*sizeof(int)) ;
            code_size   = start_size ;
            limit       = (1 << code_size)-first_code ;
            if (type_flag != GIF) limit-- ;
          }
