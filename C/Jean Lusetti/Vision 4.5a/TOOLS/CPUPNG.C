...
{
  inf->bit_depth  = png_get_bit_depth( png_str, png_inf ) ;
  inf->version    = 1 ;
  inf->largeur    = (int) png_get_image_width( png_str, png_inf ) ;
  inf->hauteur    = (int) png_get_image_height( png_str, png_inf ) ;
  inf->compress   = png_get_compression_type( png_str, png_inf ) ;
  inf->channels   = png_get_channels( png_str, png_inf ) ;
  inf->nplans     = inf->bit_depth * inf->channels ;
  inf->num_passes = (int) png_set_interlace_handling( png_str ) ;
  inf->type       = png_get_color_type( png_str, png_inf ) ;
  inf->palette    = NULL ;
  inf->nb_cpal    = 0 ;
  inf->lpix       = 0x150 ;
  inf->hpix       = 0x150 ;
  LoggingDo(LL_DEBUG, "PNG type=%d", inf->type) ;
  LoggingDo(LL_DEBUG, "PNG compression=%d", inf->compress) ;
  LoggingDo(LL_DEBUG, "PNG width=%d", inf->largeur) ;
  LoggingDo(LL_DEBUG, "PNG height=%d", inf->hauteur) ;
  LoggingDo(LL_DEBUG, "PNG planes=%d", inf->bit_depth) ;
  LoggingDo(LL_DEBUG, "PNG channels=%d", inf->channels) ;
  LoggingDo(LL_DEBUG, "PNG npasses=%d", inf->num_passes) ;
  LoggingDo(LL_DEBUG, "INF nplans=%d", inf->nplans) ;
}
...
#define NB_BYTES_HEADER 8
  ...
  if ( (inf->largeur <= 0) || (inf->hauteur <= 0) || (inf->nplans <= 0) )
  {
    fclose( png_stream ) ; png_stream = NULL ;
  ...
  {
    if ( inf->bit_depth == 16 ) png_set_strip_16( png_str ) ;
    if ( inf->type == PNG_COLOR_TYPE_PALETTE ) png_set_palette_to_rgb( png_str ) ;

    /* PNG_COLOR_TYPE_GRAY_ALPHA is always 8 or 16bit depth. */
    if ( (inf->type == PNG_COLOR_TYPE_GRAY) && (inf->bit_depth < 8) ) png_set_expand_gray_1_2_4_to_8( png_str ) ;
    if ( png_get_valid( png_str, png_inf, PNG_INFO_tRNS ) ) png_set_tRNS_to_alpha( png_str ) ;

    /* These color_type don't have an alpha channel then fill it with 0xff. */
    if ( (inf->type == PNG_COLOR_TYPE_RGB) || (inf->type == PNG_COLOR_TYPE_GRAY) || (inf->type == PNG_COLOR_TYPE_PALETTE) )
      png_set_filler( png_str, 0xFF, PNG_FILLER_AFTER ) ;

    if ( (inf->type == PNG_COLOR_TYPE_GRAY) || (inf->type == PNG_COLOR_TYPE_GRAY_ALPHA) )
...
#pragma warn -par
int cpu_png_load(char* name, MFDB* out, GEM_WINDOW* wprog)
{
  png_bytep  row ;
  size_t     len_line ;
  long       all_y ;
  ...
  png_row32 = NULL ; /* Not before as PureC would optimize it and would considr it NULL in setmp routine */
  if ( Force16BitsLoad )
  {
  ...
  for ( pass = 0; pass < number_passes; pass++ )
  {
    ...
    for ( y = 0; y < out->fd_h; y++, yc++ )
    {
      ...
      if ( MAY_UPDATE( wprog, yc ) )
      {
        if ( STOP_CODE( wprog->ProgRange( wprog, yc, all_y, NULL ) ) )
        {
