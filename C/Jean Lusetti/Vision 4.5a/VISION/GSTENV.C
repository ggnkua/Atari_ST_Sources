...
void iconify_picture(GEM_WINDOW *wnd)
{
  ...
  pc  = (float)h/(float)vimage->raster.fd_h ;
  if (pc > pcx) pc = pcx ;
  /* R‚duit la taille de l'image … une icone */
  temp.fd_addr = NULL ;
  temp.fd_w    = (int) ( 0.5 + (float)vimage->raster.fd_w * pc ) ;
  temp.fd_h    = (int) ( 0.5 + (float)vimage->raster.fd_h * pc ) ;
  ...
  nw = (int) ((float)vimage->raster.fd_w*pc) ;
  if (nw > w) nw = w ;
  xy[0] = xy[1] = 0 ;
  xy[2] = nw-1 ;
  xy[3] = temp.fd_h-1 ;
...
void OnVImgDraw(GEM_WINDOW *wnd, int xywh[4])
{
  ...
  if ( vimage->zoom_level == 1 )
  {
    if ( xy[0] + w - 1 > vimage->raster.fd_w )
    {
      w          = 1 + vimage->raster.fd_w - xy[0] ;
      must_paint = 1 ;
    }

    if ( xy[1] + h - 1 > vimage->raster.fd_h )
    {
      h          = 1 + vimage->raster.fd_h - xy[1] ;
      must_paint = 1 ;
    }
  }
