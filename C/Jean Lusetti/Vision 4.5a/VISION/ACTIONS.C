...
int clip_search(void)
{
  ...
  memset( &info, 0, sizeof(INFO_IMAGE) ) ;
...
GEM_WINDOW *nouvelle_wimage(char *nom, int nx, int ny, int nc, int tpmx, int tpmy)
{
  ...
  LoggingDo(LL_DEBUG, "nouvelle_wimage; width=%d, height=%d, nb_plane=%d", vimage->raster.fd_w, vimage->raster.fd_h, vimage->raster.fd_nplanes) ;
...
LDV_STATUS do_ldv(LDV_MODULE *ldv, GEM_WINDOW *gwnd, LDV_PARAMS *ldv_params)
{
  ...
  if ( LDV_SUCCESS( err ) )
  {
    if ( config.ldv_options & LDVO_SHOWPERF )
    {
      ...
      sprintf( mparams.mesg[0], "          LDV Performances :" ) ;
      sprintf( mparams.mesg[2], "[LDV] PreRun           : %.02f sec", (float)ldv_perfs.prerun_ms / 1000.0 ) ;
      sprintf( mparams.mesg[3], "[LDV] Run              : %.02f sec", (float)ldv_perfs.run_ms / 1000.0 ) ;
      sprintf( mparams.mesg[4], "Pre Vision processing  : %.02f sec", (float)ldv_perfs.pre_ldv_ms / 1000.0 ) ;
      sprintf( mparams.mesg[5], "Post Vision processing : %.02f sec", (float)ldv_perfs.post_ldv_ms / 1000.0 ) ;
      sprintf( mparams.mesg[6], "Total time             : %.02f sec", (float)ldv_perfs.total_ms / 1000.0 ) ;
      mparams.ok_button = 1 ;
...
static int conv_img(FR_CDATA* fr_cdata)
{
  ...
  if ( DImgGetIIDFromFile( fr_cdata->filename ) != 0 )
  {
    ...
    if ( !err )
    {
      ...
      if ( (out.fd_nplanes == 24) || (inf.nplans == 24) ) out.fd_nplanes = 32 ; /* RasterZoom can't operate on 24bit */
      ...
      if ( do_it )
      {
        ...
        {
          err = load_picture( fr_cdata->filename, &out, &inf, NULL ) ;
          if ( !err )
          {
            int rw, rh ;
...
/*              sout.fd_w  &= 0xFFF0U ;  Multiple of 16 */
              ...
              else
              {
                ...
                out.fd_addr = NULL ;
                inf.largeur = sout.fd_w ;
                inf.hauteur = sout.fd_h ;
