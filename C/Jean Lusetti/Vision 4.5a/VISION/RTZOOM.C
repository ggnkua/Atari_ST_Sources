...
int init_rtmemory(void)
{
  ...
  if ( RTZoomSrc.fd_addr != NULL )    Xfree( RTZoomSrc.fd_addr ) ;
  if ( RTZoomDst.fd_addr != NULL )    Xfree( RTZoomDst.fd_addr ) ;
  if ( RTZoomSrcStd.fd_addr != NULL ) Xfree( RTZoomSrcStd.fd_addr ) ;
  if ( RTZoomDstStd.fd_addr != NULL ) Xfree( RTZoomDstStd.fd_addr ) ;
...
#pragma warn -par
int OnCloseRTZoom(void *wnd)
{
  if ( RTZoomDstStd.fd_addr != NULL ) Xfree( RTZoomDstStd.fd_addr ) ;
  memset( &RTZoomDstStd, 0, sizeof(MFDB) ) ;
  if ( RTZoomSrcStd.fd_addr != NULL ) Xfree( RTZoomSrcStd.fd_addr ) ;
  memset( &RTZoomSrcStd, 0, sizeof(MFDB) ) ;
  if ( RTZoomDst.fd_addr != NULL ) Xfree( RTZoomDst.fd_addr ) ;
  memset( &RTZoomDst, 0, sizeof(MFDB) ) ;
...
{
  return( GWCreateWindow( ATTR_WRTZOOM, 0, RTZOOM_CLASSNAME ) ) ;
}
...
int init_rtwindow(void)
{
  ...
  if ( WndRTZoom )
  {
    ...
    sprintf( name, MsgTxtGetMsg(hMsg, MSG_TITRERTZOOM) , config.rt_zoomlevel ) ;
...
int rtzoom_init(void)
{
  ...
  deltax     = RTZoomSrc.fd_w / 2 ;
  smart_offx = ( RTZoomDst.fd_w - config.rt_width ) / 2 ;
  deltay     = config.rt_height / config.rt_zoomlevel / 2 ;
...
{
  float per_sec, per_sec1 ;
  char buf[100] ;

  per_sec  = (float)t/(float)nb/(float)CLK_TCK ;
  per_sec  = 1.0 / per_sec ;
  per_sec1 = (float)t1/(float)nb/(float)CLK_TCK ;
  per_sec1 = 1.0 / per_sec1 ;
  sprintf( buf, "[Fast_zoom : %.2f zooms/s (%.2f)][OK]", per_sec, per_sec1 ) ;
  form_stop( 1, buf ) ;
}
