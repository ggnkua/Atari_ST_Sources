...
void ajust_pathconfig(void)
{
  ...
  {
    tmp[0] = 0 ;
    LoggingDo(LL_INFO, "zView LDG path %s does not exist, VISION won't use any zView plugin", path) ;
    strcpy( path, tmp ) ;
  }
...
int OnInitApp(void *ap)
{
  ...
  DImgGetImgModules( config.path_zvldg, NULL, NULL, config.im_priority_global ) ;
