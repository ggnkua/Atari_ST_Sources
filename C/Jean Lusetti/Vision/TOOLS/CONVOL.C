...
typedef struct
{
  char *ConvolDspCode ;    /* Code binaire DSP    */
...
int convolution_loadfilter(char *filtername, CONVOLUTION_FILTRE *filter)
{
  ...
  filter->data = (char *) malloc(filter->w*filter->h) ;
  if (filter->data == NULL)
  {
    fclose(stream) ;
...
void graylevel_init(INFO_IMAGE *info, unsigned char *tgray2index, unsigned char *tindex2gray)
{
  ...
  vdi2tos = get_ptvdi2tos(info->nplans) ;
  for (i = 0; i < 256; i++)
  {
...
void convolution_cpudogray(int w, int no_line, unsigned char *in_data, unsigned char *start_outline)
{
  ...
  for (j = 0; j < w; j++)
  {
    ...
    for (m = 0; m < compute_filter.h; m++)
    {
      ...
      if (line < 0)
      {
        line  = compute_filter.h-1 ;
        pt_in = buffer+(long)line*(long)w ;
      }
    }
    if (shift) gray >>= shift ;
...
int convolution_cpu(MFDB *in, INFO_IMAGE *info, MFDB *out, CONVOLUTION_FILTRE *filter, unsigned char *tgray2index, unsigned char *tindex2gray, GEM_WINDOW *wprog)
{
  ...
  if (in->fd_nplanes == 8)
  {
    ...
    i2r_data  = data_out+compute_filter.w/2 ;
    i2r_nb    = out->fd_w ;
    no_buffer = 0 ;
    index_in  = buffer ;
    for (i = 0; i < in->fd_h+compute_filter.h; i++)
    {
      ...
      {
        pc = (int) ((100L*(long)i)/(long)in->fd_h) ;
...
int convolution_dsp(MFDB *in, INFO_IMAGE *info, MFDB *out, CONVOLUTION_FILTRE *filter, CONVOLUTION_DSP_CODE *cdc, unsigned char *tgray2index, unsigned char *tindex2gray, GEM_WINDOW *wprog)
{
  ...
  if ( cdc->ConvolDspCode ) Dsp_ExecProg( cdc->ConvolDspCode, cdc->ConvolDspCodeSize, cdc->ConvolDspAbility ) ;
  filter_init( filter ) ;
  dspdata_init( in ) ;
  ...
  if ( in->fd_nplanes == 8 )
  {
    ...
    for ( i = 0; i < in->fd_h+compute_filter.h; i++ )
    {
      if ( i < in->fd_h )
      {
        raster2ind( in_data, buffer+compute_filter.w/2, (long)in->fd_w, in->fd_nplanes ) ;
        valc = buffer[in->fd_w-1] ;
        ptc  = buffer+(long)in->fd_w ;
        for ( pc = 0; pc < compute_filter.w/2; pc++ ) *ptc++ = valc ;
        index2graylevel( buffer+compute_filter.w/2, in->fd_w, tindex2gray ) ;
      }
...
int convolution(MFDB *in, INFO_IMAGE *info, MFDB *out, char *DSPfile, CONVOLUTION_FILTRE *filter, GEM_WINDOW *wprog)
{
  ...
  memset( &cdc, 0, sizeof(CONVOLUTION_DSP_CODE) ) ;
  ...
  if ( DSPfile[0] && DspInfo.use )
  {
    if ( ConvolDspAbility != Dsp_GetProgAbility() )
    {
      ...
      cdc.ConvolDspCode = malloc( 1000 ) ;
      if ( cdc.ConvolDspCode == NULL ) return( CONVOL_MEMORY ) ;
      ConvolDspAbility      = Dsp_RequestUniqueAbility() ;
      cdc.ConvolDspCodeSize = Dsp_LodToBinary( DSPfile, cdc.ConvolDspCode ) ;
      if ( cdc.ConvolDspCodeSize <= 0 )
      {
        free( cdc.ConvolDspCode ) ;
        return( CONVOL_BADDSPFILE ) ;
      }
    }
  ...
  if ( in->fd_nplanes == 8 )
  {
    buffer = (unsigned char *) malloc( (in->fd_w+filter->w)*filter->h ) ;
    if ( buffer == NULL )
    {
      free( out->fd_addr ) ;
      return( CONVOL_MEMORY ) ;
    }
    data_out = (unsigned char *) malloc( in->fd_w+filter->w ) ;
    if ( data_out == NULL )
    {
      free( buffer ) ;
      free( out->fd_addr ) ;
  ...
  if ( in->fd_nplanes >= 16 )
  {
    int_buffer = (int *) malloc( (in->fd_w+filter->w)*4L ) ;
    if ( int_buffer == NULL )
    {
      free( out->fd_addr ) ;
      return( CONVOL_MEMORY ) ;
  ...
  else
  {
    ...
    if ( err == CONVOL_DSPBUSY )
    {
      switch( DspInfo.WhatToDoIfLocked )
      {
        case DSPLOCK_IGNORE : err = convolution_cpu( in, info, out, filter, tgray2index, tindex2gray, wprog ) ;
                              break ;
      }
    }
  }
  ...
  if ( cdc.ConvolDspCode ) free( cdc.ConvolDspCode ) ;
  if ( err ) free( out->fd_addr ) ;
  if ( buffer ) free( buffer ) ;
  if ( data_out ) free( data_out ) ;
  if ( int_buffer ) free( int_buffer ) ;
