...
int lzw_init(int ticode, unsigned char *start, int flag)
{
  ...
  {
    oldcode = (int *) calloc(TABLE_SIZE, sizeof(int)) ;
    if (oldcode == NULL) return(-3) ;
    currentcode = (int *) calloc(TABLE_SIZE, sizeof(int)) ;
    if (currentcode == NULL)
    {
      free(oldcode) ;
      oldcode = NULL ;
    ...
    newcode = (int *) calloc(TABLE_SIZE, sizeof(int)) ;
    if (newcode == NULL)
    {
      free(currentcode) ;
      currentcode = NULL ;
  ...
  {
    sizeofstring = (int *) calloc(MAXN, sizeof(int)) ;
    if (sizeofstring == NULL) return(-3) ;
    dstack = (unsigned char *) calloc(MAXN, sizeof(char)) ;
    if (dstack == NULL)
    {
      free(sizeofstring) ;
      sizeofstring = NULL ;
    ...
    suffix = (unsigned char *) calloc(MAXN, sizeof(char)) ;
    if (suffix == NULL)
    {
      free(dstack) ;
      dstack = NULL ;
    ...
    prefix = (unsigned int *) calloc(MAXN, sizeof(int)) ;
    if (prefix == NULL)
    {
      free(suffix) ;
      suffix = NULL ;
...
void lzw_end(long *nb_out)
{
  if (operation == LZW_COMPRESS)
  {
    ...
    *nb_out     = *nb_out+block_count ;
    ...
    newcode = NULL ;
    if (currentcode != NULL) free(currentcode) ;
    currentcode = NULL ;
  ...
  else
  {
    ...
    suffix = NULL ;
    if (dstack != NULL) free(dstack) ;
    dstack = NULL ;
    if (sizeofstring != NULL) free(sizeofstring) ;
    sizeofstring = NULL ;
  }
}
