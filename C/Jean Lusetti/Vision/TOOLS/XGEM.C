...
{
  if(choix_rsrc==1) rsrc_obfix( re_otree, re_oobject );
...
int gem_init(void)
{
  ...
  path[0] = 'A' + Dgetdrv() ;
  Dgetpath( &path[2], 0 ) ;
  init_path = (char *) malloc( 1 + strlen( path ) ) ;
  ...
  if ( !Truecolor )
  {
    if ( nb_plane < 4 ) old_pal = (int *) calloc( 16, 3 * sizeof(int) ) ;
    else                old_pal = (int *) calloc( 1 << nb_plane, 3 * sizeof(int) ) ;
...
OBJECT *popup_make(int nb_objets, int nb_cara)
{
  ...
  if ( pop_up )
  {
    ...
    pop_up[1].ob_spec.free_string = calloc( 1+nb_cara, sizeof(char) ) ;
    pop_up[1].ob_x                = 0 ;
    pop_up[1].ob_y                = 0 ;
    ...
    if ( nb_objets > 1 )
    {
      for (i = 2; i <= nb_objets; i++)
      {
        ...
        pop_up[i].ob_spec.free_string = calloc( 1+nb_cara, sizeof(char) ) ;
        pop_up[i].ob_x                = 0 ;
...
void popup_kill(OBJECT *pop_up, int nb_objets)
{
  ...
  for ( i = 1; i <= nb_objets; i++ )
    if ( pop_up[i].ob_spec.free_string ) free( pop_up[i].ob_spec.free_string ) ;
...
int *cycling_init(void)
{
  ...
  save_pal = (int *) malloc( 3 * nb_colors * sizeof(int) ) ;
...
{ /* Par Eric REBOUX */
	...
	if ((new = (OBJECT *) malloc(size + strsize)) != 0)
	{
