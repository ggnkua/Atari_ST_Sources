...
LIB_ID        ImgLibPriorities[2] = { { P_ME, 0, 0 }, { P_LDV, 0, 0 } } ;
/*LIB_ID        ImgLibPriorities[1] = { { P_LDV, 0, 0 } } ;*/
...
void smart_fit(MFDB *in, MFDB *out, int *xy)
{
  ...
  lo_src = in->fd_nplanes*in->fd_wdwidth ;   /* En mots */
  lo_dst = out->fd_nplanes*out->fd_wdwidth ; /* En mots */
  src    = (int *) in->fd_addr ;
...
int img_extend(MFDB *img)
{
  ...
  temp.fd_addr    = img_alloc( temp.fd_w, temp.fd_h, img->fd_nplanes ) ;
...
{
  if ( lib_ids ) memcpy( lib_ids, ImgLibPriorities, sizeof(ImgLibPriorities)  ) ;
  return( (int) (sizeof(ImgLibPriorities)/sizeof(ImgLibPriorities[0])) ) ;
}

void SetLibPriorities(LIB_ID *lib_ids, int nb_libs)
{
  size_t nb_bytes = nb_libs * sizeof(LIB_ID) ;

  if ( nb_bytes > sizeof(ImgLibPriorities) ) nb_bytes = sizeof(ImgLibPriorities) ;
  memcpy( ImgLibPriorities, lib_ids, nb_bytes ) ;
}
...
IID img_getstdformat(char *nom, INFO_IMAGE* inf)
{
  ...
  img_module = DImgGetModuleForExtension( ext ) ;
  if ( img_module ) lfmt = img_module->Capabilities.iid ;

  if ( inf ) inf->lformat = lfmt ;
...
{
  IMG_MODULE *img_module = DImgGetModuleFromIID( lformat ) ;

  if ( img_module ) return( img_module->Capabilities.file_extension ) ;

  return( NULL ) ;
}

int ldv_img_format(char *nom, INFO_IMAGE *inf)
{
  IMG_MODULE* img_module ;
  IID         lfmt ;
  int         ret ;
  ...
  DImgGetImgModules( NULL, NULL, NULL, 0 ) ; /* Juste pour l'initialiser */

  if ( get_ext( nom, ext ) == 1 ) img_module = DImgGetModuleForExtension( ext ) ;
  else                            img_module = DImgGetModuleFromIID( inf->lformat ) ;
  if ( img_module == NULL ) return( -5 ) ;

  inf->lformat = img_module->Capabilities.iid ;
  ret          = img_module->Identify( nom, inf ) ;
  lfmt         = inf->lformat ;
  ...
  if ( ret == 0 )
  {
    ...
    switch( inf->nplans )
    {
      case 1  : if ( caps->inport1 == 0 ) ret = -6 ;
                break ;

      case 2  : if ( caps->inport2 == 0 ) ret = -6 ;
                break ;

      case 4  : if ( caps->inport4 == 0 ) ret = -6 ;
                break ;

      case 8  : if ( caps->inport8 == 0 ) ret = -6 ;
                break ;

      case 16 : if ( caps->inport16 == 0 ) ret = -6 ;
                break ;

      case 24 : if ( caps->inport24 == 0 ) ret = -6 ;
                break ;

      case 32 : if ( caps->inport32 == 0 ) ret = -6 ;
                break ;
...
int cdecl me_img_format(char *nom, INFO_IMAGE *inf)
{
  ...
  if ( load_me_ldg() == 0 )
  {
    ...
    if ( ret >= 0 )
    {
      inf->spec = malloc( sizeof(MEIMG) ) ;
      if ( inf->spec )
      {
        if ( meimg.size_DDpalette ) meimg.buff_DDpalette = malloc( meimg.size_DDpalette ) ;
        meimg.buff_DDimg = malloc( meimg.size_DDimg ) ;
...
int img_format(char *nom, INFO_IMAGE *inf)
{
  ...
  if ( !FileExist( nom ) ) return( -1 ) ;

  memset( inf, 0, sizeof(INFO_IMAGE) ) ;
  strcpy( inf->nom, nom ) ;
  inf->lformat = lfmt ;

  for ( i = 0 ; ret && (i < (int) (sizeof(ImgLibPriorities)/sizeof(LIB_ID))); i++ )
  {
    if ( ImgLibPriorities[i].id == P_LDV ) ret = ldv_img_format( nom, inf ) ;
    else                                   ret = me_img_format( nom, inf ) ;
  }

  if ( ret == 0 ) make_safepalette( inf ) ;
...
int fit_to_planes(MFDB *img, INFO_IMAGE *inf, int out_planes, GEM_WINDOW *wprog)
{
  ...
  if ( inf->nplans > out_planes )
  {
    ...
    {
      inf->palette = malloc( 6*nb_colors ) ;
  ...
  if ( ( inf->nplans < out_planes ) && ( out_planes > 8 ) )
  {
    ...
    StdProgWText( wprog, 0, msg[MSG_TRSFFMTSTD] ) ;
...
int ldv_load_picture(char *name, MFDB *out, INFO_IMAGE *inf, GEM_WINDOW *wprog)
{
  IMG_MODULE* img_module ;
  MFDB        std_img ;
  int         out_planes ;
  ...
  if ( UseStdVDI && ( out_planes <= 8 ) )
  {
    ...
    std_img.fd_addr = img_alloc( inf->largeur, inf->hauteur, out_planes ) ;
  ...
  if ( img_module ) ret = img_module->Load( name, out, wprog ) ;
  else              ret = -5 ;

  StdProgWText( wprog, 1, "" ) ;
  StdProgDisplayDSP( wprog, 0 ) ;
  StdProgDisplay030( wprog, 0 ) ;
  /* Agrandit … la taille minimale si n‚cessaire */
  if ( ret == 0 ) ret = img_extend( out ) ;

  perf_start( PERF_DITHER, &PerfInfo ) ;
  if ( ret == 0 ) ret = fit_to_planes( out, inf, out_planes, wprog ) ; /* Augmente le nombre de plans ou trame        */
  ...
  if ( UseStdVDI && ( ret == 0 ) && !CancelStdVDI )
  {
    perf_start( PERF_STDFORMAT, &PerfInfo ) ;
    if ( out_planes <= 8 )
    {
...
int load_picture(char *name, MFDB *out, INFO_IMAGE *inf, GEM_WINDOW *wprog)
{
  ...
  if ( inf->lformat == IID_ME ) ret = me_load_picture( out, inf ) ;
  else                          ret = ldv_load_picture( name, out, inf, wprog ) ;
...
int save_picture(char *name, MFDB *img, INFO_IMAGE *inf, GEM_WINDOW *wprog)
{
  ...
  if ( img_module ) ret = img_module->Save( name, img,inf,  wprog ) ;
  else              ret = -5 ;
...
int set_greylevel(int nplanes, INFO_IMAGE *inf)
{
  ...
  inf->palette = NULL ;
/*
  if ((nplanes < 8) && (inf_in->palette != NULL))
  {
    min = 1000 ; max = 0 ;
    pt  = inf_in->palette ;
    for (i = 0; i < inf_in->nb_cpal; i++)
    {
      val  = *pt++ ;
      val += *pt++ ;
      val += *pt++ ;
...
int get_imgdrivercapabilities(long lfmt, INFO_IMAGE_DRIVER *cap)
{
  ...
  if ( img_module ) img_module->GetDrvCaps( cap ) ;
  else              ret = -5 ;
...
int compute_histo(MFDB *img, INFO_IMAGE *inf, HISTO *histo, GEM_WINDOW *wprog)
{
  ...
  std_data = (unsigned char *) malloc( 16+img->fd_w ) ;
  if ( std_data == NULL )
  {
    vr_trnfm( handle, &std_img, img ) ;
    free( std_img.fd_addr ) ;
  ...
  else if (img->fd_nplanes == 16)
  {
    ...
    occup_couleurs = (unsigned char *) calloc( 65536UL, sizeof(unsigned char) ) ;
  ...
  else if (img->fd_nplanes >= 24)
  {
    ...
    occup_couleurs = (unsigned char *) calloc( size, sizeof(unsigned char) ) ;
    if ( occup_couleurs == NULL )
    {
      free( std_data ) ;
      return(-3) ;
...
int std_fct_cntload(void)
{
  ...
  if ( which & MU_KEYBD )
    if ( event.ev_mkreturn == TCTRL_C ) cont = 0 ;
