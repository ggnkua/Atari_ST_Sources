...
int bmp_identify(char *nom, INFO_IMAGE *inf)
{
  ...
  if (bmp_entete.type != BMP_ID) return(-2) ;

  if (bmp_entete.planes != 1) return(-6) ;
  inf->version  = 0L ; /* Sans importance */
  inf->compress = (int) bmp_entete.compression ;
  if ( inf->compress != 0 ) return( -6 ) ;
  ...
  if ((inf->nplans != 1) && (inf->nplans != 4) &&
      (inf->nplans != 8) && (inf->nplans != 24)) return(-6) ;
  if (bmp_entete.xpix_meter > 0)
    inf->lpix = (int) (1000000.0/(float)bmp_entete.xpix_meter) ;
  else
    inf->lpix = 0x152 ;
  if (bmp_entete.ypix_meter > 0)
    inf->hpix = (int) (1000000.0/(float)bmp_entete.ypix_meter) ;
  else
    inf->hpix = 0x152 ;
  if (inf->nplans <= 8)
  {
    ...
    inf->palette = (int *) malloc(6*inf->nb_cpal) ;
    if (inf->palette == NULL)
    {
      fclose(stream) ;
      return(-3) ;
...
int bmp_read(FILE *stream, MFDB *out, GEM_WINDOW *wprog)
{
  ...
  i2r_data = malloc(nb_octets) ;
  if (i2r_data == NULL) return(-1) ;
...
int bmp_sauve(char *name, MFDB *img, INFO_IMAGE *info, GEM_WINDOW *wprog)
{
  ...
  if (info->nplans == 24)
  {
    ...
    b24 = (unsigned char *) malloc(nb_bytes_per_sample*(3+img->fd_w)) ;
    if (b24 == NULL)
    {
      fclose(stream) ;
      return(-3) ;
  ...
  else
  {
    ...
    buffer = (unsigned char *) malloc(3+img->fd_w) ;
    if (buffer == NULL)
    {
      fclose(stream) ;
      return(-3) ;
