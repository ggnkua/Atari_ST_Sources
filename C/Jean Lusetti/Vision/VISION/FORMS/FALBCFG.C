...
int OnTxtBubbleAlbCfg(void *w, int mx, int my, char *txt)
{
  ...
  switch( objet )
  {
    case ACONF_VIS2MASK  : strcpy(txt, msg[MSG_HACONFV2M]) ;
                           break ;
    case ACONF_MASK2VIS  : strcpy(txt, msg[MSG_HACONFM2V]) ;
                           break ;
    case ACONF_TSAVEINDEX:
    case ACONF_SAVEINDEX : strcpy(txt, msg[MSG_HACONFINDEX]) ;
                           break ;
    case ACONF_SHIFTST   :
    case ACONF_SHIFTEND  : strcpy(txt, msg[MSG_HACONFSHIFT]) ;
                           break ;
    default              : trouve = 0 ;
  }
  if ((objet >= ACONF_VISIBLE1) && (objet <= ACONF_VISIBLE6))
  {
    strcpy(txt, msg[MSG_HACONFVIS]) ;
    trouve = 1 ;
  }
  if ((objet >= ACONF_MASKED1) && (objet <= ACONF_MASKED6))
  {
    strcpy(txt, msg[MSG_HACONFMASK]) ;
    trouve = 1 ;
  }
...
int album_config(void)
{
  ...
  if ( code == IDOK )
  {
    ...
    catalog_setsaveindex( wext.save_index ) ;
