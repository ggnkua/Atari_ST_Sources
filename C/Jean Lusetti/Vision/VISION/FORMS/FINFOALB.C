...
{
  CATALOG_STATS stats ;
  OBJECT *popup_categorie ; /* Cree par l'appelant */
}
WEXTENSION_ALBINFO ;
...
void lstd_input(char *text, char *buf)
{
  ...
  d = NULL ;
  c = strchr( 1+msg[MSG_FILENOTEXIST], '[' ) ;
...
void OnInitDialogAlbInfo(void *w)
{
...
int OnObjectNotifyAlbInfo(void *w, int obj)
{
  GEM_WINDOW   *wnd = (GEM_WINDOW *) w ;
  WEXTENSION_ALBINFO *wext = wnd->DlgData->UserData ;
  OBJECT       *adr_infos = wnd->DlgData->BaseObject ;
  ...
  switch( obj )
  {
    case ALBINF_DEFCAT : deselect(adr_infos, obj) ;
                         ...
                         {
                           lstd_input(msg[MSG_CATENEW], buf) ;
                           if ( buf[0] )
                           {
                             char tmp[200] ;

                             catalog_header.default_categorie = catalog_addcategory(buf) ;
                             wext->popup_categorie = make_category_popup( wnd ) ;
                             sprintf( tmp, "  %s", buf ) ;
                             write_text( adr_infos, obj, tmp ) ;
                           }
                         }
                         xobjc_draw( wnd->window_handle, adr_infos, ALBINF_DEFCAT ) ;
    case ALBINF_RDEFCAT:
    case ALBINF_RTDEFCAT:
                         select( adr_infos, ALBINF_RDEFCAT ) ;
                         xobjc_draw( wnd->window_handle, adr_infos, ALBINF_RDEFCAT ) ;
                         deselect( adr_infos, ALBINF_RINICAT ) ;
                         xobjc_draw( wnd->window_handle, adr_infos, ALBINF_RINICAT ) ;
                         break ;
    ...
    case ALBINF_INICAT : buf[0] = 0 ;
                         ...
                         xobjc_draw( wnd->window_handle, adr_infos, ALBINF_INICAT ) ;
    case ALBINF_RINICAT:
...
int show_infos(OBJECT *popup_categorie)
{
