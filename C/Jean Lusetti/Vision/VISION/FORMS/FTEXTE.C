...
void show_fonts(GEM_WINDOW *wnd, int start_num)
{
  ...
  if (num_fonts > 6)
    obj[TEXTE_FONTVUES].ob_y = (l*(start_num-num_sys_font))/(1+num_fonts-7) ;
  else
    obj[TEXTE_FONTVUES].ob_y = 0 ;
...
int cdecl draw_text(PARMBLK *paramblk)
{
  ...
  vsf_color(handle, 0) ;
  xt = paramblk->pb_x+paramblk->pb_w/2 ;
  yt = paramblk->pb_y+paramblk->pb_h/2 ;
  v_hide_c(handle) ;
  vr_recfl(handle, xy) ;
...
void OnInitDialogText(void *w)
{
  ...
  dlg_data->tsort_fonts = (int *) malloc( 2 * (1 + num_fonts) ) ;
  ...
  write_text(adr_ptext, TEXTE_FONT1, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT2, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT3, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT4, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT5, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT6, "                    ") ;
  write_text(adr_ptext, TEXTE_FONT7, "                    ") ;
...
int OnObjectNotifyText(void *w, int obj)
{
  GEM_WINDOW *wnd = w ;
  OBJECT    *adr_ptext = wnd->DlgData->BaseObject ;
  DLGTEXTE_USER_DATA *dlg_data = wnd->DlgData->UserData ;
