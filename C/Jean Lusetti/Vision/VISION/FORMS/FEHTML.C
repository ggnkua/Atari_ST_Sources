...
int OnObjectNotifyEHTML(void *w, int obj)
{
  ...
  switch( obj )
  {
     ...
     case EHTML_SAVE      : slash = strrchr( catalog_name, '\\' ) ;
                            if ( slash ) strcpy( sug_name, 1 + slash ) ;
                            else         strcpy( sug_name, catalog_name ) ;
                            ...
                            if ( file_name( buf, sug_name, filename ) == 1 )
                            {
                              ...
                              if ( slash )
                              {
                                ...
                                if ( point ) *point = 0 ;
                                memset( wext->exp->base_name, 0, sizeof(wext->exp->base_name) ) ;
...
void HTMLCatalogExport(long first, long last)
{
  GEM_WINDOW   *wprog ;
  CHTML_EXPORT exp ;
  char         buf[100] ;

  memset( &exp, 0, sizeof(CHTML_EXPORT) ) ;

  if ( FGetHTMLExport( &exp ) == IDOK )
  {
    sprintf( buf, msg[MSG_EHTML], exp.page_title ) ;
