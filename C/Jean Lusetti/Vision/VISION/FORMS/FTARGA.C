...
void OnInitDialogTarga(void *w)
{
  ...
  if ( wext->inf->nplans == 16 ) select( adr_targa, TARGA_16BITS ) ;
...
int OnObjectNotifyTarga(void *w, int obj)
{
  ...
  if ( code == IDOK )
  {
    if (selected(adr_targa, TARGA_16BITS)) wext->inf->nplans = 16 ;
    else                                   wext->inf->nplans = 24 ;

    if (selected(adr_targa, TARGA_FLIPFLOP)) wext->inf->flipflop = FLIPFLOP_Y ;
    else                                     wext->inf->flipflop = 0 ;
  }
