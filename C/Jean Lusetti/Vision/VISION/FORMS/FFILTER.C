...
int GetFilterPathForLanguage(char *path)
{
  ...
  if ( path_filter[0] == 0 )
  {
    ...
    nb_filtres = (int)count_files( path_filter, "*.VCF", 0, NULL ) ;
    if ( nb_filtres <= 0 )
    {
...
{
  if ( tab_filtre != NULL ) free( tab_filtre ) ;
...
void get_filterinfos(char *path, CONVOLUTION_FILTRE *tab, int nb_filtres)
{
  ...
  wprog = DisplayStdProg( msg[MSG_LOOKFILTERS], "", "", CLOSER ) ;
...
int reload_filters(void)
{
  ...
  if ( nb_filtres > 0 )
  {
    ...
    names = NULL ;
    if ( tab_filtre != NULL ) free( tab_filtre ) ;
    tab_filtre = (CONVOLUTION_FILTRE *) malloc(nb_filtres*sizeof(CONVOLUTION_FILTRE)) ;
    if (tab_filtre == NULL)
    {
      mouse_restore() ;
      form_error(8) ;
    ...
    names = (char * ) malloc( NB_MAX_CAR_NOM_FILTRE*nb_filtres ) ;
    if (names == NULL)
    {
      free(tab_filtre) ;
      tab_filtre = NULL ;
