...
void InitLDVList(GEM_WINDOW *wnd, char *path_ldv)
{
  ...
  if ( LDVList ) free( LDVList ) ;
  LDVList = NULL ;
  if ( TotalNbLDV > 0 )
  {
    LDVList = (LDV_MODULE *) calloc( 1 + TotalNbLDV, sizeof(LDV_MODULE) ) ;
    if ( LDVList )
    {
...
static void OnInitDialog(void *w)
{
  ...
  if ( !DisplayAllLDV ) select( bobj, LDV_COMPAT ) ;
...
#ifdef SML_LDVLIST
  if ( TotalLDVList ) free( TotalLDVList ) ;
#else
