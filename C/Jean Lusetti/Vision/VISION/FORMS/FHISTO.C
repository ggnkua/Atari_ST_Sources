...
void OnInitDialogHisto(void *w)
{
  ...
  if (vimage->raster.fd_nplanes <= 8)
  {
    sprintf(buf, "%d", (1 << vimage->raster.fd_nplanes)-1) ;
  ...
  if ( wext->histo.nb_colors_used > 65000L )
    ...
    sprintf( buf, "%ld", wext->histo.nb_colors_used ) ;
...
void traite_histo(GEM_WINDOW *gwnd)
{
  ...
  wprog = DisplayStdProg( msg[MSG_HISTO], "", "", CLOSER ) ;
  mouse_busy() ;
  wext.hlarg = (int) ((float)adr_histo[HISTO_ROUGE].ob_width/(float)(1 << vimage->raster.fd_nplanes)) ;
  if ( compute_histo(&vimage->raster, inf_img, &wext.histo, wprog ) != 0 )
  {
    mouse_restore() ;
