...
void traite_convolution(GEM_WINDOW *gwnd)
{
  ...
  if (choix == 1)
  {
    memcpy(&virtuel, &vimage->raster, sizeof(MFDB)) ;
    undo_return = undo_put( gwnd, 0, 0, vimage->raster.fd_w-1, vimage->raster.fd_h-1, UFILTRE, NULL ) ;
  }
  else
  {
    ...
    else
    {
      ...
      if (x1 < 0) x1 = 0 ;
      if (y1 < 0) y1 = 0 ;
      if (x2 > vimage->raster.fd_w-1) x2 = vimage->raster.fd_w-1 ;
      if (y2 > vimage->raster.fd_h-1) y2 = vimage->raster.fd_h-1 ;
  
      virtuel.fd_w       = 1+x2-x1 ;
      virtuel.fd_h       = 1+y2-y1 ;
      virtuel.fd_wdwidth = virtuel.fd_w/16 ;
      if (virtuel.fd_w % 16) virtuel.fd_wdwidth++ ;
      virtuel.fd_nplanes = vimage->raster.fd_nplanes ;
      virtuel.fd_stand   = vimage->raster.fd_stand ;
      ...
      vro_cpyfm(handle, S_ONLY, xy, &vimage->raster, &virtuel) ;
    }
  ...
  do
  {
    wprog = DisplayStdProg( msg[MSG_FILTRE], "", "", CLOSER ) ;
    ...
    if ((ret == CONVOL_DSPBUSY) && (config.dsphandlelock == DSPLOCK_WARN))
    {
      ...
      rep = form_interrogation(1, msg[MSG_DSPLOCK]) ;
      switch(rep)
      {
        case 1 : /* Forcer */
  ...
  if (ret == 0)
  {
    if (choix == 2)
    {
      ...
      if (vclip.form == BLOC_RECTANGLE)
        vro_cpyfm(handle, S_ONLY, xy, &out, &vimage->raster) ;
      else
      {
        int xyarray[8] ;
    ...
    {
      free(vimage->raster.fd_addr) ;
      vimage->raster.fd_addr = out.fd_addr ;
    }
    set_modif( gwnd ) ;
  ...
  else
  {
    switch(ret)
    {
      case CONVOL_MEMORY        : form_error(8) ;
                                  ...
                                  break ;
      case CONVOL_BADFILTERFILE : form_stop(1, msg[MSG_BADFILTER]) ;
                                  break ;
      case CONVOL_BADNBCOLORS   : form_stop(1, msg[MSG_FNBCOLORS]) ;
                                  break ;
      case CONVOL_BADDSPFILE    : form_stop(1, msg[MSG_BADLOD]) ;
                                  break ;
      case CONVOL_DSPBUSY       : form_stop(1, msg[MSG_DSPISLOCKED]) ;
                                  break ;
      case CONVOL_LODNOTEXIST   : form_stop(1, msg[MSG_DSPNOLOD]) ;
                                  break ;
    }
  }
  mouse_restore() ;

  if (filtre.data != NULL) free(filtre.data) ;
}
