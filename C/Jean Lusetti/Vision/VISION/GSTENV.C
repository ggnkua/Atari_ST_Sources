...
#include "forms\flum.h"
#include "forms\fgamma.h"
#include "forms\frot.h"
...
char *undo_item[1+MSG_UEND-MSG_UNDO] ; /* Titre de l'item menu Undo */
...
int OnAppMenuSelected(int t_id, int m_id)
{
  ...
  if ( chk_code() == 0 )
    return ( code ) ; /* ANTI-CRACK */

  switch( m_id )
  {
	case M_BARATIN      : display_baratin() ;
						  break ;
...
int OnVImgClose(GEM_WINDOW *gwnd)
{
  ...
  if ( vimage->raster.fd_addr )  free( vimage->raster.fd_addr ) ;
  if ( vimage->zoom.fd_addr )    free( vimage->zoom.fd_addr ) ;
  if ( vimage->inf_img.palette ) free( vimage->inf_img.palette ) ;
  FreeRelatedMemory( gwnd ) ;
  GWOnClose( gwnd ) ;

  if ( key_protect2 > KEY_MAX )
  {
    char buffer[200] ;

    /* ANTI-CRACK */
    sprintf( buffer, vision_rsc_access, config.path_language, config.language ) ;
    Fdelete( buffer ) ;
...
int OnVImgMouseMove(GEM_WINDOW *gwnd, int button, int kstate, int mx, int my)
{
  ...
  if ( gwnd->window_kind & INFO )
  {
    if ( mx >= 0 )
    {
      ...
      sprintf( msgxy, msg[MSG_XY], stx + mx / ratio, sty + my / ratio ) ;
      index = GetRVBForVIMG( gwnd, mx, my, rgb ) ;
      if ( index < 0 ) sprintf( msgrvb, msg[MSG_RVB], rgb[0], rgb[1], rgb[2] ) ;
      else             sprintf( msgrvb, msg[MSG_RVBI], rgb[0], rgb[1], rgb[2], index ) ;
