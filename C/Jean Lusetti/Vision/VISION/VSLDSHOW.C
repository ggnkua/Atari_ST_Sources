...
int add_cmd( VSS_INFO *vss_info,  int cmd_id, char *params )
{
  ...
  new_cmd = (CMD *) calloc( 1, sizeof(CMD) ) ;
  if ( new_cmd == NULL )
  ...
  if ( params != NULL )
  {
    new_cmd->params = (char *) malloc( 1+strlen( params ) ) ;
    if ( new_cmd->params == NULL )
    {
      free( new_cmd ) ;
      return( VSSERR_MEMORY ) ;
    }
    strcpy( new_cmd->params, params ) ;
  }

  current_cmd = vss_info->cmds ;
  if ( current_cmd != NULL )
  {
    while ( current_cmd->next_cmd != NULL ) current_cmd = current_cmd->next_cmd ;

    current_cmd->next_cmd = new_cmd ;
  }
  else
    vss_info->cmds = new_cmd ;
