...
void load_wpicture(char *name, long lformat)
{
  ...
  int        w, h ;
  int        ret ;
  int        force_bug_bad_key = 0 ;

  if ( IsRegistered )
  {
    if ( KeyDecrypt( ident.crypted_key ) < KEY_MIN ) force_bug_bad_key = 1 ; /* ANTI-CRACK */
  }

  if ( !FileExist(name) || force_bug_bad_key )
  {
  ...
  if ( gwnd )
  {
    ...
    if ( ret != 0 )
    {
      ...
      switch( ret )
      {
        case -1  : form_stop(1, msg[MSG_FILENOTEXIST]) ;
                   break ;
        case -2  : form_stop(1, msg[MSG_ERRFORMAT]) ;
                   break ;
        case -3  : form_error(8) ;
                   break ;
        case -4  : form_stop(1, msg[MSG_DECOMPRESERR]) ;
                   break ;
        case -5  : form_stop(1, msg[MSG_UNKNOWNFMT]) ;
                   break ;
        case -6  : form_stop(1, msg[MSG_FMTNOSUPPORT]) ;
                   break ;
        case -7  : form_stop(1, msg[MSG_UNKNOWNCOMP]) ;
                   break ;
        case -8  : /* Annulation par Control C */
                   break ;
        case -9  : form_stop(1, msg[MSG_NOJPEGDRIVER]) ;
                   break ;
    ...
    {
      if (form_interrogation(1, msg[MSG_RESOLSCREEN]) == 2)
      {
        if ( inf.palette ) free( inf.palette ) ;
        return ;
      }
    ...
    wprog = DisplayStdProg( msg[MSG_LOADINGIMG], "", name, 0 ) ;
    mouse_busy() ;
    ...
    else
    {
      switch(ret)
      {
        ...
        case -5 : form_stop(1, msg[MSG_UNKNOWNFMT]) ;
                  break ;
        case -9 : form_stop(1, msg[MSG_DSPISLOCKED]) ;
                  break ;
      }
    }
  }
...
void traite_slideshow(char *name)
{
  ...
  switch( err )
  {
    case VSSERR_NOERROR    : vss_doslideshow( &vss_info ) ;
                             break ;
    case VSSERR_FILE       : form_stop(1, msg[MSG_FILENOTEXIST]) ;
                             break ;
    case VSSERR_MEMORY     : form_error( 8 ) ;
                             break ;
    case VSSERR_CMDUNKNOWN : sprintf( buf, msg[MSG_VSSERRCMDU], vss_info.line_err ) ;
                             form_stop( 1, buf ) ;
                             break ;
  }
...
void traite_sauves(GEM_WINDOW *gwnd, int flag)
{
  ...
  if ( gwnd )
  {
    ...
    if ( img_module == NULL )
    {
      form_stop( 1,msg[MSG_UNKNOWNSAVE] ) ;
      return ;
    }
	...
	if ( valid )
	{
      ...
      if ( ck_code[8] != 0x00 ) vimage = NULL ; /* ANTI-CRACK */
      ...
      for ( y = 0; y < dcp.nb_lignes; y++ )
      {
        ...
        for ( x = 0; x < dcp.nb_colonnes; x++ )
        {
          ...
          if ( r != 2 )
          {
            ...
            wprog = DisplayStdProg( msg[MSG_SAVINGIMG], "", name, CLOSER ) ;
            res = save_picture( name, &raster, &inf, wprog ) ;
            GWDestroyWindow( wprog ) ;
            switch( res )
            {
              case -1 : form_stop(1,msg[MSG_WRITEERROR]) ;
                        return ;
              case -2 : form_stop(1,msg[MSG_WRITEERROR]) ;
                        return ;
              case -3 : form_error(8) ;
                        return ;
              case -4 : form_stop(1,msg[MSG_ERRIMGFMT]) ;
                        return ;
              case -5 : form_stop(1,msg[MSG_UNKNOWNSAVE]) ;
                        return ;
            }
          }
      ...
      if ( dcp.valid ) free( raster.fd_addr ) ;
	}
  }
}
