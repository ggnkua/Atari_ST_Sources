...
{
  int  cursorkeys[] = { CURSOR_UP, CURSOR_LSUP, CURSOR_RSUP, CURSOR_DN, CURSOR_LSDN, CURSOR_RSDN, CURSOR_RT, CURSOR_LSRT, CURSOR_RSRT, CURSOR_PUP, CURSOR_PDN, CURSOR_LT, CURSOR_LSLT, CURSOR_RSLT, 0 } ;
...
int GeneralKbdHandler(int key)
{
  ...
  if ( cur_window )
  {
    if ( cur_window->OnKeyPressed ) code = cur_window->OnKeyPressed( cur_window, key ) ;

    if ( (code == GW_EVTCONTINUEROUTING) && cur_window->OnArrow && is_cursor_key( key ) )
    {
...
{
  wind_set( wnd->window_handle, WF_HSLIDE, GWSlidPos(pos) ) ;
}

void GWSetVSlider(GEM_WINDOW* wnd, int pos)
{
  wind_set( wnd->window_handle, WF_VSLIDE, GWSlidPos(pos) ) ;
}

void GWSetHSliderSize(GEM_WINDOW* wnd, int pos)
{
  wind_set( wnd->window_handle, WF_HSLSIZE, GWSlidPos(pos) ) ;
}

void GWSetVSliderSize(GEM_WINDOW* wnd, int pos)
{
  wind_set( wnd->window_handle, WF_VSLSIZE, GWSlidPos(pos) ) ;
}
