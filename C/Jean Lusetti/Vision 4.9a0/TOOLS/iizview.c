...
static int iizview_Load(IMG_MODULE* ImgModule, INFO_IMAGE* inf)
{
  ...
  if ( ret == EIMG_SUCCESS )
  {
    ...
    short          y ;
    unsigned long  status_ok = 1 ;
    int            code ;

    for ( y = 0 ; status_ok && (y < out->fd_h); y++ )
    {
      /* Get data line from plugin */
      status_ok = zvdata->reader_read( ImgModule, zvinf, ibuffer ) ;
      /* And fill out MFDB from it */
      if ( status_ok  ) status_ok = fill_mfdb_out( out, zvnplanes_out, mfdb_buffer_out, ibuffer, tctab ) ;
