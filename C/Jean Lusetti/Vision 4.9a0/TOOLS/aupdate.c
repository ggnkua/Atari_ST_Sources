...
static short AUGetCmdLine4PExec(char* updater, char* url, char* cmd_line, char* url_file, char* local_file, char* trusted_ca_file)
{
  ...
  cmd_line[0] = (char)strlen( &cmd_line[1] ) ; /* Pascal convention */
...
short AUCheckUpdate(AU_CONTEXT* AUContext)
{
  ...
  if ( AUGetCmdLine4PExec( AUContext->updater, AUContext->url, cmd_line, AUContext->inet_app_fversion, AUContext->local_version_file, AUContext->trusted_ca_file ) != 0 ) return AU_UPDATERUNKNOWN ;
