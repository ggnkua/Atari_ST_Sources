...
#define CMD_LOOPFOLDER    19
#define CMD_ENDLOOPFOLDER 20
#define CMD_SAVE          21
#define CMD_QUIT          100
...
COMMAND_LABEL_IDS CmdLabelIds[] = {
                                    ...
                                    { "loop_folder",    CMD_LOOPFOLDER },
                                    { "endloop_folder", CMD_ENDLOOPFOLDER },
                                    { "save",           CMD_SAVE },
                                  } ;
...
static short GetCmdAndParams(VCMD_SCRIPT* vcmd_script, char* command_line)
{
  ...
  if ( space ) *space++ = 0 ;
  if ( command_line[0] == ';' ) cmd = CMD_COMMENT ;
  ...
  vcmd_script->nparams = GetParams( space, ' ', vcmd_script->params, ARRAY_SIZE(vcmd_script->params) ) ;
...
static short ExeCommandLine(VCMD_SCRIPT* vcmd_script, char* command_line)
{
  ...
  status = GetCmdAndParams( vcmd_script, command_line ) ;
  switch( status )
  {
    ...
    case CMD_COMMENT:       status = SVCS_SUCCESS ;
                            break ;
    case CMD_OPEN:          status = CmdOpen( vcmd_script ) ;
                            break ;
    case CMD_CLOSE:         status = CmdClose( vcmd_script ) ;
                            break ;
    case CMD_WAIT:          status = CmdWait( vcmd_script ) ;
                            break ;
    case CMD_LOOP:          status = CmdLoop( vcmd_script ) ;
                            break ;
    case CMD_ENDLOOP:       status = CmdEndLoop( vcmd_script ) ;
                            break ;
    case CMD_RSCRIPT:       status = CmdRScript( vcmd_script ) ;
                            break ;
    case CMD_CATCRE:        status = CmdCatCre( vcmd_script ) ;
                            break ;
    case CMD_CATAFI:        status = CmdCatAfi( vcmd_script ) ;
                            break ;
    case CMD_CATAFO:        status = CmdCatAfo( vcmd_script ) ;
                            break ;
    case CMD_CATOICN:       status = CmdCatOIcn( vcmd_script ) ;
                            break ;
    case CMD_CATSINI:       status = CmdCatSIni( vcmd_script ) ;
                            break ;
    case CMD_IBOPEN:        status = CmdIBOpen( vcmd_script ) ;
                            break ;
    case CMD_IBNEXT:        status = CmdIBMove( IMGB_NEXT ) ;
                            break ;
    case CMD_IBPREV:        status = CmdIBMove( IMGB_PREVIOUS ) ;
                            break ;
    case CMD_BCONV:         status = CmdBConv( vcmd_script ) ;
                            break ;
    case CMD_IPSET:         status = CmdIPSet( vcmd_script ) ;
                            break ;
    case CMD_LDVDO:         status = CmdLDVDo( vcmd_script, 0 ) ; /* LDV parameters from command line */
                            break ;
    case CMD_LDVDOIP:       status = CmdLDVDo( vcmd_script, 1 ) ; /* LDV Parameters from INI */
                            break ;
    case CMD_LOOPFOLDER:    status = CmdLoopFolder( vcmd_script ) ;
                            break ;
    case CMD_ENDLOOPFOLDER: status = CmdEndLoopFolder( vcmd_script ) ;
                            break ;
    case CMD_SAVE:          status = CmdSave( vcmd_script ) ;
                            break ;
