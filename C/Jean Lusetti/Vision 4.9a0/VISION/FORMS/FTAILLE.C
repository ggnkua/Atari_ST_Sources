  ...
  *pcxy += add ;
  ...
  if ( flag_propor )
  {
    *pcyx = *pcxy ;
    ...
    xobjc_draw( wnd->window_handle, adr_taille, obj_num2 ) ;
  }
  xobjc_draw( wnd->window_handle, adr_taille, obj_num1 ) ;
...
static int OnObjectNotifyTaille(void* w, int obj)
{
  ...
  switch( obj )
  {
    ...
    case TAILLE_YPLUS  :  handle_sizechange( wnd, 1, 1 ) ;
                          break ;
    case TAILLE_YMOINS  : handle_sizechange( wnd, 0, -1 ) ;
                          break ;
...
void traite_taille(GEM_WINDOW* gwnd)
{
  ...
  GWSetWindowInfo( gwnd, MsgTxtGetMsg(hMsg, MSG_CALCULZOOM)  ) ;
  mouse_busy() ;

  undo_put( gwnd, 0, 0, vimage->inf_img.mfdb.fd_w-1, vimage->inf_img.mfdb.fd_h-1, UTAILLE, NULL ) ;

  wprog = DisplayStdProg( MsgTxtGetMsg(hMsg, MSG_TAILLE) , "", "", CLOSER ) ;
/*  if ( raster_pczoom(&vimage->inf_img.mfdb, &out, wext.pcx, wext.pcy, wprog ) == -1 )*/
  out.fd_addr = NULL ;
