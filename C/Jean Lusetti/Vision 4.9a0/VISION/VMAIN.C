...
long SaveVISIONIniFile(VISION_CFG* cfg)
{
  ...
  SetIniKeyBool( h_ini, section_diaporama, "IgnoreVSS", cfg->ignore_vss ) ;
...
void LoadVISIONIniFile(VISION_CFG* cfg)
{
  ...
  cfg->ignore_vss        = ReadEntry_short( &re, "IgnoreVSS", 0 ) ;
  ...
  cfg->ldv_preview_src_size = ReadEntry_short( &re, "PreviewSrcSize", LDV_PREVIEWDEFAULTSIZE ) ;
  ...
  strcpy( cfg->vu_url, "http://vision.atari.org/vupdate/staging/" ) ;
...
int OnAppDragDrop(DD_DATA* dd_data)
{
...
int vget_txtbubblemsg(int obj, int* objs, int* msgs, int nb_elem, char* txt)
{
  ...
  for ( i = 0; i < nb_elem; i++ )
    if ( obj == objs[i] ) { strcpy( txt, MsgTxtGetMsg(hMsg, msgs[i]) ) ; break ; }
