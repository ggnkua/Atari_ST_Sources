...
int GeneralKbdHandler(int key)
{
  ...
  /* Mint does not report shift cursor keys with different scancodes */
  /* Let's fix this here (should be also possible with keyedit.app)  */
  graf_mkstate( &dummy, &dummy, &dummy, &kstate ) ;
  if ( kstate & (K_LSHIFT|K_RSHIFT) )
  {
    if ( key == CURSOR_UP )       key = CURSOR_SUP ;
    else if ( key == CURSOR_DN )  key = CURSOR_SDN ;
    else if ( key == CURSOR_RT )  key = CURSOR_SRT ;
    else if ( key == CURSOR_LT )  key = CURSOR_SLT ;
    else if ( key == KBD_DELETE ) key = KBD_SDELETE ;
    if ( oldkey != key ) LoggingDo(LL_KEYCODE, "Key $%04X moved to $%04X because shift is hold", oldkey, key) ;
  }
  ...
  if ( kstate == K_CTRL )
  {
    /* Normalize Control- keys */
    key = CTRL(key) ;
    LoggingDo(LL_KEYCODE, "Key $%04X normalized to $%04X because control is hold", oldkey, key) ;
  }
...
/*    if ( dlg->EditObject && dlg->ShowEdit )
  ...
  switch( mesag[0] )
  {
    ...
    case WM_TOPPED   :
    case WM_ONTOP    :
    case WM_NEWTOP   : if ( mesag[0] == WM_TOPPED ) LoggingDo(LL_GW2, "WM_TOPPED for %s", GWInfoW(wnd) ) ;
                       else if ( mesag[0] == WM_ONTOP ) LoggingDo(LL_GW2, "WM_ONTOP for %s", GWInfoW(wnd) ) ;
                       else if ( mesag[0] == WM_NEWTOP ) LoggingDo(LL_GW2, "WM_NEWTOP for %s", GWInfoW(wnd) ) ;
                       if ( wnd && ( !in_modal_dialog || ( in_modal_dialog && ( wnd == cur_wnd ) ) ) )
                       {
    ...
    default :         if ( (mesag[0] >= WM_USER_FIRST) && (mesag[0] <= WM_USER_LAST) )
                       {
                         ...
                         if ( wnd )
                         {
                           if ( wnd->OnMsgUser )
                           {
                             code = wnd->OnMsgUser( wnd, mesag[0], &mesag[4] ) ;
                           }
                         }
                         else if ( GemApp.OnMsgUser ) code = GemApp.OnMsgUser( &GemApp, mesag[0], mesag ) ;
                       }
                       else if ( GemApp.OnUnknownMsg ) code = GemApp.OnUnknownMsg( &GemApp, mesag ) ;
                       break ;
  }
...
int GWLoop(void)
{
  ...
  do
  {
    ...
    /* MU_TIMER sert au minimum a gerer les bulles d'aide */
    ...
    /* On re-demande les coordonnes de la souris parce qu'elles ont pu changer depuis */
    /* la sortie de EvntMulti() suite aux handlers de messages GEM, clavier, et bouton de souris */
  ...
  LoggingDo(LL_GW2, "GWLoop:exit with $%x" ) ;
  return code ;
}
...
int GWBasicModalHandler(void)
{
  ...
  LoggingDo(LL_INFO, "GWBasicModalHandler:enter" );
  ...
  LoggingDo(LL_INFO, "GWBasicModalHandler:exit with $%x", scode );
  return scode ;
}
