...
CATALOG* alloc_catalog(void)
{
  ...
  {
    cat->magic = CAT_MAGIC ;
    cat->IniCat = -1 ; /* No associated INI file yet */
  }
...
void compute_visibles(CATALOG* cat)
{
  ...
  cat->catalog_filter.nb_visibles = 0 ;
  if ( cat->catalog_filter.show_all )
  {
    for ( i = 0; i < cat->catalog_header.nb_records+cat->catalog_header.nb_deleted; i++ )
    {
      ...
      {
        record->is_visible = 1 ;
        cat->catalog_filter.nb_visibles++;
      }
  ...
  else
  {
    for ( i = 0; i < cat->catalog_header.nb_records+cat->catalog_header.nb_deleted; i++ )
    {
      ...
      if ( record->name[0] != MARKED_DELETED )
      {
        presult = cat->catalog_filter.planes & (1 << record->nbplanes) ;
        if ( presult )
        {
          fresult = cat->catalog_filter.format & (1 << record->format) ;
          if (fresult)
          {
            cresult = cat->catalog_filter.category & (1UL << record->category_1) ||
                      cat->catalog_filter.category & (1UL << record->category_2) ||
                      cat->catalog_filter.category & (1UL << record->category_3) ;
          }
        }
...
void init_catalog_header(CATALOG* cat, char *catalog_name)
{
  ...
  memset(cat->catalog_header.comment, 0, sizeof(cat->catalog_header.comment)) ;
  memset(cat->catalog_header.rfu, 0, sizeof(cat->catalog_header.rfu)) ;
  memset(cat->catalog_header.category_names, 0, MAX_CATEGORY*CATEGORY_SIZE) ;
...
int item_exist(CATALOG* cat, char *filename)
{
  ...
  while (current <= last_item)
  {
    ...
    {
      fseek(cat->catalog_stream, current->offset, SEEK_SET) ; /* V‚rifions alors le nom physique */
...
int catalog_setcatini(HCAT hCat, int cat_ini)
{
  ...
  if ( cat_ini )
  {
    if ( cat->IniCat != -1 ) CloseIni( cat->IniCat ) ;
    cat->IniCat = OpenIni( cat->catalog_header.cat_inifile ) ;
  }
  else if ( cat->IniCat != -1 )
  {
    CloseIni( cat->IniCat ) ;
    cat->IniCat = -1 ;
  }
...
int catalog_applyini(HCAT hCat, char* msg)
{
  ...
  for ( i = 0; i < cat->catalog_header.nb_records+cat->catalog_header.nb_deleted; i++, rec++ )
  {
    ...
    {
      pc = (int) (((long)(1+i)*100L)/(cat->catalog_header.nb_records+cat->catalog_header.nb_deleted)) ;
...
int add_icon(CATALOG* cat, char *filename, INFO_IMAGE *inf, MFDB *icon)
{
  ...
  memset(cat->item, 0, sizeof(INDEX_RECORD)) ;
...
int catalog_removesel(HCAT hCat, int files, GEM_WINDOW *wprog)
{
  ...
  for ( n = 0; n < total; n++, rec++ )
  {
    if ( rec->selected )
    {
      ...
      if ( MAY_UPDATE( wprog, n ) )
      {
        int  pc = (int) (((1+n)*100L)/total) ;
        char buf[10] ;

        memcpy( buf, rec->name, 8 ) ;
        buf[8] = 0 ;
        wprog->ProgPc( wprog, pc, buf ) ;
      }
    }
  }
...
HCAT catalog_new(char *access_path, char *cat_name, int* status)
{
  ...
  cat->catalog_stream = fopen(cat->catalog_name, "wb+") ;
  ...
  if ( out->fd_nplanes < nb_plane )
  {
    ...
    if ( Truecolor && ( out->fd_nplanes == 16 ) )
    {
      ...
      switch( nb_plane )
      {
        case 16 : 
                  break ;
        case 24 : buffer = (unsigned char *) Xalloc( 3L*(long)out->fd_w*(long)out->fd_h ) ;
                  if ( buffer != NULL )
                  {
                  ...
                  break ;
        case 32 : buffer = (unsigned char *) Xalloc( 4L*(long)out->fd_w*(long)out->fd_h ) ;
                  if ( buffer != NULL )
                  {
...
int ConvImg(char *file, char *dest_path, char *out_name)
{
  ...
  if ( strcmpi( ext, "gif" ) && strcmpi( ext, "jpg" ) && strcmpi( ext, "jpeg" ) )
  {
    ...
    if ( mfdb_conv.fd_addr ) Xfree( mfdb_conv.fd_addr ) ;
    if ( inf_conv.palette ) Xfree( inf_conv.palette ) ;
...
int migrate_from0101(FILE* in, size_t in_size, CATALOG_HEADER* ch, char* cat_name, GEM_WINDOW *wprog)
{
  ...
  out = fopen( cat_name, "wb+" ) ;
  /* Version without date/time in INDEX_RECORD */
  if ( !out ) return ECAT_FILENOTFOUND ;
  ...
  while ( !done && !err )
  {
    ...
    if ( MAY_UPDATE( wprog, nb ) )
    {
      int pc = (int)( (100L*nb) / total) ;

      if ( pc != last_pc )
      {
        wprog->ProgPc( wprog, pc, NULL ) ;
        LoggingDo(LL_INFO, "%ld records updated on a total of %ld (%d%%)", nb, total, pc) ;
        last_pc = pc ;
      }
    }
  }
