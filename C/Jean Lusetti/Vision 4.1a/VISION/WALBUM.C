...
/*  vro_cpyfm(handle, tab_index[wext->start_visible+i].selected ? S_OR_NOTD:S_ONLY, xy, &v->img, &screen) ;*/
  ...
  if ( tab_index[wext->start_visible+i].selected )
  {
    ...
    coords[6] = xy[6] - 4 ;
    coords[7] = xy[5] + 4 + 8 ;
    vro_cpyfm(handle, D_INVERT, coords, &v->img, &screen) ;
  }
...
{
    if ( v->img.fd_addr && (v->img.fd_addr != empty_img_addr) )
    {
      Xfree( v->img.fd_addr ) ;
      v->img.fd_addr = NULL ;
    }
...
static int charge_vignettes(GEM_WINDOW* wnd, long start)
{
  ...
  for (i = 0; i < wext->nb_vignettes; i++)
  {
    ...
    err = catalog_loadicon( wext->hCat, start+i, &tab_vignette[i].img, &tab_vignette[i].info, tab_vignette[i].com ) ;
    if ( err == ECAT_NOERR )
    {
    ...
    else if ( err == ECAT_BADINDEX )
    {
      ...
      memcpy( &tab_vignette[i].img, &wext->emptyImg, sizeof(MFDB) ) ;
      err = 0 ;
    }
  }
...
static int charge_nvignettes(GEM_WINDOW* wnd, long pos, long nb)
{
  ...
  for (i = pos; i < pos+nb; i++)
  {
    ...
    GWSetWindowInfo( wnd, buf ) ;
    tab_vignette[i].nom[0] = 0 ;
    err = catalog_loadicon( wext->hCat, wext->start_visible+i, &tab_vignette[i].img, &tab_vignette[i].info, tab_vignette[i].com ) ;
    if ( err == ECAT_NOERR )
    {
      if ( tab_index[wext->start_visible+i].is_visible )
      {
    ...
    else LoggingDo(LL_ERROR, "charge_nvignettes(%ld):%p (failed)", wext->start_visible+i, tab_vignette[i].img.fd_addr);
  }
...
static void make_info(WALB_EXTENSION* wext, long num, char *buf)
{
  ...
  if ( !catalog_header || !tab_index || !tab_vignette )
  {
    LoggingDo(LL_ERROR, "Invalid hCat");
    return ;
  }
...
static int OnKeyPressedAlbum(void *w, int key)
{
  ...
  switch( key )
  {
    ...
    case KBD_DELETE : nsel = catalog_getnsel(wext->hCat) ;
                      ...
                      break ;
    case KBD_SDELETE: nsel = catalog_getnsel(wext->hCat) ;
                      if ( nsel > 0 )
                      {
...
static int OnLButtonDownAlbum(void *w, int mk_state, int x, int y)
{
  ...
  if ( ( num_vignette >= 0 ) && ( num_vignette < wext->nb_vignettes_max ) && wext->tab_vignette[num_vignette].info.palette )
  {
    img_palette = (int *) wext->tab_vignette[wext->num_vignette].info.palette ;
...
static int OnRButtonDownAlbum(void *w, int mk_state, int mx, int my)
{
  ...
  switch ( MSG_PLOADIMG+clicked-1 )
  {
    ...
    case MSG_PEHTML+4  : /* --> MSG_WADELFILES */
                         OnKeyPressedAlbum( w, KBD_SDELETE ) ;
                         break ;
  }
...
GEM_WINDOW* album_window(HCAT hCat, GEM_WINDOW* wnd_album, GEM_WINDOW** pwnd)
{
  ...
  wext->emptyImg.fd_wdwidth = wext->emptyImg.fd_w/16 ;
  if ( wext->emptyImg.fd_w% 16 ) wext->emptyImg.fd_wdwidth++ ;
  wext->emptyImg.fd_nplanes = nb_plane ;
