...
void bconv_inform_error(char *name, int err, BATCH_CONV *batch_conv)
{
  ...
  if ( batch_conv->err_stream )
  {
    ...
    {
      case -1  :
      case -2  : strcpy( msg_werr, 1+MsgTxtGetMsg(hMsg, MSG_WRITEERROR)  ) ;
                 p = strchr( msg_werr, '.' ) ;
                 if ( p ) *p = 0 ;
                 sprintf( buf, "%s\n", msg_werr ) ;
                 break ;
      case -3  : strcat( buf, MsgTxtGetMsg(hMsg, MSG_CATAMEM)  ) ;
                 break ;
      case -4  : 
      case -10 : strcat( buf, MsgTxtGetMsg(hMsg, MSG_BCONVOUTERR)  ) ;
                 break ;
      case -11 : sprintf( buf, MsgTxtGetMsg(hMsg, MSG_BCONVMOVERR) , name ) ;
                 break ;
      case -12 : sprintf( buf, MsgTxtGetMsg(hMsg, MSG_BCONVMOVERR) , name ) ;
                 break ;
      default  : strcat( buf, "???\n" ) ;
                 break ;
    }
...
int conv_img(FR_CDATA* fr_cdata)
{
...
void traite_batch_conversion(void)
{
  ...
  batch_conv.bconv = (BCONV_PARAMS *) Xcalloc( sizeof(BCONV_PARAMS), 1 ) ;
