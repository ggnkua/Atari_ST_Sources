...
void LoadVISIONIniFile(VISION_CFG *cfg)
{
  ...
  save_ini |= GetOrSetIniKey( h_ini, section_logging, "LoggingFile", config.logging_file ) ;

  sprintf( buf, "%x", LL_WARNING ) ;
...
void ajust_pathconfig(void)
{
  ...
  if ( !path || (path[0] == 0) || !FolderExist(path) )
  {
...
/* This block was used to generate VMSG.TXT files after loading VSMG.RSC
 ...
 It has just been used once */
  ...
  if ( hMsg == NULL )
  {
    ...
    sprintf( buf, "[File VMSG.TXT is missing !|You should delete VISION.INI|for a fresh start!][ End ]", name ) ;
    form_stop( 1, buf ) ; 
  }
...
int OnAppUnknownMsg(void *app, int *mesg)
{
  int           code = 0 ;
/*  LDG           *ldg ;
...
int OnInitApp(void *ap)
{
  ...
  if ( Xrsrc_load( rsc_name ) == 0 )
  {
	...
	sprintf(buf, "[File %s is missing !|You should delete VISION.INI|for a fresh start!][ End ]", rsc_name);
	form_stop( 1, buf ) ;
    OnTerminateApp( app ) ;
