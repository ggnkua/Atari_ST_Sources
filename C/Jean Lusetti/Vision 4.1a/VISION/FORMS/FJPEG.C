...
void OnInitDialogJpeg(void *w)
{
  ...
  if ( wext->inf->quality == 0 ) wext->inf->quality = 90 ; /* Was 75 by default which is poor */
  sprintf( buf, "%03d", wext->inf->quality ) ;
...
int jpeg_options(INFO_IMAGE *inf)
{
  ...
  code = GWDoModal( dlg, JPEG_QUALITY ) ;
