...
void fmt_line(HCAT hCat, OBJECT *adr_album, int line, int first)
{
  char buf[FMT_SIZE+1] ;
  char nom[10] ;
  char fmt[10] ;
...
int add_img(FR_CDATA* fr_cdata)
{
  ...
  if ( stream )
  {
    fprintf( stream, "[%04ld]%s\r\n", add_album->nb_files++, fr_cdata->filename ) ;
    fclose (stream ) ;
    sprintf( buf, "%ld/%ld", add_album->nb_files, add_album->total_nb_files_expected ) ;
    if ( add_album->waddprog ) add_album->cancel_adds = add_album->waddprog->ProgRangeEx( add_album->waddprog, add_album->nb_files, add_album->total_nb_files_expected, buf ) ;
  }
  if ( stream ) return 0 ;
#endif
  ...
  if ( DImgGetIIDFromFile( fr_cdata->filename ) != 0 )
  {
    if ( add_album->waddprog && ( add_album->total_nb_files_expected >= 0 ) )
    {
      ...
      sprintf( buf, "%ld/%ld", add_album->nb_files, add_album->total_nb_files_expected ) ;
      add_album->cancel_adds = add_album->waddprog->ProgRange( add_album->waddprog, add_album->nb_files, add_album->total_nb_files_expected, buf ) ;
    }
...
void add_folders(WEXTENSION_ALBUM *wext)
{
  ADD_ALBUM       add_album ;
...
HCAT fcatalog_open(char *cat_name, GEM_WINDOW *wprog, int* err)
{
  HCAT hCat ;
  ...
  switch( obj )
  {
   ...
   case  ALBUM_ADDIMG    : strcpy(buf, config.path_img) ;
                           ...
                           if (file_name(buf, "", buf))
                           {
                             ...
                             switch(err)
                             {
                               case ECAT_FILENOTFOUND  : form_stop(1, MsgTxtGetMsg(hMsg, MSG_FILENOTEXIST) ) ;
                                                         break ;
                               case ECAT_NOMEMORY      : form_error(8) ;
                                                         break ;
                               case ECAT_ITEMEXIST     : form_stop(1, MsgTxtGetMsg(hMsg, MSG_CATITEMEXIST) ) ;
                                                         break ;
                               default                 : if (err) form_stop(1, MsgTxtGetMsg(hMsg, MSG_CATADDERR) ) ;
                           ...
                           wext->hf =(float) (adr_album[ALBUM_BOX].ob_height-adr_album[ALBUM_SLIDER].ob_height) ;
                           break ;
    case ALBUM_ADDFOLDER : add_folders(wext) ;
    ...
    case ALBUM_TOVSS     : sprintf( vss_name, "%s\\*.VSS", config.path_album ) ;
                           ...
                           if ( valid && catalog_filter )
                           {
                             ...
                             for ( num = 0;  num < catalog_filter->nb_visibles ; num++ )
                             {
                               ...
                               if ( MAY_UPDATE( wprog, num ) && catalog_filter )
                               {
...
#pragma warn -par
int OnMsgUserAlb(void* w, int msg_id, int mesag[4])
{
  GEM_WINDOW* wnd = (GEM_WINDOW*) w ;
