...
{
  if ( tab_filtre != NULL ) Xfree( tab_filtre ) ;
  if ( names != NULL) Xfree( names ) ;
}

void get_filterinfos(char *path, CONVOLUTION_FILTRE *tab, int nb_filtres)
{
  DTA                dta, *old_dta ;
  GEM_WINDOW         *wprog ;
  CONVOLUTION_FILTRE filtre ;
  ...
  wprog = DisplayStdProg( MsgTxtGetMsg(hMsg, MSG_LOOKFILTERS) , "", "", CLOSER ) ;
  ...
  while ( Fsnext() == 0 )
  {
    ...
    wprog->ProgPc( wprog, (int) ((100L*(long)nb)/(long)nb_filtres), NULL ) ;
  }
  GWDestroyWindow( wprog ) ;
  Fsetdta( old_dta ) ;
}

void setup_filters(GEM_WINDOW *wnd, CONVOLUTION_FILTRE *tab_filtre, int nb_filtres, int flag_show)
{
  ...
  if ( flag_show )
  {
    ...
    for (i = CONVOL_INFO1; i <= CONVOL_INFOLAST; i++)
      xobjc_draw( wnd->window_handle, adr_convol, i ) ;
    ...
    xobjc_draw( wnd->window_handle, adr_convol, CONVOL_BOX ) ;
  }
}

int reload_filters(void)
{
  char path[PATH_MAX] ;

  mouse_busy() ;
  nb_filtres = GetFilterPathForLanguage( path ) ;
  ...
  if ( nb_filtres > 0 )
  {
    ...
    names = NULL ;
    if ( tab_filtre != NULL ) Xfree( tab_filtre ) ;
    tab_filtre = (CONVOLUTION_FILTRE *) Xalloc(nb_filtres*sizeof(CONVOLUTION_FILTRE)) ;
    if (tab_filtre == NULL)
    {
      mouse_restore() ;
      form_error(8) ;
    ...
    names = (char * ) Xalloc( NB_MAX_CAR_NOM_FILTRE*nb_filtres ) ;
    if (names == NULL)
    {
      Xfree(tab_filtre) ;
      tab_filtre = NULL ;
...
void OnInitDialogFilter(void *w)
{
...
int OnObjectNotifyFilter(void *w, int obj)
{
  GEM_WINDOW         *wnd = (GEM_WINDOW *) w ;
  WEXTENSION_FILTER  *wext = wnd->DlgData->UserData ;
  OBJECT             *adr_convol = wnd->DlgData->BaseObject ;
  ...
  switch( obj )
  {
    case CONVOL_EDITTHIS : for (i = CONVOL_INFO1; i <= CONVOL_INFOLAST; i++)
                           ...
                           if ( convolution_loadfilter( buf, filtre ) )
                             form_stop(1, MsgTxtGetMsg(hMsg, MSG_BADFILTER) ) ;
                           else
                           {
                             if ((filtre->w == 3) && (filtre->w == 3))
                               edit_3x3filter( filtre ) ;
                             else if ((filtre->w == 5) && (filtre->w == 5))
                               edit_5x5filter( filtre ) ;
                             Xfree(filtre->data) ;
                           }
    ...
    case CONVOL_TOBJSEL  : if (adr_convol[CONVOL_TOBJSEL].ob_state & DISABLED) break ;
                           if (!selected(adr_convol, CONVOL_OBJSEL))
                           {
                             ...
                             xobjc_draw( wnd->window_handle, adr_convol, CONVOL_OBJSEL ) ;
                             xobjc_draw( wnd->window_handle, adr_convol, CONVOL_OBJIMG ) ;
                           }
                           break ;
