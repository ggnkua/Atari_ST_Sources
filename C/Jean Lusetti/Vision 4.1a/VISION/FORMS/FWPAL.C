...
int OnMouseMoveSelPalIndex(void *w, int button, int kstate, int mx, int my )
{
  ...
  if ( mx >= 0 )
  {
    ...
    v_show_c( handle, 1 ) ;
    vq_color( handle, index, 1, rgb ) ;
    sprintf( buf, MsgTxtGetMsg(hMsg, MSG_RVB), rgb[0], rgb[1], rgb[2], index ) ;
  }
...
void OnDrawSelPalIndex(void *wn, int xycoords[4])
{
  ...
  for ( yc = 0; yc < wext->nbcy; yc++ )
  {
    ...
    for ( xc = 0; xc < wext->nbcx; xc++ )
    {
      x = xi + ftaille * xc ;
      set_filltype( &fstyle ) ;
      if ( intersect( x, y, taille, taille, xycoords[0], xycoords[1], xycoords[2], xycoords[3], xy ) )
        vr_recfl( handle, xy ) ;
      fstyle.color++ ;
    }
  }
...
int OnFulledSelPalIndex(void *w)
{
  ...
  xywh[0] = xdesk + ( wdesk-wext->taille * wext->nbcx ) / 2 ;
  xywh[1] = ydesk ;
...
void traite_selpalindex(GEM_WINDOW *cur_window)
{
  ...
  else
  {
    ...
    if ( WndSelPalIndex )
    {
      ...
      {
        SetMaxWindow( wext ) ;
        xo = xdesk + ( wdesk-wext->taille * wext->nbcx ) / 2 ;
        yo = ydesk ;
      }
      ...
      GWSetWindowCaption( WndSelPalIndex, MsgTxtGetMsg(hMsg, MSG_PALETTEDISPO)  ) ;
      wind_calc( WC_BORDER, WndSelPalIndex->window_kind, xdesk + ( wdesk - wext->taille * wext->nbcx ) / 2, ydesk + ( hdesk - wext->taille * wext->nbcy ) / 2,
                 wext->nbcx * wext->taille, wext->nbcy * wext->taille, &x, &y, &w, &h ) ;
      GWOpenWindow( WndSelPalIndex, xo, yo, w, h ) ;
    }
    else form_stop( 1, MsgTxtGetMsg(hMsg, MSG_NOMOREWINDOW)  ) ;
  }
}
