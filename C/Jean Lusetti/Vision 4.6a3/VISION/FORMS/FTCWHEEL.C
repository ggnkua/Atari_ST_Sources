...
void Adapt2Resolution(OBJECT* form_tcwheel)
{
  if ( (Xmax > 1439) && (Ymax > 899) )
  {
    ...
    form_tcwheel[TCPAL_TITLE].ob_x     += add_tcpal_box/2 ;
    ...
    form_tcwheel[TCPAL_TRED].ob_y    += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_TGREEN].ob_y  += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_TBLUE].ob_y   += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_RED].ob_y     += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_GREEN].ob_y   += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_BLUE].ob_y    += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_COLOR].ob_y   += add_tcpal_box/2 ;
    form_tcwheel[TCPAL_PERCENT].ob_y += add_tcpal_box/2 ;
...
void OnInitDialogTCWheel(void *w)
{
  ...
  if ( T24 == NULL ) err = T24_init(wext->TCWheel.fd_w/2) ;
  if ( err )
  {
    form_error( 8 ) ;
    return ;
