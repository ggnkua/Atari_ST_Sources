...
int cdecl draw_visionimg(PARMBLK *paramblk)
{
  ...
  v_hide_c(handle) ;
  vro_cpyfm(handle, S_ONLY, xy, &wext->inf_visionimg.mfdb, &screen) ;
  v_show_c(handle, 1) ;
  vs_clip(handle, 0, xy) ;
...
/*    adr_baratin[0].ob_width = 242 ;*/
  ...
  wext->inf_visionimg.lformat = IID_IMG ;
  ...
  wext->inf_visionimg.mfdb.fd_nplanes = nb_plane ;
  if ( nb_plane == 16 ) Force16BitsLoad = 1 ;
  else                  Force16BitsLoad = 0 ;
  ret = load_picture( &wext->inf_visionimg ) ;
  if ( ret != 0 )
  {
    adr_baratin[BARATIN_IMG].ob_flags |= HIDETREE ;
    adr_baratin[0].ob_width = 242 ;
  ...
  if ( wext->inf_visionimg.palette ) Xfree( wext->inf_visionimg.palette ) ;
...
int OnCloseDialogBaratin(void *w)
{
  ...
  if ( wext->use_img ) Xfree( wext->inf_visionimg.mfdb.fd_addr ) ;
