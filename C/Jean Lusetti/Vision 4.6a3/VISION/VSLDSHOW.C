...
int load_nextimg(char *name, INFO_IMAGE *inf, VSS_INFO *vss_info, GEM_WINDOW *wprog)
{
  int ret ;
  ...
  if ( ret != EIMG_SUCCESS ) if ( inf->palette ) Xfree( inf->palette ) ;
  else
  {
    MFDB* mfdb = &inf->mfdb ;
...
static int wait_for_event(VSS_INFO *vss_info, size_t total_load_time)
{
  EVENT event ;
  int   ret = 0 ;
  int   which ;
  ...
  if ( 100L*vss_info->wait_mini_sec > total_load_time )
    event.ev_mtlocount = (int) ( 1000L*vss_info->wait_mini_sec - 10L*total_load_time ) ;
  ...
  while ( !fini )
  {
    ...
    switch( cmd->cmd_id )
    {
      case CMD_DISPLAY     : LoggingDo(LL_INFO, "Slide-show: loading %s...", cmd->params);
                             ...
                             {
                               compute_colors( vss_info, &inf ) ;
                               if ( ( num_courant > 0 ) && ( wait_for_event( vss_info, inf.perf_info.total_load_time ) < 0 ) )
                                 fini = 1 ;
                               else
                               {
