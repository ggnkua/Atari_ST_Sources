...
static short GetParam(char* command_line, short param_no, char* param)
{
  ...
  LoggingDo(LL_DEBUG, "GetParam(%d) retuns %d, param=%s", param_no, status, param) ;
  return status ;
}
...
short ExeNextCommandLine(VCMD_SCRIPT* vcmd_script)
{
  ...
  if ( vcmd_script->Buffer )
  {
    ...
    else
    {
      ...
      if ( strlen(line) >= 4 )
      {
        LoggingDo(LL_INFO, "Executing Command line: %s", line) ;
        ret = ExeCommandLine( vcmd_script, line ) ;
