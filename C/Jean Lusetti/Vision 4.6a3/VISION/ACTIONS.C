...
int clip_search(void)
{
  ...
  info.lformat = IID_UNKNOWN ;
  if ( img_format(name, &info ) != 0 ) return 0 ;
  img->fd_nplanes = nb_plane ;
  Force16BitsLoad = ( nb_plane == 16 ) ;
  if ( load_picture( &info ) != 0 ) return 0 ;

  if ( info.palette ) Xfree( info.palette ) ;
  FreeClipboard() ;
...
void traite_imprime(GEM_WINDOW* gwnd)
{
  ...
  else memset(&inf, 0, sizeof(INFO_IMAGE)) ;
...
GEM_WINDOW *nouvelle_wimage(char *nom, int nx, int ny, int nc, int tpmx, int tpmy)
{
  ...
  vimage->inf_img.mfdb.fd_addr = img_alloc( nnx, nny, nb_plane ) ;
  ...
  vimage->inf_img.mfdb.fd_nplanes = nb_plane ;
  vimage->inf_img.mfdb.fd_wdwidth = nnx/16 ;
  if (nnx % 16) vimage->inf_img.mfdb.fd_wdwidth++ ;
  vimage->inf_img.mfdb.fd_stand = 0 ;
  memset( &vimage->zoom, 0, sizeof(MFDB) ) ;
  vimage->zoom_level = 1 ;
  vimage->modif      = 0 ;
  sprintf( vimage->nom, "%s\\%s", config.path_img, nom ) ;
  LoggingDo(LL_DEBUG, "nouvelle_wimage; width=%d, height=%d, nb_plane=%d", vimage->inf_img.mfdb.fd_w, vimage->inf_img.mfdb.fd_h, vimage->inf_img.mfdb.fd_nplanes) ;
...
int traite_loupe(GEM_WINDOW *cur_window, int flag_level, int mx, int my)
{
  ...
  if ( raster_zoom(&vimage->inf_img.mfdb, &vimage->zoom,
...
LDV_STATUS traite_ldvundo(LDV_MODULE *ldv, LDV_PARAMS *ldv_params, GEM_WINDOW *cur_window)
{
  ...
  memcpy( &ldv_img.Raster, &vimage->inf_img.mfdb, sizeof(MFDB) ) ;
...
LDV_STATUS do_ldv(LDV_MODULE *ldv, GEM_WINDOW *gwnd, LDV_PARAMS *ldv_params)
{
  ...
  memset( &ldv_perfs, 0, sizeof(ldv_perfs) ) ;
  err = LDVDo( ldv, ldv_params, &ldv_img, 0 ) ;
  if ( LDV_SUCCESS( err ) )
  {
    memcpy( &vimage->inf_img.mfdb, &ldv_img.Raster, sizeof(MFDB) ) ;
...
void bconv_inform_error(char *name, int err, BATCH_CONV *batch_conv)
{
  ...
  if ( batch_conv->err_stream == NULL )
  {
    ...
    sprintf( buf, MsgTxtGetMsg(hMsg, MSG_BCONVERR) , batch_conv->bconv_errfile ) ;
  ...
  if ( batch_conv->err_stream )
  {
    ...
    sprintf( buf, MsgTxtGetMsg(hMsg, MSG_ERRCONV) , name ) ;
    switch( err )
    {
      case EIMG_FILENOEXIST  :
      case EIMG_DATAINCONSISTENT  : strcpy( msg_werr, 1+MsgTxtGetMsg(hMsg, MSG_WRITEERROR)  ) ;
                                    p = strchr( msg_werr, '.' ) ;
                                    if ( p ) *p = 0 ;
                                    sprintf( buf, "%s on %s\n", msg_werr, name ) ;
                                    break ;
      case EIMG_NOMEMORY  :         strcat( buf, MsgTxtGetMsg(hMsg, MSG_CATAMEM)  ) ;
                                    break ;
      case EIMG_DECOMPRESERR  : 
      case -110 :                   strcat( buf, MsgTxtGetMsg(hMsg, MSG_BCONVOUTERR)  ) ;
                                    break ;
      case -111 :                   sprintf( buf, MsgTxtGetMsg(hMsg, MSG_BCONVMOVERR) , name ) ;
                                    break ;
      case -112 :                   sprintf( buf, MsgTxtGetMsg(hMsg, MSG_BCONVMOVERR) , name ) ;
                                    break ;
      default  :                    strcat( buf, "???\n" ) ;
                                    break ;
    }
...
static int conv_img(FR_CDATA* fr_cdata)
{
  ...
  if ( DImgGetIIDFromFile( fr_cdata->filename ) != 0 )
  {
    ...
    err = img_format( fr_cdata->filename, &inf ) ;
    if ( !err )
    {
      ...
      if ( (out->fd_nplanes == 24) || (inf.nplanes == 24) ) out->fd_nplanes = 32 ; /* RasterZoom can't operate on 24bit */
      ...
      if ( do_it )
      {
        ...
        Force16BitsLoad = ( batch_conv->bconv->inf.nplanes == 16 ) ;
        if ( batch_conv->wconvprog && ( batch_conv->total_nb_files_expected >= 0 ) )
        {
          batch_conv->nb_files++ ;
          StdProgWText( batch_conv->wconvprog, 0, MsgTxtGetMsg(hMsg, MSG_LOADINGIMG) ) ;
          ...
          batch_conv->stop_conv = GWProgRange( batch_conv->wconvprog, batch_conv->nb_files, batch_conv->total_nb_files_expected, buf ) ;
        }
        if ( !batch_conv->stop_conv )
        {
          ...
          if ( !err )
          {
            ...
            StdProgWText( batch_conv->wconvprog, 0, MsgTxtGetMsg(hMsg, MSG_SAVINGIMG) ) ;
            ...
            if ( get_resized( batch_conv->bconv->resize, batch_conv->bconv->rwidth, batch_conv->bconv->rheight, &rw, &rh ) )
            {
              ...
              memset( &sout, 0, sizeof(MFDB) ) ;
              sout.fd_w   = rw ;
/*              sout.fd_w  &= 0xFFF0U ;  Multiple of 16 */
              sout.fd_h   = rh ;
              ...
              else
              {
                ...
                err = save_picture( name_out, &sout, &inf, NULL ) ;
                if ( sout.fd_addr ) Xfree( sout.fd_addr ) ;
              }
            }
            else err = save_picture( name_out, out, &inf, NULL ) ;
          }
        }
      }
...
void traite_batch_conversion(void* external_call)
{
  ...
  if ( external_call || (FBConv( batch_conv.bconv ) == 1) )
  {
    ...
    {
      batch_conv.wconvprog = DisplayStdProg( MsgTxtGetMsg(hMsg, MSG_LOADINGIMG) , "", "", 0 ) ;
      GWSetWindowCaption( batch_conv.wconvprog, MsgTxtGetMsg(hMsg, MSG_BCONV)  ) ;
      mouse_busy() ;
      ...
      if ( batch_conv.err_stream )
      {
        sprintf( buffer, MsgTxtGetMsg(hMsg, MSG_CATAEND) , batch_conv.nb_bconv_fails ) ;
        fwrite( buffer, strlen(buffer), 1, batch_conv.err_stream ) ;
        fclose( batch_conv.err_stream ) ;
        batch_conv.err_stream = NULL ;
        sprintf( buffer, MsgTxtGetMsg(hMsg, MSG_ERRSEEFILE) , batch_conv.bconv_errfile ) ;
        form_stop( 1, buffer ) ;
      }
