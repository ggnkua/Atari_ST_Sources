...
int gif_load(INFO_IMAGE* inf)
{
  ...
  ret = gif_decompress( data, taille, inf ) ;
...
int gif_identify(char *name, INFO_IMAGE *inf)
{
  ...
  nplanes = 1 + (entete.color_inf & 0x07) ;
  ...
  inf->width  = descriptor.width ;
  inf->height = descriptor.height ;
  if ( descriptor.color_inf & 0x80 )
    inf->nb_cpal = 1 << ( 1 + (descriptor.color_inf & 0x07) ) ;
  inf->nplanes  = 1 + (entete.color_inf & 0x07) ;
