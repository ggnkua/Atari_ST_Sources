...
int bmp_identify(char *nom, INFO_IMAGE *inf)
{
  ...
  inf->width  = (int) bmp_entete.width ;
  inf->height = (int) bmp_entete.height ;
  inf->nplanes   = bmp_entete.bit_count*bmp_entete.planes ;
  ...
  if (inf->nplanes <= 8)
  {
    ...
    fseek(stream, 14+bmp_entete.size, SEEK_SET) ;
    pal = inf->palette ;
    for (n = 0; n < total; n++)
    {
...
static int bmp_read(FILE *stream, INFO_IMAGE* inf)
{
  ...
  LoggingDo(LL_DEBUG, "bmp_read: bit_count=%d, nplanes=%d, NaturalFormat=%p, img_analyse=%d", bmp_entete.bit_count, out->fd_nplanes, NaturalFormat, img_analyse) ;
  for (l = 0; l < bmp_entete.height; l++)
  {
    ...
    code = GWProgRange( wprog, l, bmp_entete.height, NULL ) ;
    if ( STOP_CODE( code ) )
    {
  ...
  for (l = 0; l < img->fd_h; l++)
  {
    switch(info->nplanes)
    {
      case  1 : if (fwrite(pt_img, 1, nb_octets, stream) != nb_octets)
                ...
                break ;
      case  4 : raster2ind(pt_img, buffer, img->fd_w, info->nplanes) ;
                ...
                break ;
      case  8 : raster2ind(pt_img, buffer, img->fd_w, info->nplanes) ;
                if (fwrite(buffer, 1, nb_octets, stream) != nb_octets)
                {
                  fclose(stream) ;
                  Xfree(buffer) ;
