...
int jpeg_load(INFO_IMAGE *inf)
{
  ...
  LoggingDo(LL_DEBUG, "jpeg_load(%s), fd_addr=%p, fd_w=%d, fd_h=%d, fd_nplanes=%d", CanUseDsp ? "DSP":"CPU", inf->mfdb.fd_addr, inf->mfdb.fd_w, inf->mfdb.fd_h, inf->mfdb.fd_nplanes) ;
