...
static int check_module_caps(IMG_MODULE* img_module, INFO_IMAGE* inf)
{
  ...
  switch( inf->nplanes )
  {
    ...
    default : /* Don't fail if nplanes <= 16 as nplanes=7 (128color) is possible (e.g. QPAINT) */
              /* But VISION does not have support for this exotic number of planes             */
              if ( inf->nplanes > 16 ) ret = EIMG_OPTIONNOTSUPPORTED ;
              break ;
  }

  /* Don't forget to free palette if we failed at this stage */
  if ( ret != EIMG_SUCCESS )
  {
...
int img_format(char* name, INFO_IMAGE* inf)
{
  ...
  if ( lfmt != IID_UNKNOWN )            inf->nb_modules = DImgGetModulesCandidates( NULL, lfmt, inf->img_modules, ARRAY_SIZE(inf->img_modules) ) ;
  ...
  for ( i = 0; i < inf->nb_modules; i++ )
  {
    ...
    if ( ret == EIMG_SUCCESS ) ret = check_module_caps( img_module, inf ) ; /* Free palette if required */
...
static int fit_to_planes(INFO_IMAGE *inf, int out_planes, GEM_WINDOW *wprog)
{
  ...
  {
    if ( incnbplanes( img, out_planes ) != 0 ) return EIMG_NOMEMORY ;
  }
...
static int ldv_load_picture(INFO_IMAGE* inf, IMG_MODULE* img_module)
{
  ...
  if ( UseStdVDI && ( out_planes <= 8 ) )
  {
    ...
    std_img.fd_wdwidth = inf->width/16 ;
    if ( inf->width % 16 ) std_img.fd_wdwidth++ ;
...
{
  if ( tms != PERF_NA ) LoggingDo(LL_PERF, fmt_txt, tms) ;
  else
  {
    char* pc = strchr( fmt_txt, '%' ) ;

    if ( pc ) { *pc = 0 ; strcat( fmt_txt, "NA") ; }
    LoggingDo(LL_PERF, fmt_txt) ;
  }
}
...
int load_picture(INFO_IMAGE* inf)
{
  ...
  {
    adapt_palette( inf ) ;
    LoggingDo(LL_PERF, "Image loading performances for %s:", name) ;
...
int MakeMFDBClassicAtari(MFDB *in, MFDB *std)
{
  ...
  if ( UseStdVDI && ( in->fd_nplanes <= 8 ) )
  {
    ...
    if ( std != NULL ) memcpy( std, &std_img, sizeof(MFDB) ) ;
