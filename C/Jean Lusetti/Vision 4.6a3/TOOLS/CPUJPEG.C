...
int cpu_jpeg_load(INFO_IMAGE* inf)
{
  ...
  else if ( out->fd_nplanes < 16 ) out->fd_nplanes  = ( Force16BitsLoad ? 16 : 32 ) ;
  ...
  /* It seems that nor there is nothing wrong here, possibly on vro_cpyfm, parameters look valid, and even if ot->fd_addr points to a white    */
...
/*  struct jpeg_error_mgr       jerr ;*/
  ...
  switch( img->fd_nplanes )
  {
    case  8 : add_line >>= 1 ;
              ...
              if ( indexes == NULL ) return EIMG_NOMEMORY ;
              break ;

    case 16 : add_line >>= 1 ;
              buffer24 = img_alloc( img->fd_w, 1, 24 ) ;
              if ( buffer24 == NULL ) return EIMG_NOMEMORY ;
              break ;
...
/*  cinfo.err = jpeg_std_error( &jerr ) ;*/
   ...
   */
  jpeg_set_quality( &cinfo, info->c.f.jpg.quality, TRUE /* limit to baseline-JPEG values */ ) ;
  cinfo.dct_method  = JDCT_IFAST ; /* Selection de la DCT rapide sur entiers */
  ...
  while ( cinfo.next_scanline < cinfo.image_height )
  {
    ...
    (void)GWProgRange( wprog, cinfo.next_scanline, cinfo.image_height, NULL ) ;
  }
