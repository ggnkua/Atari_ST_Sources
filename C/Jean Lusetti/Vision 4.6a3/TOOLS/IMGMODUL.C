...
static int DImgUnloadImgModule(IMG_MODULE* img_module)
{
  switch( img_module->Type )
  {
    ...
    case MST_ZVLDG: if ( img_module->hLib )
                    {
                      LoggingDo(LL_IMG, "Unloading ZVLDG #%ld %s (used %ld times)", IMG_MODULE_INDEX(img_module), img_module->LibFilename ? img_module->LibFilename:"", img_module->nUsages) ;
    ...
    case MST_ZVSLB: if ( img_module->hLib )
                    {
                      LoggingDo(LL_IMG, "Unloading ZVSLB #%ld %s (used %ld times)", IMG_MODULE_INDEX(img_module), img_module->LibFilename ? img_module->LibFilename:"", img_module->nUsages) ;
...
static short DImgLoadLDI(char* filename, IMG_MODULE* img_module)
{
  ...
  /* It is just in case VISION image modules be  */
...
static int OnNewModule(FR_CDATA* fr_cdata)
{
  ...
  LoggingDo(LL_DEBUG, "New module %s (type %s) found", fr_cdata->dta->d_fname, GetModuleType(mod_scan->type)) ;
...
static void LoadModulesInformations(char *paths[NB_MST_MAX], char *msg_for_prog, char* priorities)
{
  ...
  /* paths[MST_LDI-MST_LDI]:   path where VISION modules are located (unused) */
