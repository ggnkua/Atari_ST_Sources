...
int tag_interpret(FILE *stream, TIFF_TAG *tag, INFO_IMAGE *inf)
{
  ...
  switch(tag->tag)
  {
    case BITS_PER_SAMPLE   : if (tag->length == 1) BitsPerSample = (unsigned int) get_dim(tag) ;
                             ...
                             inf->nplanes = (short)BitsPerSample*(short)SamplesPerPixel ;
                             break ;
    ...
    case IMAGE_WIDTH       : ImageWidth = get_dim(tag) ;
                             if (ImageWidth > 32767L) return EIMG_OPTIONNOTSUPPORTED ;
                             inf->width = (int) ImageWidth ;
                             break ;
    case IMAGE_LENGTH      : ImageLength  = get_dim(tag) ;
                             if (ImageLength > 32767L) return EIMG_OPTIONNOTSUPPORTED ;
                             inf->height = (int) ImageLength ;
                             break ;
    case COMPRESSION       : Compression   = (unsigned int) get_dim(tag) ;
                             ...
                             if ((Compression != 1) && (Compression != 5)  && (Compression !=32773U))
                               return EIMG_UNKNOWNCOMPRESSION ;
                             break ;
    ...
    case PREDICTOR         : Predictor = (unsigned int) get_dim(tag) ;
                             if ( (Predictor != 1) && (Predictor != 2) )
                               return(-6) ;
                             inf->c.f.tif.predictor = Predictor ;
                             break ;
    case TILE_WIDTH:
    case TILE_LENGTH:
    case TILE_OFSETS:
    case TILE_BYTECOUNTS:    LoggingDo(LL_WARNING, "TIFF tag %u related to tiles is not supported", tag->tag) ;
                             return EIMG_OPTIONNOTSUPPORTED ;
    default:                 LoggingDo(LL_INFO, "Unknown TIFF tag %u", tag->tag) ;
                             break ;
  }
...
int tiff_tags(FILE *stream, INFO_IMAGE *inf)
{
  ...
  inf->nplanes        = BitsPerSample*SamplesPerPixel ;
  ResolutionUnit      = 2 ;
  Compression         = 1 ;
  StripByteCounts     = 0 ;
...
int tiff_identify(char *nom, INFO_IMAGE *inf)
{
  ...
  inf->c.f.tif.byteorder = ByteOrder ;
...
int tiff_read(FILE *stream, INFO_IMAGE* inf)
{
  ...
  LoggingDo(LL_DEBUG, "TIFFRead, RowsPerStrip=%lu, nb_strip=%lu", RowsPerStrip, nb_strip) ;
  if (TStripByteCounts == TSHORT) taille = nb_strip*2 ;
  else                            taille = nb_strip*4 ;
  itab_sbc = (unsigned int *) Xalloc(taille) ;
  if (itab_sbc == NULL) return EIMG_NOMEMORY ;
  ...
  for (i = 0; i < nb_strip; i++)
  {
    ...
    switch(BitsPerSample)
    {
      case 1 : temp = (unsigned char *) i2rout ;
               ...
               break ;
      case 4 : if ( NaturalFormat == NULL ) find2raster() ;
               else
               {
                 long          n ;
                 unsigned char *data ;
      ...
      case 8 : temp = (unsigned char *) i2rout ;
               if (SamplesPerPixel == 1)
               {
                 ...
                 if ( NaturalFormat == NULL ) ind2raster() ;
                 else
                 {
               ...
               else
               {
                 ...
                 if (PlanarConfiguration == 1)
                 {
                   if ( out->fd_nplanes == 16 ) tc24to16_i2r() ;
...
int tags_make(INFO_IMAGE *info, FILE *stream)
{
  ...
  NbTags = 13 ;
  if (info->nplanes >= 16) info->nplanes = 24 ;

  if ((info->palette != NULL) && (info->nplanes != 1)) NbTags++ ;
  if ((info->compress != 5) || (BitsPerSample < 8)) info->c.f.tif.predictor = 1 ;
  if (info->c.f.tif.predictor == 2) NbTags++ ;
  if (info->c.f.tif.byteorder == INTEL)
  {
  ...
  ImageLength  = info->height ;
  StripOffsets = 14+NbTags*sizeof(TIFF_TAG) ;
  taille = img_size((int) ImageWidth, 1, info->nplanes) ;
  ...
  Predictor        = info->c.f.tif.predictor ;
  ByteOrder        = info->c.f.tif.byteorder ;
  ...
  if (info->nplanes != 24)
  {
    ...
    if (info->c.f.tif.byteorder == INTEL)
      ...
      tag.offset = ((unsigned long)info->nplanes) << 16 ;
  }
  else
  {
  ...
  if ((info->palette != NULL) && (info->nplanes != 1) && (info->nplanes != 24))
  {
    tag.tag    = COLOR_MAP ;
    tag.type   = TSHORT ;
...
int tiff_sauve(char *name, MFDB *img, INFO_IMAGE *info, GEM_WINDOW *wprog)
{
  ...
  if ((info->c.f.tif.byteorder != MOTOROLA) && (info->c.f.tif.byteorder != INTEL))
    info->c.f.tif.byteorder = MOTOROLA ;
  ...
  if (info->nplanes == 24)
  {
    ...
    SamplesPerPixel = 3 ;
    nbits           = 8 ;
    if (info->c.f.tif.byteorder == INTEL) nbits = int_reverse(nbits) ;
  ...
  for (i = 0; i < StripsPerImage; i++)
  {
    ...
    (void)GWProgRange( wprog, 1+i, StripsPerImage, NULL ) ;
  }
