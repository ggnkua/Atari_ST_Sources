...
void traite_fullscreen(GEM_WINDOW* gwnd)
{
  ...
  else        form_stop(  MsgTxtGetMsg(hMsg, MSG_NOIMG)  ) ;
}
...
void traite_colle(GEM_WINDOW* cur_window, short x, short y)
{
  ...
  if ((x < xi+wx-1) && (y < yi+wy-1) && (bm != 2))
  {
    ...
    if (zoom_level == 1)
    {
      ...
      else vro_cpyfm(handle, mode, xy, &clipboard.img, &vimage->inf_img.mfdb) ;
    }
...
void traite_imprime(GEM_WINDOW* gwnd)
{
  ...
  {
    case PNOGDOS   : form_stop( MsgTxtGetMsg(hMsg, MSG_PNOGDOS) ) ;
                     break ;
    case PTIMEOUT  : form_stop( MsgTxtGetMsg(hMsg, MSG_PTIMEOUT) ) ;
                     break ;
    case PWRITERR  : form_stop( MsgTxtGetMsg(hMsg, MSG_PWRITERR) ) ;
                     break ;
    case PNOHANDLE : form_stop( MsgTxtGetMsg(hMsg, MSG_PNOHANDLE) ) ;
                     break ;
    case PROTERR   : form_error(8) ;
                     break ;
    case PNODRIVER : form_stop( MsgTxtGetMsg(hMsg, MSG_PNODRIVER) ) ;
                     break ;
  }
}
...
void traite_blocsimple(void)
{
  ...
  do
  {
    ...
    sprintf( buf, MsgTxtGetMsg(hMsg, MSG_XYLH), MIN(xe,x2), MIN(ye,y2), abs(1+x2-xe), abs(1+y2-ye) ) ;
    GWSetWindowInfo( cur_window, buf ) ;
  }
  while (bm == 1) ;
...
GEM_WINDOW* nouvelle_wimage(char* nom, int nx, int ny, int nc, int tpmx, int tpmy)
{
  ...
  if ( gwnd == NULL )
  {
    form_stop(  MsgTxtGetMsg(hMsg, MSG_NOMOREWINDOW)  ) ;
    return NULL ;
  }
  vimage = gwnd->Extension ;
...
char* get_ldv_msgerr(LDV_STATUS err)
{
  ...
  for ( i = 0; i < ARRAY_SIZE(errs); i++ )
    if ( err == errs[i] )
    {
      if ( msgs[i] > 0 ) msg_err = MsgTxtGetMsg( hMsg, msgs[i] ) ;
      break ;
    }

  if ( i == ARRAY_SIZE(errs) ) msg_err = MsgTxtGetMsg( hMsg, MSG_ELDVGENERAL )  ;
