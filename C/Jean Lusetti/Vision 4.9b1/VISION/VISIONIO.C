    ...
    if ( ret != EIMG_SUCCESS )
    {
      ...
      switch( ret )
      {
        ...
        case EIMG_UNKOWNFORMAT :       LoggingDo(LL_WARNING, "Unknown file format for file %s", name) ;
                                       ...
                                       else form_stop(  MsgTxtGetMsg(hMsg, MSG_UNKNOWNFMT) ) ;
                                       break ;
        ...
        case EIMG_NOJPEGDRIVER :       LoggingDo(LL_ERROR, "No JPEG driver for file %s", name) ;
                                       form_stop(  MsgTxtGetMsg(hMsg, MSG_NOJPEGDRIVER) ) ;
                                       break ;
        case EIMG_NOPNGSLB :           LoggingDo(LL_ERROR, "PNG shared library NOT found, please install it") ;
                                       form_stop(  MsgTxtGetMsg(hMsg, MSG_NOPNGSLB) ) ;
                                       break ;
...
/*      log_event( CMD_DISPLAY, name ) ; Logfile feature no longer used */
      if ( inf.multiple_images ) form_stop( MsgTxtGetMsg(hMsg, MSG_MULTIMG) ) ;
    ...
    else
    {
      switch( ret )
      {
        case EIMG_FILENOEXIST :      form_stop(  MsgTxtGetMsg(hMsg, MSG_FILENOTEXIST) ) ;
                                     break ;
        case EIMG_DATAINCONSISTENT : form_stop(  MsgTxtGetMsg(hMsg, MSG_ERRFORMAT) ) ;
                                     break ;
        case EIMG_NOMEMORY :         form_error( 8 ) ;
                                     break ;
        case EIMG_DECOMPRESERR :     form_stop(  MsgTxtGetMsg(hMsg, MSG_DECOMPRESERR) ) ;
                                     break ;
      }
  ...
  else form_stop( MsgTxtGetMsg(hMsg, MSG_NOMOREWINDOW) ) ;
}
...
void traite_slideshow(char* name)
{
  ...
  switch( err )
  {
    case VSSERR_NOERROR    : vss_doslideshow( &vss_info ) ;
                             vss_free( &vss_info ) ;
                             break ;
    case VSSERR_FILE       : form_stop(  MsgTxtGetMsg(hMsg, MSG_FILENOTEXIST) ) ;
                             break ;
...
int input_format(GEM_WINDOW* gwnd)
{
  ...
  if ( !Truecolor && ((inf_img->lformat == IID_TIFF) || (inf_img->lformat == IID_GIF) || (inf_img->lformat == IID_IMG)) )
    if ( inf_img->palette == NULL )
    {
      ...
      if ( inf_img->palette == NULL ) form_error( 8 ) ;
      else
      {
        get_tospalette( pal ) ;
  ...
  if ( inf_img->lformat == IID_DEGAS ) inf_img->compress = 0 ;
...
void traite_sauves(GEM_WINDOW* gwnd, int flag)
{
  ...
  else
  {
    ...
    {
      form_stop(  MsgTxtGetMsg(hMsg, MSG_UNKNOWNSAVE) ) ;
      return ;
    }
  }
        ...
        for ( x = 0; x < dcp.nb_colonnes; x++ )
        {
          ...
          if ( r != 2 )
          {
            if ( dcp.valid )
            {
              ...
              xy[6] = xy[4] + ( xy[2]-xy[0] ) ;
              xy[7] = xy[5] + ( xy[3]-xy[1] ) ;
              vro_cpyfm( handle, S_ONLY, xy, &inf_vimage->mfdb, &raster ) ;
              rx += raster.fd_w ;
            }
            ...
            switch( res )
            {
              ...
              case EIMG_DATAINCONSISTENT :   form_stop( MsgTxtGetMsg(hMsg, MSG_WRITEERROR) ) ;
                                             break ;
              case EIMG_NOMEMORY :           form_error(8) ;
                                             break ;
              case EIMG_DECOMPRESERR :       form_stop( MsgTxtGetMsg(hMsg, MSG_ERRIMGFMT) ) ;
                                             break ;
              case EIMG_UNKOWNFORMAT :       form_stop( MsgTxtGetMsg(hMsg, MSG_UNKNOWNSAVE) ) ;
                                             break ;
              case EIMG_OPTIONNOTSUPPORTED : form_stop( MsgTxtGetMsg(hMsg, MSG_FMTNOSUPPORT) ) ;
                                             break ;
            }
          }
