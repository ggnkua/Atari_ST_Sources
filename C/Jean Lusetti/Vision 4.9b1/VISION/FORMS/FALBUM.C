...
static int cdecl draw_albumimg(PARMBLK* paramblk)
{
 ...
 /* Try understand weird Aranym behavior with memory corruption with JIT
  ...
  if ( wext->imginf.mfdb.fd_addr )
  {
    ...
    GWHideCursor( 1 ) ;
    vro_cpyfm( handle, S_ONLY, xy, &wext->imginf.mfdb, &screen ) ;
    GWShowCursor( 1 ) ;
  ...
  if ( wext->name )
  {
    ...
    *n = 0 ;
    ...
    else
    {
      ...
      switch(err)
      {
        case -1 : form_stop( MsgTxtGetMsg(hMsg, MSG_FILENOTEXIST) ) ;
                  break ;
        case -2 : form_stop( MsgTxtGetMsg(hMsg, MSG_CATBADVER) ) ;
                  break ;
        case -3 : form_error(8) ;
                  break ;
      }
  ...
  switch( obj )
  {
    ...
    case ALBUM_NEW       : strcpy( buf, wext->album_name ) ;
                           ...
                           switch( i )
                           {
                             case  0 : if ( wext->hCat ) catalog_close(wext->hCat) ;
                                       ...
                                       break ;
                             case -1 :
                             case -2 : form_stop( MsgTxtGetMsg(hMsg, MSG_WRITEERROR) ) ;
                                       break ;
                             case -3 : form_error(8) ;
                                       break ;
                           ...
                           if ( file_name(buf, "", buf) )
                           {
                             ...
                             *n = 0 ;
                             ...
                             if ( ( err == 0 ) || ( err == -4 ) ) /* Pas d'erreur ou erreur fseek */
                             {
                               ...
                               if ( err == -4 ) ret = form_stop(  MsgTxtGetMsg(hMsg, MSG_CATERRMINOR)  ) ;
                               if ( ret == 1 ) err = 0 ;
                               if ( !err )
                               {
   ...
   case  ALBUM_ADDIMG    : if ( external_call == NULL )
                           ...
                           if ( do_it )
                           {
                             ...
                             switch(err)
                             {
                               case ECAT_FILENOTFOUND  : if ( !external_call ) form_stop( MsgTxtGetMsg(hMsg, MSG_FILENOTEXIST) ) ;
                                                         break ;
                               case ECAT_NOMEMORY      : if ( !external_call ) form_error(8) ;
                                                         break ;
                               case ECAT_ITEMEXIST     : if ( !external_call ) form_stop( MsgTxtGetMsg(hMsg, MSG_CATITEMEXIST) ) ;
                                                         break ;
                               default                 : if (err && !external_call ) form_stop( MsgTxtGetMsg(hMsg, MSG_CATADDERR) ) ;
...
static int OnTxtBubbleAlb(void* w, int mx, int my, char* txt)
{
  ...
  {
    sprintf( txt, MsgTxtGetMsg(hMsg, MSG_HALMCAT) , 1+obj-ALBUM_CATEGORIE1 ) ;
    return 1 ;
  }
