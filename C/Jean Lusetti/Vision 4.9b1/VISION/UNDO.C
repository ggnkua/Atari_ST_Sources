...
void update_raster(GEM_WINDOW* gwnd, int xmin, int ymin, int xmax, int ymax)
{
  ...
  GWHideCursor( 0 ) ;
  if ( zoom_level == 1 ) vro_cpyfm( handle, S_ONLY, xy, &screen, &vimage->inf_img.mfdb ) ;
  else
  {
    int xyarray[8] ;
    ...
    if ( raster_zoom(&screen, &zoom, xy, -zoom_level, -zoom_level) == 0 )
    {
      ...
      vro_cpyfm(handle, S_ONLY, xy, &zoom, &vimage->inf_img.mfdb) ;
      Xfree(zoom.fd_addr) ;
    }
  }
  GWShowCursor( 0 ) ;
...
void clip_put(GEM_WINDOW* gwnd, int x1, int y1, int x2, int y2)
{
  ...
  vro_cpyfm(handle, S_ONLY, xy, &vimage->inf_img.mfdb, &clipboard.img) ;
...
/*                                          else                             type_alloc = 1 ;*/
                      ...
                      case TUNDO_SMART :  pt_alloc = Xalloc( taille ) ;
                                          ...
                                          {
                                            Xfree( pt_alloc ) ; /* Keep type_alloc=1 */
/*                                            type_alloc = 1 ;*/
                                          }
                                          break ;
...
int undo_disk(GEM_WINDOW* wnd, UNDO_DEF* undo, int* xy, MFDB* raster)
{
  ...
  if ( img.fd_addr == NULL )
  {
    LoggingDo(LL_ERROR, "undo_disk:Can't allocate %ld byte for undo buffer", taille) ;
    form_stop(  MsgTxtGetMsg(hMsg, MSG_UNDOERROR)  ) ;
    ret = -3 ;
  }
  else
  {
    ...
    if ( stream == NULL )
    {
      LoggingDo(LL_ERROR, "undo_disk:Can't open %s for writing", name) ;
      form_stop(  MsgTxtGetMsg(hMsg, MSG_UNDOERROR)  ) ;
      ret = - 1 ;
    }
    else
...
void restore_undo_disk(GEM_WINDOW* wnd, UNDO_DEF* undo, int* xy, MFDB* raster)
{
  ...
  if ( img.fd_addr == NULL )
  {
    LoggingDo(LL_ERROR, "restore_undo_disk:can't allocate %ld bytes", taille) ;
    form_stop(  MsgTxtGetMsg(hMsg, MSG_UNDOERROR)  ) ;
    return ;
  }
  else
  {
    ...
    {
      Xfree( img.fd_addr ) ;
      LoggingDo(LL_ERROR, "restore_undo_disk:can't open %s to read", name) ;
      form_stop( MsgTxtGetMsg(hMsg, MSG_UNDOERROR) ) ;
      return ;
    }
    else
...
int undo_put(GEM_WINDOW* gwnd, int x1, int y1, int x2, int y2, int type_modif, XUNDO* xundo)
{
  ...
  else
  {
    ...
    if ( undo_buf->disk || undo_buf->img.fd_addr )
    {
      ...
      else                  vro_cpyfm( handle, S_ONLY, xy, &vimage->inf_img.mfdb, &undo_buf->img ) ;
    }
  }
...
int uundo_put(GEM_WINDOW* gwnd)
{
  ...
  if ( type_alloc )
  {
    ...
    if ( type_alloc == 1 )
    {
      ...
      vro_cpyfm( handle, S_ONLY, xy, &vimage->inf_img.mfdb, &vimage->Redo->img ) ;
    }
...
void undo_restore(GEM_WINDOW* gwnd)
{
  ...
  {
    rep = form_stop(  MsgTxtGetMsg(hMsg, MSG_UNDOIMG) ) ;
    if ( rep == 2 ) return ;
  }
  ...
  if ( uundo_put( gwnd ) ) form_stop(  MsgTxtGetMsg(hMsg, MSG_UUNDO)  ) ;
  ...
  {
    if ( handle_changed_size( undo, &vimage->inf_img.mfdb ) != 0 ) form_error( 8 ) ;
    else
    {
      xy[0] = 0 ;         xy[1] = 0 ;
      ...
      else              vro_cpyfm( handle, S_ONLY, xy, &undo->img, &vimage->inf_img.mfdb ) ;
    }
  }
  else
  {
...
void traite_refaire(GEM_WINDOW* gw)
{
  ...
  {
    if ( handle_changed_size( vimage->Redo, &vimage->inf_img.mfdb ) != 0 ) form_error( 8 ) ;
    else
    {
      xy[0] = 0 ;                 xy[1] = 0 ;
      ...
      else                      vro_cpyfm(handle, S_ONLY, xy, &vimage->Redo->img, &vimage->inf_img.mfdb) ;
    }
  }
