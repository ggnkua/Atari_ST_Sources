  ...
  if ( num != -1 )
  {
    ...
    switch( num )
    {
      case GOMME       : mouse.gr_monumber = USER_DEF ;
                         ...
                         set_mform(&mouse) ;
                         GWMenuCheck( M_OGOMME ) ;
                         break ;
      case LOUPE       : mouse.gr_monumber = USER_DEF ;
                         if (!config.souris_viseur) CopyMForm( &mouse.gr_mof, &mf_loupe.gr_mof ) ;
                         else                       CopyMForm( &mouse.gr_mof, &mf_viseur.gr_mof ) ;
                         set_mform(&mouse) ;
                         GWMenuCheck( M_OLOUPE ) ;
                         break ;
      case PEINTURE    : mouse.gr_monumber = USER_DEF ;
                         if (!config.souris_viseur) CopyMForm( &mouse.gr_mof, &mf_peint.gr_mof ) ;
                         else                       CopyMForm( &mouse.gr_mof, &mf_viseur.gr_mof ) ;
                         set_mform(&mouse) ;
                         GWMenuCheck( M_OPEINTURE ) ;
                         break ;
      case BLOC       :  if ( !config.souris_viseur ) mouse.gr_monumber = POINT_HAND ;
                         else
                         {
                           mouse.gr_monumber = USER_DEF ;
                           CopyMForm( &mouse.gr_mof, &mf_viseur.gr_mof ) ;
      ...
      case PALETTE    :  CopyMouseDef( &save, &mouse ) ;
                         ...
                         set_mform(&mouse) ;
                         break ;
	    case TRACE     :  GWMenuCheck( M_OTRACE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case POINT      : GWMenuCheck( M_OPOINT ) ;
	                      set_goodmouse() ;
	                      break ;
	    case KLIGNE    :  GWMenuCheck( M_OKLIGNE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case LIGNE      : GWMenuCheck( M_OLIGNE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case CERCLE     : GWMenuCheck( M_OCERCLE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case DISQUE     : GWMenuCheck( M_ODISQUE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case RECTANGLE  : GWMenuCheck( M_ORECTANGLE ) ;
	                      set_goodmouse() ;
	                      break ;
	    case PLAN       : GWMenuCheck( M_OPLAN ) ;
	                      set_goodmouse() ;
	                      break ;
	    case TEXTE      : GWMenuCheck( M_OTEXTE ) ;
	                      set_goodmouse() ;
	                      break ;
      ...
      case AEROSOL    : mouse.gr_monumber = USER_DEF ;
                        GWMenuCheck( M_OAEROSOL ) ;
                        if (!config.souris_viseur) CopyMForm( &mouse.gr_mof, &mf_aerosol.gr_mof ) ;
                        else                       CopyMForm( &mouse.gr_mof, &mf_viseur.gr_mof ) ;
                        set_mform(&mouse) ;
                        break ;
	    case TABLEAU    : GWMenuCheck( M_OTABLEAU ) ;
	                      set_goodmouse() ;
	                      break ;
...
/*
	case M_OPOINT       : OnAppKeyPressed( F1 ) ;
                        break ;
	case M_OTRACE       : OnAppKeyPressed( LSF1 ) ;
                        break ;
	case M_OLIGNE       : OnAppKeyPressed( LSF3 ) ;
                        break ;
	case M_OKLIGNE      : OnAppKeyPressed( F3 ) ;
                        break ;
	case M_ORECTANGLE   : OnAppKeyPressed( LSF4 ) ;
                        break ;
	case M_OPLAN        : OnAppKeyPressed( LSF5 ) ;
                        break ;
	...
	case M_OPALETTE     : OnAppKeyPressed( F10 ) ;
	                      break ;
	case M_OGOMME       : OnAppKeyPressed( LSF6 ) ;
	                      break ;
	case M_OTEXTE       : OnAppKeyPressed( F7 ) ;
	                      break ;
	case M_OPINCEAU     : OnAppKeyPressed( F2 ) ;
	                      break ;
	case M_OAEROSOL     : OnAppKeyPressed( LSF2 ) ;
	                      break ;
	case M_OTABLEAU     : OnAppKeyPressed( LSF8 ) ;
	                      break ;
	case M_OTEMP        : OnAppKeyPressed( F8 ) ;
	                      break ;
	case M_OLOUPE       : OnAppKeyPressed( LSF7 ) ;
	                      break ;
	case M_OMAIN        : OnAppKeyPressed( F9 ) ;
	                      break ;
	case M_OBLOC        : OnAppKeyPressed( LSF9 ) ;
	                      break ;
  }
*/
...
/*  LogMFDB(&vimage->inf_img.mfdb) ;
  ...
  LogMFDB(&screen) ;*/
  vro_cpyfm( handle, S_ONLY, xy, (vimage->zoom_level == 1) ? &vimage->inf_img.mfdb:&vimage->zoom, &screen ) ;
...
void OnVImgDraw(GEM_WINDOW* wnd, int xywh[4])
{
  ...
  if ( vimage->zoom_level == 1 ) vro_cpyfm( handle, S_ONLY, xy, &vimage->inf_img.mfdb, &screen ) ;
