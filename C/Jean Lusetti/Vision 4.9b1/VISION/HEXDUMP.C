...
static void SaveFile(GEM_WINDOW* gwnd, int save_as)
{
  ...
  if ( stream == NULL ) form_stop(  MsgTxtGetMsg(hMsg, MSG_WRITEERROR) ) ;
  else
  {
    ...
    if ( err ) form_stop(  MsgTxtGetMsg(hMsg, MSG_WRITEERROR) ) ;
  }
...
static int OnRButtonDown(void* w, int mk_state, int x, int y)
{
  ...
  for ( i = 0 ; i < ARRAY_SIZE(msgs); i++ )
    txt_popup[ntxt_popup++] = MsgTxtGetMsg( hMsg, msgs[i] ) ;
/*
