...
/*pat*/ rs_global[4]=0;
	...
	if ( *(LONG *)&rs_global[7]) Xfree ((RSXHDR *)*(LONG *)&rs_global[7]);
...
LOCAL WORD rs_read (WORD *global, CONST BYTE *fname)
{
  ...
  WORD ret = TRUE;
	...
	strcpy ((char*)tmpname, (char*)fname);

	if (!shel_find (tmpname))
		return (FALSE);
  ...
  old_dta = Fgetdta ();
  Fsetdta (&dta);
	if (Fsfirst ((char*)tmpname, 0x10) == 0) size = dta.d_length;
	else                                    size = 0;
	Fsetdta (old_dta);

	if (size > sizeof (RSHDR) && (fh = Fopen ((char*)tmpname, 0)) > 0)
	{
		if ((hdr_buf = (RSXHDR *)Xcalloc (size + sizeof (RSXHDR), 1)) != NULL)
		{
			(LONG)rs_hdr = (LONG)hdr_buf + sizeof (RSXHDR);
...
LOCAL VOID fix_chp (WORD *pcoord, WORD flag)
{
	...
	if (!flag && ncoord == 0x50)
		ncoord = xrdesk.g_w;											/* xrdesk.w = largeur de l'‚cran en points */
	else
...
LOCAL VOID _xfill_farbtbl(VOID)
{
  RGB   rgb ;
  WORD  color, c ;
  MFDB  img ;
  ...
  pw      = (WORD*) farbtbl2 ;
  pl      = (LONG*) farbtbl2 ;
  /* Build RGB8 colors for each potential color in palette */
  for ( color = 0; color < 256; color++ )
  {
    c         = color ;
    rgb.Red   = RGBPMToRGB8B[rgb_palette[c][0]] ;
    rgb.Green = RGBPMToRGB8B[rgb_palette[c][1]] ;
    rgb.Blue  = RGBPMToRGB8B[rgb_palette[c][2]] ;
    switch( xscrn_planes )
    {
      case 16:
               *pw++ = RVB16( &rgb ) ;
  ...
  /* Now convert this line of 256 colors into screen format */
  tc_convert( &img ) ;
}
	...
	if ((LONG)cicon_liste - header > rsxhdr->rsh_rssize && (LONG)cicon_liste - header < rs_len)
	{
        ...
        xpixelbytes = xscrn_planes >> 3 ;
				...
				for (i = 0; i < rsxhdr->rsh_nobs; i++)
				{
					...
					{
						if (xadd_cicon (obj, nub++, NULL) == NULL)
						{
							memzero (&((CICON *)*(LONG *)&rs_global[2])[nub-1], sizeof (CICON));
...
{	WORD     i, i2, num = 0;
	...
	for (i = 0; i < num; i++)
	{
		...
		iclen = (cblk->monoblk.ib_wicon >> 3) * cblk->monoblk.ib_hicon;
		p += iclen;
	...
	if (num != NIL)
	{
	  ...
	  for (ob = 0; ob < rsxhdr->rsh_nobs; ob++)
		...
		if ((*(LONG *)rs_global = (LONG)Xcalloc (num * sizeof (OBBLK), 1)) != 0L)
			memzero ((VOID *)*(LONG *)rs_global, num * sizeof (OBBLK));
		if ((*(LONG *)&rs_global[2] = (LONG)Xcalloc (num * sizeof (CICON), 1)) != 0L)
			memzero ((VOID *)*(LONG *)&rs_global[2], num * sizeof (CICON));
		rs_global[4] = num;
	}
/*pat*/	
	else
...
#if COLOR_ICONS == TRUE
GLOBAL CICON* xadd_cicon(OBJECT *obj, WORD nub, USERBLK* ublk)
{
  CICONBLK* cicnblk = (CICONBLK*)obj->ob_spec.index ;
  CICON     Cicon ;
  ...
  MFDB      d;
	...
	len = (cicnblk->monoblk.ib_wicon >> 3) * cicnblk->monoblk.ib_hicon;

	if (ublk) color_icn = &Cicon;
	else      color_icn = &((CICON *)*(LONG *)&rs_global[2])[nub];
	...
	if ((color_icn->col_data = Xcalloc (len * color_icn->num_planes, 1)) == NULL)
		return NULL;
	if (color_icn->sel_data)
	{
		if ((color_icn->sel_data = Xcalloc (len * color_icn->num_planes, 1)) == NULL)
		{
			Xfree (color_icn->col_data);
	...
	if (best_planes > 1)
	{
		if (best_icn->sel_data == NULL)
		{
			...
			if ((color_icn->sel_mask = Xcalloc (len, 1)) == NULL)
			{
				Xfree (color_icn->col_data);
				if (color_icn->sel_data)
					Xfree (color_icn->sel_data);
 				return NULL;
			}
	
			xmax = cicnblk->monoblk.ib_wicon >> 4 ;
  ...
  if ( ublk == NULL )
  {
    OBBLK *ub = (OBBLK *)*(LONG *)rs_global ;
    ublk = &ub[nub].ublk ;
  }
  ublk->ub_parm = obj->ob_spec.index ;
  ublk->ub_code = xdraw_cicon ;
  obj->ob_spec.index  = (LONG)ublk ;
...
/*****************************************************************************/
LOCAL WORD test_rez ()
{	WORD     i, np, color, pxy[8], vro_pxy[8], rgb[3], bpp = 0;
	...
	if (xscrn_planes >= 8)
	{
		...
		{
			color = 0xff;
			memzero (test, xscrn_planes * sizeof (WORD));
		...
		{
			xrect2array (&xrdesk, pxy); /*printf("%c",7);*/
			vs_clip (xvdi_handle, TRUE, pxy); /*FALSE*/
			screen.mp = NULL;
			
			memzero (backup, sizeof (backup));
		
			vswr_mode (xvdi_handle, MD_REPLACE);
			vsl_ends (xvdi_handle, 0, 0);
			vsl_type (xvdi_handle, 1);
			vsl_width (xvdi_handle, 1);
			memzero (pxy, sizeof (pxy));
			memzero (vro_pxy, sizeof (vro_pxy));
      vro_pxy[2] = vro_pxy[6] = 15 ; /* Because on Radeon card vro_cpyfm is buggy if vro_cpyfm is 1 pixel width, see https://groups.google.com/g/atari-mon-amour/c/ckRQcf7tK28 */
			
			graf_mouse (M_OFF, NULL);
		
			pixel.mp = backup;	
			vro_cpyfm (xvdi_handle, S_ONLY, vro_pxy, &screen, &pixel);
      ...
      bpp = (xscrn_planes + 7) >> 3 ;
			...
			vro_cpyfm (xvdi_handle, S_ONLY, vro_pxy, &pixel, &screen);
		
			graf_mouse (M_ON, NULL);
	
			xrect2array (&xrdesk, pxy);
...
/*****************************************************************************/
/* No longer used, replaced by _xfill_farbtbl */
LOCAL VOID xfill_farbtbl ()
{
  WORD color, pxy[8], vro_pxy[8], backup[32], rgb[3];
  LONG test[32] ;
  MFDB screen;
  MFDB pixel = {NULL, 16, 1, 1, 0, 1, 0, 0, 0};
  WORD pixtbl[16] = {0, 2, 3, 6, 4, 7, 5, 8, 9, 10, 11, 14, 12, 15, 13, 16};
...
{
  LONG  x, i, old_len, rest_len, mul[32];
  UWORD *new_data, mask, back[32], old_col[32];
  WORD  got_mem = FALSE;
  MFDB  d;
  ...
  {	
    if ( new_planes == xscrn_planes )
    {
      ...
      if (d.mp == s->mp)
      {
        if ((d.mp = Xcalloc (len * 2 * new_planes, 1)) == NULL) d.mp = s->mp;
        else                                                    got_mem = TRUE;
      }
      ...
      if (d.mp != s->mp && got_mem == TRUE)
      {
		...
		else memcpy (s->mp, col_data, len * 2 * new_planes);
		return;
	}
  ...
  if (new_planes <= 8)
  {
    ...
    d = *s;
    d.ff = 0;
    if ((d.mp = Xcalloc (len * 2 * new_planes, 1)) == NULL)
      d.mp = s->mp;
...
{	LONG  x, i, mul[32], pos;
	...
	if (s->mp == col_data)
	{
		if ((col_data = Xcalloc (len * 2 * s->np, 1)) == NULL)
			return;
	...
	if (old_planes < 8) farbtbl2[(1 << old_planes) - 1] = colback;

	if (memflag) Xfree (col_data);
}
...
/*****************************************************************************/
static void DumpBitBlk(WORD num_planes, WORD* p, WORD w, WORD h)
{
  WORD* l = (WORD*) p ;
  WORD i,j,nwords;
  char buf[1024] ;
  ...
  nwords = (w*num_planes)>>4;
  LoggingDo( LL_DEBUG, "DumpBitBlk%d %p, w=%d, h=%d", num_planes, p, w, h);
  for ( i=0; i < h; i++)
  {
    buf[0] = 0 ;
    for (j=0; j < nwords ; j++)
    {
      sprintf( buf2, "%04X ", *l ) ; l++ ;
      strcat( buf, buf2 ) ;
    }
    LoggingDo(LL_DEBUG, buf) ;
  }
}
