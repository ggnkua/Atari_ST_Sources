...
/* zView extended error codes (Loony Pursell) */
/* Returned in thumbnail field                */
#include "zvec_err.h"

/* This is extracted from zView.h and arranged for internal tools */
/* Purpose is to locate all the required zView specific           */
/* Structures here without official zView headers which are       */
...
typedef struct
{
  ...
  unsigned char	green ;
  unsigned char	blue ;
  /* Note that compiler includes a char filled here to align each structure element on 16bit */
}
ZVCOLOR_MAP ;
...
typedef struct _img_info
{
  ...
  short          thumbnail ;           /* 1 if true else 0.
                                          ...
                                          Note: now also used for reporting error code (Lonny)              */
  ...
  unsigned short real_height ;         /* Real picture height (if the thumbnail variable is 1)   */
  ...
  long  _priv_var ;
  long  _priv_var_more ;
  char  im_pad[30-5] ; /* Structure is larger for IM modules as compression field is 30 bytes long */
}
