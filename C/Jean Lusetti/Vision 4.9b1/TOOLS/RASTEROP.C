...
int raster_symvrt(int handle, int *xyarray, MFDB *src, MFDB *dest)
{
  ...
  if ( (src->fd_addr == NULL) || (dest->fd_addr == NULL) ) GWHideCursor( 0 ) ;
  xy[0] = xyarray[0] ;
  xy[2] = xyarray[2] ;
  xy[4] = xyarray[0] ;
  xy[6] = xyarray[2] ;
  ...
  if ( (src->fd_addr == NULL) || (dest->fd_addr == NULL) ) GWShowCursor( 0 ) ;
...
int raster_vsym(MFDB *img, int *pxy)
{
  ...
  if (img->fd_nplanes == 16)
  {
    ...
    if (img->fd_addr == NULL) GWShowCursor( 0 ) ;
  ...
  rcxy[6] = pxy[2] ;
  img_raz(&temp) ;
  if (img->fd_addr == NULL) GWHideCursor( 0 ) ;
