...
static int LoadIniIntoMemory(INI_FILE* ini_file)
{
  ...
  if ( stream )
  {
    ...
    ini_file->Buffer = (char*) Xalloc( 2+fsize ) ;
    if ( ini_file->Buffer )
    {
      ini_file->Buffer[fsize]   = '\n' ; /* Make sure we end up with a \n and \0 */
      ini_file->Buffer[1+fsize] = 0 ;    /* Make sure we end up with a \n and \0 */
      ini_file->BufferLength    = fsize ;
      ini_file->BufferMaxLength = fsize ;
      if ( fsize )
      {
        if ( fread(ini_file->Buffer, fsize, 1, stream) == 1 )
        {
          if ( ini_file->Buffer[fsize-1] != '\n' )
          {
            ini_file->BufferLength++ ;
            ini_file->BufferMaxLength++ ;
         }
          ret = 0 ;
        }
        else
  ...
  else
  {
    ...
    ini_file->Buffer = (char*) Xalloc( 1+BUFFER_MIN ) ;
    if ( ini_file->Buffer )
    {
      dbg_set_check( ini_file->Buffer, BUFFER_MIN ) ;
      ini_file->Buffer[BUFFER_MIN] = 0 ; /* Make sure we end up with a \0 */
      ini_file->BufferMaxLength    = BUFFER_MIN ;
      ret = 0 ;
    }
  }
...
static int LoadIniSections(INI_FILE* ini_file)
{
  ...
  while ( (ret == 0) && (line < last_c) )
  {
    ...
    {
      *cr = 0 ;
      /* If this line starts a section, it has to begin with '[' */
      if ( line[0] == '[' )
      {
        /* New section begins */
...
static SECTION* GetOverSection(INI_FILE* ini_file)
{
  return &ini_file->Sections[ini_file->nSections] ;
}

static SECTION* GetSection(INI_FILE* ini_file, char* section_name)
{
  SECTION* section ;
  SECTION* over_section = GetOverSection( ini_file ) ;
...
static char* GetItem(INI_FILE* ini_file, char* section_name, char* key)
{
  ...
  if ( section )
  {
    ...
    if ( first_section_entry )
    {
      ...
      section++ ;
      if ( section < GetOverSection( ini_file ) ) last_c = section->ptr ;
      else                                        last_c = ini_file->Buffer + ini_file->BufferLength - 1 - 3 ; /* Last character in buffer is \0 + '[x]' string */
    }
  }
...
static long SetIniKeyXShort(HINI h_ini, char* section_name, char* key_name, char* fmt, unsigned short key_value)
{
  char buf[16] ;

  sprintf( buf, fmt, key_value ) ;
  return SetIniKey( h_ini, section_name, key_name, buf ) ;
}

long SetIniKeyUShort(HINI h_ini, char* section_name, char* key_name, unsigned short key_value)
{
  return SetIniKeyXShort( h_ini, section_name, key_name, "%u", key_value ) ;
}

long SetIniKeyShort(HINI h_ini, char* section_name, char* key_name, short key_value)
{
  return SetIniKeyXShort( h_ini, section_name, key_name, "%d", (unsigned short)key_value ) ;
}
...
long SetIniKeyBool(HINI h_ini, char* section_name, char* key_name, short key_value)
{
  ...
  if ( key_value ) bval = 1 ;
  return SetIniKeyUShort( h_ini, section_name, key_name, bval ) ;
}
