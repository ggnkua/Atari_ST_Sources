...
void OnInitDialogConv(void *w)
{
  ...
  pos = strrchr(vimage->inf_img.filename, '\\') ;
  if (pos != NULL)
  {
    pos++ ;
  ...
  wext->nplan = vimage->inf_img.nplanes ;
...
void rvb2gris(GEM_WINDOW *gwnd)
{
  ...
  mouse_busy() ;
  make_greyimage( &vimage->inf_img.mfdb, &vimage->inf_img ) ;

  if ( vimage->inf_img.mfdb.fd_nplanes <= 8 ) set_imgpalette( vimage ) ;
  else                                        GWRePaint( gwnd ) ;
...
void raz_imgplanes(GEM_WINDOW *gwnd, int new_nplanes)
{
  ...
  taille = vimage->inf_img.mfdb.fd_nplanes-new_nplanes ;
  if (taille <= 0) return ;
  add    = vimage->inf_img.mfdb.fd_nplanes ;
  taille = taille*2 ;
  pt_img = (int *) vimage->inf_img.mfdb.fd_addr ;
  pt_img = pt_img+new_nplanes ;
  toti   = (long)vimage->inf_img.mfdb.fd_h*(long)vimage->inf_img.mfdb.fd_wdwidth ;
  for (i = 0; i < toti; i++)
  {
...
void traite_conversion(GEM_WINDOW *gwnd)
{
  ...
  undo_put( gwnd, 0, 0, vimage->inf_img.mfdb.fd_w-1, vimage->inf_img.mfdb.fd_h-1, UCONVERT, NULL ) ;
  ...
  if ( wext.nplan < wext.newnplan ) vimage->inf_img.nplanes = wext.newnplan ;
  else
  {
    int rep = form_interrogation(2, MsgTxtGetMsg(hMsg, MSG_DECNPLAN) ) ;
    ...
    else
    {
      ...
      wprog = DisplayStdProg( MsgTxtGetMsg(hMsg, MSG_DITHER) , "", "", CLOSER ) ;
      mouse_busy() ;
      if ( dither( DITHER_SEUIL, &vimage->inf_img, &temp, wprog ) == -1 )
      {
        mouse_restore() ;
        form_error(8) ;
      ...
      if (temp.fd_nplanes < nb_plane ) incnbplanes( &temp, nb_planes(nb_colors), NULL ) ;
      vimage->inf_img.mfdb.fd_addr = temp.fd_addr ;
    }
    vimage->inf_img.nplanes = wext.newnplan ;
