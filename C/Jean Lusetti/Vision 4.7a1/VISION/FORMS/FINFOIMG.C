...
static void OnInitDialogInfoImg(void *w)
{
  ...
  size = img_size( vimage->inf_img.mfdb.fd_w, vimage->inf_img.mfdb.fd_h, nb_plane ) ;
...
static int get_informations(INFO_IMAGE* inf, MLINES_PARAMS* mlp)
{
  ...
  if ( informations )
  {
    memset( mlp, 0, sizeof(MLINES_PARAMS) ) ;
    ...
    strncpy( mlp->title, MsgTxtGetMsg(hMsg, MSG_IMGADDPROP), sizeof(mlp->title)-1 ) ;
    strcpy( mlp->ok_caption, "OK" ) ;
    strcpy( mlp->cancel_caption, MsgTxtGetMsg(hMsg, MSG_CANCEL) );
  ...
  switch( obj )
  {
    case INFO_UNITE :   deselect(adr_inf, INFO_UNITE) ;
     ...
     case INFO_PLUS:    if ( get_informations( info_image, &mlp ) == IDOK )
                        ...
                        xobjc_draw( wnd->window_handle, adr_inf, obj ) ;
                        break ;
