...
static short AdjustPath(char* path, char* def_dir)
{
  ...
  if ( (path[0] == 0) || !FolderExist(path) )
  {
    ...
    sprintf( tmp, "%s\\%s", init_path, def_dir ) ;
...
int ajust_pathconfig(void)
{
  ...
  save_ini |= AdjustPath( config.path_img, "" ) ;
  save_ini |= AdjustPath( config.path_album, "" ) ;
...
static int OnAppMsgUser(void* app, int id, int* mesg)
{
  ...
  if ( id == WM_UPDATE_AVAILABLE )
  {
    ...
    if ( config.vu_choice ) do_it = ( form_interrogation( 2, MsgTxtGetMsg(hMsg, MSG_UPDATEAVAILABLE) ) == 1 ) ;
    if ( do_it )
    {
      short status ;
