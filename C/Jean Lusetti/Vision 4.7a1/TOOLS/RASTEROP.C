...
int raster_r90(MFDB *in, MFDB *out, GEM_WINDOW *wprog)
{
  ...
  if ( out->fd_nplanes <= 8 )
  {
    ...
    for ( col = 0; !cancel && (col < in->fd_w); col++ )
    {
      ...
      ind2raster() ;
      cancel = GWProgRange( wprog, col, in->fd_w, NULL ) ;
    }
  }
...
int raster_r270(MFDB *in, MFDB *out, GEM_WINDOW *wprog)
{
  ...
  if ( out->fd_nplanes <= 8 )
  {
    ...
    for ( col = in->fd_w-1; !cancel && (col >= 0); col-- )
    {
      ...
      ind2raster() ;
      cancel = GWProgRange( wprog, in->fd_w-1-col, in->fd_w, NULL ) ;
    }
  }
  else if (out->fd_nplanes == 16)
  {
...
int raster_rfree(MFDB *in, MFDB *out, int angle, GEM_WINDOW *wprog)
{
  ...
  if (out->fd_nplanes <= 8)
  {
    ...
    for (ligne = 0; !cancel && (ligne < out->fd_h); ligne++)
    {
      ...
      ind2raster() ;
      cancel = GWProgRange( wprog, ligne, out->fd_h, NULL ) ;
    }
  }
  else if (out->fd_nplanes == 16)
  {
...
int find_rgb32(long rvb32)
{
  ...
  LoggingDo(LL_DEBUG, "find_rgb32 on %04lX", rvb32) ;
...
int FindScreenOrg(RVB_ORG *rvb_org)
{
  RVB_ORG*       def_rvborg ;
  MFDB           mfdb ;
  long           original_pixel[16] ; /* Better allocate a 16 pixel MFDB for vro_cpyfm calls */
  int            xy[8] ;
  int            red[]   = { 1000, 0, 0 } ;
  int            green[] = { 0, 1000, 0 } ;
  int            blue[]  = { 0, 0, 1000 } ;
  ...
  if ( Truecolor )
  {
    long tested_pixel[16] ;   /* Better allocate a 16 pixel MFDB for vro_cpyfm calls */
    ...
    switch( nb_plane )
    {
      ...
      case 24 :
                ...
                pset( x, y ) ;
                vro_cpyfm( handle, S_ONLY, xy, &screen,  &mfdb ) ;
                rvb_org->RRed = find_rgb24( rvb24 ) ;

                vs_color( handle, 1, green ) ;
                pset( x, y ) ;
                vro_cpyfm( handle, S_ONLY, xy, &screen,  &mfdb ) ;
                rvb_org->RGreen = find_rgb24( rvb24 ) ;

                vs_color( handle, 1, blue ) ;
                pset( x, y ) ;
                vro_cpyfm( handle, S_ONLY, xy, &screen,  &mfdb ) ;
                rvb_org->RBlue = find_rgb24( rvb24 ) ;
    ...
    LoggingDo( LL_INFO, "FindScreenOrg(TC%d): UseStdVDI=%d, This machine org.: (%d %d %d) Default org.: (%d %d %d) 32K:%d", nb_plane, UseStdVDI,
  ...
  else
  {
    ...
    LoggingDo(LL_INFO, "FindScreenOrg(bitplanes#%d): StdVDI=%d", nb_plane, UseStdVDI) ;
  }
...
void make_rgbpal2tc(unsigned char* rgbapal, int nb_cpal, void* tctab, int nplanes)
{
  ...
  if ( nb_cpal == 0 )
  {
    ...
    /* nb_cpal = 2 ; */ /* Does caller need this? Hopes not */
