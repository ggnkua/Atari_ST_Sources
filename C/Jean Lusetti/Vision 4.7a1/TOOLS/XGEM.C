...
static void remove_tail_aslash(char* string)
{
...
static void remove_after_last_slash(char* string)
{
  char* lslash = strrchr( string, '\\' ) ;
...
int filefolder_name(char *masque, char *def, int want_folder, char *name)
{
  ...
  len = strlen( path ) ;
  if ( (len > 4) && (path[2] == '\\') && (path[3] == '\\') ) memmove( &path[3], &path[4], len-3) ;
  if ( def ) strcpy( fnam, def ) ;
  else       fnam[0] = 0 ;
  remove_tail_aslash( path ) ;
  ...
  if ( ret && (button == 1) )
  {
    ...
    /* Remove tailing '\\' is any */
    remove_after_last_slash( path ) ;
    remove_tail_aslash( fnam ) ;
...
char* reduce_string_if_required(char* string, char* buffer, size_t dst_len)
{
  ...
  {
    size_t nchars_to_copy_per_side = (dst_len-3)/2 ;
...
void get_machineinformation(void)
{
  ...
  if ( cookie ) MachineInfo.machine = cookie->data ;
  ...
  if ( cookie ) MachineInfo.fpu = cookie->data ;

  cookie = cookie_find("_VDO") ;
  if ( cookie ) MachineInfo.video = cookie->data ;

  cookie = cookie_find("_SND") ;
  if ( cookie ) MachineInfo.sound = cookie->data ;
...
int gem_init(void)
{
  ...
  {
    /* appl_getinfo only avaiable for AES >= 4.00 but stock AES Falcon is 3.30                          */
    /* However it is possible to install EmuTOS on a FAlcon but in this case non popup support          */
  ...
  work_in[10] = 2 ; /* Raster coordonates */
...
static void justify_text(OBJECT* obj, char* string)
{
  ...
  switch ( ted->te_just )
  {
    ...
    case TE_RIGHT: pos = gui_str_len-str_len ;
                   break ;
    default :      pos = (gui_str_len-str_len)/2 ; /* Center */
                   break ;
  }
...
{
  short internet_may_be_here ;

  if ( Multitos ) internet_may_be_here = 1 ;
  else            internet_may_be_here = (cookie_find("STiK") != NULL) ;
...
long XPexec(int mode, char* name, void* cmdline, void* envptr)
{
  ...
  if ( name[1] != ':' )
  {
    ...
    if ( path )
    {
      ...
      LoggingDo(LL_DEBUG, "XPexec:PATH=%s", path) ;
      while ( c && !found )
      {
        size = (size_t) (c-cp) ;
        strncpy( tmp, cp, size ) ;
        ...
        LoggingDo(LL_DEBUG, "XPexec:Testing presence of %s", full_name) ;
  ...
  return Pexec( mode, full_name, cmdline, envptr ) ;
}
