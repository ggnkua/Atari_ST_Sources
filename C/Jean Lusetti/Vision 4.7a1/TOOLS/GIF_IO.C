...
#define DCPRS_SIZE    (80L*1024L) /* 64000 should be enough but LZW decompress might overflow */
...
static int is_interleaved(INFO_IMAGE* inf)
{
  int interleaved = 0 ;
  ...
  if ( inf )
  {
    ...
    if ( specific && (specific->descriptor.color_inf & 0x40) ) interleaved = 1 ;
  }
...
static int gif_decompress(unsigned char *buffer, long taille, INFO_IMAGE* inf)
{
  ...
  while ( ( pt <= last_data ) && ( *pt != GIF_TERMINATOR ) && ( nb_tpts < total ) )
  {
    if ( ( *pt > 1 ) && ( *pt < 13 ) ) /* Code Size */
    {
      ...
      do
      {
        ...
        {
          LoggingDo(LL_WARNING, "LZW overflow: buffer is %ld bytes, %ld inflated", DCPRS_SIZE, nb_pts) ;
          nb_pts = DCPRS_SIZE ;
        }
  ...
  if ( is_interleaved( inf ) ) ret = de_entrelace( inf ) ;

  return ret ;
}

int pos_on_image_descriptor(INFO_IMAGE* inf, GIF_DESCRIPTOR* descriptor, unsigned char* buf )
{
  union
  {
    struct
...
int gif_identify(char* name, INFO_IMAGE* inf)
{
  ...
  nplanes = 1 + (specific->header.color_inf & 0x07) ;
  if ( (nplanes != 1) && (nplanes != 4) && (nplanes != 8) )
  {
    free_info( inf ) ;
    return EIMG_OPTIONNOTSUPPORTED ;
  }

  memcpy(&mot_int, &specific->header.width, 2) ;
  specific->header.width = mot_int.i.lsb+256*mot_int.i.msb ;
  memcpy(&mot_int, &specific->header.height, 2) ;
  specific->header.height = mot_int.i.lsb+256*mot_int.i.msb ;
  ...
  if ( pos_on_image_descriptor( inf, &specific->descriptor, buf ) != 0 )
  {
    free_info( inf ) ;
    return EIMG_DATAINCONSISTENT ;
  }
  ...
  if ( specific->descriptor.color_inf & 0x80 )
    inf->nb_cpal = 1 << ( 1 + (specific->descriptor.color_inf & 0x07) ) ;
  inf->nplanes  = 1 + (specific->header.color_inf & 0x07) ;
