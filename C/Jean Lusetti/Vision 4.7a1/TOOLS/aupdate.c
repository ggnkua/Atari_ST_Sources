...
static short AUGetCmdLine4PExec(char* updater, char* url, char* cmd_line, char* url_file, char* local_file)
{
  char buf[256] ;
  ...
  if ( strstr( updater, "curl" ) )      sprintf( &cmd_line[1], "-L --get %s%s --output %s", buf, url_file, local_file ) ;
  else if ( strstr( updater, "wget" ) ) sprintf( &cmd_line[1], "%s%s -O %s", buf, url_file, local_file ) ;
  else                                  return -1 ;

  cmd_line[0] = strlen( &cmd_line[1] ) ; /* Pascal convention */
...
short AUCheckUpdate(AU_CONTEXT* AUContext)
{
  ...
  if ( AUGetCmdLine4PExec( AUContext->updater, AUContext->url, cmd_line, AUContext->inet_app_fversion, AUContext->local_version_file ) != 0 ) return AU_UPDATERUNKNOWN ;
...
static short AUUpdateFile(char* filename, AU_CONTEXT* AUContext)
{
  ...
  if ( AUGetCmdLine4PExec( AUContext->updater, AUContext->url, cmd_line, filename, src ) != 0 ) return -1 ;
...
short AUPerformUpdate(AU_CONTEXT* AUContext)
{
  ...
  GWProgRange( AUContext->wprog, pc, -100L, NULL ) ;
