...
static int tiff_read(INFO_IMAGE* inf)
{
  ...
  for (i = 0; i < nb_strip; i++)
  {
    ...
    if ( tiff_spec->Compression == TC_LZW )
    {
      ...
      if ( i2r_nb > size )
      {
        LoggingDo(LL_WARNING, "LZW overflow on row #%lu/%lu: buffer is %ld bytes, %ld inflated", i, nb_strip, size, i2r_nb) ;
        i2r_nb = size ;
      }
      if (i2r_nb == 0)
      {
      ...
      case 1 : temp = (unsigned char*) i2rout ;
               if ( tiff_spec->Compression == TC_NONE )
               {
                 ...
                 else
                 {
                   ...
                   for (i = 0; i < tiff_spec->RowsPerStrip; i++)
                   {
...
static int tags_make(INFO_IMAGE* info, FILE* stream)
{
  ...
  if ( tiff_spec->RowsPerStrip > tiff_spec->ImageLength ) tiff_spec->RowsPerStrip = tiff_spec->ImageLength/2 ;
...
int tiff_sauve(char* name, MFDB* img, INFO_IMAGE* info, GEM_WINDOW* wprog)
{
  ...
  memset( &local_tiff_spec, 0, sizeof(TIFF_SPECIFIC) ) ;
