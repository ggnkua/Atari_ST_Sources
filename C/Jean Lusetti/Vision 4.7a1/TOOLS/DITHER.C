...
static int dither_seuil(INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  MFDB*         in = &inf->mfdb ;
  long          taille ;
  int           *pt_ligne ;
  ...
  if ( inf->natural_format ) nb_px = inf->natural_width ;
  else                       nb_px = in->fd_w ;
  ...
  {
    buffer = inf->natural_format ;
    i2r_init( out, inf->natural_width, 1 ) ;
  }
  else
  {
  ...
  if (in->fd_nplanes <= 8)
  {
    ...
    for (y = 0; y < in->fd_h; y++)
    {
      ...
      ind2raster() ;
      if ( !inf->natural_format ) pt_ligne += taille ;
      else                       (unsigned char*)i2r_data += inf->natural_width ;
  ...
  if ( !inf->natural_format ) Xfree(buffer) ;

  return 0 ;
}

static int dither_gris(INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  MFDB*         in = &inf->mfdb ;
  INFO_IMAGE    new_pal ;
  long          taille ;
...
static int dither_tc_216(INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  MFDB*         in = &inf->mfdb ;
  MFDB          img ;
...
/* Unfinished, also considering how to call/test it as output MFDB will be 1 plane */
static int dither_bichro(INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  MFDB*          in = &inf->mfdb ;
  size_t         inline_inc = in->fd_wdwidth*in->fd_nplanes ;
  int            ret = -1 ;
  unsigned char* word_out ;
  ...
  out->fd_nplanes = 1 ;
  out->fd_addr    = img_alloc( out->fd_w, out->fd_h, out->fd_nplanes ) ;
  if ( out->fd_addr == NULL ) return ret ;
  img_raz( out ) ;
  ...
  {
    unsigned short* pt_line = in->fd_addr ;
    unsigned short* pt_pixel ;
    unsigned short  r, g, b, pixel ;
    int             word_out_add = out->fd_wdwidth-(out->fd_w<<4) ; /* In case fd_w is not aligned on 16 pixels */
    int             line ;

    for ( line = 0; line < in->fd_h; line++, pt_line += inline_inc )
    {
      bitmask = 0x8000 ;
      for ( pt_pixel = pt_line ; pt_pixel < pt_line+out->fd_w; pt_pixel++ )
      {
        ...
        r       = pixel >> 5 ;   /* 5bit red */
        if ( r+g+b < 16*3 ) /* Less than average grey on 3 5bit components */
          *word_out |= bitmask ;
        bitmask >>= 1 ;
        if ( bitmask == 0 )
        {
      ...
      (void)GWProgRange( wprog, line, out->fd_h, NULL ) ;
      word_out += word_out_add ;
    }
    ret = 0 ;
  }
  else if ( in->fd_nplanes == 24 )
...
int dither(int method, INFO_IMAGE* inf, MFDB* out, GEM_WINDOW* wprog)
{
  ...
  switch( method )
  {
    case DITHER_216   : ret = dither_tc_216( inf, out, wprog ) ;
                        break ;

    case DITHER_SEUIL : ret = dither_seuil( inf, out, wprog ) ;
                        break ;

    case DITHER_GRIS  : ret = dither_gris( inf, out, wprog ) ;
                        break ;

/*    case DITHER_BICHRO: ret = dither_bichro( inf, out, wprog ) ;
                        break ;*/
 }
