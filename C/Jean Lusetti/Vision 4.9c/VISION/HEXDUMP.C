...
#define MAX_SEARCH_RESULTS 1000L
typedef struct
{
...
static void SaveFile(GEM_WINDOW* gwnd, int save_as)
{
  ...
  if ( stream == NULL ) vform_stop( MSG_WRITEERROR ) ;
  else
  {
    ...
    if ( err ) vform_stop( MSG_WRITEERROR ) ;
  }
...
static void ShowCursor(GEM_WINDOW* gwnd, char show)
{
  ...
  if ( hex_dump->sel_is_xor != show )
  {
    ...
    GWHideCursor() ;
    svro_cpyfm( D_INVERT, xy, &screen, &screen ) ;
    GWShowCursor() ;
...
static void SmartInfos(GEM_WINDOW* gwnd)
{
  ...
  else
  {
    ...
    hex_dump->nspat-- ;
    if ( hex_dump->nsfound > 0 ) sprintf( buf, "%s Pattern '%s' found at offset $%lX(%ld), occurence %ld/%ld/%ld", stype, GetStringPattern(&hex_dump->parm_pattern, tmp), offset, offset, 1+hex_dump->sindex, hex_dump->nsfound, hex_dump->nsfound_total ) ;
    else                         sprintf( buf, "%s Pattern '%s' not found", stype, GetStringPattern(&hex_dump->parm_pattern, tmp) ) ;
  }
  GWSetWindowInfo( gwnd, buf ) ;
}

static void CSmartInfos(GEM_WINDOW* gwnd, long old_xcursor, long old_ycursor, long old_ystart)
{
  HEX_DUMP* hex_dump = (HEX_DUMP*) gwnd->Extension ;
...
static void FindNextPattern(GEM_WINDOW* gwnd, int local_offset)
{
  ...
  if ( hex_dump->sindex >= hex_dump->nsfound )
  {
    ...
    hex_dump->nsfound_total = hex_dump->nsfound ;
    if ( hex_dump->nsfound > MAX_SEARCH_RESULTS )
    {
      LoggingDo(LL_INFO, "Note that only %ld found patterns have been stored on a total of %ld", MAX_SEARCH_RESULTS, hex_dump->nsfound ) ;
      hex_dump->nsfound = MAX_SEARCH_RESULTS ;
    }
  ...
  if ( hex_dump->nsfound > 0 )
  {
    ...
    if ( parms->count_them )
    {
      LoggingDo(LL_INFO, "%s Pattern %s found at offset $%lX(%ld), occurence %ld/%ld/%ld", stype, GetStringPattern(parms, buf), offset, offset, 1+hex_dump->sindex, hex_dump->nsfound, hex_dump->nsfound_total ) ;
      hex_dump->nspat = 5 ;
    }
    else
...
static void OnSize(void* wnd, int xywh[4])
{
  ...
  CSmartInfos( gwnd, old_xcursor, old_ycursor, old_ystart ) ;
/*  if ( (old_xcursor != hex_dump->xcursor) || (old_ycursor != hex_dump->ycursor) ) SmartInfos( gwnd ) ;
  if ( old_ystart != hex_dump->ystart ) SmartRePaint( gwnd, old_ystart ) ;
*/
  if ( w > w_max ) w = w_max ;
...
static int OnKeyPressed(void* wnd, int key)
{
  ...
  if ( !repaint_done ) CSmartInfos( gwnd, old_xcursor, old_ycursor, old_ystart ) ;
/*  {
      if ( (old_xcursor != hex_dump->xcursor) || (old_ycursor != hex_dump->ycursor) ) SmartInfos( gwnd ) ;
      if ( old_ystart != hex_dump->ystart ) SmartRePaint( gwnd, old_ystart ) ;
  }*/
...
static int OnArrow(void* wnd, int action)
{
  GEM_WINDOW*   gwnd = (GEM_WINDOW*) wnd ;
  HEX_DUMP*     hex_dump = (HEX_DUMP*) gwnd->Extension ;
  long          old_xcursor, old_ycursor, old_ystart ;
  ...
  switch( action )
  {
    case WA_RTLINE : hex_dump->xcursor++ ;
                     if ( hex_dump->xcursor >= hex_dump->sizeof_raw_offset_array ) hex_dump->xcursor = hex_dump->sizeof_raw_offset_array - 1 ;
                     plus = 1 ;
                     break ;

    case WA_LFPAGE:  hex_dump->xcursor = SearchNextBlock( hex_dump, hex_dump->xcursor, -1L ) ;
                     break ;

    case WA_LFLINE : hex_dump->xcursor-- ;
                     if ( hex_dump->xcursor < 0 ) hex_dump->xcursor = 0 ;
                     break ;

    case WA_RTPAGE:  hex_dump->xcursor = SearchNextBlock( hex_dump, hex_dump->xcursor, 1L ) ;
                     break ;

    case WA_UPLINE : hex_dump->ycursor-- ;
                     if ( hex_dump->ycursor < 0 ) hex_dump->ycursor = 0 ;
/*                     if ( hex_dump->ystart > 0 ) hex_dump->ystart-- ;*/
                     break ;

    case WA_UPPAGE:  hex_dump->ystart  -= hex_dump->nb_lines_on_window ;
                     hex_dump->ycursor -= hex_dump->nb_lines_on_window ;
                     break ; 

    case WA_DNLINE : hex_dump->ycursor++ ;
                     if ( hex_dump->ycursor > hex_dump->total_nb_lines ) hex_dump->ycursor = hex_dump->total_nb_lines ;
/*                     if ( hex_dump->ystart < hex_dump->total_nb_lines - hex_dump->nb_lines_on_window ) hex_dump->ystart++ ;*/
                     break ;

    case WA_DNPAGE:  hex_dump->ystart  += hex_dump->nb_lines_on_window ;
                     hex_dump->ycursor += hex_dump->nb_lines_on_window ;
                     break ; 
  }
...
static int OnLButtonDown(void* wnd, int mk_state, int xm, int ym)
{
  ...
  CSmartInfos( gwnd, old_xcursor, old_ycursor, hex_dump->ystart ) ;
  /*if ( (old_xcursor != hex_dump->xcursor) || (old_ycursor != hex_dump->ycursor) ) SmartInfos( gwnd ) ;*/
...
static int OnRButtonDown(void* w, int mk_state, int x, int y)
{
  ...
  HEX_DUMP*   hex_dump = (HEX_DUMP*) gwnd->Extension ;
  OBJECT*     popup ;
  int         msgs[] = { MSG_HPLBYTE, MSG_HPLWORD, MSG_HPLLONG, MSG_HPLMIXED, MSG_HPLFIND, MSG_HPLFINDNEXT } ;
  ...
  for ( i = 0 ; i < ARRAY_SIZE(msgs); i++ )
    txt_popup[ntxt_popup++] = vMsgTxtGetMsg( msgs[i] ) ;
