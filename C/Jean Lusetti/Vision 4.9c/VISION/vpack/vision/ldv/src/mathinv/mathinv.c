...
void init_computation(short xc, short yc, long power, MFDB* in, RECT* rect)
{
  long maxrd2 ;
...
LDV_STATUS bpRun(MFDB *in, long power, short xc, short yc, MFDB *out, RECT* rect, short use_sym)
{
  MFDB          dst_line ;
  ...
  init_computation(xc, yc, power, in, rect) ;
...
LDV_STATUS Run16(MFDB *in, long power, short xc, short yc, MFDB *out, RECT* rect, short use_sym)
{
  LDV_STATUS    status = ELDV_NOERROR ;
...
LDV_STATUS Run32(MFDB *in, long power, short xc, short yc, MFDB *out, RECT* rect, short use_sym)
{
  LDV_STATUS    status = ELDV_NOERROR ;
...
short GetSymetryUsageFromIni(void)
{
  ...
  if ( path_ldv && language && Vapi->InGetKeyFromFile )
  {
    ...
    sprintf( filename, "%s\\%s\\MATHINV.INI", path_ldv, language ) ;
    ...
    if ( !found )
    {
      sprintf( filename, "%s\\MATHINV.INI", path_ldv ) ;
...
LDV_STATUS cdecl Run(LDV_IMAGE *in, LDV_PARAMS *params, LDV_IMAGE *out)
{
  ...
  LDV_STATUS    (*RunRoutine)(MFDB *in, long power, short xc, short yc, MFDB *out, RECT* rect, short use_sym) ;
  LDV_STATUS     status = ELDV_NOERROR ;
  long           power, np=0 ;
  short          xc, yc, n ;
  ...
  for ( n=0; (status != ELDV_CANCELLED) && (n < sgProps.nrect); n++)
  {
    ...
    if ( VapiLogLevel >= LL_PERF )
    {
      ...
      t0 = clock() ;
      status = RunRoutine( &in->Raster, power, xc, yc, &out->Raster, &sgProps.rect[n], (sgProps.nsym == n) ) ;
