...
static void adapt_display(GEM_WINDOW* wnd, int set_window)
{
  ...
  GWObjcDraw( wnd, bobj, IMGB_TEXT ) ;

  i = 1 ;
  if ( wb->pzoom_index > 1 ) i += wb->pzoom_index ;
  write_text( bobj, IMGB_SCALE, wdlg->popup_zoom[i].ob_spec.free_string ) ;
/*
  ...
  GWObjcDraw( wnd, bobj, IMGB_SCALE ) ;
}
...
static void new_edit_vximage(WBROWSER* wb)
{
  ...
  if ( new_wnd )
  {
    ...
    svro_cpyfmSO( xy, raster, &new_vimage->inf_img.mfdb ) ;
  }
  else form_error_mem() ;
}
...
static void update_nav(GEM_WINDOW* wnd)
{
  ...
  GWObjcDraw( wnd, bobj, IMGB_PREVIOUS ) ;
  GWObjcDraw( wnd, bobj, IMGB_NEXT ) ;
  GWObjcDraw( wnd, bobj, IMGB_TEXT2 ) ;
  GWObjcDraw( wnd, bobj, IMGB_EDIT ) ;
  GWObjcDraw( wnd, bobj, IMGB_DELETE ) ;
}
...
int ObjectNotifyIBrowser(void* gw, int obj, void* external_call)
{
  ...
  switch( obj )
  {
    ...
    case IMGB_DELETE   : deselect( bobj, obj ) ;
                         if ( wb->fimg_list )
                           if ( form_interrogation( 2, vMsgTxtGetMsg(MSG_CONFIRMDEL) ) == 1 )
                             if ( Fdelete( wb-> nom ) ) vform_stop( MSG_WRITEERROR ) ;
                         GWObjcDraw( wnd, bobj, obj ) ;
                         break ;
#ifndef __NO_PRINT
...
static int OnKeyPressedIBrowser(void* w, int key)
{
  ...
  switch( wnd->original_key ) /* Cursor key must rely on original_key */
  {
  ...
  if ( code == GW_EVTCONTINUEROUTING )
  {
    ...
    else if ( key == IMPRIMER )
    {
      OnObjectNotifyIBrowser( w, IMGB_PRINT ) ;
      code = GW_EVTSTOPROUTING ;
    }
  }
