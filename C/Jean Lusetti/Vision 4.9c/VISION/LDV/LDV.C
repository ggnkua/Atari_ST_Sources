...
{
  raster->fd_addr = Xalloc( img_size( raster->fd_w, raster->fd_h, raster->fd_nplanes ) ) ;
...
static int LDVSetValueFromString(char* buf, int type, LDV_BASICPARAM* p)
{
  ...
  switch( type )
  {
    ...
    case LDVT_FILE  :
    case LDVT_TEXT  : if ( buf[0] ) p->str = Xstrdup( buf ) ;
                       break ;
...
static void LDVFreeParameters(LDV_MODULE* ldv_module)
{
  ...
  for ( i = 0; i < LDVNBMAX_PARAMETERS; i++ )
  {
    ...
    if ( ( (param->Type == LDVT_FILE) || (param->Type == LDVT_TEXT) ) && param->DefaultValue.str )
    {
...
void LDVFreeDynParams(LDV_PARAMS* params)
{
  ...
  for ( i = 0; i < LDVNBMAX_PARAMETERS; i++ )
  {
    if ( ( (params->ParamType[i] == LDVT_FILE) || (params->ParamType[i] == LDVT_TEXT) ) && params->Param[i].str )
    {
...
long cdecl ImSave(char* filename, void* v, char* save_options)
{
  ...
  err = vsave_picture( filename, &inf->mfdb, inf, NULL ) ;
      ...
      case LDVF_STDFORMAT   : if ( ldv_img->Raster.fd_nplanes <= 8 )
                              {
                                ...
                                {
                                  std_img.fd_stand = 1 ;
                                  svr_trnfm( &ldv_img->Raster, &std_img ) ;
...
LDV_STATUS LDVDo(LDV_MODULE* ldv, LDV_PARAMS* pldv_params, LDV_IMAGE* pldv_img, int flags)
{
     ...
     )
  {
    ...
    svro_cpyfmSO( xy, &ldv_img->Raster, &img_sel.Raster ) ;
    ldv_img = &img_sel ;
  ...
  LoggingDo(LL_DEBUG, "Original format: %d, to provide to LDV: %d", img_format_src, img_format_ldv) ;
  ...
  if ( LDV_SUCCESS( err ) )
  {
    ...
    if ( LDV_SUCCESS( BuildLdvImgFromLDV( &out, img_format_src ) ) && LDV_SUCCESS( err ) )
    {
      if ( img_sel.Raster.fd_addr )
      {
        ...
        xy[6] = xy[4] + w - 1 ;
        xy[7] = xy[5] + h - 1 ;
        svro_cpyfm( mode, xy, &out.Raster, &pldv_img->Raster ) ;
      ...
      if ( caps->Flags & LDVF_AFFECTPALETTE ) memcpy( &pldv_img->Palette, &out.Palette, sizeof(LDV_PALETTE) ) ;
