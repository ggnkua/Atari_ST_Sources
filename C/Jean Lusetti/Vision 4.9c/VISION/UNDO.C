...
int handle_changed_size(UNDO_DEF *undo, MFDB *current_img)
{
  if ( size_has_changed( undo, current_img ) )
  {
    ...
    void* pt = Xalloc( img_size( undo->w, undo->h, screen.fd_nplanes ) ) ;
    ...
    current_img->fd_wdwidth = W2WDWIDTH( current_img->fd_w ) ;
...
void update_raster(GEM_WINDOW* gwnd, int xmin, int ymin, int xmax, int ymax)
{
  ...
  svs_clipON( cxy ) ;
  GWHideCursor() ;
  if ( zoom_level == 1 ) svro_cpyfmSO( xy, &screen, &vimage->inf_img.mfdb ) ;
  else
  {
    int xyarray[8] ;
...
void clip_put(GEM_WINDOW* gwnd, int x1, int y1, int x2, int y2)
{
  ...
  if (h < MINIH)       h = MINIH ;
  clipboard.img.fd_addr = img_alloc( larg, h, screen.fd_nplanes ) ;
  ...
  clipboard.img.fd_wdwidth = larg >> 4 ;
  clipboard.img.fd_nplanes = screen.fd_nplanes ;
  ...
  svro_cpyfmSO( xy, &vimage->inf_img.mfdb, &clipboard.img ) ;
  ...
  if ( config.clip_gem == 1 )
  {
    ...
    if ( clbd_getpath( name ) == 0 )
    {
      ...
      info.width    = clipboard.img.fd_w ;
      info.height   = clipboard.img.fd_h ;
      info.compress = 1 ;
      strcat( name, "SCRAP." ) ;
      ...
      if ( img_module )
      {
        ...
        vsave_picture( name, &clipboard.img, &info, NULL ) ;
      }
    }
    mouse_restore() ;
  }
...
void clip_putlasso(GEM_WINDOW* gwnd, int x1, int y1, int x2, int y2, MFDB* mask, MFDB* masked_img)
{
  ...
  CopyMFDB( &clipboard.img, masked_img ) ;
  CopyMFDB( &clipboard.mask, mask ) ;
  clipboard.gwindow = gwnd ;
  clipboard.x   = x1 ;
  clipboard.y   = y1 ;
  clipboard.w   = 1+x2-x1 ;
  ...
  if ( config.clip_gem == 1 )
  {
    ...
    if ( clbd_getpath( name ) == 0 )
    {
      ...
      vsave_picture( name, &clipboard.img, &vimage->inf_img, NULL ) ;
    }
    mouse_restore() ;
  }
}

int must_alloc(int modif, long taille)
{
  size_t memory_left ;
  size_t limit_bytes ;
  int    type_alloc = 1 ; /* RAM */
  void*  pt_alloc ;

  if ( taille == 0 ) return 0 ;

  limit_bytes = (size_t)config.ko_undo*1024UL ;
  /* Only consider cases where type_alloc changes to something different than 1 */
  switch( modif )
  {
    case UNEGATIF :
    case USYMX    :
    case USYMY    :
    case USYMXY   : type_alloc = 0 ; /* No need for memory as operation is reversible */
                    break ;
    default       :
                    switch( config.type_undo )
                    {
                      case TUNDO_KO :     memory_left = XmemAvailableG() ;
                                          if ( memory_left < limit_bytes ) type_alloc = 2 ;
                                          break ;

                      case TUNDO_SMART :  pt_alloc = Xalloc( taille ) ;
                                          if ( pt_alloc == NULL ) type_alloc = 2 ;
                                          else                    Xfree( pt_alloc ) ; /* Keep type_alloc=1 */
                                          break ;
...
static long prepare_img_undo(int undo_num, MFDB* img, int* xy, MFDB* raster, char* name)
{
  long line_size ;

  ZeroMFDB( img ) ;
  sprintf( name, "%s\\~%02d~.TMP", config.path_temp, undo_num ) ;
  img->fd_w       = 1+xy[2]-xy[0] ;
  img->fd_wdwidth = W2WDWIDTH( img->fd_w ) ;
  img->fd_h       = 1 ;
  img->fd_nplanes = raster->fd_nplanes ;
  line_size       = img_size( img->fd_w, 1, img->fd_nplanes ) ;
  img->fd_addr    = Xalloc ( line_size ) ;
  if ( img->fd_addr == NULL )
  {
    LoggingDo(LL_ERROR, "prepare_img_undo:can't allocate %ld bytes", line_size) ;
    vform_stop( MSG_UNDOERROR ) ;
  }

  return line_size ;
}

static void undo_disk(GEM_WINDOW* wnd, UNDO_DEF* undo, int* xy, MFDB* raster)
{
  MFDB img ;
  long taille ;
  char name[200] ;

  taille = prepare_img_undo( undo->undo_num, &img, xy, raster, name ) ;
  if ( taille > 0 )
  {
    ...
    if ( stream == NULL )
    {
      LoggingDo(LL_ERROR, "undo_disk:Can't open %s", name) ;
      vform_stop( MSG_UNDOERROR ) ;
    }
    else
    {
      ...
      {
        txy[1] = txy[3] = y ;
        svro_cpyfmSO( txy, raster, &img ) ;
        if ( fwrite( img.fd_addr, taille, 1, stream ) != 1 ) err = 1 ;
        (void)GWProgRange( wnd, y - xy[1], xy[3]- xy[1], (undo->mitem == REDO) ? "Redo":"Undo" ) ;
      }
      fclose( stream ) ;
      (void)GWProgRange( wnd, 0, 100, NULL ) ;
      if ( err )
      {
        unlink( name ) ;
        LoggingDo(LL_ERROR, "undo_disk:error while writing to %s", name) ;
        vform_stop( MSG_UNDOERROR ) ;
      }
      else undo->disk = 1 ;
    }
...
static void restore_undo_disk(GEM_WINDOW* wnd, UNDO_DEF* undo, int* xy, MFDB* raster)
{
  MFDB img ;
  long taille ;
  char name[200] ;
  ...
  if ( taille > 0 )
  {
    ...
    {
      Xfree( img.fd_addr ) ;
      LoggingDo(LL_ERROR, "restore_undo_disk:can't open %s", name) ;
      vform_stop( MSG_UNDOERROR ) ;
      return ;
    }
    else
    {
      ...
      for ( y = xy[5]; !err && ( y <= xy[7] ); y++ )
      {
        ...
        svro_cpyfmSO( txy, &img, raster ) ;
        (void)GWProgRange( wnd, y - xy[5], xy[7]- xy[5], (undo->mitem == REDO) ? "Redo":"Undo" ) ;
      }
      fclose( stream ) ;
      (void)GWProgRange( wnd, 0, 100, NULL ) ;
      if ( err )
      {
        LoggingDo(LL_ERROR, "restore_undo_disk:error while reading from %s", name) ;
        vform_stop( MSG_UNDOERROR ) ;
      }
    }
    Xfree( img.fd_addr ) ;
...
int undo_put(GEM_WINDOW* gwnd, int x1, int y1, int x2, int y2, int type_modif, XUNDO* xundo)
{
  ...
  else          taille = img_size( larg, wy, screen.fd_nplanes ) ;

  undo_buf->original_width  = vimage->inf_img.mfdb.fd_w ;
  undo_buf->original_height = vimage->inf_img.mfdb.fd_h ;

  type_alloc = must_alloc( type_modif, taille ) ;
  ...
  else
  {
    ...
    if ( undo_buf->disk || undo_buf->img.fd_addr )
    {
      undo_buf->img.fd_w       = larg ; /* Si possible, on m‚morise */
      undo_buf->img.fd_h       = wy ;   /* Les nouvelles donn‚es    */
      undo_buf->img.fd_wdwidth = larg >> 4 ;
      undo_buf->img.fd_nplanes = screen.fd_nplanes ;
      ...
      xy[4] = 0 ;    xy[5] = 0 ;
      xy[6] = wx-1 ; xy[7] = wy-1 ;
      if ( undo_buf->disk ) undo_disk( gwnd, undo_buf, xy, &vimage->inf_img.mfdb ) ;
      else                  svro_cpyfmSO( xy, &vimage->inf_img.mfdb, &undo_buf->img ) ;
    }
  }
...
int uundo_put(GEM_WINDOW* gwnd)
{
  ...
  vimage->Redo->w     = vimage->inf_img.mfdb.fd_w ;
  vimage->Redo->h     = vimage->inf_img.mfdb.fd_h ;
  ...
  taille = img_size( vimage->Redo->w, vimage->Redo->h, screen.fd_nplanes ) ;

  vimage->Redo->original_width  = vimage->inf_img.mfdb.fd_w ;
  vimage->Redo->original_height = vimage->inf_img.mfdb.fd_h ;

  type_alloc = must_alloc( vimage->Redo->mitem, taille ) ;
  if ( type_alloc )
  {
    ...
    if ( type_alloc == 1 )
    {
      ...
      vimage->Redo->img.fd_h       = vimage->Redo->h ;
      vimage->Redo->img.fd_wdwidth = W2WDWIDTH( vimage->Redo->w ) ;
      vimage->Redo->img.fd_nplanes = screen.fd_nplanes ;

      vimage->Redo->img.fd_addr = Xalloc( taille ) ;
      if ( vimage->Redo->img.fd_addr == NULL ) return -1 ;
      svro_cpyfmSO( xy, &vimage->inf_img.mfdb, &vimage->Redo->img ) ;
    }
    else undo_disk( gwnd, vimage->Redo, xy, &vimage->inf_img.mfdb ) ;
  }
...
void handle_new_size(MFDB* original, GEM_WINDOW* gwnd)
{
  ...
  if ( gwnd == NULL) return ;
  vimage = (VXIMAGE*) gwnd->Extension ;
  if ( (original->fd_w == vimage->inf_img.mfdb.fd_w) && (original->fd_h == vimage->inf_img.mfdb.fd_h) ) return ;
...
static void undo_sym(GEM_WINDOW* gwnd, UNDO_DEF* undo, int sym_mask)
{
  ...
  xy[2]     = xy[0]+undo->w-1 ;
  xy[3]     = xy[1]+undo->h-1 ;
  use_vclip = (vclip.gwindow == gwnd) && (xy[0] == 0) && (xy[1] == 0) && (xy[2] == vimage->inf_img.mfdb.fd_w-1) && (xy[3] == vimage->inf_img.mfdb.fd_h-1) ;

  if ( sym_mask & 1 )
  {
    raster_vsym( &vimage->inf_img.mfdb, xy ) ;
    if ( use_vclip )
    {
  ...
  if ( sym_mask & 2 )
  {
    raster_hsym( &vimage->inf_img.mfdb, xy ) ;
    if ( use_vclip )
    {
...
void undo_restore(GEM_WINDOW* gwnd)
{
  ...
  {
    rep = vform_stop( MSG_UNDOIMG ) ;
    if ( rep == 2 ) return ;
  }

  CopyMFDB( &original_img, &vimage->inf_img.mfdb )  ;
  mouse_busy() ;

  if ( uundo_put( gwnd ) ) vform_stop( MSG_UUNDO ) ;
  ...
  {
    if ( handle_changed_size( undo, &vimage->inf_img.mfdb ) != 0 ) form_error_mem() ;
    else
    {
      xy[0] = 0 ;         xy[1] = 0 ;
      ...
      if ( undo->disk ) restore_undo_disk( gwnd, undo, xy, &vimage->inf_img.mfdb ) ;
      else              svro_cpyfmSO( xy, &undo->img, &vimage->inf_img.mfdb ) ;
    }
  }
  else
  {
    ...
    switch( undo->mitem )
    {
      case UNEGATIF : xy[0] = undo->x ;
                      ...
                      xy[3] = xy[1]+undo->h-1 ;
                      memcpy( &xy[4], xy, 8 ) ;
                      svro_cpyfm( D_INVERT, xy, &vimage->inf_img.mfdb, &vimage->inf_img.mfdb ) ;
                      break ;
      case USYMX    : undo_sym( gwnd, undo, 1 ) ;
                      break ;
      case USYMY    : undo_sym( gwnd, undo, 2 ) ;
                      break ;
      case USYMXY   : undo_sym( gwnd, undo, 3 ) ;
                      break ;
    }
  }
  ...
  if ( resize && (vimage->zoom_level != 1) )
  {
    ...
    if ( raster_zoom( &vimage->inf_img.mfdb, &vimage->zoom, xy, vimage->zoom_level, vimage->zoom_level ) == -1 )
      form_error_mem() ;
  }
  
  if ( gwnd->window_icon.fd_addr ) Xfree( gwnd->window_icon.fd_addr) ;
  ZeroMFDB( &gwnd->window_icon ) ;
  if ( gwnd->is_icon ) iconify_picture( gwnd ) ;
  GWRePaint( gwnd ) ;
...
void traite_refaire(GEM_WINDOW* gw)
{
  ...
  if ( (vimage->Redo->w != vimage->inf_img.mfdb.fd_w) || (vimage->Redo->h != vimage->inf_img.mfdb.fd_h) )
    undo_put( gw, 0, 0, vimage->inf_img.mfdb.fd_w-1, vimage->inf_img.mfdb.fd_h-1, vimage->Redo->mitem, NULL ) ;
  else
  ...
  {
    if ( handle_changed_size( vimage->Redo, &vimage->inf_img.mfdb ) != 0 ) form_error_mem() ;
    else
    {
      xy[0] = 0 ;                 xy[1] = 0 ;
      ...
      if ( vimage->Redo->disk ) restore_undo_disk( gw, vimage->Redo, xy, &vimage->inf_img.mfdb ) ;
      else                      svro_cpyfmSO( xy, &vimage->Redo->img, &vimage->inf_img.mfdb ) ;
    }
  }
