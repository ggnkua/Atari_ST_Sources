...
#ifndef __NO_LOGGING
void LoggingDoInvalidhCat(void)
{
...
/*  svro_cpyfm( tab_index[wext->start_visible+i].selected ? S_OR_NOTD:S_ONLY, xy, &v->info.mfdb, &screen) ;*/
  svro_cpyfmSOS( xy, &v->info.mfdb ) ;
  if ( tab_index[wext->start_visible+i].selected )
  {
    ...
    coords[6] = xy[6] - 4 ;
    coords[7] = xy[5] + 4 + 8 ;
    svro_cpyfm( D_INVERT, coords, &v->info.mfdb, &screen ) ;
  }
  svsf_perimeter( 0 ) ;
...
static void update_vslider(GEM_WINDOW* wnd)
{
  ...
  total = catalog_filter->nb_visibles-wext->nb_vignettes ;

  wh = pmf( wext->nb_vignettes, catalog_filter->nb_visibles ) ;
  if ( (wh > 1000) || (wh < 0) ) wh = 1000 ;
  wind_set( wnd->window_handle, WF_VSLSIZE, wh ) ;

  ypos = pmf( wext->start_visible, total ) ;
...
static void OnVSliderAlbum(void* w, int pos)
{
  ...
  GWOnVSlider( wnd, pos ) ;
  if ( catalog_filter )
    wext->start_visible = pml( pos, catalog_filter->nb_visibles-wext->nb_vignettes ) ;
...
static int OnArrowAlbum(void* w, int action)
{
  GEM_WINDOW*     wnd = (GEM_WINDOW*) w ;
  WALB_EXTENSION* wext = wnd->Extension ;
  CATALOG_FILTER* catalog_filter = catalog_get_filter(wext->hCat) ;
  ...
  if ( done )
  {
    ...
    GWSetVSlider( wnd, pmf(wext->start_visible, total) ) ;
    ajour_vignettes( wnd,0 ) ;
  }
...
static void refresh(GEM_WINDOW* wnd)
{
  WALB_EXTENSION* wext = wnd->Extension ;
...
static int OnKeyPressedAlbum(void* w, int key)
{
  ...
  switch( key )
  {
    case STD_PAL    :   wext->std_palette = 1-wext->std_palette ;
                        if ( wext->std_palette && old_pal ) set_tospalette( old_pal, 16 ) ;
    ...
    case KBD_DELETE :   nsel = catalog_getnsel(wext->hCat) ;
                        if ( nsel > 0 )
                        {
                          sprintf( buf, vMsgTxtGetMsg(MSG_CATDELICON), nsel ) ;
                          if ( form_interrogation(2, buf ) == 1 )
                          {
                            GEM_WINDOW* wprog = NULL ;

                            if ( nsel > 100 ) wprog = DisplayStdProg( vMsgTxtGetMsg(MSG_CATSDELREC) , "", "", CLOSER ) ;
    ...
    case KBD_LSDELETE:  nsel = catalog_getnsel(wext->hCat) ;
                        if ( nsel > 0 )
                        {
                          sprintf( buf, vMsgTxtGetMsg(MSG_CATSDELICON), nsel ) ;
                          if ( form_interrogation(2, buf ) == 1 )
                          {
                            GEM_WINDOW* wprog = NULL ;

                            if ( nsel > 10 ) wprog = DisplayStdProg( vMsgTxtGetMsg(MSG_CATSDELRECIMG) , "", "", CLOSER ) ;
...
static int OnRButtonDownAlbum(void* w, int mk_state, int mx, int my)
{
  ...
  {
    strncpy( popup[1+i-MSG_PLOADIMG].ob_spec.free_string, vMsgTxtGetMsg(i), nchars ) ;
  ...
  strncpy( popup[1+i-MSG_PLOADIMG].ob_spec.free_string, vMsgTxtGetMsg(MSG_WASALL), nchars ) ;
  popup[1+i-MSG_PLOADIMG].ob_flags |= SELECTABLE ;
  i++ ;

  if ( nsel > 0 )
  {
    strncpy( popup[1+i-MSG_PLOADIMG].ob_spec.free_string, vMsgTxtGetMsg(MSG_WADELICONS), nchars ) ;
    popup[1+i-MSG_PLOADIMG].ob_flags |= SELECTABLE ;
    i++ ;
    strncpy( popup[1+i-MSG_PLOADIMG].ob_spec.free_string, vMsgTxtGetMsg(MSG_WADELFILES), nchars ) ;
    popup[1+i-MSG_PLOADIMG].ob_flags |= SELECTABLE ;
  }
...
static int OnMouseMoveAlbum(void* w, int button, int kstate, int mx, int my)
{
  ...
  else if ( wext->NumVignetteInfo != -1L )
  {
    ...
    sprintf( buf, vMsgTxtGetMsg(MSG_CATINFO) , catalog_filter->nb_visibles, catalog_header->nb_records ) ;
    get_info_fonname( catalog_filter, buffer ) ;
    strcat(buf, buffer) ;
    GWSetWindowInfo( wnd, buf ) ;
...
GEM_WINDOW* album_window(HCAT hCat, GEM_WINDOW* wnd_album, GEM_WINDOW** pwnd)
{
  ...
  wind_calc( WC_BORDER, WALB_STYLE, xe, ye, wext->nb_w*catalog_header->width, wext->nb_h*wext->h_vignette, &xe, &ye, &we, &he ) ;
