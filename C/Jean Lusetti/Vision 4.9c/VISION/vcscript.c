...
static short GetParams(char* str, char separator, char** pparams, short nb_pparams)
{
  ...
  do
  {
    if ( (separator == ' ') && (*str == '"') )
    {
      /* Text parameter with space inside */
      separator = '"' ;
      str++ ;
    }
...
static short CmdNew(VCMD_SCRIPT* vcmd_script)
{
  char  name[PATH_MAX] ;
  short status ;

  status = GetParam( vcmd_script, 1, name ) ;
  if ( status == 0 )
  {
    int  nx, ny ;
    char buf[100] ;

    status = GetParam( vcmd_script, 2, buf ) ;
    nx = atoi( buf ) ;
    if ( status == 0 )
    {
      status = GetParam( vcmd_script, 3, buf ) ;
      ny = atoi( buf ) ;
      if ( status == 0 )
      {
        nouvelle_wimage( name, nx, ny, screen.fd_nplanes, 72, 72 ) ;
        status = SVCS_SUCCESS ;
      }
      else status = SVCS_MISSINGP3 ;
    }
  ...
  if ( vcmd_script->LoopDepth >= 0 )
  {
    ...
    if ( loop_data->NbLoopsToGo > 0 )
    {
      ...
      else
      {
        ...
        if ( vcmd_script->LoopDepth < 0 )
        {
          vcmd_script->current_loop_step = 0 ;
          vcmd_script->total_loop_steps  = 0 ;
        }
      }
      status = SVCS_SUCCESS ;
    }
        ...
        if ( wnd )
        {
          ...
          if ( hcat )
          {
            ...
            if ( catalog_header )
            {
              ...
              ret = catalog_applyini( hcat, vMsgTxtGetMsg(MSG_CATUPTINI) ) ;
...
static void ScriptProg(VCMD_SCRIPT* vcmd_script, char* tree_script, char* ln)
{
  LoggingDo(LL_INFO, "Script Progress for %s", tree_script ) ;
  LoggingDo(LL_INFO, "  Executing Command line: %02d/%02d (%s)", vcmd_script->LineIndex, vcmd_script->LastLineIndex, ln) ;
  if ( vcmd_script->LoopFolderData.Loop.NbLoopsToGo > 0 )
    LoggingDo(LL_INFO, "  LoopFolder:             %02d/%02d", 1+vcmd_script->LoopFolderData.Loop.LoopIndex, vcmd_script->LoopFolderData.Loop.NbLoopsToGo) ;

  if ( vcmd_script->LoopDepth >= 0 )
  {
    long  total, sub_total, cur ;
    short i, j ;
    char  buf[300] ;
    char  tmp[60] ;

    /* Compute the number of times we go through a 'EndLoop' command 
        With potentially nested loop levels
        e.g. for:
        Loop 5
          Loop 4
            Loop 3
        We go through EndLoop for Loop 3, 5x4x3 times,
                              for Loop 2, 5x4 times
                              for loop 5, 5 times
        Total: 60+20+5
     */  
    total = -1 ;
    for ( j = 0; j <= vcmd_script->LoopDepth; j++ )
    {
      sub_total = 1 ;
      for ( i = 0; i <= vcmd_script->LoopDepth-j; i++ )
        sub_total *= vcmd_script->LoopData[i].NbLoopsToGo ;
      total += sub_total ;
    }

    memzero( buf, sizeof(buf) ) ;
    cur = 1 ;
    for ( i = 0; i <= vcmd_script->LoopDepth; i++ )
    {
      cur *= (1+vcmd_script->LoopData[i].LoopIndex) ;
      sprintf( tmp, "%02d/%02d--", 1+vcmd_script->LoopData[i].LoopIndex, vcmd_script->LoopData[i].NbLoopsToGo) ;
      strcat( buf, tmp ) ;
    }
    if ( total > vcmd_script->total_loop_steps ) vcmd_script->total_loop_steps = total ;
    LoggingDo(LL_INFO, "  Loop:                   %s (%ld-%ld %ld%% done)", buf, vcmd_script->current_loop_step, vcmd_script->total_loop_steps, (100L*vcmd_script->current_loop_step)/vcmd_script->total_loop_steps) ;
  }
}
...
short ExeNextCommandLine(VCMD_SCRIPT* vcmd_script)
{
  ...
  short        script_depth = 0 ;
  char         tree_script[200] ;
  char*        aslash ;

  /* Find the child being ran */
  while ( vcmd_script->Magic == VCS_MAGIC )
  {
    memzero( tree_script, sizeof(tree_script) ) ;
    aslash = strrchr( vcmd_script->FileName, '\\' ) ;
    if ( aslash ) strcat( tree_script, 1+aslash ) ;
    else          strcat( tree_script, vcmd_script->FileName ) ;
    script_depth++ ;
    if ( script_depth > 1 ) strcat( tree_script, "-->" ) ;
    if ( vcmd_script->ChildScript )
    {
  ...
  if ( vcmd_script->Buffer )
  {
    ...
    else
    {
      ...
      if ( strlen(ln) >= 4 )
      {
        ...
        ScriptProg( vcmd_script, tree_script, ln ) ;
