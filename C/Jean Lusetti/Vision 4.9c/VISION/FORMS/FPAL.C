...
static void cree_degrade(int start, int end)
{
  float r, g, b ;
  float c ;
  int   rgb[3] ;
  ...
  svq_color( start, 0, srgb) ;
  svq_color( end, 0, ergb) ;
  nb = 1+end-start ;
...
static void charge_palette(void)
{
  int   pal[256*3] ;
  int   ncol ;
  char  name[200] ;
...
static void sauve_palette(void)
{
  int  pal[256*3] ;
  char name[200] ;
...
static int cdecl draw_colors(PARMBLK *parmblock)
{ 
...
static int cdecl draw_currentcolor(PARMBLK* parmblock)
{ 
  WEXTENSION_PAL* wext = (WEXTENSION_PAL *) parmblock->pb_parm ;
  int             xyc[4], xy[4] ;
...
static int rrgb(int color, int hf)
{
  return (int) ((1000-color)*(float)hf/1000.0) ;
}

static void OnInitDialogPal(void* w)
{
  ...
  wext->hf    = adr_mkpal[MKPAL_REDBOX].ob_height-adr_mkpal[MKPAL_RED].ob_height ;
  svq_color( wext->index, 0, wext->rgb ) ;
  ...
  off_y = rrgb( wext->rgb[0], wext->hf ) ;
  adr_mkpal[MKPAL_RED].ob_y = off_y ;
  off_y = rrgb( wext->rgb[1], wext->hf ) ;
  adr_mkpal[MKPAL_GREEN].ob_y = off_y ;
  off_y = rrgb( wext->rgb[2], wext->hf ) ;
  adr_mkpal[MKPAL_BLUE].ob_y = off_y ;
  wext->flag_aff = 0 ;
}

static void upd_rgb(int* rgb, int plus, int add)
{
  if ( !plus ) add = -add ;

  *rgb += add ;
  if ( *rgb < 1) *rgb = 1 ;
   if ( *rgb > 1000 ) *rgb = 1000 ;
}

static int OnObjectNotifyPal(void* w, int obj)
{
  GEM_WINDOW*     wnd = (GEM_WINDOW*) w ;
  WEXTENSION_PAL* wext = wnd->DlgData->UserData ;
  OBJECT*         adr_mkpal = wnd->DlgData->BaseObject ;
  ...
  {
    case MKPAL_MRED      :
    case MKPAL_PRED      : upd_rgb( &wext->rgb[0], obj == MKPAL_PRED, 1 ) ;
                           off_y = rrgb( wext->rgb[0], wext->hf ) ;
                           adr_mkpal[MKPAL_RED].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_REDBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[0]) ;
                           write_text(adr_mkpal, MKPAL_TRED, buf) ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_TRED ) ;
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
    case MKPAL_MGREEN    :
    case MKPAL_PGREEN    : upd_rgb( &wext->rgb[1], obj == MKPAL_PGREEN, 1 ) ;
                           off_y = rrgb( wext->rgb[1], wext->hf ) ;
                           adr_mkpal[MKPAL_GREEN].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_GREENBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[1]) ;
                           write_text(adr_mkpal, MKPAL_TGREEN, buf) ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_TGREEN ) ;
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
    case MKPAL_MBLUE     :
    case MKPAL_PBLUE     : upd_rgb( &wext->rgb[2], obj == MKPAL_PBLUE, 1 ) ;
                           off_y = rrgb( wext->rgb[2], wext->hf ) ;
                           adr_mkpal[MKPAL_BLUE].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_BLUEBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[2]) ;
                           write_text(adr_mkpal, MKPAL_TBLUE, buf) ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_TBLUE ) ;
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
    case MKPAL_REDBOX    : 
                           objc_offset(adr_mkpal, MKPAL_RED, &off_x, &off_y) ;
                           upd_rgb( &wext->rgb[0], ym < off_y, 20 ) ;
                           off_y = rrgb( wext->rgb[0], wext->hf ) ;
                           adr_mkpal[MKPAL_RED].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_REDBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[0]) ;
    ...
    case MKPAL_GREENBOX  : 
                           ...
                           upd_rgb( &wext->rgb[1], ym < off_y, 20 ) ;
                           off_y = rrgb( wext->rgb[1], wext->hf ) ;
                           adr_mkpal[MKPAL_GREEN].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_GREENBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[1]) ;
    ...
    case MKPAL_BLUEBOX   : 
                           ...
                           upd_rgb( &wext->rgb[2], ym < off_y, 20 ) ;
                           off_y = rrgb( wext->rgb[2], wext->hf ) ;
                           adr_mkpal[MKPAL_BLUE].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_BLUEBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[2]) ;
                           ...
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
    case MKPAL_RED       : wext->rgb[0] = 1000-graf_slidebox(adr_mkpal, MKPAL_REDBOX, MKPAL_RED, 1) ;
                           off_y = rrgb( wext->rgb[0], wext->hf ) ;
                           adr_mkpal[MKPAL_RED].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_REDBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[0]) ;
                           ...
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
     case MKPAL_GREEN    : wext->rgb[1] = 1000-graf_slidebox(adr_mkpal, MKPAL_GREENBOX, MKPAL_GREEN, 1) ;
                           off_y = rrgb( wext->rgb[1], wext->hf ) ;
                           adr_mkpal[MKPAL_GREEN].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_GREENBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[1]) ;
                           ...
                           svs_color( wext->index, wext->rgb ) ;
                           break ;
    case MKPAL_BLUE      : wext->rgb[2] = 1000-graf_slidebox(adr_mkpal, MKPAL_BLUEBOX, MKPAL_BLUE, 1) ;
                           off_y = rrgb( wext->rgb[2], wext->hf ) ;
                           adr_mkpal[MKPAL_BLUE].ob_y = off_y ;
                           GWObjcDraw( wnd,adr_mkpal, MKPAL_BLUEBOX ) ;
                           sprintf(buf, "%.4d", wext->rgb[2]) ;
    ...
    case MKPAL_END      : if ( obj == MKPAL_START ) wext->stdeg  = wext->index ;
                          ...
                          if (selected(adr_mkpal, MKPAL_START) && selected(adr_mkpal, MKPAL_END))
                          {
                            ...
                            GWObjcDraw( wnd,adr_mkpal, MKPAL_START ) ;
                            GWObjcDraw( wnd,adr_mkpal, MKPAL_END ) ;
    ...
    case MKPAL_BOX      : objc_offset(adr_mkpal, obj, &pb_x, &pb_y) ;
                          ...
                          sprintf(buf, "%.4d", wext->rgb[2]) ;
                          write_text(adr_mkpal, MKPAL_TBLUE, buf) ;
                          GWObjcDraw( wnd,adr_mkpal, MKPAL_RGB ) ;
                          ...
                          rectangle(x1, y1, x2, y2) ;
                          GWShowCursor() ;
                          GWObjcDraw( wnd,adr_mkpal, MKPAL_CURRCOL ) ;
                          off_y = rrgb( wext->rgb[0], wext->hf ) ;
                          adr_mkpal[MKPAL_RED].ob_y = off_y ;
                          GWObjcDraw( wnd,adr_mkpal, MKPAL_REDBOX ) ;
                          off_y = rrgb( wext->rgb[1], wext->hf ) ;
...
void traite_makepalette(GEM_WINDOW* gwnd)
{
  ...
  if ( FGetPal( &wext ) == IDCANCEL ) set_vdipalette( wext.curr_pal, nb_colors ) ;
  else
  {
    int the_pal[3*256] ;
    int nbc ;
