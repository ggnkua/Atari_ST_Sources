...
static void OnInitDialogSldShow(void* w)
{
  ...
  checkbox_setstate( config.display_type & DISPLAYTYPE_ZOOM, adr_sldshow, SSH_AFFZOOM ) ;
  checkbox_setstate( config.display_type & DISPLAYTYPE_REDUC, adr_sldshow, SSH_AFFREDUC ) ;
  wext->transition = config.transition ;
  wdlg->popup_transition = popup_make( 3, 13 ) ;
  if ( wdlg->popup_transition == NULL ) return ;
  for ( i = 0; i <= 2; i++ )
    strcpy( wdlg->popup_transition[1+i].ob_spec.free_string, vMsgTxtGetMsg(MSG_TRANSIT0+i) ) ;
  write_text( adr_sldshow, SSH_TRANSITION, vMsgTxtGetMsg(MSG_TRANSIT0+wext->transition) ) ;
  ...
  strcpy( wdlg->popup_back[1].ob_spec.free_string, vMsgTxtGetMsg(MSG_NOIR) ) ;
  strcpy( wdlg->popup_back[2].ob_spec.free_string, vMsgTxtGetMsg(MSG_BLANC) ) ;
  write_text( adr_sldshow, SSH_FOND, wdlg->popup_back[1+wext->back].ob_spec.free_string ) ;

  checkbox_setstate( config.cycle, adr_sldshow, SSH_CYCLE ) ;
  checkbox_setstate( !config.do_not_cls_scr, adr_sldshow, SSH_CLSSCREEN ) ;
  checkbox_setstate( config.aff_name, adr_sldshow, SSH_SHOWNAME ) ;
  checkbox_setstate( config.aff_prog, adr_sldshow, SSH_SHOWPROG ) ;
  ...
  {
    write_text( adr_sldshow, SSH_WAITMINI, "00" ) ;
    adr_sldshow[SSH_WAITMINI].ob_state   |= DISABLED ;
    adr_sldshow[SSH_TWAITMINI].ob_state  |= DISABLED ;
...
static int OnObjectNotifySldShow(void* w, int obj)
{
  ...
  switch( obj )
  {
    case SSH_TRANSITION  : deselect( adr_sldshow, SSH_TRANSITION ) ;
                           ...
                           write_text( adr_sldshow, SSH_TRANSITION, vMsgTxtGetMsg(MSG_TRANSIT0+wext->transition) ) ;
                           GWObjcDraw( wnd, adr_sldshow, SSH_TRANSITION ) ;
                           break ;
    case SSH_FOND        : deselect( adr_sldshow, SSH_FOND ) ;
                           ...
                           if ( wext->back == BACK_WHITE ) write_text( adr_sldshow, SSH_FOND, vMsgTxtGetMsg(MSG_BLANC) ) ;
                           else                            write_text( adr_sldshow, SSH_FOND, vMsgTxtGetMsg(MSG_NOIR) ) ;
                           GWObjcDraw( wnd, adr_sldshow, SSH_FOND ) ;
                           break ;
    ...
    case SSH_TNOTIMER    : inv_select( adr_sldshow, SSH_NOTIMER ) ;
                           ...
                           GWObjcDraw( wnd, adr_sldshow, SSH_WAITMINI ) ;
                           GWObjcDraw( wnd, adr_sldshow, SSH_TWAITMINI ) ;
                           break ;
...
static int OnTxtBubbleSldShow(void* w, int mx, int my, char* txt)
{
  ...
  int         objs[] = { SSH_TRANSITION, SSH_CYCLE, SSH_TCYCLE, SSH_WAITMINI, SSH_TWAITMINI, SSH_CLSSCREEN, SSH_TCLSSCREEN, SSH_SHOWNAME, SSH_TSHOWNAME, SSH_AFFZOOM, SSH_TAFFZOOM, SSH_AFFREDUC, SSH_TAFFREDUC, SSH_SHOWPROG, SSH_TSHOWPROG, SSH_NOTIMER, SSH_TNOTIMER } ;
  int         msgs[] = { MSG_HSSHTRANSIT, MSG_HSSHCYCLE, MSG_HSSHCYCLE, MSG_HSSHWAITMINI, MSG_HSSHWAITMINI, MSG_HSSHCLS, MSG_HSSHCLS, MSG_HSSHAFFNAME, MSG_HSSHAFFNAME, MSG_HSSHAFFZOOM, MSG_HSSHAFFZOOM, MSG_HSSHAFFREDUC, MSG_HSSHAFFREDUC, MSG_HSSHAFFPROG, MSG_HSSHAFFPROG, MSG_HSSHNOTIMER, MSG_HSSHNOTIMER } ;
