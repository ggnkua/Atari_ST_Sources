...
static void OnInitDialogRFiles(void* w)
{
...
static int OnObjectNotifyRFiles(void* w, int obj)
{
  GEM_WINDOW*       wnd = (GEM_WINDOW*) w ;
  WEXTENSION_RFILE* wext = wnd->DlgData->UserData ;
  OBJECT*           adr_find = wnd->DlgData->BaseObject ;
...
/* Small helper prototyped in defs.h */
short GetLongFileNamesStatus(void)
{
  return (config.flags & FLG_LONGFNAME) ? FFR_LONGNAMES|FFR_NOLONGNAMESDETAILS:0 ;
}
...
long GetRFilesEx(RFILE* rfiles, int nb_max_rfiles, int return_nb_files, char* log_file, int (*user_func)(long nfiles_so_far, char* current_filename, void* user_data), void* user_data)
{
  ...
  if ( rfiles[0].path[0] == 0 )
  {
    ...
    memzero( &wext, sizeof(WEXTENSION_RFILE) ) ;
    wext.rfiles    = rfiles ;
    ...
    code = GWCreateAndDoModal( &dlg_data, 0 ) ;
  }
