...
static void OnInitDialogFGeneral(void* w)
{
...
static int OnObjectNotifyFGeneral(void* w, int obj)
{
  GEM_WINDOW*          wnd = (GEM_WINDOW*) w ;
  WEXTENSION_PGENERAL* wext = wnd->DlgData->UserData ;
  OBJECT*              adr_pgen = wnd->DlgData->BaseObject ;
  ...
  switch( obj )
  {
    case MODE_TRE : if ( selected(adr_pgen, MODE_REMPLACE) ) break ;
                    ...
                    if ( selected(adr_pgen, MODE_TRANSPARENT) )   inv_select( adr_pgen, MODE_TRANSPARENT ) ;
                    else if ( selected(adr_pgen, MODE_XOR) )      inv_select( adr_pgen, MODE_XOR ) ;
                    else if ( selected(adr_pgen, MODE_INVTRANS) ) inv_select( adr_pgen, MODE_INVTRANS ) ;
                    break ;
    case MODE_TTR : if ( selected(adr_pgen, MODE_TRANSPARENT) ) break ;
                    inv_select( adr_pgen, MODE_TRANSPARENT ) ;
                    if ( selected( adr_pgen, MODE_REMPLACE) )     inv_select( adr_pgen, MODE_REMPLACE ) ;
                    else if ( selected(adr_pgen, MODE_XOR) )      inv_select( adr_pgen, MODE_XOR ) ;
                    else if ( selected(adr_pgen, MODE_INVTRANS) ) inv_select( adr_pgen, MODE_INVTRANS ) ;
                    break ;
    case MODE_TXO : if ( selected(adr_pgen, MODE_XOR) ) break ;
                    ...
                    if ( selected(adr_pgen, MODE_TRANSPARENT) )   inv_select( adr_pgen, MODE_TRANSPARENT ) ;
                    else if ( selected(adr_pgen, MODE_REMPLACE) ) inv_select( adr_pgen, MODE_REMPLACE ) ;
                    else if ( selected(adr_pgen, MODE_INVTRANS) ) inv_select( adr_pgen, MODE_INVTRANS ) ;
                    break ;
    ...
    case MASK_TIN : select( adr_pgen, MASK_IN ) ;
                    deselect( adr_pgen, MASK_OUT ) ;
                    GWObjcDraw( wnd, adr_pgen, MASK_IN ) ;
                    GWObjcDraw( wnd, adr_pgen, MASK_OUT ) ;
                    break ;
    case MASK_OUT :
  ...
  if ( code == IDOK )
  {
    ...
    else if ( selected(adr_pgen, MODE_TRANSPARENT) ) wext->mode = MD_TRANS ;
    else if ( selected(adr_pgen, MODE_XOR) )         wext->mode = MD_XOR ;
    else if ( selected(adr_pgen, MODE_INVTRANS) )    wext->mode = MD_ERASE ;
