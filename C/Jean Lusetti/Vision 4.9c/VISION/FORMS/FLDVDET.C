...
void setup_details(GEM_WINDOW* wnd, int display)
{
  ...
  if ( caps )
  {
    ...
    buf[0] = 0 ;
    if ( caps->Flags & LDVF_STDFORMAT ) strcat( buf, vMsgTxtGetMsg(MSG_LDVSTDFMT)  ) ;
    if ( caps->Flags & LDVF_ATARIFORMAT ) strcat( buf, vMsgTxtGetMsg(MSG_LDVATARIFMT)  ) ;
    if ( caps->Flags & LDVF_SPECFORMAT ) strcat( buf, vMsgTxtGetMsg(MSG_LDVSPECFMT)  ) ;
    write_text( bobj, LDVD_FORMAT, buf ) ;

    buf[0] = 0 ;
    if ( caps->Flags & LDVF_SUPPORTPREVIEW ) strcat( buf, vMsgTxtGetMsg(MSG_LDVSUPPREV)  ) ;
    if ( caps->Flags & LDVF_SUPPORTPROG ) strcat( buf, vMsgTxtGetMsg(MSG_LDVSUPPROG)  ) ;
    write_text( bobj, LDVD_SUPPORT, buf ) ;

    if ( caps->Flags & LDVF_REALTIMEPARAMS ) bobj[LDVD_RTPREVIEW].ob_flags &= ~HIDETREE ;
    else                                     bobj[LDVD_RTPREVIEW].ob_flags |= HIDETREE ;

    if ( caps->Flags & LDVF_OPINPLACE ) bobj[LDVD_OPINPLACE].ob_flags &= ~HIDETREE ;
    else                                bobj[LDVD_OPINPLACE].ob_flags |= HIDETREE ;
    ...
    if ( caps->Flags & LDVF_SUPPORTCANCEL ) bobj[LDVD_CANCEL].ob_flags &= ~HIDETREE ;
    else                                    bobj[LDVD_CANCEL].ob_flags |= HIDETREE ;
  }
  else
  {
    ...
    bobj[LDVD_AFFECTPAL].ob_flags |= HIDETREE ;
    for ( i = 0; i < ARRAY_SIZE(nplanes); i++ )
      if ( caps == NULL ) caps = LDVGetNPlanesCaps( wext->LdvModule, nplanes[i] ) ;
  ...
  if ( display )
  {
    GWObjcDraw( wnd, bobj, LDVD_GROUPDET ) ;
    GWObjcDraw( wnd, bobj, LDVD_TFEATURES ) ;
  }
}
