...
void show_fonts(GEM_WINDOW* wnd, int start_num)
{
  ...
  for ( i = start_num; i < end_num; i++ )
  {
    ...
    GWObjcDraw( wnd, obj, TEXTE_FONT1+i-start_num ) ;
  }
  ...
  GWObjcDraw( wnd, obj, TEXTE_FONTBOX ) ;
  GWObjcDraw( wnd, obj, TEXTE_FONTVUES ) ;
}
...
void OnInitDialogText(void* w)
{
  ...
  dlg_data->tsort_fonts = (int*) Xcalloc( 1+num_fonts, 2 ) ;
...
static int OnObjectNotifyText(void* w, int obj)
{
  ...
  hf = (float) (adr_ptext[TEXTE_FONTBOX].ob_height-adr_ptext[TEXTE_FONTVUES].ob_height) ;
  objc_offset( adr_ptext, TEXTE_VUE, &x, &y ) ;
  switch( obj )
  {
    case TEXTE_NORMAL   :
    case TEXTE_TNORMAL  : select(adr_ptext, TEXTE_NORMAL) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_NORMAL ) ;
                          deselect(adr_ptext, TEXTE_GRAS) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_GRAS ) ;
                          deselect(adr_ptext, TEXTE_FIN) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_FIN ) ;
                          deselect(adr_ptext, TEXTE_ITALIQUE) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_ITALIQUE ) ;
                          deselect(adr_ptext, TEXTE_SOULIGNE) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_SOULIGNE ) ;
                          deselect(adr_ptext, TEXTE_CONTOURS) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_CONTOURS ) ;
                          break ;
    case TEXTE_TGRAS    :
    case TEXTE_TFIN     :
    ...
    case TEXTE_HPLUS    : if (obj == TEXTE_HMOINS) ttype.hcar -= 1 ;
                          ...
                          GWObjcDraw( wnd, adr_ptext, obj ) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_NTAILLE ) ;
                          break ;
    case TEXTE_RMOINS   :
    case TEXTE_RPLUS    : read_text(adr_ptext, TEXTE_ANGLE, buf) ;
                          ...
                          deselect( adr_ptext, obj ) ;
                          GWObjcDraw( wnd, adr_ptext, obj ) ;
                          GWObjcDraw( wnd, adr_ptext, TEXTE_ANGLE ) ;
                          break ;
    case TEXTE_FONT1    :
    case TEXTE_FONT2    :
    case TEXTE_FONT3    :
...
void traite_ptexte(void)
{
  ...
  if ( GWCreateAndDoModal( &dlg_data, TEXTE_NTAILLE ) == IDCANCEL ) memcpy( &ttype, &user_data.text, sizeof(TEXT_TYPE) ) ;
}
