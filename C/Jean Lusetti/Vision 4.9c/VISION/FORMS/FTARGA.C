...
static void OnInitDialogTarga(void* w)
{
...
static int OnObjectNotifyTarga(void* w, int obj)
{
  GEM_WINDOW*       wnd = (GEM_WINDOW*) w ;
  WEXTENSION_TARGA* wext = wnd->DlgData->UserData ;
  OBJECT*           adr_targa = wnd->DlgData->BaseObject ;
  ...
  switch( obj )
  {
     ...
     case TARGA_16BITS    : select( adr_targa, TARGA_16BITS ) ;
                            deselect( adr_targa, TARGA_24BITS ) ;
                            GWObjcDraw( wnd, adr_targa, TARGA_16BITS ) ;
                            GWObjcDraw( wnd, adr_targa, TARGA_24BITS ) ;
                            break ;
     case TARGA_T24BITS   :
