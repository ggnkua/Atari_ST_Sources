...
static void select_undo_type(GEM_WINDOW* wnd, int selected_one)
{
  ...
  GWObjcDraw( wnd, adr_undo, UNDO_ALWAYS ) ;
  GWObjcDraw( wnd, adr_undo, UNDO_NEVER ) ;
  GWObjcDraw( wnd, adr_undo, UNDO_KO ) ;
  GWObjcDraw( wnd, adr_undo, UNDO_SMART ) ;
}
...
static void OnInitDialogUndo(void* w)
{
  ...
  {
    case TUNDO_MEMORY: select_undo_type( wnd, UNDO_NEVER ) ;
                       break ;
    case TUNDO_KO:     select_undo_type( wnd, UNDO_KO ) ;
                       break ;
    case TUNDO_SMART:  select_undo_type( wnd, UNDO_SMART ) ;
                       break ;
    case TUNDO_DISK:   select_undo_type( wnd, UNDO_ALWAYS ) ;
                       break ;
  }
}
