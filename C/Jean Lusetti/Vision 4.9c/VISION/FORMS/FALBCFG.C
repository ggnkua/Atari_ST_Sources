...
char          obj_formats[] = { ACONF_ART, ACONF_DEGAS, ACONF_GIF, ACONF_JPEG, ACONF_IMG, ACONF_MAC, ACONF_NEO, ACONF_TARGA, ACONF_TIFF, ACONF_BMP } ;
unsigned char imgformats[] =  {     F_ART,     F_DEGAS,     F_GIF,     F_JPEG,     F_IMG,     F_MAC,     F_NEO,     F_TARGA,     F_TIFF,     F_BMP } ;


static void update_categorylist(GEM_WINDOW* wnd, OBJECT *adr_albumconf, unsigned long s_cat, int flag_aff)
{
  ...
  if (flag_aff)
  {
    for (i = ACONF_VISIBLE1; i <= ACONF_VISIBLE6; i++) GWObjcDraw( wnd, adr_albumconf, i ) ;
    for (i = ACONF_MASKED1; i <= ACONF_MASKED6; i++) GWObjcDraw( wnd, adr_albumconf, i ) ;
    GWObjcDraw( wnd, adr_albumconf, ACONF_VISIBLEBOX ) ;
    GWObjcDraw( wnd, adr_albumconf, ACONF_MASKEDBOX ) ;
  }
}

static void setup_category(unsigned long *s_cat, int num, int flag_mask, int nb_categories)
{
  long ishift = 1L ;
  int  i, j ;

  j = -1 ;
  for ( i = 0; i < nb_categories; i++, ishift <<= 1 )
  {
    if ( *s_cat & ishift )
    {
      if ( flag_mask )
  ...
  if ( j == num )
  {
    if ( flag_mask ) *s_cat &= ~ishift ;
    else             *s_cat |= ishift ;
  }
}

static void OnInitDialogAlbCfg(void* w)
{
  ...
  for ( i = 0; i < ARRAY_SIZE(obj_formats); i++ )
    if ( catalog_filter->format & (1 << imgformats[i]) ) select( adr_albumconf, obj_formats[i] ) ;
...
static int OnObjectNotifyAlbCfg(void* w, int obj)
{
  GEM_WINDOW*        wnd = (GEM_WINDOW*) w ;
  WEXTENSION_ALBCFG* wext = wnd->DlgData->UserData ;
  WDLG_ALBCFG*       wdlg = wnd->Extension ;
  ...
  switch( obj )
  {
    ...
    case ACONF_SHIFTST    : deselect(adr_albumconf, obj) ;
                            ...
                            sprintf(buf, "  %c  ", 'A'+wext->s1) ;
                            write_text(adr_albumconf, ACONF_SHIFTST, buf) ;
                            GWObjcDraw( wnd, adr_albumconf, ACONF_SHIFTST ) ;
                            break ;
    case ACONF_SHIFTEND   : deselect(adr_albumconf, obj) ;
                            ...
                            sprintf(buf, "  %c  ", 'A'+wext->s2) ;
                            write_text(adr_albumconf, ACONF_SHIFTEND, buf) ;
                            GWObjcDraw( wnd, adr_albumconf, ACONF_SHIFTEND ) ;
                            break ;
  ...
  if ( code == IDOK )
  {
    ...
    for ( i = 0; i < ARRAY_SIZE(obj_formats); i++ )
      if ( !selected(adr_albumconf, obj_formats[i]) ) wext->imgformat &= ~(1 << imgformats[i]) ;
...
static int OnCloseAlbCfg(void* w)
{
  GEM_WINDOW*  wnd = (GEM_WINDOW*) w ;
  WDLG_ALBCFG* wdlg = wnd->Extension ;
...
static int OnTxtBubbleAlbCfg(void* w, int mx, int my, char* txt)
{
  GEM_WINDOW* wnd = (GEM_WINDOW*) w ;
  OBJECT*     adr_form =  wnd->DlgData->BaseObject ;
  int         obj = objc_find( adr_form, 0, MAX_DEPTH, mx, my ) ;
  int         objs[] = { ACONF_VIS2MASK, ACONF_MASK2VIS, ACONF_TSAVEINDEX, ACONF_SAVEINDEX, ACONF_SHIFTST,   ACONF_SHIFTEND } ;
  int         msgs[] = { MSG_HACONFV2M,  MSG_HACONFM2V,  MSG_HACONFINDEX,  MSG_HACONFINDEX, MSG_HACONFSHIFT, MSG_HACONFSHIFT } ;

  if ( vget_txtbubblemsg( obj, objs, msgs, ARRAY_SIZE(objs), txt ) ) return 1 ;

  if ( (obj >= ACONF_VISIBLE1) && (obj <= ACONF_VISIBLE6) )
  {
    strcpy( txt, vMsgTxtGetMsg(MSG_HACONFVIS) ) ;
    return 1 ;
  }
  if ( (obj >= ACONF_MASKED1) && (obj <= ACONF_MASKED6) )
  {
    strcpy( txt, vMsgTxtGetMsg(MSG_HACONFMASK) ) ;
    return 1 ;
  }
...
static int FGetAlbCfg(WEXTENSION_ALBCFG* wext)
{
  GEM_WINDOW* dlg ;
  DLGDATA     dlg_data ;
...
int album_config(HCAT hCat)
{
  ...
  wext.hCat            = hCat ;
  wext.fplanes         = catalog_filter->planes ;
  wext.fshow_all       = catalog_filter->show_all ;
  wext.imgformat       = catalog_filter->format ;
  wext.show_categories = catalog_filter->category ;
