...
static void OnInitDialogEHTML(void* w)
{
...
static int OnObjectNotifyEHTML(void* w, int obj)
{
  GEM_WINDOW*          wnd = (GEM_WINDOW*) w ;
  WEXTENSION_ECATHTML* wext = wnd->DlgData->UserData ;
  ...
  switch( obj )
  {
     case EHTML_TLINKIMG  : inv_select( adr_ehtml, EHTML_LINKIMG ) ;
                            GWObjcDraw( wnd, adr_ehtml, EHTML_LINKIMG ) ;
                            break ;

     case EHTML_TSHOWNAMES: inv_select( adr_ehtml, EHTML_SHOWNAMES ) ;
                            GWObjcDraw( wnd, adr_ehtml, EHTML_SHOWNAMES ) ;
                            break ;
...
int FGetHTMLExport(CHTML_EXPORT* exp)
{
  ...
  return GWCreateAndDoModal( &dlg_data, EHTML_TITLE ) ;
}
...
void HTMLCatalogExport(HCAT hCat, long first, long last)
{
  ...
  {
    sprintf( buf, vMsgTxtGetMsg(MSG_EHTML), exp.page_title ) ;
