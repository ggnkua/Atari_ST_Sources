...
static void OnInitDialogBConv(void* w)
{
  ...
  wdlg->pb = 0 ;
  write_text(adr_bconv, BCONV_PBCONV, vMsgTxtGetMsg(MSG_BCONVDIT) ) ;
  strcpy(wdlg->popup_pb[1].ob_spec.free_string, vMsgTxtGetMsg(MSG_BCONVDIT) ) ;
  strcpy(wdlg->popup_pb[2].ob_spec.free_string, vMsgTxtGetMsg(MSG_BCONVCAN) ) ;
...
static void objc_redraws(GEM_WINDOW* wnd)
{
  ...
  GWObjcDraw( wnd, adr_bconv, BCONV_DONOTHING ) ;
  GWObjcDraw( wnd, adr_bconv, BCONV_REMOVESRC ) ;
  GWObjcDraw( wnd, adr_bconv, BCONV_MOVESRC ) ;
}
...
static int OnObjectNotifyBConv(void* w, int obj)
{
  ...
  switch( obj )
  {
    ...
    case BCONV_DSTFMT :     objc_offset(adr_bconv, BCONV_DSTFMT, &off_x, &off_y) ;
                            ...
                            GWObjcDraw( wnd, adr_bconv, obj ) ;
                            GWObjcDraw( wnd, adr_bconv, BCONV_OPT ) ;
                            break ;
    ...
    case BCONV_MOVEDEST  :  read_text( adr_bconv, obj, wdlg->buffer ) ;
     ...
     case BCONV_TERASEDST : inv_select( adr_bconv, BCONV_ERASEDST ) ;
                            GWObjcDraw( wnd, adr_bconv, BCONV_ERASEDST ) ;
                            break ;
...
int FBConv(BCONV_PARAMS *p)
{
  ...
  return( GWCreateAndDoModal( &dlg_data, 0 ) == IDOK ) ;
}
