...
static int T24_init(short wd)
{
  ...
  xctc = (int*) Xcalloc( wd, 2*sizeof(int) ) ;
  if ( xctc == NULL ) return -1 ;
  xr3 = (int*) Xcalloc( wd, 2*sizeof(int) ) ;
  if ( xr3 == NULL )
  {
    Xfree( xr3 ) ;
    return -1 ;
  ...
  af     = 3.0*3.141592653589793*(float)r2 ;
  taille = 100L+(long)af ;
  ...
  if ( stream )
  {
    LoggingDo(LL_INFO, "Reading %ld bytes from %s...", taille, name) ;
  ...
  LoggingDo(LL_INFO, "Generating True Color wheel (%ld bytes)...", taille) ;
  pt     = T24 ;
  lshift = 128/wd ;
  ...
  if ( stream )
  {
    LoggingDo(LL_INFO, "Saving %s (%ld bytes)...", name, taille) ;
...
static void makeT24(MFDB *img, unsigned char *new, float pc)
{
  float         val ;
  int           i ;
  int           x, y, k, l ;
...
static int cdecl draw_tcwheel(PARMBLK* paramblk)
{
...
static void show_color(GEM_WINDOW* wnd, int flag_lum)
{
  ...
  else
  {
    ...
    /* It's because on Aranym, this box object is whited after a click on it */
    int  x, y ;
...
static void show_pc(GEM_WINDOW* wnd, float pc)
{
  ...
  GWObjcDraw( wnd,adr_24bits, TCPAL_PERCENT ) ;
}
...
static void Adapt2Resolution(OBJECT* form_tcwheel)
{
  if ( (Xmax > 1439) && (Ymax > 899) )
  {
    int add_tcpal_box = form_tcwheel[TCPAL_BOX].ob_width ;

    LoggingDo(LL_INFO, "Resolution is over 1440x900, adding %d pixels to TC wheel", add_tcpal_box) ;
...
static void OnInitDialogTCWheel(void* w)
{
  GEM_WINDOW*         wnd = (GEM_WINDOW*) w ;
  WEXTENSION_TCWHEEL* wext = wnd->DlgData->UserData ;
  OBJECT*             adr_24bits = wnd->DlgData->BaseObject ;
  ...
  off_y = off_y-off_yp ;
  wext->hf    = (float) (adr_24bits[TCPAL_BOXPC].ob_height-adr_24bits[TCPAL_LUMSLIDER].ob_height) ;
  wext->pclum = 100.0*(1.0-(float)off_y/wext->hf) ;
...
static void handle_tcpal(GEM_WINDOW* wnd)
{
  WEXTENSION_TCWHEEL* wext = wnd->DlgData->UserData ;
  OBJECT*             adr_24bits = wnd->DlgData->BaseObject ;
  int                 off_y ;
  ...
  else if ( wext->pclum > 100.0 ) wext->pclum = 100.0 ;
  off_y = (int) ((100.0-wext->pclum)*(float)wext->hf/100.0) ;
  adr_24bits[TCPAL_LUMSLIDER].ob_y = off_y ;
  memcpy( wext->newT24, T24, wext->taille ) ;
  makeT24( &wext->TCWheel, wext->newT24, wext->pclum ) ;
  show_pc( wnd, wext->pclum ) ;
  GWObjcDraw( wnd, adr_24bits, TCPAL_BOX ) ;
  GWObjcDraw( wnd, adr_24bits, TCPAL_BOXPC ) ;
  GWObjcDraw( wnd, adr_24bits, TCPAL_LUMSLIDER ) ;
}
...
static int OnObjectNotifyTCWheel(void* w, int obj)
{
  ...
  switch( obj )
  {
    case TCPAL_LUM       : show_color( wnd, 0 ) ;
                           if ( config.use_rtzoom ) rtzoom_display( xm, ym, 1 ) ;
                           valid = 1 ;
                           break ;
    case TCPAL_BOX       : show_color( wnd, 1 ) ;
                           if ( config.use_rtzoom ) rtzoom_display( xm, ym, 1 ) ;
                           valid = 1 ;
                           break ;
    case TCPAL_LUMMOINS  :
  ...
  if ( valid )
  {
    ...
    wext->rvbnew[0] = RGB8BToRGBPM[atoi(buffer)] ;
    read_text( adr_24bits, TCPAL_GREEN, buffer ) ;
    wext->rvbnew[1] = RGB8BToRGBPM[atoi(buffer)] ;
    read_text( adr_24bits, TCPAL_BLUE, buffer ) ;
    wext->rvbnew[2] = RGB8BToRGBPM[atoi(buffer)] ;
    svs_color( 17, wext->rvbnew ) ;
    ltype.color  = 17 ;
    mtype.color  = 17 ;
    ttype.color  = 17 ;
    ftype.color  = 17 ;
...
static int OnCloseDialogTCWheel(void* w)
{
  GEM_WINDOW*         wnd = (GEM_WINDOW*) w ;
  WEXTENSION_TCWHEEL* wext = wnd->DlgData->UserData ;
