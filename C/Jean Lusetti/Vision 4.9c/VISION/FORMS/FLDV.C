...
void setup_ldvlist(GEM_WINDOW* wnd, int display)
{
  ...
  if ( display )
  {
    GWObjcDraw( wnd, bobj, LDV_BOX ) ;
    for ( i = LDV_ITEM1; i <= LDV_ITEM8; i++ )
      GWObjcDraw( wnd, bobj, i ) ;
  }
}

void display_ldvinfos(GEM_WINDOW* wnd)
{
  char           objs[] = { LDV_LONGNAME, LDV_TYPE, LDV_VERSION, LDV_AUTHORS, LDV_DESC1, LDV_DESC2, LDV_SELECT, LDV_DETAIL } ;
  LDV_MODULE     *ldv ;
  LDV_INFOS      *ldv_infos ;
  ...
  else
  {
    ...
    if ( LDVGetNPlanesCaps( ldv, vximage->inf_img.mfdb.fd_nplanes ) == NULL )
    {
      bobj[LDV_SELECT].ob_state |= DISABLED ;
/*      bobj[LDV_DETAIL].ob_state |= DISABLED ;*/
    }
    ...
    if ( LDVList )
    {
      if ( selected( bobj, LDV_COMPAT ) )
      {
       int i ;
        ...
        {
          if ( vimage && LDVGetNPlanesCaps( mod, vimage->inf_img.mfdb.fd_nplanes ) )
...
static int OnObjectNotify(void *w, int obj)
{
  ...
  switch( obj )
  {
     ...
     case LDV_TCOMPAT: inv_select( bobj, LDV_COMPAT ) ;
                       GWObjcDraw( wnd, bobj, LDV_COMPAT ) ;
...
void LDVFreeAllModules(void)
{
  ...
  Vapi = NULL ;
#else
  LDVFreeModuleList( TotalLDVList ) ;
#endif
  TotalLDVList = NULL ;
  TotalNbLDV   = 0 ;
}
