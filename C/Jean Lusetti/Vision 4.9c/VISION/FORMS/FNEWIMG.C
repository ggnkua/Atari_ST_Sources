...
void OnInitDialogNewImg(void *w)
{
  ...
  sprintf(buf, "%.4d", screen.fd_w) ;
  write_text(adr_new, NEW_X, buf) ;
  sprintf(buf, "%.4d", screen.fd_h) ;
  write_text(adr_new, NEW_Y, buf) ;
}

static void sel_color(OBJECT* adr_new, int sel)
{
  deselect( adr_new, COLOR_2 ) ;
  deselect( adr_new, COLOR_4 ) ;
  deselect( adr_new, COLOR_16 ) ;
...
int OnObjectNotifyNewImg(void *w, int obj)
{
  ...
  switch( obj )
  {
     case COLOR_T2   : sel_color( adr_new, COLOR_2 ) ;
                       break ;
     case COLOR_T4   : sel_color( adr_new, COLOR_4 ) ;
                       break ;
     case COLOR_T16  : sel_color( adr_new, COLOR_16 ) ;
                       break ;
     case COLOR_T256 : sel_color( adr_new, COLOR_256 ) ;
                       break ;
  ...
  GWObjcDraw( wnd, adr_new, COLOR_2 ) ;
  GWObjcDraw( wnd, adr_new, COLOR_4 ) ;
  GWObjcDraw( wnd, adr_new, COLOR_16 ) ;
  GWObjcDraw( wnd, adr_new, COLOR_256 ) ;
  ...
  if ( code == IDOK )
  {
    ...
    tpmx = atoi(buf) ;
    if (tpmx == 0) tpmx++ ;
    tpmx = (int) (25400.0/(float)tpmx) ;
    read_text(adr_new, NEW_DPIY, buf) ;
    tpmy = atoi(buf) ;
    if (tpmy == 0) tpmy++ ;
    tpmy = (int) (25400.0/(float)tpmy) ;
...
void traite_nouveau(void)
{
  ...
  (void)GWCreateAndDoModal( &dlg_data, NEW_X ) ;
}
