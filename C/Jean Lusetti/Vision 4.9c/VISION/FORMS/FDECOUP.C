...
static void OnInitDialogDecoup(void* w)
{
...
static int OnObjectNotifyDecoup(void* w, int obj)
{
  GEM_WINDOW*        wnd = (GEM_WINDOW*) w ;
  WEXTENSION_DECOUP* wext = wnd->DlgData->UserData ;
  OBJECT*            adr_rsc = wnd->DlgData->BaseObject ;
...
int get_decoup(DECOUP* dcp)
{
  ...
  if ( code == IDOK ) memcpy( dcp, &wext.decoup, sizeof(DECOUP) ) ;
