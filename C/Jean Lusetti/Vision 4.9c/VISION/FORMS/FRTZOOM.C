...
static void show_prtzoom(int show, GEM_WINDOW* wnd)
{
  ...
  for ( i = 0; i < ARRAY_SIZE(ids); i++ )
    GWObjcDraw( wnd, adr_rtzoom, ids[i] ) ;
}

static void OnInitDialogRTZoom(void* w)
{
...
static void draw_objs(GEM_WINDOW* wnd, OBJECT* adr_prtzoom)
{
  GWObjcDraw( wnd, adr_prtzoom, ZOOMRT_IMG ) ;
  GWObjcDraw( wnd, adr_prtzoom, ZOOMRT_SCREEN ) ;
  GWObjcDraw( wnd, adr_prtzoom, ZOOMRT_SMART ) ;
}
...
static int OnObjectNotifyRTZoom(void* w, int obj)
{
  ...
  switch( obj )
  {
   ...
   case ZOOMRT_INCSIZE  : if ( wext->w < RTZ_MAXSIZE ) wext->w += 16 ;
                          ...
                          write_text( adr_prtzoom, ZOOMRT_SIZE, buf ) ;
                          GWObjcDraw( wnd, adr_prtzoom, ZOOMRT_SIZE ) ;
                          break ;
...
static int OnCloseDlgRTZoom(void* w)
{
  GEM_WINDOW*  wnd = (GEM_WINDOW*) w ;
  WDLG_RTZOOM* wdlg = wnd->Extension ;
...
int pref_rtzoom(void)
{
  ...
  code = GWCreateAndDoModal( &dlg_data, -1 ) ;
