...
static void g_color(short index, short gcolor_level)
{
  int rgb[3] ;

  rgb[0] = rgb[1] = rgb[2] = gcolor_level ;
  svs_color( index, rgb ) ;
}
...
void traite_coupeff(int flag)
{
  ...
  if ( vclip.form == BLOC_RECTANGLE )
  {
    ...
    svro_cpyfm( ALL_WHITE, xy, &vimage->inf_img.mfdb, &vimage->inf_img.mfdb ) ;
    if ( Truecolor )
  ...
  else
  {
    ...
    svro_cpyfm( NOTS_AND_D, xy, &clipboard.mask, &vimage->inf_img.mfdb ) ;
  }
...
void traite_copie(void)
{
  if ( vclip.form == BLOC_RECTANGLE )
  {
    LoggingDo(LL_INFO,"Copy (%d,%d,%d,%d) image block for window $%p", vclip.x1, vclip.y1, vclip.x2, vclip.y2, vclip.gwindow) ;
    clip_put( vclip.gwindow, vclip.x1, vclip.y1, vclip.x2, vclip.y2 ) ;
  }
  else LoggingDo(LL_INFO,"Copy error") ;
}
...
void traite_negatif(void)
{
  ...
  else
  {
    if (config.flags & FLG_MASKOUT)
    {
      ...
      svro_cpyfm( D_INVERT, xy, &clipboard.mask, &clipboard.mask) ;
    }
    xy[0] = xy[1] = 0 ;
    xy[2] = vclip.x2-vclip.x1 ;
    xy[3] = vclip.y2-vclip.y1 ;
    svro_cpyfm( S_XOR_D, xy, &clipboard.mask, &vimage->inf_img.mfdb) ;
    if (config.flags & FLG_MASKOUT)
    {
      xy[0] = xy[1] = xy[4] = xy[5] = 0 ;
      xy[2] = xy[6] = clipboard.mask.fd_w-1 ;
...
void put_bloc(GEM_WINDOW* wnd, int x, int y, int mode)
{
  ...
  if ( clipboard.mask.fd_addr ) /* Image au lasso */
  {
    svro_cpyfm( NOTS_AND_D, xy, &clipboard.mask, &screen ) ;
    svro_cpyfm( S_OR_D, xy, &clipboard.img, &screen ) ;
  }
  else
    svro_cpyfm( mode, xy, &clipboard.img, &screen ) ;
}
...
void restore_img(int posx, int posy, int x, int y)
{
  ...
  if ( vimage->zoom_level == 1 ) src = &vimage->inf_img.mfdb ;
  else
  {
    src = &vimage->zoom ;
    posx -= vimage->x1 ;
  ...
  xy[6] = xy[4]+w-1 ;
  xy[7] = xy[5]+h-1 ;
  svro_cpyfmSOS( xy, src ) ;
}

void traite_colle(GEM_WINDOW* cur_window, short x, short y)
{
  VXIMAGE*  vimage ;
  MOUSE_DEF save ;
  int       xy[8] ;
  ...
  if ( clipboard.img.fd_nplanes > vimage->inf_img.mfdb.fd_nplanes )
  {
    ...
    rep = form_interrogation( 1, vMsgTxtGetMsg(MSG_WARNPASTE) ) ;
    if ( rep == 2 ) return ;
  }
  ...
  if ( (x == -1) && (y == -1) )
  {
    posx = vimage->x1 ;
    posy = vimage->y1 ;
    graf_mkstate( &xm1, &ym1, &dummy, &dummy ) ;
    GWHideCursor() ;
    ...
    for (;;)
    {
      ...
      if ( ((xm2 < xi) || (xm2 > xi+wx-1) || (ym2 < yi) || (ym2 > yi+wy-1)) && (zoom_level == 1) )
      {
        ...
        GWHideCursor() ;
        svro_cpyfmSOS( xy, &vimage->inf_img.mfdb ) ;
      ...
      else if ( (xm2 != xm1) || (ym2 != ym1) )
      {
        if ( (zoom_level == 1) || ( (xm2 >= xi) && (ym2 >= yi) ) )
        {
          ...
          xm1 = xm2 ; ym1 = ym2 ;
          if ( config.use_rtzoom ) rtzoom_display( xm2, ym2, zoom_what ) ;
          sprintf( buf, vMsgTxtGetMsg(MSG_XYLH) , posx+xm2-xi, posy+ym2-yi, clipboard.w, clipboard.h ) ;
  ...
  else bm = 0 ; /* Location of where to put block passed as parameter */
  ...
  if ( (x < xi+wx-1) && (y < yi+wy-1) && (bm != 2) )
  {
    ...
    if ( xy[4]+rw > vimage->inf_img.mfdb.fd_w ) rw = vimage->inf_img.mfdb.fd_w-xy[4] ;
    xy[2] = xy[0]+rw-1 ; xy[6] = xy[4]+rw-1 ;
    rh = clipboard.h/zoom_level ;
    if ( xy[5]+rh > vimage->inf_img.mfdb.fd_h ) rh = vimage->inf_img.mfdb.fd_h-xy[5] ;
    xy[3] = xy[1]+rh-1 ; xy[7] = xy[5]+rh-1 ;
  
    undo_put( cur_window, xy[4], xy[5], xy[6], xy[7], UCOLLE, NULL ) ;
    if ( zoom_level == 1 )
    {
      if ( clipboard.mask.fd_addr )
      {
        svro_cpyfm( NOTS_AND_D, xy, &clipboard.mask, &vimage->inf_img.mfdb ) ;
        svro_cpyfm( S_OR_D, xy, &clipboard.img, &vimage->inf_img.mfdb ) ;
      }
      else svro_cpyfm( mode, xy, &clipboard.img, &vimage->inf_img.mfdb ) ;
    }
    else update_raster( cur_window, xi, yi, xi+wx-1, yi+wy-1 ) ;
...
void traite_imprime(GEM_WINDOW* gwnd)
{
  ...
  {
    case PNOGDOS   : vform_stop( MSG_PNOGDOS ) ;
                     break ;
    case PTIMEOUT  : vform_stop( MSG_PTIMEOUT ) ;
                     break ;
    case PWRITERR  : vform_stop( MSG_PWRITERR ) ;
                     break ;
    case PNOHANDLE : vform_stop( MSG_PNOHANDLE ) ;
                     break ;
    case PROTERR   : form_error_mem() ;
                     break ;
    case PNODRIVER : vform_stop( MSG_PNODRIVER ) ;
                     break ;
  }
}
...
void analyse_lasso(int* xy, int nb)
{
  ...
  GWSetWindowInfo( cur_window, vMsgTxtGetMsg(MSG_CALCLASSO)  ) ;
  cur_window->GetWorkXYWH( cur_window, &xi, &yi, &wx, &wy ) ;
  xmin = ymin = 32767 ;
  xmax = ymax = -32767 ;
  c = xy ;
  ...
  svro_cpyfmSO( xyarray, &vimage->inf_img.mfdb, &masked_img ) ;
  ...
  GWHideCursor() ;
  svr_recfl( cxy ) ;
  svsf_color( color_mask ) ;
  svsl_color( color_mask ) ;
  ...
  GWHideCursor() ;
  svro_cpyfmSO( xyarray, &screen, &mask ) ;
  GWShowCursor() ;

  memcpy(xyarray, &xyarray[4], 4*sizeof(int)) ;
  svro_cpyfm( S_AND_D, xyarray, &mask, &masked_img ) ;
...
void traite_lasso(void)
{
  GEM_WINDOW* cur_window = GemApp.CurrentGemWindow ;
  VXIMAGE*    vimage ;
  int         lassoxy[1000] ; /* NVDI doesn't go above */
  int         nbpt = 0 ;
  int         xy[4] ;
  ...
  last_x = last_y = -1 ;
  sprintf( buf, vMsgTxtGetMsg(MSG_XYLASSO) , posx+xm1-xi, posy+ym1-yi, nbpt ) ;
  GWSetWindowInfo( cur_window, buf ) ;
  do
  {
    graf_mkstate( &xm2, &ym2, &bm, &dummy ) ;
    if ( ((xm1 != xm2) || (ym1 != ym2)) && (last_x != -1) )
    {
      ...
      sprintf( buf, vMsgTxtGetMsg(MSG_XYLASSO) , posx+xm2-xi, posy+ym2-yi, nbpt ) ;
      GWSetWindowInfo( cur_window, buf ) ;
    }
    if ( (bm == 1) && (nbpt < 500) )
    {
...
void traite_blocsimple(void)
{
  ...
  do
  {
    ...
    if ( (x2 < xi) || (x2 > xi+wx-1) || (y2 < yi) || (y2 > yi+wy-1) )
    {
      ...
      if ( x2 < xi )      posx += 16 ;
      if ( x2 > xi+wx-1 ) posx -= 16 ;
      if ( y2 < yi )      posy += 16 ;
      if ( y2 > yi+wy-1 ) posy -= 16 ;
      if ( posx < 0 ) posx = 0 ;
      if ( posx+wx > vimage->inf_img.mfdb.fd_w ) posx = vimage->inf_img.mfdb.fd_w-wx ;
      if ( posy < 0 ) posy = 0 ;
      if ( posy+wy > vimage->inf_img.mfdb.fd_h ) posy = vimage->inf_img.mfdb.fd_h-wy ;
    ...
    sprintf( buf, vMsgTxtGetMsg(MSG_XYLH), MIN(xe,x2)-xi+posx, MIN(ye,y2)-yi+posy, abs(1+x2-xe), abs(1+y2-ye) ) ;
    GWSetWindowInfo( cur_window, buf ) ;
  }
  while ( bm == 1 ) ;
  ...
  vclip_sel( cur_window, xr, yr, posx+x2-xi, posy+y2-yi ) ;
}
...
GEM_WINDOW* nouvelle_wimage(char* nom, int nx, int ny, int nc, int tpmx, int tpmy)
{
  ...
  vimage->inf_img.c.f.jpg.quality = 85 ; /* Set a fair JPEG quality by default */
  ...
  {
    inf_img->palette = Xcalloc( nb_colors, 6 ) ;
...
int micron2inch(int pix)
{
 return (int) (25400.0/(float)pix) ;
}
...
void traite_ncolle(char* fname)
{
  ...
  svro_cpyfmSO( xy, &clipboard.img, &new_vimage->inf_img.mfdb ) ;
...
int traite_loupe(GEM_WINDOW* cur_window, int flag_level, int mx, int my)
{
  ...
  if ( vimage->x2 >= vimage->inf_img.mfdb.fd_w ) vimage->x2 = vimage->inf_img.mfdb.fd_w-1 ;
  if ( vimage->y2 >= vimage->inf_img.mfdb.fd_h ) vimage->y2 = vimage->inf_img.mfdb.fd_h-1 ;
  xy[0] = vimage->x1 ; xy[1] = vimage->y1 ;
  xy[2] = vimage->x2 ; xy[3] = vimage->y2 ;
  GWSetWindowInfo( cur_window, vMsgTxtGetMsg(MSG_CALCULZOOM)  ) ;
  ...
  xi = pmfr( vimage->x1, vimage->inf_img.mfdb.fd_w-wx ) ;
  yi = pmfr( vimage->y1, vimage->inf_img.mfdb.fd_h-wy ) ;
  GWSetHSlider( cur_window, xi ) ;
  GWSetVSlider( cur_window, yi ) ;
  wxe = pmfr( wx, vimage->inf_img.mfdb.fd_w ) ;
  whe = pmfr( wy, vimage->inf_img.mfdb.fd_h ) ;
...
void traite_main(void)
{
  ...
  while ( cont )
  {
    ...
    if ( (xm2 != xm1) || (ym1 != ym2) )
    {
      ...
      GWHideCursor() ;
      svro_cpyfmSOS( xyarray, &vimage->inf_img.mfdb ) ;
      GWShowCursor() ;
      if ( config.use_rtzoom )
      {
        svs_clipOFF( cxy ) ;
...
void VSetIHVSliders(GEM_WINDOW* gwnd, int ww, int wh)
{
  GWSetHSlider( gwnd, 1 ) ;
  GWSetVSlider( gwnd, 1 ) ;
...
{
  char  errs[] = { ELDV_INVALIDPARAMETER, ELDV_NBPLANESNOTSUPPORTED, ELDV_NOTENOUGHMEMORY, ELDV_PREVIEWNOTSUPPORTED, ELDV_RUNNOTSUPPORTED, ELDV_BADVAPIVERSION, ELDV_IMGFORMATMISMATCH, ELDV_CANCELLED } ;
  int   msgs[] = { MSG_ELDVPINVALID,      MSG_ELDVNBPLANES,          MSG_ELDVMEMORY,       MSG_ELDVNOPREV,           MSG_ELDVRUN,          MSG_ELDVVAPI,        MSG_ELDVIMGFMT,         -1 } ;
  char* msg_err = NULL ;
  int   i ;
  ...
  for ( i = 0; i < ARRAY_SIZE(errs); i++ )
    if ( err == errs[i] )
    {
      if ( msgs[i] > 0 ) msg_err = vMsgTxtGetMsg( msgs[i] ) ;
      break ;
    }

  if ( i == ARRAY_SIZE(errs) ) msg_err = vMsgTxtGetMsg( MSG_ELDVGENERAL )  ;
...
void traite_iconpopup(GEM_WINDOW* cur_window)
{
  ...
  {
    strcpy( popup[1+i-MSG_POPOPEN].ob_spec.free_string, vMsgTxtGetMsg(i)  ) ;
  ...
  switch ( st_pop+clicked-1 )
  {
    ...
    case MSG_POPRUN     : if ( st_pop+clicked-1 == MSG_POPRUN ) strcpy( mask, "*.PRG" ) ;
                          ...
                          {
                            if ( mask[2] == 'P' ) Pexec( Multitos ? 100:0, file, NULL, NULL ) ;
