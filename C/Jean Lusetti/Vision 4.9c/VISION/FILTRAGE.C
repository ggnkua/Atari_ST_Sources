...
void traite_convolution(GEM_WINDOW* gwnd)
{
  ...
  else
  {
    ...
    else
    {
      ...
      svro_cpyfmSO( xy, &vimage->inf_img.mfdb, &virtuel) ;
    }
    undo_return = undo_put( gwnd, x1, y1, x2, y2, UFILTRE, NULL ) ;
  }

  mouse_busy() ;
  sprintf( DSPfile, "%s\\CONVOL.LOD", config.path_lod ) ;
  handlelock = DspInfo.WhatToDoIfLocked ;
  do
  {
    wprog = DisplayStdProg( vMsgTxtGetMsg(MSG_FILTRE) , "", "", CLOSER ) ;
    ...
    {
      int rep = form_interrogation(1, vMsgTxtGetMsg(MSG_DSPLOCK) ) ;
  ...
  if ( ret ==  0)
  {
    if ( choix == 2 )
    {
      ...
      if ( vclip.form == BLOC_RECTANGLE )
        svro_cpyfmSO( xy, &out, &vimage->inf_img.mfdb ) ;
      else
      {
        int xyarray[8] ;
        ...
        if ( config.flags & FLG_MASKOUT )
        {
          ...
          svro_cpyfm( S_AND_D, xy, &clipboard.mask, &vimage->inf_img.mfdb ) ;
          svro_cpyfm( NOTS_AND_D, xy, &clipboard.mask, &out ) ;
          memcpy( xy, &xy[4], 4*sizeof(int) ) ;
          svro_cpyfm( S_OR_D, xy, &vimage->inf_img.mfdb, &out ) ;
          Xfree( vimage->inf_img.mfdb.fd_addr ) ;
          CopyMFDB( &vimage->inf_img.mfdb, &out ) ;
        }
        else
        {
          svro_cpyfm( NOTS_AND_D, xyarray, &clipboard.mask, &vimage->inf_img.mfdb ) ;
          xyarray[4] = filtre.w ;
          xyarray[5] = filtre.h ;
          xyarray[6] = filtre.w+xy[2]-xy[0] ;
          xyarray[7] = filtre.h+xy[3]-xy[1] ;
          svro_cpyfm( S_AND_D, xyarray, &clipboard.mask, &out ) ;
          svro_cpyfm( S_OR_D, xy, &out, &vimage->inf_img.mfdb ) ;
        }
      }
      if ( !(config.flags & FLG_MASKOUT) )
      {
  ...
  else
  {
    switch(ret)
    {
      case CONVOL_MEMORY        : form_error_mem() ;
                                  if ( undo_return == 0 ) free_lastundo( gwnd ) ;
                                  break ;
      case CONVOL_BADFILTERFILE : vform_stop( MSG_BADFILTER ) ;
                                  break ;
      case CONVOL_BADNBCOLORS   : vform_stop( MSG_FNBCOLORS ) ;
                                  break ;
      case CONVOL_BADDSPFILE    : vform_stop( MSG_BADLOD ) ;
                                  break ;
      case CONVOL_DSPBUSY       : vform_stop( MSG_DSPISLOCKED ) ;
                                  break ;
      case CONVOL_LODNOTEXIST   : vform_stop( MSG_DSPNOLOD ) ;
                                  break ;
    }
  }
  mouse_restore() ;

  if ( filtre.data ) Xfree( filtre.data ) ;
}
