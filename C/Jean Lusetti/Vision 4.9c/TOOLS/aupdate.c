...
static short AUGetCmdLine4PExec(char* updater, char* url, char* cmd_line, char* url_file, char* local_file, char* trusted_ca_file)
{
  ...
  if ( strstr( updater, "curl" ) )
  {
    ...
    sprintf( cmd_line, "-L %s --get %s%s --output %s", tls_param, buf, url_file, local_file ) ;
  }
  else if ( strstr( updater, "wget" ) ) sprintf( cmd_line, "%s%s -O %s", buf, url_file, local_file ) ;
...
short AUCheckUpdate(AU_CONTEXT* AUContext)
{
  ...
  LoggingDo(LL_DEBUG, "AUCheckUpdate: request update with %s %s", AUContext->updater, cmd_line) ;
  cmd_status = XPexec( 0, AUContext->updater, cmd_line ) ;
  if ( cmd_status != 0 )
  {
    if ( cmd_status == EFILNF ) return AU_NOUPDATER ;
    LoggingDo(LL_ERROR, "AUCheckUpdate: command line %s %s returned error %ld", AUContext->updater, cmd_line, cmd_status) ;
...
static short AUUpdateFile(char* filename, AU_CONTEXT* AUContext)
{
  ...
  LoggingDo(LL_DEBUG, "AUUpdateFile: fetching file from Internet with %s %s", AUContext->updater, cmd_line) ;
  cmd_status = XPexec( 0, AUContext->updater, cmd_line ) ;
