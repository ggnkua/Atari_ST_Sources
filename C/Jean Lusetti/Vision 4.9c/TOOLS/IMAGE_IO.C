...
int incnbplanes(MFDB* data, int nb_plan, int* cancel_stdvdi)
{
  ...
  {                            /* VDI s'occupe de la conversion */
    ...
    cmin = 3*1000 ; cmax = 0 ;
    imin = imax = 0 ;
    for ( i = 0; i < (1 << screen.fd_nplanes); i++ )
    {
      val  = *pt_pal++ ;
      val += *pt_pal++ ;
      val += *pt_pal++ ;
    ...
    color_index[0] = imin ; /* Color index for pixel set     */
    color_index[1] = imax ; /* Color index for pixel not set */
    LoggingDo( LL_DEBUG, "incnbplanes with 1 plane source, index black=%d(%d,%d,%d), index_white=%d(%d,%d,%d)", imin, pal[3*imin], pal[3*imin+1], pal[3*imin+2], imax, pal[3*imax], pal[3*imax+1], pal[3*imax+2] ) ;
...
int fit_to_planes(INFO_IMAGE* inf, int out_planes, GEM_WINDOW* wprog)
{
  ...
  LoggingDo(LL_DEBUG, "fit_to_planes, fd_nplanes=%d, out_planes=%d, inf->nplans=%d, %d colors for palette=%p", img->fd_nplanes, out_planes, inf->nplanes, (short)inf->nb_cpal, inf->palette) ;
  ...
  if ( inf->nplanes > out_planes )
  {
    ...
    {
      inf->palette = Xcalloc( nb_colors, 6 ) ;
  ...
  if ( (inf->nplanes < out_planes) && (out_planes > 8) )
  {
    ...
    StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_TRSFFMTSTD) ) ;
...
void adapt_palette(INFO_IMAGE* info_img)
{
  ...
  {
    int* new_pal = (int*) Xcalloc( nb_colors, 6 ) ;
...
/*  DImgGetPriorities( img_fmt, NB_MST_MAX ) ;*/
  ...
  for ( i = 0; img_fmt[i]; i++ )
  {
    ...
    if ( img_module )
    {
      ...
      {
        /* There is an extension which is not expected, let's do it but log a warning */
        char* pext = strrchr( name, '.' ) ;

        if ( pext ) LoggingDo(LL_INFO, "File extension is %s, inconsistent with IID %08lX, will save it anyway", pext, img_module->Capabilities.iid) ;
      }
...
int compute_histo(INFO_IMAGE* inf, HISTO* histo, GEM_WINDOW* wprog, int use_stdvdi)
{
  ...
  if ( use_stdvdi )
  {
    ...
    else                         svr_trnfm( &rsc_histo->std_img, img ) ;
  }
  FreeRscHisto( rsc_histo ) ;
...
static void internal2ms(size_t* data)
{
  if ( *data != PERF_NA ) *data = (1000L*(*data))/CLK_TCK ;
}

static void ms2internal(size_t* data)
{
  if ( *data != PERF_NA ) *data = (CLK_TCK*(*data))/1000L ;
}
...
void perf_setunit(int unit, PERF_INFO* perf_info)
{
  if ( unit != perf_info->unit )
  {
    if ( unit == PERF_UNITMS )
    {
      ...
      internal2ms( &perf_info->total_load_time ) ;
      internal2ms( &perf_info->dither_time ) ;
      internal2ms( &perf_info->disk_access_time ) ;
      internal2ms( &perf_info->compress_time ) ;
      internal2ms( &perf_info->ind2raster_time ) ;
      internal2ms( &perf_info->stdformat_time ) ;
      internal2ms( &perf_info->identify_time ) ;
    }
    else
    {
      /* Milli Seconds --> INTERNAL */
      ms2internal( &perf_info->total_load_time ) ;
      ms2internal( &perf_info->dither_time ) ;
      ms2internal( &perf_info->disk_access_time ) ;
      ms2internal( &perf_info->compress_time ) ;
      ms2internal( &perf_info->ind2raster_time ) ;
      ms2internal( &perf_info->stdformat_time ) ;
      ms2internal( &perf_info->identify_time ) ;
    }
    perf_info->unit = unit ;
  }
}
...
short info_image_duplicate(INFO_IMAGE* in, INFO_IMAGE* out)
{
  ...
  else
  {
    ...
    {
      out->palette = Xcalloc( in->nb_cpal, 3*sizeof(short) ) ;
      if ( out->palette == NULL )
      {
...
char* ImgMsgGeTxt(short id)
{
  static char def_txt[32] ;
  char*       txt = def_txt ;

  if ( ImgGetTxt4ID ) txt = ImgGetTxt4ID( id ) ;
  else                sprintf( def_txt, "Invalid Message ID %d", id ) ;

  return txt ;
}

void ImgSetTxt4ID(char* (*fct_mapidtxt)(int id))
{
  ImgGetTxt4ID = fct_mapidtxt ;
}
