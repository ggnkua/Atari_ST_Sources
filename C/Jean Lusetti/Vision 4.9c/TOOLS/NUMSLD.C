...
static char* GetLabel(NUM_SLD *num_sld)
{
  char* label = "???" ;
  char* pt = num_sld->Labels ;
  int   label_no = num_sld->ValMin ;

  while ( *pt )
  {
    if ( label_no == num_sld->Val )
    {
      label = pt ;
      break ;
    }
...
void UpdateValLabel(NUM_SLD* num_sld)
{
  ...
  if ( num_sld->Labels == NULL ) sprintf( buf, "%d", num_sld->Val ) ;
  else                           sprintf( buf, "%d(%s)", num_sld->Val, GetLabel(num_sld) ) ;
...
int Pos2Val(NUM_SLD* num_sld)
{
  ...
  else if ( val > num_sld->ValMax ) val = num_sld->ValMax ;
...
int SldNumGetVal(NUM_SLD* num_sld)
{
  ...
  if ( num_sld->Labels == NULL ) val = atoi( buf ) ;
...
{
/* Code size optimizeation untested as SldNumShowControl is not called
  int  objs[8] ;
  int* pobj ;

  pobj = &objs[0] ;
  *pobj++ = num_sld->IdBLess ;
  *pobj++ = num_sld->IdBox ;
  *pobj++ = num_sld->IdSlider ;
  *pobj++ = num_sld->IdBPlus ;
  *pobj++ = num_sld->IdTitle ;
  *pobj++ = num_sld->IdValMin ;
  *pobj++ = num_sld->IdValMax ;
  *pobj++ = num_sld->IdCurrentVal ;

  for ( pobj = &objs[0]; pobj < &objs[0] + ARRAY_SIZE(objs); pobj++ )
    if ( visible ) num_sld->Obj[*pobj].ob_flags &= ~HIDETREE ;
    else           num_sld->Obj[*pobj].ob_flags |= HIDETREE ;
*/
