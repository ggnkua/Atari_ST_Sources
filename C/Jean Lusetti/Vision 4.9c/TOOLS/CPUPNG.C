...
static void PNGCBAPI PNG_read_function( png_structp png_ptr, png_bytep data, png_size_t length)
{
  INFO_IMAGE* inf = (INFO_IMAGE*) png_get_io_ptr( png_ptr ) ;

  ImgFRead( inf, data, length ) ;
}
...
#define NB_BYTES_HEADER 8
  ...
  err = ImgFOpen( inf, name, NULL ) ;
  if ( err != EIMG_SUCCESS ) return err ;

  ImgFRead( inf, header, NB_BYTES_HEADER ) ;
  ...
  png_set_read_fn( png_str, inf, PNG_read_function ) ; /* Define our own fread function as fread is slow on NFS connections */
...
int cpu_png_load(INFO_IMAGE* inf)
{
  ...
  else out->fd_nplanes = png_get_bit_depth( png_str, png_inf ) * png_get_channels( png_str, png_inf ) ;
  ...
  for ( pass = 0; pass < number_passes; pass++ )
  {
    ...
    for ( y = 0; y < out->fd_h; y++, yc++ )
    {
      ...
      if ( PROG_CANCEL_CODE( GWProgRange( wprog, yc, all_y, NULL ) ) )
      {
