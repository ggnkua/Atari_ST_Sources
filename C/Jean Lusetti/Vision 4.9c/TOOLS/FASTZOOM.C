...
{
  if ( UseStdVDI && (nplanes <= 8) ) return 1;
  return 0 ;
}
...
int FastZoom(FZOOM_DATA *data)
{
  ...
  int        use_std_vdi = FZNeedStdFormat(data->src->fd_nplanes) ;
  ...
  if ( use_std_vdi )
  {
    ...
    svr_trnfm( pstd_dst, fzdata.dst ) ;
    svr_trnfm( pstd_src, data->src ) ;
