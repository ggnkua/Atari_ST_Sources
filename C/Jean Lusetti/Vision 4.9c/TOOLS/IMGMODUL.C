...
#ifndef __NO_LOGGING
static void DImgLogUsage(IMG_MODULE* img_module)
{
  char* module_type = " ???" ;

  if ( img_module->Type == MST_ZVLDG )       module_type = "ZVLDG" ;
  else if ( img_module->Type == MST_IMGMOD ) module_type = "ImgView IM" ;
#ifdef MST_ZVSLB
 else if ( img_module->Type == MST_ZVSLB ) module_type = "ZVSLB" ;
#endif

  LoggingDo(LL_IMG, "Unloading %s #%ld (used %ld times)", module_type, IMG_MODULE_INDEX(img_module), img_module->nUsages) ;
}
#else
  #define DImgLogUsage(x)
#endif

static void DImgUnloadImgModule(IMG_MODULE* img_module)
{
...
/*    case MST_LDI:*/
    case MST_ZVLDG:  if ( img_module->hLib )
                     {
                       DImgLogUsage( img_module ) ;
                       ldg_term( ap_id, img_module->hLib ) ;
                       img_module->hLib = NULL ;
                     }
                     break ;
    case MST_IMGMOD: if ( img_module->hLib )
                     {
                       DImgLogUsage( img_module ) ;
                       Mfree( img_module->hLib ) ;
                       img_module->hLib = NULL ;
                     }
                     break ;
#ifdef MST_ZVSLB
...
static short DImgLoadZView(char* filename, IMG_MODULE* img_module, short type)
{
  short ret = -1 ;
  ...
  if ( img_module->LibFilename == NULL )
  {
    LoggingDo(LL_ERROR, "Can't allocate module filename %s", filename) ;
    return ret ;
  }

  /* Zview has 3 possible types: MST_ZVLDG (original zView), MST_IMGMOD (Lonny Pursell), MST_ZVSLB (Thorsten Otto) */
  /* Don't use img_module->Type as it may have not been initialized yet */
  if ( type == MST_ZVLDG )
  {
    img_module->hLib = ldg_exec( ap_id, filename ) ;
    if ( img_module->hLib ) ret = EIMG_SUCCESS ;
  }
  else if ( type == MST_IMGMOD )
  {
    long status = Pexec( 3, filename, NULL, NULL ) ;

    if ( status > 0 )
    {
      BASEPAGE* basepage = (BASEPAGE*) status ;

      img_module->hLib = basepage ;
      /* mshrink() it with some values from the basepage pointer
          then call main() with basepage+256 which returns a pointer to a table of pointers for all the functions
          See PExec(3 at: http://websites.umich.edu/~archive/atari/Programming/pexeccbk.txt                       
          And IM documentation at http://atari.gfabasic.net/stg/img_mod.htm#Developer's_Corner                     */
      Mfree( basepage->p_env ) ; /* free the env ram, not needed                                         */
/*  ~ssystem(22,bp%,-1)           !flush caches for just incase*/
      Mshrink( 0, basepage, 256L + basepage->p_tlen + basepage->p_dlen + basepage->p_blen ) ; /* shrink to basepage values */
      ret = 0 ;
    }
    else ret = EIMG_MODULERROR ;
  }
#ifdef MST_ZVSLB
  else if ( type == MST_ZVSLB )
  {
    char  name[260] ;
    char  path[260] ;
    char* last_slash ;
    ...
    if ( ret >= 0 ) img_module->SlbVersion = ret ;
    else            ret = EIMG_MODULERROR ;
  }
#endif
  ...
  else ret = iizview_init( type, img_module ) ;
...
static void DMMemoryManager(IMG_MODULE* NotThisOne, size_t required_memory)
{
  if ( (NbImgModules > 0) && DMNeedUnload(required_memory) )
  {
    ...
    if ( n_mod_loaded > 0 )
    {
      ...
      LoggingDo(LL_INFO, "%d modules unloaded, %lu bytes freed (%lu by module unload)", n, bytes_freed, bytes_freed-n*sizeof(ZVIEW_SPECIFIC) ) ;
    }
  }
}
...
static void DMMMFreeModules(IMG_MODULE* NotThisOne)
{
  ...
  /* Depending on how memory was allocated/allocator algo, memory freed may not show here */
  if ( bytes_freed > n*sizeof(ZVIEW_SPECIFIC) ) LoggingDo(LL_INFO, "%d modules unloaded, %lu bytes freed (%lu by module unload)", n, bytes_freed, bytes_freed-n*sizeof(ZVIEW_SPECIFIC) ) ;
...
static int LoadModuleIfRequired(IMG_MODULE* ImgModule)
{
  ...
  if ( ImgModule->IsDynamic && (ImgModule->hLib == NULL) && ImgModule->LibFilename )
  {
    ...
    {
/* Can't happen LDI modules are static as of now
      ...
      case MST_ZVSLB:
#endif
                      ret = DImgLoadZView( ImgModule->LibFilename, ImgModule, ImgModule->Type ) ;
                      break ;
...
static void add_internalmod(IMG_MODULE** pimg_module, II_INTERNAL* iii)
{
  iiinternal_init( *pimg_module, iii ) ;
  SetStaticModule( *pimg_module ) ;
  (*pimg_module)++ ;
}
...
static void LoadStaticModules(void)
{
  ...
  if ( img_module )
  {
    ...
    add_internalmod( &img_module, &iii ) ;
#endif
#ifdef STATIC_PNG
...
/* Can't happen LDI modules are static as of now
    ...
    case MST_ZVSLB:
#endif
                    ret = DImgLoadZView( filename, img_module, type ) ;
                    break ;
    default:        ret = -1 ; /* Should not happen */
                    break ;
  }
  ...
  else LoggingDo(LL_ERROR, "Failed to load module %s with %d", filename, ret) ;
...
static long LoadImgDataBase(char *mod_name, long signature, HINI h_ini, IMG_MODULE* img_module)
{
  ...
  memzero( img_module, sizeof(IMG_MODULE) ) ;
  img_module->Signature = signature ;
...
static char* GetModuleType(char type)
{
  switch( type )
  {
    ...
    case MST_IMGMOD: return "ImgView IM" ;
#ifdef MST_ZVSLB
...
static short OnNewModule(FR_CDATA* fr_cdata)
{
  ...
  if ( signature == fsignature )
  {
    ...
    if ( img_module->LibFilename == NULL )
    {
      LoggingDo(LL_ERROR, "Can't allocate LibFilename") ;
      return 0 ;
    }
  }
...
static void LoadDynamicModules(int type, char* path, char* msg_for_prog)
{
  ...
  switch( type )
  {
    ...
    case MST_ZVLDG:   strcpy( ext_mask, "*.LDG" ) ;
                      break ;
    case MST_IMGMOD : strcpy( ext_mask, "*.IM" ) ;
                      break ;
#ifdef MST_ZVSLB
  ...
  mod_scan.type     = type ;
  LoggingDo(LL_IMG, "%ld modules of type %s found", mod_scan.nb_files, GetModuleType(type)) ;
  if ( mod_scan.nb_files > 0 )
  {
    ...
    {
      LoggingDo(LL_WARNING, "Fail to allocate img module array") ;
      return ;
    }
...
static void LoadModulesInformations(char *paths[NB_MST_MAX], char *msg_for_prog, char* priorities)
{
  ...
  /* paths[MST_IMGMOD-MST_LDI]: path where zView IM modules are located          */
  ...
  for ( type = MST_LDI; type < MST_LDI+NB_MST_MAX; type++ )
  {
    ...
    if ( path ) LoggingDo(LL_INFO, "Loading dynamic modules %s from path %s...", GetModuleType(type), path) ;
    else        LoggingDo(LL_INFO, "No path for dynamic modules %s", GetModuleType(type)) ;
...
#ifndef __NO_LOGGING
{
  ...
  LoggingDo(LL_INFO,"%d image modules available", NbImgModules) ;
...
static short ModuleCanHandleExtension(IMG_MODULE* img_module, char* ext)
{
  ...
  {
     /* If known_ext starts with '*', it means there is no known extension or they are so many it's too long to list them all (e.g. recoil) */
    if ( (*know_ext == '*') || (strcmpi( know_ext, ext ) == 0) ) can_handle = 1 ;
...
short DImgGetModulesCandidates(char* ext, IID lformat, void** mods, short nmodules_max)
{
  ...
  while ( ImgGlobalPriority[i] )
  {
    ...
    /* Move modules with '*' known extension at the end of the list for this type */
    /* To let the specific modules with extension try before */
    for ( j = start_type_nmods; j < nmods; j++ )
    {
      if ( modules[j]->Capabilities.known_extensions[0] == '*' )
      {
        short k = nmods-1 ;

        while ( (k > j) && (modules[k]->Capabilities.known_extensions[0] == '*') ) k-- ;
        /* Swap this one with the last one of the list which has specific extension if possible */
        if ( k != j )
        {
          IMG_MODULE* swap_img_module = modules[j] ;

          LoggingDo(LL_INFO, "DImgGetModulesCandidates: swapping module %s and %s", modules[j]->Capabilities.short_name, modules[k]->Capabilities.short_name) ;
          modules[j] = modules[k] ;
          modules[k] = swap_img_module ;
        }
      }
    }
    i++ ;
...
void DImgSetPriorities(char* priorities)
{
  if ( priorities )
  {
    memzero( ImgGlobalPriority, sizeof(ImgGlobalPriority) ) ;
