...
static void filter_init(CONVOLUTION_FILTRE* filter)
{
  int   i ;
  char* pt ;
  ...
  if ( norme <= 0 ) shift = 0 ;
  else              shift = (int) (0.5+log((float)norme)/log(2.0)) ;
}

static void graylevel_init(INFO_IMAGE* info, unsigned char* tgray2index, unsigned char* tindex2gray)
{
  int*           pal ;
  int            i, j, min, ind, val ;
...
static void convolution_cpudo16(int w, int no_line, int* in_data, unsigned int* start_outline, long larg)
{
  int           j, m, n, idx, jdx ;
...
static void convolution_cpudo32(int w, int no_line, unsigned long* in_data, unsigned long* start_outline, long larg)
{
  RGB            srgb ;
  unsigned long  rgb ;
  int            j, m, n, idx, jdx ;
...
static void convolution_cpudogray(int w, int no_line, unsigned char* in_data, unsigned char* start_outline)
{
  int            lline ;
  int            j, m, n, jdx ;
...
static void graylevel2index(unsigned char* buf, int nb, unsigned char* tgray2index)
{
  int            i ;
  unsigned char* pt ;
...
static void index2graylevel(unsigned char* buf, int nb, unsigned char* tindex2gray)
{
  int            i ;
  unsigned char* pt ;
...
static void dspdata_init(MFDB* in)
{
...
static int convolution_dsp(MFDB* in, INFO_IMAGE* info, MFDB* out, CONVOLUTION_FILTRE* filter, CONVOLUTION_DSP_CODE* cdc, unsigned char* tgray2index, unsigned char* tindex2gray, GEM_WINDOW* wprog)
{
  long           nbpts ;
  long           larg ;
  unsigned int*  start_outline ;
