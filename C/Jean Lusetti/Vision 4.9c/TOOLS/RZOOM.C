...
void sraster_zoom(RASTER_ZOOM* rz) ; /* Dans RZOOMS.S */
...
void LogRasterZoom(RASTER_ZOOM* rz)
{
  LogMFDB( rz->in ) ;
  LogMFDB( rz->out ) ;
  LoggingDo(LL_DEBUG, "src_addr_line     = %p", rz->src_addr_line) ;
  LoggingDo(LL_DEBUG, "xoffsets          = %p", rz->xoffsets) ;
  LoggingDo(LL_DEBUG, "yoffsets          = %p", rz->yoffsets) ;
  LoggingDo(LL_DEBUG, "flags             = %d", rz->flags) ;
  LoggingDo(LL_DEBUG, "indexes_in        = %p", rz->indexes_in) ;
  LoggingDo(LL_DEBUG, "indexes_out       = %p", rz->indexes_out) ;
  LoggingDo(LL_DEBUG, "lout              = %ld", rz->lout) ;
  LoggingDo(LL_DEBUG, "increment         = %ld", rz->increment) ;
  LoggingDo(LL_DEBUG, "srcw16            = %ld", rz->srcw16) ;
  LoggingDo(LL_DEBUG, "dstw16            = %ld", rz->dstw16) ;
  LoggingDo(LL_DEBUG, "nplanes           = %d", rz->nplanes) ;
  LoggingDo(LL_DEBUG, "wprog             = %p", rz->wprog) ;
  LoggingDo(LL_DEBUG, "progress_function = %p", rz->progress_function) ;
  LoggingDo(LL_DEBUG, "xoffset_jump      = %d", rz->xoffset_jump) ;
  LoggingDo(LL_DEBUG, "xoffset_scale     = %d", rz->xoffset_scale) ;
}
...
int RasterZoom(MFDB *in, MFDB *out, GEM_WINDOW* wprog)
{
  ...
  int         nmax = 9008 ; /* Our routine can't go further (tested OK for this value, not sure about the limit) */
  short       i ;
  char        local_alloc = 0 ;
  ...
  if ( out->fd_addr == NULL )
  {
    if ( out->fd_w == 0 ) out->fd_w = 1 ;
    if ( out->fd_h == 0 ) out->fd_h = 1 ;
  ...
  if ( in->fd_nplanes <= 8 )
  {
    ...
    memzero( rz.indexes_in, rz.srcw16 + rz.dstw16 ) ;
  }
  rz.lout    = img_size( out->fd_w, 1, out->fd_nplanes ) ;
  rz.wprog   = wprog ;
  rz.nplanes = in->fd_nplanes ;
  if ( wprog ) rz.progress_function = GWProgRange ;
  else         rz.progress_function = NULL ;
...
/*  LogRasterZoom( &rz ) ;*/
  if ( rz.flags & 0x01 ) /* use Std VDI */
  {
    ...
    svr_trnfm( in, &std_src ) ;
    StdVDI2ClassicAtari( &std_src, in ) ;
    sraster_zoom( &rz ) ;
    ClassicAtari2StdVDI( out, &std_dst ) ;
    svr_trnfm( &std_dst, out ) ;
    svr_trnfm( &std_src, in ) ;
    Xfree( std_src.fd_addr ) ;
