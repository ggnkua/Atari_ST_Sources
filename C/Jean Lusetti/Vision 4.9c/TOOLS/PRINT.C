...
#include  "logging.h"
#include  "stdprog.h"
#include   "pdialog.h" /* Make sure to sync this file with application RSC.H file ((e.g. VISION.H) */
...
{
  OBJECT* popup_page ;
  OBJECT* popup_printer ;
  OBJECT* popup_type ;
}
DLGPRINTER_WEXTENSION ;

PAGE  A3          = {CM, "    A3    ", 29.70, 42.00 } ;
PAGE  A4          = {CM, "    A4    ", 21.00, 29.70 } ;
PAGE  USletter    = {CM, " USletter ", 23.02, 27.49 } ;
PAGE  USlegal     = {CM, " USlegal  ", 20.32, 35.56 } ;
PAGE  USexecutive = {CM, " USexec.  ", 18.40, 25.40 } ;
PAGE  Special     = {CM, " Special  ", 21.00, 29.70 } ;
...
static int open_work(int prt_num)
{
  int i ;
  int prt_handle ;
...
static void scan_drivers(void)
{
  if ( nb_drivers == 0 )
  {
    ...
    mouse_busy() ;
    wprog = DisplayStdProg( ImgMsgGeTxt(IMGMID_LOOKSPDDRV) , "", "", CLOSER ) ;

    for ( devnum = 21 ; devnum < 31 ; devnum++ ) /* on va tester tous les No Gdos pour l'impression */
    {
      prt_handle = open_work( devnum ) ; /* Open printer workstation          */
      if ( prt_handle )                  /* There is a driver for that devnum */
      {                                
         vqt_devinfo( prt_handle, devnum, &devexist, devstring ) ;
         id[nb_drivers] = devnum ;
         strcpy( driver_name, (char*)&_VDIParBlk.ptsout[1] ) ;
         new_driver = vq_driver_info( prt_handle, &libr, &driv, &xplane, &attrib, devstring ) ;
         if ( new_driver ) strncpy( &printer_names[nb_drivers][0], devstring, 19 ) ;
         else              strncpy( &printer_names[nb_drivers][0], driver_name, 19 ) ;
         StdProgWText( wprog, 1, &printer_names[nb_drivers][0] ) ;
         nb_drivers++ ;
         v_clswk( prt_handle ) ; /* Close printer workstation */
      }
    }
    (void)GWProgRange( wprog, 10*(devnum-20), 100, NULL ) ;
    GWDestroyWindow( wprog ) ;
    mouse_restore() ;
  }
}

static int page_compare(PAGE* p1, PAGE* p2)
{
  ...
  return 1 ; /* Pagers are different */
}

static void setup_pagename(PAGE* page)
{
  PAGE* page_match = tab_page[SPECIAL_INDEX] ;
  short i ;

  for ( i = 0; i < ARRAY_SIZE(tab_page); i++ )
    if ( page_compare( page, tab_page[i] ) == 0 ) { page_match = tab_page[i] ; break ; }
  strcpy( page->nom, page_match->nom ) ;
}

static int tab_page_index(PAGE *page)
{
  int   index_match = SPECIAL_INDEX ;
  short i ;

  for ( i = 0; i < ARRAY_SIZE(tab_page); i++ )
    if ( page_compare( page, tab_page[i] ) == 0 ) { index_match = i ; break ; }

  return index_match ;
}

static int micron2dpi(int number)
{
  float x ;
  int   result ;

  x      = 10000.0/(float)number ; /* Nb pixels/cm */
  result = (int) (x*INCH2CM) ;

  return result ;
}

static void button_center(GEM_WINDOW* wnd, OBJECT* adr_print, int axis, int click)
{
  int xc, yc ;
  ...
  if ( axis == 0  )
  {
    ...
    {
      adr_print[PRINT_IMAGE].ob_x = (adr_print[PRINT_PAGE].ob_width-(adr_print[PRINT_IMAGE].ob_width)>>1) ;
      objc_offset( adr_print, PRINT_PAGE, &xc, &yc ) ;
      GWObjcDraw( wnd, adr_print, PRINT_PAGE ) ;
    }
  }
  else
  {
    ...
    {
      adr_print[PRINT_IMAGE].ob_y = (adr_print[PRINT_PAGE].ob_height-(adr_print[PRINT_IMAGE].ob_height)>>1) ;
      objc_offset( adr_print, PRINT_PAGE, &xc, &yc ) ;
      GWObjcDraw( wnd, adr_print, PRINT_PAGE ) ;
    }
  }
...
static void compute_page(DIALOG* dlg)
{
  ...
  adr_print[PRINT_PAGE].ob_x      = (l-dlg_user_data->spx) >> 1 ;
  adr_print[PRINT_PAGE].ob_y      = (l-dlg_user_data->spy) >> 1 ;
...
static void show_dimpage(GEM_WINDOW* wnd, PAGE* page, OBJECT* adr_print)
{
  char buf[20] ;

  sprintf( buf, "%.1f", page->largeur ) ;
  write_text( adr_print, PRINT_SIZEPAGEW, buf ) ;
  GWObjcDraw( wnd, adr_print, PRINT_SIZEPAGEW ) ;
  sprintf( buf, "%.1f", page->hauteur ) ;
  write_text( adr_print, PRINT_SIZEPAGEH, buf ) ;
  GWObjcDraw( wnd, adr_print, PRINT_SIZEPAGEH ) ;
}

static float fsize(short w, short lpi)
{
  return (float)w*(float)lpi/10000.0 ; /* cm */
}

static void OnInitPrinterDialog(void* w)
{
  ...
  wext->popup_page = popup_make( ARRAY_SIZE(tab_page), 11 ) ;
  for ( i = 0; i < ARRAY_SIZE(tab_page); i++ )
  ...
  strcpy( wext->popup_type[1].ob_spec.free_string, ImgMsgGeTxt(IMGMID_PRINTCOLOR) ) ;
  strcpy( wext->popup_type[2].ob_spec.free_string, ImgMsgGeTxt(IMGMID_PRINTBW) ) ;

  dlg_user_data->sizex = fsize( dlg_user_data->info.mfdb.fd_w, dlg_user_data->info.lpix ) ;
  sprintf( buffer, "%.1f", dlg_user_data->sizex ) ;
  write_text( adr_print, PRINT_SIZEX, buffer ) ;
  dlg_user_data->sizey = fsize( dlg_user_data->info.mfdb.fd_h, dlg_user_data->info.hpix ) ;
  ...
  if ( dlg_user_data->info.nplanes == 1 )
  {
...
static void bcenter_ifsel(GEM_WINDOW* wnd, int obj)
{
  DIALOG* dlg = (DIALOG*) wnd->DlgData ;

  if ( selected(dlg->BaseObject, obj) ) button_center( wnd, dlg->BaseObject, (obj == PRINT_CENTREY), 0 ) ;
}

static float fpc(float x, int pc)
{
  return x*(float)pc/100.0 ;
}

static void update_gui(GEM_WINDOW* wnd, int pc, int obj)
{
  DIALOG*               dlg = (DIALOG*) wnd->DlgData ;
  OBJECT*               adr_print = dlg->BaseObject ;
  DLGPRINTER_USER_DATA* dlg_user_data = dlg->UserData ;
  int                   i, rx, ry ;
  char                  buffer[100] ;
  char                  redraw_objs[6+1] = { PRINT_RESOL, PRINT_XPC, PRINT_YPC, PRINT_SIZEX, PRINT_SIZEY, PRINT_PAGE } ;

  rx = (int) fpc( (float)dlg_user_data->info.lpix, pc ) ;
  ry = (int) fpc( (float)dlg_user_data->info.hpix, pc ) ;
  sprintf( buffer, "%.4d/%.4d", micron2dpi(rx), micron2dpi(ry) ) ;
  write_text( adr_print, PRINT_RESOL, buffer ) ;
  deselect( adr_print, obj ) ;
  redraw_objs[ARRAY_SIZE(redraw_objs)-1] = (char) obj ;
  for ( i = 0; i < ARRAY_SIZE(redraw_objs); i++ ) GWObjcDraw( wnd, adr_print, redraw_objs[i] ) ;
  bcenter_ifsel( wnd, PRINT_CENTREX ) ;
  bcenter_ifsel( wnd, PRINT_CENTREY ) ;
...
static int OnObjectNotifyPrinterDialog(void* wi, int obj) /* >=0 : out of window, code IDOK or IDCANCEL */
{
  ...
  switch( obj )
  {
    ...
    case PRINT_XPCPLUS    : read_text( adr_print, PRINT_XPC, buffer ) ;
                            ...
                            number                          = fpc( dlg_user_data->sizex, pc ) ;
      ...
      case PRINT_TR90       : inv_select( adr_print, PRINT_R90 ) ;
                              ...
                              GWObjcDraw( wnd, adr_print, PRINT_SIZEX ) ;
                              GWObjcDraw( wnd, adr_print, PRINT_SIZEY ) ;
                              read_text( adr_print, PRINT_XPC, buf ) ;
                              read_text( adr_print, PRINT_YPC, buffer ) ;
                              write_text( adr_print, PRINT_XPC, buffer ) ;
                              write_text( adr_print, PRINT_YPC, buf ) ;
                              GWObjcDraw( wnd, adr_print, PRINT_XPC ) ;
                              GWObjcDraw( wnd, adr_print, PRINT_YPC ) ;
                              bcenter_ifsel( wnd, PRINT_CENTREX ) ;
                              bcenter_ifsel( wnd, PRINT_CENTREY ) ;
/*                              if ( selected(adr_print, PRINT_CENTREX) ) button_center( wnd, adr_print, 0, 0 ) ;
                              if ( selected(adr_print, PRINT_CENTREY) ) button_center( wnd, adr_print, 1, 0 ) ;*/
      ...
      case PRINT_IMAGE      : w = adr_print[PRINT_IMAGE].ob_width ;
                              ...
                              GWObjcDraw( wnd, adr_print, PRINT_PAGE ) ;
                              inv_select_ifsel( adr_print, PRINT_CENTREX ) ;
                              inv_select_ifsel( adr_print, PRINT_CENTREY ) ;
                              break ;

      case PRINT_CONFIGPAGE : dlg_user_data->page_index = tab_page_index(&dlg_user_data->page) ;
                              ...
                              if ( clicked > 0 )
                              {
                                ...
                                compute_page( dlg ) ;
                                GWObjcDraw( wnd, adr_print, PRINT_GROUP ) ;
                              }
      ...
      case PRINT_PRINTER    : deselect( adr_print, obj ) ;
      ...
      case PRINT_FILE       : strcpy( buffer, dlg_user_data->filename ) ;
                              ...
                              if ( file_name(buffer, "", buffer) == 1 )
                              {
                                ...
                                if ( img_format(dlg_user_data->filename, &inf) == 0 )
                                {
                                  ...
                                  deselect( adr_print, PRINT_CENTREX ) ;
                                  deselect( adr_print, PRINT_CENTREY ) ;
                                  ZeroMFDB( &dlg_user_data->info.mfdb ) ;
                                  dlg_user_data->info.mfdb.fd_w       = dlg_user_data->info.width ;
                                  dlg_user_data->info.mfdb.fd_h       = dlg_user_data->info.height ;
                                  dlg_user_data->info.mfdb.fd_nplanes = dlg_user_data->info.nplanes ;
                                  dlg_user_data->info.mfdb.fd_wdwidth = W2WDWIDTH( dlg_user_data->info.mfdb.fd_w ) ;
                                  dlg_user_data->sizex = fsize( dlg_user_data->info.mfdb.fd_w, dlg_user_data->info.lpix ) ;
                                  sprintf( buffer, "%.1f", dlg_user_data->sizex ) ;
                                  write_text( adr_print, PRINT_SIZEX, buffer ) ;
                                  dlg_user_data->sizey = fsize( dlg_user_data->info.mfdb.fd_h, dlg_user_data->info.hpix ) ;
...
static int OnClosePrinterDialog(void* w)
{
  GEM_WINDOW*            wnd = (GEM_WINDOW*) w ;
  DLGPRINTER_WEXTENSION* wext = wnd->Extension ;
  ...
  popup_kill( wext->popup_page, ARRAY_SIZE(tab_page) ) ;

  return( GWCloseDlg( wnd ) ) ;
}

static int ttr(float x, float ptpx)
{
  return (int) (10000.0*x/ptpx) ;
}

int print_image(INFO_IMAGE* in_info, char* app_title)
{
  ...
  memzero( &dlg_user_data, sizeof(DLGPRINTER_USER_DATA) ) ;
  if ( in_info ) memcpy( &dlg_user_data.info, in_info, sizeof(INFO_IMAGE) ) ;

  while ( dlg_user_data.info.mfdb.fd_w == 0 )
  {
    ...
    if ( file_name( buffer, "", buffer ) == 1 )
    {
      ...
      strcpy( dlg_user_data.filename, buffer ) ;
      inf2.lformat = IID_UNKNOWN ;
      if ( img_format(dlg_user_data.filename, &inf2) == 0 )
      {
        memcpy( &dlg_user_data.info, &inf2, sizeof(INFO_IMAGE) ) ;
        ZeroMFDB( &dlg_user_data.info.mfdb ) ;
        dlg_user_data.info.mfdb.fd_w       = inf2.width ;
        dlg_user_data.info.mfdb.fd_h       = inf2.height ;
        dlg_user_data.info.mfdb.fd_nplanes = inf2.nplanes ;
        dlg_user_data.info.mfdb.fd_wdwidth = W2WDWIDTH( dlg_user_data.info.mfdb.fd_w ) ;
        free_info( &dlg_user_data.info ) ;
      }
    }
    else return PCANCEL ;
  }
  ...
  dlg_data.ExtensionSize  = 0 ; /* OnInitPrinterDialog will allocate it */
  ...
  if ( GWCreateAndDoModal( &dlg_data, PRINT_XPC ) == IDCANCEL ) return PCANCEL ;
  ...
  LoggingDo(LL_INFO, "Printing process starts...") ;
  ...
  if ( dlg_user_data.filename[0] )
  {
    ...
    if ( img_format( dlg_user_data.filename, &inf ) == 0 )
    {
      ...
      wprog = DisplayStdProg( ImgMsgGeTxt(IMGMID_LOADINGIMG) , "", dlg_user_data.filename, 0 ) ;
      if ( load_picture( &inf ) != EIMG_SUCCESS )
      {
        if ( bfree_info ) free_info( &dlg_user_data.info ) ;
        GWDestroyWindow( wprog ) ;
        return PCANCEL ;
      }
  ...
  else CopyMFDB( src, &dlg_user_data.info.mfdb ) ;
  ...
  if ( dither_mono ) /* Mode noir et blanc */
  {
    ...
    LoggingDo(LL_INFO, "Printing: dithering to B/W...") ;
    StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_DITHER) ) ;
    virtuel.fd_nplanes = 1 ;
    if ( dlg_user_data.rotate90 ) pc_x = (int) (dlg_user_data.xpc*(float)dlg_user_data.info.lpix/ptpy) ;
    else                          pc_x = (int) (dlg_user_data.xpc*(float)dlg_user_data.info.lpix/ptpx) ;
    if ( dlg_user_data.rotate90 ) pc_y = (int) (dlg_user_data.ypc*(float)dlg_user_data.info.hpix/ptpx) ;
    else                          pc_y = (int) (dlg_user_data.ypc*(float)dlg_user_data.info.hpix/ptpy) ;
    mouse_busy() ;
    if ( dither_ratio( src, &dlg_user_data.info, &virtuel, pc_x, pc_y, wprog ) != 0 )
    {
      if ( bfree_info ) free_info( &dlg_user_data.info ) ;
      mouse_restore() ;
      GWDestroyWindow( wprog ) ;
    ...
    (void)GWProgRange( wprog, 20, 100, NULL ) ;
    mouse_restore() ;
    if ( dlg_user_data.filename[0] ) Xfree( src->fd_addr ) ;
    CopyMFDB( src, &virtuel ) ;
    bfree_src = 1 ;
  }
  ...
  if ( dlg_user_data.rotate90 )
  {
    ...
    StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_ROTATING)  ) ;
    mouse_busy() ;
    LoggingDo(LL_INFO, "Printing: Rotating by 90...") ;
    ZeroMFDB( &virtuel ) ;
  ...
  if ( !print_color )
  {
    StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_DOGDOSIMG)  ) ;
    strcpy( nom, "VSCRAP.IMG" ) ;
    dlg_user_data.info.lformat = IID_IMG ;
    if ( save_picture( nom, src, &dlg_user_data.info, wprog ) )
    {
      if ( bfree_info ) free_info( &dlg_user_data.info ) ;
      GWDestroyWindow( wprog ) ;
      v_clswk( print_handle ) ;
  ...
  else
  {
    ...
    CopyMFDB( &new_info.mfdb, src ) ;
    if ( new_info.mfdb.fd_nplanes <= 8 )
    {
      new_info.nb_cpal         = 0 ;
      new_info.palette         = NULL ;
      new_info.nplanes         = 16 ;
      new_info.mfdb.fd_addr    = NULL ;
      new_info.mfdb.fd_nplanes = 16 ;
      StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_DOGDOSIMG)  ) ;
      if ( index2truecolor( src, &dlg_user_data.info, &new_info.mfdb, wprog ) != 0 )
      {
        if ( bfree_info ) free_info( &dlg_user_data.info ) ;
        v_clswk( print_handle ) ;
        GWDestroyWindow( wprog ) ;
    ...
    new_info.lformat = IID_TARGA ;
    if ( save_picture( nom, &new_info.mfdb, &new_info, wprog ) )
    {
      if ( bfree_info ) free_info( &dlg_user_data.info ) ;
      v_clswk( print_handle ) ;
      GWDestroyWindow( wprog ) ;
    ...
    if ( src->fd_nplanes <= 8 ) Xfree( new_info.mfdb.fd_addr ) ;
  }

  LoggingDo(LL_INFO, "Printing: temporary file %s generated", nom) ;

  if ( bfree_src ) Xfree(src->fd_addr) ;
  if ( bfree_info ) free_info( &dlg_user_data.info ) ;

  StdProgWText( wprog, 0, ImgMsgGeTxt(IMGMID_PRINTING)  ) ;
  vst_unload_fonts( handle, 0 ) ;
  xoffset = ttr( dlg_user_data.posx, ptpx ) ;
  yoffset = ttr( dlg_user_data.posy, ptpy ) ;
  xyarray[0] = xoffset ;
  xyarray[1] = yoffset ;
  xyarray[2] = xoffset + ttr( sizex, ptpx ) ;
  xyarray[3] = yoffset + ttr( sizey, ptpy ) ;

  mouse_busy() ;
  if ( app_title ) vs_page_info( print_handle, APPL, app_title ) ;
  else             vs_page_info( print_handle, APPL, "" ) ;
  vs_page_info( print_handle, DOC, nom ) ;
  vs_page_info( print_handle, CREAT,	"" ) ;
  vs_page_info( print_handle, REM,	"Thanks to Thierry Rodolfo!" ) ;

  if ( dither_mono ) v_bit_image( print_handle, nom, 0, INTEGER, INTEGER, 0, 0, xyarray ) ;
  else               v_bit_image( print_handle, nom, 0, FRACTIONAL, FRACTIONAL, 0, 0, xyarray ) ;
  ...
  unlink( nom ) ;
  GWDestroyWindow( wprog ) ;
  LoggingDo(LL_INFO, "Printing: done") ;
