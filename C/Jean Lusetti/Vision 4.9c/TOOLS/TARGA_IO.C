...
{
  TARGA_HEADER* header ;
  char          buffer[18] ; /* We need first 18 bytes for identifying */
  int           ret ;

  ret = ImgFOpen( inf, nom, NULL ) ;
  if ( ret != EIMG_SUCCESS ) return ret ;

  inf->specific = Xalloc( sizeof(TARGA_HEADER) ) ;
  header        = (TARGA_HEADER*) inf->specific ;
  if ( header == NULL )
  {
    free_info( inf ) ;
  ...
  memcpy( &header->cmap, &buffer[3], 5 ) ;
  memcpy( &header->image, &buffer[3+5], 10 ) ;

  header_reverse( header ) ;

  inf->compress = header->image_type ;
  /* Supported types:                  */
  ...
  inf->width   = header->image.width ;
  inf->height  = header->image.height ;
  inf->nplanes = header->image.depth ;
  ...
  inf->palette = NULL ;
  inf->nb_cpal = 0 ;
  if ( (header->image.descriptor & 0x20) == 0 ) inf->c.flipflop = FLIPFLOP_Y ;
...
static void targa_pos_on_image(INFO_IMAGE* inf)
{
  TARGA_HEADER* header   = (TARGA_HEADER*) inf->specific ;
  int           lut_size = header->cmap.depth/3 + header->cmap.length ;
  long          offset   = 18 + header->length_of_identification + (lut_size >> 3) ;
...
static int targa_read_raw(INFO_IMAGE* inf)
{
  ...
  TCTarga = getTCTarga( header->image.depth, out->fd_nplanes ) ;
  if ( TCTarga == NULL ) return EIMG_OPTIONNOTSUPPORTED ;

  i2r_init( out, header->image.width, 1 ) ;
  i2r_nb   = (long)header->image.width * (long)(header->image.depth >> 3) ;
  i2r_data = Xalloc( i2r_nb ) ;
  if ( i2r_data == NULL ) return EIMG_NOMEMORY ;

  targa_pos_on_image( inf ) ;

  for ( l = 0; l < header->image.height; l++ )
  {
    perf_start( PERF_DISK, &inf->perf_info ) ;
    size = (long)header->image.width * (long)(header->image.depth >> 3) ;
    ...
    if ( PROG_CANCEL_CODE( GWProgRange( wprog, l, header->image.height, NULL ) ) )
    {
...
static int targa_read_rle(INFO_IMAGE* inf)
{
  ...
  int           done = 0 ;
  int           ret = EIMG_SUCCESS ;
  int           pattern_size = header->image.depth >> 3 ;
  ...
  last_npix          = npix = 0 ;
  npixmax            = (long)header->image.width * (long)header->image.height ;
...
int targa_load(INFO_IMAGE* inf)
{
  ...
  if ( !inf->xfile.valid )
  {
    LoggingDo(LL_ERROR, "targa_load: invalid file descriptor") ;
    return EIMG_FILENOEXIST ;
  }

  out->fd_w = ALIGN16( header->image.width ) ;
  out->fd_h = (int) header->image.height ;
  if ( header->image.depth <= 8 ) out->fd_nplanes = header->image.depth ;
  else
  {
    ...
    else
    {
      ...
      else                              out->fd_nplanes = header->image.depth ;
    }
  }
...
int targa_sauve(char* name, MFDB* img, INFO_IMAGE* info, GEM_WINDOW* wprog)
{
  ...
  if ( img->fd_nplanes >= 24 ) nb_bytes_per_sample = 3 ;
  else                         nb_bytes_per_sample = 2 ;
  b = Xcalloc( img->fd_w, nb_bytes_per_sample ) ;
  if ( b == NULL )
  {
    fclose( stream ) ;
    return EIMG_NOMEMORY ;
