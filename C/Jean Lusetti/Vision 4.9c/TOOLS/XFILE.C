...
long XFinstatS(short fd) ; /* In XFILES.S*/
...
void* CreateFileR(char* name)
{
  ...
  stream = fopen( name, "w+b" ) ; /* w+b is important here as we want to make sure that file can be also read as well, if wb only then reads will fail */
  if ( stream == NULL )
  {
...
long XFseek(XFILE* xfile, long offset, short seek_mode)
{
  ...
  {
    if ( xfile->file_size == (size_t)(-1) ) xfile->file_size = Fseek( 0L, xfile->gdos_fd, SEEK_END ) ;
