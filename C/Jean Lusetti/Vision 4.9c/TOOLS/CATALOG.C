...
{
  long  lformats[] = { IID_IMG, IID_DEGAS, IID_ART, IID_NEO, IID_MAC, IID_GIF, IID_TIFF, IID_TARGA, IID_JPEG, IID_BMP, IID_PNG, 0 } ;
  int   formats[] =  {   F_IMG,   F_DEGAS,   F_ART,   F_NEO,   F_MAC,   F_GIF,   F_TIFF,   F_TARGA,   F_JPEG,   F_BMP,   F_PNG, F_UNKNOWN } ;
  long* plf = &lformats[0] ;
  int*  pf = &formats[0] ;

  while ( *plf && (*plf != lformat) ) { plf++ ; pf++ ; }
...
void catalog_update(HCAT hCat, char* text)
{
  ...
  for ( i = 0; i < cat->catalog_header.nb_records+cat->catalog_header.nb_deleted; i++ )
  {
    if ( rec->name[0] != MARKED_DELETED )
    {
      ...
      if ( !exist )
      {
        LoggingDo(LL_INFO, "catalog_update: removing %s", filename) ;
      ...
      else
      {
        ...
        {
          LoggingDo(LL_INFO, "catalog_update: timestamp changed, updating %s", filename) ;
          c1 = rec->category_1 ;
          c2 = rec->category_2 ;
          c3 = rec->category_3 ;
          catalog_remove( hCat, i ) ;
  ...
  GWDestroyWindow( wprog ) ;
  LoggingDo(LL_INFO, "catalog_update: %ld image removed and %ld images updated", nb_removed, nb_updated) ;
}
