...
unsigned short RRGB8ToRGB5[256] ;
unsigned short GRGB8ToRGB5[256] ;
unsigned short BRGB8ToRGB5[256] ;
...
static float invpc(int pc)
{
  return 100.0/(float)pc ;
}
...
static short rpc100(short pc, short val)
{
  return (short) (0.5+(float)pc*(float)val/100.0) ;
}
...
int raster_pczoom(MFDB* in, MFDB* out, int pcx, int pcy, GEM_WINDOW* wprog)
{
  ...
  w = rpc100( pcx, in->fd_w ) ;
  h = rpc100( pcy, in->fd_h ) ;
...
int raster_symvrt(int* xyarray, MFDB* src, MFDB* dest)
{
  int xy[8] ;
  int ls, ld ;

  if ( (src->fd_addr == NULL) || (dest->fd_addr == NULL) ) GWHideCursor() ;
  xy[0] = xyarray[0] ;
  xy[2] = xyarray[2] ;
  xy[4] = xyarray[0] ;
  xy[6] = xyarray[2] ;
  ...
  if ( (src->fd_addr == NULL) || (dest->fd_addr == NULL) ) GWShowCursor() ;
...
int raster_vsym(MFDB* img, int* pxy)
{
  ...
  if ( img->fd_nplanes == 16 )
  {
    ...
    if ( img->fd_addr == NULL ) GWShowCursor() ;
  ...
  rcxy[6] = pxy[2] ;
  img_raz( &temp ) ;
  if ( img->fd_addr == NULL ) GWHideCursor() ;
  for ( l = pxy[1]; l <= pxy[3]; l++ )
  {
    xy[1] = xy[3] = l ;
    svro_cpyfmSO( xy, img, &temp ) ;
...
int pixel_init(MFDB* img)
{
  ...
  pixel_mode  = 0 ;
  pixel_lgoff = (long*) Xcalloc( img->fd_h, sizeof(long) ) ;
  if ( pixel_lgoff == NULL )
  {
    pixel_mode = -1 ;
  ...
  if ( img->fd_nplanes <= 8 ) pixel_adrin = (unsigned char*) Xalloc( nbpts ) ;
  else                        pixel_adr   = img->fd_addr ;
...
#define PLOT_INDEX_COLOR 16
  ...
  if ( Truecolor )
  {
    ...
    {
      case 16 : svs_color( PLOT_INDEX_COLOR, white ) ;
                pset( x, y ) ;
                svro_cpyfmSO( xy, &screen, &mfdb ) ;
...
static unsigned short RGB8To5or6(short val, float ref)
{
  return (unsigned short) (0.5 + (float)val*ref/255.0) ;
}
...
unsigned short RVB16(RGB* rgb)
{
  ...
  if ( RRGB8ToRGB5[128] == 0 )
  {
    ...
    for ( i = 0; i < 256; i++ )
    {
      ...
      RRGB8ToRGB5[i] = v << 11 ;
      BRGB8ToRGB5[i] = v ;
      GRGB8ToRGB5[i] = RGB8To5or6( i, 63.0 ) << 5 ;
    }
  }

  r = RRGB8ToRGB5[rgb->Red] ;
  g = GRGB8ToRGB5[rgb->Green] ;
  b = BRGB8ToRGB5[rgb->Blue] ;

  return( r | g | b ) ;
}

short tc24_32totc16(short tc32, MFDB* in, MFDB* out)
{
  /* TC32 RGB0 --> TC16 Falcon */
  RGB             rgb ;
  unsigned short* rgb16 ;
  unsigned long   n, nb_pixels ;
  unsigned char*  rgb32 ;
...
short tc32totc16(MFDB*in, MFDB* out)
{
  return tc24_32totc16( 1, in, out ) ;
}

short tc24totc16(MFDB*in, MFDB* out)
{
  return tc24_32totc16( 0, in, out ) ;
}
