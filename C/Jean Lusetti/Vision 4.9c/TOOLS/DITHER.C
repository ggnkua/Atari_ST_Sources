...
#include "..\tools\dither16.h"
#include "..\tools\logging.h"
#include "..\tools\rasterop.h"


typedef struct _DITHER_GCONTEXT
{
  ...
  unsigned char tab_grey[256*3] ;
  unsigned char new_ind[256*3] ;
  short         c5bto1000[32] ;
  /* For dither216, not ready as quot_roug is not using a register in DIT216c.S
  unsigned char quot_rouge[5*51+1] ;
  unsigned char quot_vert6[5*51+1] ;
  unsigned char quot_bleu36[5*51+1] ;
  unsigned char restes[5*51+1] ; */
}
DITHER_GCONTEXT, *PDITHER_GCONTEXT ;

/* For DIT216.S */
typedef struct _DDATA216
{
  MFDB*          img ;
  unsigned char* indexes ;
  unsigned char* quot_rouge ;
  unsigned char* quot_vert6 ;
  unsigned char* quot_bleu36 ;
  unsigned char* restes[5*51+1] ;
}
DDATA216, *PDDATA216 ;

/* For DITHER.S */
typedef struct _DDATA2PLANES
{
  void*          pt_rvb ; /* Could be char* or short* */
  short          w ;
  unsigned char* pt ;
  short          b32 ; /* 1 if 32bit, else 24bit */
  void*          grey_or_ind ; /* Could be tab_grey or new_ind */
}
DDATA2PLANES, *PDDATA2PLANES ;

void dither_rvb16to8planes(DDATA2PLANES* ddata) ;
void dither_rvb24to8planes(DDATA2PLANES* ddata) ;

/* iN DIT216.S : TC --> 216 colOrs  */
void dither_216c(MFDB* in, unsigned char* indexes) ;


static DITHER_GCONTEXT* DitherGContext = NULL ;
...
*/
/*
static void d216_init_quotes(DITHER_GCONTEXT* dgc)
{
  short i, i6, i36, j, k, r ;
  ...
  for ( k = 0, i = 0; i <= 4; i++ )
  {
    ...
    for ( j = 0; j < 51; j++, k++ )
    {
      dgc->quot_rouge[k]  = i ;
      dgc->quot_vert6[k]  = i6 ;
      dgc->quot_bleu36[k] = i36 ;
      dgc->restes[k]      = r++ ;
    }
  }
  dgc->quot_rouge[k]  = 5 ;
  dgc->quot_vert6[k]  = 30 ;
  dgc->quot_bleu36[k] = 180 ;
  dgc->restes[k]      = 0 ;
}
*/
static DITHER_GCONTEXT* dither_lifecycle(short terminate)
{
  float          coe ;
  int            i, j, k, k1, k2 ;
  unsigned char  D[8][8] ;
  ...
  if ( terminate )
  {
    if ( DitherGContext ) Xfree( DitherGContext ) ;
    DitherGContext = NULL ;
    return NULL ;
  }

  if ( DitherGContext ) return DitherGContext ;
  DitherGContext = Xcalloc( 1, sizeof(DITHER_GCONTEXT) ) ;
  if ( DitherGContext == NULL ) return NULL ;
  ...
  for ( i = 0; i < 8; i++ )
    for ( j = 0; j < 8; j++ ) DitherGContext->dither_matrix[i][j] = 4*D[i][j] ;

  for ( i = 8; i < 16; i++ )
    for ( j = 0; j < 8; j++ ) DitherGContext->dither_matrix[i][j] = 4*D[i-8][j]+2 ;

  for ( i = 0; i < 8; i++ )
    for ( j = 8; j < 16; j++ ) DitherGContext->dither_matrix[i][j] = 4*D[i][j-8]+3 ;

  for ( i = 8; i < 16; i++ )
    for ( j = 8; j < 16; j++ ) DitherGContext->dither_matrix[i][j] = 4*D[i-8][j-8]+1 ;

  coe = 255.0/3.0/31.0 ;
  for ( i = 0; i <= 3*31; i++ ) DitherGContext->tc16[i] = (unsigned char) (0.5+(float)i*coe) ;
  coe = 1/3.0 ;
  for ( i = 0; i <= 3*255; i++ ) DitherGContext->tc24[i] = (unsigned char) (0.5+(float)i*coe) ;

  vdi2tos = get_ptvdi2tos( 1 ) ;
  DitherGContext->white = vdi2tos[0] ;
  DitherGContext->black = vdi2tos[1] ;

  /* Array to map a 5bit color coding (TC16b, 0...31) to a VDI value (0...1000) */
#define rf (1000.0/31.0)
  for ( i = 0; i < 32; i++ )
    DitherGContext->c5bto1000[i] = (short)(0.5+(float)i*rf) ;
/*
...
/*  d216_init_quotes( DitherGContext ) ; not ready */

  return DitherGContext ;
}
...
static void init_intensity(INFO_IMAGE* inf)
{
  ...
  if ( inf->mfdb.fd_nplanes <= 8 )
  {
    ...
    for ( i = 0; i < nbc; i++ )
    {
      ...
      val  = *red + *green + *blue ;
      fval = (float)val*coe ;
      DitherGContext->pal_intense[i] = (unsigned char) fval ;
      red   += 3 ;
      green += 3 ;
      blue  += 3 ;
    }
...
static int dither_seuil(INFO_IMAGE* inf_in, INFO_IMAGE* inf_out, GEM_WINDOW* wprog)
{
  ...
  if ( in->fd_nplanes <= 8 )
  {
    ...
    for ( y = 0; y < in->fd_h; y++ )
    {
      ...
      j = y & 0x0F ;
      matrix_line = &DitherGContext->dither_matrix[0][0]+(j << 4) ;
      for (x = 0; x < nb_px; x++)
      {
        ind = *pt ;
        intensity = DitherGContext->pal_intense[ind] ;
  ...
  else if ( in->fd_nplanes == 16 )
  {
    ...
    for (y = 0; y < in->fd_h; y++)
    {
      ...
      for (x = 0; x < in->fd_w; x++)
      {
        ...
        intensity = r+v+b ;
        intensity = DitherGContext->tc16[intensity] ;
        if (intensity == 0) *pt++ = local_back ;
        else
        {
          if (intensity < matrix_line[x & 0x0F]) *pt++ = local_back ;
  ...
  else if ( in->fd_nplanes == 24 )
  {
    ...
    for (y = 0; y < in->fd_h; y++)
    {
      ...
      for (x = 0; x < in->fd_w; x++)
      {
        ...
        v = *pt_rvb24++ ;
        b = *pt_rvb24++ ;
        intensity = DitherGContext->tc24[r+v+b] ;
        if (intensity == 0) *pt++ = local_back ;
        else
        {
          if (intensity < matrix_line[x & 0x0F]) *pt++ = local_back ;
...
static int dither_gris(INFO_IMAGE* inf_in, INFO_IMAGE* inf_out, GEM_WINDOW* wprog, int force_allocate)
{
  ...
  if ( in->fd_nplanes <= 8 )
  {
    ...
    for ( x = 0; x < 256; x++ )
    {
      ...
      DitherGContext->tab_grey[x] = ind ;
    }
    for ( x = 0; x < 256; x++ ) DitherGContext->new_ind[x] = DitherGContext->tab_grey[DitherGContext->pal_intense[x]] ;
    ...
    for ( y = 0; y < in->fd_h; y++ )
    {
      ...
      for ( x = 0; x < nb_px; x++, pt++ ) *pt = DitherGContext->new_ind[*pt] ;
  ...
  else if ( in->fd_nplanes == 16 )
  {
    ...
    ddata.pt          = buffer ;
    ddata.b32         = 0 ;
    ddata.grey_or_ind = DitherGContext->tab_grey ;
    if ( out->fd_nplanes == 8 ) memcpy( ddata.grey_or_ind, tg_16bto8b, 32*3 ) ;
    else                        memcpy( ddata.grey_or_ind, tg_16bto4b, 32*3 ) ;
    taille = 16*in->fd_wdwidth ;
    for ( y = 0; y < in->fd_h; y++ )
    {
      ddata.pt_rvb = pt_rvb ;
      dither_rvb16to8planes( &ddata ) ;
      ind2raster() ;
      pt_rvb += taille ;
      (void)GWProgRange( wprog, y, in->fd_h, NULL ) ;
    }
  ...
  {
    unsigned char* ptc_rvb =  (unsigned char*) in->fd_addr ;

    ddata.w           =  in->fd_w ;
    ddata.pt          = buffer ;
    ddata.b32         = 0 ;
    ddata.grey_or_ind = DitherGContext->new_ind ;
    if ( out->fd_nplanes == 8 ) memcpy( ddata.grey_or_ind, ni_24bto8b, 256*3 ) ;
    else                        memcpy( ddata.grey_or_ind, ni_24bto4b, 256*3 ) ;
    taille = 3L*(long)(in->fd_wdwidth << 4) ;
    for ( y = 0; y < in->fd_h; y++ )
    {
      ddata.pt_rvb = ptc_rvb ;
      dither_rvb24to8planes( &ddata ) ;
      ind2raster() ;
      ptc_rvb += taille ;
      (void)GWProgRange( wprog, y, in->fd_h, NULL ) ;
    }
...
static void dither_line(unsigned char* buffer, int nb, unsigned char* intensity, int mline)
{
  ...
  for ( x = 0; x < nb; x++ )
  {
    ...
    if ( intense == 0 ) *pt++ = DitherGContext->black ;
    else
    {
      if ( intense < matrix_line[i] ) *pt++ = DitherGContext->black ;
      else                            *pt++ = DitherGContext->white ;
    }
  }
}
...
static void dither_tc16line(int* buffer, unsigned char* out, int nb, unsigned char* tc16lum, int mline)
{
  ...
  for ( x = 0; x < nb; x++ )
  {
    ...
    i = x & 0x0F ;
    if ( intensity == 0 ) *pt_out++ = DitherGContext->black ;
    else
    {
      if ( intensity < matrix_line[i] ) *pt_out++ = DitherGContext->black ;
      else                              *pt_out++ = DitherGContext->white ;
    }
    pt_rvb++ ;
  }
}
...
int dither_ratio(MFDB* in, INFO_IMAGE* inf, MFDB* out, int pc_x, int pc_y, GEM_WINDOW* wprog)
{
  ...
  if ( dither_lifecycle( 0 ) == NULL ) return -3 ;
  ...
  for ( ligne = 0; ligne < out->fd_h; ligne++ )
  {
    ...
    if ( in->fd_nplanes <= 8 )
    {
      ...
      dither_line( buffer_out, out->fd_w, DitherGContext->pal_intense, ligne ) ;
    }
    else if ( in->fd_nplanes == 16 )
    {
      tc16_strech( pt_raster, ibuffer_out, out->fd_w, strech_table ) ;
      dither_tc16line( ibuffer_out, buffer_out, out->fd_w, DitherGContext->tc16, ligne ) ;
    }
    ind2raster() ;
...
{
  short i ;
  short* c5bto1000 = &DitherGContext->c5bto1000[0] ;
...
static int dither_16(short alg_msize, INFO_IMAGE* inf_in, INFO_IMAGE* inf_out, GEM_WINDOW* wprog)
{
  MFDB out16 ;
  int  ret ;
  char free_out16 = 1 ;
  ...
  out16.fd_addr = NULL ;
  LoggingDo(LL_INFO, "dither_16: nplanes=%d", inf_in->mfdb.fd_nplanes) ;
  switch( inf_in->mfdb.fd_nplanes )
  {
    case  1:
    case  4: return EIMG_DATAINCONSISTENT ;

    case  8: out16.fd_nplanes = 16 ;
             ret = index2truecolor( &inf_in->mfdb, inf_in, &out16, NULL ) ;
             break ;

    case 16: CopyMFDB( &out16, &inf_in->mfdb ) ;
             ret         = 0 ;
             free_out16  = 0 ;
             break ;

    case 24: ret = tc24totc16( &inf_in->mfdb, &out16 ) ;
             break ;

    case 32: ret = tc32totc16( &inf_in->mfdb, &out16 ) ;
             break ;
  }

  /* We now have a 16bit image */
  if ( ret == EIMG_SUCCESS ) ret = _dither16( alg_msize, &out16, inf_out, wprog ) ; /* 0x04 = 0 matrix 4x4, 0x00: no matrix */

  if ( free_out16 && out16.fd_addr ) Xfree( out16.fd_addr ) ;

  LoggingDo(LL_INFO, "dither_16: returns %d", ret) ;
  return ret ;
}
...
int dither(int method, INFO_IMAGE* inf_in, INFO_IMAGE* inf_out, GEM_WINDOW* wprog)
{
  ...
  /* inf_out->mfdb.fd_nplanes is the number of colors we want the image to be converted to */
  ...
  if ( method & 0x80 )
  {
    ...
    /* Been added for LDV which does not know if dither_gris will work in place or not     */
    method &= ~0x80 ;
    force_allocate = 1 ;
  }
  ...
  if ( method == DITHER_AUTO )
  {
    ...
    switch( nplanes_out )
    {
      ...
      case 4:
      case 8: if ( !is_grey( &inf_in->mfdb ) ) method = (nplanes_out == 4) ? DITHER_16_4x4M:DITHER_216 ;
              break ;
    }
  }
  ...
  LoggingDo(LL_DEBUG, "DitherMethod=%d, in_planes=%d, out_planes=%d", method, inf_in->mfdb.fd_nplanes, nplanes_out) ;
  switch( method )
  {
    case DITHER_16:
    case DITHER_16_4x4M:    /*ret = dither_16( (method == DITHER_16) ? 0x00:0x04, inf_in, inf_out, wprog ) ;*/
                            ret = dither16( (method == DITHER_16) ? 0x00:0x04, inf_in, inf_out, wprog ) ;
                            break ;
