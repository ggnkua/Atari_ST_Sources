...
WORD	drucke( UBYTE *p, LONG start_x, LONG start_y, LONG weite, LONG max_zeile, LONG h_dpi, LONG v_dpi, WORD th, WORD flag )
{
	...
	WORD			rep, i, work_in[12], work[56];
	UBYTE			l;

	if(  th!=3  ||  flag!=7  ||  vq_gdos()==0  ||  start_x>0  )
	{
		Cconout( 7 );
		return -1;
	}
  ...
  work_in[0] = i = 21;
	...
	start_y = hdr.pixelh*start_y/(long)work[4];
	if(  i>0  &&  (flag&6)  )
	{
		Cconws( "\033H*" );
		...
		if(  flag&2  )
			v_bit_image(	i, tmp_file, 0,
										1, 1, /* Ganzahlig xy */
										0, (WORD)start_y, /* Links oben */
										work_in );
		Cconws( "\033H*" );
		v_updwk( i );
		Cconws( "\033H+" );
		if(  flag&4  )
			v_clrwk( i );	/* Seitenvorschub */
		Cconws( "\033H*" );
		v_clswk( i );
		Cconws( "\033H+" );
		Fdelete( tmp_file );
