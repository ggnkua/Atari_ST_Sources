...
OpenPipe()
	{
	sprintf(PipeName, "%s\\%s", TmpFilePath, "Jove-I");
   	close(creat(PipeName, 0));
	if ((PipeFile = open(PipeName,2)) == -1)
		return -1;
	if ((StdOut = dup(1)) == -1)
		return -1;
	if (dup2(PipeFile,1) == -1)
		return -1;
	return (PipeFile);
	}
ReOpenPipe()
	...
	if (close(StdOut) !=0)
		return -1; 
	return(PipeFile = open(PipeName,2));
	}
