...
int getAtariCharacter(buffer)
char *buffer;
	{
	/* Here we need to interpret the keys on the key pad */
	/* For now we will just return the ascii value of the key */
	char upper,lower;
	long int Scan;
	Scan = Crawcin();
	lower = Scan & 0x00FF;
	upper = (Scan >> 16 & 0x00FF);
	if (lower == 0x00)
		{
		*buffer++ = 255;
		*buffer = upper;
