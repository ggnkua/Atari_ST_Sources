...
#if !(defined(IBMPC) || defined(MAC)|| defined(ATARI))
#define sputc(c)	((*cursor != (char) (c)) ? dosputc(c) : (cursor++, i_col++))
#endif /* (defined(IBMPC) || defined(MAC)) || defined(ATARI) */
...
register char	c;
{
	...
	if (*cursor != c) {
		...
		if (IN_INSmode)
			INSmode(0);
#endif
#else /* defined(IBMPC) defined(ATARI) */
	if ((force) || (*cursor != c)) {
#endif /* defined(IBMPC) || defined(ATARI) */
...
register int	abortable;
{
	...
	if (n <= 0)
		return 1;
	while (c = *line++) {
#if !(defined(IBMPC) || defined(MAC)||defined(ATARI))	/* don't check after every character */
...
BufSwrite(linenum)
{
	...
	if (c != '\0') while (c = *bp++) {
#if !(defined(IBMPC) || defined(MAC) || defined(ATARI))	/* will never get true so why bother */
		if (AbortCnt < 0) {
			AbortCnt = BufSize;
			if (InputPending = charp()) {
				aborted = 1;
...
#if !(defined(MAC) || defined(ATARI))	/* remainder of this file */
#ifdef IBMPC
