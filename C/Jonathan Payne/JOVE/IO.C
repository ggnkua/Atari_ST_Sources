...
#if defined(ATARI) && defined(CHDIR)
#include <osbind.h>
#endif
...
#if (defined(ATARI) || defined(MSDOS)) && defined(CHDIR)
	* fixpath(char *),
#endif
...
#if defined(VMUNIX) || defined(MSDOS) || defined(ATARI)
char	iobuff[LBSIZE],
	genbuf[LBSIZE],
	linebuf[LBSIZE];
#else
...
#if !(defined(MAC) || defined(MSDOS) || defined(ATARI))
			chk_mtime(curbuf, curbuf->b_fname, "save");
#endif /* MAC || MSDOS || ATARI */
...
#if defined(MSDOS) || defined(ATARI)
		filefix[FILESIZE],
#endif	/* defined(MSDOS) || defined(ATARI) */
		*sp;
...
{
#ifdef BSD4_2
	/* This works properly on suns running Yellow Pages. Do non BSD systems 
	   have getpwnam() */
	struct passwd	*pw;
	...
	*intobuf;
{
	...
	if (*name == '~') {
		if (name[1] == '/' || name[1] == '\0') {
			...
			name += 1;
#if !(defined(MSDOS) || defined(MAC) ||defined(ATARI))
						/* may add for mac in future */
		} else {
...
tmpclose()
{
	...
	tmpfd = -1;
	if (unlink(tfname)== -1)
		complain("Warning: cannot unlink tmp file!");;
...
#if defined(ATARI) /* ensure that the directory ends in / */
