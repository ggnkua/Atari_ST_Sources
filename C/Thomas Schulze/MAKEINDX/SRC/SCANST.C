...
char	idx_keyword[LONG_MAX]   = IDX_KEYWORD;
...
char	preamble[LONG_MAX]  = PREAMBLE_DEF;
char	postamble[LONG_MAX] = POSTAMBLE_DEF;
int	prelen  = PREAMBLE_LEN;
int	postlen = POSTAMBLE_LEN;

char	setpage_open[LONG_MAX] = SETPAGEOPEN_DEF;
char	setpage_close[LONG_MAX] = SETPAGECLOSE_DEF;
int	setpagelen = SETPAGE_LEN;

char	group_skip[LONG_MAX]	= GROUPSKIP_DEF;
char	lethead_pre[LONG_MAX]	= LETHEADPRE_DEF;
char	lethead_suf[LONG_MAX]	= LETHEADSUF_DEF;
int	lethead_flag = LETHEADFLAG_DEF;
int	skiplen = GROUPSKIP_LEN;
int	headprelen = LETHEADPRE_LEN;
int	headsuflen = LETHEADSUF_LEN;

char	item_r[FIELD_MAX][LONG_MAX] = {ITEM0_DEF, ITEM1_DEF, ITEM2_DEF};
char	item_u[FIELD_MAX][LONG_MAX] = {"", ITEM1_DEF, ITEM2_DEF};
char	item_x[FIELD_MAX][LONG_MAX] = {"", ITEM1_DEF, ITEM2_DEF};
...
char	delim_p[FIELD_MAX][LONG_MAX] = {DELIM_DEF, DELIM_DEF, DELIM_DEF};
char	delim_n[LONG_MAX] = DELIM_DEF;   /* page number separator */
char	delim_r[LONG_MAX] = DELIMR_DEF;  /* page range designator */

char	encap_p[LONG_MAX] = ENCAP0_DEF;    /* encapsulator prefix */
char	encap_i[LONG_MAX] = ENCAP1_DEF;    /* encapsulator infix */
char	encap_s[LONG_MAX] = ENCAP2_DEF;    /* encapsulator postfix */

int	linemax = LINE_MAX;
int	indent_length = INDENTLEN_DEF;
char	indent_space[LONG_MAX] = INDENTSPC_DEF;

char	page_comp[LONG_MAX] = COMPOSITOR_DEF;
...
static char	page_prec[LONG_MAX] = PRECEDENCE_DEF;
		...
		while (TRUE)
			...
			default:
				...
				if (i < LONG_MAX)
					...
					STY_SKIPLINE;
					STY_ERROR2("Attribute string %s too long (max %d).\n",
						  clone, LONG_MAX);
					return (FALSE);
				}
				break;
...
process_precedence()
{
	...
	while ((i < PAGETYPE_MAX) && (page_prec[i] != NULL)) {
		switch (page_prec[i]) {
		case ROMAN_LOWER:
			if (roml) {
