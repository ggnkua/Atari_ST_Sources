...
#ifndef __AFFICHE_ERROR_C__
#define __AFFICHE_ERROR_C__
...
log_print(FALSE);
	...
	switch( error_code )
	{
		...
		case APPLI_ERROR_AVR_NOT_FOUND:
			idx_alt_error=ALT_ERR_AVR_NF;
			break;

		case APPLI_ERROR_AVR_FILE:
			idx_alt_error=ALT_AVR_FILE_ERR;
			break;
		...
		case APPLI_ERROR_RESERVE_RAM_AVR:
			idx_alt_error=ALT_ERR_RAM_AVR;
			break;
		...
		default:
			{
				...
				if(reponse==0)
				{
					printf(CRLF"affiche error(): rsrc_gaddr( R_TREE, DL_ALERTE_APPLI, &adr_formulaire ) error!"CRLF);
				}
				else
				{
