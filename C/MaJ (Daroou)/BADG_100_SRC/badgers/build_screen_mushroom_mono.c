...
void build_screen_mushroom_mono( void );
...
//		vro_cpyfm(global_vdihandle_framebuffer,S_ONLY,pxy_screen, &mfdb_mushroom[mushroom_idx], &mfdb_framebuffer );
		vrt_cpyfm(global_vdihandle_framebuffer, VRT_COPY_MODE, pxy_screen, &mfdb_mushroom[mushroom_idx], &mfdb_framebuffer, couleur_vrt_copy );
	}
