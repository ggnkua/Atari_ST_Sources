...
#include	"badgers_global.h"

#include	"variables_application.h"
...
int main( void )
{
	...
	if( init==TRUE )
	{
		...
		/* voir init_badgers() -> gestion appel init_surface()      */
		...
		/* Permet de redemmarer si une erreur arrive dans init_badgers() */
		...
		do
		{
				...
				if( init == 0 )
				{
					...
					build_screen_restart=TRUE; /* Force redemarrage anim */
					pf_build_screen();
					...
					if( global_mute_sound==FALSE && global_animation_pause==FALSE )
					{
						sound_start();
					}
