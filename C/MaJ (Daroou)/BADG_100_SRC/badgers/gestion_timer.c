...
#include "check_progression_dma.c"
#include "check_sequence_animation.c"
...
//			return;
...

	{
		...
		pf_build_screen();  /* construction de la frame dans le framebuffer */
		affiche_screen();   /* affichage de la frame a l'ecran */
		frame_cpt++;        /* on compte le nombre de framme affichée */

		/* controle de la sequence d'animation */
		check_progression_dma();
		check_sequence_animation();
