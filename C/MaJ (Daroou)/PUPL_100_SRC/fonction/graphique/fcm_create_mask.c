/* ***************************** */
/* * 30/01/2013 MaJ 24/09/2017 * */
/* ***************************** */


/* !!! le mode ecran 24 bits est a verifier !!! */
...
void Fcm_create_mask( MFDB *surface, MFDB *mask, uint16 x, uint16 y );
...
void Fcm_create_mask( MFDB *surface, MFDB *mask, uint16 x, uint16 y )
{
	...
	#ifdef LOG_FILE
	sprintf( buf_log, "Fcm_create_mask (%p, %p, %d, %d)"CRLF, surface, mask, x, y );
	log_print(FALSE);
	#endif
	...
	/* Ecran en 8 bits                     */
	/* ----------------------------------- */
	/*if( Fcm_screen.nb_plan==8 || Fcm_screen.nb_plan==4 || Fcm_screen.nb_plan==1)*/
