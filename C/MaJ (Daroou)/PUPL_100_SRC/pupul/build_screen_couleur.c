...
void build_screen_couleur( void )
{
	...
	vro_cpyfm(global_vdihandle_framebuffer, VRO_COPY_MODE, pxy_fond_ecran, &mfdb_fond_ecran, &mfdb_framebuffer );
	...
	/* ---------------------------------------------------------------------- */
	{
		...
		vro_cpyfm(global_vdihandle_framebuffer,VRO_COPY_MODE,pxy_damier, &mfdb_damier[damier_idx], &mfdb_framebuffer );
...
log_print(FALSE);
	...
	/* ---------------------------------------------------------------------- */
	{
		...
		for( idx_sprite=0; idx_sprite<7; idx_sprite++)
		{
			...
			/* affichage du sprite */
			vro_cpyfm(global_vdihandle_framebuffer,VRO_SPRITE_MODE,pxy_demons, &mfdb_demons_sprite[ animation[idx_sprite].sprite_forme ], &mfdb_framebuffer);
...
log_print(FALSE);
		...
		vro_cpyfm(global_vdihandle_framebuffer,3,pxy_fond_scrolltext, &mfdb_fond_scrolltext, &mfdb_framebuffer );
		...
		/* ------------------------------------------------------------------------------ */
		{
			...
			if( premier_plan==0 )
			{
				vro_cpyfm(global_vdihandle_framebuffer,3,pxy_barre_scrolltext, &mfdb_barre_scrolltext, &mfdb_framebuffer );
			}
			...
			/* ------------------------------------------------------------------------------ */
			{
				...
				for( lettre=0; lettre<nb_lettre; lettre++ )
				{
					...
					if( scrolltext_message[idx_lettre+lettre] != 111 )
					{
						...
						vro_cpyfm(global_vdihandle_framebuffer,VRO_SPRITE_MODE,pxy_texte, &mfdb_fonte_texte, &mfdb_framebuffer );
					}
				}
