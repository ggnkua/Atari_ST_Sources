...
void get_tag_config( const char *pt_fichier_config )
{
	...
	/* MOD LDG - update mode         */
	/* ----------------------------- */
	global_mod_ldg_update = Fcm_get_tag_int32( "MOD_UPDATE_POPUP=", pt_fichier_config );
	if( global_mod_ldg_update < MODLDG_UPDATE_POLLING   ||   global_mod_ldg_update > MODLDG_UPDATE_GSXB )
	{
		global_mod_ldg_update = MODLDG_UPDATE_POLLING;
	}
	FCM_LOG_PRINT1( "  global_mod_ldg_update=%d", global_mod_ldg_update );


	/* ------------------------------- */
	/* MOD LDG - frequence restitution */
	/* ------------------------------- */
	global_mod_ldg_frequence = Fcm_get_tag_int32( "MOD_FREQ_POPUP=", pt_fichier_config );
	if( global_mod_ldg_frequence < MODLDG_FREQ_12KHZ   ||   global_mod_ldg_frequence > MODLDG_FREQ_50KHZ )
	{
		global_mod_ldg_frequence = MODLDG_FREQ_12KHZ;
	}
	FCM_LOG_PRINT1( "  global_mod_ldg_frequence=%d", global_mod_ldg_frequence );
