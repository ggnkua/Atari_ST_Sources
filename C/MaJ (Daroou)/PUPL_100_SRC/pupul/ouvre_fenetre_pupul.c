...
#ifdef LOG_FILE
sprintf( buf_log, " ~ ouvre_fenetre_pupul(), hwin(%d)=", W_PUPUL );
log_print(FALSE);
#endif
	...
	else
	{
		...
		wind_calc( 0, win_widgets[W_PUPUL], x, y, SCREEN_WIDTH, SCREEN_HEIGHT, &winx, &winy, &winw, &winh );

		/* Replace la fenetre … sa derniŠre position enregistr‚ */
		Fcm_set_win_position( W_PUPUL, &winx, &winy, &dummy_pos, &dummy_pos  );

		/* On demande … l'AES de cr‚er notre fenˆtre */
		h_win[W_PUPUL] = wind_create( win_widgets[W_PUPUL],winx,winy,winw,winh ); 

		if( h_win[W_PUPUL]>0 )
		{
			/* on m‚morise la position de la fenˆtre */
			win_posxywh[W_PUPUL][0]=winx;
			win_posxywh[W_PUPUL][1]=winy;
			win_posxywh[W_PUPUL][2]=winw;
			win_posxywh[W_PUPUL][3]=winh;

#ifdef LOG_FILE
sprintf( buf_log, "%2d ; w=%3d, h=%3d"CRLF, h_win[W_PUPUL], winw, winh );
log_print(FALSE);
#endif
...
#ifdef LOG_FILE
			//snprintf( fcm_win_titre_texte[W_PUPUL], FCM_WIN_TITRE_MAXSIZE, "%s *Debug*", adr_formulaire[win_titre[W_PUPUL]].ob_spec.free_string);
			snprintf( fcm_win_titre_texte[W_PUPUL], FCM_WIN_TITRE_MAXSIZE, "%s *Debug*", PRG_FULL_NOM );
#else
			//snprintf( fcm_win_titre_texte[W_PUPUL], FCM_WIN_TITRE_MAXSIZE, "%s", adr_formulaire[win_titre[W_PUPUL]].ob_spec.free_string);
			snprintf( fcm_win_titre_texte[W_PUPUL], FCM_WIN_TITRE_MAXSIZE, "%s", PRG_FULL_NOM );
#endif
			wind_set_str(h_win[W_PUPUL],WF_NAME, fcm_win_titre_texte[W_PUPUL]);
			wind_set_str(h_win[W_PUPUL],WF_INFO, win_info_texte);
			/* ... et on demande … l'AES de l'ouvrir */
			wind_open(h_win[W_PUPUL],winx,winy,winw,winh);
		}
		else
		{
			int16 dummy;
...
log_print(FALSE);
#endif
			h_win[W_PUPUL]=FCM_NO_OPEN_WINDOW;
