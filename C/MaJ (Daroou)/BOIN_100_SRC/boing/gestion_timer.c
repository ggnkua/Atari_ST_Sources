...
//	static  int16 appl_write_cpt   = 0;
...

	{
		...
		calcul_screen();    /* calcul de la nouvelle position des sprites */
		build_screen();     /* construction de la frame */
		affiche_screen();   /* affichage de la frame    */
	...
	if( (timer - old_timer_fps) >= 200 )
	{
		...
		/* temps mis pour réaliser un cycle (en milliseconde) temps moyen (global_charge_build_screen en 1/200 de seconde) */
...
//		sprintf( win_info_texte, "%d appl/s", appl_write_cpt );
