/* **[Boing]******************** */
/* *                           * */
/* * 20/09/2015 MaJ 28/05/2020 * */
/* ***************************** */



//#include "../fonction/coeur/coeur_define.c"
#include "affiche_error.c"



/* prototype */
int32 check_fichier( void );



/* Fonction */
int32 check_fichier( void )
{
	char  mon_chemin[ (TAILLE_CHEMIN_DATA+16) ];
	char  mono[]="MONO\\";



	#ifdef LOG_FILE
	sprintf( buf_log, CRLF"# check_fichier - init_generale()"CRLF);
	log_print(FALSE);
	#endif



	/***********************************/
	/* m‚morisation du chemin des data */
	/***********************************/

	/* On v‚rifie que la taille du chemin est inf‚rieur */
	/* … la capacit‚ du buffer                          */
	if( (strlen(Fcm_chemin_courant) + strlen("DATA\\") + strlen("MONO\\")) > TAILLE_CHEMIN_DATA )
	{
		return(APPLI_ERROR_DATA_PATH_OVER);
	}

	strcpy( chemin_data, Fcm_chemin_courant );
	strcat( chemin_data, "DATA\\" );

	if( global_affichage_mode != AFFICHAGE_MODE_MONO )
	{
		/* on vide le buffer */
		mono[0] = 0;
	}

	#ifdef LOG_FILE
	sprintf( buf_log, "  chemin data {%s}"CRLF, chemin_data );
	log_print(FALSE);
	#endif






	/****************************************************/
	/* V‚rification de la presence de tous les fichiers */
	/****************************************************/
	{
		int16 erreur=0;


/*		strcpy( mon_chemin, chemin_data );
		strcat( mon_chemin, "FOND.TGA" );*/
		
		sprintf( mon_chemin, "%s%s%s", chemin_data, mono, "FOND.TGA");
		if( Fcm_file_exist( mon_chemin ) != TRUE )
		{
			#ifdef LOG_FILE
			sprintf( buf_log, "  ERREUR: %s absent"CRLF, mon_chemin );
			log_print(FALSE);
			#endif

			erreur++;
		}

		/*strcpy( mon_chemin, chemin_data );
		strcat( mon_chemin, "OMBRE.TGA" );*/
		sprintf( mon_chemin, "%s%s%s", chemin_data, mono, "OMBRE.TGA");
		if( Fcm_file_exist( mon_chemin ) != TRUE )
		{
			#ifdef LOG_FILE
			sprintf( buf_log, "  ERREUR: %s absent"CRLF, mon_chemin );
			log_print(FALSE);
			#endif

			erreur++;
		}


		{
			int16 idx;

			for(idx=1; idx<8; idx++)
			{
				/*sprintf( mon_chemin, "%sBOING%1d.TGA", chemin_data, idx );*/
				sprintf( mon_chemin, "%s%sBOING%1d.TGA", chemin_data, mono, idx);

				if( Fcm_file_exist(mon_chemin) != TRUE )
				{
					#ifdef LOG_FILE
					sprintf( buf_log, "  ERREUR: %s absent"CRLF, mon_chemin );
					log_print(FALSE);
					#endif

					erreur++;
				}
			}

		}



		if( erreur != 0 )
		{
			#ifdef LOG_FILE
			sprintf( buf_log, CRLF"# %d fichier(s) absent(s)"CRLF, erreur);
			log_print(FALSE);
			#endif

			return(APPLI_ERROR_DATAFILE_NOT_FOUND);
		}








		/* les sons sont dans le dossier DATA, en mode graphique mono ou non */

		sprintf( mon_chemin, "%s%s", chemin_data, "BOING_M.AVR");
		if( Fcm_file_exist( mon_chemin ) != TRUE )
		{
			#ifdef LOG_FILE
			sprintf( buf_log, "  ERREUR: %s absent"CRLF, mon_chemin );
			log_print(FALSE);
			#endif

			global_flag_audio_ok=FALSE;
			affiche_error(BOING_ERROR_BOINGM_NOT_FOUND);
		}

		sprintf( mon_chemin, "%s%s", chemin_data, "BOING_S.AVR");
		if( Fcm_file_exist( mon_chemin ) != TRUE )
		{
			#ifdef LOG_FILE
			sprintf( buf_log, "  ERREUR: %s absent"CRLF, mon_chemin );
			log_print(FALSE);
			#endif

			global_flag_audio_ok=FALSE;
			affiche_error(BOING_ERROR_BOINGS_NOT_FOUND);
		}

	}




	return 0L;


}

