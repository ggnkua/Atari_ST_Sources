...
void set_config( void )
{
	...
	/* ******************************************************* */
	{
		...
		/* ---------------------------- */
		{
			char chemin[FCM_TAILLE_CHEMIN*2];

			if( strlen(Fcm_chemin_image_cache)==0 )
			{
//				sprintf( Fcm_chemin_image_cache, "%sDATA\\CACHE\\", Fcm_chemin_courant);
				sprintf( chemin, "%sDATA\\CACHE\\", Fcm_chemin_courant);
				if( strlen(chemin) < FCM_TAILLE_CHEMIN )
				{
					strcpy( Fcm_chemin_image_cache, chemin );
				}
				else
				{
					FCM_CONSOLE_DEBUG2( "set_config(): chemin trop long %ld/%d", strlen(chemin), FCM_TAILLE_CHEMIN );
				}
			}
		...
		/* ---------------------------- */
		{
			...
			else
			{
				...
				SET_BIT_W( adr_formulaire[CHECK_FRAME_BUF].ob_state, OS_SELECTED, 0U);
			}
	...
	/* ---------------------------------------- */
#ifdef LOG_FILE
	snprintf( fcm_win_titre_texte[W_BOING], FCM_WIN_TITRE_MAXSIZE, "%s *Debug*", PRG_FULL_NOM );
#else
	snprintf( fcm_win_titre_texte[W_BOING], FCM_WIN_TITRE_MAXSIZE, "%s", PRG_FULL_NOM );
#endif
	fcm_win_info_texte[W_BOING] = win_info_texte;

	snprintf( fcm_win_titre_texte[W_RAM    ], FCM_WIN_TITRE_MAXSIZE, APP_WIN_PREFIX_TITRE" %s", Fcm_adr_RTREE[DL_TITRE_W_APPLI][ win_titre[W_RAM] ].ob_spec.free_string);
