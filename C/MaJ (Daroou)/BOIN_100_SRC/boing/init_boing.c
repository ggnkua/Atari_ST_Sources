/* **[Boing]******************** */
/* *                           * */
/* * 20/09/2015 MaJ 25/09/2023 * */
/* ***************************** */



#include "../fonction/ldg/tga_sa_ldg.h"
#include "../fonction/graphique/pixel_format_id.h"
...
int32 init_boing( void )
{
	...
	#ifdef LOG_FILE
	sprintf( buf_log, CRLF"# init_boing()"CRLF);
	log_print(FALSE);
	#endif
	...
	if( Fcm_screen.nb_plan<4 || global_force_mode_mono==TRUE )
	{
		...
		/* affichage en mono */
		/* on affecte les pointeurs de fonctions */
	...
	global_flag_audio_ok=TRUE;


	/* pour afficher et debugger la mire de selection du format des pixels */
//Fcm_screen.pixel_xformat=PIXEL_FORMAT_INCONNU;


	/* mode 256 couleurs ou plus, format des pixels reconnu ? */
	/* par défaut, tous les labels sont en mode AUTO (voir boing_global.h) */
	if( global_pixel_label_actif == POP_PIXEL_AUTO )
	{
		...
		if( global_force_mode_mono==FALSE && (Fcm_screen.nb_plan >= 8 && Fcm_screen.pixel_xformat <= 0) )
		{
			...
			{
				int16 alerte = Fcm_select_pixel_xformat();

				/* Si le format est toujours inconnu, une erreur s'est produite... */
				if( Fcm_screen.pixel_xformat == PIXEL_FORMAT_INCONNU )
				{
					/* erreur dans la fonction Fcm_select_pixel_xformat() */
					global_app_restart = FALSE; /* ne pas redemarrer */

					if( alerte )  affiche_error( alerte );

					return( APPLI_ERROR_APPLI_STOP );
				}
			}
	...
	Fcm_fenetre_launch( LH_INIT, LAUNCH_INIT, 20-1 ); /* nombre total d'appel a Fcm_fenetre_launch => bar progression */
...

	{
		...
		if( open_LDG_TGA(&ldg_tga_version) != TGA_LDG_NO_ERROR )
		{
			/* erreur   */
			...
			#ifdef LOG_FILE
			sprintf( buf_log, " Erreur open_LDG_TGA !!! TGA_SA.LDG version : $%04x, (%x.%x)"CRLF, ldg_tga_version, (ldg_tga_version>>8), (ldg_tga_version&0xff) );
			log_print(FALSE);
			#endif
		...
		#ifdef LOG_FILE
		sprintf( buf_log, " OK, TGA_SA.LDG version : $%04x, (%x.%x)"CRLF, ldg_tga_version, (ldg_tga_version>>8), (ldg_tga_version&0xff) );
		log_print(FALSE);
		#endif
	}
	...
	/* ******************************************************** */
	/* * VDI handle pour l'ecran                              * */
	/* ******************************************************** */
	global_vdihandle_ecran = vdihandle; /* vdi handle de l'application */
	...
	if( global_flag_audio_ok==TRUE && Fcm_systeme.xbios_sound==TRUE )
	{
		...
		Fcm_fenetre_launch(LH_LOAD_SOUND, LAUNCH_UPDATE, LAUNCH_WAIT_FAST);
		...
		if( reponse != 0 )
		{
			/* on décharge les eventuels sons qui auraient été chargé */
			unload_sound();

			global_flag_audio_ok=FALSE;

			if( reponse==APPLI_ERROR_RESERVE_RAM ) /* modifier directement dans load_sound() ce code d'erreur */
			{
				affiche_error(BOING_ERROR_SOUND_RAM);
			}
			else
			{
	...
	{
		global_flag_audio_ok = FALSE; /* est-il utilisé ailleur ??? */
