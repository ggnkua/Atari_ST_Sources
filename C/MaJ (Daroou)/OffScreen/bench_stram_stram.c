...
void bench_stram_stram( void )
{
	...
	vro_cpyfm(vdihandle, 3, pxy_screen, &mfdb_fond, &mfdb_fond_stram );
...

	{
		...
		Fcm_objet_draw( adr_formulaire, h_win[W_BENCH], BC_STRAM_STRAM_R, FCM_WU_BLOCK );
	}
...

	{
		...
		do
		{
			...
			{
				vro_cpyfm(vdihandle, 3, pxy,  &mfdb_fond_stram, &mfdb_offscreen_stram);
			}
...
/*{
	...
	v_gtext(vdihandle,61*8,3*16,texte);
		...
		gb_stats_stram_stram=(((BENCH_TAILLE_ECRAN*nb_boucle)/timer)*200)/1048576;
