...
void Fcm_objet_draw( OBJECT *adr_formulaire, int16 handle_win, uint16 objet, int16 windupdate_mode )
{
	...
	/* -------------------------------------------------------------------- */
	/* Si handle_win == FCM_NO_OPEN_WINDOW  affichage brut de l'objet       */
	/* Si handle_win est valide, affichage standard AES                     */
	/* Sinon on quitte (fenetre shaded ou erreur d'handle)                  */
	/* -------------------------------------------------------------------- */



	if( handle_win != FCM_NO_OPEN_WINDOW )
	{
		int16 index_win;
		...
		#ifdef LOG_FILE
		sprintf( buf_log, TAB8" - index_win=%d"CRLF, index_win);
		log_print(FALSE);
		#endif
		...
		if( index_win != FCM_NO_MY_WINDOW )
		{
			...
			{
				#ifdef LOG_FILE
				sprintf( buf_log, TAB8" - win_shaded, no draw"CRLF);
				log_print(FALSE);
				#endif
		...
		{
			#ifdef LOG_FILE
			sprintf( buf_log, TAB8" - ERREUR handle"CRLF);
			log_print(FALSE);
			#endif
		...
		#ifdef LOG_FILE
		sprintf( buf_log, TAB8" - pas d'erreur index_win"CRLF);
		log_print(FALSE);
		#endif
	}
...

	{
		...
		#ifdef LOG_FILE
		sprintf( buf_log, TAB8" - redraw objet fenˆtr‚"CRLF);
		log_print(FALSE);
		#endif
...
/*					sprintf(texte,"Fcm_objet_draw, type objet non reconnu (%d)", type_objet );
		...
		if( handle_win != FCM_NO_OPEN_WINDOW )
		{
			...
			/* on cale le formulaire en cas de MOVED sur OS multitache */
			adr_formulaire->ob_x=r2.g_x;
			adr_formulaire->ob_y=r2.g_y;

			/*   Affichage standard AES                                     */
			/* ------------------------------------------------------------ */
		...
		else
		{
			...
			/* ------------------------------------------------------------ */
			objc_draw( adr_formulaire, objet,7,r1.g_x,r1.g_y,r1.g_w,r1.g_h );
		}
