...
int32 Fcm_bload(const char *nom_fichier, const char * adresse, int32 saut, int32 nombre)
{
	...
	if( nombre>0 )
	{
		...
		do
		{
			/* on charge par bloc de 128ko max pour ne pas bloquer le systŠme */
			sizetoload = MIN( dummy, 131072L );
			evnt_timer(20);

			nombre_charger = Fread( handle_fichier, /*dummy*/sizetoload, adresse );
...
*/
	...
	if(nombre==-1)
	{
		do
		{
			nombre_charger = Fread(handle_fichier, 131072L, adresse);
...
*/
	...
	if( reponse>0 )
	{
		...
		log_print( FALSE );
		sprintf( buf_log, TAB8" en %ld/200Šme seconde(s)"CRLF, (Fcm_get_timer()-timer) );
		log_print( FALSE );
		#endif
	}
