...
/*	sprintf(texte," Fcm_gestion_va_start");
...

	{
		...
		ptc_va_start_parms=(CHAR *)( ((ULONG)buffer_aes[3]<<16) + (UWORD)buffer_aes[4] );
...
/*				Fcm_form_alerte(0, ALT_AV_NO_FILE);*/
				Fcm_affiche_alerte( DATA_MODE_COEUR, ALC_AV_NO_FILE, &dummy );
			}
			else
			{
				int16 dummy;
...
/*				Fcm_form_alerte(0, ALT_AV_BAD_FILE);*/
				Fcm_affiche_alerte( DATA_MODE_COEUR, ALC_AV_BAD_FILE, &dummy );
