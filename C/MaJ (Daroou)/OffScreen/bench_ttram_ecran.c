...
void bench_ttram_ecran( void )
{
	...
	vro_cpyfm(vdihandle, 3, pxy_screen, &mfdb_fond, &mfdb_offscreen_ttram );
...

	{
		...
		Fcm_objet_draw( adr_formulaire, h_win[W_BENCH], BC_TTRAM_ECR_R, FCM_WU_BLOCK );
	}
...

	{
		...
		do
		{
			...
			{
				vro_cpyfm(vdihandle, 3, pxy,  &mfdb_offscreen_ttram, &Fcm_mfdb_ecran);
			}
...
/*{
	...
	v_gtext(vdihandle,61*8,3*16,texte);
		...
		gb_stats_ttram_ecran=(((BENCH_TAILLE_ECRAN*nb_boucle)/timer)*200)/1048576;
