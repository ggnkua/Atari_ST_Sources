...
/*	vro_cpyfm(vdihandle, 3, pxy_screen, &mfdb_fond_stram, &mfdb_offscreen_stram );
	return;
*/
	...
	/* ---------------------------------------- */
	{
		...
		else
		{
			...
			vro_cpyfm(vdihandle,3,pxy_fond, &mfdb_fond_stram, &mfdb_offscreen_stram );
		...
		if( bar_index_y[0]<20 && bar_sens[0]==1 )
		{
			for( idx=0; idx<NB_BAR; idx++ )
			{
				if( bar_sens[idx]==1 )
				{
					...
					bar_index_y[idx]--;
					vro_cpyfm(vdihandle,3,pxy_bar, &mfdb_bar_stram[idx], &mfdb_offscreen_stram );
				}
			}
		}
		else
...
/*	{
		...
		vrt_cpyfm(vdihandle,2,pxy_bee, &mfdb_bee_mask_stram, &mfdb_offscreen_stram, mask_couleur );
		vro_cpyfm(vdihandle,7,pxy_bee, &mfdb_bee_stram, &mfdb_offscreen_stram );
	}*/
