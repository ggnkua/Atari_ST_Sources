...
void ouvre_fenetre_info( void )
{
	...
	#ifdef LOG_FILE
	sprintf( buf_log, TAB8"* ouvre_fenetre_info() : hwin(%d)=", W_INFOSYS );
	log_print(FALSE);
	#endif



	if( h_win[W_INFOSYS] > 0 )
	{
		buffer_aes[0]=WM_TOPPED;
		buffer_aes[1]=ap_id;
		buffer_aes[2]=0;
		buffer_aes[3]=h_win[W_INFOSYS];
	...
	else
	{
		...
		Fcm_set_win_position( W_INFOSYS, &winx, &winy );


		/* On demande … l'AES de cr‚er notre fenˆtre */
		h_win[W_INFOSYS]=wind_create(win_flag, winx, winy, winw, winh); 


		if( h_win[W_INFOSYS]>0 )
		{
			/* on m‚morise la position de la fenˆtre */
			win_posxywh[W_INFOSYS][0]=winx;
			win_posxywh[W_INFOSYS][1]=winy;
			win_posxywh[W_INFOSYS][2]=winw;
			win_posxywh[W_INFOSYS][3]=winh;

			#ifdef LOG_FILE
			sprintf( buf_log, "%2d ; x=%4d, y=%4d, w=%3d, h=%3d"CRLF, h_win[W_INFOSYS], winx, winy, winw, winh );
			log_print(FALSE);
			#endif
			...
			wind_set_str(h_win[W_INFOSYS],WF_NAME,(adr_formulaire+TITRE_INFO_SYS)->ob_spec.free_string);
			/* ... et on demande … l'AES de l'ouvrir */
			wind_open(h_win[W_INFOSYS],winx,winy,winw,winh);
		...
		else
		{
			...
			h_win[W_INFOSYS]=FCM_NO_OPEN_WINDOW;
