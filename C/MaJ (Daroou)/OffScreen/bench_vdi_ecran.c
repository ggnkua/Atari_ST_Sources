...
void bench_vdi_ecran( void )
{
	...
	vro_cpyfm(vdihandle, 3, pxy_screen, &mfdb_fond, &mfdb_offscreen_vdi );
...

	{
		...
		Fcm_objet_draw( adr_formulaire, h_win[W_BENCH], BC_OFF_ECR_R, FCM_WU_BLOCK );
	}
...

	{
		...
		do
		{
			...
			{
				vro_cpyfm(vdihandle, 3, pxy,  &mfdb_offscreen_vdi, &Fcm_mfdb_ecran);
			}
...
/*{
	...
	sprintf(texte,"nb_boucle=%ld timer=%ld (%ld)", nb_boucle, timer, timer_start );
	v_gtext(vdihandle,61*8,3*16,texte);
		...
		gb_stats_vdi_ecran=(((BENCH_TAILLE_ECRAN*nb_boucle)/timer)*200)/1048576;
