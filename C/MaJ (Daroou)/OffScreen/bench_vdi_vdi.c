...
void bench_vdi_vdi( void )
{
...

	{
		...
		Fcm_objet_draw( adr_formulaire, h_win[W_BENCH], BC_OFF_OFF_R, FCM_WU_BLOCK );
	}
...

	{
		...
		do
		{
			...
			{
				vro_cpyfm(vdihandle, 3, pxy,  &mfdb_fond, &mfdb_offscreen_vdi);
			}
...
/*{
	...
	sprintf(texte,"nb_boucle=%ld timer=%ld (%ld) offscreenhandle=%d  ", nb_boucle, timer, octet_total, timer_start, offscreenhandle );
	v_gtext(vdihandle,61*8,3*16,texte);
		...
		gb_stats_vdi_vdi=(((BENCH_TAILLE_ECRAN*nb_boucle)/timer)*200)/1048576;
