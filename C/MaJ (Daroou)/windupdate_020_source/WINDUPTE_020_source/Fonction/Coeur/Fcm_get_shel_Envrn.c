...
int32 Fcm_get_shel_envrn( char *string, const char *name, const uint16 size_max )
{
	...
	if ( shel_envrn(chaine, name) == 1 )
	{
		...
		#ifdef LOG_FILE
		sprintf( buf_log, TAB8"-> chaine=%p -> chaine[0]=%p"CRLF, chaine, chaine[0] );
		log_print(FALSE);
		#endif
		...
		if( chaine[0] != NULL )
		{
			...
			if( strlen(chaine[0]) < size_max )
			{
				strcpy( string, chaine[0] );
			...
			{
				#ifdef LOG_FILE
				sprintf( buf_log, TAB8"-> reponse trop longue (%ld) => {%s}"CRLF,  strlen(chaine[0]), chaine[0] );
				log_print(FALSE);
				#endif
			}
		}
