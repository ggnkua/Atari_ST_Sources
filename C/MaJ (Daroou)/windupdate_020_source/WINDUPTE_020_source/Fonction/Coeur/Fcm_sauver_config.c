 ...
 */
	...
	if( force_save==FALSE )
	{
		...
		if( ((adr_formulaire+PREF_SAVE_AUTO)->ob_state & OS_SELECTED)==0 )
		{
			#ifdef LOG_FILE
	...
	if(handle_fichier>0)
	{
		Fcm_fprint( handle_fichier, PRG_FULL_NOM" "PRG_VERSION" * Fichier de Configuration" );
