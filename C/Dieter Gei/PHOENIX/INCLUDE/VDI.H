...
#define PAGE_DEFAULT     0 /* use default values */
...
#if GEM & GEM1
VOID v_set_app_buff    _((WORD handle, VOID FAR *address, WORD nparagraphs));
WORD v_bez_qual        _((WORD handle, WORD prcnt, WORD *actual));
VOID v_bez_fill        _((WORD handle, WORD count, WORD *xyarr, BYTE *bezarr, WORD *extent, WORD *totpts, WORD *totmoves));
#else
...
WORD v_copies          _((WORD handle, WORD count));
WORD v_orient          _((WORD handle, WORD orientation));
...
VOID  vqt_f_extent     _((WORD handle, BYTE *string, WORD *extent));
VOID  v_ftext          _((WORD handle, WORD x, WORD y, BYTE *string));
VOID  v_ftext_offset   _((WORD handle, WORD x, WORD y, BYTE *string, WORD *offset));
VOID  v_killoutline    _((WORD handle, VOID *component));
VOID  v_getoutline     _((WORD handle, WORD ch, WORD *xyarray, BYTE *bezarray, WORD maxverts, WORD *numverts));
VOID  vst_scratch      _((WORD handle, WORD mode));
VOID  vst_error        _((WORD handle, WORD mode, WORD *errorvar));
VOID  vqt_advance      _((WORD handle, WORD ch, WORD *advx, WORD *advy, WORD *remx, WORD *remy));
VOID  vqt_advance32    _((WORD handle, WORD ch, fix31 *advx, fix31 *advy));
WORD  vst_arbpt        _((WORD handle, WORD point, WORD *chwd, WORD *chht, WORD *cellwd, WORD *cellht));
fix31 vst_arbpt32      _((WORD handle, fix31 point, WORD *chwd, WORD *chht, WORD *cellwd, WORD *cellht));
VOID  vst_charmap      _((WORD handle, WORD mode));
VOID  v_getbitmap_info _((WORD handle, WORD ch, fix31 *advx, fix31 *advy,
                          fix31 *xoff, fix31 *yoff, fix31 *width, fix31 *height));
VOID  vqt_pairkern     _((WORD handle, WORD ch1, WORD ch2, fix31 *x, fix31 *y));
VOID  vqt_trackkern    _((WORD handle, fix31 *x, fix31 *y));
VOID  vqt_fontheader   _((WORD handle, BYTE *buffer, BYTE *pathname));
VOID  vst_kern         _((WORD handle, WORD tmode, WORD pmode, WORD *tracks, WORD *pairs));
fix31 vst_setsize32    _((WORD handle, fix31 point, WORD *chwd, WORD *chht, WORD *cellwd, WORD *cellht));
VOID  vqt_devinfo      _((WORD handle, WORD devnum, WORD *devexits, BYTE *devstr));
WORD  v_flushcache     _((WORD handle));
VOID  vqt_cachesize    _((WORD handle, WORD which_cache, long *size));
VOID  vqt_get_table    _((WORD handle, WORD **map));
WORD  v_loadcache      _((WORD handle, BYTE *filename, WORD mode));
WORD  v_savecache      _((WORD handle, BYTE *filename));
WORD  vst_setsize      _((WORD handle, WORD point, WORD *chwd, WORD *chht, WORD *cellwd, WORD *cellht));
WORD  vst_skew         _((WORD handle, WORD skew));
#endif /* GEM1 */
