 ...
 * 24.12.02: Suche nach OLGA aus init_global entfernt wird schon in init_term
 *					 gemacht.
 * 05.07.02: Globaler Speicher nach einer Methode von Reiner Seitel eingebaut.
 * 23.06.02: Es wird ein globaler Speicherbereich reserviert.
...
/* Ende */
  ...
  if (Sysconf(-1) != EINVFN)
      global_mem1 = (char *)Mxalloc ( 1024L, 0x23);
  else
      global_mem1 = (char *)Malloc ( 1024L );
	if ( global_mem1 == NULL )
		return FALSE;
  if (Sysconf(-1) != EINVFN)
      global_mem2 = (char *)Mxalloc ( 1024L, 0x23);  /* Warum 0x23 und nicht 0x22 ? */
  else
      global_mem2 = (char *)Malloc ( 1024L );
	if ( global_mem2 == NULL )
		return FALSE;
