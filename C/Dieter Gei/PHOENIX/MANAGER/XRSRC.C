 ...
 *
 * 04.01.2004	An die Header von Phoenix angepasst.
 *
...
LOCAL VOID xrect2array   _((CONST RECT *rect, WORD *array));
...
EXTERN WORD xrsrc_load (CONST BYTE *re_lpfname, WORD *pglobal)
{
...
EXTERN WORD xrsrc_free (WORD *pglobal)
{
...
EXTERN WORD xrsrc_gaddr (WORD re_gtype, WORD re_gindex, VOID *re_gaddr, WORD *pglobal)
{
...
EXTERN WORD xrsrc_saddr (WORD re_stype, WORD re_sindex, VOID *re_saddr, WORD *pglobal)
{
...
EXTERN WORD xrsrc_obfix (OBJECT *re_otree, WORD re_oobject)
{
...
LOCAL VOID fix_chp (WORD *pcoord, WORD flag)
{
	...
	if (!flag && ncoord == 0x50)
		ncoord = xdesk.w;											/* xdesk.w = Breite des Bildschirms in Pixel */
	else
	...
	if (num != -1)
	{
	  ...
	  for (ob = 0; ob < rsxhdr->rsh_nobs; ob++)
  		if ((pobject[ob].ob_type & 0xff) == G_CICON)
  			pobject[ob].ob_spec/*.index*/ = cicon_liste[pobject[ob].ob_spec/*.indirect*/];
...
{	WORD     x, y, line, xmax, best_planes, find_planes;
  ...
  OBBLK    *ub;
	...
	obj->ob_spec/*.userblk*/ = (LONG)&ub[nub].ublk;
...
LOCAL VOID xrect2array (CONST RECT *rect, WORD *array)
