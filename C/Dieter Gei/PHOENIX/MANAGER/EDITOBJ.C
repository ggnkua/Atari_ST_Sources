 ...
 *
 * History:
 * 08.09.04: ed_paste: Kommt ein Text vom externen Clipboard werden die
 *					 Tabulatoren in Leerzeichen gewandelt. UNIX Linenende in Atari.
 *					 TabSize wird benutzt.
 * 24.01.04: ed_key erlaubt das lîschen von Zeilen per Ctrl-Y
 * 22.06.03: edit_obj wieder entfernt. ed_copy, ed_cut, ed_past und
 *           ed_can_uccpcs unterstÅtzen nun das externe Klemmbrett.
 * 19.03.03: New parameter edit_obj and ed_copy
 * 07.11.02: ed_pastetxt added
...
LOCAL WORD    ed_pastetxt			_(( EDOBJP ed, VOID *buffer));
...
*/
    case ED_PASTEBUF     : ret = ed_pastebuf (edobj, p, wparam); break;
		case ED_PASTETXT		 : ret = ed_pastetxt ( edobj, p );			 break;
...
/* [GS] 5.1e Start */
							     ...
							     for ( i = 0; i < TabSize; i++ )
									   s[i] = ' ';
									 s[i] = EOS;
                   if (ins_chars (ed, &s [ed->act_col % TabSize], TRUE))
/* alt:
                   strcpy (s, "        ");
...
/* [GS] 5.1d */
      if ( mk->ascii_code == EM && mk->scan_code == 44 ) /* Ctrl-Y */
      {
				if (! (ed->flags & ED_OUTPUT))
        {
        	ok = FALSE;
...
/* [GS] 5.1e Start */
	{
  	lClipSize = strlen ( pClipBuffer );
		buf = mem_alloc ( 4 * lClipSize );
		if ( buf == NULL )
			return ED_NOMEMORY;
		l = k = 0;
		while ( *(pClipBuffer + l) != EOS )				/* Filtern von UNIX Zeilenenden */
		{																					/* Tabulator auf zwei Zeichen		*/
			if ( *(pClipBuffer + l) == LF )
			{
				if ( l == 0 || *(pClipBuffer + l -1 ) != CR )
				{
					*(buf + k++) = CR;
					*(buf + k++) = LF;
				}
...
LOCAL WORD ed_pastetxt ( EDOBJP ed, VOID *buffer)
{
  WORD  ret, lines;
  LONG  lBufferSize, lRestSize, lMoveSize, l;
  EDOBJ ed_old;
  CHAR  *buf;

  if (buffer == NULL) return (ED_GENERAL);

  ed_old    = *ed;
  lBufferSize = strlen ( buffer ) - 1;	/* size without EOS */
  lRestSize = ed->bufsize - ed->bufindex - lBufferSize;
  lMoveSize = min (lBufferSize, ed->bufsize - ed->bufindex);

  if (lRestSize > 0)	/* move rest of text to the right */
    mem_lmove (&ed->text [ed->bufindex + lBufferSize], &ed->text [ed->bufindex], lRestSize);

  ed->actsize = min (ed->actsize + lBufferSize, ed->bufsize);
  mem_lmove (&ed->text [ed->bufindex], buffer, lMoveSize);
  ed->text [ed->actsize] = EOS;
