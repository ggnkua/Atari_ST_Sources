...
LOCAL WORD cdecl handle_exit( WD_DIALOG *dialog, EVNT *events, WORD obj,WORD clicks, void *data );
...
{
	fenster=wdlg_create(handle_exit,SetUp_Tree,NULL,0,NULL,WDLG_BKGD);
	if( fenster )
	{
...
WORD cdecl handle_exit( WD_DIALOG *dialog, EVNT *events, WORD obj, WORD clicks, void *data )
{
	OBJECT	*tree;
	GRECT		rect;
