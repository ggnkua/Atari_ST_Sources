...
output_ltype()
{
  ...
  fprintf(fattrs, "#define\tYYACCEPT\treturn(0)\n");
  fprintf(fattrs, "#define\tYYABORT\treturn(1)\n");
  fprintf(fattrs, "#define\tYYERROR\treturn(1)\n");

  if (fdefines)
    {
      fprintf(fdefines, "#define\tYYACCEPT\treturn(0)\n");
      fprintf(fdefines, "#define\tYYABORT\treturn(1)\n");
      fprintf(fdefines, "#define\tYYERROR\treturn(1)\n");
    }
}
  ...
  while (count > 0)
    {
      while (c != '}')
		...
		fatal("$%c is invalid",c);	/* JF changed format */
