...
int		cdecl do_function2(uchar *c, uchar *m, uchar *y, int w, int h, GRECT *area, int prv)
{
	register uchar *cc=c, *mm=m, *yy=y, hell, max, min;
	...
	if(cc && mm && yy)
	{
		...
		{/* Mehr Kontrast */
			...
			for(yc=area->g_h; yc > 0; --yc)
			{
				for(xc=ww; xc > 0; --xc)
				{
					if(*cc & 128)	{if(*cc < max) *cc++ +=hell; else *cc++=255;}
					else {if(*cc > hell) *cc++-=hell; else	*cc++=0;}
					if(*mm & 128)	{if(*mm < max) *mm++ +=hell; else *mm++=255;}
					else {if(*mm > hell) *mm++-=hell; else	*mm++=0;}
					if(*yy & 128)	{if(*yy < max) *yy++ +=hell; else *yy++=255;}
					else {if(*yy > hell) *yy++-=hell; else	*yy++=0;}
				}
		...
		{/* Weniger Kontrast */
			...
			for(yc=area->g_h; yc > 0; --yc)
			{
				for(xc=ww; xc > 0; --xc)
				{
					if(*cc & 128)	{if(*cc > min) *cc++ -=hell; else *cc++=128;}
					else {if(*cc < max) *cc++ +=hell; else	*cc++=128;}
					if(*mm & 128)	{if(*mm > min) *mm++ -=hell; else *mm++=128;}
					else {if(*mm < max) *mm++ +=hell; else	*mm++=128;}
					if(*yy & 128)	{if(*yy > min) *yy++ -=hell; else *yy++=128;}
					else {if(*yy < max) *yy++ +=hell; else	*yy++=128;}
				}
				cc+=ldif;	mm+=ldif;	yy+=ldif;
			}
		}
