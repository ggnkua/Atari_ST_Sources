 ...
 *	      and GNU-C on a SUN-SPARCstation.
 *
 ...
 *
 * gcc ximgtool.c l3decode.c encoders.c
 *
 * NOTE: The UNIX version does _not_ support the -m
...
static void Fbufwrite(FBUF *fp)
{
  ...
  count = BUF_LENGTH;
  if (count -= fp->pub.bytes_left)
  {
    fp->pub.pbuf = fp->fbuf;
    fp->pub.bytes_left += count;
    if (fp->EOF_flag) return;
    fp->length += count;
    if (fp->handle < 0) return;
...
static void do_proc(char *src, char *des, short outlev, short outpat, int ident)
{
  ...
  input.length = 0; input.EOF_flag = 0;

  if (ident == 0) {
    if (des == 0) output.handle = -1;
    else {
#ifndef __TOS__
    ...
    output.length = 0; output.EOF_flag = 0;
  }
...
static void ProcHeader(int ident)
{
  ...
  p = (char *)&header; count = sizeof(header);
  do FGETC(&input.pub, *p++); while (--count);
  ...
  if (ident == 0) {
    ...
    p = (char *)&header; count = sizeof(header);
    do FPUTC(&output.pub, *p++); while (--count);
