/*
	...
	v3.1 (27.05.1995), Ch. Zwerschke
		kleine Korrektur, englische Resource
*/
...
CPX_INFO *cdecl init(CPX_PARAMS *par)
{
	...
	params=par;		/* Adresse des Parameterblocks merken */

	/*
		Schon beim Booten wird eventuell die WHITEBOX ben”tigt,
		daher werden zuerst die Resource-Koordinaten transformiert
	*/
	if ( !params->rsc_init ) {
		...
		if ( (*par->find_cookie)(LEDP_MAGIC,(long*)&ledp) &&
			...
			*/
			if ( conf.saved )
				set_conf(&conf);
			/*
				Gr”že des Systemzeichensatzes holen und an LED-Panel weitergeben
			*/
...
{
	static char *txt_year=0;
	
	if ( !txt_year )
		txt_year=objc_spec(dialog2,JAHR);
	objc_spec(dialog2,JAHR)=
...
int init_conf(CONF *conf)
{
	...
	static const char txt_delay[3],txt_dist[3];
