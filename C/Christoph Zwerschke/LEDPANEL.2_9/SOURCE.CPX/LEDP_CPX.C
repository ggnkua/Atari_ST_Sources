/*
	...
	Source-Code fÅr Pure-C v1.1
	written by Th. Morus Walter
	v2.8 (12.12.1994), erste Version, Th. M. Walter
	v2.9 (04.01.1995), kleine Verbesserungen, Ch. Zwerschke
*/
...
typedef struct {
	...
	int active;		/* ab hier aus der Cookie-Struktur von LEDPANEL Åbernommen */
	long ledbits;
	int alldrvs;
	int delay;
	struct	{	int top; int right;	}	margin;
	struct	{	res1: 7; bgcol: 1; res2: 3; ledfrm: 1; res3: 3; ledcol: 1; } color;
	struct	{	lftspc: 1; caps: 1; cpspc: 1; rwsym: 1;	rwspc: 1; onlya: 1;
				hrdspc: 1; ramspc: 1; allspc: 1; frespc: 1; leds: 1; datspc: 1;
...
#pragma warn -rpt /* Zeigerumwandlungen vorÅbergehend erlauben */
#include "ledp_cpx.rsh"	/* Resource-daten, NACH conf laden!!! */
#pragma warn .rpt /* Zeigerumwandlungen wie immer behandeln */

LEDP *ledp;			/* Zeiger auf Ledpanel-Cookie-Struktur */

CPX_PARAMS *params;

GRECT *cpx_wind; /* Arbeitsbereich des XControl-Fensters */
OBJECT *dialog1,*dialog2,*wbox; /* ObjektbÑume aus der Resource */

/*
	init (!!! muû erste Funktion sein !!!)
*/
	...
	if ( par->booting ) {
		...
		*/
		if ( !(*par->find_cookie)(LEDP_MAGIC,(long*)&ledp) ||
			 ...
			 (ledp->header.version&0xff00)!=0x0200 ||
			 ledp->header.varlen!=LEDP_VARLEN ) {
			/*
					LED-Panel nicht da  oder
					falsche Version -> terminiere CPX-Modul
			*/
			CPX_BLOCK *blk;
			blk=(*par->get_rootblock)();
			do {
				if ( *(long*)blk->header.cpx_id=='LEDP' )
					blk->header.flags.set_only=1;
				blk=blk->next;
			} while ( blk!=0l );
			return 0l;
		}
...
/*
		Setze Buttons fÅr TIME und DATE
*/
...
int init_conf(CONF *conf)
{
	...
	if ( (ledp->header.version & 0xff00)!=0x0200 ||
		 ledp->header.varlen!=LEDP_VARLEN )
...
{
	int i;
	static const unsigned char caps[]="C^¯˘";
...
int cdecl do_cpx(GRECT *curr_wind)
{
	...
	cpx_wind=curr_wind; /* curr_wind global zur VerfÅgung stellen */
