...
TokenP copy_token(T1)
  register TokenP T1;
{
  register TokenP T2 = alloc_token();
...
void tok_shutdown()
{
#if 0				/* experimental -- let the OS clean up behind
				   us to save time */
  ...
  register TokenP T1, T2;
{
  ...
  if (*t != '\0') {
    warning("Invalid token \"%s\" created by concatenation", T->txt);
    T->type = UNKNOWN;
  }
  return T;
}
  ...
  TokenP T;
{
  ...
  fprintf(stderr, "[%s, '%c', \"%s\", \"%s\", %ld, %x]@%p",
	  ...
	  (T->pre_ws ? T->pre_ws : "(null)"), (T->txt ? T->txt : "(null)"),
