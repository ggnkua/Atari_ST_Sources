...
*
*  $Id: bezier.c 1.2 2001/03/16 22:00:00 tbb Exp $
*
*
...
*
*
*  this file implements the gem vdi bezier functions
*
****************************************/
...
{
const ACTIVE_WKS_REC *const pd = find_wkstnp( vdi_param_adr->contrl_adr->handle );
   vdi_param_adr->intout_adr[0] = pd->bez_qual;
...
{
ACTIVE_WKS_REC *const pd = find_wkstnp( vdi_param_adr->contrl_adr->handle );
...
* array[6] : anchor 2
*/
static void gen_segs( int16 *const array, int16 *px, const int bez_qual, int16 *const pmin, int16 *const pmax, int16 rc_flag )
{
...
*/
static void get_driver_vertices( const ACTIVE_WKS_REC * const pd )
{
...
*/
static void draw_segs( const ACTIVE_WKS_REC * const pd,
                       int16 nr_vertices,
                       const int16 *const xptsin, const int16 mode )
{
Vdi_Params xpb = { &xcontrl, xintin, xptsin, xintout, xptsout };
   ...
   while( nr_vertices >= 2 ) {
      ...
      call_driver( &xpb, pd );   /* output to driver, converting ndc if necessary */
...
uint16 *xptsin;
   ...
   if( pd->pw->id>=31 && pd->pw->id<=40 ) {
      call_driver( vdi_param_adr, pd );
      return;
   } /* if */

   bez_qual = pd->bez_qual;
   vertices_per_bez = 1<<bez_qual;
   xmin = ymin = 32767;
   xmax = ymax = 0;

   get_driver_vertices( pd );
   ...
   int flag = bezarr[i^1];
      ...
      if( IS_BEZ(flag) ) {  /** bezier start point found **/
         ...
         gen_segs( ptsin, xptsin, bez_qual, &xmin, &xmax, pd->rc_flag );	/* x coords */
         gen_segs( ptsin+1, xptsin+1, bez_qual, &ymin, &ymax, pd->rc_flag );	/* y coords */
         i += 3;	/* skip to coord pairs at end of bez curve */
         ptsin += 6;
         total_vertices += vertices_per_bez-3;
         draw_segs( pd, vertices_per_bez+1, xptsin, NO_FILL );
      }
      else {     /** polyline **/
      ...
      int16 *const ptsin0 = ptsin;
	    ...
	    register int old_flag = flag;
         ...
         draw_segs( pd, output_vertices, ptsin0, NO_FILL );
      } /* if */
   ...
   int flag = bezarr[i^1];
	 ...
	 if( i!=i0 ) {
      ...
      if( i>=nr_ptsin || IS_JUMP(flag) ) {
         draw_segs( pd, output_vertices, xptsin0, FILL );
         bez_qual = pd->bez_qual;
