...
*
*  $Id: ttf-defs.h 1.3 2001/02/24 23:56:47 tbb Exp $
*
...
***/
typedef struct {
   const WORKSTN_RECORD *pw;	/* pointer to workstation data, it mustn't change */
   ...
   uint16         dpix;
   uint16         dpiy;
   int16          font_id; /* id of currently selected font for this device */
   int16          bez_qual; /* current quality of bezier curves for this device */
   int16        (*entry_point)(void);	/* device driver entry point */
   struct basep  *base_pagep;
   FONT_CONTRL_RECORD wks_fonts;
} ACTIVE_WKS_REC;

#define ISOPEN(pd)  ((pd)->root_handle!=0)
#define ISEMPTY(pd) ((pd)->pw==NULL)
...
__inline__ static void xcall_driver( const Vdi_Params *vdi_param_adr, const ACTIVE_WKS_REC *const pd )
{
#ifndef MSDOS
   register Vdi_Params *vdi_in __asm__("d1");;
   vdi_param_adr->contrl_adr->handle = pd->drv_handle;
