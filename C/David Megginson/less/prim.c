	...
	int wantmatch;
{
	...
	} else
	{
		...
		/*
		 * Constrained to beginning of line?
		 */
		if( *pattern == '^' )
		{
			bol_flag = 1;
...
#else
#if REGEXP
		linematch = regexec(cpattern,line,bol_flag);
#else
...
#if REGEXP
/*
 * We need this function to report errors in the regular expression.
 */
