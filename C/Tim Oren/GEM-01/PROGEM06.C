06601030301800
1Tim Oren- Professional GEM -RASTER OPERATIONS
2Appendix A- PROGEM06.C -Page A#
F0110030
9[...............................................]001
€/*************************** MFDB Structure ***************************/

typedef struct fdbstr                 /* Memory Form Definition Block */
{
   ...
   int   fd_r2;                                   /* reserved for     */
   int   fd_r3;                                   /* future use       */
} MFDB;
...
vdi_trans(saddr, swb, daddr, dwb, h)   /* Transform the standard form */
   ...
   src.ff = TRUE;                            /* Set its std form flag */
...
trans_bitblk(obspec)                 /* Transform the image belonging */
   ...
   {                                 /* obspec.  This routine may     */
   LONG  taddr;                      /* also be used with free images */
   WORD  wb, hl;
...
/*-----------------*/
   VOID
trans_obj(tree, obj)              /* Examine the input object.  If it */
   LONG  tree;                    /* is an icon or image, transform   */
   WORD  obj;                     /* the associated raster forms in   */
   {                              /* place.  This routine may be used */
   WORD  type, wb, hl;            /* with map_tree() to transform an  */
   LONG  taddr, obspec;           /* an entire resource tree          */
   ...
   switch (type) {
      case G_IMAGE:
         ...
         return (TRUE);
      case G_ICON:
         hl = LWGET(IB_HICON(obspec));              /* Load icon size */
         wb = (LWGET(IB_WICON(obspec)) + 7) >> 3;
         if ((taddr = LLGET(IB_PDATA(obspec))) != -1L)
            vdi_trans(taddr, wb, taddr, wb, hl);    /* Transform data */
