...
map_tree(tree, this, last, routine)
   ...
   while (this != last && this != NIL)      /* Look until final node, */
      ...
      if ((tree + this)->ob_tail != tmp1)/* Did we 'pop' into this    */
         ...
         if (this == NIL)                  /* Subtree path not taken, */
            this = (tree + tmp1)->ob_next; /* so traverse right       */
         }
      else                                /* Revisiting parent, so */
...
beg_prog(rect)
   ...
   form_dial(0, 0, 0, 0, 0, rect->g_x, rect->g_y, rect->g_w, rect->g_h);
   objc_draw(tree, ROOT, MAX_DEPTH, rect->g_x, 
