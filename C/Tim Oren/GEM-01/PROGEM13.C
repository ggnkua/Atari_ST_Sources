06601030301800
1Tim Oren- Professional GEM -A NEW FORM MANAGER
2Appendix A- PROGEM13.C -Page A#
F0110030
9[...............................................]001
...
#define DOWN 0x5000
#define DEL 0x5300
                       /* Global variables used by 'mapped' functions */
...
€/*************** Utility routines for new forms manager ***************/
   ...
   VOID                          /* Reverse the SELECT state of an    */
objc_toggle(tree, obj)           /* object, and redraw it immediately */
   LONG  tree;
   WORD  obj;
   {
   WORD  state, newstate;
   ...
   objc_change(tree, obj, 0, root.g_x, root.g_y, root.g_w, root.g_h, 
               newstate, 1);
   }
...
objc_sel(tree, obj)                   /* SELECTED, make it so         */
   ...
   VOID                                      /* If the object is      */
objc_dsel(tree, obj)                         /* SELECTED, deselect it */
   LONG  tree;
   WORD  obj;
   {
   if (LWGET(OB_STATE(obj)) & SELECTED)
   ...
   VOID                                        /* Return the object's */
objc_xywh(tree, obj, p)                        /* GRECT through 'p'   */
   LONG  tree;
   WORD  obj;
   GRECT *p;
   {
...
map_tree(tree, this, last, routine)     /* object tree.  This routine */
   ...
   while (this != last && this != NIL)      /* Look until final node, */
                                            /* or off the end of tree */
      if (LWGET(OB_TAIL(this)) != tmp1)  /* Did we 'pop' into this    */
         {                               /* node for the second time? */
         tmp1 = this;                           /* This is a new node */
         this = NIL;
         if ((*routine)(tree, tmp1))     /* Apply operation, testing  */
            this = LWGET(OB_HEAD(tmp1)); /* for rejection of sub-tree */
         if (this == NIL)                  /* Subtree path not taken, */
            this = LWGET(OB_NEXT(tmp1));   /* so traverse right       */
         }
      else                                  /* Revisiting parent:  no */
   ...
   WORD                       /* Find the parent object by traversing */
get_parent(tree, obj)         /* right until we find nodes whose NEXT */
   LONG  tree;                /* and TAIL links point to each other   */
   WORD  obj;
   {
   WORD  pobj;
...
break_x(pxy)
   WORD  *pxy;
   if (br_mx < pxy[0])       /* Breaking object is right of mouse,    */
      {                      /* so reduce width of bounding rectangle */
      br_rect.g_w = pxy[0] - br_rect.g_x;
      return (TRUE);
      }
   if (br_mx > pxy[2])             /* Object to left, so reduce width */
      {                            /* and move rectangle to right     */
      br_rect.g_w += br_rect.g_x - pxy[2] - 1;
      br_rect.g_x = pxy[2] + 1;
      return (TRUE);
      }
   return (FALSE);                    /* Mouse within object segment, */
   }                                  /* so break attempt fails       */

   WORD
break_y(pxy)
   WORD  *pxy;
   {
   if (br_my < pxy[1])               /* Object below mouse, so reduce */
      {                              /* height of bounding rectangle  */
      br_rect.g_h = pxy[1] - br_rect.g_y;
      return (TRUE);
      }
   if (br_my > pxy[3])               /* Object above mouse, so reduce */
      {                              /* height and shift downward     */
      br_rect.g_h += br_rect.g_y - pxy[3] - 1;
      br_rect.g_y = pxy[3] + 1;
      return (TRUE); 
      }
                 /* Emergency escape test! Protection vs. turkeys who */
                 /* nest non-selectable objects inside of selectables */
   if (br_mx >= pxy[0] && br_mx <= pxy[1])      /* Will X break fail? */
      {                                         /* If so, punt!       */
      br_rect.g_x = br_mx;
      br_rect.g_y = br_my;
   ...
   WORD                    /* Called once per object to check if the  */
break_obj(tree, obj)       /* bounding rectangle needs to be modified */
   LONG  tree;
   WORD  obj;
   {
   GRECT s;
...
€
   flags = LWGET(OB_FLAGS(obj));               /* Is this object a    */
   if (flags & HIDETREE)                       /* potential hot-spot? */
      return (FALSE);
   if ( !(flags & SELECTABLE) )
      return (TRUE);
...
form_hot(tree, hot_obj, mx, my, rect, mode)
   ...
   objc_xywh(tree, ROOT, &root);             /* If there is already a */
   if (hot_obj != NIL)                       /* hot-spot, turn it off */
   ...
   state = LWGET(OB_STATE(hot_obj));    /* Is this object a hot-spot? */
   if (LWGET(OB_FLAGS(hot_obj)) & SELECTABLE)
   if (!(state & DISABLED))
      {                                    /* Yes!  Set up wait state */
      *mode = M1_EXIT;
      objc_xywh(tree, hot_obj, rect);
      if (state & SELECTED)                /* But only toggle if it's */
         return (NIL);                     /* not already SELECTED!   */
      else
         {
         objc_toggle(tree, hot_obj);
         return (hot_obj);
...
find_def(tree, obj)                 /* Check if the object is DEFAULT */
   ...
   WORD  obj;
   {
   if (HIDETREE & LWGET(OB_FLAGS(obj)))        /* Is sub-tree hidden? */
      return (FALSE);
   if (DEFAULT & LWGET(OB_FLAGS(obj)))          /* Must be DEFAULT... */
   if (!(DISABLED & LWGET(OB_STATE(obj))))      /* and not DISABLED   */
...
find_tab(tree, obj)               /* Look for target of TAB operation */
   ...
   WORD  obj;
   {
   if (HIDETREE & LWGET(OB_FLAGS(obj)))   /* Check for hidden subtree */
      return (FALSE);
   if (!(EDITABLE & LWGET(OB_FLAGS(obj))))
      return (TRUE);                   /* If not EDITABLE, who cares? */
   if (fn_dir && fn_prev == fn_last)   /* Check for forward tab match */
      fn_obj = obj;            
   if (!fn_dir && obj == fn_last)     /* Check for backward tab match */
      fn_obj = fn_prev;
...
form_keybd(tree, edit_obj, next_obj, kr, out_obj, okr)
   ...
   {
   if (LLOBT(kr))                        /* If lower byte valid, mask */
      kr &= 0xff;                        /* out extended code byte    */
   fn_dir = 0;                   /* Default tab direction is backward */
   switch (kr) {
      case CR:
         *okr = 0;                                   /* Zap character */
         fn_obj = NIL;                   /* Look for a DEFAULT object */
         map_tree(tree, ROOT, NIL, find_def);
         if (fn_obj != NIL)        /* If found, SELECT and force exit */
            {
      ...
      case DOWN:
         fn_dir = 1;                         /* Set forward direction */
      case BTAB:
      case UP:
         *okr = 0;                                   /* Zap character */
         fn_last = edit_obj;
€         fn_prev = fn_obj = NIL;
         map_tree(tree, ROOT, NIL, find_tab);  /* Look for TAB object */
...
form_button(tree, obj, clicks, next_obj, hot_obj)
   ...
   if (sble && !dsbld)                                  /* Hot stuff! */
      {
      if (flags & RBUTTON)                    /* Process radio        */
         do_radio(tree, obj);                 /* buttons immediately! */
      else if (!texit)
         {
...
€            *next_obj = 0;
   ...
   if (obj == *hot_obj)                    /* We're gonna do it! So   */
      *hot_obj = NIL;                      /* don't turn it off later */
...
form_do(tree, start_fld)
   ...
   next_obj = *start_fld;                          /* Initial editing */
   edit_obj = 0;

   hot_obj = NIL; hot_mode = M1_ENTER;        /* Initial hotspot cndx */
   objc_xywh(tree, ROOT, &hot_rect);
                  
   cont = TRUE;                                    /* Main event loop */
   while (cont)
      {
      ...
      if (edit_obj != next_obj)
         ...
         objc_edit(tree, edit_obj, 0, &idx, EDINIT);
                         ...
                         /* ...Wait for button or key or rectangle... */
      ...
      if (which & MU_M1)                    /* Handle rectangle event */
      ...
      if (next_obj == NIL)
         ...
         cont = form_button(tree, next_obj, br, &next_obj, &hot_obj);
      }
€                                               /* Handle end of field */
   ...
   if (hot_obj != (next_obj & 0x7fff))       /* If defaulted, may     */
   if (hot_obj != NIL)                       /* need to clear hotspot */
      objc_toggle(tree, hot_obj);               /* Return exit object */
   *start_fld = edit_obj;                       /* hi bit may be set  */
   return(next_obj);                            /* if exit object was */
   }                                            /* double-clicked     */
