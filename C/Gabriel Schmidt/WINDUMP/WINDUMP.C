...
#define MONO_ALERT "[2][Diese Graphik|ist monochrom.|" \
				"Welches Format soll|erzeugt werden?][XIMG|IMG]"

#define FILE_ALERT "[3][Datei konnte nicht|geschrieben werden!][Abbruch]"

#define MEMORY_ALERT "[3][Der Speicher ist voll!][Abbruch]"
...
void do_windowdump(void)
	...
	if ((bstate&2) == 0)
		...
		graf_mouse(M_OFF,NULL);
		error = bitmap_to_file(typ,wind_w,wind_h,VDI_PIXW,VDI_PIXH,
													 VDI_PLANES,VDI_RESC,filename,
													 get_vdi_color,get_vdi_pixel);
		if (error	!= 0)
			{ 
			switch(error)
				{
