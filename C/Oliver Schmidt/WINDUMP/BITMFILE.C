...
typedef enum {NONE, SOLID0, SOLID1, BITSTR} IMG_MODE;
                ...
                solid = 0x80 | (unsigned int)(&line_buf[cnt*line_len+byte]-startpnt);
					...
					case BITSTR:
						if (line_buf[cnt*line_len+byte] == 0x00 ||
								...
								&line_buf[cnt*line_len+byte]-startpnt == 0xFF ||
								byte == line_len)
								{
								bitstring.bs_cnt = (unsigned int)(&line_buf[cnt*line_len+byte]-startpnt);
								if ( write(file,&bitstring,sizeof(bitstring)) != sizeof(bitstring) ||
