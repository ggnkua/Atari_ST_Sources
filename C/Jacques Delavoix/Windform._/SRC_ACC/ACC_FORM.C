...
#include <vdi.h>
#include "windform.h"
#include "acc_rsc.h"	/* … remplacer par le nom du Header de votre Ressource. */
...
void main(void)
{
	...
	{
		if (_app == FALSE && test != 0)	/* Si Accessoire ET Inits r‚ussies : */
			menu_id = menu_register(app_id, rsrc_get_string(ACC_TITLE));
		if (_app != 0)	/* Si lancement en programme...	*/
		{
			graf_mouse(ARROW, 0);
...
int init_prog(void)
{
	int i, work_in[11], work_out[57]; /* Tableaux Entr‚es et Sorties VDI. */

	if (load_rsc("acc_rsc.rsc", NO_WINDOW) == 0) /*	Nom de votre Ressource.	*/
	{
		return FALSE;
	}

	vdi_handle = graf_handle(&i, &i, &i, &i);
	for (i = 0 ; i < 10 ; work_in[i++] = 1); /* Remplissage tableau d'Entr‚es. */
	...
	return TRUE;	/* Les initialisations ont r‚ussi.	*/
}
...
int utilisateur(int event)
{
	...
	if (event == OPEN_DIAL)
	{
		open_dialog(ptr_var, rsrc_get_string(WINDOW_TITLE), 0);
	}
	else
	{
