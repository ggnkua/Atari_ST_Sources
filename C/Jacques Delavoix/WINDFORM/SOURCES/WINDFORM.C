...
/* int user_count;	REM : pour USERDEFs en tableau		*/
/* USERBLK	userdefs[NOMBRE];	Idem					*/
...
void init_var(WINDFORM_VAR *ptr_var, int form_index, int inf_x, int inf_y)
{
	...
	if (inf_flag) {				/* ... alors recaler le formulaire sur la fenàtre. */
		...
		switch(buff[0])
			...
			case WM_REDRAW :
				...
				if (cur_objc) {			/* si champ Çditable et ....*/
					...
					if (buff[3] == wx) { /*...si fenàtre au 1¯ plan -> redessin total */
