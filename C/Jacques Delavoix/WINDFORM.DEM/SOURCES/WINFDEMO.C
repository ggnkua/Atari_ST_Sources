...
/*		if (buff[3] == 0)	Pr‚vu pour gŠrer le Clic sur un objet du Bureau.
			gere_bureau(event);
else	*/
...
void color_window(int event)
{
	...
	int choix, i, change = FALSE, mbuf[8];
	...
	if (event == OPEN_DIAL)	{	/* Partie "Ouverture : " */
		...
		if (choix != 0) {
			if (choix == COL_APPLI)
				change = TRUE;		/* Redessiner le bouton */

			if (choix > 0)
				objc_change(ptr_var->adr_form, choix, 0, ptr_var->w_x,
						ptr_var->w_y, ptr_var->w_w, ptr_var->w_h, 0, change);
...
void init_prog(void)
{
	int i, attributes[12], work_out[57];
	OBJECT *ptr_client;
	app_id = appl_init();
...
ne se touchent. */
	...
	for (i = 0 ; i < 9 ; _GemParBlk.intin[i++] = 1);
	_GemParBlk.intin[9] = 0;	/* Couleur de Remplissage = 0 */
	_GemParBlk.intin[10] = 2;	/* SystŠme de coordonn‚es RC */
	v_opnvwk(_GemParBlk.intin, &vdi_handle, work_out);
...
que la premiŠre ligne de texte ne touche le haut de la fenˆtre. */
	...
	if (rsrc_load("winfdemo.rsc") == 0) {
		form_alert(1,"[2][| Le Fichier Ressource |     winfdemo.rsc"
				"| n'a pu ˆtre trouv‚.][ Quitter ]");
		end_prog();
	}
	rsrc_gaddr(0, LE_MENU, &adr_menu);
	init_var(&infos_var, INFORME, 0, 0);
	init_var(&client_var, FICHE_CLIENT, 0, 0);
...
{
	char **alert;
	rsrc_gaddr(R_FRSTR, index, &alert);	/* Adresse pointeur sur pointeur sur chaine */
	form_alert(1, *alert);
}
