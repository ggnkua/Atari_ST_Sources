...
global
void Block(XA_CLIENT *client, short which)
{
	client_waiting_mask |=  (1L << client->kernel_end);
	DIAG((D.kern,client->pid,"[%d]Blocked %s(%d)\n", which, client->name, client->pid));
}
...
void Unblock(XA_CLIENT *client, unsigned long value, short which)
{
	...
	client_waiting_mask &= ~(1L << client->kernel_end);
	DIAG((D.kern,client->pid,"[%d]Unblocked %s(%d) 0x%lx\n", which, client->name, client->pid, value));
}
...
static
AES_function XA_debug  /*  XA_CLIENT *client, AESPB *pb  */
{
	short t = pb->contrl[0];
	
	switch(t)
	{
	case XA_RREGEN:		/* Regenerate rectangle list for a window */
		generate_rect_list(lock, pb->addrin[0]);
		break;
			
	case XA_WREDRAW:		/* Redraw window */
		display_non_topped_window(lock, 31, pb->addrin[0], nil);
		break;
	}
	return XAC_DONE;
}
...
void close_client(LOCK lock, XA_CLIENT *client)
{
	...
	DIAGS(("Free client: '%s'(%d)\n",client->proc_name, client->pid));
...
bool kernel_key(LOCK lock, KEY *key)
{
	...
	if ((key->raw.conin.state&(K_CTRL|K_ALT)) == (K_CTRL|K_ALT))
	{
		...
		DIAG((D.keybd,-1,"CTRL+ALT+%04x --> %d\n", key->aes, key->norm));

		key->norm = nkc_tconv(key->raw.bcon);

		switch(key->norm&0xff)
		{
		case NK_TAB:						/* TAB, switch menu bars */
			pid = next_app();
			DIAGS(("next_app() :: %d\n", pid));
			if (pid)
				app_in_front(lock, pid);
			return true;
		case 'R':						/* attempt to recover a hung system */
			recover();
			return true;
	#if FILESELECTOR
		case 'F':						/* test the file selector */
			open_fileselector(lock, C.AESpid, "u:" sbslash "*", "FS Tester",nil,nil);
			return true;
	#endif
		...
		case 'K':						/* tidy up after any clients that have died without calling appl_exit() */
			...
			return true;
		case 'S':						/* ctrl+alt+S, dump the current system status */
			status_dump();
			return true;
...
/*		Feedback: */
		...
		DIAG((D.kern,-1,"Fselect mask: 0x%08lx\n", input_channels));
	...
	/* Wait via Fselect() for keyboard, mouse and the AES command pipe(s). */
		...
		DIAG((D.kern,-1,">>Fselect(t%d) :: %d, channels: 0x%08lx, U%dM%d\n",
			...
			{												/* System alert? Alert and add it to the log */
				...
				if (n > 0)		/* HR MiNT's Fselect bug still there? */
				{
					...
					else
					{
						...
						do_form_alert(lock, 1, buf, C.AESpid);				/* Bring up an alert */
					}
				} else
				{
...
*/
					...
					if (n == sizeof(MOOSE_DATA))
					{
						...
						case MOOSE_MOVEMENT_PREFIX:				/* HR: new mouse rectangle events */
							DIAG((D.mouse,-1,"mouse move to: %d/%d\n", mdata.x,mdata.y));
							XA_move_event(lock, &mdata);				/* Call the mouse movement event handler */
							break;
						default:
							DIAG((D.mouse,-1,"Unknown mouse datatype 0x%x\n",mdata.ty));
							break;
						}
					othw
						DIAG((D.mouse,-1,"Moose channel yielded %ld\n", n));
					}
				othw
					MOUSE_DATA md;
					n = Fread(C.MOUSE_dev, 3, &md);
					if (n == 3)
					{
						DIAGS(("MOUSE n=%ld: x%x,%d,%d\n", n, (unsigned char)md.state, md.dx, md.dy));
						mdata.state = md.state&3;
						mdata.clicks = 1;
						if (md.state < 0 and (md.state&7)== 7)
						{
							mdata.x = nx + md.dx;
							mdata.y = ny - md.dy;
							if (md.dx or md.dy)
					...
					othw
						DIAG((D.mouse,-1,"Mouse channel yielded %ld\n", n));
					}
				}
			}
			...
			{												/* Did we get some keyboard input? */
				...
				DIAG((D.k,-1,"XA_keyboard_event: 0x%08lx\n",key.raw.bcon));
			...
			while(input_channels)
			{
				if (input_channels & 1L)
				{
					...
					if (r == sizeof(K_CMD_PACKET) and b.pb)					/* HR */
					{
						...
						short cmd = b.pb->contrl[0];
						repl = XAC_DONE;
#if GENERATE_DIAGS
						if (cmd >= 0 and cmd <= MAX_NAMED_DIAG)			/* HR >= 0 */
						{
							DIAG((D.trap,b.pid,">>CMD_PIPE: pid=%d, %s[%d]\n", b.pid, op_code_names[cmd], cmd));
						} else
							DIAG((D.trap,b.pid,">>CMD_PIPE: pid=%d, op-code=%d\n", b.pid, cmd));

						if (cmd == 107)			/* strange spurious wind_update	call. */
						{
							DIAG((D.trap,b.pid," **** wind_update @%lx\n", &Ktab[cmd].d));
							goto cont;
						}
#endif
						...
						if (client)						/* HR */
						{
							if (cmd >= 0 and cmd < KtableSize)
							{
								...
								{
									DIAG((D.trap,b.pid,"WARNING: cmd_pipe: pid:%d, Opcode %d not implemented\n", b.pid, cmd));
									rtn = XA_UNIMPLEMENTED;				/* Unimplemented functions :-( */
								}
							} else
							{
								DIAG((D.trap,b.pid,"WARNING: illegal AES opcode=%d\n", cmd));
							...
							else
							{
								...
								case XAC_M_TIMEOUT:
									...
									break;
								default:
#if GENERATE_DIAGS					/* must be XAC_DONE. */
									if (cmd == XA_EVNT_MULTI)
										evnt_diag_output(client->waiting_pb, client->pid, "kernel ");
#endif
									break;
								}
...
*/	Ktab[XA_GRAF_SLIDEBOX ].f = XA_graf_slidebox;
	...
	/* Debug calls (only generated by DUdebug) */
	
	Ktab[XA_RREGEN].f = XA_debug;
	Ktab[XA_WREDRAW].f = XA_debug;
}
...
global
void free_task(Tab *t,short *nester)
{
	if (t)
	{
		t->ty = NO_TASK;
