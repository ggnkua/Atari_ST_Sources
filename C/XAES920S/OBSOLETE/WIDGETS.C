...
static
XA_PENDING_WIDGET *
set_widget_active(XA_WINDOW *wind, XA_WIDGET *widg, WidgetBehaviour *wc, short i)
{
	XA_PENDING_WIDGET *pw;

	if (wind->widget_active)			/* HR: widget_active belongs to the window. */
		pw = wind->widget_active;
	else
	{
		pw = xmalloc(sizeof(XA_PENDING_WIDGET),7);
		if (!pw)
			return nil;
		wind->widget_active = pw;
		wind->owner->widget_active = pw;		/* This is for the event handler. */
	}
...
{
	if (wind->widget_active)
	{
		free(wind->widget_active);
		wind->widget_active = nil;
		wind->owner->widget_active = nil;
...
/* HR 260501: Some programs do this! */
	   ...
	   ) )
	{
		...
		disp = widg->behaviour[XACB_DISPLAY];
		while (rl)
...
void calc_work_area(XA_WINDOW *wi)
{
	...
	/*	hinside = wi->wa.h + wa_margins; */
		...
		if (wi->window_status != XAWS_ICONIFIED)
		{
			...
			if (right || ((k&SIZER) && !(right || bottom)))	/* sizer only retain right bar */
...
/* HR: tiny pixel correction (better spacing) */
	...
	if MONO
	{
		...
		if (wind == window_list && wind == C.focus)	/* HR */
	...
	} else
	{
		...
		XA_3D_pushbutton(-1,&r,nil,widg->state,0,0);
	}

	if (cfg.windowner && wind->owner && !wind->winob && !wind->dial)
	{
		char *ow = nil;
		if (cfg.windowner == 1)
...
WidgetBehaviour click_scroll	/* (LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg) */
{
	...
	if (!(   wind->widget_active
		  && slider
		  && slider->stuff
		  && ((XA_SLIDER_WIDGET *)slider->stuff)->position == widg->limit
		 )
	   )
	{
...
exactly that amount smaller than the standard. (The standard is 1 outline border) */
	...
	if (MONO)
		XA_3D_pushbutton(-2, &sl->r, nil, widg->state, 0, 3);
	else
	{
		sl->r.y += 1, sl->r.h -= 2;		
#if TOP_SLIDER
		if (wind == window_list)
			XA_3D_pushbutton(-3, &sl->r, nil, widg->state, -2, 3);
		else
#endif
			XA_3D_pushbutton(-1, &sl->r, nil, widg->state, 0, 3);
	} 
...
#if NEW_SLIDE
/* HR: Heavily cleaned up the code.
...
WidgetBehaviour drag_vslide	/* (LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg) */
{
	...
	RECT r;
	XA_PENDING_WIDGET *pw = wind->widget_active;
	...
	vq_mouse(C.vh, &mb, &pmx, &pmy);  /* need to do is anyhow, for mb */

	if (pw)		/* pending widget: take that */
	{
		pmy = pw->y;
		offs = pw->offs;
	}
	...
	*/	}
		wind->send_message(lock, wind, wind->pid, WM_VSLID, 0, wind->handle, noffs, 0, 0, 0);
		pw = set_widget_active(wind, widg, drag_vslide,2);
		pw->y = my;
		pw->offs = noffs;
		return false;	/* We return false here so the widget display status stays selected whilst it repeats */
	}
...
WidgetBehaviour drag_vslide	/* (LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg) */
{
	short      pmy, mx, my, mb, wcy, dy;
	short imx, imy, clip[4], offs, noffs, len, orig_offs, start_offs;
	RECT r;
	XA_SLIDER_WIDGET *sl = widg->stuff;
	XA_RECT_LIST *rl;
	XA_PENDING_WIDGET *pw = wind->widget_active;
	...
	len = sl->r.h;				/* these are already absolote */
	orig_offs = offs = sl->position;

	vq_mouse(C.vh, &mb, &imx, &imy);
	...
	{
		wcy = widg->click_y + r.y - sl->r.y; /* (((long)widg->loc.r.h - len) * sl->position) / 1000; */
		start_offs = orig_offs;
		if (wcy < 0)
		{
			if (offs > 0)
				wcy = -1;
	...
	{				/* Drag slider */
		...
		wind->send_message(lock, wind, wind->pid, WM_VSLID, 0, wind->handle, offs, 0, 0, 0);
		pw = set_widget_active(wind, widg, drag_vslide,4);
		pw->y = imy;
		pw->stoffs = start_offs;
		pw->offs = offs;		/* Not necessarily the same as the new sl->position */
		return false;	/* We return false here so the widget display status stays selected whilst it repeats */
	} else
	{
		if (wcy == -1)	/* Page up */
			wind->send_message(lock, wind, wind->pid, WM_ARROWED, 0, wind->handle, WA_UPPAGE, 0, 0, 0);
		else if (wcy == -2)	/* Page down */
			wind->send_message(lock, wind, wind->pid, WM_ARROWED, 0, wind->handle, WA_DNPAGE, 0, 0, 0);
		display_widget(lock, wind, widg);
		if (mb && !(wcy == -3))	/* If the button has been held down, set a pending/active widget for the client */
		{
			pw = set_widget_active(wind, widg, drag_vslide,5);
			pw->y = wcy;
			return false;	/* We return false here so the widget display status stays selected whilst it repeats */
		}
...
*/		}
		...
		wind->send_message(lock, wind, wind->pid, WM_HSLID, 0, wind->handle, noffs, 0, 0, 0);
		pw = set_widget_active(wind, widg, drag_hslide,3);
		pw->x = mx;
		pw->offs = noffs;
		return false;	/* We return false here so the widget display status stays selected whilst it repeats */
	}
...
WidgetBehaviour drag_hslide	/* (LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg) */
{
	...
	len = sl->r.w; /* ((long)widg->loc.r.w * sl->length) / 1000; */
	orig_offs = offs = sl->position;
	...
	{				/* Drag slider */
		...
		sl->position = orig_offs;
		wind->send_message(lock, wind, wind->pid, WM_HSLID, 0, wind->handle, offs, 0, 0, 0);
		pw = set_widget_active(wind, widg, drag_hslide,6);
		pw->x = imx;
		pw->stoffs = start_offs;
		pw->offs = offs;		/* Not necessarily the same as the new sl->position */
		return false;	/* We return false here so the widget display status stays selected whilst it repeats */
	...
	{
		if (wcx == -1)	/* Page left */
			wind->send_message(lock, wind, wind->pid, WM_ARROWED, 0, wind->handle, WA_LFPAGE, 0, 0, 0);
		else if (wcx == -2)	/* Page right */
			wind->send_message(lock, wind, wind->pid, WM_ARROWED, 0, wind->handle, WA_RTPAGE, 0, 0, 0);
		display_widget(lock, wind, widg);
		if (mb && !(wcx == -3))	/* If the button has been held down, set a pending/active widget for the client */
		{
			pw = set_widget_active(wind, widg, drag_hslide,7);
			pw->x = wcx;
			return false;	/* We return false here so the widget display status stays selected whilst it repeats */
		}
		cancel_widget_active(wind,6);
	}
...
#if 0
#include "obsolete/hslide_c.h"			/* a cancelled version */
#endif
#endif
...
WidgetBehaviour display_object_widget	/* (LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg) */
{
	...
	OBJECT *root = w_otree->tree;

	rp_2_ap(wind, widg, &root->r);	/* Convert relative coords and window location to absolute screen location */
	DIAG((D.form,wind->pid,"after rp_2_ap: %d/%d\n", root->r.x, root->r.y));

	DIAG((D.form,wind->pid,"display_object_widget(wind=%d), is_edit=%d, form: %d/%d\n", wind->handle,w_otree->edit_obj, root->r.x, root->r.y));
	/* HR: */
	draw_object_tree(lock, w_otree, root, widg->start ,100, is_menu(root));
...
#if ! NEW_SLIDE			/* then sliders have no click behaviour */
		widg->behaviour[XACB_CLICK] = drag_vslide;
#endif
...
#if ! NEW_SLIDE			/* then sliders have no click behaviour */
		widg->behaviour[XACB_CLICK] = drag_hslide;
#endif
...
/* HR 120201 */
void remove_toolbar_widget(LOCK lock, XA_WINDOW *wind)
{
	XA_WIDGET *widg = &wind->widgets[XAW_TOOLBAR];
	XA_TREE *wt = widg->stuff;
DIAG((D.form,-1," -- wt: %ld\n", wt));
...
#if NEW_SLIDE
		if (disp && !is_page(f))		/* !is_page() to avoid m_inside() */
#else
		if (disp)
#endif
		{
...
#if NEW_SLIDE
/* temporary code for page arrows (This has moved from drag_..slide) */
/* In this version page arrows are separate widgets,
	they overlap the slider widget, hence this kind of a hack.
	The real solution would be nested widgets and recursive handling. */
				...
				{
					widg = w->widgets + f +ax;
					(*widg->behaviour[XACB_DRAG])(lock, w, widg);		/* we know there is only 1 behaviour for these arrows */
/*					rtn = false;				/* we also know there is no real disp for page arrows */
*/				}
				else /* normal widget */
...
#endif
				{
					...
					if ((f != XAW_MENU) && (f != XAW_TOOLBAR))
					{
						...
						widg->state |= SELECTED;		/* Flag the widget as selected */
						if (w->winob)
						{
							hidem();
							(*disp)(lock, w, widg);		/* Not on top, but visible */
							showm();
						}
						else
							display_widget(lock, w, widg);	/* Display the selected widget */
					}
				...
				{
					XA_RECT_LIST *rl;
					widg->state &= ~SELECTED;			/* Flag the widget as de-selected */
			DIAG((D.mouse,-1,"widget deselect\n"));
					if (w->winob)
					{
						hidem();
						(*disp)(lock, w, widg);		/* Not on top, but visible */
