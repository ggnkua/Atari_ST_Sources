...
static char copyright[] =
	"\n"
	" \x1Bp                               \x1Bq\r\n"
	" \x1Bp         FalconScreen          \x1Bq\r\n"
	" \x1Bp                               \x1Bq\r\n"
	" \x1Bp   (c)1993 by Markus Gutshke   \x1Bq\r\n"
	" \x1Bp GEM patch by SHARP MAN of TBC \x1Bq\r\n"
	" \x1Bp                               \x1Bq\r\n\n"
...
long	setscreen(long rc,void *log,void *phys,int rez,int mode)
{
	if (rez == 3 && (mode & settings.mask) == settings.value)
		{
		init(!log && !phys);
		sharp(settings.width,settings.height);
		}
	return(rc);
}
			...
			if ((Kbshift(-1) & 3) && !Cconis()) {
				...
				if (!mode) {
					...
					if ((frq - table[settings.n].frq) > -20 &&
						...
						Cconws("\x1BY\x35 Current resolution: ");
						Cconws(s);
						Cconws("(+/-)"); } }
		Mfree(newscr);
