...
_install(name, type, sval, fval, h) char *name, *sval; double fval; SYMBOL *h[];{
  ...
  p = (SYMBOL *) emalloc((unsigned int) (sizeof(*p)));
  u = (CELL *) emalloc((unsigned int) (sizeof(*u)));
...
getvar(s, h, typ) char *s; SYMBOL *h[];
{
  ...
  if ((u = lookup(s, h)) == (CELL *)NULL) {
	if (prmflg) {
		...
		t = emalloc((unsigned int) (sizeof(SYMBOL *) * HASHSIZE));
...
*/
		u->c_type = typ;
		sfree(u->c_sval);
		u->c_sval = emalloc((unsigned int) (sizeof(SYMBOL *) * HASHSIZE));
		for (i = 0; i < HASHSIZE; i++)
