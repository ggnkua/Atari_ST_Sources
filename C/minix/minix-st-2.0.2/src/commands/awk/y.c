...
interactive()
{
  ...
  for (lex(); sym; lex()) {
	...
	if (p->n_type != PRINT && !iscntl(p->n_type)) {
		q = (NODE *) emalloc((unsigned int) (sizeof(NODE) + sizeof(NODE *) * 4));
		q->n_type = PRINT;
...
/*	q = expr10();*/
	...
	q = (NODE *) (op = 0);	/* stdout */
  printflg = 0;
  r = (NODE *) emalloc((unsigned int) (sizeof(*r) + sizeof(r) * (n + 3)));
...
doreturn()
{
  ...
  r = (NODE *) emalloc((unsigned int) (sizeof(*r) + sizeof (r) * (n + 1)));
  r->n_type = JUMP;
  r->n_next = NULL;
...
g1:
  ...
  case IDENT: case ARG:
	if ((c = Getc()) == '[') {	/* array */
		/* 940403 */
		if (sym == ARG) {
			u = (CELL *)emalloc((unsigned int) (sizeof(CELL)));
			u = mkcell(POS, NULL, (double)sym1);
			p = doarray(u);
		}
		else {
...
*/
  ...
  p = (NODE *) emalloc((unsigned int) (sizeof(*p) + sizeof(p) * (n + 2)));
  p->n_type = fun;
  p->n_next = NULL;
  p->n_arg[0] = (NODE *) op;
  p->n_arg[1] = (NODE *) n;
...
doarray(u) CELL *u;
{
  ...
  if (sym != ']')
	synerr("']' expected");
  /* left ']' for expr10() */
  p = (NODE *) emalloc((unsigned int) (sizeof(*p) + sizeof(p) * (n + 1)));
  p->n_type = ARRAY;
  p->n_next = NULL;
  p->n_arg[0] = (NODE *)u;
  p->n_arg[1] = (NODE *) n;
