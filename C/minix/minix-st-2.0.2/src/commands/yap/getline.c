...
# include <sys/types.h>	/* VS 20050604 */
# include <stdlib.h>	/* VS 20050604 */
# include <unistd.h>	/* VS 20050604 */
# include <errno.h>
# include "in_all.h"
# include "getline.h"
# include "options.h"
...
alloc(size, isblock) unsigned size; {
	...
	/*char *malloc(); VS 20050604*/
	/*long lseek();   VS 20050604*/
	register long i;
...
nextblock(pblock) register struct block *pblock; {
	...
	for (;;) {
		 ...
		 * Read loop
		 */
		cnt = read(stdf, c1, (int) (BLOCKSIZE - (c1 - pblock->b_info)));
		if (cnt < 0) {
			/*
			 * Interrupted read
			 */
			...
			if (poff == pblock->b_offs + siz) {
				...
				pblock->b_offs = (int *)
					re_alloc((char *) pblock->b_offs,
						 (unsigned) (siz+1) * sizeof(int),
						 (unsigned) (siz + 51) * sizeof(int));
