...
mq_t *m;
{
	...
	*q_tail_ptr= *q_head_ptr= m;
	...
	case DEV_IOCTL:
		...
		{
			DBLOCK(1, printf("replying EINVAL\n"));
			r= sr_put_userdata((int)(sr_fd-sr_fd_table), EINVAL, 
				NULL, 1);
			assert(r == OK);
			return OK;
		}
...
int fd;
size_t offset; /* vir_bytes */
size_t count;  /* vir_bytes */
int for_ioctl;
{
	sr_fd_t *loc_fd;
...
PRIVATE int cp_u2b (proc, src, var_acc_ptr, reqsize)
int proc;
char *src;
acc_t **var_acc_ptr;
int reqsize;
{
	static message mess;
	acc_t *acc;
	int i, size, totsize = 0;

	acc= bf_memreq(reqsize);
	...
	*var_acc_ptr= acc;
	...
	DIFBLOCK(0x80, (reqsize!=totsize),
		printf("cp_u2b gibt %d statt %d zurueck\n", totsize, reqsize));
	return OK;
}
