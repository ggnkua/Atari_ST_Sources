...
*/
		...
		if (seg_ack == tcp_conn->tc_SND_UNA)
		{
			...
			tc_SND_NXT))
		{
/*	VS		DBLOCK(0x40, printf(
			"tcp_Lmod4G && tcp_LEmod4G tc_SND_UNA=%lu, seg_ack=%lu, SND_NXT=%lu\n",
				tcp_conn->tc_SND_UNA,
...
/*			DBLOCK(0x80,printf("VS: NOT sending immediate ACK 1\n"));*/
			tcp_conn->tc_flags |= TCF_SEND_ACK;
			tcp_conn_write(tcp_conn, 1);
...
int data_len;
{
	...
	if ((tcp_hdr_flags & THF_FIN) && 
		...
		!(tcp_conn->tc_flags & TCF_FIN_RECV)) /* backport from 2.0.4 */
	{
		tcp_conn->tc_RCV_NXT++;
		tcp_conn->tc_flags |= TCF_FIN_RECV;
	}
...
int data_len;
{
	 ...
	 * retransmit.
	 */
	DBLOCK(0x80,printf("VS: NOT sending immediate ACK!!!\n"));
	tcp_conn->tc_flags |= TCF_SEND_ACK;
	tcp_conn_write(tcp_conn, 1);
