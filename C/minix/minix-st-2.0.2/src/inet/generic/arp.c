...
arp_port_t *arp_port;
{
	...
	case APS_INITIAL:
		arp_port->ap_eth_fd= eth_open(arp_port->ap_eth_port,
			(int)(arp_port-arp_port_table), arp_getdata, arp_putdata, (put_pkt_t)0);
	...
	while (arp_port->ap_flags & APF_MORE2WRITE)
	{
		if (arp_port->ap_flags & APF_CLIENTWRITE)
		{
			...
			clck_timer(&arp_port->ap_timer,
				...
				arp_timeout, (int)(arp_port-arp_port_table));
		}
		else
		{
			arp_cache_t *cache;
...
ether_addr_t *ethaddr;
{
	...
	(*arp_port->ap_arp_func)((int)(arp_port-arp_port_table), ipaddr, ethaddr);
}
