...
ip_hdr_t *pack_hdr;
{
		...
		/* it's now a complete packet */
	{
		...
		if ((ass_ent->ia_min_ttl) * HZ + first_time <
			get_time())
			icmp_snd_time_exceeded((int)(ip_port-ip_port_table),
				pack, ICMP_FRAG_REASSEM);
		else
			return pack;
	}
	return NULL;
...
ipaddr_t dst;
{
	register ip_ass_t *new_ass_ent, *tmp_ass_ent;
	int i;
...
assert (pack->acc_length >= IP_MIN_HDR_SIZE);
	...
	if (ip_hdr->ih_ttl < 2)
	{
		icmp_snd_time_exceeded((int)(ip_port-ip_port_table), pack, ICMP_TTL_EXC);
		return;
	}
	ip_hdr->ih_ttl--;
		...
		IPNT_CLASS_C)
	{
		...
		{
			icmp_snd_unreachable((int)(ip_port-ip_port_table),
				pack, ICMP_HOST_UNRCH);
		}
		return;
	}
	iroute= iroute_frag((int)(ip_port-ip_port_table), dest);
	if (iroute == NULL || iroute->irt_dist == IRTD_UNREACHABLE)
	{
		 ...
		 * trouble.
		 */
		icmp_snd_unreachable((int)(ip_port-ip_port_table), pack,
			ICMP_HOST_UNRCH);
		return;
	}
		...
		ip_port->ip_subnetmask) == 0)
	{
		 ...
		 */
		pack->acc_linkC++;
		icmp_snd_redirect((int)(ip_port-ip_port_table), pack,
