...
ip_port_t *ip_port;
{
	...
	ip_port->ip_dl.dl_eth.de_fd= eth_open(ip_port->
		ip_dl.dl_eth.de_port, (int)(ip_port-ip_port_table),
...
ip_port_t *ip_port;
{
	...
	case IES_EMPTY:
		...
		/* drops through */
	case IES_SETPROTO:
		result= arp_set_cb((int)(ip_port-ip_port_table),
...
ip_port_t *ip_port;
{
	arp_set_ipaddr ((int)(ip_port-ip_port_table), ip_port->ip_ipaddr);
...
int broadcast;
{
	...
	else
	{
		...
		r= arp_ip_eth((int)(ip_port-ip_port_table), dest, &eth_hdr->eh_dst);
		if (r == NW_SUSPEND)
		{
...
ether_addr_t *eth_addr;
{
	...
	while (ip_port->ip_dl.dl_eth.de_arp_head)
	{
		...
		xmit_hdr= (xmit_hdr_t *)ptr2acc_data(eth_pack);
		r= arp_ip_eth((int)(ip_port-ip_port_table), xmit_hdr->xh_ipaddr,
			&tmp_eth_addr);
		if (r == NW_SUSPEND)
			break;				/* Normal case */
