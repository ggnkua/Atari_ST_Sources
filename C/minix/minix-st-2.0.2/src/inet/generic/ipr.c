...
time_t timeout;
{
	...
	for (i= 0, route_ind= oroute_table; i<OROUTE_NR; i++, route_ind++)
	{
		...
		result= ipr_add_oroute(port_nr, route_ind->ort_dest, 
			...
			timeout, ORTD_UNREACHABLE, FALSE, (i32_t)0, NULL);
		assert(result == NW_OK);
	}
}
...
time_t timeout;
{
	...
	else
	{
		result= ipr_add_oroute(port_nr, dest, netmask, 
			oroute->ort_gateway, (time_t)HZ, ORTD_UNREACHABLE, 
			FALSE, (i32_t)0, NULL);
		assert(result == NW_OK);
	}
	result= ipr_add_oroute(port_nr, dest, netmask, new_gateway,
		timeout, 1, FALSE, (i32_t)0, NULL);
	assert(result == NW_OK);
}
...
time_t timeout;
{
	...
	result= ipr_add_oroute(port_nr, dest, netmask, oroute->ort_gateway, 
		timeout, new_dist, FALSE, (i32_t)0, NULL);
	assert(result == NW_OK);
}
