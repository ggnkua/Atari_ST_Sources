...
register tcp_port_t *tcp_port;
{
	register tcp_conn_t *tcp_conn;
	acc_t *pack2write;
	int r;
...
register tcp_conn_t *tcp_conn;
{
	...
	case TCS_CLOSING:
		...
		if (tcp_conn->tc_SND_UNA != tcp_conn->tc_SND_NXT)
		{
			...
			{
				DBLOCK(2,
					printf("conn[%d] no data: window is closed\n", (int)(tcp_conn-tcp_conn_table)));
				goto after_data;
			}
			...
			{
				DBLOCK(0x20,
				printf("conn[%d] no data: no data available\n", (int)(tcp_conn-tcp_conn_table)));
				goto after_data;
			}
			...
			{
				DBLOCK(0x20,
	printf("conn[%d] no data: partial packet (%lu <? %u)\n", (int)(tcp_conn-tcp_conn_table),seg_hi_data-seg_lo_data+tot_hdr_size, tcp_conn->tc_mss);
	printf("  seg_seq=%lu, tc_SND_UNA=%lu, seg_hi_data=%lu, seg_lo_data=%lu, tot_hdr_size=%u, tc_mss=%u\n",
		seg_seq, tcp_conn->tc_SND_UNA, seg_hi_data, seg_lo_data, tot_hdr_size, tcp_conn->tc_mss));
				seg_flags &= ~THF_FIN; /* backport 2.0.4 */
				goto after_data;
			}
			...
			{
				seg_flags |= THF_PSH;
/*				DBLOCK(0x40,printf("Setze THF_PSH fuer 0x%lu\n", tcp_conn->tc_SND_PSH));*/
			}
...
u16_t new_win;
{
		...
		tcp_Gmod4G(seg_ack, tcp_conn->tc_rt_seq))
	{
		...
		"tcp_release_retrans, conn[%d]: retrans_time= %ld ms\n",
			(int)(tcp_conn-tcp_conn_table), retrans_time*1000/HZ));
		...
		else if (retrans_time > rtt)
		{
			...
			if (tcp_conn->tc_rtt > TCP_RTT_MAX)
			{
				...
				{
					printf(
"tcp_release_retrans: warning retransmission time is limited to %ld ms\n",
						(u32_t)TCP_RTT_MAX*1000/HZ);
					warned= 1;
				}
#endif
...
"tcp_release_retrans, conn[%d]: (maybe too big) retrans_time= %ld ms, rtt= %ld ms\n",
	...
	DIFBLOCK(2, (tcp_conn->tc_snd_cwnd == tcp_conn->tc_SND_TRM || 
		     tcp_conn->tc_SND_NXT == tcp_conn->tc_SND_TRM),
		printf("conn[%d] not sending: zero window\n", (int)(tcp_conn-tcp_conn_table)));
...
struct timer *timer;
{
	...
	"tcp_send_timeout: conn[%d] setting timer to %ld ms (+%ld ms)\n",
		...
		clck_timer(&tcp_conn->tc_transmit_timer,
			...
			tcp_send_timeout, (int)(tcp_conn-tcp_conn_table));
		return;
	}
	...
	DBLOCK(0x20, printf(
	"tcp_send_timeout: conn[%d] setting timer to %ld ms (+%ld ms)\n",
		(int)(tcp_conn-tcp_conn_table), timeout*1000/HZ,
...
register tcp_conn_t *tcp_conn;
{
	...
	DBLOCK(0x80,
	printf("tcp_fd_write conn[%d]: tf_write_count=%d, max_trans=%d, write_count=%d\n",
		(int)(tcp_conn-tcp_conn_table),tcp_fd->tf_write_count,max_trans,write_count));
	if (write_count)
	{
		if (tcp_conn->tc_flags & TCF_BSD_URG)
		{
	...
	{
		DBLOCK(0x80,
		printf("tcp_fd_write conn[%d]: replying[%d]\n",
			(int)(tcp_conn-tcp_conn_table), tcp_fd->tf_write_offset));
