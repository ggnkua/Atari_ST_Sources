...
size_t data_len;
{
	register ip_port_t *ip_port;
	register ip_fd_t *ip_fd;
	register ip_hdr_t *ip_hdr, *tmp_hdr;
	...
	if (ip_fd->if_ipopt.nwio_flags & NWIO_HDR_O_SPEC)
	{
		...
		if (hdr_opt_len)
		{
			...
			memcpy ((void*)ptr2acc_data(tmp_pack),
			 ip_fd->if_ipopt.nwio_hdropt.iho_data, hdr_opt_len);
	...
	r= oroute_frag ((int)(ip_port - ip_port_table), dstaddr, ttl, &nexthop);
...
int first_size;
{
	...
	*ref_last= 0;
	...
	{
		icmp_snd_unreachable((int)(ip_port-ip_port_table), first_pack,
			ICMP_FRAGM_AND_DF);
		return NULL;
	}
