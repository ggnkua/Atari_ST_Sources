...
PUBLIC void tcp_init()
{
	...
	for (i=0, tcp_port= tcp_port_table; i<TCP_PORT_NR; i++, tcp_port++)
	{
		...
		result= sr_add_minor (tcp_port->tp_minor,
			(int)(tcp_port-tcp_port_table), tcp_open, tcp_close,
...
register tcp_port_t *tcp_port;
{
	...
	case TPS_EMPTY:
		...
		tcp_port->tp_ipfd= ip_open(tcp_port->tp_ipdev,
			(int)(tcp_port-tcp_port_table), tcp_get_data,
...
assert (data->acc_length == sizeof(nwio_tcpconf_t));
	...
	{
		newconf.nwtc_locport= find_unused_port((int)(tcp_fd-
			tcp_fd_table));
	}
...
int fd;
{
	register tcpport_t port, nw_port;
...
tcp_hdr_t *tcp_hdr;
{
	...
	register tcp_conn_t *best_conn, *listen_conn, *tcp_conn;
	tcp_fd_t *tcp_fd;
	int i;
	ipaddr_t locaddr;
	ipaddr_t remaddr;
...
register tcp_fd_t *tcp_fd;
{
	...
	tcp_conn->tc_rt_dead= TCP_DEF_RT_MAX_CONNECT; /* backport 2.0.4 */
...
register tcp_conn_t *tcp_conn;
{
	...
	{
		if (tcp_conn->tc_state == TCS_SYN_SENT) /* backport */
