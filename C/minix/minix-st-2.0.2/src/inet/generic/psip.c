...
acc_t *pack;
{
	register psip_port_t *psip_port;
	register psip_fd_t *psip_fd;
	psip_fd_t *mark_fd;
	...
	for(;;)
	{
		...
		if (buf_size <= psip_fd->pf_rd_count)
		{
			...
			if (result == NW_OK)
				result= buf_size;
			DBLOCK(0x80,printf("psip_send[%d] result=%d, buf_size=%d\n", port_nr, result, buf_size));
		}
		else
		{
			DBLOCK(0x20, printf(
	"psip_send, port_nr[%d]: packet greater than expected: %d > %d\n",
			port_nr, buf_size, psip_fd->pf_rd_count));
			result= EPACKSIZE;
		}
