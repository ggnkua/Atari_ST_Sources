...
PUBLIC void udp_init()
{
	...
	for (i= 0, udp_port= udp_port_table; i<UDP_PORT_NR; i++, udp_port++)
	{
		...
		result= sr_add_minor (udp_port->up_minor,
			(int)(udp_port-udp_port_table), udp_open, udp_close, udp_read,
...
udp_port_t *udp_port;
{
	...
	case UPS_EMPTY:
		...
		udp_port->up_ipfd= ip_open(udp_port->up_ipdev, 
			(int)(udp_port-udp_port_table), udp_get_data, udp_put_data,
			udp_ip_arrived);
...
assert (data->acc_length == sizeof(nwio_udpopt_t));
	...
	{
		newopt.nwuo_locport= find_unused_port((int)(udp_fd-udp_fd_table));
	}
...
udp_port_t *udp_port;
{
	...
	{
		udp_port->up_flags |= UPF_READ_IP;
		result= ip_read(udp_port->up_ipfd, (size_t) UDP_MAX_DATAGRAM);
		if (result == NW_SUSPEND)
		{
			udp_port->up_flags |= UPF_READ_SP;
			return;
