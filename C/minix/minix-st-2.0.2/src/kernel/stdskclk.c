...
int		minor; /* 0= bms100 1=bms200 */
{
  ...
  if (rw == DEV_READ && minor == DC_RBMS100) {
  ...
  } else if (rw == DEV_READ && minor == DC_RBMS200) {
	DMA->dma_mode = FDC | HDC;
...
/* lbuf format: bcd H L */
  ...
  } else if (rw == DEV_READ && (minor==DC_RSUPRA || minor==DC_RICD)) { /* supra or icd*/
	if(minor == DC_RSUPRA)
		l = getsupra();
	else
		l = geticd();
...
/* lbuf format: bcd H L */
  ...
  } else if (rw == DEV_WRITE && minor == DC_WBMS100) {
	...
	DMA->dma_mode = FDC ;
	r = 0; /* good return */
  } else if (rw == DEV_WRITE && minor == DC_WBMS200) {
	int l;
...
printf("disk clock fail; index: %d, lbuf: %x, r: %x\n", index, lbuf[index], r);
	...
	DMA->dma_mode = FDC ;
	r = 0; /* good return */
  } else if (rw == DEV_WRITE && minor == DC_RSUPRA) {
	r = EINVAL;
  } else {
