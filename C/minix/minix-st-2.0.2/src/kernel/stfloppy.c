...
#else
PRIVATE phys_bytes fl_dmabuf[MAX_TRACK_LEN/sizeof(phys_bytes)];
...
PRIVATE struct xfer {
  ...
  struct iorequest_s *x_iop;	/* iorequest of this call */
} xfer;
...
/* Prepare for I/O on a device. */
  ...
  if (f_device > MINOR_fd0a && ((f_device - MINOR_fd0a) >> DEV_TYPE_SHIFT) >= NR_PARTITIONS)
	return(NIL_DEV);
...
struct iorequest_s *iop;	/* pointer to read or write request */
{
  ...
  xp = &xfer;
  xp->x_rw = iop->io_request & ~OPTIONAL_IO;
  
  /* check iop values */
  if (open_count[f_drive].formatting && xp->x_rw == DEV_READ)
	return(iop->io_nbytes = EIO);
  if (!(f = gparam(f_device))) {printf("ENXIO 1\n"); return(iop->io_nbytes = ENXIO);}
  f_fp->fl_fparam = f;

  /* get and check # of bytes to transfer */
  nbytes = iop->io_nbytes;
  if ((nbytes % f->sector_size) != 0) return(iop->io_nbytes = EINVAL);

  /* get and check position */
  pos = iop->io_position;
  if ((pos % f->sector_size) != 0) return(iop->io_nbytes = EINVAL);
  
  /* calc block number, check for EOF */
  if (pos > f_dv->dv_size && ! open_count[f_drive].formatting)
	return(OK);	/* at EOF */
  xp->x_block = (int)((f_dv->dv_base + pos)/f->sector_size);

  /* get and check users buf address */
  xp->x_address = numap(proc_nr, (vir_bytes) iop->io_buf, (vir_bytes) nbytes);
  if (xp->x_address == 0) return(iop->io_nbytes = EINVAL);
  ...
  TRACE(printf("f_schedule:\nf_device=%x, rw=%d, nbytes=%d, pos=%ld, secnum=%d, proc=%d\n",
  	f_device, xp->x_rw, nbytes, pos, xp->x_block, fl_procnr));

  /* Because sectors are copied one by one, it's not worth setting
   * up transfers. Instead, any schedule is finished immediately
   */
  f_finish();
  return(OK);
}
...
PRIVATE int f_finish()
{
  ...
  if (xp->x_count == 0) return(OK); 	/* spurious finish */
  ...
  if (is_fast_ram(xp->x_address)) {
  	tp = xp->x_address;
  	xp->x_address = dmabuf;
  	if (xp->x_rw != DEV_READ)
  		phys_copy(tp, dmabuf, (phys_bytes) xp->x_count);
  }
  ...
  if (xp->x_address == dmabuf) {
  	if (xp->x_rw == DEV_READ)
		phys_copy(dmabuf, tp, (phys_bytes) xp->x_count);
  	xp->x_address = tp;
  }
  ...
  if (xp->x_errors >= MAX_ERRORS)
  	return(xp->x_iop->io_nbytes = EIO);
  if (xp->x_count == xp->x_iop->io_nbytes)
	return(xp->x_iop->io_nbytes = EIO);	/* nothing transferred */
  xp->x_iop->io_nbytes = xp->x_count;
  return(OK);
}
...
message *mp;
{
  ...
  if (f_prepare(mp->DEVICE) == NIL_DEV) {printf("ENXIO 2\n"); return (ENXIO);}

  dtype = DTYPE(f_device);
  if (f_device >= MINOR_fd0a) dtype = 0;
  if (dtype > NR_TYPES) {printf("ENXIO 3\n"); return(ENXIO);}
  if (open_count[f_drive].formatting)	return(EBUSY);	/* XXX */
  if (!(f = gparam(f_device))) {printf("ENXIO 4\n"); return(ENXIO);	/* XXX */}
  ...
  dmafree(FLOPPY);
  if (r == OK) {
  	f_fp->fl_geom.dv_size =
  		(long) f->nr_cylinders * f->sector_size * f->nr_sectors;
  	if (f->nr_sides == SIDES2) f_fp->fl_geom.dv_size *= 2;
	...
	printf("fl_geom=%lu/%lu\n",
			f_fp->fl_geom.dv_base, f_fp->fl_geom.dv_size);
...
/* Carry out a partition setting/getting request. */
  ...
  if ((dv = f_prepare(m_ptr->DEVICE)) == NIL_DEV) {printf("ENXIO 5\n"); return(ENXIO);}
