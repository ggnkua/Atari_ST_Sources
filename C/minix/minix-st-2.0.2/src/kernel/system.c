...
/* Handle sys_newmap().  Fetch the memory map from MM. */
  ...
  src_phys = umap(proc_addr(caller), D, (vir_bytes) map_ptr,
					(vir_bytes) sizeof(rp->p_map));
...
/* Handle sys_getmap().  Report the memory map to MM. */
  ...
  /* Copy the map to MM. */
  dst_phys = umap(proc_addr(caller), D, (vir_bytes) map_ptr,
					(vir_bytes) sizeof(rp->p_map));
  if (dst_phys == 0) panic("bad call to sys_getmap", NO_NUM);
  phys_copy(vir2phys(rp->p_map), dst_phys, (phys_bytes) sizeof(rp->p_map));
...
/* Handle sys_sendsig, POSIX-style signal */
  ...
  memcpy(&sc.sc_regs, &rp->p_reg, (int) sizeof(struct sigregs));
 ...
 */
  ...
  memcpy(&rp->p_reg, (char *)&sc.sc_regs, (int) sizeof(struct sigregs));
...
/* Handle sys_vcopy(). Copy multiple blocks of memory */
  ...
  src_phys= numap (m_ptr->m_source, vect_addr,
				(vir_bytes) (vect_s * sizeof(cpvec_t)));
