...
char **_penviron;		/* ATARI MINIX uses head.o for startup
				 * which does not define _penviron XXX
				 */

_PROTOTYPE( void tell,	(int fd, char *s)				);
_PROTOTYPE( void report,(int fd, char *label)				);
_PROTOTYPE( void wtmp,	(int type, int linenr, char *line, pid_t pid)	);
_PROTOTYPE( void startup,(int linenr, struct ttyent *ttyp)		);
_PROTOTYPE( int execute,(char **cmd)					);
_PROTOTYPE( void onhup,	(int sig)					);
_PROTOTYPE( void onterm,(int sig)					);
_PROTOTYPE( void onabrt,(int sig)					);
...
static char *path[] = { "/sbin", "/bin", "/usr/sbin", "/usr/bin" };
#ifdef __STDC__
