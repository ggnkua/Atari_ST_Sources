...
struct sigframe {               /* stack frame created for signalled process */
  _PROTOTYPE( void (*sf_retadr), (void) );      /* ___sigreturn function */
#ifdef __MLONG__                /* shared between Kernel and User */
  short sf_signo;               /* signal # */
  short sf_code;
#else
  int sf_signo;                 /* signal # */
  int sf_code;
#endif /* __MLONG__ */
  struct sigcontext *sf_scp;    /* sigcontext used by 2.0.0 programs */
#if (SIGFRAME_COMPAT_16 == 1)   /* 1.6.x programs expect sigcontext here */
  struct sigcontext *sf_scpcopy;
#endif /* SIGFRAME_COMPAT_16 */
#ifdef __MLONG__
  short sf_fp;
#else
  int sf_fp;
...
#if (CHIP == M68000)
#ifdef __MLONG__                /* shared between Kernel and User */
  short sc_flags;               /* sigstack state to restore */
#else
