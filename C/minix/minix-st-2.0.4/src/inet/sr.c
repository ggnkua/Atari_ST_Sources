...
DBLOCK(1, printf("cp_u2b(%d, %lx, %lx, %u)\n", proc, src, var_acc_ptr, size));
	acc= bf_memreq(size);
	*var_acc_ptr= acc;
	i=0;
	...
	while (acc)
	{
		...
		cpvec[i].cpv_size= (vir_bytes)size;
DBLOCK(1, printf("cpvec[%d]: src %lx -> dst %lx #%ld\n",	\
		i, cpvec[i].cpv_src, cpvec[i].cpv_dst, cpvec[i].cpv_size));
		src += size;
		acc= acc->acc_next;
		i++;
...
assert(m->mq_mess.m_type == REVIVE);
		if (m->mq_mess.m_source == SYSTASK && 
		    m->mq_mess.m_type == SYS_GETSP) {
			printf("Supressing SYS_GETSP to SYSTASK (1)\n");
		} else {
...
assert(m_cancel->mq_mess.m_type == REVIVE);
		if (m->mq_mess.m_source == SYSTASK && 
		    m->mq_mess.m_type == SYS_GETSP) {
			printf("Supressing SYS_GETSP to SYS_TASK (2)\n");
		} else {
