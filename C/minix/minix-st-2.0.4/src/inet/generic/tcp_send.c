...
tcp_conn_t *tcp_conn;
{
	...
	DBLOCK(0x20,
printf("max_seq=%lu, max_count=%d, max_trans=%d, UNA=%lu, wnd=%lu, NXT=%lu\n",
max_seq, max_count, max_trans, tcp_conn->tc_SND_UNA, tcp_conn->tc_snd_wnd, tcp_conn->tc_SND_NXT));
	if (write_count)
	{
		if (tcp_conn->tc_flags & TCF_BSD_URG)
		{
		...
		DBLOCK(0x20,
			printf("conn[%d] off=%d, count=%d, tf_count=%d\n",
		tcp_fd->tf_srfd, tcp_fd->tf_write_offset, write_count, tcp_fd->tf_write_count));
