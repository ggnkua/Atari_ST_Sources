...
#else
#if ENABLE_SWAP
  /* parent and child are shadowed, don't swap them out */
  rmp->mp_flags |= MM_DONT_SWAP;
  rmc->mp_flags |= MM_DONT_SWAP;
#endif /* ENABLE_SWAP */
#endif /* SHADOWING */
  rmc->mp_exitstatus = 0;
  rmc->mp_sigstatus = 0;
 ...
 */
  ...
  struct mproc *p_mp;
#if (SHADOWING == 1)
  phys_clicks base, size;		/* base and size used on 68000 only */
#endif /* SHADOWING */
  ...
  p_mp = mproc_addr(rmp->mp_parent);	/* process' parent */
 ...
 */
  ...
  struct mproc *parent = mproc_addr(child->mp_parent);
  int exitstatus;
