 ...
 */
  ...
  struct mproc *sh_mp;
  int m, r, fd, ft, sn;
#if (MACHINE == ATARI)		/* buf has to be aligned for relo calculation */
...
/* Read the header and extract the text, data, bss and total sizes from it. */
   ...
   * Beware the byte order in the 8 first bytes!
 ...
 */
  ...
  sys_fresh(who, rmp->mp_seg, (phys_clicks)(data_bytes >> CLICK_SHIFT),
				&base, &size);
#if ENABLE_SWAP
  /* no way back, parent and child may be swapped out now */
  rmp->mp_flags &= ~MM_DONT_SWAP;
  mproc[rmp->mp_parent].mp_flags &= ~MM_DONT_SWAP;
#endif /* ENABLE_SWAP */
