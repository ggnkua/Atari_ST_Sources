...
#if (MACHINE == ATARI) && (ENABLE_NETWORKING == 1)
#define RS_IBUFSIZE          8192       /* RS232 input buffer size */
#define RS_OBUFSIZE          8192       /* RS232 output buffer size */
#else
#define RS_IBUFSIZE          1024       /* RS232 input buffer size */
#define RS_OBUFSIZE          1024       /* RS232 output buffer size */
#endif /* MACHINE == ATARI && ENABLE_NETWORKING == 1 */
 ...
 * CLOCAL is in effect for a line without a CTS wire.
 */
#define devready(rs)         (((~MFP->mf_gpip & IO_SCTS) | \
                                (rs->cts & MS_CTS)) ? MS_CTS : 0)
...
#define PRINT_LEVEL	20	/* report after 20 errors */
  int print_count;
...
#endif
#if (MACHINE == ATARI && C_RS232_INT_HANDLERS) || (MACHINE == IBM_PC)
...
#endif /* (MACHINE == ATARI && C_RS232_INT_HANDLERS) || MACHINE == IBM_PC */
#if (MACHINE == ATARI && !C_RS232_INT_HANDLERS)
extern  _PROTOTYPE( void out_int, (rs232_t *rs)                         );
#else
FORWARD _PROTOTYPE( void out_int, (rs232_t *rs)                         );
#endif /* MACHINE == ATARI && !C_RS_232_INT_HANDLERS */
...
/* Generate a break condition by setting the BREAK bit for 0.4 sec. */
  rs232_t *rs = tp->tty_priv;
  int line_controls;
#if (MACHINE == IBM_PC) || defined(ALLDONE)
...
#if (MACHINE == ATARI && C_RS232_INT_HANDLERS == 1)
/*==========================================================================*
 *				siaint					    *
 *==========================================================================*/
...
#endif /* MACHINE == ATARI && C_RS232_INT_HANDLERS == 1 */

#if (MACHINE == IBM_PC) || (MACHINE == ATARI && C_RS232_INT_HANDLERS == 1)
/*==========================================================================*
 *				in_int					    *
 *==========================================================================*/
...
/* Check for and record errors. */
  ...
  if (--rs->print_count <= 0) {
	printf("rs232: lint_int, status=%x %s/%s/%s/%s\n", rs->lstatus,
		rs->lstatus & LS_FRAMING_ERR ? "frame_error" : "",
		rs->lstatus & LS_OVERRUN_ERR ? "overrun"     : "",
		rs->lstatus & LS_PARITY_ERR  ? "parity err"  : "",
		rs->lstatus & LS_BREAK_INTERRUPT ? "break"   : "");
	rs->print_count = PRINT_LEVEL;
  }
  MFP->mf_rsr &= R_ENA;
...
#endif /* MACHINE == IBM_PC || (MACHINE == ATARI && C_RS232_INT_HANDLERS == 1) */
