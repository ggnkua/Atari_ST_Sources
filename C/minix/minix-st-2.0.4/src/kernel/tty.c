...
tty_t *tty_tbl_ptr[NR_CONS + NR_RS_LINES + NR_PTYS];
/* Address of a tty structure. */
#define	tty_addr(line)	(tty_tbl_ptr[line])
#else
/* Address of a tty structure. */
...
#if OPTIMIZE_FOR_SPEED
#define END_TTY		&tty_table[sizeof(tty_table) / sizeof(tty_table[0])]
#else
...
/* Main routine of the terminal task. */
...
#if OPTIMIZE_FOR_SPEED
  for (line = 0; line < NR_CONS + NR_RS_LINES + NR_PTYS; line++)
	tty_tbl_ptr[line] = &tty_table[line];
#endif /* OPTIMIZE_FOR_SPEED */
  ...
  switch (atari_type) {
  case ST:   s = "ST"; break;
  case STE:  s = "STE"; break;
  case MSTE: s = "MSTE"; break;
  case TT:   s = "TT"; break;
  default:   s = "unknow Type";
  }
  printf("Atari Type = %s, compiled %s %s.\n", s, __DATE__, __TIME__);
#endif /* MACHINE == ATARI && ATARI_TYPE == DETECT_TYPE */
