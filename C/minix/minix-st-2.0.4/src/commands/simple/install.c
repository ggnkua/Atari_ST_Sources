 ...
 *								21 Feb 1993
 *				VS 01 Mar 2001: copy relocation information
 */
#define nil 0
...
/* Set the stack size in a header.  Return true if something changed. */
{
	...
	while (wordpow > 0) {
#if (MACHINE == ATARI)
		total *= hdr->a_cpu == A_I8086 || hdr->a_cpu == A_M68K ? 2 : 4;
#else
...
#if (MACHINE == ATARI)
				relo_off = hdr->a_hdrlen +
					hdr->a_text + hdr->a_data + hdr->a_syms;
				relo_size = sst.st_size - relo_off;
#endif /* MACHINE == ATARI */
				if (strip) {
					count= hdr->a_hdrlen
...
#if (MACHINE == ATARI)
		/* copy relocation info to output */
		if (relo_size != 0) {
			count = relo_size;
			lseek(sfd, relo_off, SEEK_SET);
			while (count > 0 && (n= read(sfd, buf, sizeof(buf))) > 0) {
				if (write(dfd, buf, n) < 0) {
					report(dest);
					close(sfd);
