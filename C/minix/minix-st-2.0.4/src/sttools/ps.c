...
struct pstat *bufp;
{
  ...
  if ((long)lseek(memfd, (off_t) iframe, 0) < 0)
	return NULL; 
...
#if ENABLE_SWAP
  else if (bufp->ps_mflags & ONSWAP)
	bufp->ps_args = "<swapped out>";
#endif /* ENABLE_SWAP */
...
{
#if (MACHINE == ATARI)
  if ((long)lseek(fd, addr, 0) < 0)
#else
  if ((long)lseek(fd, ((off_t) base << CLICK_SHIFT) + addr, 0) < 0)
#endif /* MACHINE == ATARI */
	return -1;
