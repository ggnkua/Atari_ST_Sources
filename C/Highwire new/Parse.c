...
*parse(char *symbol,short *minimum_width)
{
	...
	enum bool code_processed=false,space_found=false,pre=false;
	...
	short pre_minimum_width=0;
	short calculated_word_width=0;
	...
	*minimum_width=0;
	...
	while(*symbol !='\0')
	{
		switch(*symbol)
		{
			case '<':
				code_processed=true;
				current_word=add_word(current_word,active_word_buffer,active_word,&calculated_word_width,current_indent_distance);
				if(pre==true)
					pre_minimum_width+=calculated_word_width;
				else
					*minimum_width=max(*minimum_width,calculated_word_width);
				active_word=active_word_buffer;
				symbol++;
				switch(*symbol)
				{	case '\0':
					...
					case 'C':
						switch(*(symbol+1))
						{
							...
							case 'E':
								...
								current_paragraph=add_paragraph(current_paragraph,current_word,active_word_buffer,active_word,&calculated_word_width,current_indent_distance);
								*minimum_width=max(*minimum_width,calculated_word_width);
								current_word=current_paragraph->item;
								active_word=active_word_buffer;
					...
					case 'L':
						switch(*(symbol+1))
						{
							...
							case 'I':
								...
								current_word->changed.font=true;
								current_word=list_marker(current_word,current_list,&calculated_word_width,current_indent_distance);
								if(pre==true)
									pre_minimum_width+=calculated_word_width;
								else
									*minimum_width=max(*minimum_width,calculated_word_width);
							...
							case 'p':
								switch(*(symbol+1))
								{
									...
									case 'R':
										...
										current_word->word_code=br;
										pre=false;
										*minimum_width=max(*minimum_width,pre_minimum_width);
										pre_minimum_width=0;
										break;
								}
								break;
