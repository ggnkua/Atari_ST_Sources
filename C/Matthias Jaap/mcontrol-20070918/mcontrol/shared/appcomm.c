...
int16	av_server( void )
{
	...
	if( desk_id < 0 )
	{
		desk_id = appl_find("DIRECT  ");
	}
	if( desk_id < 0 )
	{
		desk_id = appl_find("TERADESK");
	}
	return desk_id;
}
...
void	av_startprog( uint8 *path, uint8 *cmd )
{
	...
	if( strnicmp( p, ".cpx", 4 ) == 0 ) {
		cpx_id = appl_find("COPS    ");
		if (cpx_id > 0)	{
			send_vastart(cpx_id, path);
			start = 1;
		}
	}
	if( desk_id > NIL && start==0)
		{
			memset(msgbuff, 0, (int16)sizeof(msgbuff));
...
boolean	open_hyp( uint8 *file, uint8 *arg )
{
	...
	else if(getenv("HELPVIEWER") != NULL)
		strcpy( stg_path, getenv("HELPVIEWER") );
