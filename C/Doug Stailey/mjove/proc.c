...
/*  Under MiNT, I found a problem with JOVE trying to write to the
    terminal while it was owned by the child process' process group.
    While I could have used Psetpgrp(), I decided to just go ahead and
    use an ioctl.  Now that I'm done, I wish I had used Psetpgrp()
*/
#ifdef MiNT
	if ((tfd = open("U:\\dev\\tty",0)) < 0) {
		complain("can't open tty");
		return;
	}
#define	TOSTOP 0x0100
	if (ioctl(tfd, TIOCGETP, &sg))
		complain("can't get ioctl");
	if (set = (sg.sg_flags & TOSTOP)) {
		sg.sg_flags &= ~(TOSTOP);
		if (ioctl(tfd, TIOCSETP, &sg))
			complain("can't set ioctl");
	}
#endif /* MiNT */
...
#else
	status = UnixToBuf(bnm, (char *)NULL, YES, 0, !is_an_arg(),
		Shell, ShFlags, command, (char *)NULL);
#endif /* MiNT */
