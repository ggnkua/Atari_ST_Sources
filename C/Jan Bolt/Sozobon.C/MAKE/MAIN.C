...
bool xarg = FALSE;			  /*  XARG arg passing */
...
char *argv[];
{
	   ...
	   fprintf(stderr, "\033v");	/* VT52 line wrap */
...
end_of_args:
	   ...
	   /* put macros PATH, SHELL to the environment */
	   strcpy(scratch,"PATH=");
	   if (*(p=getmacro("PATH")) != '\0')
		 strncat(scratch,p,256-5);
	   else
		 scratch[4] = '\0';   /* remove PATH */
	   if (!putenv(scratch))
		 fatal("memory allocation failure");
	   strcpy(scratch,"SHELL=");
	   if (*(p=getmacro("SHELL")) != '\0')
		 strncat(scratch,p,256-6);
	   else
		 scratch[5] = '\0';   /* remove SHELL */
	   if (!putenv(scratch))
		 fatal("memory allocation failure");
