...
static AESPB	pb	= 
{
	...
	intin,
	intout,
	(int *) addrin,														/* schwachsinniger Typecast... */
	(int *) addrout
};
...
extern WORD	VdiHandle;													/* Handle der VDI-Workstation fr Userdefs */
extern WORD	PwChar;														/* Zeichenbreite (8) */
extern WORD	PhChar;														/* Zeichenh”he (16) */
extern WORD AesFont, AesHeight;
	...
	*ver_3d = 0;
	...
	if ( magic )															/* MagiC vorhanden? */
	{
		if ( magic->aesvars )											/* MagiC-AES aktiv? */
		{
			...
			magic_date = (magic->aesvars)->date << 16L;				/* jjjj0000 */
			magic_date |= (magic->aesvars)->date >> 24L;				/* jjjj00tt */
			magic_date |= ((magic->aesvars)->date >> 8L) & 0xff00L;		/* jjjjmmtt */
...
static WORD	cdecl check_button( PARMBLK *parmblock )
{
	...
	*(GRECT *) clip = *(GRECT *) &parmblock->pb_xc;				/* Clipping-Rechteck... */
	clip[2] += clip[0] - 1;
	clip[3] += clip[1] - 1;
	vs_clip( VdiHandle, 1, clip );									/* Zeichenoperationen auf gegebenen Bereich beschr„nken */

	*(GRECT *) rect = *(GRECT *) &parmblock->pb_x;				/* Objekt-Rechteck... */
	rect[2] = rect[0] + PhChar - 2;
	rect[3] = rect[1] + PhChar - 2;

	vswr_mode( VdiHandle, 1 );										/* Ersetzend */

	vsl_color( VdiHandle, 1 );										/* schwarz */
	xy[0] = rect[0];
	xy[1] = rect[1];
	xy[2] = rect[2];
	xy[3] = rect[1];
	...
	xy[8] = rect[0];
	xy[9] = rect[1];
	v_pline( VdiHandle, 5, xy );										/* schwarzen Rahmen zeichnen */

	vsf_color( VdiHandle, 0 );										/* weiž */
	
	xy[0] = rect[0] + 1;
	xy[1] = rect[1] + 1;
	xy[2] = rect[2] - 1;
	xy[3] = rect[3] - 1;
	vr_recfl( VdiHandle, xy );										/* weiže Box zeichnen */

	if ( parmblock->pb_currstate & SELECTED )
	{
		parmblock->pb_currstate &= ~SELECTED;						/* Bit l”schen */
		
		vsl_color( VdiHandle, 1 );									/* schwarz - fr das Kreuz */
		xy[0] = rect[0] + 2;
		xy[1] = rect[1] + 2;
		xy[2] = rect[2] - 2;
		xy[3] = rect[3] - 2;
		v_pline( VdiHandle, 2, xy );
...
static WORD	cdecl radio_button( PARMBLK *parmblock )
{
	...
	*(GRECT *) clip = *(GRECT *) &parmblock->pb_xc;				/* Clipping-Rechteck... */
	...
	vrt_cpyfm( VdiHandle, MD_REPLACE, xy, &src, &des, image_colors );
	userdef_text( parmblock->pb_x + PhChar + PwChar, parmblock->pb_y, string );
...
static void	userdef_text( WORD x, WORD y, BYTE *string )
{
	...
	vswr_mode( VdiHandle, MD_TRANS );
	vst_font( VdiHandle, AesFont );								/* Font einstellen */
/* [GS] */
	vst_color( VdiHandle, 1 );										/* schwarz */
/* Ende; */
	vst_effects( VdiHandle, 0 );										/* keine Effekte */
	vst_alignment( VdiHandle, 0, 5, &tmp, &tmp );				/* an der Zeichenzellenoberkante ausrichten */
	vst_height( VdiHandle, AesHeight, &tmp, &tmp, &tmp, &tmp );
