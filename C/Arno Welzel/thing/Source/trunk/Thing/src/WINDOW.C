...
static void winUpdate(struct wininfo *win, int type)
{
	...
	if (anzahlEintraege > 0) /* Nur wenn Objekte vorhanden sind */
	{
		...
		if (n != imx) /* Spaltenzahl verÑndert ? */
		{
			if (type == WPATH)
	...
	if (tree)
	{
		...
		else /* Keine Objekte - Baum immer Fenstergrîûe */
		{
			w = win->work.w;
			h = win->work.h;
		}
...
void pwin_update(struct wininfo *win) {
	...
	if (!wpath->index.text && tree) /* Bei Icondarstellung */
	{
		if (wpath->e_total > 0) /* Nur wenn Objekte vorhanden sind */
		{
			...
			if (!n)
				n = 1;
			if (n != wpath->imx) /* Spaltenzahl verÑndert ? */
			{
				wpath_tree(win); /* Ja - dann neu aufbauen */
				w_draw(win);
				win_slide(win, S_INIT, 0, 0);
...
void gwin_update(struct wininfo *win) {
	...
	if (!wgrp->text && tree) /* Bei Icondarstellung */
	{
		if (wgrp->e_num > 0) /* Nur wenn Objekte vorhanden sind */
		{
			...
			if (!n)
				n = 1;
			if (n != wgrp->imx) /* Spaltenzahl verÑndert ? */
			{
				wgrp_tree(win); /* Ja - dann neu aufbauen */
				w_draw(win);
				win_slide(win, S_INIT, 0, 0);
...
void wf_draw(void) {
	...
	else /* Darstellung als Icons */
	{
		...
		else /* Text grîûer oder gleich groû wie Icon */
		{
			x += tx;
			w = tw;
			ix -= tx;
	...
	/* Koordinatenberechnung fÅr Icondarstellung */
	if (pnum == 2) {
		pxy[0] = x + tx;
		pxy[1] = y + ty;
		pxy[2] = x + ix;
...
void w_full(WININFO *win) {
	...
	if (!ax) /* Textmodus */
	{
		...
		else /* Sonst nîtige bzw. mîgliche Spaltenzahl ermitteln */
		{
			if (imx > 1) {
#if 1
