...
int	SetRS( WINDIA *wd, int baud, int flow, int flag )
{
	...
	if ( flow )
	{
		redraw |= SetButton( wd, SHAKEOFF, 0, flag );
		if ( flow & 1 )
	...
	{
		redraw |= SetButton( wd, SHAKEOFF, 1, flag );
...
static	int		SwitchObj( TOOL *tl, int obj )
{
	...
	switch ( obj )
	{
		...
		case PARIEVEN:	ucr.p_oddeven = 1;
						ucr.p_onoff = 1;
						break;
		case SHAKEOFF:	Select(tr+SHAKEOFF);
						RedrawWinObj( &tl->wd, SHAKEOFF, NULL );
						flow = 0;
						Deselect(tr+SHAKERTS);
						RedrawWinObj( &tl->wd, SHAKERTS, NULL );
						Deselect(tr+SHAKEXON);
						RedrawWinObj( &tl->wd, SHAKEXON,NULL );
						do
