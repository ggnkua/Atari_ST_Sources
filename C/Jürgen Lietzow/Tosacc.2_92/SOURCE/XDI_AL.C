...
static	int		Save( TOOL *tl, FILE *fp )
{
	if ( fp )
	{
		if ( fwrite( &xdSwitch, sizeof (int), 1L, fp ) == 1L &&
			 ...
			 fwrite( &xedSwitch, sizeof (int), 1L, fp ) == 1L &&
			 fwrite( &dpSwitch, sizeof (int), 1L, fp ) == 1L &&
...
static	int		HdlEvent( TOOL *tl, int evtype, EVENT *ev, int top )
{
	...
	if ( evtype & MU_BUTTON )
		obj = DialButton( &tl->wd, ev->ev_mmox, ev->ev_mmoy, ev->ev_mbreturn );
	else if ( evtype & MU_KEYBD )
		obj = DialKeybd( &tl->wd, ev->ev_mkreturn, ev->ev_mmokstate );

	switch ( obj & 0x7fff )
	{
		case	XEDOFF:		xedSwitch = 0;	break;
		case	XEDON:		xedSwitch = 1;	break;
		case	ALDEFPOS:	do
								graf_mkstate( &obj, &obj, &mstate, &obj );
							while ( mstate );
							if ( IsSelected(tl->wd.tree+ALDEFPOS) )
								dpSwitch = 1;
							else
								dpSwitch = 0;
							break;
...
TOOL	*XdaInit( FILE *fp, int handle )
{
	...
	if ( fp )
	{
		if ( fread( &xdSwitch, sizeof (int), 1L, fp ) == 1L &&
			 ...
			 fread( &xedSwitch, sizeof (int), 1L, fp ) == 1L &&
			 fread( &dpSwitch, sizeof (int), 1L, fp ) == 1L &&
