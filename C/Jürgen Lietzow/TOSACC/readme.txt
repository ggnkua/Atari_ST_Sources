...
FÅhren Sie nun einen Reset durch.
...
-------------
	...
	Fenster geîffnet wurde, lÑût sich auf der rechten Seite mit der Maus
	(oder mit <SHIFT hoch> und <SHIFT runter> zwischen den Tools wechseln.
	SÑmtliche Eingaben werden sofort umgesetzt, benîtigen also keine
	weitere BestÑtigung.
	>Sichern< speichert die Parameter aller Tools, die beim nÑchsten
	Booten geladen und gesetzt werden.
	Mit <CONTROL U> lÑût sich das Accessory-Fenster schlieûen.
...
a)	Erweiterte Dialogboxen
	...
	Die form_do()- und objc_edit()-Funktion mit zusÑtzlichen
	FÑhigkeiten. Aus allen Programmen heraus.
	...
	Das Zeichnen von Grow- und Shrinkboxen lÑût sich Åber alle
	...
	<ALT erster Groûbuchstabe> entspricht einem Klick auf den
	...
	Rechte Maustaste etwas lÑnger gedrÅckt halten entspricht
...
b) Erweiterte Alertboxen
	...
	Default-Button geÑndert werden (mit Klick auf <<Default>> oder 
	<ALT D> ), oder eine Alertbox wird fÅr immer unterdrÅckt
	(mit Klick auf <<Auto>> oder <ALT A>).
	
	<ALT A>		: ein/aus-schalten

	<Alt P>		: positioniert die Alertbox automatisch so, daû sich die
				  Maus Åber dem Default-Button befindet.
	
	<ALT L> (<<Lîsche Alerts>>) : Hier kînnen die Orginal-Alerts
...
c) Bildschirmschoner
	...
	<<Abschaltzeit>> :	Anzahl der Sekunden, die vergehen mÅssen
						bevor der Bildschirm auf <<Invers>>
						umschaltet.
						
d) Tastaturwiederholung

	Hier stellt man ein, wie schnell bei gedrÅcktgehaltener Taste
	das nÑchste Zeichen folgen soll.
	
e)	Tastaturansprechzeit

	Hier stellt man ein, wie lange eine Taste gedrÅckt gehalten werden
	muû, um mit den Tastaturwiederholungen zu beginnen.
...
f)	Maus Doppelklick
	...
	Mit (<<Doppelklick>>) lÑût sich der Doppelklick gleich testen.
...
g)	Dynamische Maus
	...
	<ALT B>		:	Beforzuge Richtung. Wenn die Differenz der zwei
					Bewegungskomponenten x und y sehr groû ist,
					wird die kleinere der beiden auf Null gesetzt.
					Besonders Hilfreich in Grafikprogrammen.
					
	Die Dynamik wird fÅr die X- und Y-Richtung getrennt eingegeben.
...
i)	Serielle Schnittstelle: konfigurieren und anzeigen
	...
	Baud:			stellt die öbertragungsgeschwindigkeit ein
...
k)	Residenter Speicher
	...
	Speichergrîûe	gibt die aktuell zur VerfÅgung stehende Speicher-
					grîûe an.
	Blockgrîûe		gibt die aktuelle Speicheraufteilung an.
					FÅr die RAM-Disk ist dies die Sektorgrîûe, und
					fÅr den Cache die Sektorgrîûe die maximal noch
					unterstÅtzt wird.
	
	öber die Slider kînnen neue Werte fÅr den nÑchsten Kaltstart
	eingegeben werden. Bei der Initialisierung (Kaltstart) wird geprÅft
	ob ein AHDI 3.0 kompatibler Treiber installiert ist. Wenn ja, wird
	dessen maximale Blockgîûe Åbernommen, es sei denn, die Åber TOSACC
	eingestellte Blockgrîûe ist noch grîûer.
	
	Installieren	Åbernimmt und speichert alle eingestellten Parameter,
					und fÅhrt einen	Kaltstart durch.
...
l)	RAM-Disk
	...
	<ALT I>			RAM-Disk mit aktuellen Parametern neu installieren.
					Dazu fÅhrt TOSACC nach Abfrage einen Reset durch.
	<<Laufwerk>>	Bei der Installation wird ab dieser Laufwerksnummer
					ein freier Eintrag gesucht.
	
	>Startprogramm<	Bei Klick auf diesen Button, lÑût sich ein Programm
					auswÑhlen, das direkt nach der Installation aufgerufen
					wird.
	>Parameter<		Hiermit lÑût sich dem Programm noch eine Kommandozeile
					Åbergeben.
...
m)	Drucker-Spooler
	...
	Weiterarbeiten am Rechner noch wÑhrend der Drucker beschÑftigt ist.
	...
	<ALT S>			Schnelle Cetronics Ansteuerung
					...
					Die schnelle Ansteuerung begÅnstigt auch Druckausgaben,
	...
	Systemzeit		Mit der Systemzeit bestimmt man die Rechnerauslastung
					wÑhrend dem Drucken. Ein globaler optimaler Wert
					lÑût sich nicht bestimmen, da dies wesentlich von der
					Geschwindigkeit des Druckers abhÑngt.
					
	Anmerkung:	WÑhrend des <<Abspoolens>> sollte auf die RAM-Disk nur
				lesend zugegriffen werden. Bedingt durch den temporÑren
				groûen Speicherbedarf des Spoolers meldet die RAM-Disk
				unter UmstÑnden einen >>Sektor-Schreibfehler.
...
n)	Disk-Cache
	...
	<ALT R>			Cache-Speicher-Reset lîscht alle im Cache befindlichen
					...
					recht nÅtzlich.
	Schreibverzîgerung
					Die Schreibverzîgerung ist die Zeit die zwischen zwei
					Laufwerkszugriffe verstreichen muû, um dann im
	...
	Laufwerk		FÅr jedes Laufwerk lassen sich nun weitere Optionen
					einstellen
					
	<ALT U>			Den Cache fÅr dieses Laufwerk ausschalten
	<ALT L>			Den Leses-Cache fÅr dieses Laufwerk einschalten
	<ALT S>			Den Lese- und Schreib-Cache fÅr dieses Laufwerk
					einschalten.
	<ALT B>			FÅr dieses Laufwerk einen Schreibschutz setzen.
	
					FÅr Wechselplattenbesitzer lassen sich diese Optionen
					auch fÅr noch nicht installierte Laufwerke
...
o)	Statistik
	...
	Das Statistik-Feld gibt Aufschluû Åber die EffektivitÑt des
	Disk-Cache und Åber die interne Speicherverteilung.
	...
	Rate gelesen/geschrieben
		...
		gibt das VerhÑltnis zwischen logischen und physikalischen
		Zugriffen an.
		
	Blîcke
	
		gibt die aktuelle Verteilung des Speichers an.
		Die Blockgrîûe ist unter k) beschrieben.
		
	>>Cache-Reset<< und >>Spooler-Reset<< (siehe [m und [n) setzen die
	Statistikdaten auf Null zurÅck.
...
TOSACC\AUTO
	...
	AUTOTACC.PRG	- Muû im AUTO-Ordner stehen
...
TOSACC\SOURCE\
	...
	TOSACC.DEF		|
	TOSACC.DFN		| - Info-Dateien fÅr verschiedene Resource-Editoren
	TOSACC.HRD		|
	
	TOSACC.PRJ		- Projektdatei von TOSACC.ACC (fÅr Turbo C bzw Pure C)
	
	TOSGLOB.H		- Header-Datei die alle Tools benîtigen

	ALLOCRES.C		- Header-Datei fÅr die residente Speicherallozierung

	TRANSFER.H		- Header-Datei, die die gemeinsamen Strukturen von
					  TOSACC.ACC und AUTOTACC.PRG beherrbergt

	TRANSFER.INC	- <<TRANSFER.H>> fÅr Assembler

	_TOSACC.S		- Assemblermodul fÅr die verschiedenen SystemsprÅnge
...
TOSACC\SOURCE\TGEMLIB\
	...
	TGEMLIB.H		- Header-Datei, die alles fÅr die GEM-Hilfsfunktionen
					  bereitstellt.
	
	HIGHGEM.C		- enthÑlt verschiedene Hilfsfunktionen fÅr GEM

	LOWGEM.C		- enthÑlt Low-Level Hilfsfunktionen fÅr GEM
	
	XGEM.*			- Resource-Datei fÅr die GEM-Bibliothek
...
TOSACC\SOURCE\AUTOTACC
	...
	AUTOSTAR.S		- EnthÑlt den residenten Assembler-Programm-Teil

	AUTOSTAR.O		- Objektdatei von AUTOSTAR.S

	AUTORES.C		- EnthÑlt den residenten C-Programm-Teil
	
	INSTALL.C		- Installiert den Residenten Teil

	ALLOCRES.C		- Alloziert den residenten Speicher

	RESALLOC.S		- enthÑlt das Master fÅr eine Speicherseite
