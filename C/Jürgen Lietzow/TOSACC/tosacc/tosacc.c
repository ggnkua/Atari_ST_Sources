...
*	CloseTool()
*
*	Dem Tool mitteilen, daû es nicht lÑnger aktiv ist
*/
...
static	int		OpenWin( void )
{
	...
	{
		form_alert( 1, "[3][Kann kein Fenster îffnen][ OK ]" );
...
static	int		HandleMessage( EVENT *ev )
{
	...
	if ( !taccSwitch )
	{
		if ( buf[4] == gemv.menuId  && buf[0] == AC_OPEN )
			form_alert( 1, "[1][TOS-Accessory ist auûer|"
						   "Dienst !][ OK ]" );
		return( 0 );
	}
	switch ( buf[0] )
...
*	egal ob das TOSACC-Fenster geîffnet oder geschlossen ist
*/
...
*	InitFirst()
*
*	ÅberprÅft, ob >>AUTOTACC<< installiert ist, und ob es
*	Åberhaupt Sinn macht, das Accessory anzumelden
*/
