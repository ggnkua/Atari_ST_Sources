		...
		case TIMER_ACCCLOSE	:
			...
			if ( mode )
			{
				if ( edObj != CTIME && edObj != CDATE )
				{
					...
					RedrawWinObj( &tl->wd, CDATE, winHdl );
					RedrawWinObj( &tl->wd, CTIME, winHdl );
				}
			}
