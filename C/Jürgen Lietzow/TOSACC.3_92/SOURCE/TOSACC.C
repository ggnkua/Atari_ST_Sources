...
SYS		*sys = NULL;
char	saveFile[18] = "A:\\" SAVE_FILE;	/* Name der Konfigurationsdatei */
TACC_INF taccInf = { VERSION, 0, 0, 32, "", "", 0 };
...
static	int		OpenWin( void )
{
	...
	wind_set( mainWD.window, WF_NAME, " TOS - ACCESSORY Special  v1.40 " );
...
void	EventLoop( void )
{
	...
	FormAlert( 1, "[2][hgbjlhsj|hjklhjklhjk][OK|nee]" );
#endif
...
void	Init( void )
{
	...
	if ( ( fp = fopen( saveFile, "rb" ) ) != NULL )
		if ( fread( &taccInf, sizeof (TACC_INF), 1L, fp ) != 1L
