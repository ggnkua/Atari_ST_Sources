...
WORD	drucke_datei( UBYTE *p, LONG weite, LONG max_zeile, LONG h_dpi, LONG max_spalte, LONG zeile )
{
	...
	if(  max_spalte<=1  )
	{
		...
		/* In die obere Ecke */
	Fwrite( th, 26, "\33*p0Y\33*p-300Y\33*p0X\33*p-300X" );
		/* Grafikdruck Anfang */
...
WORD	drucke( UBYTE *p, LONG weite, LONG max_zeile, LONG h_dpi, LONG v_dpi )
{
	...
	p += (2*max_spalte*i) + j;	/* Nicht druckbare RÑnder*/

	if(  weite<(WEITE+LINKS*8)*h_dpi/MAX_DPI  /* LINKS*8 nicht druckbar!*/  )
		max_spalte -= j;
	else
		max_spalte = (WEITE*h_dpi)/(8L*MAX_DPI);
	...
	if(  max_spalte<=1  )
	{
		...
		/* In die obere Ecke */
	print_block( 26, "\33*p0Y\33*p-300Y\33*p0X\33*p-300X" );
		/* Grafikdruck Anfang */
