...
WORD	drucke( UBYTE *p, LONG weite, LONG max_zeile, LONG h_dpi, LONG v_dpi )
{
	...
	if(  weite<WEITE*h_dpi/360L  ) /* Fr DINA3 vergr”žern! */
	...
	Fwrite( th, strlen(INIT), INIT ); /* Reset + Pic-Dichte + Zeilenabstand 1/216 */

	/* Endlich drucken */
	while(  zeile<max_zeile  )
 	{
 		/* Leerzeilen berspringen */
 		for(  ;  ist_leerzeile( p, max_spalte )  &&  zeile<max_zeile;  lz++, zeile++  )
 			p += weite;
		...
		h_len = 5;
		strcpy( tmp, grafik );
		tmp[3] = (UBYTE)((24*(rechts-links)+len) % 256);
		tmp[4] = (UBYTE)((24*(rechts-links)+len) / 256);
		if(  len>0  )
		...
		for(  lz=0;  lz<v_modus;  zeile++  )
		{
			...
			Fwrite( th, len+h_len+24*(rechts-links), tmp );
 			p += weite;
			zeile ++;
			if(  ++lz<v_modus  )
			{
