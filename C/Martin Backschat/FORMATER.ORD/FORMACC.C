...
main()
{
int       msgbuff[8],mx,my,ret,event;
int       ergebnis,menu_id;  
char      *resource = "format.rsc";

     appl_init();
     menu_id = menu_register(gl_apid,"  MaxiTrack v0.2 ");
...
while (TRUE) {
   ...
   while (TRUE) {
     ...
     objc_change(zeiger,index,0,x1,y1,w1,h1,NORMAL,0); /* QUIT deinvert */
     schliess();                       /* schliesst Formular */
     v_clsvwk(handle);                  /* und STATION wieder closen */

   }      /* end of if-- */

     wind_update(FALSE);      /* Multitasking wieder erlauben */
