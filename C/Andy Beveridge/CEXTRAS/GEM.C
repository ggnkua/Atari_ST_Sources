/*------------------------------------------------------*/ 
/* the GEM library					*/
/* contains both AES and VDI stuff, though not all	*/
/*------------------------------------------------------*/
/* version 101  22/6/88      				*/
/* corrections for v_gtext, v_justified,		*/
/* 		vst_alignment, evnt_multi		*/
/* additions: vqt_attributes, vst_font, vqt_name	*/
/*	      v_hide_c, v_show_c, vq_mouse,		*/
/*            evnt_mesag                                */
/* known errors: vsl_width				*/
/*------------------------------------------------------*/
...
vqt_name(handle, n, name) char *name; {
int *ip, i;
	ct0 = 130;
	ct3 = 1;
	ct6 = handle;
...
/* show cursor */
v_show_c(handle, reset) { vset(handle, 122, reset); }

/* hide cursor */
v_hide_c(handle) { vset(handle, 123, 0); }

/* test mouse button */
vq_mouse(handle, state, x, y) int *state, *x, *y; {
	ct0 = 124;
	ct6 = handle;
	vdi();
	*state = io0;
