/*----------------------------------------------------------------*
* a C library test set                                            *
*-----------------------------------------------------------------*/


#include        <stdio.h>
#include        <gem.h>      /* assorted useful GEM macro's    */
#include        <obdefs.h>
...
box_test()
{
        ...
        v_ellipse( handle, 479, 130, 150, 20 ); /* put filled ellipse */
...
/* now draw the spiral */
        ...
        for(revs=0;revs<6;revs++){
                for(ang=0;ang<3600;ang+=da){
                        v_arc(handle,225,160,radius,ang,ang+da);
                        radius++;
                }
        }
...
pie_test()
{
        ...
        for(seg=0;seg<nsegs;seg++){
                ...
                v_pieslice(handle,472,50,40,ang,ang+da);
        }
}
...
/*      s = vst_load_fonts(handle,0);
        ...
        v_justified( handle, 380, 170, "Hi There!", 180, 1, 1 );
