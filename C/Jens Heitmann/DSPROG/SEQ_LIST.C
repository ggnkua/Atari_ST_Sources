...
play_all = (long *)malloc(sizeof(long) * 2 * (lst_num + 1));
if (play_all)
	{
	for (i = 0, j = 0; i < lst_num; i++)
		{
		play_all[j++] = (long)(seq_data[play_list[i]].pos + o_bytes);
		play_all[j++] = seq_data[play_list[i]].len;
		}
	play_all[j++] = 0;
	play_all[j++] = 0;
...
if (new_pos != akt_pos)
	...
	for (i = 0; i < 6; i++)
		...
		if (new_pos + i >= max_pos)
			{
			strcpy(list_fm[idc[i]].ob_spec.free_string + 3 - flg, "\0");
			list_fm[idc[i]].ob_state &= ~CHECKED;
			}
		else
		  if (flg == 1)
		  	{
				strcpy(list_fm[idc[i]].ob_spec.free_string + 2, seq_data[new_pos + i].name);

				if (seq_data[new_pos + i].link)
					list_fm[idc[i]].ob_state |= CHECKED;
				else
					list_fm[idc[i]].ob_state &= ~CHECKED;
				}
			else
				strcpy(list_fm[idc[i]].ob_spec.free_string + 1, seq_data[play_list[new_pos + i]].name);
