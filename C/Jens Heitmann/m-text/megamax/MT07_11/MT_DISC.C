...
do
	...
	*(rindex(del_file, '\\') + 1) = 0;
	...
	while(s >= 0)
		{
		if (!(dta.attr & 0x10) && dta.file[0] != '.')
			{
			strcpy(kill_buf, del_file);
			strcpy(rindex(kill_buf, '\\') + 1, dta.file);

			strcpy(alert, DELASK_1);
			strcpy(alert + 5, dta.file);
...
make_rasc((long)trks, 10L, form_adr[TRK].ob_spec);
make_rasc((long)sect, 10L, form_adr[SEKTOR].ob_spec);
...
buf = DFREE;
make_rasc(free[0] * free[2] * free[3], 1000000L, buf + DFREE_BY);
form_alert(1, buf);
