...
/* | Zeile zurck suchen | */
/* ----------------------- */
unsigned char *go_back(line_adr)
	...
	if (j < buf_len)
		...
		switch( line_buffer[j] )
			...
			case 0xF:
				...
				if (zw)
					c_x[akt_id] += (strlen(txt_lineal[akt_id]) << 3) - zw;
...
if (line_gadr == txt_start[akt_id] + txt_lens[akt_id])
	if (!buf_len)
		...
		{
		move_mem(line_adr, buf_len - old_blen + 6);
	...
	{		
	move_mem(line_gadr, buf_len - old_blen + 1);
...
/* ---------------------------- */
insert_bytes(des, src, len)
register int des, len;
register char *src;
{
...
/* ------------------------------------- */
remove_one(kil_adr, len)
register unsigned char *kil_adr;
register long len;
{
