...
main()
{
  ...
  *pf=Dgetdrv()+65;     /*aktuelle Laufwerk*/
  ...
  wd_title(whandle[1],"TOY-PROLOGshell");/*Text fÅr Titelzeile ausgeben*/
   ...
   if (event & MU_BUTTON)
   {
      ...
      switch(obval)
      {
         case PROLOG:/* Es war der TOY-PROLOG-Knopf*/
         	...
         	xlopt=strcat(xlopt,"'");			/*der Umweg Åber */
         ...
         case OPTIONEN:	/*WÑhle Optionsfile*/
            ...
            strcpy(was," Prolog Hilfsfile wÑhlen ");	/*Hilfe der File Selector-Routine*/
            ...
            {
            	form_alert(1,"[3][*.TOY Files mÅssen mit|sysload(Name)|geladenwerden][Grummel]");
            }
            else
            {
         ...
         case PROFILE:		/*Prolog Workfile wÑhlen*/
            strcpy(d,lspfile);/*sonst wie bei Options*/
            strcpy(was," Prolog File wÑhlen ");
            strcpy(akt," OK ");
