...
/******
   Process the given line as a shell cmd:
   store it in history (if savehist != 0),
   expand it, and finally
   run it.

The globals strg, emsg, valu are initialized in docmd(),
and contain the results at the end.
The semicolon counting is there to break possible recursion in
