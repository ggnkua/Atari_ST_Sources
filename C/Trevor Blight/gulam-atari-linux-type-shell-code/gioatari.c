...
#ifdef Bconmap
   Bconmap((int)varnum(Mdmport));
#elif defined __GNUC__
   (long)trap_14_ww((short)0x2c,(short)(varnum(Mdmport)));
#else
   xbios(44,(int)varnum(Mdmport));	/* (was 43) Bconmap() */
#endif
}
...
/* TODO: this array should be a KEYTAB struct */
static
char	unsh[128*3] = {
/* UNSHIFTED */
...
{
#if 1
KEYTAB *k = (KEYTAB*)Keytbl((char*)-1, (char*)-1, (char*)-1);
int i;

   /*  need to patch table above for non-US keyboards:
       if entry in above table <= 0200 (=128 decimal) copy local definition there
   */
   for(i=0; i<128; i++) {
       if( unsh[i] <= 128 ) {
           unsh[i] = k->unshift[i];
   ...
   /* It should return the ptrs to old tables as follows:
      typedef struct
      {
         char *unshift;
         char *shift;
