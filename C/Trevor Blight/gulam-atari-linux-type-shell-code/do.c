...
/********
 Collect options from the users command.
 This is invoked only if the cmd is a builtin.
 All non-option words are given back to the lex module.
*/

static void addoptions(register uchar	*p)
{	register uchar	*q;
	register int	i;
...
/******
 ...
 if there are no redirs, p will == q all the time.
  ...
  we need to know which part of tyhe comamnd line a '+' or '-' appears so we can
