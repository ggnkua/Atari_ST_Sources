...
*
*  $Id: font.c 3277 2021-08-29 16:45:08Z trevor $
*
*
...
***********************************************************
*
* manage font loading & generation:-
*
...
*
* font data is kept in a font cache, and refreshed whenever it is used.
* Any font may drop out of the cache if it is unused for a while, so if used
* again it needs to be regenerated.
...
#define CH_FL_SZ (GEM_LAST_CH - GEM_FIRST_CH + 8)/8
...
*/
static ACTIVE_WKS_REC *find_partner( const ACTIVE_WKS_REC *const pw )
{
register ACTIVE_WKS_REC *scan     = active_wk_tbl;
register int16     const sav_root = pw->root_handle;	/* <<< use wsid ??? */
register int             n;
   ...
   assert( (n<=0) ||
               (scan->root_handle == pw->root_handle)
...
{
   if( find_partner( pw ) == NULL ) {
   register tGEM_FONT_HDR *fp = pw->wks_fonts.font_chain;
      while( fp != NULL ) {
      ...
      if(pw->wks_fonts.scratch_adr != NULL ) {
         (void)free(pw->wks_fonts.scratch_adr);        /* assume successful */
         pw->wks_fonts.scratch_adr = NULL;
      } /* if */
   } /* if */
...
*/
static void read_font_chain( ACTIVE_WKS_REC *const pw )
{
register tGEM_FONT_HDR *fp IPTR;
FILE_NAME *fdp = GemFont_Tablep + pw->pdev->first_font;
register int n, ttf_nr;

   assert( pw->wks_fonts.font_chain == NULL );

   /* first load gem fonts */
   for( n=pw->pdev->nr_gemfonts ; n>0; n-- ) { /** all gem fonts **/
      fp = load_gem_font( *fdp );
      if( fp != NULL ) {
         insert( &pw->wks_fonts.font_chain, fp );
      } /* if */
      fdp++;
   } /* for */

   if( (pw->pdev->id <= 30) || (pw->pdev->id > 40) )  {  /* don't load for meta file */
         ...
         if( fh > 0 ) {
            ...
            if( get_ttf_data( ttf_nr, font_template.font_data.gemFont.name, fh ) == OK
               ...
               for( q=fdp->psize_mask, p=7; q!=0; q<<=1, p++ ) {
                  ...
                  if( (q<0) && !exist_font( &pw->wks_fonts.font_chain, fdp->id, p ) ) {
                  tFONT *const fp0 = (tFONT*)hi_malloc( sizeof(tFONT) + sizeof(uint16)*(GEM_LAST_CH - GEM_FIRST_CH + 2) );
                     if( fp0 != NULL ) {
                        *fp0 = font_template;
                        ...
                        gen_font_header( ttf_nr, fp, pw->dpix, pw->dpiy );
                        assert( ttf_count >= 0 );
                        ttf_count++;
                        if( (uint32)fp->form_height*fp->form_width <= max_data_size-RESERVED ) {
                           insert( &pw->wks_fonts.font_chain, fp );
                           assert( exist_font( &pw->wks_fonts.font_chain, fp->font_id, fp->size ) );
                        }
                        else {
...
register ACTIVE_WKS_REC *const pw = find_wkstnp( vdi_param_adr->contrl_adr->handle );
   ...
   if( pw == NULL ) {
        DEBUGMSG( "\rbad handle" );
      goto error;
   } /* if */

   if( pw->wks_fonts.font_chain == NULL ) {
   register const ACTIVE_WKS_REC *const pp = find_partner( pw );
      /*
      ...
      */
      ...
      if( pp == NULL ) {
      register const uint16 size_bytes = ((pw->dpix>264?(pw->dpix>>1):132)*(pw->dpiy>>1)+3)&~3;
         /*** no partner found ***/
         read_font_chain( pw );
         if( pw->wks_fonts.font_chain == NULL ) {
              DEBUGMSG( "\rload_Fonts: empty font chain" );
            goto error;
         }
         pw->wks_fonts.scratch_size = size_bytes >> 1;
         assert( (pw->wks_fonts.scratch_size&1) == 0);
         pw->wks_fonts.scratch_adr = hi_malloc( size_bytes );
         if( pw->wks_fonts.scratch_adr == NULL ) {
            pw->wks_fonts.scratch_size = 0;
         } /* if */
      }
      else {
         /*** share font data with partner ***/
         pw->wks_fonts = pp->wks_fonts;
         assert( pw->pdev->id <= 10 );
         assert( pp->pdev->id <= 10 );
         assert( pp != pw );
         assert( pw->wks_fonts.font_chain != NULL );
      } /* if */
   }
   ...
   {
#if 1
   /* control[] is large enough, just use a pointer to it */
   register struct {
      struct contrl_io    contrl;
      FONT_CONTRL_RECORD  wks_fonts;
      } *const pcontrl = (void *)vdi_param_adr->contrl_adr;
      pcontrl->wks_fonts = pw->wks_fonts;   /* set up font data in contrl[] */
      call_driver( vdi_param_adr, pw ); /* this assigns nr fonts in intout[0] */
#else
   /** if contrl[] is not 12 words long, use this code */
   /* make a new struct wher it is big enough */
   struct {
      struct contrl_io    contrl;
      FONT_CONTRL_RECORD  wks_fonts;
   } const new_contrl = { *vdi_param_adr->contrl_adr, pw->wks_fonts };
   ...
   Vdi_Params xpb = { &xcontrl, xintin, xptsin, xintout, xptsout };
      ...
      pw->font_id = xpb.intout_adr[0];   /* initial font id selected by driver */
   }
...
register ACTIVE_WKS_REC * const pw = find_wkstnp( vdi_param_adr->contrl_adr->handle );
   ...
   if( pw != NULL ) {
      call_driver( vdi_param_adr, pw );
      pw->font_id = vdi_param_adr->intout_adr[0];   /* font id selected by driver */
   } /* if */
} /* vst_font() */
...
register const ACTIVE_WKS_REC * const pw = find_wkstnp( vdi_param_adr->contrl_adr->handle );
   ...
   register tGEM_FONT_HDR *fp = pw->wks_fonts.font_chain;
      ...
      */
      if( fp != NULL && pw->font_id >= 2 ) {
      Vdi_Params xpb = { &xcontrl, xintin, xptsin, xintout, xptsout };
      register const int16 font_id = pw->font_id;

         xcontrl.opcode = 131;  /* opcode for vqt_fontinfo */
         xcontrl.nptsin = 0;   /* entries in ptsin[] */
         xcontrl.nintin = 0;   /* entries in intin[] */
         xcall_driver( &xpb, pw );   /* inquire font height */
         ...
         if( fp != NULL && IS_TTF(fp) ) {
            ...
            find_mem( size, &(void*)fp->form_data ); /* from cache */
            ...
            xcall_driver( &xpb, pw );	/* set font height */
               ...
               if( fh >= 0 ) {
                  if( set_tables( ttf_nr, fh ) == OK ) {
                     ...
                     ** ttf tables.  Check, just to be sure */
                     if( fp->form_data != NULL ) {
                          gen_font_data( fh, ttf_nr, fp, pw, nr_gen, (uint16 *)xintin );
                     }
                     else {
                        /* failed!!, shouldn't happen as fonts exist only if data fits into cache */
#ifndef NDEBUG
