/****************************************************
*
*  $Id: tv1.c 3341 2021-10-25 15:12:54Z trevor $
*
...
const int16 sub = vdi_param_adr->contrl_adr[5];
   for( i=0; i<128; i++ ) {
     ...
     vdi_param_adr->intout_adr[i] = -1;
     vdi_param_adr->ptsout_adr[i] = -1;
   } /* for */
...
static void vs_clip( int16 handle, bool clipen, int16 x0, int16 y0,
                                                 int16 x1, int16 y1 )
{
   contrl[0] = 129;	/* opcode */
...
static int16 vst_load_fonts( int16 handle )
{
   ...
   if(contrl[2] != 0) {
      /* unspecified      */
      fprintf( fout, "\t%d: load fonts, pts out sb 0, %d returned\n", __LINE__, contrl[2]);
   } /* if */
   if(contrl[4] != 1) {
...
static void v_gtext( int16 handle, int16 x, int16 y, char *string )
{
register int i;
   contrl[0] = 8;   	/* v_gtext() */
   contrl[1] = 1;
...
static void v_justified( int16 handle, int16 x, int16 y, char *string, int16 span )
{
register int i;
...
int fi;
   fprintf( fout, "%d: starting tests, handle is %d\n", (int)__LINE__, handle );
   fprintf( fout, "%d: clipping\n", __LINE__);
   vs_clip( handle, 1, 0, 0, NDC_MAX, NDC_MAX );
   fprintf( fout, "%d: aligning\n", __LINE__);
   vst_alignment( handle, 0, 5); /* 0 = left justified; 5 = Top line */
...
uint32 memrc, memndc;
int16 handle_ndc, handle_rc, handle_meta, handle_21, handle_22;
...
uint32* p = (uint32 *)&((OSHEADER *)get_sysvar(_sysbase))->p_rsv2;
   ...
   if( *p == 0x45544f53 ) /* 'ETOS' */ {
      fprintf( fout, "using emuTOS" );
      if( p[2] == 0x4f535848 ) /* 'OSXH' */ {
         fprintf( fout, " version %s", p[5] + (char *)get_sysvar(_sysbase));
      }
    }
    else {
       fprintf( fout, "Atari TOS version %0x",
             ((OSHEADER *)get_sysvar(_sysbase))->os_version );
    } /* if */

   fprintf( fout, " gdos version: " );
   switch( vq_vgdos() ) {
   case -2:
...
******************/
   ...
   handle_meta = open_wkstn( 31, 0 );    /* meta file id, 0 = ndc */
   if( handle_meta != 0 ) {
      ...
      v_clswk( handle_meta );
      if( MEMFREE != memfree1 ) {
          fprintf( fout, "%d: >>>>>> after testing meta wkstn, free mem is %ld, expected %ld\n", (int)__LINE__, MEMFREE, memfree0 );
      }
      else {
          fprintf( fout, "%d: after testing meta wkstn, free mem is ok\n",
                   (int)__LINE__);
      } /* if */
   }
   else {
   ...
   handle_21 = open_wkstn( 21, 0 );    /* printer id, 0 = ndc */
   if( handle_21 != 0 ) {
      test_wkstn( handle_21 );
      fprintf( fout, "%d: now updating\n", (int)__LINE__ );
      v_updwk( handle_21 );
      fprintf( fout, "%d: now clearing\n", (int)__LINE__ );
      v_clrwk( handle_21 );
      vst_unload_fonts( handle_21 );

      fprintf( fout, "%d: testing printer 22\n", (int)__LINE__ );
      handle_22 = open_wkstn( 22, 0 );	  /* printer id, 0 = ndc */
      if( handle_22 != 0 ) {
         test_wkstn( handle_22 );
         fprintf( fout, "%d: now updating\n", (int)__LINE__ );
         v_updwk( handle_22 );
         fprintf( fout, "%d: now clearing\n", (int)__LINE__ );
         v_clrwk( handle_22 );	/* ejects page */
         fprintf( fout, "%d: now closing\n", (int)__LINE__ );
         v_clswk( handle_22 );	/* implicit update & unload */
      }
      else {
          fprintf( fout, "%d: couldn't open printer 22 wkstatn\n", (int)__LINE__ );
      } /* if */
      fprintf( fout, "%d: now closing printer 21\n", (int)__LINE__ );
      v_clswk( handle_21 );	   /* implicit update, fonts already unloaded */
   }
   else {
      fprintf( fout, "%d: couldn't open printer (21) wkstatn\n", (int)__LINE__ );
   } /* if */
   if( MEMFREE != memfree1 ) {
      fprintf( fout, "%d: >>>>>> after testing printer 21 & 22 wkstn, free mem is %ld, expected %ld\n", (int)__LINE__, MEMFREE, memfree0 );
   }
   else {
      fprintf( fout, "%d: after testing printer 21 & 22 wkstn, free mem is ok\n", (int)__LINE__);
   } /* if */
