/****************************************************************
*
* $Id: mot2int.c 3286 2021-09-12 13:06:44Z trevor $
   ...
   if (Fsfirst( file_name, 0) == 0) {
	 ...
	 int query_flag;
	    ...
	    /* swap header words and longs */
            if( convert_flag ) {
               long  size;
               long coff_base = hdr.coff_base;
               int nr_entries = hdr.last_ade - hdr.first_ade + 2;
		  ...
		  exit(-1);
	       ...
	       fp = (WORD*)malloc( size );
	       if (fp == NULL) {
                    printf( "not enough memory (%ld) to load %s\n", size, dtap->dta_name );
		  exit(-1);
	       } /* if */
	 
               lseek( fd, coff_base, 0 );
	       /* read file */
         ...
         printf( "%s is already intel format\n", dtap->dta_name );
	 } /* if */	   
