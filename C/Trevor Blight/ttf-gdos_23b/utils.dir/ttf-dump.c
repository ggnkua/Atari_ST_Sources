/*******************************************************
*
*  $Id: ttf-dump.c 3274 2021-08-28 13:12:54Z trevor $
*
...
char *err_str;
   ...
   *phdr = *pgf;        /* copy header data */
   ...
   if( fwrite( phdr, sizeof( *phdr), 1, fp ) != 1 ) {
      err_str = "[3][can't write font hdr|to gem font file][OK]";
      goto error2;
   } /* if */
   ...
   if( size_hor != 0 ) {
      ...
      if( fwrite( pgf->h_table, nr_entries, 1, fp ) != 1 ) {
         err_str = "[3][can't write hor offset tbl|to gem font file][OK]";
         goto error2;
      } /* if */
   } /* if */
   ...
   if( fwrite( phdr+1, size_wid, 1, fp ) != 1 ) {
      err_str = "[3][can't write char offset tbl|to gem font file][OK]";
      goto error2;
   } /* if */
   ...
   if( !fWidOnly ) {
      ...
      if( fwrite( pgf->dat_table, pgf->form_width, pgf->form_height, fp )
         ...
         err_str = "[3][can't write font data|to gem font file][OK]";
         goto error2;
      } /* if */
   } /* if */
   fclose( fp );
