...
* usage:
*        show-all <dev>
* where <dev> is device number, screen if not entered
*
***************************************/
#include <stdio.h>
#include <string.h>
...
void pr_line( int justify )
{
   ...
   if( y > page_height-YMARGIN ) {
      ...
      v_updwk( handle );
      if( wkstn<=10 ) {
         Cconws( "\a press a key" );  /* beep */
         Cnecin();  /* wait for keypress before proceeding **/
      } /* if */
      v_clrwk( handle );
      y = YMARGIN+h;
      Cconws(status);
...
int psize;
   ...
   sprintf( status, "\33lfont[%d]: %s, id is %d", font_index, name, id );
   size_pos = status + strlen(status);
...
long mem0 = Malloc(-1);
   ...
   wkstn = argc < 2? Getrez()+2: atoi( argv[1]);
   ...
   printf( "there are %d new fonts loaded\n", nr_fonts );
   ...
   **  (for screen) **/
   ...
   Cconws( "\33lfinished, press a key" );
   (void)Cnecin();
   Cconout( '\n' );
   exit( 0 );
