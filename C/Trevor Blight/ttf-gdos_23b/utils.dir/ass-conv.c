...
/* TODO:
   - generate warning if cache size <= 100k
   - check file is writable, eg not a:\ set to readonly
 */
...
TTF_GDOS_INF_RECORD head;
   ...
   *title = '\0';
   ...
   /** now scan for devices, fonts and special info ... **/
   /** device definition lines start with a digit (eg 03p screen.sys) **/
            ...
            *t_tail = pTfnt;
            ...
            if( q != NULL
              ...
              && tolower(q[4])=='e' ) {
               ...
               if( head.cache_size < 100 ) {
                  quit("cache size must be at least 100kB");
               }
            } /* if */
         } /* if */
      } /* if */
   ...
   int   pi = 7;
      ...
      {
         fprintf( stderr, "error writing true type font to %s\n", pfnout );
         exit(1);
      } /* if */
   ...
   for( pGfnt = g_head; pGfnt!=NULL; pGfnt=pGfnt->f_next) {
      ...
      {
         fprintf( stderr, "error writing gem font to %s\n", pfnout );
         exit(1);
      } /* if */
      printf( "font is %s\n", pGfnt->f_name );
   } /* for */
   ...
   {
       fprintf( stderr, "error writing path to %s\n", pfnout );
       exit(1);
   } /* if */
   if( fwrite( title, strlen(title)+1, 1, fout ) < 1 )
   {
      fprintf( stderr, "error writing title to %s\n", pfnout );
      exit(1);
  } /* if */
