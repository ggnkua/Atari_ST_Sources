 ...
 *
 * $Id: ttf2gem.c 3298 2021-09-17 13:00:14Z trevor $
 * 
...
char *s;
   ...
   for( gem_char = gem_font.first_ade; gem_char <= gem_font.last_ade; gem_char++ ) {
      ...
      /** If the glyph could not be loaded for some reason, just continue with next glyph **/
   ...
   if( s!=NULL && s[-1] == '?') {
        ...
        s[-1] = '0' + pFont->size / 10;   /* tens digit */
        s[0] = '0' + pFont->size % 10;   /* units digit */
   } /* if */
