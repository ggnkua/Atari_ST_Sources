/***** (C) 1996-2021, Trevor Blight *****
*
*  $Id: ttf-gdos.c 3274 2021-08-28 13:12:54Z trevor $
*
*
...
*   1. device_table, the decice definition table
*   2. active_wk_tbl, the active workstation table
*
*  device_table is essentially ttf-gdos.inf and defines device
*  specific data, like driver, device id number, etc.
*
*  An entry is made in active_wk_tbl whenever a workstation is opened, and
...
{
register const DEVICE_RECORD *pd = setup_inf.device_table;
register ACTIVE_WKS_REC *pw = active_wk_tbl;
int     iw;

   for( iw=setup_inf.nr_devices; iw>0 && pw<active_wk_tbl+MAX_WKS; iw-- ) {
      if( pd->load_type != DYNAMIC ) {
         pw->pdev = pd;
         if( pd->load_type == PERMANENT ) {
            pw->base_pagep = NULL;
            pw->entry_point = rom_entry;
         }
         else {  /* RESIDENT */
            switch( load_driver( pd->driver_name, pw ) ) {
            case MEM_ERR:
...
int na = 0;	/* nr cookies available */
   ...
   register tCOOKIE *const a = (tCOOKIE *)(((uint32)end_mem + 3) & ~3L);
      na += 8;
      *(tCOOKIE **)_p_cookies = p = a; /* new cookie area */
      end_mem = a + na;
      while( --nc >= 0 ) *p++ = *p0++;
   } /* if */
...
bad_file:
   ...
   if( nr_bytes != setup_inf.file_len )
      goto bad_file;

   TTFont_Tablep = (FONT_RECORD *)(setup_inf.device_table + setup_inf.nr_devices);
...
/********************************************************
*
*  call driver associated with device record at pw
...
static void call_driver( const Vdi_Params * const vdi_param_adr,
             register const ACTIVE_WKS_REC *const pw )
{
int16 *const contrl6_ptr = &vdi_param_adr->contrl_adr->handle;
int16 sav_handle = *contrl6_ptr;
int16 yscale;
register uint16  n = vdi_param_adr->contrl_adr->nptsin;

   *contrl6_ptr = pw->drv_handle;

   /* don't want to malloc a new ptsin[] & convert if ptsin is empty */
   if( pw->rc_flag == ND_COORDS && n > 0) {
   Vdi_Params new_vdi_par = *vdi_param_adr;
   register int16 *p_rc;

      /*** nd coords, so create a new ptsin array with raster coords ***/

      assert(vdi_param_adr->ptsout_adr == new_vdi_par.ptsout_adr);
      assert(vdi_param_adr->contrl_adr == new_vdi_par.contrl_adr);
      new_vdi_par.ptsin_adr = p_rc = malloc(4*n);  /* a new ptsin array, 2*n int16s */
      if( p_rc == NULL ) {
           DEBUGMSG( "\rout of memory for ptsin array" );
           return;
      }
      else {
      register const int16 *p_ndc = vdi_param_adr->ptsin_adr;
      register const int16  h1 = pw->dev_height-1;
      register const int16  h2 = 2*pw->dev_height;
      register const int16  w2 = 2*pw->dev_width;
         for( ; n>0; n-- ) {
           assert( *p_ndc<0 || scale_siz(*p_ndc, w2) == (((int32)(*p_ndc)+1)*pw->dev_width-1)/32768L );
           assert( abs(scale_siz(*p_ndc, w2) - (((int32)(*p_ndc)+1)*pw->dev_width-1)/32768L) <= 1 );
           *p_rc++ = scale_siz( *p_ndc++, w2 );    /* the x coord */
           assert( *p_ndc<0 || scale_siz(*p_ndc, h2) == (((int32)(*p_ndc)+1)*pw->dev_height-1)/32768L );
           assert( abs(scale_siz(*p_ndc, h2) - (((int32)(*p_ndc)+1)*pw->dev_height-1)/32768L) <= 1 );
         ...
         register Vdi_Params *vdi_in __asm__("d1");;
           ...
           yscale = (*pw->entry_point)(); /* call_driver() */
         }
   ...
   if( pw->rc_flag == ND_COORDS ) {
      scale_to_ndc( vdi_param_adr, pw, yscale );
   } /* if */
...
__inline__ static int isopen(ACTIVE_WKS_REC *pw)
{
     return pw->root_handle != 0;
} /* isopen() */

__inline__ static int isempty(ACTIVE_WKS_REC *apw)
{
     return apw->pdev == NULL;
} /* isempty() */
...
{
register ACTIVE_WKS_REC *pw IPTR;

if( handle > MAX_WKS || handle <= 0
      || !isopen( pw = &active_wk_tbl[handle-1] ) ) {
      /*** this handle is invalid, or has no open workstation ***/
      pw = NULL;
   } /* if */

   assert( (pw==NULL) ||
                ((pw>=active_wk_tbl) && (pw<=active_wk_tbl+MAX_WKS-1)
                  && isopen(pw) ) );
   return pw;
...
register int16  t;
   ...
   for( ; n>0; n-- ) {
      assert( abs(*p)>=pw->dev_width || (int32)(*p)*32768L/pw->dev_width == scale_32k(*p, w2) );
      *p++ = scale_32k(*p, w2);         /* the x coordinate */
      t = *p;
      if( yscale == YCOORDS ) t = h1-t;
#if 0
      if( (int32)t*32768L/pw->dev_height != scale_32k(t, h2) ) {
         sprintf( assert_buf, "t is %d, h is %d, scale is %d, func code is %d, vertex nr %d\r\n",
                              (int)t, (int)pw->dev_height, (int)scale_32k(t, h2),
                         vdi_param_adr->contrl_adr->opcode, vdi_param_adr->contrl_adr->nptsout-n);
         Cconws( assert_buf );
         TRAP;
      }
#endif
      assert( abs(*p)>=pw->dev_height || (int32)t*32768L/pw->dev_height == scale_32k(t, h2) );
...
{
register const DEVICE_RECORD *pd = setup_inf.device_table;
register const int dev_id = vdi_param_adr->intin_adr[0];	/* device id */
...
int16                    yscale;
   ...
   n = setup_inf.nr_devices;
   assert( n > 0 );
   while( pd->id != dev_id ) {
      pd++; n--;
      if( n<=0 ) {	/* error: device id not found */
         DEBUGMSG( "\ropnwk: device id not found" );
         goto error;	   /* <<< set intout length ?? */
      } /* if */
   } /* while */

   if( pd->load_type != DYNAMIC ) {
      /** already loaded - find entry in load table **/
      pw = active_wk_tbl;
      handle = 1;
      while( pw->pdev != pd ) {
         pw++;
      ...
      while( !isempty(pw) ) {
         ...
         if( n <= 0 ) {      /* error: empty space not found */
            DEBUGMSG( "\ropnwk: too many handles" );
            goto error;
         } /* if */
      } /* while */
      ...
      r = load_driver( pd->driver_name, pw );
      if( r != OK ) {	/* error: couldn't load driver */
         vdi_param_adr->intout_adr[0] = r;       /* <<< */
         vdi_param_adr->contrl_adr->nintout = 1;
         DEBUGMSG( "\ropnwk: couldn't load driver" );
         goto error;
      } /* if */
      pw->pdev = pd;
   } /* if */

   pw->rc_flag = (vdi_param_adr->intin_adr)[10];
   assert( pw->wks_fonts.font_chain == NULL );
   pw->wks_fonts.font_chain = NULL; /* not needed, but best to be safe */

   yscale = open_driver( vdi_param_adr, pw );

   if( (vdi_param_adr->contrl_adr->nptsout == 0)
    && (vdi_param_adr->contrl_adr->nintout == 0) ) {
      /* driver returned failure */
      if( pw->pdev->load_type == DYNAMIC ) {
         (void)free( pw->base_pagep );   /* assume successful */
         pw->pdev = NULL;      /* free slot */
      } /* if */
      DEBUGMSG( "\ropnwk: driver failed to initialise" );
     goto error;
   } /* if */

   /** opened successfully, so set up info **/
   pw->root_handle = handle;
   pw->dev_width  = (vdi_param_adr->intout_adr)[0]+1;
   pw->dev_height = (vdi_param_adr->intout_adr)[1]+1;
   pw->dpix = get_dpi( vdi_param_adr->intout_adr[3] );
   pw->dpiy = get_dpi( vdi_param_adr->intout_adr[4] );
   pw->bez_qual = 7; /* default */
...
register       int             n;
register       ACTIVE_WKS_REC *pw IPTR;
register       int             handle = vdi_param_adr->contrl_adr->handle;    /* wkstn handle */
int16 yscale;
const ACTIVE_WKS_REC *root_wkstn;
   ...
   register int n = MAX_WKS;
      ...
      /* assume the root is the first open screen */
      while( (n>0)	/* there are entries in table */
         && (isempty(pw) || (pw->pdev->id > 10)   /* !screen */
             || !isopen(pw) ) ) {           /* !opened */
         pw++; n--;
      } /* while */
      ...
      root_wkstn = pw;
      assert(n<=0 || pw->root_handle == 1+MAX_WKS-n);  /* root's root_handle should be itself */
   }
   else { /* handle != 0 */
        root_wkstn = find_wkstnp( handle );
   } /* if */

   if( root_wkstn == NULL /* || root_wkstn->pdev->id != vdi_param_adr->intin_adr[0] */ ) {
      DEBUGMSG( "\ropnvwk: root device id not found" );
      goto error;
   } /* if */

   /*vdi_param_adr->intin_adr[0] = root_wkstn->pdev->id; *//** id of root wkstn **/
   ...
   while( !isempty(pw) ) {
      pw++; n--;
      if( n<=0 ) {	/* error: empty space not found */
         DEBUGMSG( "\ropnvwk: too many handles" );
         goto error;
      } /* if */
   } /* while */
   handle = MAX_WKS + 1 - n;
   assert(handle > 0);
   assert(handle <= MAX_WKS);
   assert(isempty(active_wk_tbl + handle - 1));
   assert(handle == 1 || !isempty(active_wk_tbl + handle - 2));

   *pw = *root_wkstn;	/* copy root's wkstn data */
   assert( &active_wk_tbl[pw->root_handle-1] == root_wkstn );
   pw->rc_flag = (vdi_param_adr->intin_adr)[10];
   pw->wks_fonts.font_chain = NULL;
   vdi_param_adr->contrl_adr->handle = root_wkstn->drv_handle;
   ...
   if( (vdi_param_adr->contrl_adr->nptsout == 0)
      ...
      /* driver returned failure */
      pw->pdev = NULL;	   /* free slot */
      pw->root_handle = 0; /* workstn now closed */  /* redundant?*/
      DEBUGMSG( "\ropnvwk: device did not open" );
      assert( !isopen(pw) );
      assert( isempty(pw) );
      goto error;
   } /* if */
...
{
register ACTIVE_WKS_REC *const pw = find_wkstnp( vdi_param_adr->contrl_adr->handle );
   ...
   int16 sav_op = vdi_param_adr->contrl_adr->opcode;
      ...
      call_driver( vdi_param_adr, pw);	/* this can corrupt opcode !! */
      free_wks_fonts(pw);

      if( sav_op == OP_CLSWK ) {
         assert( &active_wk_tbl[pw->root_handle-1] == pw );
         if( pw->pdev->load_type == DYNAMIC ) {
            (void)free( pw->base_pagep );   /* assume successful */
            pw->pdev = NULL;	/* free slot */
         } /* if */
         ...
         assert( &active_wk_tbl[pw->root_handle-1] != pw );
         assert( active_wk_tbl[pw->root_handle-1].root_handle == pw->root_handle );
         pw->pdev = NULL;	/* free slot */
         assert(isempty(pw));
      } /* if */
      pw->root_handle = 0;	/* workstn now closed */
      assert( find_wkstnp( vdi_param_adr->contrl_adr->handle ) == NULL );
      assert(!isopen(pw));
      assert(pw->pdev->load_type != DYNAMIC || isempty(pw) );
   }
   else {
        /* bad handle */
   } /* if */
...
{
register const int dev_id = vdi_param_adr->intin_adr[0];
register int  n;
register ACTIVE_WKS_REC *pw = active_wk_tbl;
...
int16 nr_intout = 0;
   ...
   n = MAX_WKS;
   assert( n>0 );
   while( isempty(pw) || pw->pdev->id != dev_id ) {
      pw++; n--;
      if( n<=0 ) {	/* dev_id not found */
         goto done;
      } /* if */
   } /* while */

   /** copy the driver filename to intout[] **/
   assert( pw->pdev->id == dev_id );
   open_flag = isopen(pw);
   s=pw->pdev->driver_name;
   p=vdi_param_adr->intout_adr;
   while( nr_intout<sizeof(pw->pdev->driver_name) /*max file len*/
      && (*p++ = *s++) != 0 ) nr_intout++;

   /** copy device name to pts out **/
   /** scan back from end of data area looking for '_FSM_HDR' **/

   if( pw->base_pagep != NULL ) {
      p = (int16*)((uint32)pw->base_pagep->p_bbase - 76);	/* end of data - strlen - offset */
      assert( ((int16)p&1) == 0 );	/* work on word boundaries */
      n = pw->base_pagep->p_dlen;
...
* set entry point & basepage in *pw
...
*/
static tTTF_ERROR
load_driver( const FILE_NAME fname, ACTIVE_WKS_REC *const pw )
{
...
struct basep *const gdos_bp = &((struct basep*)start)[-1];
...
#if 1
   if( (fh == -33) /* && (pw->load_type == RESIDENT) */ ) {
   ...
   pw->base_pagep = driver_bp;
   pw->entry_point = (int16(*)(void))driver_bp->p_tbase;
...
error0:
   ...
   int sprintf(char *, const char *, ...);
        char msg[50];
        sprintf(msg, "\r\nfailed to load driver %s, error code is %d\r\n", fname, error_code);
        Cconws(msg);
   }
#endif
...
int i;
   ...
   if( (vdi_param_adr->contrl_adr->opcode == OP_OPNWK)
    ...
    || (active_wk_tbl[vdi_param_adr->contrl_adr->handle-1].pd->id == LOG_DEVICE) ) {
