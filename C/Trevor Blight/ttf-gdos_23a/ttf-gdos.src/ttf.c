...
*
*  $Id: ttf.c 3277 2021-08-29 16:45:08Z trevor $
*
*
...
{
     if( get_table( fh, tableDirp, (void *)&pg->maxp ) != OK ) return FILE_ERR;
...
register tTTF_GDATA *const pg = &gdata[ttf_nr];
   ...
   if( pg->ptbl == NULL ) {
      /** no tables for this ttf, so make them in cache **/
        find_mem( sizeof(tTTF_TBL), &(void *)pg->ptbl );
...
register tTTF_GDATA *const pg = &gdata[ttf_nr];
   ...
   if( get_table( fh, &pg->glyf_dir, (void *)glyfTablep ) == OK )
