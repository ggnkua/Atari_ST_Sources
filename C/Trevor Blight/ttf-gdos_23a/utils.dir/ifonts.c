/****************************************************************
*
* $Id: ifonts.c 3279 2021-09-03 20:13:24Z trevor $
*  
...
register int i;
   ...
   if (Fsfirst( file_name, 0) == 0) {
      do {
         ...
         printf( "%s: \"%.32s\" at %d pts, id is %d, %s format\n",
                 dtap->dta_name, hdr.name, hdr.size, hdr.font_id, is_mot? "motorola": "intel" );
