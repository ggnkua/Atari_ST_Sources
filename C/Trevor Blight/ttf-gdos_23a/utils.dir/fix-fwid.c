...
*
*  $Id: fix-fwid.c 3274 2021-08-28 13:12:54Z trevor $
*
...
static struct {			/* save/restore old ttf-gdos info */
   uint8 nr_gemfonts;
   uint8 first_font;
   DEVICE_RECORD *pd;	/* pointer to device record to be altered */
} save[30];
...
int  x,y,w,h;
   ...
   save[dev_nr].pd->first_font = gem_font_count;
   ...
   */
   ...
   save[dev_nr].pd->nr_gemfonts = gem_font_count - save[dev_nr].pd->first_font;
...
{
register uint16 n = pInfo->nr_devices;
register DEVICE_RECORD *pd = pInfo->device_table;
   for( ; n>0; n--, pd++ ) {
      save[pd->id].nr_gemfonts = pd->nr_gemfonts;
      save[pd->id].first_font = pd->first_font;
      save[pd->id].pd = pd;
      /* printf( "wkstn %d, %s, 1st font is %d, nr is %d\n", pd->id, &pd->driver_name, pd->first_font, pd->nr_gemfonts ); */
   } /* for */
} /* save_inf() */
...
int  x,y,w,h;
   ...
   for( ; n>0; n--, pd++ ) {
      /* printf( "wkstn %d, %s, 1st font is %d, nr is %d\n", pd->id, &pd->driver_name, pd->nr_gemfonts, pd->first_font ); */
      pd->nr_gemfonts = save[pd->id].nr_gemfonts;
      pd->first_font = save[pd->id].first_font;
   } /* for */
