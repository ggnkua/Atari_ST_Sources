...
LDGLIB LibLdg[] = { { 0x0002,	29, LibFunc,	"SSL/TLS functions from PolarSSL", 1} };
...
int CDECL ldg_ssl_init(ssl_context *ssl, ctr_drbg_context *ctr, int *server_fd, const char *servername, x509_crt *cacert, x509_crt *cert, pk_context *pk)
{
  ...
  if ((ret = ssl_init(ssl)) == 0)
  {
    ...
    if (strstr(servername, ".gmx.") != NULL)
    {
#if defined(POLARSSL_DEBUG_C)
      (void)Cconws("Ciphesuite for gmx servers\n\r");
#endif
      ssl_set_ciphersuites(ssl, gmx_ciphersuites);
    }
