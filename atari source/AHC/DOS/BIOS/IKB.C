/*  ikb.c - interrupt keyboard drivers.  				*/


/*
**  Unfortunately, as of this writing, there is no actual manual available
**	for the keyboard hardware for the vme/10.  But the code here is
**	fairly accurate, and most of the complicated stuff has to do with
**	the vme/10 control registers.  So a good reference manual is the
**	"VME/10 Microcomputer System Reference Manual", M68KVSREF/D1.
*/
...
#define	MAXUNITS	1
	...
	SAVE THIS STUFF FOR A TIME WHEN GEMDOS HAS A DRIVER HEADER
...
/*
**  KBVECNUM - interrupt vector number for keyboard interrupts
*/


#define KBVECNUM	0x42

/*
**
**	KEYBOARD -- ROUTINES FOR HANDLING THE VME/10 KEYBOARD.
**
**	ENVIRONMENT:  This is part of the VME/10 console driver for 
**	  GEMDOS
**
...
/****************************************************************************
**
**  modes - value of modes reflect the status (up | down) of mode keys on
*/
...
/****************************************************************************
** vmec1_status - static pointer to vme status register.
*/
...
/****************************************************************************
** vmecr - static pointer to VME/10 control register block.
*/
...
/****************************************************************************
**  shift_keys - Bits in this byte reflect status (up * or down) 
**	of two SHIFT keys. 
*/
...
/****************************************************************************
**  sc_tbl - entry in the scan code table	     
*/
...

/****************************************************************************
**  epci_map - Declarations for the 2661 (EPCI) connecting us to the keyboard
*/
...
#define MODE1_INIT	0x05e		/* Initialization for mode reg 1     */
#define MODE2_INIT	0x07b		/* Initialization for mode reg 2     */
#define COMM_INIT	0x015		/* Initialization for command reg    */
 
	/* Commands for keyboard.  In each, bits 0-3 = $0 are keyboard's ID. */
 
#define SELECT_KB	0x080		/* SELECT command.		     */
#define READ_KB		0x0a0		/* READ   command.		     */
#define AGAIN_KB	0x0d0		/* AGAIN  command; this causes last
                                           scan code sent to be repeated.    */
 
#define KB_RESET	0x0010		/* when low, holds keyboard in reset.*/
#define KB_ENABLE	0x0080		/* when high, enables receipt 
...
**
**	(this may or may not be true for GEMDOS - ktb)
*/
...
/*
** RTN - The rtn macro is used to make entries in the scan code table for those
...
/*
**  scan_code_tbl - the table itself
*/

static sc_tbl	scan_code_tbl[] = {	
				/* code   key			*/
				/* ----   ---			*/
...
#else
	RTN(IGNORE,0)		/* 6C -- Tab key on cursor pad.	*/
#endif
...
static BYTE	tbl___[] = {	/* ---------			     */
	...
	_PREV			/* 9 -- BACKTAB on cursor pad - prev window */
#else
	_DOWN			/* 6 -- the downarrow key.	     */
#endif
};
...
static char scntbl0 [] = {	/* IBM scancodes for group 0 items */
	0x1c,			/* <cr> */
	0x47,			/* home */
	0x0f,			/* backtab */
	0x0f			/* tab */
...
static char scntbl1 [] = {	/* IBM scancodes for group 1 items */
	0x29,			/* ~ */
	0x02,			/* 1 , note that 2 is hardcoded in so skip */
	0x04,			/* 3 */
	0x05,			/* 4 */
	0x06,			/* 5 */
	0x07,			/* 6 */
...
static char scntbl2 [] = {	/* IBM scancodes for group 2 items */
	0x0b,			/* 0 */
	0x02,			/* 1 */
	0x03,			/* 2 */
	0x04,			/* 3 */
...
static char alttbl [] =  {	/* ALT table for Group 1 elem for IBM */
	0,			/* ~ */ 
	0x78,			/* 1 , note that 2 is hardcoded in so skip */ 
	0x7a,			/* 3 */ 
	0x7b,			/* 4 */
	0x7c,			/* 5 */
	0x7d,			/* 6 */
...
static char	*scntbl_ptr [3] = {	/* ptr to IBM scan codes for group cds*/
	scntbl0,			/* group 0 codes */
	scntbl1,			/* group 1 codes */
	scntbl2				/* group 2 codes */
};
...
static char ascan [] = {	/* this is the IBM scan codes for Alpha char */
	0x1e,			/* A	*/
	0x30,			/* B	*/
	0x2e,			/* C	*/
	0x20,			/* D	*/
...
ERROR	kbinit()
{
	REG WORD	 i ;		/*  loop variable		*/
	BYTE		ch ;		/*  dummy char for ack		*/
	BYTE		cr ;		/*  dummy contents of ctrl reg	*/

	extern VOID	kbdint() ;	/*  asm front end in ikba.ps	*/


	vmecr = (VMECRP) VME10CR ;
	epci = (epci_map *) EPCI_ADDR ;
	vmec1_status = VME10ST ;
	...
	*/
	...
	kbqinit() ;			/* init kbq  			*/
	SETVEC(KBVECNUM,kbdint) ;	/*  set up interrupt vector	*/

	/*
	**  disable keyboard interrupts at the ctrl register, reset kbd.
	*/

	vmecr->cr2 &= (~KB_ENABLE);	/* disable keybd interrupts 	*/
	vmecr->cr2 &= (~KB_RESET);	/* clear reset bit		*/
	kbdelay(0x0000ffffL) ;		/* wait				*/

	vmecr->cr2 |= KB_RESET;		/*  set reset bit		*/
	kbdelay(0x0000ffffL) ;		/* wait				*/
	...
	*/
	...
	epci->data = SELECT_KB ;	/*  select the keyboard		*/
	if( !char_avail() )		/*  if keyboard select fails...	*/
		kpanic("kbinit: keyboard does not select!") ;
	...
	*/
	...
	if( epci->data != 0 )
		kpanic("kbinit: keyboard does not ack select!") ;

	/*
	**  read initial keyboard status bytes and init kb
	*/
	...
	for( i = 13 ; i-- ; )
	{
		epci->data = READ_KB ;
		if( ! char_avail() )
			kpanic("kbinit: keyboard does not input status!") ;
		ch = epci->data ;
	}
...
/****************************************************************************
**  char_avail - wait for the char avail flag in 2661 status register.  returns
**	false if time out, true if char avail.
*/

BOOLEAN	char_avail()
{
	/*  wait for keyboard to be selected  */
...
KBCHAR	kbdecode( kbqe , status ) 
	KBQENTRY	kbqe ;
	UWORD		*status ;
{
	REG char	scn_tbl_arg;	/* save the scan table argument field */
	REG int		group_x;	/* group/routine index from scan table*/
	REG rtn_index	rtn_x;		/* index in switch statement	      */
	REG KBCHAR	rv;		/* return value--char val f typed key */
	UBYTE	scn_code;
	UBYTE	kbstatus ;


	kbstatus = (kbqe>>8) & 0x00ff ;	/*  get status word part of ent	*/
	scn_code = kbqe & 0x00ff ;	/*  get scan code part of entry	*/

	/* save arg from the scan table entry for later use     */
	scn_tbl_arg = scan_code_tbl[scn_code].arg;

	/* 
	**  Use scan code to get group/routine index and 
	**  parameter. 
	*/
	...
	*status = NOCHAR;
	...
	if (group_x < 0)		
	{	/* 
		**  negative group_x indicates a switch 
		**  routine is invoked		     
		*/
		...
		{	/* routine aff'd by lock 	*/
			/* if lock on ignore key 	*/
			if (!(kbstatus & KBD_UNLOCK))
				rtn_x = IGNORE;	
		}

		/*
		**  most of the time, rv is set so that the 
		**  high word is DOS scan code, low word is
		**  'char value'
		*/
		...
		case TWO_AT :
			...
			** This code is entered when the 2 @ key is 
			** pressed.  Its purpose is to generate the 
			** appropriate character code depending on the 
			** state of the SHIFT and CTRL mode keys.  The 
			** reason this key can't be handled by the 
			** tables is that if CTRL is pressed, a NUL
			** ($00) character must be generated.  A value 
			** of 0 in the tables is reserved to mean NO 
			** CHARACTER.			     
			*/
		...
		case ESC_RESET :
			...
			/*  ESC_RESET -- HANDLE ESC RESET KEY.
   					**
			**  This code is entered when the ESC 
			**  RESET key is pressed.  Its purpose 
			**  is to generate the appropriate char
			**  code depending on the state of the 
			**  SHIFT mode keys.		     
			*/
		...
		case BREAK_CLEAR :
			...
			/* BREAK_CLEAR -- HANDLE BREAK CLEAR KEY
   					**
			**  This code is entered when the BREAK
			**  CLEAR key is pressed.  Its purpose 
			**  is to generate the appropriate char
			**  code depending on the state of the 
			**  SHIFT mode keys. 	
			*/
		...
		case GEN_ALPHA :
			...
			/* GEN_ALPHA -- HANDLE ALPHABETIC KEYS.
   			**
			**  This code is entered when an alpha 
			**  key is pressed.  Its purpose is to 
			**  generate the appropriate char code
			**  depending on the state of the SHIFT,
			**  ALPHA LOCK, and CTRL mode keys.
			*/

			/* scan code */
			rv |= ascan[scn_tbl_arg - 'A'];	
		...
		case FNKEY :
			...
			/* FNKEY -- HANDLE FUNCTION KEYS.
   					**
			**  This code is entered when a function
			**  key is pressed.  Its purpose is to 
			**  generate the appropriate char code
			**  depending on the state of the SHIFT
			**  mode keys.	
		...
		case SET_SHIFT :
			...
			/* SET_SHIFT AND CLR_SHIFT -- SET OR 
			**  CLEAR 1 OF THE 2 SHIFT KEYS.
			** 
			**  Use these routines when a shift key
			**  is pressed or released to update the
			**  SHIFT_KEYS byte and make a mode 
			**  change for SHIFT if necessary.
			*/
		...
		case CLR_SHIFT :
			...
			if (shift_keys == 0)		
			{	/* both shift keys released   */
				/* clear the shift mode	      */
				modes &= (~SHIFT);
				mode_chg();
			}
			break;
		...
		case SET_MODE :
			...
			/* SET_MODE AND CLR_MODE -- SET OR 
			**  CLEAR A MODE (E.G., SHIFT).
   			**
			**  Use these routines when a mode key 
			**  is pressed or released to update the
			**  MODES byte and set the appropriate 
			**  table offsets in the GRP_TBL.
			*/
	...
	else	/* else it is a positive scan code */

	{	/* CHARACTER DESIGNATED BY SCAN CODE */

		if (kbstatus & KBD_UNLOCK) /* is kbd locked? */
		{
			/* 
			**  It's not a routine, it's a group, so
			**  use the parameter to get the char 
			**  from the table currently in effect 
			**  for that group.
			*/

			if(!((modes & ALT) && (group_x == 1)))
			{
				/* get scan cd */
				rv |= (scntbl_ptr[group_x])[scn_tbl_arg];  
				rv <<= 16;
				rv += (grp_tbl_ptr[group_x])[scn_tbl_arg];
			}
			else /* is ALT group 1 */
			{
				rv |= alttbl[scn_tbl_arg];
				rv <<= 16;
				rv &= 0x00ff0000;
			}
...
ISR	kbint()
{
	...
	if (!(epci->status & ANY_ERROR))/* NO errors from the keyboard */
	{
		...
		{	/* [1] */
			kbqadd((KBQENTRY)((*VME10ST<<8) | (scn_code&0x00ff))) ;
			epci->data = READ_KB;	/* send another rd to kbd */
...
/*
** [1]	This paragraph is where we should check for magic scan codes which
**	get the systems attention.
*/
...
#if	CDOS
/*
**  this ASR stuff is strictly CDOS/68.  But GEMDOS might end up with 
**	some sort of forking code, which will allow us to decode the
**	scan_code in the background, instead of when the user calls.
**	so I left this here for posterity.
*/
/************************************************************************/
/*									*/
/*	kbasr()								*/
