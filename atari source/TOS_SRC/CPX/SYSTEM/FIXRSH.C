         ...
         long *rs_frimg, long *rs_trindex, struct foobar *rs_imdope )
{
    ...
    for( i = 0; i < num_obs; i++ ) {
	index = ( long )rs_object[i].ob_spec;
	...
	case G_FBOXTEXT:
	    ...
	    /* fix ob_spec */
	    rs_object[i].ob_spec = &rs_tedinfo[index];
	    break;
	/* ob_spec -> ICONBLK */
	case G_ICON:
	    ...
	    rs_object[i].ob_spec = &rs_iconblk[index];
	    break;
	/* ob_spec -> BITBLK */
	case G_IMAGE:
	    ...
	    rs_object[i].ob_spec = &rs_bitblk[index];
	    break;
	/* ob_spec -> string */
	case G_BUTTON:
	case G_STRING:
	case G_TITLE:
	    rs_object[i].ob_spec = rs_strings[index];
	    break;
