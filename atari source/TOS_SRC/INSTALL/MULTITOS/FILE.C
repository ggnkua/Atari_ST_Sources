...
void
Modify_gmcnf( char *Source, char *Destination )
{
	int fd, value, index, oindex;
	long fsize, time, out;
	struct FILEINFO *oldinfo, newinfo;
	char *buff, *obuff;
	...
	if( !Fsfirst("C:\\GEM.CNF",0) )
		Rename( "C:\\GEM.CNF", "C:\\GEM.OLD" );
	
redo_find:
	if( Fsfirst( Source, 0 ) )
	{
		value = CheckError( NOT_FOUND_ERROR );
		if( value == 1 ) goto redo_find;	/* Skip */

		Fsetdta( oldinfo );
		if( value == 3 ) Gem_Exit( -1 );	/* Quit */
		return;
	} else {
redo_mem:
		buff = obuff = 0L;
		/* Allocate buffer for source, then dest */
		buff = (char *)calloc( 1L, newinfo.size * 2L ); 
		obuff = (char *)calloc( 1L, newinfo.size * 2L );
...
redo_read:
			...
			{
				value = CheckError( OPEN_ERROR );
				if( value == 1 ) goto redo_read;
				free( buff );
				free( obuff );
				buff = obuff = 0L;
			
				Fsetdta( oldinfo );
				if( value == 3 ) Gem_Exit( -1 );
				return;
			}
			...
			do {
				...
				if( strstr( ScratchString, ";PATH" ) )
				{
					sprintf( ScratchString, "setenv PATH=.,%s",
					AssignPath );
					/*strupr( ScratchString );*/
				}

				if( strstr( ScratchString, ";SHSHOW" ) )
				{
					sprintf( ScratchString, "setenv SHSHOW=%s\\VIEWER.APP",
					AssignPath );
					/*strupr( ScratchString );*/
				}

				if( strstr( ScratchString, ";SHPRINT" ) )
				{
					sprintf( ScratchString, "setenv SHPRINT=%s\\LPR.APP",
					AssignPath );
					/*strupr( ScratchString );*/
				}
			
				if( strstr( ScratchString, ";TOSRUN" ) )
				{
					sprintf( ScratchString, "setenv TOSRUN=%s\\MINIWIN.APP",
					AssignPath );
					/*strupr( ScratchString );*/
				}
...
void
Modify_mtcnf( char *Source, char *Destination )
{
	int fd, value, index, oindex;
	long fsize, time, out;
	struct FILEINFO *oldinfo, newinfo;
	...
	if( !Fsfirst("C:\\MINT.CNF",0) )
		Rename( "C:\\MINT.CNF", "C:\\MINT.OLD" );
...
redo_read:
			...
			do {
				...
				if( strstr( ScratchString, "#INIT" ) )
				{
					sprintf( ScratchString, "GEM=%s\\GEM.SYS",
					AssignPath );
					strupr( ScratchString );
				}
...
{
	struct FILEINFO *oldinfo, newinfo;
	char dname[128], oname[128];

	oldinfo = Fgetdta();
	Fsetdta( &newinfo );

	/* mv auto\*.* auto\temp\*.* */
	if( Dcreate("C:\\\FULLCHAU.TMP") == 0 )
	{
		if( !Fsfirst( "C:\\AUTO\\*.*", 0 ) )
		{
			do {
				sprintf( dname, "C:\\FULLCHAU.TMP\\%s", newinfo.name );
				sprintf( oname, "C:\\AUTO\\%s", newinfo.name );
				Rename( oname,dname );		
			} while( !Fsnext() );
		}
	}
	...
	/* mv auto\temp\*.* auto\*.* */
	if( !Fsfirst( "C:\\FULLCHAU.TMP\\*.*", 0 ) );
	do {
		sprintf( dname, "C:\\AUTO\\%s", newinfo.name );
		sprintf( oname, "C:\\FULLCHAU.TMP\\%s", newinfo.name );
		Rename( oname,dname );
	} while( !Fsnext() );

	Ddelete( "C:\\FULLCHAU.TMP" );

	/* reset the old DTA structure */
	Fsetdta( oldinfo );
}
