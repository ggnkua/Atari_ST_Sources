 ...
 *	 March	16, 1993	Let Other Page Size handle Page Offsets
 *
	 ...
	 if( ( obj == button ) && ( !IsDisabled( obj ) ) )
	 {
	    switch( button )
	    {
		  ...
		  /* ZERO BASED - so we add one pixel */
		        ...
		        if( IsSelected( AREAON ) )	/* Check if PageOffsets are ON or OFF */
		        {				/* If OFF, Don't add anything */
		       	   /* ON - find offsets to add them to...*/
			    ...
			    */
		     	   Yres += mhdr->Margin[ ( index * 4 ) + 0 ];
   			   Yres += mhdr->Margin[ ( index * 4 ) + 1 ];
 
   			   Xres += mhdr->Margin[ ( index * 4 ) + 2 ];
		   	   Xres += mhdr->Margin[ ( index * 4 ) + 3 ];
     			}
		  ...
		  if( output == XYOK )
		  {
			...
			tempy = ( tempy * ydpi ) - 1;
			    ...
			    * and shove it into the TopMargin etc...
			    */
		     	   tempy -= mhdr->Margin[ ( index * 4 ) + 0 ];
   			   tempy -= mhdr->Margin[ ( index * 4 ) + 1 ];
 
   			   tempx -= mhdr->Margin[ ( index * 4 ) + 2 ];
		   	   tempx -= mhdr->Margin[ ( index * 4 ) + 3 ];
     			}

		        xres_value = hdr->xres = tempx;
		        yres_value = hdr->yres = tempy;
