...
/*		check_boxes()	- replace scale_iv() with scaler()	  */
/*		check_prim_box()- replace scale_iv() with scaler()	  */	
/*									  */
/* 11/07/89  -  World_setup() - replaced world_rc_x() with hmupix()	  */
/*									  */
/* 11/07/89  -  Replace all vmutopix() with vmupix()			  */
/*		initialize() - replace vmutopix() with vmupix()		  */
/*		do_nada()    - replace vmutopix() with vmupix()		  */
/**************************************************************************/
...
/*       get_work(prev_handle);*/		/* CJG 11/07/89 */
       wind_get(prev_handle,WF_WORKXYWH,&pwork.g_x,&pwork.g_y,&pwork.g_w,&pwork.g_h);
       prev_size = view_size;
       size_fit_handle();
		...
		while(opcode != -1)
	 	{
			...
			case 4:
				...
				while(i<count*2)
				{
				   ...
				   ndeltax = scaler(odeltax,new_width,
					old_width); 
				   ptsarray[i++] = new_murect[0] + ndeltax;

				   odeltay = ptsarray[i] - old_murect[1];
				   ndeltay = scaler(odeltay,new_height,
					old_height);
