...
/* HISTORY								  */
/* 									  */
/* 11/07/89  -  Replace all scale_iv() calls with scaler()		  */
/*		calc_size() -   "          "        "                     */
/*		repel_images() - "         "        "                     */
/*									  */
/* 11/07/89  -  Replace all hmutopix() calls with hmupix()		  */
/*	 	pdraw_scale()  -   "       "        "			  */
/*		calc_size_fit()-   "       "        "                     */
/*									  */
/* 11/07/89  -  Replace all vmutopix() calls with vmupix()		  */
/*		pdraw_scale() - replace vmutopix() with vmupix() 	  */
/*		do_update_text() - replace vmutopix() with vmupix()	  */
/*		do_artcleanup()  - replace vmutopix() with vmupix()	  */
/*		calc_size_fit()  - replace vmutopix() with vmupix()       */
/**************************************************************************/
...
unsigned long pgeptr;			/* equal to page_ptr in do_grids*/
...
pdraw_scale()
{
      ...
      zdevice  = SCREEN;
      pagew = hmupix(hpage_size);		/* can make this a variable */
      pageh = vmupix(vpage_size);		/* for later...		    */

      scale(&pagew,&pageh,&txoffset,&tyoffset,0);

      pxy[0] = pwork.g_x + hmupix(txoffset);
      pxy[1] = pwork.g_y + vmupix(tyoffset); 
...
int item;
{
	...
	{
		pagew = dpwork.g_w;
		pageh = scaler(dpwork.g_w,pixh,hwidth);
	}
	else
	{
		pageh = dpwork.g_h;
		pagew = scaler(dpwork.g_h,hwidth,pixh);
		pageh = dpwork.g_h = scaler(pagew,pixh,hwidth);
	}
        pxy[0] = dpwork.g_x;
        pxy[1] = dpwork.g_y;
      ...
      opcode = get_fprimitive(region_ptr,&count,&wmode);
		   ...
		   case 3:
			   ...
			   while(i<count*2)
			   {
				   ...
				   ndeltax = scaler(odeltax,newwidth,
					oldwidth); 
				   ptsarray[i++] = nrect[0] + ndeltax;

				   odeltay = ptsarray[i] - muy1;
				   ndeltay = scaler(odeltay,newheight,
					oldheight);
