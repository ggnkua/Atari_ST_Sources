...
rerescan:
	...
	atst:
			if (dev < 7)	/* there is a ACSI drive in the system */
				noacsi = 0;
	    	livedevs[dev] = 1;
			yesscan = 1;
	...
	if ((maxloop > 16) && (ttscsi))	{
		maxloop = 16;
		dev = 8;
		goto rerescan;
	} else if (((maxloop > 16) && (!ttscsi)) || ((ttscsi)&&(maxloop>8)))	{
		maxloop = 8;
		dev = 0;
		goto rerescan;
	}
...
char *bs;
{
	...
	for (i = 0; i < 32; i++)
		sendata[i] = 0;
	if ((set) || (scsidrv))	{
		ret = mdsense(dev, 4, 0, 32, sendata); 	/* use page code 4, but get */
											/* info from the mdsense header */
		if (!get3bytes(sendata+5))	{		/* no hsize retured */
    		delay();
			ret = mdsense(dev, 0, 0, 16, sendata);/* use page code 4, but get */
			if (!get3bytes(sendata+5))	{		/* no hsize retured */
				err(needinfo);
    			delay();
    			Super(ostack);
