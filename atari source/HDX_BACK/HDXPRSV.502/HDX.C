...
char spscsixst;		/* sparrow SCSI hard disk drive */
...
int ok2draw;
int ndevs;			/* # of devices in the system */
	...
	switch (msgbuff[0]) {
		...
		switch(msgbuff[3]) {
			...
			if(msgbuff[4] == DEABOUT) {
			    strcpy(abtdial[ABVERSN].ob_spec, "Version 5.01");
...

{
	...
	if (tformat)	{	/* when format, enable all devices in dialog box */
		if ((spscsixst) || (atexst))	{ /* notebook or sparrow */
			ndevs = 9;
			tpdev = 8;
		} else if (ttscsi) {
			ndevs = 16;
