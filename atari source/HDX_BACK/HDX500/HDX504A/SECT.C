 ...
 * May-25-93 jye. Added code to check if it is removabel drive, then send the
 *				  'mdsense' commend with page code 0x2e and allocation length
 *				  42 to unlock the floptical drive.
 * Jul-15-93 jye. Fixed a bug that didn't set the flag for the scsi drive in the
 * 				  MEGA STE computer.
 * Jul-23-93 jye. Fixed a bug that if some type of errors occor, the scan would
 *				  stop.
 */
...
jjj:
	delay();
	/*	Now the hardware doesn't support the slave drive, so comment it out
		...
		if ((inqret == OK)||(inqret == 7)) { /*it is not a regular hard drive*/
			if (sendata[0])  	{	/* it is not a hard drive */
				continue;
   				...
   				ostack = Super(NULL);
   				delay();
				mdsense(dev, 0x2e, 0, 42, buf);	/* unlock the floptical drive */
   				delay();
   				Super(ostack);
				if (spscsixst)	{
		  			/* it is a SCSI drive */
			...
			/*	Jul 15, 93 jye: consider the Mega STE which is also a use the 
				scsi drive under the acsi bus, in this case, we need set the
				'typedrv', if so, the bug that can't format the unit 0 will be
				killed. If the 'inqret' = OK, this means they drive is SCSI
				drive.
			} else if (dev > 7)	{
			*/
			} else	{
		  		/* it is a SCSI drive, because inqret = OK */
				/* set the relative bit, 1 is a SCSI, other is not */
				typedrv |= setmask << dev;
			}
		}
    	if ((ret = getroot(dev, buf, (SECTOR)0)) < 0) {
		    ...
		    if (ret > 0) {
				...
				/* if non-0, report error if medium changed */
						erasemsg();
						/* Jul-23-93 jye: Fixed this bug that we should conti-
						   nue scan the rest drives. So delet this 2 lines
						...
						*/
						...
						if (((inqret == OK)||(inqret == 7)) && 
							...
							err(instdrv);
   							ostack = Super(NULL);
   							delay();
							/* unlock the floptical drive */
							mdsense(dev, 0x2e, 0, 42, buf);	
   							delay();
   							Super(ostack);
							/* set the relative bit, 1 is a removable */
							typedev |= setmask << dev;
