...
char *fpnam, *id;
{
    ...
    fdefault(hinfop);
jjj:
 	if (*noinfo)	{ /* no information in wincap */
		if ((set) || (scsidrv) || (dev > 15))		{
			*modesel = 0;
			return (ret);
		}
		...
		if ((set) || (scsidrv) || (dev > 15))		
			if((!hinfop->hi_cc)||(!hinfop->hi_dhc)||(!hinfop->hi_spt))	{
				*noinfo = 1;
				*modesel = 0;
			}
	}
...

{
	...
	for (i = 0; i < 32; i++)
		sendata[i] = 0;
	if (mdsense(dev, 3, 0, 32, sendata) == OK)	{
		for (i=0; i<32; i++)	{
			if (sendata[i])
				break;
		}
		if (i == 32)	{	 no info return in the buf 'sendata' 
			goto dopg0;
		}
		if (!(*spt = getword(sendata+22)))	
			goto dopg0;
		if (mdsense(dev, 4, 0, 22, sendata) == OK)	{
			for (i=0; i<22; i++)	{
				if (sendata[i])
					break;
			}
