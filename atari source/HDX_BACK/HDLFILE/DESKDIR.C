...
redel:
	   if ( (ret = Fdelete(fixsrc)) )	
	   {
		...
		goto redel;
	  	...
	  	dormfd();
		/* delete the first folder from source */
		deleted();
...

{
	...
	/*
	if (showbox(mvtitle) == CCANCEL)		{
		goto endmv;
	}
	*/
	if (flag == DTOD)	{	/* files and directories mv */
		domvfd();
		deleted();			/* rm the first folder */
	} else {				/* a file mv */
		updatedir();
		updatefile();
		wrfile(fixsrc, fixdst);
...

{
	...
	addr = buf = (char *)Malloc((long)sizeof(s));
	strcpy(s, buf);
...

{
	...
	if (!Fsfirst(".\\*.*", 0xFF))	
	{
	  ...
	  do 	
	  {
		...
		countrec();	/* to the recursion setpath to one back */
		if (Dsetpath(".\\.."))
                  Pterm(1);
	      }
	      else 
...

{
	...
	dstlen = strlen(d) + 17; 	/* 1 for numll, 4 for \*.* and 13 for folder */
	srcbuf = 500;
	dstbuf = 500;
	while (srclen > srcbuf)	{
		srcbuf *= 2;
	...
	if (code)	{		/* do directories or files cp or mv */
		...
		if ((sdir) && (ddir))	{ 				/* dir to dir */
			if (*filestr)	{					/* folder cp */
				chkdstbuf(FILELEN);				/* check buf size */
				addfile(fixdst, filestr);		/* add the folder to dst */
				created(filestr);				/* create the 1st folder */
				strcat(bckslsh, fixdst);
			}
			return (DTOD);
...

{
	...
	unpack(dir);
	strcpy(dir, ((TEDINFO *)(cpbox[CPDIR].ob_spec))->te_ptext);
	strcpy("_", ((TEDINFO *)(cpbox[CPFILE].ob_spec))->te_ptext);
	objc_draw(cpbox, CPFILE, MAX_DEPTH, 0, 0, wdesk, hdesk);
	objc_draw(cpbox, CPDIR, MAX_DEPTH, 0, 0, wdesk, hdesk);
	/* update check the dir existing or not */
	if ((code = chkdir(dir, cpbox)) == QUIT)	{
		Pterm(1);
	} 
	if ((editdir) || (code == OK))	{
		repeat:
			if (Dcreate(fixdst))	{
				if ((ret = err(cntcrtdr)) == 2)	{	/* retry */
					goto repeat;
				} else if (ret == 3)	{	/* abort */
					Pterm(1);
				}
...

{
	...
	lbintoas(--numfiles, ((TEDINFO *)(cpbox[NUMFILE].ob_spec))->te_ptext);
...

{
	...
	lbintoas(--numdirs, ((TEDINFO *)(cpbox[NUMDIR].ob_spec))->te_ptext);
