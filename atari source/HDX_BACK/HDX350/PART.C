...

{
	...
	tmp1 = dsize/(2048*4);
	tmp2 = (siz - tmp1*4)/4;
	itoas(partnames, tmp1+tmp2, 0);
	itoas(partnames, tmp1+tmp2, 1);

	if (siz > 47)	/* when index > 2, siz should > 47 */
		index = siz/16;
	part34 = dsize/(2048*4) - index;
	part12 = part34 + 2*index;
	for (i = 0; i < 4; i++)	{
		if (!part34)		/* add the remainder into part12 */
			part12 += (siz - 2 * part12) / 2;
