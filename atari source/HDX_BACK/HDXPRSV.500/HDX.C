...
extern int atexst;		/* 1: for the flag of the AT drive exist */
extern int slvexst;		/* 1: for the flag of the AT slave drive exist */
...
extern SAVEPART savepart[]; 	/* the structure for the keeping partition */
...
main()
{
	...
	/* check hte existence of the BLiTTER */
	blitxst = chkblit();
	...
	switch (msgbuff[0]) {
		...
		switch(msgbuff[3]) {
			...
			if(msgbuff[4] == DEABOUT) {
			    strcpy(abtdial[ABVERSN].ob_spec, "Version 4.01");
...
long hsize;
{
	...
	*(seldev + 10) = fine + '0';
	...
	/* do the keeped partition. pinfo is in the ext order */
	if (docpkeep(dev, pinfo) != OK)	{
        ret = ERROR;
        goto partend;
    }
	...
	/* inside the stlaybs, when call the sortpart, the p_st will minus one  */
	/* root sector for the ext partition									*/
...
PART *part;
{
	...
	*/
	...
	for (i = 4; i < npart; i++)	{
		...
		spart(bs, part, i, &prvst);	/* set partition parameters */
...
register PART *pinfo;
{
	...
	if (ext != NO_EXT)	{
		mvkpbk(&savepart[0], pinfo);/* move the extended scheme back to normal*/
		sortpart(pinfo, SCAN_BS);	/* move the extended scheme back to normal*/
	}

    for (i = 0; i < npart; ++i, ++pinfo) {

		if (!(pinfo->p_flg & P_EXISTS)) { /* end of the structure */
		    return OK;
		}
	...
	/* Don't care the rest if the partition is kept from the */
	/* previous partition */
...
dodiship()
{
  int i, seldev[24], selected=0;
  int j, endup, index=0, unit=8, ret;
	...
	if ((!ttscsi) && (!atexst))			{/* no need to do the driver select */
		;								 /* do the ACSI driver select */
	} else if ((noacsi) && (!atexst))	{/* do the SCSI driver select */
		index = 8;
	} else if ((!ttscsi) && (noacsi))	{/* do the AT driver select */
		index = 16;
		unit = 2;
	} else { 	/* do the SCSI or ACSI or AT driver selection */
		if (!atexst)	{
