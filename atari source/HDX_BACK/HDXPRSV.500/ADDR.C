...
getalladdr()
{
	...
	|| rsrc_gaddr(0, TYPEDIAL, &typedial) == 0
...
/* get addresses of selection dialogues from resource file */
	...
	if (rsrc_gaddr(5, MAXLOGMS, &maxlogms) == 0
		|| rsrc_gaddr(5, NODEV, &nodev) == 0
		|| rsrc_gaddr(5, NEEDBOOT, &needboot) == 0
		|| rsrc_gaddr(5, NEEDINFO, &needinfo) == 0
		|| rsrc_gaddr(5, MAXPSIZE, &maxpsize) == 0
		|| rsrc_gaddr(5, DOKPMSG, &dokpmsg) == 0
		|| rsrc_gaddr(0, SCRPNL, &scrpnl) == 0
		|| rsrc_gaddr(0, KEEPAS, &keepas) == 0
