	     ...
	     int start_index )
{
    ...
    ( *xcpb->Sl_size )( tree, Mbase, Mslider, Total, MAX_SLITS, VERTICAL, 0 );
    (*xcpb->Sl_y)( tree, Mbase, Mslider, Cur_Slit,
...
mover_button( int obj )
{
   ...
   int   ox,oy;
	...
	if( obj == Mup )
	{        
	   (*xcpb->Sl_arrow)( tree, Mbase, Mslider, Mup, -1,
        ...
        if( obj == Mdown )
        {
           (*xcpb->Sl_arrow)( tree, Mbase, Mslider, Mdown, 1,
           		      max( Total - MAX_SLITS, 0 ), 0,
           		      &Cur_Slit, VERTICAL, Draw_Slit );
           return;			    	     
        }   			    	     
...
/*	   (*xcpb->MFsave)( MFSAVE, &Mbuffer );*/
	   graf_mouse( FLAT_HAND, 0L );
           (*xcpb->Sl_dragy)( tree, Mbase, Mslider,
           		      max( Total - MAX_SLITS, 0 ), 0,
           	    	      &Cur_Slit, Draw_Slit );
...
/*	   (*xcpb->MFsave)( MFRESTORE, &Mbuffer );*/
	   graf_mouse( ARROW, 0L );
        ...
        {   		 
           ...
           (*xcpb->Sl_arrow)( tree, Mbase, Mslider, -1, ox,
             	    	      max( Total - MAX_SLITS, 0 ), 0,
             	    	      &Cur_Slit, VERTICAL, Draw_Slit );
           return;           		  	    	     
        }   		  	    	     
	       ...
	       FON_PTR Slit_Array[],  void (*foo)() )
{
     ...
     /* Only for the INACTIVE FONTS SELECTIONS Box 
      * and ACTIVE Selections box.
      */
#if 1
     if( IsActiveTree( ad_inactive ) || IsActiveTree( ad_active ) )
     {
	...
	rect.g_h += 6;
	    ...
	    if( IsDisabled( tobject ) )
	    {
	       MakeExit( tobject );
	       Enable( tobject );
	       Objc_draw( tree, tobject, MAX_DEPTH, &rect );
	    }
        }
        else
        {
	...
	if( newvalue != oldvalue ) {
	    ...
	    *numvar = newvalue;
	    if( direction == HORIZONTAL )
	      (*xcpb->Sl_x)( tree, base, slider, newvalue, min, max, foo );
	    else
	      (*xcpb->Sl_y)( tree, base, slider, newvalue, min, max, foo );
		/* undraw old */
