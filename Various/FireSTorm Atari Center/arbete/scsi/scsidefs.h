 ...
 *
 * $Source: /dev/f/kleister.ii/cbhd\RCS\SCSIDEFS.H,v $
 *
 * $Revision: 1.5 $
 *
 * $Author: S_Engel $
 *
 * $Date: 1995/11/14 22:15:58 $
 *
 * $State: Exp $
 *
 *****************************************************************************
...
#define MAXBUS = 31;      /* maximal mîgliche Busnummer */
...
#define PARITIYERROR   -13L /* Transfer verursachte Parity-Fehler            */
...
typedef struct
{
  ...
  void    cdecl (*Install)    (WORD       bus,
                               tpTargetHandler Handler);
  void    cdecl (*Deinstall)  (tpTargetHandler Handler);
  BOOLEAN cdecl (*GetCmd)     (WORD       bus,
                               BYTE      *Cmd);
  BOOLEAN cdecl (*SendData)   (WORD       bus,
                               BYTE      *Buffer,
                               ULONG      Len);
  BOOLEAN cdecl (*GetData)    (WORD       bus,
                               void      *Buffer,
                               ULONG      Len);
  BOOLEAN cdecl (*SendStatus) (WORD       bus,
                               UWORD      Status);
  BOOLEAN cdecl (*SendMsg)    (WORD       bus,
                               UWORD      Msg);
  BOOLEAN cdecl (*GetMsg)     (WORD       bus,
                               UWORD     *Msg);
